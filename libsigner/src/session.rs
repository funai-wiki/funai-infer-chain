...
use libfunaidb::{
    funaidb_get_chunk_path, funaidb_get_metadata_path, funaidb_post_chunk_path, SlotMetadata,
    FunaiDBChunkAckData, FunaiDBChunkData,
};
...
/// Trait for connecting to and querying a signer Funai DB replica
pub trait SignerSession {
    ...
    fn put_chunk(&mut self, chunk: &FunaiDBChunkData) -> Result<FunaiDBChunkAckData, RPCError>;
    ...
    /// Get a single latest chunk from the FunaiDB and deserialize into `T` using the
    /// FunaiMessageCodec.
    fn get_latest<T: FunaiMessageCodec>(&mut self, slot_id: u32) -> Result<Option<T>, RPCError> {
        let Some(latest_bytes) = self.get_latest_chunk(slot_id)? else {
            return Ok(None);
        };
        Some(
            T::consensus_deserialize(&mut latest_bytes.as_slice()).map_err(|e| {
                let msg = format!("FunaiMessageCodec::consensus_deserialize failure: {e}");
...
/// signer session for a funaidb instance
pub struct FunaiDBSession {
    /// host we're talking to
    pub host: String,
    /// contract we're talking to
    pub funaidb_contract_id: QualifiedContractIdentifier,
...
impl FunaiDBSession {
    ...
    pub fn new(host: &str, funaidb_contract_id: QualifiedContractIdentifier) -> FunaiDBSession {
        FunaiDBSession {
            host: host.to_owned(),
            funaidb_contract_id,
            sock: None,
    ...
    where
        F: FnOnce(&mut FunaiDBSession, &mut TcpStream) -> R,
    {
...
impl SignerSession for FunaiDBSession {
    /// connect to the replica
    fn connect(
        &mut self,
        host: String,
        ...
        self.host = host;
        self.funaidb_contract_id = funaidb_contract_id;
        self.connect_or_reconnect()
    }
    ...
    fn list_chunks(&mut self) -> Result<Vec<SlotMetadata>, RPCError> {
        let bytes = self.rpc_request(
            "GET",
            &funaidb_get_metadata_path(self.funaidb_contract_id.clone()),
            None,
            &[],
        )?;
    ...
    fn get_chunks(
        ...
        for (slot_id, slot_version) in slots_and_versions.iter() {
            let path = funaidb_get_chunk_path(
                self.funaidb_contract_id.clone(),
                *slot_id,
                Some(*slot_version),
            );
    ...
    fn get_latest_chunks(&mut self, slot_ids: &[u32]) -> Result<Vec<Option<Vec<u8>>>, RPCError> {
        ...
        for slot_id in slot_ids.iter() {
            let path = funaidb_get_chunk_path(self.funaidb_contract_id.clone(), *slot_id, None);
            let chunk = match self.rpc_request("GET", &path, None, &[]) {
                Ok(body_bytes) => Some(body_bytes),
                Err(RPCError::HttpError(code)) => {
                    if code != 404 {
    ...
    fn put_chunk(&mut self, chunk: &FunaiDBChunkData) -> Result<FunaiDBChunkAckData, RPCError> {
        ...
        let path = funaidb_post_chunk_path(self.funaidb_contract_id.clone());
        let resp_bytes = self.rpc_request("POST", &path, Some("application/json"), &body)?;
        let ack: FunaiDBChunkAckData = serde_json::from_slice(&resp_bytes)
            .map_err(|e| RPCError::Deserialize(format!("{:?}", &e)))?;
        Ok(ack)
    }
}
