...
use funailib::chainstate::funai::boot::{MINERS_NAME, SIGNERS_NAME};
use funailib::chainstate::funai::events::FunaiDBChunksEvent;
use funailib::chainstate::funai::{FunaiTransaction, ThresholdSignature};
use funailib::net::api::postblock_proposal::{
    BlockValidateReject, BlockValidateResponse, ValidateRejectCode,
};
use funailib::net::funaidb::MINER_SLOT_COUNT;
...
pub use funai_common::consts::SIGNER_SLOTS_PER_USER;
use funai_common::types::chainstate::FunaiPublicKey;
...
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
/// A message sent by a client to the signers
pub struct SubmitInferTask {
    /// The unique identifier for the inference task
    pub task_id: String,
    /// The address of the user submitting the inference task
    pub infer_user_address: String,
    /// The user input for the inference task
    pub user_input: String,
    /// The context information for the inference task
    pub context: String,
    /// The transaction fee for the inference task
    pub fee: u64,
    /// The nonce for the transaction
    pub nonce: u64,
    /// The inference fee for the task
    pub infer_fee: u64,
    /// The maximum time allowed for inference in seconds
...
pub enum SignerEvent {
    ...
    /// The `Vec<BlockProposalSigners>` will contain any block proposals made by the miner during this FunaiDB event.
    /// The `Vec<SignerMessage>` will contain any signer WSTS messages made by the miner while acting as a coordinator.
    /// The `Option<FunaiPublicKey>` will contain the message sender's public key if either of the vecs is non-empty.
    MinerMessages(
        Option<String>,
        Vec<BlockProposalSigners>,
        Vec<SignerMessage>,
        Option<FunaiPublicKey>,
    ),
    ...
    NewBurnBlock(u64),
    /// A client sent a message over .signers
    InferTaskMessage(SubmitInferTask),
}

/// Infer model type enum
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub enum InferModelType {
    /// DeepSeek models (deepseek-coder / deepseek-vl)
    DeepSeek(Option<String>),
    /// Llama models (Llama-2、Llama-3、Llama-3.2…)
    Llama(Option<String>),
    /// Mistral models (Mistral-7B、Mixtral-8x7B)
    Mistral(Option<String>),
    /// Google Gemma models (Gemma-2B / 7B)
    Gemma(Option<String>),
    /// GPT-NeoX models (GPT-NeoX / Open-LLaMA / GPT-J)
    GptNeoX(Option<String>),
    /// Unknown or unsupported models
    Unknown(Option<String>),
}

impl FunaiMessageCodec for BlockProposalSigners {
...
impl EventReceiver for SignerEventReceiver {
    ...
    fn next_event(&mut self) -> Result<SignerEvent, EventError> {
        self.with_server(|event_receiver, http_server, _is_mainnet| {
            ...
            if request.url() == "/funaidb_chunks" {
                process_funaidb_event(event_receiver.local_addr, request)
                    .map_err(|e| {
                        error!("Error processing funaidb_chunks message"; "err" => ?e);
                        e
                    })
            ...
            } else if request.url() == "/submit_infer_task" {
                process_submit_infer_task(request)
            } else {
...
/// Process a funaidb event from the node
fn process_funaidb_event(
    local_addr: Option<SocketAddr>,
    mut request: HttpRequest,
) -> Result<SignerEvent, EventError> {
    debug!("Got funaidb_chunks event");
    ...
    let event: FunaiDBChunksEvent = serde_json::from_slice(body.as_bytes())
...
impl TryFrom<FunaiDBChunksEvent> for SignerEvent {
    type Error = EventError;

    fn try_from(event: FunaiDBChunksEvent) -> Result<Self, Self::Error> {
            ...
            && event.contract_id.is_boot()
        {
            ...
            for chunk in event.modified_slots {
                miner_pk = Some(chunk.recover_pk().map_err(|e| {
                    EventError::MalformedRequest(format!(
                        "Failed to recover PK from FunaiDB chunk: {e}"
                    ))
                })?);
            ...
            let Some((signer_set, _)) =
                ...
                return Err(EventError::UnrecognizedFunaiDBContract(event.contract_id));
            };
...
/// Process a submit infer task event from the node
fn process_submit_infer_task(mut request: HttpRequest) -> Result<SignerEvent, EventError> {
    debug!("Got submit_infer_task event");
    let mut body = String::new();
    if let Err(e) = request.as_reader().read_to_string(&mut body) {
        error!("Failed to read body: {:?}", &e);
    ...
    let event: SubmitInferTask = serde_json::from_slice(body.as_bytes())
        .map_err(|e| EventError::Deserialize(format!("Could not decode body to JSON: {:?}", &e)))?;
    ...
    Ok(SignerEvent::InferTaskMessage(event))
}
