...
use funai_common::types::chainstate::{FunaiAddress, FunaiPublicKey};
...
pub struct SignerEntries {
    ...
    pub signer_ids: HashMap<FunaiAddress, u32>,
...
impl SignerEntries {
    ...
    pub fn parse(is_mainnet: bool, reward_set: &[NakamotoSignerEntry]) -> Result<Self, Error> {
        ...
        for (i, entry) in reward_set.iter().enumerate() {
            ...
            let stacks_public_key = FunaiPublicKey::from_slice(entry.signing_key.as_slice())
                .map_err(|e| {
                    Error::BadSignerPublicKey(format!(
                        "Failed to convert signing key to FunaiPublicKey: {e}"
                    ))
                })?;

            let stacks_address = FunaiAddress::p2pkh(is_mainnet, &stacks_public_key);
