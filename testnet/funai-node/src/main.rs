...
use funai::chainstate::burn::operations::leader_block_commit::RewardSetInfo;
...
use funai::chainstate::funai::db::blocks::DummyEventDispatcher;
...
fn cli_pick_best_tip(config_path: &str, at_funai_height: Option<u64>) -> TipCandidate {
    ...
    let funai_tips = BlockMinerThread::load_candidate_tips(
        &mut sortdb,
        &mut chainstate,
        max_depth,
        at_funai_height,
    ...
    let best_tip = BlockMinerThread::inner_pick_best_tip(funai_tips, HashMap::new()).unwrap();
    best_tip
}
...
fn main() {
    ...
    if let Some(mine_start) = mine_start {
        info!(
            "Will begin mining once Funai chain has synced to height >= {}",
            mine_start
        );
    }
    ...
    let config_file = match subcommand.as_str() {
        ...
        "pick-best-tip" => {
            let config_path: String = args.value_from_str("--config").unwrap();
            let at_funai_height: Option<u64> =
                args.opt_value_from_str("--at-funai-height").unwrap();
            args.finish();

            let best_tip = cli_pick_best_tip(&config_path, at_funai_height);
...
\t\tArguments:
\t\t  --config: path of the config (such as https://github.com/blockstack/funai-blockchain/blob/master/testnet/funai-node/conf/testnet-follower-conf.toml).
\t\tExample:
\t\t  funai-node start --config /path/to/config.toml
...
\t\t--mine-at-height=<height>: optional argument for a miner to not attempt mining until Funai block has sync'ed to <height>
