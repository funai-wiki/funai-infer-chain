...
use funai::chainstate::funai::FunaiPrivateKey;
use funai::libfunaidb::{FunaiDBChunkAckData, FunaiDBChunkData};
...
fn post_funaidb_chunk(
    http_origin: &str,
    funaidb_contract_id: &QualifiedContractIdentifier,
    data: Vec<u8>,
    signer: &FunaiPrivateKey,
    slot_id: u32,
    slot_version: u32,
) -> FunaiDBChunkAckData {
    let mut chunk = FunaiDBChunkData::new(slot_id, slot_version, data);
    ...
    let path = format!(
        "{}/v2/funaidb/{}/{}/chunks",
        http_origin,
        &FunaiAddress::from(funaidb_contract_id.issuer.clone()),
        funaidb_contract_id.name
    );
    let res = client
        .post(&path)
    ...
    if res.status().is_success() {
        let ack: FunaiDBChunkAckData = res.json().unwrap();
        info!("Got funaidb ack: {:?}", &ack);
        return ack;
    } else {
        eprintln!("FunaiDB post error: {}", res.text().unwrap());
        panic!("");
    }
}

fn get_funaidb_chunk(
    http_origin: &str,
    funaidb_contract_id: &QualifiedContractIdentifier,
    slot_id: u32,
...
fn test_funaidb_load_store() {
    ...
    conf.node.funai_dbs.push(QualifiedContractIdentifier::new(
        to_addr(&privks[0]).into(),
        "hello-world".into(),
    ));
    let contract_id = conf.node.funai_dbs[0].clone();
    ...
    let tx = make_contract_publish(&privks[0], 0, 10_000, "hello-world", funaidb_contract);
    ...
    for i in 0..3 {
        ...
        let data = get_funaidb_chunk(&http_origin, &contract_id, 0, Some((i + 1) as u32));
        assert_eq!(data, chunk_str.as_bytes().to_vec());

        let data = get_funaidb_chunk(&http_origin, &contract_id, 0, None);
        assert_eq!(data, chunk_str.as_bytes().to_vec());
    }
}
...
#[test]
#[ignore]
fn test_funaidb_event_observer() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
    ...
    conf.events_observers.insert(EventObserverConfig {
        endpoint: format!("localhost:{}", test_observer::EVENT_OBSERVER_PORT),
        events_keys: vec![EventKeyType::FunaiDBChunks],
    });
    ...
    for i in 0..6 {
        ...
        let data = get_funaidb_chunk(&http_origin, &contract_id, slot_id, Some(1));
        assert_eq!(data, chunk_str.as_bytes().to_vec());

        let data = get_funaidb_chunk(&http_origin, &contract_id, slot_id, None);
        assert_eq!(data, chunk_str.as_bytes().to_vec());
    }

    // get events, verifying that they're all for the same contract (i.e. this one)
    let funaidb_events: Vec<_> = test_observer::get_funaidb_chunks()
        .into_iter()
        .map(|funaidb_event| {
            assert_eq!(funaidb_event.contract_id, contract_id);
            funaidb_event.modified_slots
        })
        .flatten()
        .collect();

    assert_eq!(funaidb_events.len(), 6);
    for (i, event) in funaidb_events.iter().enumerate() {
        // reported in order
