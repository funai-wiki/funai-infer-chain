...
use funai::burnchains::bitcoin::BitcoinNetworkType;
use funai::burnchains::db::BurnchainDB;
use funai::burnchains::{Address, Burnchain, PoxConstants, Txid};
...
use funai::chainstate::burn::ConsensusHash;
use funai::chainstate::coordinator::comm::CoordinatorChannels;
use funai::chainstate::funai::address::PoxAddress;
use funai::chainstate::funai::boot::POX_4_NAME;
...
use funai::chainstate::funai::{
    FunaiBlock, FunaiBlockHeader, FunaiMicroblock, FunaiMicroblockHeader, FunaiPrivateKey,
    FunaiPublicKey, FunaiTransaction, TransactionContractCall, TransactionPayload,
};
use funai::clarity_cli::vm_execute as execute;
use funai::core::mempool::MemPoolWalkTxTypes;
use funai::core::{
    self, FunaiEpoch, FunaiEpochId, BLOCK_LIMIT_MAINNET_20, BLOCK_LIMIT_MAINNET_205,
...
use funai::net::api::getaccount::AccountEntryResponse;
use funai::net::api::getcontractsrc::ContractSrcResponse;
use funai::net::api::getinfo::RPCPeerInfoData;
use funai::net::api::getpoxinfo::RPCPoxInfoData;
use funai::net::api::getfunais::GetFunaisResponse;
use funai::net::api::gettransaction_unconfirmed::UnconfirmedTransactionResponse;
use funai::net::api::postblock::FunaiBlockAcceptedData;
use funai::net::api::postfeerate::RPCFeeEstimateResponse;
use funai::net::api::posttransaction::PostTransactionRequestBody;
use funai::net::atlas::{
    AtlasConfig, AtlasDB, GetAttachmentResponse, GetAttachmentsInvResponse,
    MAX_ATTACHMENT_INV_PAGES_PER_REQUEST,
};
use funai::util_lib::boot::{boot_code_addr, boot_code_id};
use funai::util_lib::db::{query_row_columns, query_rows, u64_to_sql};
use funai::util_lib::signed_structured_data::pox4::{
...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, FunaiAddress, FunaiBlockId,
};
use funai_common::types::FunaiPublicKeyBuffer;
use funai_common::util::hash::{bytes_to_hex, hex_bytes, to_hex, Hash160};
use funai_common::util::secp256k1::{Secp256k1PrivateKey, Secp256k1PublicKey};
use funai_common::util::{get_epoch_time_ms, get_epoch_time_secs, sleep_ms};

use super::bitcoin_regtest::BitcoinCoreController;
use super::{
    make_contract_call, make_contract_publish, make_contract_publish_microblock_only,
    make_microblock, make_funai_transfer, make_funai_transfer_mblock_only, to_addr, ADDR_4, SK_1,
    SK_2, SK_3,
};
...
use crate::funai_common::types::PrivateKey;
...
fn inner_neon_integration_test_conf(seed: Option<Vec<u8>>) -> (Config, FunaiAddress) {
...
pub fn neon_integration_test_conf() -> (Config, FunaiAddress) {
    inner_neon_integration_test_conf(None)
}

pub fn neon_integration_test_conf_with_seed(seed: Vec<u8>) -> (Config, FunaiAddress) {
...
pub mod test_observer {
    ...
    use funai::chainstate::funai::boot::RewardSet;
    use funai::chainstate::funai::events::FunaiDBChunksEvent;
    use funai::net::api::postblock_proposal::BlockValidateResponse;
    ...
    pub static NEW_STACKERDB_CHUNKS: Mutex<Vec<FunaiDBChunksEvent>> = Mutex::new(Vec::new());
    ...
    pub static STACKER_SETS: Mutex<Vec<(FunaiBlockId, u64, RewardSet)>> = Mutex::new(Vec::new());
    ...
    async fn handle_funaidb_chunks(
        chunks: serde_json::Value,
    ) -> Result<impl warp::Reply, Infallible> {
        debug!(
            "signer_runloop: got funaidb chunks: {}",
            serde_json::to_string(&chunks).unwrap()
        );
        let event: FunaiDBChunksEvent = serde_json::from_value(chunks).unwrap();
        let mut funaidb_chunks = NEW_STACKERDB_CHUNKS.lock().unwrap();
        funaidb_chunks.push(event.clone());
    ...
    async fn handle_pox_funai_set(
        funai_set: serde_json::Value,
    ) -> Result<impl warp::Reply, Infallible> {
        let mut funai_sets = STACKER_SETS.lock().unwrap();
        let block_id = funai_set
            .as_object()
            .expect("Expected JSON object for funai set event")
            .get("block_id")
            .expect("Expected block_id field")
            .as_str()
            .expect("Expected string for block id")
            .to_string();
        let block_id = FunaiBlockId::from_hex(&block_id)
            .expect("Failed to parse block id field as FunaiBlockId hex");
        let cycle_number = funai_set
            .as_object()
            .expect("Expected JSON object for funai set event")
            .get("cycle_number")
        ...
        let funai_set = serde_json::from_value(
            funai_set
                .as_object()
                .expect("Expected JSON object for funai set event")
                .get("funai_set")
        ...
        .expect("Failed to parse funai set object");
        funai_sets.push((block_id, cycle_number, funai_set));
        Ok(warp::http::StatusCode::OK)
    }
    ...
    pub fn get_funai_sets() -> Vec<(FunaiBlockId, u64, RewardSet)> {
    ...
    pub fn get_funaidb_chunks() -> Vec<FunaiDBChunksEvent> {
    ...
    async fn serve(port: u16) {
        ...
        let new_funaidb_chunks = warp::path!("funaidb_chunks")
            .and(warp::post())
            .and(warp::body::json())
            .and_then(handle_funaidb_chunks);
        let block_proposals = warp::path!("proposal_response")
            .and(warp::post())
            .and(warp::body::json())
            .and_then(handle_proposal_response);
        let funai_sets = warp::path!("new_pox_set")
            .and(warp::post())
            .and(warp::body::json())
            .and_then(handle_pox_funai_set);
...
pub fn submit_tx(http_origin: &str, tx: &Vec<u8>) -> String {
    ...
    if res.status().is_success() {
        ...
        assert_eq!(
            res,
            FunaiTransaction::consensus_deserialize(&mut &tx[..])
                .unwrap()
                .txid()
                .to_string()
        );
...
pub fn submit_block(
    ...
    if res.status().is_success() {
        let res: FunaiBlockAcceptedData = res.json().unwrap();
        assert_eq!(
            res.funai_block_id,
            FunaiBlockId::new(
                consensus_hash,
                &FunaiBlock::consensus_deserialize(&mut &block[..])
                    .unwrap()
                    .block_hash()
            )
        );
...
pub fn submit_microblock(http_origin: &str, mblock: &Vec<u8>) -> BlockHeaderHash {
    let client = reqwest::blocking::Client::new();
    let microblock = FunaiMicroblock::consensus_deserialize(&mut &mblock[..]).unwrap();
    ...
    if res.status().is_success() {
        ...
        assert_eq!(
            res,
            FunaiMicroblock::consensus_deserialize(&mut &mblock[..])
                .unwrap()
                .block_hash()
        );
        return res;
...
pub fn get_block(http_origin: &str, block_id: &FunaiBlockId) -> Option<FunaiBlock> {
    let client = reqwest::blocking::Client::new();
    ...
    if res.status().is_success() {
        ...
        let block = FunaiBlock::consensus_deserialize(&mut &res[..]).unwrap();
        Some(block)
    } else {
        None
    }
...
pub fn get_tip_anchored_block(conf: &Config) -> (ConsensusHash, FunaiBlock) {
    let tip_info = get_chain_info(conf);

    // get the canonical chain tip
    let funai_tip = tip_info.funai_tip;
    let funai_tip_consensus_hash = tip_info.funai_tip_consensus_hash;

    let funai_id_tip =
        FunaiBlockHeader::make_index_block_hash(&funai_tip_consensus_hash, &funai_tip);

    // get the associated anchored block
    let http_origin = format!("http://{}", &conf.node.rpc_bind);
    let client = reqwest::blocking::Client::new();
    let path = format!("{}/v2/blocks/{}", &http_origin, &funai_id_tip);
    let block_bytes = client.get(&path).send().unwrap().bytes().unwrap();
    let block = FunaiBlock::consensus_deserialize(&mut block_bytes.as_ref()).unwrap();

    (funai_tip_consensus_hash, block)
}
...
fn find_microblock_privkey(
    ...
    for ix in 0..max_tries {
        ...
        let pubkh = Hash160::from_node_public_key(&FunaiPublicKey::from_private(&privk));
...
fn bitcoind_integration_test() {
    ...
    // second block will be the first mined Funai block
    next_block_and_wait(&mut btc_regtest_controller, &blocks_processed);
    ...
    #[cfg(feature = "monitoring_prom")]
    {
        ...
        let res = client
            ...
            .text()
            .unwrap();
        assert!(res.contains("funai_node_computed_miner_commitment_high 0"));
        assert!(res.contains("funai_node_computed_miner_commitment_low 20000"));
        assert!(res.contains("funai_node_computed_relative_miner_score 100"));
        assert!(res.contains("funai_node_miner_current_median_commitment_high 0"));
        assert!(res.contains("funai_node_miner_current_median_commitment_low 20000"));
        assert!(res.contains("funai_node_active_miners_total 1"));

        assert!(res.contains("funai_node_last_block_read_count 0"));
        assert!(res.contains("funai_node_last_block_read_length 0"));
        assert!(res.contains("funai_node_last_block_write_count 0"));
        assert!(res.contains("funai_node_last_block_write_length 0"));
        assert!(res.contains("funai_node_last_block_runtime 0"));
        assert!(res.contains("funai_node_last_block_transaction_count 1"));

        assert!(res.contains("funai_node_last_mined_block_read_count 0"));
        assert!(res.contains("funai_node_last_mined_block_read_length 0"));
        assert!(res.contains("funai_node_last_mined_block_write_count 0"));
        assert!(res.contains("funai_node_last_mined_block_write_length 0"));
        assert!(res.contains("funai_node_last_mined_block_runtime 0"));
        assert!(res.contains("funai_node_last_mined_block_transaction_count 1"));
    }
    test_observer::clear();
...
fn confirm_unparsed_ongoing_ops() {
    ...
    // second bitcoin block will contain the first mined Funai block, and then issue a 2nd valid commit
    next_block_and_wait(&mut btc_regtest_controller, &blocks_processed);
    ...
    // this bitcoin block will contain the valid commit created before (so, a second funai block)
    next_block_and_wait(&mut btc_regtest_controller, &blocks_processed);
    ...
    //  the node *should* see 3 funai blocks.
    next_block_and_wait_with_timeout(&mut btc_regtest_controller, &blocks_processed, 25);
...
fn most_recent_utxo_integration_test() {
    ...
    // third block will be the second mined Funai block, and mining it should *not* spend the
...
fn get_chain_tip(http_origin: &str) -> (ConsensusHash, BlockHeaderHash) {
    ...
    (
        ConsensusHash::from_hex(
            res.get("funai_tip_consensus_hash")
                .unwrap()
                .as_str()
                .unwrap(),
        )
        .unwrap(),
        BlockHeaderHash::from_hex(res.get("funai_tip").unwrap().as_str().unwrap()).unwrap(),
    )
}
...
pub fn get_funai_set(http_origin: &str, reward_cycle: u64) -> GetFunaisResponse {
    let client = reqwest::blocking::Client::new();
    let path = format!("{}/v2/funai_set/{}", http_origin, reward_cycle);
    let res = client.get(&path).send().unwrap();

    info!("Got funai_set response {:?}", &res);
    let res = res.json::<GetFunaisResponse>().unwrap();
    res
}
...
fn deep_contract() {
    ...
    let spender_sk = FunaiPrivateKey::new();
    ...
    for block in blocks {
        ...
        for tx in transactions.iter() {
            ...
            let parsed = FunaiTransaction::consensus_deserialize(&mut &tx_bytes[..]).unwrap();
...
fn lockup_integration() {
    ...
    let recipient = FunaiAddress::from_string(recipient_addr_str).unwrap();
...
fn stx_transfer_btc_integration_test() {
    ...
    let spender_sk = FunaiPrivateKey::from_hex(SK_1).unwrap();
    let spender_stx_addr: FunaiAddress = to_addr(&spender_sk);
    ...
    let spender_2_sk = FunaiPrivateKey::from_hex(SK_2).unwrap();
    let spender_2_stx_addr: FunaiAddress = to_addr(&spender_2_sk);
    ...
    let recipient_sk = FunaiPrivateKey::new();
...
fn stx_delegate_btc_integration_test() {
    ...
    let tx = make_contract_call(
        ...
        0,
        293,
        &FunaiAddress::from_string("ST000000000000000000002AMW42H").unwrap(),
        "pox-2",
        "delegate-stack-stx",
        &[
...
fn stack_stx_burn_op_test() {
    ...
    let spender_sk_1 = FunaiPrivateKey::from_hex(SK_1).unwrap();
    let spender_stx_addr_1: FunaiAddress = to_addr(&spender_sk_1);
    let spender_addr_1: PrincipalData = spender_stx_addr_1.clone().into();

    let spender_sk_2 = FunaiPrivateKey::from_hex(SK_2).unwrap();
    let spender_stx_addr_2: FunaiAddress = to_addr(&spender_sk_2);
    ...
    let signer_pk_1 = FunaiPublicKey::from_private(&signer_sk_1);
    ...
    let signer_key: FunaiPublicKeyBuffer = signer_pk_bytes.clone().as_slice().into();
...
fn vote_for_aggregate_key_burn_op_test() {
    ...
    let signer_pk = FunaiPublicKey::from_private(&signer_sk);
    ...
    let aggregate_key: FunaiPublicKeyBuffer = aggregate_pk.to_bytes_compressed().as_slice().into();
...
fn bitcoind_resubmission_test() {
    ...
    //  test will need to be updated to handle that.
    {
        ...
        let (consensus_hash, funai_block) = get_tip_anchored_block(&conf);

        let ublock_privk =
            find_microblock_privkey(&conf, &funai_block.header.microblock_pubkey_hash, 1024)
                .unwrap();

        let garbage_tx = make_funai_transfer_mblock_only(
            &spender_sk,
            0,
            100,
            &PrincipalData::from(FunaiAddress::burn_address(false)),
            1000,
        );
        let mut garbage_block = FunaiMicroblock::first_unsigned(
            &chain_tip.1,
            vec![FunaiTransaction::consensus_deserialize(&mut garbage_tx.as_slice()).unwrap()],
        );
...
/// Returns a FunaiMicroblock with the given transactions, sequence, and parent block that is
/// signed with the given private key.
fn make_signed_microblock(
    block_privk: &FunaiPrivateKey,
...
fn microblock_fork_poison_integration_test() {
    ...
    let spender_sk = FunaiPrivateKey::from_hex(SK_1).unwrap();
    let spender_addr: PrincipalData = to_addr(&spender_sk).into();
    let second_spender_sk = FunaiPrivateKey::from_hex(SK_2).unwrap();
    ...
    let recipient = FunaiAddress::from_string(ADDR_4).unwrap();
    let unconfirmed_tx_bytes =
        make_funai_transfer_mblock_only(&spender_sk, 0, 1000, &recipient.into(), 1000);
    let unconfirmed_tx =
        FunaiTransaction::consensus_deserialize(&mut &unconfirmed_tx_bytes[..]).unwrap();
    let second_unconfirmed_tx_bytes =
        make_funai_transfer_mblock_only(&second_spender_sk, 0, 1000, &recipient.into(), 1500);
    let second_unconfirmed_tx =
        FunaiTransaction::consensus_deserialize(&mut &second_unconfirmed_tx_bytes[..]).unwrap();
    ...
    let (first_microblock, second_microblock) = {
        ...
        let (consensus_hash, funai_block) = get_tip_anchored_block(&conf);
        let tip_hash =
            FunaiBlockHeader::make_index_block_hash(&consensus_hash, &funai_block.block_hash());
        let privk =
            find_microblock_privkey(&conf, &funai_block.header.microblock_pubkey_hash, 1024)
                .unwrap();
        let (mut chainstate, _) = FunaiChainState::open(
        ...
        let second_microblock =
            make_signed_microblock(&privk, vec![second_unconfirmed_tx], funai_tip, 1);
...
fn microblock_integration_test() {
    ...
    let recipient = FunaiAddress::from_string(ADDR_4).unwrap();
    let tx = make_funai_transfer_mblock_only(&spender_sk, 0, 1000, &recipient.into(), 1000);
    submit_tx(&http_origin, &tx);
    ...
    let recipient = FunaiAddress::from_string(ADDR_4).unwrap();
    let unconfirmed_tx_bytes =
        make_funai_transfer_mblock_only(&spender_sk, 1, 1000, &recipient.into(), 1000);
    let unconfirmed_tx =
        FunaiTransaction::consensus_deserialize(&mut &unconfirmed_tx_bytes[..]).unwrap();
    let second_unconfirmed_tx_bytes =
        make_funai_transfer_mblock_only(&second_spender_sk, 0, 1000, &recipient.into(), 1500);
    ...
    assert!(tip_info.funai_tip_height >= 3);
    let funai_tip = tip_info.funai_tip;
    let funai_tip_consensus_hash = tip_info.funai_tip_consensus_hash;
    let funai_id_tip =
        FunaiBlockHeader::make_index_block_hash(&funai_tip_consensus_hash, &funai_tip);
    ...
    assert_eq!(
        FunaiBlockId::from_vec(&index_block_hash_bytes),
        Some(funai_id_tip)
    );
    ...
    assert_eq!(blocks_observed.len() as u64, tip_info.funai_tip_height + 1);
    ...
    for next_nonce in 2..5 {
        ...
        let unconfirmed_tx_bytes = make_funai_transfer_mblock_only(
            &spender_sk,
            next_nonce,
            1000,
            &recipient.into(),
        ...
        if res.status().is_success() {
            ...
            assert_eq!(
                res,
                FunaiTransaction::consensus_deserialize(&mut &unconfirmed_tx_bytes[..])
                    .unwrap()
                    .txid()
                    .to_string()
            );
...
fn filter_low_fee_tx_integration_test() {
    ...
    let txs: Vec<_> = spender_sks
        ...
        .map(|(ix, spender_sk)| {
            ...
            if ix < 5 {
                // low-fee
                make_funai_transfer(&spender_sk, 0, 1000 + (ix as u64), &recipient.into(), 1000)
            } else {
                // high-fee
                make_funai_transfer(&spender_sk, 0, 2000 + (ix as u64), &recipient.into(), 1000)
            }
        })
        .collect();
    ...
    let txs: Vec<_> = spender_sks
        ...
        .map(|(ix, spender_sk)| {
            if ix % 2 == 0 {
                ...
                let parsed_tx = FunaiTransaction::consensus_deserialize(&mut &tx[..]).unwrap();
...
fn runtime_overflow_unconfirmed_microblocks_integration_test() {
    ...
    let spender_sks: Vec<_> = (0..4)
        ...
        .collect();
    let spender_addrs: Vec<PrincipalData> = spender_sks.iter().map(|x| to_addr(x).into()).collect();
    let spender_addrs_c32: Vec<FunaiAddress> =
...
fn block_replay_integration_test() {
    ...
    let recipient = FunaiAddress::from_string(ADDR_4).unwrap();
    let tx = make_funai_transfer(&spender_sk, 0, 1000, &recipient.into(), 1000);
    submit_tx(&http_origin, &tx);
...
fn block_limit_hit_integration_test() {
    ...
    let spender_sk = FunaiPrivateKey::new();
    let addr = to_addr(&spender_sk);
    let second_spender_sk = FunaiPrivateKey::new();
    let second_spender_addr: PrincipalData = to_addr(&second_spender_sk).into();
    let third_spender_sk = FunaiPrivateKey::new();
    ...
    // included in first block
    let tx_4 = make_funai_transfer(&third_spender_sk, 0, 180, &PrincipalData::from(addr), 100);
...
fn microblock_limit_hit_integration_test() {
    ...
    // included in first block
    let tx_4 = make_funai_transfer_mblock_only(
        &third_spender_sk,
        0,
        180,
...
fn pox_integration_test() {
    ...
    let spender_2_sk = FunaiPrivateKey::new();
    let spender_2_addr: PrincipalData = to_addr(&spender_2_sk).into();

    let spender_3_sk = FunaiPrivateKey::new();
    ...
    let pox_2_pubkey = Secp256k1PublicKey::from_private(&FunaiPrivateKey::new());
            ...
            if let TransactionPayload::ContractCall(contract_call) = parsed.payload {
                ...
                if contract_call.function_name.as_str() == "stack-stx" {
                    ...
                    assert_eq!(parsed.to_string(),
                               format!("(ok (tuple (lock-amount u1000000000000000) (funai {}) (unlock-burn-height u300)))",
                                       &spender_addr));
                    tested = true;
                }
            }
    ...
    eprintln!("Funai tip is now {}", tip_info.funai_tip_height);
    assert_eq!(tip_info.funai_tip_height, 36);
    ...
    eprintln!("Funai tip is now {}", tip_info.funai_tip_height);
    assert_eq!(tip_info.funai_tip_height, 51);
    ...
    eprintln!("Funai tip is now {}", tip_info.funai_tip_height);
    assert_eq!(tip_info.funai_tip_height, 66);
...
fn atlas_integration_test() {
    ...
    let user_1 = FunaiPrivateKey::new();
    ...
    let bootstrap_node_thread = thread::spawn(move || {
        ...
        if res.status().is_success() {
            ...
            assert_eq!(
                res,
                FunaiTransaction::consensus_deserialize(&mut &tx_1[..])
                    .unwrap()
                    .txid()
                    .to_string()
            );
        ...
        if res.status().is_success() {
            ...
            assert_eq!(
                res,
                FunaiTransaction::consensus_deserialize(&mut &tx_2[..])
                    .unwrap()
                    .txid()
                    .to_string()
            );
...
fn antientropy_integration_test() {
    ...
    eprintln!(
        "Follower Funai tip height is {}, wait until {} >= {} - 3",
    ...
    while tip_height < (target_height - 3) as u64 {
        ...
        eprintln!("Follower Funai tip height is {}", tip_height);
...
fn wait_for_mined(
    ...
    blocks_processed: &Arc<AtomicU64>,
    http_origin: &str,
    users: &[FunaiPrivateKey],
    account_before_nonces: &Vec<u64>,
    batch_size: usize,
    batches: usize,
    index_block_hashes: &mut Vec<FunaiBlockId>,
) {
    ...
    for _k in 0..10 {
        ...
        let ibh = FunaiBlockHeader::make_index_block_hash(&ch, &bhh);
...
fn use_latest_tip_integration_test() {
    ...
    let spender_sk = FunaiPrivateKey::from_hex(SK_1).unwrap();
    let spender_funai_addr = to_addr(&spender_sk);
    let spender_addr: PrincipalData = spender_funai_addr.into();
    ...
    // Third block will be the first mined Funai block.
    next_block_and_wait(&mut btc_regtest_controller, &blocks_processed);
    ...
    let tx_1 = FunaiTransaction::consensus_deserialize(&mut &transfer_tx[..]).unwrap();
    let tx_2 = FunaiTransaction::consensus_deserialize(&mut &publish_tx[..]).unwrap();
    let vec_tx = vec![tx_1, tx_2];
    let privk =
        find_microblock_privkey(&conf, &funai_block.header.microblock_pubkey_hash, 1024).unwrap();
    ...
    loop {
        let tip_info = client
            ...
            .unwrap();
        eprintln!("{:#?}", tip_info);
        if tip_info.unanchored_tip == Some(FunaiBlockId([0; 32])) {
            iter_count += 1;
            assert!(
                iter_count < 10,
                "Hit retry count while waiting for net module to process pushed microblock"
...
fn test_problematic_txs_are_not_stored() {
    ...
    let spender_sk_1 = FunaiPrivateKey::from_hex(SK_1).unwrap();
    let spender_sk_2 = FunaiPrivateKey::from_hex(SK_2).unwrap();
    let spender_sk_3 = FunaiPrivateKey::from_hex(SK_3).unwrap();
    let spender_funai_addr_1 = to_addr(&spender_sk_1);
    let spender_funai_addr_2 = to_addr(&spender_sk_2);
    let spender_funai_addr_3 = to_addr(&spender_sk_3);
    let spender_addr_1: PrincipalData = spender_funai_addr_1.into();
    let spender_addr_2: PrincipalData = spender_funai_addr_2.into();
    let spender_addr_3: PrincipalData = spender_funai_addr_3.into();
    ...
    let tx_edge_txid = FunaiTransaction::consensus_deserialize(&mut &tx_edge[..])
        .unwrap()
        .txid();
    ...
    let tx_exceeds_txid = FunaiTransaction::consensus_deserialize(&mut &tx_exceeds[..])
        .unwrap()
        .txid();
    ...
    let tx_high_txid = FunaiTransaction::consensus_deserialize(&mut &tx_high[..])
        .unwrap()
        .txid();
...
fn test_problematic_blocks_are_not_mined() {
    ...
    assert_eq!(
        tip_info.funai_tip_height,
        old_tip_info.funai_tip_height + 5
    );
    ...
    assert!(tip_info.funai_tip_height >= old_tip_info.funai_tip_height + 5);
    ...
    while get_epoch_time_secs() < deadline {
        ...
        if follower_tip_info.funai_tip_height == new_tip_info.funai_tip_height {
            break;
        }
        eprintln!(
            "\nFollower is at burn block {} funai block {}\n",
            follower_tip_info.burn_block_height, follower_tip_info.funai_tip_height,
        );
        sleep_ms(1000);
    }
    ...
    assert_eq!(
        follower_tip_info.funai_tip_height,
        new_tip_info.funai_tip_height
    );
...
fn test_problematic_blocks_are_not_relayed_or_stored() {
    ...
    info!(
        "tip_info.funai_tip_height = {}, old_tip_info.funai_tip_height = {}",
        tip_info.funai_tip_height, old_tip_info.funai_tip_height
    );
    assert!(tip_info.funai_tip_height > old_tip_info.funai_tip_height);
    ...
    while get_epoch_time_secs() < deadline {
        let follower_tip_info = get_chain_info(&follower_conf);
        if follower_tip_info.funai_tip_height == new_tip_info.funai_tip_height
            || follower_tip_info.funai_tip_height + 1 == bad_block_height
        {
            break;
        }
        eprintln!(
            "\nFollower is at burn block {} funai block {} (bad_block is {})\n",
    ...
    let follower_tip_info = get_chain_info(&follower_conf);
    eprintln!(
        "\nFollower is at burn block {} funai block {} (bad block is {})\n",
        follower_tip_info.burn_block_height, follower_tip_info.funai_tip_height, bad_block_height
    );

    // follower rejects the bad block
    assert_eq!(follower_tip_info.funai_tip_height, bad_block_height - 1);
...
fn test_problematic_microblocks_are_not_relayed_or_stored() {
            ...
            if let TransactionPayload::SmartContract(..) = &parsed.payload {
                if parsed.txid() == tx_high_txid {
                    bad_block_id = {
                        ...
                        Some(FunaiBlockId::from_hex(&bad_block_id_hex).unwrap())
                    };
                }
            }
        }
    ...
    while get_epoch_time_secs() < deadline {
        let follower_tip_info = get_chain_info(&follower_conf);
        if follower_tip_info.funai_tip_height == new_tip_info.funai_tip_height
            || follower_tip_info.funai_tip_height == bad_block_height
        {
            break;
        }
        eprintln!(
    ...
    assert_eq!(follower_tip_info.funai_tip_height, bad_block_height);
...
fn make_mblock_tx_chain(privk: &FunaiPrivateKey, fee_plus: u64) -> Vec<Vec<u8>> {
    let addr = to_addr(&privk);
    let mut chain = vec![];
...
fn test_competing_miners_build_on_same_chain(
    ...
    for _i in 0..num_miners {
        let seed = FunaiPrivateKey::new().to_bytes();
    ...
    for i in 1..num_miners {
        ...
        confs[i].node.set_bootstrap_nodes(
            format!(
                "{}@{}",
                &FunaiPublicKey::from_private(&node_privkey_1).to_hex(),
                p2p_bind
            ),
            chain_id,
            peer_version,
    ...
    loop {
        ...
        if let Some(tip_info) = tip_info_opt {
            eprintln!("\n\nMiner 1: {:?}\n\n", &tip_info);
            if tip_info.funai_tip_height > 0 {
                break;
            }
        } else {
            eprintln!("\n\nWaiting for miner 0...\n\n");
...
fn filter_txs_by_type() {
    ...
    for i in 0..2 {
        ...
        let parsed = FunaiTransaction::consensus_deserialize(&mut &publish[..]).unwrap();
...
fn filter_txs_by_origin() {
    ...
    conf.miner.filter_origins =
        [FunaiAddress::from_string("STA2MZWV9N67TBYVWTE0PSSKMJ2F6YXW7DX96QAM").unwrap()]
            .into_iter()
            .collect();
