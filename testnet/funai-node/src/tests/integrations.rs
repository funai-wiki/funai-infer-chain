...
use clarity::vm::types::{
    QualifiedContractIdentifier, ResponseData, FunaiAddressExtensions, TupleData,
};
use clarity::vm::{ClarityVersion, Value};
use lazy_static::lazy_static;
use reqwest;
use serde_json::json;
use funai::burnchains::Address;
use funai::chainstate::funai::db::blocks::{MemPoolRejection, MINIMUM_TX_FEE_RATE_PER_BYTE};
use funai::chainstate::funai::db::FunaiChainState;
use funai::chainstate::funai::{
    FunaiBlockHeader, FunaiPrivateKey, FunaiTransaction, TokenTransferMemo,
    TransactionContractCall, TransactionPayload,
};
use funai::clarity_vm::clarity::ClarityConnection;
use funai::core::mempool::MAXIMUM_MEMPOOL_TX_CHAINING;
use funai::core::{
    FunaiEpoch, FunaiEpochId, PEER_VERSION_EPOCH_2_0, PEER_VERSION_EPOCH_2_05,
...
use funai::net::api::getistraitimplemented::GetIsTraitImplementedResponse;
use funai_common::codec::FunaiMessageCodec;
use funai_common::types::chainstate::{FunaiAddress, FunaiBlockId, VRFSeed};
use funai_common::util::hash::{hex_bytes, to_hex, Sha256Sum};

use super::{
    make_contract_call, make_contract_publish, make_funai_transfer, to_addr, ADDR_4, SK_1, SK_2,
    SK_3,
};
use crate::config::InitialBalance;
use crate::helium::RunLoop;
use crate::tests::make_sponsored_funai_transfer_on_testnet;
          ...
          (let ((block-to-check (unwrap-panic (get-block-info? id-header-hash height)))
            ...
            (and (is-eq (print (unwrap-panic (at-block block-to-check (get-block-info? id-header-hash (- block-height u1)))))
                 ...
                 (is-eq (print (unwrap-panic (at-block block-to-check (get-block-info? header-hash (- block-height u1)))))
                        ...
                        (print (get funai-hash block-info)))
                 ...
                 (is-eq (print (unwrap-panic (at-block block-to-check (get-block-info? miner-address (- block-height u1)))))
                        ...
                        (print (get funai-miner block-info))))))

        (define-private (inner-update-info (height uint))
            (let ((value (tuple
              (funai-hash (unwrap-panic (get-block-info? header-hash height)))
              ...
              (funai-miner (unwrap-panic (get-block-info? miner-address height))))))
...
fn integration_test_get_info() {
    ...
    let spender_addr = to_addr(&FunaiPrivateKey::from_hex(SK_3).unwrap()).into();
    let principal_sk = FunaiPrivateKey::from_hex(SK_2).unwrap();
    let contract_sk = FunaiPrivateKey::from_hex(SK_1).unwrap();
    ...
    run_loop
        ...
        .on_new_tenure(|round, _burnchain_tip, chain_tip, tenure| {
            ...
            let contract_sk = FunaiPrivateKey::from_hex(SK_1).unwrap();
            let principal_sk = FunaiPrivateKey::from_hex(SK_2).unwrap();
            let spender_sk = FunaiPrivateKey::from_hex(SK_3).unwrap();
            ...
            if round == 1 {
                ...
                let old_tip = FunaiBlockId::new(&consensus_hash, &header_hash);
                use std::fs;
                use std::io::Write;
            ...
            if round >= 1 {
                let tx_xfer = make_funai_transfer(
                    &spender_sk,
                    (round - 1).into(),
                    10,
                    &FunaiAddress::from_string(ADDR_4).unwrap().into(),
                    100,
                );
                tenure
                    .mem_pool
    ...
    run_loop.callbacks.on_new_funai_chain_state(|round, _burnchain_tip, chain_tip, chain_state, burn_dbconn| {
        let contract_addr = to_addr(&FunaiPrivateKey::from_hex(SK_1).unwrap());
        ...
        match round {
            1 => {
                ...
                let blocks = FunaiChainState::list_blocks(&chain_state.db()).unwrap();
                assert!(chain_tip.metadata.funai_block_height == 2);
                ...
                for block in blocks.iter() {
                    let header = FunaiChainState::get_anchored_block_header_info(chain_state.db(), &block.0, &block.1).unwrap().unwrap();
                ...
                let mut miners = vec![];
                for block in blocks.iter() {
                    let miner = FunaiChainState::get_miner_info(chain_state.db(), &block.0, &block.1).unwrap().unwrap();
                    miners.push(miner);
                }
                ...
                eprintln!("Last block info: funai: {}, burn: {}", last_tip.1, last_tip.0);
                let last_block = FunaiChainState::load_block(&chain_state.blocks_path, &last_tip.0, &last_tip.1).unwrap().unwrap();
            ...
            4 => {
                ...
                let sender_addr = to_addr(&FunaiPrivateKey::from_hex(SK_3).unwrap());
                ...
                let contract_analysis = mem_type_check(GET_INFO_CONTRACT, ClarityVersion::Clarity2, FunaiEpochId::Epoch21).unwrap().1;
                ...
                let tx_xfer = make_funai_transfer(&spender_sk, round.into(), 200,
                                                   &FunaiAddress::from_string(ADDR_4).unwrap().into(), 123);
                ...
                assert_eq!(res, format!("{}", FunaiTransaction::consensus_deserialize(&mut &tx_xfer[..]).unwrap().txid()));
                ...
                let tx_xfer_invalid = make_funai_transfer(&spender_sk, (round + 30).into(), 200,     // bad nonce
                                                           &FunaiAddress::from_string(ADDR_4).unwrap().into(), 456);

                let tx_xfer_invalid_tx = FunaiTransaction::consensus_deserialize(&mut &tx_xfer_invalid[..]).unwrap();
                ...
                let old_tip = FunaiBlockId::consensus_deserialize(&mut &buf[..]).unwrap();
...
fn contract_stx_transfer() {
    ...
    let contract_sk = FunaiPrivateKey::from_hex(SK_1).unwrap();
    let sk_3 = FunaiPrivateKey::from_hex(SK_3).unwrap();
    let addr_3 = to_addr(&sk_3);

    conf.burnchain.commit_anchor_block_within = 5000;
    conf.add_initial_balance(addr_3.to_string(), 100000);
    conf.add_initial_balance(
        to_addr(&FunaiPrivateKey::from_hex(&SK_2).unwrap()).to_string(),
        1000,
    );
    ...
    run_loop
        ...
        .on_new_tenure(|round, _burnchain_tip, chain_tip, tenure| {
            ...
            let contract_sk = FunaiPrivateKey::from_hex(SK_1).unwrap();
            let sk_2 = FunaiPrivateKey::from_hex(SK_2).unwrap();
            let sk_3 = FunaiPrivateKey::from_hex(SK_3).unwrap();
            let header_hash = chain_tip.block.block_hash();
            let consensus_hash = chain_tip.metadata.consensus_hash;
            ...
            let contract_identifier = QualifiedContractIdentifier::parse(&format!(
                "{}.{}",
                to_addr(&FunaiPrivateKey::from_hex(SK_1).unwrap()).to_string(),
                "faucet"
            ))
            .unwrap();
            ...
            if round == 1 {
                ...
                let xfer_to_contract =
                    make_funai_transfer(&sk_3, 0, 10, &contract_identifier.into(), 1000);
                tenure
                    .mem_pool
                    .submit_raw(
                        &mut chainstate_copy,
                ...
                for i in 0..MAXIMUM_MEMPOOL_TX_CHAINING {
                    let xfer_to_contract = make_funai_transfer(
                        &sk_3,
                        1 + i,
                        200,
                    ...
                    let xfer_to_contract =
                        FunaiTransaction::consensus_deserialize(&mut &xfer_to_contract[..])
                            .unwrap();
                    tenure
                        .mem_pool
                        .submit(
                ...
                let xfer_to_contract =
                    make_funai_transfer(&sk_3, 3, 190, &contract_identifier.clone().into(), 1000);
                let xfer_to_contract =
                    FunaiTransaction::consensus_deserialize(&mut &xfer_to_contract[..]).unwrap();
                match tenure
                    .mem_pool
                    .submit(
                        &mut chainstate_copy,
    ...
    run_loop.callbacks.on_new_funai_chain_state(
        |round, _burnchain_tip, chain_tip, chain_state, burn_dbconn| {
            ...
            match round {
                    ...
                    assert_eq!(
                        chain_state
                            .with_read_only_clarity_tx(
                                burn_dbconn,
                                &FunaiBlockHeader::make_index_block_hash(&cur_tip.0, &cur_tip.1),
                                |conn| {
                ...
                2 => {
                    assert!(chain_tip.metadata.funai_block_height == 3);
                    // Block #2 should have 2 txs -- coinbase + publish
                    assert_eq!(chain_tip.block.txs.len(), 2);
                }
                3 => {
                    assert!(chain_tip.metadata.funai_block_height == 4);
                ...
                4 => {
                    assert!(chain_tip.metadata.funai_block_height == 5);
                    assert_eq!(
...
fn mine_transactions_out_of_order() {
    ...
    let sk = FunaiPrivateKey::from_hex(SK_3).unwrap();
    let addr = to_addr(&sk);
    conf.burnchain.commit_anchor_block_within = 5000;
    ...
    run_loop
        ...
        .on_new_tenure(|round, _burnchain_tip, chain_tip, tenure| {
            ...
            if round == 1 {
                // block-height = 2
                let xfer_to_contract =
                    make_funai_transfer(&sk, 1, 10, &contract_identifier.into(), 1000);
                tenure
                    .mem_pool
                    .submit_raw(
                        &mut chainstate_copy,
                ...
                let xfer_to_contract =
                    make_funai_transfer(&sk, 3, 10, &contract_identifier.into(), 1000);
                tenure
                    .mem_pool
                    .submit_raw(
                        &mut chainstate_copy,
                ...
                let xfer_to_contract =
                    make_funai_transfer(&sk, 0, 10, &contract_identifier.into(), 1000);
                tenure
                    .mem_pool
                    .submit_raw(
                        &mut chainstate_copy,
    ...
    run_loop.callbacks.on_new_funai_chain_state(
        |round, _burnchain_tip, chain_tip, chain_state, burn_dbconn| {
            ...
            match round {
                1 => {
                    assert_eq!(chain_tip.metadata.funai_block_height, 2);
                    assert_eq!(chain_tip.block.txs.len(), 1);
                }
                2 => {
                    assert_eq!(chain_tip.metadata.funai_block_height, 3);
                    assert_eq!(chain_tip.block.txs.len(), 1);
                }
                3 => {
                    assert_eq!(chain_tip.metadata.funai_block_height, 4);
                    assert_eq!(chain_tip.block.txs.len(), 1);
                }
                4 => {
                    assert_eq!(chain_tip.metadata.funai_block_height, 5);
                    assert_eq!(chain_tip.block.txs.len(), 5);
                    ...
                    assert_eq!(
                        chain_state
                            .with_read_only_clarity_tx(
                                burn_dbconn,
                                &FunaiBlockHeader::make_index_block_hash(&curr_tip.0, &curr_tip.1),
                                |conn| {
                                    conn.with_clarity_db_readonly(|db| {
                                        db.get_account_stx_balance(
                                            &contract_identifier.clone().into(),
    ...
    run_loop
        ...
        .on_new_tenure(|round, _burnchain_tip, _chain_tip, tenure| {
            ...
            if round == 1 {
                ...
                // block-height = 3
                let xfer_to_contract = make_funai_transfer(&sk_3, 1, 10, &addr_2.into(), 1000);
                let (consensus_hash, block_hash) = (
                    &tenure.parent_block.metadata.consensus_hash,
                    &tenure.parent_block.metadata.anchored_header.block_hash(),
                );
                ...
                // doesn't consistently get mined by the FunaiBlockBuilder, because order matters!
                let xfer_to_contract = make_funai_transfer(&sk_3, 2, 10, &addr_2.into(), 3000);
                tenure
                    .mem_pool
                    .submit_raw(
                        &mut chainstate_copy,
...
fn make_keys(seed: &str, count: u64) -> Vec<FunaiPrivateKey> {
    let mut seed = {
    ...
    while (ret.len() as u64) < count {
        if let Ok(sk) = FunaiPrivateKey::from_slice(seed.as_bytes()) {
            ret.push(sk);
        }
...
fn mempool_errors() {
    ...
    run_loop.callbacks.on_new_funai_chain_state(
        |round, _chain_state, _block, _chain_tip_info, _burn_dbconn| {
            ...
            let send_to = FunaiAddress::from_string(ADDR_4).unwrap().into();

            if round == 1 {
                // let's submit an invalid transaction!
                eprintln!("Test: POST {} (invalid)", path);
                let tx_xfer_invalid = make_funai_transfer(
                    &spender_sk,
                ...
                let tx_xfer_invalid_tx =
                    FunaiTransaction::consensus_deserialize(&mut &tx_xfer_invalid[..]).unwrap();
                ...
                let tx_xfer_invalid = make_sponsored_funai_transfer_on_testnet(
                    &spender_sk,
                    &contract_sk,
