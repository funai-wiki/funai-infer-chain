...
use funai::chainstate::funai::FunaiBlockHeader;
use funai::clarity_cli::vm_execute as execute;
use funai::core;
use funai::core::BURNCHAIN_TX_SEARCH_WINDOW;
...
use crate::funai_common::address::AddressHashMode;
use crate::funai_common::types::Address;
use crate::funai_common::util::hash::{bytes_to_hex, hex_bytes};
...
fn advance_to_2_1(
    ...
    for _i in 0..5 {
        ...
        .unwrap();
        let res = FunaiChainState::block_crosses_epoch_boundary(
            &chainstate.db(),
            &tip_info.funai_tip_consensus_hash,
            &tip_info.funai_tip,
        )
        .unwrap();
        debug!(
...
fn transition_adds_burn_block_height() {
    ...
    let spender_addr_c32 = FunaiAddress::from(to_addr(&spender_sk));
    ...
    // block height at which the Funai blockchain begins).
    let contract = "
    ...
    let cc_txid = FunaiTransaction::consensus_deserialize(&mut &tx[..])
        .unwrap()
        .txid();
...
fn transition_adds_pay_to_alt_recipient_contract() {
    ...
    for _i in 0..funai_common::consts::MINER_REWARD_MATURITY + 1 {
        next_block_and_wait(&mut btc_regtest_controller, &blocks_processed);
    }
...
fn transition_fixes_bitcoin_rigidity() {
    ...
    // let's fire off a transfer op that will not land in the Funai 2.1 epoch.  It should not be
    // applied, even though it's within 6 blocks of the next Funai block, which will be in epoch
    // 2.1.  This verifies that the new burnchain consideration window only applies to sortitions
    // that happen in Funai 2.1.
    ...
    // build a couple bitcoin blocks without a funai block to mine it, up to the edge of the
    // window
...
fn transition_adds_get_pox_addr_recipients() {
    ...
    let spender_addr_c32 = FunaiAddress::from(to_addr(&spender_sks[0]));
    ...
    let cc_txid = FunaiTransaction::consensus_deserialize(&mut &cc_tx[..])
        .unwrap()
        .txid();
...
fn transition_adds_mining_from_segwit() {
    ...
    assert_eq!(
        tip_info_before.funai_tip_height + 1,
        tip_info_after.funai_tip_height
    );
...
fn transition_empty_blocks() {
    ...
    for _i in 0..30 {
        ...
        if tip_info.burn_block_height + 1 >= epoch_2_1 {
            ...
            let res =
                bitcoin_controller.submit_operation(FunaiEpochId::Epoch21, op, &mut op_signer, 1);
...
pub fn wait_pox_stragglers(confs: &[Config], max_funai_tip: u64, block_time_ms: u64) {
    loop {
        let mut straggler = false;
        let mut funai_tip_ch = None;
        let mut funai_tip_bhh = None;

        for (i, c) in confs.iter().enumerate() {
            let tip_info = get_chain_info(&c);
            info!("Tip for miner {}: {:?}", i, &tip_info);

            if tip_info.funai_tip_height < max_funai_tip {
                straggler = true;
            }

            if let Some(funai_tip_ch) = funai_tip_ch.as_ref() {
                if *funai_tip_ch != tip_info.funai_tip_consensus_hash {
                    straggler = true;
                }
            } else {
                funai_tip_ch = Some(tip_info.funai_tip_consensus_hash);
            }
            if let Some(funai_tip_bhh) = funai_tip_bhh.as_ref() {
                if *funai_tip_bhh != tip_info.funai_tip {
                    straggler = true;
                }
            } else {
                funai_tip_bhh = Some(tip_info.funai_tip);
            }
        }
        if !straggler {
            break;
...
fn test_pox_reorgs_three_flaps() {
    ...
    // prevent Funai at these heights from propagating
    env::set_var(
    ...
    info!("####################### end of cycle ##############################");
    let mut max_funai_tip = 0;
    for (i, c) in confs.iter().enumerate() {
        let tip_info = get_chain_info(&c);
        info!("Tip for miner {}: {:?}", i, &tip_info);

        // miner 0's affirmation map is now the heaviest, and there's no longer a tie.
        max_funai_tip = std::cmp::max(tip_info.funai_tip_height, max_funai_tip);
    }
    info!("####################### end of cycle ##############################");
    ...
    wait_pox_stragglers(&confs, max_funai_tip, block_time_ms);
...
fn test_pox_reorg_one_flap() {
    ...
    // wait for all blocks to propagate
    eprintln!(
        "Wait for all blocks to propagate; funai tip height is {}",
        max_funai_tip
    );
    wait_pox_stragglers(&confs, max_funai_tip, block_time_ms);

    // nodes now agree on funai affirmation map
    for (i, c) in confs.iter().enumerate() {
        let tip_info = get_chain_info(&c);
        info!("Final tip for miner {}: {:?}", i, &tip_info);
    }
...
fn test_pox_reorg_flap_duel() {
    ...
    // wait for all blocks to propagate
    // NOTE: the funai affirmation maps will differ from the heaviest affirmation map, because the
...
///
/// Verify that PoX-1 funais continue to receive PoX payouts after v1 unlock height, and that
/// PoX-2 funais only begin receiving rewards at the start of the reward cycle following the one
/// that contains v1 unlock height.
fn test_v1_unlock_height_with_current_funais() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
    ...
    let tip_info = get_chain_info(&conf);
    let tip = FunaiBlockId::new(&tip_info.funai_tip_consensus_hash, &tip_info.funai_tip);
...
fn test_v1_unlock_height_with_delay_and_current_funais() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
