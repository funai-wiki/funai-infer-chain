...
use funai::burnchains::{Burnchain, PoxConstants};
use funai::chainstate::funai::address::PoxAddress;
use funai::chainstate::funai::db::FunaiChainState;
use funai::chainstate::funai::miner::{signal_mining_blocked, signal_mining_ready};
use funai::clarity_cli::vm_execute as execute;
use funai::core;
use funai::core::STACKS_EPOCH_MAX;
...
use crate::neon_node::FunaiNode;
use crate::funai_common::types::Address;
use crate::funai_common::util::hash::bytes_to_hex;
...
///
/// Verify the (buggy) funai-increase behavior in PoX-2, and then verify that Epoch-2.2
...
fn disable_pox() {
    ...
    let pox_addr_1 = PoxAddress::Standard(
        FunaiAddress::new(26, Hash160::from_hex(&pox_pubkey_hash_1).unwrap()),
        Some(AddressHashMode::SerializeP2PKH),
    );
    let pox_addr_2 = PoxAddress::Standard(
        FunaiAddress::new(26, Hash160::from_hex(&pox_pubkey_hash_2).unwrap()),
        Some(AddressHashMode::SerializeP2PKH),
    );
    let pox_addr_3 = PoxAddress::Standard(
        FunaiAddress::new(26, Hash160::from_hex(&pox_pubkey_hash_3).unwrap()),
        Some(AddressHashMode::SerializeP2PKH),
    );
    ...
    for block in blocks {
        ...
        for tx in transactions {
            ...
            let parsed =
                FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
...
fn pox_2_unlock_all() {
    ...
    //  the funai-node to mine the funai block which will be included
    ...
    // perform a transfer
    let tx = make_funai_transfer(&spender_sk, 5, tx_fee, &spender_3_addr, 1_000_000);
    ...
    let expected_slots = HashMap::from([
        ...
        //  enough time for the funais to begin in this cycle
...
fn test_pox_reorg_one_flap() {
    ...
    let node_privkey_1 =
        FunaiNode::make_node_private_key_from_seed(&confs[0].node.local_peer_seed);
