...
use funai::chainstate::funai::{
    FunaiPrivateKey, FunaiTransaction, ThresholdSignature, TransactionAnchorMode,
    TransactionAuth, TransactionPayload, TransactionPostConditionMode, TransactionSmartContract,
    TransactionVersion,
};
use funai::core::FunaiEpoch;
...
use funai_common::types::chainstate::{
    ConsensusHash, FunaiAddress, FunaiBlockId, FunaiPublicKey, TrieHash,
};
use funai_common::types::FunaiEpochId;
use funai_common::util::hash::{hex_bytes, MerkleTree, Sha512Trunc256Sum};
use funai_common::util::secp256k1::MessageSignature;
use funai_signer::client::{FunaiDB, FunaiClient};
use funai_signer::config::{build_signer_config_tomls, GlobalConfig as SignerConfig, Network};
use funai_signer::runloop::RunLoopCommand;
use funai_signer::signer::{Command as SignerCommand, SignerSlotID};
...
struct SignerTest {
    ...
    pub signer_funai_private_keys: Vec<FunaiPrivateKey>,
    // link to the funai node
    pub funai_client: FunaiClient,
...
impl SignerTest {
    fn new(num_signers: usize) -> Self {
        ...
        let signer_funai_private_keys = (0..num_signers)
        ...
        let node = setup_stx_btc_node(naka_conf, &signer_funai_private_keys, &signer_configs);
        let config = SignerConfig::load_from_str(&signer_configs[0]).unwrap();
        let funai_client = FunaiClient::from(&config);
    ...
    fn boot_to_epoch_3(&mut self, timeout: Duration) -> Point {
        boot_to_epoch_3_reward_set(
            ...
            &self.signer_funai_private_keys,
            &self.signer_funai_private_keys,
    ...
    fn run_until_epoch_3_boundary(&mut self) {
        ...
        let epoch_3 =
            &epochs[FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch30).unwrap()];
    ...
    fn get_signer_index(&self, reward_cycle: u64) -> SignerSlotID {
        ...
        let signer_funaidb_contract_id = boot_code_id(SIGNERS_NAME, false);

        self.funai_client
            .get_funaidb_signer_slots(&signer_funaidb_contract_id, valid_signer_set)
            .expect("FATAL: failed to get signer slots from funaidb")
            .iter()
            .position(|(address, _)| address == self.funai_client.get_signer_address())
            .map(|pos| {
    ...
    fn generate_invalid_transactions(&self) -> Vec<FunaiTransaction> {
        let host = self
            .running_nodes
            .conf
            .node
        ...
        let signer_private_key = self.signer_funai_private_keys[0];
        ...
        let invalid_not_contract_call = FunaiTransaction {
        ...
        let invalid_contract_address = FunaiClient::build_signed_contract_call_transaction(
            &FunaiAddress::p2pkh(false, &FunaiPublicKey::from_private(&signer_private_key)),
            contract_name.clone(),
        ...
        let invalid_contract_name = FunaiClient::build_signed_contract_call_transaction(
            &contract_addr,
        ...
        let invalid_signers_vote_function = FunaiClient::build_signed_contract_call_transaction(
            &contract_addr,
            contract_name.clone(),
        ...
        let invalid_function_arg_signer_index =
            FunaiClient::build_signed_contract_call_transaction(
                &contract_addr,
                contract_name.clone(),
                SIGNERS_VOTING_FUNCTION_NAME.into(),
                &[
        ...
        let invalid_function_arg_key = FunaiClient::build_signed_contract_call_transaction(
            &contract_addr,
            contract_name.clone(),
            SIGNERS_VOTING_FUNCTION_NAME.into(),
            &[
        ...
        let invalid_function_arg_round = FunaiClient::build_signed_contract_call_transaction(
            &contract_addr,
            contract_name.clone(),
            SIGNERS_VOTING_FUNCTION_NAME.into(),
            &[
        ...
        let invalid_nonce = FunaiClient::build_signed_contract_call_transaction(
            &contract_addr,
            contract_name.clone(),
            SIGNERS_VOTING_FUNCTION_NAME.into(),
            &valid_function_args,
        ...
        let invalid_funai_client = FunaiClient::new(
            FunaiPrivateKey::new(),
            host,
            "12345".to_string(), // That's amazing. I've got the same combination on my luggage!
            false,
        );
        let invalid_signer_tx = invalid_funai_client
    ...
    fn stop_signer(&mut self, signer_idx: usize) -> FunaiPrivateKey {
        ...
        let signer_key = self.signer_funai_private_keys.remove(signer_idx);
    ...
    fn restart_signer(&mut self, signer_idx: usize, signer_private_key: FunaiPrivateKey) {
...
fn spawn_signer(
    ...
    let runloop: funai_signer::runloop::RunLoop = funai_signer::runloop::RunLoop::from(config);
    let mut signer: Signer<
        RunLoopCommand,
        Vec<OperationResult>,
        funai_signer::runloop::RunLoop,
        SignerEventReceiver,
...
fn setup_stx_btc_node(
    ...
    signer_funai_private_keys: &[FunaiPrivateKey],
    ...
    for key in signer_funai_private_keys {
...
fn funaidb_dkg() {
    ...
    boot_to_epoch_3_reward_set(
        ...
        &signer_test.signer_funai_private_keys,
        &signer_test.signer_funai_private_keys,
...
fn funaidb_sign() {
    ...
    let header2 = NakamotoBlockHeader {
        ...
        parent_block_id: FunaiBlockId([0x06; 32]),
    ...
    let signer_message = loop {
        assert!(
            ...
            "Timed out while waiting for signers block response funai db event"
        );

        let nakamoto_blocks = test_observer::get_funaidb_chunks();
...
pub fn find_block_response(chunk_events: Vec<FunaiDBChunksEvent>) -> Option<SignerMessage> {
...
///
/// Test Setup:
/// The test spins up five funai signers, one miner Nakamoto node, and a corresponding bitcoind.
/// The funai node is advanced to epoch 2.5. forcibly triggering DKG to set the key correctly
/// The funai node is next advanced to epoch 3.0 boundary to allow block signing.
///
/// Test Execution:
/// The node attempts to mine a Nakamoto block, sending a block to the observing signers via the
/// .miners funai db instance. The signers submit the block to the funai node for verification.
...
/// Test Setup:
/// The test spins up five funai signers, one miner Nakamoto node, and a corresponding bitcoind.
/// The funai node is advanced to epoch 2.5, triggering a DKG round. The funai node is then advanced
...
fn funaidb_mine_2_nakamoto_reward_cycles() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
...
fn funaidb_filter_bad_transactions() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
    ...
    let signer_private_key = signer_test
        ...
        .find(|pk| {
            let addr = to_addr(pk);
            addr == *signer_test.funai_client.get_signer_address()
        })
        .cloned()
    ...
    debug!(
        "Signer address is {}",
        &signer_test.funai_client.get_signer_address()
    );
    ...
    // Submit transactions to funaidb for the signers and miners to pick up during block verification
    funaidb
        .send_message_with_retry(SignerMessage::Transactions(invalid_txs))
        .expect("Failed to write expected transactions to funaidb");
...
///
/// Test Setup:
/// The test spins up three funai signers, one miner Nakamoto node, and a corresponding bitcoind.
/// The funai node is advanced to epoch 2.5, triggering a DKG round. The funai node is then advanced
/// to Epoch 3.0 boundary to allow block signing.
///
/// Test Execution:
...
fn funaidb_sign_after_signer_reboot() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
