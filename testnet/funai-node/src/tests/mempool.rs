...
use funai::chainstate::funai::db::blocks::MemPoolRejection;
use funai::chainstate::funai::{
    Error as ChainstateError, FunaiBlockHeader, FunaiMicroblockHeader, FunaiPrivateKey,
...
use super::{
    make_coinbase, make_contract_call, make_contract_publish, make_poison, make_funai_transfer,
...
pub fn make_bad_funai_transfer(
    ...
    tx_signer.sign_origin(&FunaiPrivateKey::new()).unwrap();
...
fn mempool_setup_chainstate() {
    ...
    run_loop.callbacks.on_new_funai_chain_state(
        |round, _burnchain_tip, chain_tip, chain_state, _burn_dbconn| {
            ...
            let other_sk = FunaiPrivateKey::from_hex(SK_2).unwrap();
            ...
            if round == 3 {
                ...
                let tx_bytes = make_funai_transfer(&contract_sk, 5, 200, &other_addr, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                chain_state
                    .will_admit_mempool_tx(
                ...
                // bad signature
                let tx_bytes = make_bad_funai_transfer(&contract_sk, 5, 200, &other_addr, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                let bad_addr = FunaiAddress::from_public_keys(
                    88,
                    &AddressHashMode::SerializeP2PKH,
                    1,
                    &vec![FunaiPublicKey::from_private(&other_sk)],
                )
                .unwrap()
                .into();
                ...
                let tx_bytes = make_funai_transfer(&contract_sk, 5, 200, &bad_addr, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                // bad fees
                let tx_bytes = make_funai_transfer(&contract_sk, 5, 0, &other_addr, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                // bad nonce
                let tx_bytes = make_funai_transfer(&contract_sk, 0, 200, &other_addr, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                // not enough funds
                let tx_bytes = make_funai_transfer(&contract_sk, 5, 110000, &other_addr, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                let tx_bytes = make_funai_transfer(&contract_sk, 5, 300, &contract_princ, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                let tx_bytes = make_funai_transfer(&contract_sk, 5, 300, &mainnet_princ, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                let tx_bytes = make_funai_transfer(&contract_sk, 5, 300, &other_addr, 0);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                let tx_bytes = make_funai_transfer(&contract_sk, 5, 99700, &other_addr, 1000);
                let tx =
                    FunaiTransaction::consensus_deserialize(&mut tx_bytes.as_slice()).unwrap();
                let e = chain_state
                    .will_admit_mempool_tx(
                ...
                let microblock_1 = FunaiMicroblockHeader {
                    version: 0,
                    sequence: 0,
                ...
                let microblock_2 = FunaiMicroblockHeader {
                    version: 0,
                    sequence: 1,
                    prev_block: BlockHeaderHash([0; 32]),
                    tx_merkle_root: Sha512Trunc256Sum::from_data(&[]),
                ...
                let mut microblock_1 = FunaiMicroblockHeader {
                    version: 0,
                    sequence: 0,
                    prev_block: BlockHeaderHash([0; 32]),
                    tx_merkle_root: Sha512Trunc256Sum::from_data(&[]),
                ...
                let mut microblock_2 = FunaiMicroblockHeader {
                    version: 0,
                    sequence: 0,
                    prev_block: BlockHeaderHash([0; 32]),
                    tx_merkle_root: Sha512Trunc256Sum::from_data(&[1, 2, 3]),
                ...
                for i in 0..4 {
                    ...
                    let pubkey_hash = FunaiBlockHeader::pubkey_hash(&microblock_pubkey);
