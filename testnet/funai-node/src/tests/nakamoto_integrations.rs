...
use libsigner::{SignerSession, FunaiDBSession};
use rand::RngCore;
use funai::burnchains::{MagicBytes, Txid};
...
use funai::chainstate::nakamoto::miner::NakamotoBlockBuilder;
use funai::chainstate::nakamoto::test_signers::TestSigners;
use funai::chainstate::nakamoto::{NakamotoBlock, NakamotoChainState};
use funai::chainstate::funai::address::{PoxAddress, FunaiAddressExtensions};
use funai::chainstate::funai::boot::{
    MINERS_NAME, SIGNERS_VOTING_FUNCTION_NAME, SIGNERS_VOTING_NAME,
};
use funai::chainstate::funai::db::FunaiChainState;
use funai::chainstate::funai::miner::{BlockBuilder, BlockLimitFunction, TransactionResult};
use funai::chainstate::funai::{FunaiTransaction, ThresholdSignature, TransactionPayload};
use funai::core::{
    FunaiEpoch, FunaiEpochId, BLOCK_LIMIT_MAINNET_10, HELIUM_BLOCK_LIMIT_20,
...
use funai::libfunaidb::SlotMetadata;
use funai::net::api::callreadonly::CallReadOnlyRequestBody;
use funai::net::api::getfunais::GetFunaisResponse;
use funai::net::api::postblock_proposal::{
    BlockValidateReject, BlockValidateResponse, NakamotoBlockProposal, ValidateRejectCode,
};
use funai::util::hash::hex_bytes;
...
use funai_common::consts::{CHAIN_ID_TESTNET, STACKS_EPOCH_MAX};
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, FunaiAddress, FunaiPrivateKey, FunaiPublicKey,
};
use funai_common::types::FunaiPublicKeyBuffer;
use funai_common::util::hash::{to_hex, Sha512Trunc256Sum};
...
use crate::tests::{get_chain_info, make_funai_transfer, to_addr};
...
lazy_static! {
    pub static ref NAKAMOTO_INTEGRATION_EPOCHS: [FunaiEpoch; 9] = [
        FunaiEpoch {
...
pub fn get_funai_set(http_origin: &str, cycle: u64) -> GetFunaisResponse {
    let client = reqwest::blocking::Client::new();
    let path = format!("{http_origin}/v2/funai_set/{cycle}");
    let res = client
        .get(&path)
        .send()
        .unwrap()
        ...
        .unwrap();
    info!("Funai set response: {res}");
    let res = serde_json::from_value(res).unwrap();
    res
}

pub fn get_funaidb_slot_version(
    http_origin: &str,
    ...
    let res = client
        ...
        .unwrap();
    debug!("FunaiDB metadata response: {res:?}");
...
pub fn add_initial_balances(
    ...
    (0..accounts)
        .map(|i| {
            let privk = FunaiPrivateKey::from_seed(&[5, 5, 5, i as u8]);
...
///  of the individual signer who broadcasts the response to the FunaiDB
pub fn blind_signer(
    conf: &Config,
    signers: &TestSigners,
...
pub fn read_and_sign_block_proposal(
    ...
    let miner_pubkey = FunaiPublicKey::from_private(&conf.get_miner_config().mining_key.unwrap());
    ...
    let mut proposed_block: NakamotoBlock = {
        ...
        let mut miners_funaidb = FunaiDBSession::new(&conf.node.rpc_bind, miner_contract_id);
        miners_funaidb
    ...
    info!(
        "Fetched proposed block from .miners FunaiDB";
...
pub fn naka_neon_integration_conf(seed: Option<&[u8]>) -> (Config, FunaiAddress) {
    ...
    conf.node.add_signers_funaidbs(false);
    conf.node.add_miner_funaidb(false);
...
pub fn next_block_and_process_new_funai_block(
    btc_controller: &mut BitcoinRegtestController,
    timeout_secs: u64,
...
pub fn setup_funai(naka_conf: &mut Config) -> Secp256k1PrivateKey {
    let funai_sk = Secp256k1PrivateKey::new();
    let funai_address = tests::to_addr(&funai_sk);
    naka_conf.add_initial_balance(
        PrincipalData::from(funai_address.clone()).to_string(),
...
///
/// * `funai_sks` - must be a private key for sending a large `stack-stx` transaction in order
///   for pox-4 to activate
pub fn boot_to_epoch_3(
    naka_conf: &Config,
    blocks_processed: &Arc<AtomicU64>,
    funai_sks: &[FunaiPrivateKey],
    signer_sks: &[FunaiPrivateKey],
    self_signing: Option<&TestSigners>,
    btc_regtest_controller: &mut BitcoinRegtestController,
) {
    assert_eq!(funai_sks.len(), signer_sks.len());
    ...
    for (funai_sk, signer_sk) in funai_sks.iter().zip(signer_sks.iter()) {
        let pox_addr = PoxAddress::from_legacy(
            AddressHashMode::SerializeP2PKH,
            tests::to_addr(&funai_sk).bytes,
        );
        ...
        let stacking_tx = tests::make_contract_call(
            ...
            0,
            1000,
            &FunaiAddress::burn_address(false),
            "pox-4",
            "stack-stx",
            &[
    ...
    if let Some(signers) = self_signing {
        ...
        let signer_set = get_funai_set(&http_origin, reward_cycle + 1);
...
fn get_signer_index(
    funai_set: &GetFunaisResponse,
    signer_key: &Secp256k1PublicKey,
) -> Result<usize, String> {
    let Some(ref signer_set) = funai_set.funai_set.signers else {
...
pub fn get_key_for_cycle(
    ...
    let client = reqwest::blocking::Client::new();
    let boot_address = FunaiAddress::burn_address(is_mainnet);
...
fn signer_vote_if_needed(
    ...
    naka_conf: &Config,
    signer_sks: &[FunaiPrivateKey], // TODO: Is there some way to get this from the TestSigners?
    signers: &TestSigners,
) {
...
/// * `funai_sks` - must be a private key for sending a large `stack-stx` transaction in order
///   for pox-4 to activate
/// * `signer_pks` - must be the same size as `funai_sks`
...
fn simple_neon_integration() {
    ...
    let recipient = PrincipalData::from(FunaiAddress::burn_address(false));
    let funai_sk = setup_funai(&mut naka_conf);
    ...
    #[cfg(feature = "monitoring_prom")]
    {
        ...
        let res = client
            ...
            .text()
            .unwrap();
        let expected_result = format!("funai_node_funai_tip_height {block_height_pre_3_0}");
    ...
    // Submit a TX
    let transfer_tx = make_funai_transfer(&sender_sk, 0, send_fee, &recipient, send_amt);
    ...
    info!(
        "Latest tip";
        "height" => tip.funai_block_height,
        "is_nakamoto" => tip.anchored_header.as_funai_nakamoto().is_some(),
    );
    ...
    assert!(tip.anchored_header.as_funai_nakamoto().is_some());
    assert!(tip.funai_block_height >= block_height_pre_3_0 + 30);
    ...
    #[cfg(feature = "monitoring_prom")]
    {
        ...
        let res = client
            ...
            .text()
            .unwrap();
        let expected_result = format!("funai_node_funai_tip_height {}", tip.funai_block_height);
        assert!(res.contains(&expected_result));
    }
...
fn mine_multiple_per_tenure_integration() {
    ...
    for tenure_ix in 0..tenure_count {
        let commits_before = commits_submitted.load(Ordering::SeqCst);
        next_block_and_process_new_funai_block(&mut btc_regtest_controller, 60, &coord_channel)
            .unwrap();
        ...
        for interim_block_ix in 0..inter_blocks_per_tenure {
            ...
            let transfer_tx =
                make_funai_transfer(&sender_sk, sender_nonce, send_fee, &recipient, send_amt);
            ...
            assert_ne!(info.funai_tip, last_tip);
            assert_ne!(info.funai_tip_height, last_tip_height);

            last_tip = info.funai_tip;
            last_tip_height = info.funai_tip_height;
        }
...
fn correct_burn_outs() {
...

    {
        ...
        let epoch_24_ix = FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch24).unwrap();
        let epoch_25_ix = FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch25).unwrap();
        let epoch_30_ix = FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch30).unwrap();
    ...
    let funai_accounts = accounts[0..3].to_vec();
    let sender_signer_sk = Secp256k1PrivateKey::new();
    let sender_signer_addr = tests::to_addr(&sender_signer_sk);
    naka_conf.add_initial_balance(
        PrincipalData::from(sender_signer_addr.clone()).to_string(),
    ...
    let epochs = naka_conf.burnchain.epochs.clone().unwrap();
    let epoch_3 = &epochs[FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch30).unwrap()];
    let epoch_25 = &epochs[FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch25).unwrap()];
    ...
    info!("Chain bootstrapped to Epoch 2.5, submitting funai transaction");

    next_block_and_wait(&mut btc_regtest_controller, &blocks_processed);

    let http_origin = format!("http://{}", &naka_conf.node.rpc_bind);
    let funai_accounts_copy = funai_accounts.clone();
    let _funai_thread = thread::Builder::new()
        .name("funai".into())
        .spawn(move || loop {
            thread::sleep(Duration::from_secs(2));
            debug!("Checking for funai-necessity");
            ...
            let Some(account) = funai_accounts_copy.iter().find_map(|(sk, addr)| {
            ...
            let pox_addr_tuple: clarity::vm::Value =
                pox_addr.clone().as_clarity_tuple().unwrap().into();
            let pk_bytes = FunaiPublicKey::from_private(&sender_signer_sk).to_bytes_compressed();
    ...
    // we should already be able to query the funai set via RPC
    let burnchain = naka_conf.get_burnchain();
    ...
    let http_origin = format!("http://{}", &naka_conf.node.rpc_bind);
    let funai_response = get_funai_set(&http_origin, first_epoch_3_cycle);
    assert!(funai_response.funai_set.signers.is_some());
    assert_eq!(
        funai_response.funai_set.signers.as_ref().unwrap().len(),
        1
    );
    assert_eq!(funai_response.funai_set.rewarded_addresses.len(), 1);
    ...
    for _i in 0..30 {
        ...
        if let Err(e) = next_block_and_mine_commit(
            ...
            warn!(
                "Error while minting a bitcoin block and waiting for funai-node activity: {e:?}"
            );
            panic!();
        }
...
fn block_proposal_api_endpoint() {
    ...
    let funai_sk = setup_funai(&mut conf);
    let sender_signer_sk = Secp256k1PrivateKey::new();
    let sender_signer_addr = tests::to_addr(&sender_signer_sk);
    ...
    let tenure_cause = tenure_change.and_then(|tx: &FunaiTransaction| match &tx.payload {
    ...
    let block = {
        ...
        let tx = FunaiTransaction::consensus_deserialize(&mut &tx[..])
...
///  * The proposed Nakamoto block is written to the .miners funaidb
fn miner_writes_proposed_block_to_funaidb() {
    if env::var("BITCOIND_TEST") != Ok("1".into()) {
        return;
    }
    ...
    let tip = SortitionDB::get_canonical_burn_chain_tip(sortdb.conn()).unwrap();
    let miner_pubkey =
        FunaiPublicKey::from_private(&naka_conf.get_miner_config().mining_key.unwrap());
    ...
    let proposed_block: NakamotoBlock = {
        let miner_contract_id = boot_code_id(MINERS_NAME, false);
        let mut miners_funaidb =
            FunaiDBSession::new(&naka_conf.node.rpc_bind, miner_contract_id);
        miners_funaidb
    ...
    assert_eq!(
        ...
        "Observed miner hash should match the proposed block read from FunaiDB (after zeroing signatures)"
    );
}
...
fn vote_for_aggregate_key_burn_op() {
    ...
    let (_chainstate, _) = FunaiChainState::open(
        naka_conf.is_mainnet(),
        naka_conf.burnchain.chain_id,
        &naka_conf.get_chainstate_path_str(),
        None,
    ...
    let funai_pk = FunaiPublicKey::from_private(&funai_sk);
    let signer_key: FunaiPublicKeyBuffer = funai_pk.to_bytes_compressed().as_slice().into();
...
fn follower_bootup() {
    ...
    loop {
        ...
        info!(
            "Follower tip is now {}/{}",
            &follower_node_info.funai_tip_consensus_hash, &follower_node_info.funai_tip
        );
        if follower_node_info.funai_tip_consensus_hash == tip.consensus_hash
            && follower_node_info.funai_tip == tip.anchored_header.block_hash()
        {
            break;
        }
    }
...
fn stack_stx_burn_op_integration_test() {
    ...
    let signer_sk_1 = setup_funai(&mut naka_conf);
    ...
    let signer_key_arg_1: FunaiPublicKeyBuffer =
