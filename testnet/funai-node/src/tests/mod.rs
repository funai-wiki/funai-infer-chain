...
use funai::chainstate::funai::events::FunaiTransactionEvent;
use funai::chainstate::funai::miner::{BlockBuilderSettings, FunaiMicroblockBuilder};
use funai::chainstate::funai::{
    CoinbasePayload, FunaiBlock, FunaiMicroblock, FunaiMicroblockHeader, FunaiPrivateKey,
    FunaiPublicKey, FunaiTransaction, FunaiTransactionSigner, TokenTransferMemo,
...
pub fn serialize_sign_tx_anchor_mode_version(
    ...
    let auth = match (payer, payer_nonce) {
        (Some(payer), Some(payer_nonce)) => {
            let mut payer_spending_condition = TransactionSpendingCondition::new_singlesig_p2pkh(
                FunaiPublicKey::from_private(payer),
            )
            .expect("Failed to create p2pkh spending condition from public key.");
    ...
    let mut unsigned_tx = FunaiTransaction::new(version, auth, payload);
...
pub fn new_test_conf() -> Config {
    ...
    // funaiAddress: "ST2VHM28V9E5QCRD6C73215KAPSBKQGPWTEE5CMQT"
    ...
    conf.node.working_dir = format!(
        "/tmp/funai-node-tests/integrations-neon/{}-{}",
        to_hex(&buf),
        get_epoch_time_secs()
    );
    conf.node.seed =
    ...
    conf.burnchain.epochs = Some(FunaiEpoch::all(0, 0, 0));
...
pub fn make_sponsored_funai_transfer_on_testnet(
    sender: &FunaiPrivateKey,
    payer: &FunaiPrivateKey,
    sender_nonce: u64,
    payer_nonce: u64,
...
pub fn make_funai_transfer_mblock_only(
    sender: &FunaiPrivateKey,
    nonce: u64,
    tx_fee: u64,
    recipient: &PrincipalData,
...
pub fn make_poison(
    ...
    nonce: u64,
    tx_fee: u64,
    header_1: FunaiMicroblockHeader,
    header_2: FunaiMicroblockHeader,
) -> Vec<u8> {
...
/// Deserializes the `FunaiTransaction` objects from `blocks` and returns all those that
/// match `test_fn`.
pub fn select_transactions_where(
    blocks: &Vec<serde_json::Value>,
    test_fn: fn(&FunaiTransaction) -> bool,
...
fn should_succeed_mining_valid_txs() {
    ...
    run_loop.callbacks.on_new_tenure(|round, _burnchain_tip, chain_tip, tenure| {
        ...
        match round {
            ...
            5 => {
                // On round 5, publish a funai transaction
        ...
        |round, _burnchain_tip, chain_tip, _chain_state, _burn_dbconn| {
            match round {
                0 => {
                    ...
                    assert!(chain_tip.metadata.funai_block_height == 1);
                ...
                1 => {
                    ...
                    let events: Vec<FunaiTransactionEvent> = chain_tip
                        .receipts
                        .iter()
                ...
                3 => {
                    ...
                    assert!(match &events.last().unwrap() {
                        FunaiTransactionEvent::SmartContractEvent(data) => {
                            format!("{}", data.key.0)
                ...
                5 => {
                    ...
                    assert!(chain_tip.metadata.funai_block_height == 6);
                    ...
                    assert!(events.len() == 1);
                    assert!(match &events[0] {
                        FunaiTransactionEvent::STXEvent(STXEventType::STXTransferEvent(event)) => {
...
fn test_inner_pick_best_tip() {
    ...
    // No other tips at Funai height 2 will be accepted, nor will those at heights 3 and 4 (since
