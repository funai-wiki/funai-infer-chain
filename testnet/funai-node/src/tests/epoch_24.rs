// Copyright (C) 2023 Funai Open Internet Foundation
//
...
use funai::chainstate::funai::boot::RawRewardSetEntry;
use funai::chainstate::funai::db::FunaiChainState;
use funai::chainstate::funai::{Error, FunaiTransaction, TransactionPayload};
...
#[test]
#[ignore]
/// Verify the buggy funai-increase behavior that was possible in PoX-2 does not crash the
/// node in Epoch 2.4
///
...
#[test]
#[ignore]
/// Verify that funais that don't meet the stacking threshold get auto-unlocked in PoX-3.
fn verify_auto_unlock_behavior() {
    ...
    let pox_pubkey_2_stx_addr = FunaiAddress::from_public_keys(
    ...
    let pox_pubkey_3_stx_addr = FunaiAddress::from_public_keys(
        C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
        &AddressHashMode::SerializeP2PKH,
        1,
        &vec![pox_pubkey_3],
    ...
    for _cycle_number in first_v3_cycle..(first_v3_cycle + 6) {
        ...
        let tip_info = get_chain_info(&conf);
        let tip_block_id =
            FunaiBlockId::new(&tip_info.funai_tip_consensus_hash, &tip_info.funai_tip);
