use funai::burnchains::BurnchainSigner;
use funai::chainstate::funai::{
    FunaiPrivateKey, FunaiPublicKey, FunaiTransactionSigner, TransactionAuth,
};
...
impl Keychain {
    ...
    /// Returns the bytes that can be fed into FunaiPrivateKey
    fn make_secret_key_bytes(seed: &[u8]) -> Vec<u8> {
        let mut re_hashed_seed = seed.to_vec();
        loop {
            match FunaiPrivateKey::from_slice(&re_hashed_seed[..]) {
                Ok(_sk) => {
                    break;
                }
                Err(_) => {
    ...
    fn get_secret_key(&self) -> FunaiPrivateKey {
        let sk_bytes = Keychain::make_secret_key_bytes(&self.secret_state);
        FunaiPrivateKey::from_slice(&sk_bytes[..]).expect("FATAL: Keychain::make_secret_key_bytes() returned bytes that could not be parsed into a secp256k1 secret key!")
    }
    ...
    /// Generate a Funai keypair for this burn block height.
    /// The keypair is unique to this burn block height.
    pub fn make_funai_keypair(
        &self,
        block_height: u64,
        salt: &[u8],
    ) -> (FunaiPublicKey, FunaiPrivateKey) {
        let seed = {
            let mut secret_state = self.secret_state.clone();
            secret_state.extend_from_slice(&block_height.to_be_bytes());
        ...
        let sk = FunaiPrivateKey::from_slice(&sk_bytes[..]).expect("FATAL: Keychain::make_secret_key_bytes() returned bytes that could not be parsed into a secp256k1 secret key!");
    ...
    /// `salt` can be any byte string; in practice, it's the parent Funai block's block ID hash.
    pub fn make_microblock_secret_key(
        &mut self,
        burn_block_height: u64,
        salt: &[u8],
    ) -> FunaiPrivateKey {
        let (_, mut sk) = self.make_funai_keypair(burn_block_height, salt);
        sk.set_compress_public(true);

        debug!("Microblock keypair rotated";
               "burn_block_height" => %burn_block_height,
               "pubkey_hash" => %Hash160::from_node_public_key(&FunaiPublicKey::from_private(&sk)).to_string()
        );
        sk
    }
    ...
    pub fn get_pub_key(&self) -> Secp256k1PublicKey {
        ...
        FunaiPublicKey::from_private(&sk)
    }

    /// Get the Funai address for the inner secret state
    pub fn get_address(&self, is_mainnet: bool) -> FunaiAddress {
        let sk = self.get_secret_key();
        let pk = FunaiPublicKey::from_private(&sk);
        ...
        FunaiAddress::from_public_keys(version, &AddressHashMode::SerializeP2PKH, 1, &vec![pk])
    ...
    /// Convenience wrapper around make_funai_keypair
    pub fn get_microblock_key(&self, block_height: u64) -> FunaiPrivateKey {
        self.make_funai_keypair(block_height, &[]).1
    }

    /// Sign a transaction as if we were the origin
    pub fn sign_as_origin(&self, tx_signer: &mut FunaiTransactionSigner) -> () {
        let sk = self.get_secret_key();
        tx_signer
            .sign_origin(&sk)
    ...
    pub fn origin_address(&self, is_mainnet: bool) -> Option<FunaiAddress> {
...
mod tests {
    ...
    use funai::burnchains::PrivateKey;
    use funai::chainstate::funai::{
        FunaiPrivateKey, FunaiPublicKey, FunaiTransaction, FunaiTransactionSigner,
    ...
    pub struct KeychainOld {
        secret_keys: Vec<FunaiPrivateKey>,
        threshold: u16,
        hash_mode: AddressHashMode,
        pub hashed_secret_state: Sha256Sum,
        microblocks_secret_keys: Vec<FunaiPrivateKey>,
    ...
    impl KeychainOld {
        ...
        pub fn rotate_microblock_keypair(&mut self, burn_block_height: u64) -> FunaiPrivateKey {
            ...
            // As such, we continuously generate seeds through re-hashing until one works.
            let mut sk = loop {
                match FunaiPrivateKey::from_slice(&seed.to_bytes()[..]) {
                    Ok(sk) => break sk,
        ...
        pub fn get_microblock_key(&self) -> Option<FunaiPrivateKey> {
        ...
        pub fn get_address(&self, is_mainnet: bool) -> FunaiAddress {
            let public_keys = self
                .secret_keys
                .iter()
                .map(|ref pk| FunaiPublicKey::from_private(pk))
                .collect();
            let version = if is_mainnet {
    ...
    fn test_sign_as_origin() {
        ...
        for seed in seeds {
            ...
            let recv_addr =
                FunaiAddress::from_string("SP1Z4P459B2M5XC2PMM2CSCNZ6824DN5GZG2XYWFH").unwrap();

            let mut tx_stx_transfer_1 = FunaiTransaction::new(
            ...
            let mut tx_stx_transfer_2 = FunaiTransaction::new(
                TransactionVersion::Testnet,
            ...
            let mut signer_1 = FunaiTransactionSigner::new(&tx_stx_transfer_1);
            k1.sign_as_origin(&mut signer_1);
            let tx_1 = signer_1.get_tx().unwrap();

            let mut signer_2 = FunaiTransactionSigner::new(&tx_stx_transfer_2);
