...
/// Main body of code for the Funai node and miner.
///
/// System schematic.
/// Legend:
///    |------|    Thread
...
/// Specifically, this thread directs the ChainsCoordinator thread when to process new Funai
...
///    * (Sychronously) requesting the CoordinatorThread to process newly-stored Funai blocks and
///    microblocks
...
///
/// * **ChainsCoordinator Thread**:  This thread process sortitions and Funai blocks and
...
/// drives Funai blocks processing, the latter sortitions).
/// This thread is responsible for:
///    * Responding to requests from other threads to process sortitions
///    * Responding to requests from other threads to process Funai blocks and microblocks
...
use funai::burnchains::bitcoin::address::{BitcoinAddress, LegacyBitcoinAddressType};
use funai::burnchains::db::BurnchainHeaderReader;
use funai::burnchains::{Burnchain, BurnchainSigner, PoxConstants, Txid};
use funai::chainstate::burn::db::sortdb::SortitionDB;
use funai::chainstate::burn::operations::leader_block_commit::{
    RewardSetInfo, BURN_BLOCK_MINED_AT_MODULUS,
};
use funai::chainstate::burn::operations::{
    BlockstackOperationType, LeaderBlockCommitOp, LeaderKeyRegisterOp,
};
use funai::chainstate::burn::{BlockSnapshot, ConsensusHash};
use funai::chainstate::coordinator::{get_next_recipients, OnChainRewardSetProvider};
use funai::chainstate::nakamoto::NakamotoChainState;
use funai::chainstate::funai::address::PoxAddress;
use funai::chainstate::funai::db::blocks::StagingBlock;
use funai::chainstate::funai::db::{FunaiChainState, FunaiHeaderInfo, MINER_REWARD_MATURITY};
use funai::chainstate::funai::miner::{
    signal_mining_blocked, signal_mining_ready, BlockBuilderSettings, FunaiMicroblockBuilder,
};
use funai::chainstate::funai::{
    CoinbasePayload, Error as ChainstateError, FunaiBlock, FunaiBlockBuilder, FunaiBlockHeader,
    FunaiMicroblock, FunaiPublicKey, FunaiTransaction, FunaiTransactionSigner,
    TransactionAnchorMode, TransactionPayload, TransactionVersion,
};
use funai::core::mempool::MemPoolDB;
use funai::core::{FIRST_BURNCHAIN_CONSENSUS_HASH, STACKS_EPOCH_3_0_MARKER};
use funai::cost_estimates::metrics::{CostMetric, UnitMetric};
use funai::cost_estimates::{CostEstimator, FeeEstimator, UnitEstimator};
use funai::monitoring;
use funai::monitoring::{increment_stx_blocks_mined_counter, update_active_miners_count_gauge};
use funai::net::atlas::{AtlasConfig, AtlasDB};
use funai::net::db::{LocalPeer, PeerDB};
use funai::net::dns::{DNSClient, DNSResolver};
use funai::net::p2p::PeerNetwork;
use funai::net::relay::Relayer;
use funai::net::funaidb::{FunaiDBConfig, FunaiDBSync, FunaiDBs};
use funai::net::{
    Error as NetError, NetworkResult, PeerNetworkComms, RPCHandlerArgs, ServiceFlags,
};
use funai::util_lib::strings::{UrlString, VecDisplay};
use funai_common::codec::FunaiMessageCodec;
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, SortitionId, FunaiAddress, FunaiBlockId,
    FunaiPrivateKey, VRFSeed,
};
use funai_common::types::net::PeerAddress;
use funai_common::types::{PublicKey, FunaiEpochId};
use funai_common::util::hash::{to_hex, Hash160, Sha256Sum};
use funai_common::util::secp256k1::Secp256k1PrivateKey;
...
/// Result of running the miner thread.  It could produce a Funai block or a microblock.
pub(crate) enum MinerThreadResult {
    ...
    Microblock(
        Result<Option<(FunaiMicroblock, ExecutionCost)>, NetError>,
        MinerTip,
    ),
}

/// Fully-assembled Funai anchored, block as well as some extra metadata pertaining to how it was
...
pub struct AssembledAnchorBlock {
    /// Consensus hash of the parent Funai block
...
pub(crate) fn fault_injection_long_tenure() {
    ...
    funai_common::util::sleep_ms(tenure_time);
}
...
pub(crate) fn open_chainstate_with_faults(
    config: &Config,
) -> Result<FunaiChainState, ChainstateError> {
    let funai_chainstate_path = config.get_chainstate_path_str();
    let (mut chainstate, _) = FunaiChainState::open(
        config.is_mainnet(),
        config.burnchain.chain_id,
        &funai_chainstate_path,
...
enum Error {
    ...
    /// Can't find the funai block's offset in the burnchain block
...
struct ParentFunaiBlockInfo {
    /// Header metadata for the Funai block we're going to build on top of
    funai_parent_header: FunaiHeaderInfo,
    /// the consensus hash of the sortition that selected the Funai block parent
    parent_consensus_hash: ConsensusHash,
    /// the burn block height of the sortition that selected the Funai block parent
    parent_block_burn_height: u64,
    /// the total amount burned in the sortition that selected the Funai block parent
...
pub struct RelayerThread {
    ...
    chainstate: Option<FunaiChainState>,
    ...
    /// is used to ensure that the p2p thread attempts to download new Funai block data before
    ...
    /// if true, then the last time the miner thread was launched, it was used to mine a Funai
    /// block (used to alternate between mining microblocks and Funai blocks that confirm them)
    mined_funai_block: bool,
}
...
struct MicroblockMinerThread {
    ...
    /// Handle to the node's event dispatcher
    event_dispatcher: EventDispatcher,
    /// Parent Funai block's sortition's consensus hash
    parent_consensus_hash: ConsensusHash,
    /// Parent Funai block's hash
    ...
    quantity: u64,
    /// Block budget consumed so far by this tenure (initialized to the cost of the Funai block
...
impl MicroblockMinerThread {
    ...
    pub fn from_relayer_thread(relayer_thread: &RelayerThread) -> Option<MicroblockMinerThread> {
        ...
        let miner_tip = match relayer_thread.miner_tip.clone() {
            Some(tip) => tip,
            None => {
                debug!("Relayer: cannot instantiate microblock miner: did not win Funai tip sortition");
                return None;
            }
        };
        ...
        match FunaiChainState::get_anchored_block_header_info(chainstate.db(), &ch, &bhh) {
            Ok(Some(_)) => {
                let parent_index_hash = FunaiBlockHeader::make_index_block_hash(&ch, &bhh);
                let cost_so_far = if relayer_thread.microblock_stream_cost == ExecutionCost::zero()
                {
                    // unknown cost, or this is idempotent.
                    FunaiChainState::get_funai_block_anchored_cost(
                        chainstate.db(),
                        &parent_index_hash,
                    )
    ...
    where
        F: FnOnce(&mut Self, &mut SortitionDB, &mut FunaiChainState, &mut MemPoolDB) -> R,
    {
    ...
    fn inner_mine_one_microblock(
    ...
    ) -> Result<FunaiMicroblock, ChainstateError> {
        debug!(
        ...
        let epoch_id = SortitionDB::get_funai_epoch(sortdb.conn(), burn_height)
            .map_err(|e| {
        ...
        let mint_result = {
            ...
            let mut microblock_miner = match FunaiMicroblockBuilder::resume_unconfirmed(
                chainstate,
                &ic,
                &self.cost_so_far,
                self.settings.clone(),
    ...
    fn inner_try_mine_microblock(
        ...
        let num_attachable = FunaiChainState::count_attachable_staging_blocks(
            chainstate.db(),
            1,
    ...
    /// * there are still funai blocks to be processed in the staging db
...
pub struct TipCandidate {
    ...
    pub burn_height: u64,
    /// the number of Funai blocks *at the same height* as this one, but from earlier sortitions
    /// than `burn_height`
    pub num_earlier_siblings: u64,
}

impl TipCandidate {
    pub fn id(&self) -> FunaiBlockId {
        FunaiBlockId::new(&self.consensus_hash, &self.anchored_block_hash)
    }

    pub fn parent_id(&self) -> FunaiBlockId {
        FunaiBlockId::new(
            &self.parent_consensus_hash,
...
impl BlockMinerThread {
    ...
    fn get_coinbase_recipient(&self, epoch_id: FunaiEpochId) -> Option<PrincipalData> {
        let miner_config = self.config.get_miner_config();
        if epoch_id < FunaiEpochId::Epoch21 && miner_config.block_reward_recipient.is_some() {
    ...
    fn inner_generate_coinbase_tx(
        ...
        let mut tx = FunaiTransaction::new(
            version,
            tx_auth,
        ...
        let mut tx_signer = FunaiTransactionSigner::new(&tx);
    ...
    fn inner_generate_poison_microblock_tx(
        ...
        let mut tx = FunaiTransaction::new(version, tx_auth, poison_payload);
        tx.chain_id = chain_id;
        tx.anchor_mode = TransactionAnchorMode::OnChainOnly;
        let mut tx_signer = FunaiTransactionSigner::new(&tx);
        self.keychain.sign_as_origin(&mut tx_signer);
    ...
    /// Load all candidate tips upon which to build.  This is all Funai blocks whose heights are
    /// less than or equal to at `at_funai_height` (or the canonical chain tip height, if not given),
    /// but greater than or equal to this end height minus `max_depth`.
    /// Returns the list of all Funai blocks up to max_depth blocks beneath it.
    /// The blocks will be sorted first by funai height, and then by burnchain height
    pub(crate) fn load_candidate_tips(
        burn_db: &mut SortitionDB,
        chain_state: &mut FunaiChainState,
        max_depth: u64,
        at_funai_height: Option<u64>,
    ) -> Vec<TipCandidate> {
        let funai_tips = if let Some(start_height) = at_funai_height {
            chain_state
                .get_funai_chain_tips_at_height(start_height)
                .expect("FATAL: could not query chain tips at start height")
        } else {
            chain_state
                .get_funai_chain_tips(burn_db)
        ...
        let funai_tips: Vec<_> = funai_tips
            .into_iter()
            .filter(|candidate| {
                ...
                let candidate_burn_ht = match SortitionDB::get_block_snapshot_consensus(
                    ...
                    Ok(None) => {
                        warn!("Tried to evaluate potential chain tip with an unknown consensus hash";
                              ...
                              "funai_block_hash" => %candidate.anchored_block_hash);
                        return false;
                    },
                    Err(e) => {
        ...
        let end_height = funai_tips[0].height;

        for cur_height in end_height.saturating_sub(max_depth)..=end_height {
            let funai_tips = chain_state
                .get_funai_chain_tips_at_height(cur_height)
                .expect("FATAL: could not query chain tips at height");

            for tip in funai_tips {
                let index_block_hash =
                    FunaiBlockId::new(&tip.consensus_hash, &tip.anchored_block_hash);
                ...
                if !considered.contains(&index_block_hash) {
                    let burn_height = burn_db
                        ...
                        .expect("FATAL: no burnchain block height for Funai tip");
    ...
    /// Put all tip candidates in order by funai height, breaking ties with burnchain height.
    /// Also, count up the number of earliersiblings each tip has -- i.e. the number of funai
    ...
    pub(crate) fn sort_and_populate_candidates(
        ...
        candidates.sort_by(|tip1, tip2| {
            // funai block height, then burnchain block height
            let ord = tip1.funai_height.cmp(&tip2.funai_height);
        ...
        // this is the number of funai blocks at the same height, but later burnchain heights.
        let mut idx = 0;
        let mut cur_funai_height = candidates[idx].funai_height;
        ...
        loop {
            ...
            if cur_funai_height == candidates[idx].funai_height {
                // same funai height, so this block has one more earlier sibling than the last
                num_siblings += 1;
                candidates[idx].num_earlier_siblings = num_siblings;
            } else {
                // new funai height, so no earlier siblings
                num_siblings = 0;
                cur_funai_height = candidates[idx].funai_height;
    ...
    /// leaf nodes where the Funai block height is <= the max height -
    ...
    pub fn pick_best_tip(
        ...
        let funai_tips =
            Self::load_candidate_tips(burn_db, chain_state, max_depth, at_funai_height);

        let mut previous_best_tips = HashMap::new();
        for tip in funai_tips.iter() {
            let Some(prev_best_tip) = globals.get_best_tip(tip.funai_height) else {
                continue;
            };
            previous_best_tips.insert(tip.funai_height, prev_best_tip);
        }

        let best_tip_opt = Self::inner_pick_best_tip(funai_tips, previous_best_tips);
        if let Some(best_tip) = best_tip_opt.as_ref() {
            globals.add_best_tip(best_tip.funai_height, best_tip.clone(), max_depth);
        } else {
            // no best-tip found; revert to old tie-breaker logic
            info!("No best-tips found; using old tie-breaking logic");
            return chain_state
                .get_funai_chain_tip(burn_db)
    ...
    /// Takes the list of funai tips that are eligible to be built on, and a map of
    /// previously-chosen best tips (so if we chose a tip in the past, we keep confirming it, even
    /// if subsequent funai blocks show up).  The previous best tips should be from recent Funai
    /// heights; it's important that older best-tips are forgotten in order to ensure that miners
    /// will eventually (e.g. after `max_reorg_depth` Funai blocks pass) stop trying to confirm a
    ...
    pub(crate) fn inner_pick_best_tip(
        ...
        for (i, leaf_tip) in leaf_tips.iter().enumerate() {
            ...
            // Score each leaf tip as the number of preceding Funai blocks that are _not_ an
            // ancestor.  Because funai_tips are in order by funai height, a linear scan of this
            // list will allow us to match all ancestors in the last max_depth Funai blocks.
            // `ancestor_ptr` tracks the next expected ancestor.
            let mut ancestor_ptr = leaf_tip.parent_id();
            let mut score: u64 = 0;
            let mut score_summaries = vec![];

            // find the highest funai_tip we must confirm
            let mut must_confirm = None;
            for tip in funai_tips.iter().rev() {
                if let Some(prev_best_tip) = previous_best_tips.get(&tip.funai_height) {
            ...
            for tip in funai_tips.iter().rev() {
                if let Some(required_ancestor) = must_confirm.as_ref() {
                    if tip.funai_height < required_ancestor.funai_height
                        && leaf_tip.funai_height >= required_ancestor.funai_height
                    {
                ...
                if leaf_tip.funai_height < tip.funai_height {
                    // this tip is further along than leaf_tip, so canonicalizing leaf_tip would
                    // orphan `tip.funai_height - leaf_tip.funai_height` blocks.
                    score = score.saturating_add(tip.funai_height - leaf_tip.funai_height);
                    score_summaries.push(format!(
                        "{} (stx height diff)",
                        tip.funai_height - leaf_tip.funai_height
                    ));
                } else if leaf_tip.funai_height == tip.funai_height
                    && leaf_tip.burn_height > tip.burn_height
                {
                    // this tip has the same funai height as the leaf, but its sortition happened
                ...
                if tip.id() == ancestor_ptr {
                    ...
                    // this funai tip is the next ancestor.  However, that ancestor may have
                    ...
                    // this funai tip is not an ancestor, and would be orphaned if leaf_tip is
                    // canonical.
    ...
    fn load_block_parent_info(
        ...
        burn_db: &mut SortitionDB,
        chain_state: &mut FunaiChainState,
    ) -> (Option<ParentFunaiBlockInfo>, bool) {
        if let Some(funai_tip) = chain_state
            .get_funai_chain_tip(burn_db)
            .expect("FATAL: could not query chain tip")
        {
            ...
            let miner_address = self
                ...
                .unwrap();
            let parent_info = match ParentFunaiBlockInfo::lookup(
                chain_state,
                burn_db,
                &self.burn_block,
                miner_address,
                &best_funai_tip.consensus_hash,
                &best_funai_tip.anchored_block_hash,
            ) {
            ...
            if parent_info.is_none() {
                warn!(
                    ...
                    &best_funai_tip.consensus_hash, &best_funai_tip.anchored_block_hash
                );
            }
            let canonical = best_funai_tip.consensus_hash == funai_tip.consensus_hash
                && best_funai_tip.anchored_block_hash == funai_tip.anchored_block_hash;
            (parent_info, canonical)
        } else {
            debug!("No Funai chain tip known, will return a genesis block");
            ...
            (
                Some(ParentFunaiBlockInfo {
                    funai_parent_header: chain_tip.metadata,
    ...
    fn get_mine_attempt(
        &self,
        chain_state: &FunaiChainState,
        parent_block_info: &ParentFunaiBlockInfo,
        force: bool,
    ) -> Option<(u64, u64)> {
        let parent_consensus_hash = &parent_block_info.parent_consensus_hash;
        let funai_parent_header = &parent_block_info.funai_parent_header;
        ...
        let (attempt, max_txs) = if should_unconditionally_mine {
            ...
            let mut max_txs = 0;
            info!(
                "Consider {} in-flight Funai tip(s)",
                &last_mined_blocks.len()
            );
            for prev_block in last_mined_blocks.iter() {
                info!(
                    "Consider in-flight block {} on Funai tip {}/{} in {} with {} txs",
                ...
                if prev_block.parent_consensus_hash == *parent_consensus_hash
                    ...
                    && prev_block.anchored_block.header.parent_block
                        == funai_parent_header.anchored_header.block_hash()
                {
                        ...
                        )
                    {
                                        ...
                                        + 1)
                        {
                            if !force {
                                ...
                                // already have.
                                info!("Relayer: Funai tip is unchanged since we last tried to mine a block off of {}/{} at height {} with {} txs, in {} at burn height {}, and no new microblocks ({} <= {} + 1)",
                            ...
                            info!("Relayer: Funai tip is unchanged since we last tried to mine a block off of {}/{} at height {} with {} txs, in {} at burn height {}, but there are new microblocks ({} > {} + 1)",
                                   &prev_block.parent_consensus_hash, &prev_block.anchored_block.header.parent_block, prev_block.anchored_block.header.total_work.work,
                                   prev_block.anchored_block.txs.len(), prev_block.my_burn_hash, parent_block_burn_height, stream.len(), prev_block.anchored_block.header.parent_microblock_sequence);
                        ...
                        if !force {
                            // no microblock stream to confirm, and the funai tip hasn't changed
                            info!("Relayer: Funai tip is unchanged since we last tried to mine a block off of {}/{} at height {} with {} txs, in {} at burn height {}, and no microblocks present",
                                   &prev_block.parent_consensus_hash, &prev_block.anchored_block.header.parent_block, prev_block.anchored_block.header.total_work.work,
                    ...
                    if self.burn_block.burn_header_hash == prev_block.my_burn_hash {
                        ...
                        info!("Relayer: Funai tip has changed to {}/{} since we last tried to mine a block in {} at burn height {}; attempt was {} (for Funai tip {}/{})",
                               parent_consensus_hash, funai_parent_header.anchored_header.block_hash(), prev_block.my_burn_hash, parent_block_burn_height, prev_block.attempt, &prev_block.parent_consensus_hash, &prev_block.anchored_block.header.parent_block);
                        best_attempt = cmp::max(best_attempt, prev_block.attempt);
                    } else {
    ...
    ///
    /// In testing, we ignore the parent funai block hash because we don't have an easy way to
    /// reproduce it in integration tests.
    #[cfg(not(any(test, feature = "testing")))]
    fn make_microblock_private_key(
        &mut self,
        parent_funai_hash: &FunaiBlockId,
    ) -> Secp256k1PrivateKey {
        // Generates a new secret key for signing the trail of microblocks
        // of the upcoming tenure.
        self.keychain
            .make_microblock_secret_key(self.burn_block.block_height, &parent_funai_hash.0)
    }
    ...
    fn make_microblock_private_key(
        &mut self,
        _parent_funai_hash: &FunaiBlockId,
    ) -> Secp256k1PrivateKey {
        // Generates a new secret key for signing the trail of microblocks
        // of the upcoming tenure.
    ...
    /// Update funai_parent_header's microblock tail to point to the end of the stream we load.
    ...
    fn load_and_vet_parent_microblocks(
        ...
        sortdb: &SortitionDB,
        mem_pool: &mut MemPoolDB,
        parent_block_info: &mut ParentFunaiBlockInfo,
    ) -> Option<Vec<FunaiMicroblock>> {
        let parent_consensus_hash = &parent_block_info.parent_consensus_hash;
        let funai_parent_header = &mut parent_block_info.funai_parent_header;

        let microblock_info_opt =
            match FunaiChainState::load_descendant_staging_microblock_stream_with_poison(
                chain_state.db(),
                &FunaiBlockHeader::make_index_block_hash(
        ...
        if let Some((ref microblocks, ref poison_opt)) = &microblock_info_opt {
            ...
            funai_parent_header.microblock_tail =
    ...
    fn unprocessed_blocks_prevent_mining(
        ...
        if let Some(funai_tip) =
            NakamotoChainState::get_canonical_block_header(chainstate.db(), sortdb)
                .expect("FATAL: could not query canonical Funai chain tip")
        {
            ...
            let has_unprocessed = FunaiChainState::has_higher_unprocessed_blocks(
                chainstate.db(),
                funai_tip.anchored_header.height(),
                process_deadline,
            )
            .expect("FATAL: failed to query staging blocks");
            if has_unprocessed {
                let highest_unprocessed_opt = FunaiChainState::get_highest_unprocessed_block(
                    chainstate.db(),
                    process_deadline,
                )
                .expect("FATAL: failed to query staging blocks");
                ...
                if let Some(highest_unprocessed) = highest_unprocessed_opt {
                    ...
                    if let Some(highest_unprocessed_block_sn) = highest_unprocessed_block_sn_opt {
                        if funai_tip.anchored_header.height()
    ...
    /// Try to mine a Funai block by assembling one from mempool transactions and sending a
    ...
    pub fn run_tenure(&mut self) -> Option<MinerThreadResult> {
        ...
        let funai_chainstate_path = self.config.get_chainstate_path_str();
        ...
        let target_epoch_id =
            SortitionDB::get_funai_epoch(burn_db.conn(), self.burn_block.block_height + 1)
                .ok()?
                .expect("FATAL: no epoch defined")
                .epoch_id;
        ...
        let microblock_private_key = self.make_microblock_private_key(
            &parent_block_info.funai_parent_header.index_block_hash(),
        );
        let mblock_pubkey_hash = {
            let mut pubkh = Hash160::from_node_public_key(&FunaiPublicKey::from_private(
        ...
        let (anchored_block, _, _) = match FunaiBlockBuilder::build_anchored_block(
            &chain_state,
            &burn_db.index_conn(),
            &mut mem_pool,
            &parent_block_info.funai_parent_header,
            ...
            Ok(block) => block,
            Err(ChainstateError::InvalidFunaiMicroblock(msg, mblock_header_hash)) => {
                ...
                // try again
                match FunaiBlockBuilder::build_anchored_block(
                    &chain_state,
                    &burn_db.index_conn(),
                    &mut mem_pool,
                    &parent_block_info.funai_parent_header,
        ...
        // last chance -- confirm that the funai tip is unchanged (since it could have taken long
        // enough to build this block that another block could have arrived), and confirm that all
        // Funai blocks with heights higher than the canoincal tip are processed.
        let cur_burn_chain_tip = SortitionDB::get_canonical_burn_chain_tip(burn_db.conn())
            .expect("FATAL: failed to query sortition DB for canonical burn chain tip");

        if let Some(funai_tip) = Self::pick_best_tip(
            &self.globals,
            &self.config,
            &mut burn_db,
            &mut chain_state,
            ...
            if funai_tip.anchored_block_hash != anchored_block.header.parent_block
                || parent_block_info.parent_consensus_hash != funai_tip.consensus_hash
                ...
                || has_unprocessed
            {
                info!(
                    ...
                    "attempt" => attempt,
                    "new_funai_tip_block_hash" => %funai_tip.anchored_block_hash,
                    "new_funai_tip_consensus_hash" => %funai_tip.consensus_hash,
...
impl RelayerThread {
    /// Instantiate off of a FunaiNode, a runloop, and a relayer.
    ...
    pub fn chainstate_ref(&self) -> &FunaiChainState {
        self.chainstate
            .as_ref()
    ...
    where
        F: FnOnce(&mut RelayerThread, &mut SortitionDB, &mut FunaiChainState, &mut MemPoolDB) -> R,
    {
        let mut sortdb = self
            .sortdb
            .take()
    ...
    /// Blocks until the given funai block is processed.
    ...
    fn accept_winning_tenure(
        ...
        let epoch_id = SortitionDB::get_funai_epoch(self.sortdb_ref().conn(), burn_height)?
            .expect("FATAL: no epoch defined")
            .epoch_id;
    ...
    fn process_new_block(&self) -> Result<bool, Error> {
        // process the block
        let funai_blocks_processed = self.globals.coord_comms.get_funai_blocks_processed();
        if !self.globals.coord_comms.announce_new_funai_block() {
            return Err(Error::CoordinatorClosed);
        }
        if !self
            .globals
            .coord_comms
            .wait_for_funai_blocks_processed(funai_blocks_processed, u64::MAX)
        {
            // basically unreachable
            warn!("ChainsCoordinator timed out while waiting for new funai block to be processed");
            return Ok(false);
        }
        debug!("Relayer: Funai block has been processed");
    ...
    fn pick_higher_tip(cur: Option<MinerTip>, new: Option<MinerTip>) -> Option<MinerTip> {
        match (cur, new) {
            ...
            (None, None) => None,
            (Some(cur), Some(new)) => {
                if cur.funai_height < new.funai_height {
                    Some(new)
                } else if cur.funai_height > new.funai_height {
                    Some(cur)
    ...
    /// latest Funai mining tip we have produced so far.
    ///
    ...
    pub fn process_one_tenure(
            ...
            self.last_mined_blocks.remove(&block_header_hash)
        {
            ...
            debug!("Relayer: Won sortition!";
                  "funai_header" => %block_header_hash,
    ...
    pub fn process_new_tenures(
        ...
        if let Some(mtip) = miner_tip.take() {
            ...
            let (funai_tip_consensus_hash, funai_tip_block_hash) =
                self.with_chainstate(|_relayer_thread, sortdb, _chainstate, _| {
                    SortitionDB::get_canonical_funai_chain_tip_hash(sortdb.conn()).expect(
                        "FATAL: failed to query sortition DB for canonical funai chain tip hashes",
                    )
                });

            if mtip.consensus_hash != funai_tip_consensus_hash
                || mtip.block_hash != funai_tip_block_hash
            {
                debug!(
                    "Relayer: miner tip {}/{} is NOT canonical ({}/{})",
                    &mtip.consensus_hash,
                    &mtip.block_hash,
                    &funai_tip_consensus_hash,
                    &funai_tip_block_hash
                );
                miner_tip = None;
            } else {
                debug!(
        ...
        if num_tenures > 0 || num_sortitions > 0 {
            if self.miner_tip.is_some() {
                ...
                if self.config.node.mine_microblocks && !microblocks_disabled {
                    ...
                    self.mined_funai_block = true;
                } else {
                    // mine a Funai block first -- we won't build microblocks
                    self.mined_funai_block = false;
                }
            } else {
                // mine a Funai block first -- we didn't win
                self.mined_funai_block = false;
            }
            signal_mining_ready(self.globals.get_miner_status());
        }
    ...
    pub fn rotate_vrf_and_register(&mut self, burn_block: &BlockSnapshot) {
        ...
        let cur_epoch =
            SortitionDB::get_funai_epoch(self.sortdb_ref().conn(), burn_block.block_height)
                .expect("FATAL: failed to query sortition DB")
                .expect("FATAL: no epoch defined")
                .epoch_id;
    ...
    fn clear_stale_mined_blocks(burn_height: u64, last_mined_blocks: MinedBlocks) -> MinedBlocks {
        ...
        for (funai_bhh, (assembled_block, microblock_privkey)) in last_mined_blocks.into_iter() {
            if assembled_block.my_block_height < burn_height {
                debug!(
                    "Stale mined block: {} (as of {},{})",
                    &funai_bhh, &assembled_block.my_burn_hash, assembled_block.my_block_height
                );
                continue;
            }
            debug!(
            ...
            ret.insert(funai_bhh, (assembled_block, microblock_privkey));
        }
        ret
    }
    ...
    /// * there are no unprocessed funai blocks in the staging DB
    ...
    fn create_block_miner(
        ...
        {
            debug!(
                "Relayer: miner is blocked as of {}; cannot mine Funai block at this time",
    ...
    pub fn block_miner_thread_try_start(
        ...
        if !self.config.node.mock_mining {
            ...
            // anchored blocks
            if self.mined_funai_block && self.config.node.mine_microblocks {
                debug!("Relayer: mined a Funai block already; waiting for microblock miner");
                return false;
            }
        }
    ...
    fn can_run_microblock_tenure(&mut self) -> bool {
        ...
        if !self.mined_funai_block {
            // have not tried to mine a funai block yet that confirms previously-mined unconfirmed
            // state (or have not tried to mine a new Funai block yet for this active tenure);
    ...
    /// * we must have won the sortition on the funai chain tip
    ...
    pub fn microblock_miner_thread_try_start(&mut self) -> bool {
        ...
        debug!(
            "Relayer: mined Funai block {}/{} so can mine microblocks",
        ...
        if let Some(miner_result) = last_mined_block_opt {
            match miner_result {
                MinerThreadResult::Block(
                    ...
                    debug!(
                        "Relayer: RunTenure thread joined; got Funai block {}",
                    ...
                    // this funai block confirms all in-flight microblocks we know about,
                ...
                MinerThreadResult::Microblock(microblock_result, miner_tip) => {
                    ...
                    match microblock_result {
                        Ok(Some((next_microblock, new_cost))) => {
                            ...
                            // have not yet mined a funai block that confirms this microblock, so
...
impl ParentFunaiBlockInfo {
    ...
    pub fn lookup(
        ...
        miner_address: FunaiAddress,
        mine_tip_ch: &ConsensusHash,
        mine_tip_bh: &BlockHeaderHash,
    ) -> Result<ParentFunaiBlockInfo, Error> {
        let funai_tip_header = FunaiChainState::get_anchored_block_header_info(
            chain_state.db(),
            &mine_tip_ch,
            &mine_tip_bh,
        )
        ...
        .ok_or_else(|| {
            error!(
                ...
                "tip_funai_block_hash" => %mine_tip_bh
            );
            Error::HeaderNotFoundForChainTip
        })?;

        // the funai block I'm mining off of's burn header hash and vtxindex:
        let parent_snapshot =
        ...
        debug!("Mining tenure's last consensus hash: {} (height {} hash {}), funai tip consensus hash: {} (height {} hash {})",
        ...
        let coinbase_nonce = {
            ...
            let account = chain_state
                .with_read_only_clarity_tx(
                    &burn_db.index_conn(),
                    &FunaiBlockHeader::make_index_block_hash(mine_tip_ch, mine_tip_bh),
                    |conn| FunaiChainState::get_account(conn, &principal),
                )
                .unwrap_or_else(|| {
                    panic!(
                        "BUG: funai tip block {}/{} no longer exists after we queried it",
        ...
        Ok(ParentFunaiBlockInfo {
            funai_parent_header: funai_tip_header,
...
impl PeerThread {
    ...
    fn with_chainstate<F, R>(&mut self, func: F) -> R
    where
        F: FnOnce(&mut PeerThread, &mut SortitionDB, &mut FunaiChainState, &mut MemPoolDB) -> R,
    {
        ...
        let p2p_res = self.with_chainstate(|p2p_thread, sortdb, chainstate, mempool| {
            ...
            let handler_args = RPCHandlerArgs {
                exit_at_block_height: p2p_thread
                    ...
                    .clone(),
                genesis_chainstate_hash: Sha256Sum::from_hex(funai_genesis::GENESIS_CHAINSTATE_HASH)
                    .unwrap(),
...
impl FunaiNode {
    /// Create a FunaiPrivateKey from a given seed buffer
    pub fn make_node_private_key_from_seed(seed: &[u8]) -> FunaiPrivateKey {
        let node_privkey = {
    ...
    fn setup_peer_db(
        config: &Config,
        burnchain: &Burnchain,
        funaidb_contract_ids: &[QualifiedContractIdentifier],
    ) -> PeerDB {
        ...
        // update services to indicate we can support mempool sync and funaidb
        {
            let mut tx = peerdb.tx_begin().unwrap();
    ...
    pub(crate) fn setup_peer_network(
        ...
        let epochs = SortitionDB::get_funai_epochs(sortdb.conn())
            .expect("Error while loading funai epochs");
        ...
        let mut funaidb_machines = HashMap::new();
        let mut funaidbs = FunaiDBs::connect(&config.get_funai_db_file_path(), true).unwrap();

        let mut funaidb_configs = HashMap::new();
        for contract in config.node.funai_dbs.iter() {
            funaidb_configs.insert(contract.clone(), FunaiDBConfig::noop());
        }
        let funaidb_configs = funaidbs
            .create_or_reconfigure_funaidbs(&mut chainstate, &sortdb, funaidb_configs)
            .unwrap();

        let funaidb_contract_ids: Vec<QualifiedContractIdentifier> =
            funaidb_configs.keys().cloned().collect();
        for (contract_id, funaidb_config) in funaidb_configs {
            let funaidbs = FunaiDBs::connect(&config.get_funai_db_file_path(), true).unwrap();
            let funai_db_sync = FunaiDBSync::new(
                contract_id.clone(),
                &funaidb_config,
                PeerNetworkComms::new(),
                funaidbs,
            );
            funaidb_machines.insert(contract_id, (funaidb_config, funai_db_sync));
        }
        let peerdb = Self::setup_peer_db(config, &burnchain, &funaidb_contract_ids);
    ...
    fn set_monitoring_miner_address(keychain: &Keychain, relayer_thread: &RelayerThread) {
        ...
        let miner_addr = relayer_thread
            .bitcoin_controller
            .get_miner_address(FunaiEpochId::Epoch21, &public_key);
    ...
    pub fn spawn(
        ...
        let funaidbs = FunaiDBs::connect(&config.get_funai_db_file_path(), true)
            .expect("FATAL: failed to connect to funai DB");

        let relayer = Relayer::from_p2p(&mut p2p_net, funaidbs);
        ...
        FunaiNode::set_monitoring_miner_address(&keychain, &relayer_thread);
    ...
    pub fn relayer_sortition_notify(&self) -> bool {
        ...
        if let Some(snapshot) = self.globals.get_last_sortition() {
            debug!(
                ...
                "winning_funai_block_hash" => %snapshot.winning_funai_block_hash,
            ...
            if snapshot.sortition {
                return self
                    ...
                    .send(RelayerDirective::ProcessTenure(
                        ...
                        snapshot.winning_funai_block_hash.clone(),
                    ))
                    .is_ok();
            }
        } else {
