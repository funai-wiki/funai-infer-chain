...
use funai::chainstate::burn::operations::BlockstackOperationType;
use funai::chainstate::burn::ConsensusHash;
use funai::chainstate::coordinator::BlockEventDispatcher;
use funai::chainstate::nakamoto::NakamotoBlock;
...
use funai::chainstate::funai::db::accounts::MinerReward;
use funai::chainstate::funai::db::unconfirmed::ProcessedUnconfirmedState;
use funai::chainstate::funai::db::{MinerRewardInfo, FunaiBlockHeaderTypes, FunaiHeaderInfo};
use funai::chainstate::funai::events::{
    FunaiDBChunksEvent, FunaiBlockEventData, FunaiTransactionEvent, FunaiTransactionReceipt,
    TransactionOrigin,
};
use funai::chainstate::funai::miner::TransactionEvent;
use funai::chainstate::funai::{
    FunaiBlock, FunaiMicroblock, FunaiTransaction, TransactionPayload,
};
use funai::core::mempool::{MemPoolDropReason, MemPoolEventDispatcher, ProposalCallbackReceiver};
use funai::libfunaidb::FunaiDBChunkData;
use funai::net::api::postblock_proposal::{
    BlockValidateOk, BlockValidateReject, BlockValidateResponse,
};
use funai::net::atlas::{Attachment, AttachmentInstance};
use funai::net::funaidb::FunaiDBEventDispatcher;
use funai::util::hash::to_hex;
...
use funai_common::util::hash::{bytes_to_hex, Sha512Trunc256Sum};
...
pub const PATH_STACKERDB_CHUNKS: &str = "funaidb_chunks";
...
pub static STACKER_DB_CHANNEL: FunaiDBChannel = FunaiDBChannel::new();

/// This struct receives FunaiDB event callbacks without registering
...
/// FunaiDB events. It could either poll the database (seems like a
/// bad idea) or listen for events. Registering for RPC callbacks
/// seems bad. So instead, it uses a singleton sync channel.
pub struct FunaiDBChannel {
    sender_info: Mutex<Option<InnerFunaiDBChannel>>,
}

#[derive(Clone)]
struct InnerFunaiDBChannel {
    /// A channel for sending the chunk events to the listener
    sender: Sender<FunaiDBChunksEvent>,
    /// Does the listener want to receive `.signers` chunks?
    interested_in_signers: bool,
    /// Which FunaiDB contracts is the listener interested in?
...
impl InnerFunaiDBChannel {
    pub fn new_miner_receiver() -> (Receiver<FunaiDBChunksEvent>, Self) {
...
impl FunaiDBChannel {
    ...
    /// Consume the receiver for the FunaiDBChannel and drop the senders. This should be done
    ...
    /// open).
    ///
    /// The FunaiDBChnnel's receiver is guarded with a Mutex, so that ownership can
    /// be taken by different threads without unsafety.
    pub fn replace_receiver(&self, receiver: Receiver<FunaiDBChunksEvent>) {
        ...
        let mut guard = self
            .sender_info
            .lock()
            .expect("FATAL: poisoned FunaiDBChannel lock");
        guard.take();
    }

    /// Create a new event receiver channel for receiving events relevant to the miner coordinator,
    /// dropping the old FunaiDB event sender channels if they are still registered.
    ///  Returns the new receiver channel and a bool indicating whether or not sender channels were
    ///   still in place.
    ///
    /// The FunaiDBChannel senders are guarded by mutexes so that they can be replaced
    /// by different threads without unsafety.
    pub fn register_miner_coordinator(&self) -> (Receiver<FunaiDBChunksEvent>, bool) {
        let mut sender_info = self
            .sender_info
            .lock()
            .expect("FATAL: poisoned FunaiDBChannel lock");
        let (recv, new_sender) = InnerFunaiDBChannel::new_miner_receiver();
        let replaced_receiver = sender_info.replace(new_sender).is_some();

        (recv, replaced_receiver)
    }

    /// Is there a thread holding the receiver, and is it interested in chunks events from `funaidb`?
    /// Returns the a sending channel to broadcast the event to if so, and `None` if not.
    pub fn is_active(
        &self,
        funaidb: &QualifiedContractIdentifier,
    ) -> Option<Sender<FunaiDBChunksEvent>> {
        ...
        if sender_info.interested_in_signers
            && funaidb.is_boot()
            && funaidb.name.starts_with(SIGNERS_NAME)
        {
            return Some(sender_info.sender.clone());
        }
        if sender_info.other_interests.contains(funaidb) {
            return Some(sender_info.sender.clone());
        }
        None
    }
...
impl EventObserver {
    ...
    fn make_new_mempool_txs_payload(transactions: Vec<FunaiTransaction>) -> serde_json::Value {
    ...
    fn generate_payload_info_for_receipt(receipt: &FunaiTransactionReceipt) -> ReceiptPayloadInfo {
        ...
        let success = match (receipt.post_condition_aborted, &receipt.result) {
            ...
            _ => {
                if let TransactionOrigin::Funai(inner_tx) = &tx {
        ...
        let (txid, raw_tx, burnchain_op_json) = match tx {
            ...
            TransactionOrigin::Funai(ref tx) => {
    ...
    fn make_new_attachment_payload(
        ...
        json!({
            ...
            "block_height": attachment.0.funai_block_height,
    ...
    fn send_new_microblocks(
        ...
        filtered_events: Vec<(usize, &(bool, Txid, &FunaiTransactionEvent))>,
    ...
    fn send_funaidb_chunks(&self, payload: &serde_json::Value) {
    ...
    fn make_new_block_processed_payload(
        ...
        let mut payload = json!({
            ...
            "block_height": metadata.funai_block_height,
        ...
        if let FunaiBlockHeaderTypes::Nakamoto(ref header) = &metadata.anchored_header {
...
pub struct EventDispatcher {
    ...
    funaidb_observers_lookup: HashSet<u16>,
...
impl FunaiDBEventDispatcher for EventDispatcher {
    /// Relay new FunaiDB chunks
    fn new_funaidb_chunks(
        &self,
        contract_id: QualifiedContractIdentifier,
        chunks: Vec<FunaiDBChunkData>,
        miner_endpoint: Option<String>,
    ) {
        self.process_new_funaidb_chunks(contract_id, chunks, miner_endpoint)
    }
}
...
impl EventDispatcher {
    pub fn new() -> EventDispatcher {
        EventDispatcher {
            ...
            funaidb_observers_lookup: HashSet::new(),
    ...
    fn create_dispatch_matrix_and_event_vector<'a>(
        &self,
        receipts: &'a Vec<FunaiTransactionReceipt>,
    ) -> (
        Vec<HashSet<usize>>,
        Vec<(bool, Txid, &'a FunaiTransactionEvent)>,
    ) {
        let mut dispatch_matrix: Vec<HashSet<usize>> = self
            ...
            .map(|_| HashSet::new())
            .collect();
        let mut events: Vec<(bool, Txid, &FunaiTransactionEvent)> = vec![];
        let mut i: usize = 0;
        ...
        for receipt in receipts {
            ...
            for event in receipt.events.iter() {
                match event {
                    FunaiTransactionEvent::SmartContractEvent(event_data) => {
                    ...
                    FunaiTransactionEvent::STXEvent(STXEventType::STXTransferEvent(_))
                    | FunaiTransactionEvent::STXEvent(STXEventType::STXInferEvent(_))
                    | FunaiTransactionEvent::STXEvent(STXEventType::STXMintEvent(_))
                    | FunaiTransactionEvent::STXEvent(STXEventType::STXBurnEvent(_))
                    | FunaiTransactionEvent::STXEvent(STXEventType::STXLockEvent(_)) => {
                        for o_i in &self.stx_observers_lookup {
                            dispatch_matrix[*o_i as usize].insert(i);
                        }
                    }
                    FunaiTransactionEvent::NFTEvent(NFTEventType::NFTTransferEvent(
                        event_data,
                    )) => {
                    ...
                    FunaiTransactionEvent::NFTEvent(NFTEventType::NFTMintEvent(event_data)) => {
                        self.update_dispatch_matrix_if_observer_subscribed(
                            &event_data.asset_identifier,
                            i,
                            &mut dispatch_matrix,
                    ...
                    FunaiTransactionEvent::NFTEvent(NFTEventType::NFTBurnEvent(event_data)) => {
                        self.update_dispatch_matrix_if_observer_subscribed(
                            &event_data.asset_identifier,
                            i,
                            &mut dispatch_matrix,
                    ...
                    FunaiTransactionEvent::FTEvent(FTEventType::FTTransferEvent(event_data)) => {
                        self.update_dispatch_matrix_if_observer_subscribed(
                            &event_data.asset_identifier,
                            i,
                            &mut dispatch_matrix,
                    ...
                    FunaiTransactionEvent::FTEvent(FTEventType::FTMintEvent(event_data)) => {
                        self.update_dispatch_matrix_if_observer_subscribed(
                            &event_data.asset_identifier,
                            i,
                            &mut dispatch_matrix,
                    ...
                    FunaiTransactionEvent::FTEvent(FTEventType::FTBurnEvent(event_data)) => {
                        self.update_dispatch_matrix_if_observer_subscribed(
                            &event_data.asset_identifier,
                            i,
                            &mut dispatch_matrix,
                ...
                mature_rewards
                    ...
                    .map(|reward| {
                        json!({
                            ...
                            "from_funai_block_hash": format!("0x{}", rewards_info.from_funai_block_hash),
                            "from_index_consensus_hash": format!("0x{}", FunaiBlockId::new(&rewards_info.from_block_consensus_hash,
                                                                                            &rewards_info.from_funai_block_hash)),
                        })
                    })
                    .collect()
            } else {
    ...
    pub fn process_new_mempool_txs(&self, txs: Vec<FunaiTransaction>) {
        // lazily assemble payload only if we have observers
    ...
    pub fn process_mined_nakamoto_block_event(
        ...
        let payload = serde_json::to_value(MinedNakamotoBlockEvent {
            ...
            funai_height: block.header.chain_length,
            block_size: block_size_bytes,
            cost: consumed.clone(),
            tx_events,
    ...
    /// Forward newly-accepted FunaiDB chunk metadata to downstream `funaidb` observers.
    /// Infallible.
    pub fn process_new_funaidb_chunks(
        &self,
        contract_id: QualifiedContractIdentifier,
        modified_slots: Vec<FunaiDBChunkData>,
        miner_endpoint: Option<String>,
    ) {
        let interested_observers = self.filter_observers(&self.funaidb_observers_lookup, false);
        ...
        let event = FunaiDBChunksEvent {
            contract_id,
            modified_slots,
            miner_endpoint,
        };
        let payload = serde_json::to_value(&event)
            .expect("FATAL: failed to serialize FunaiDBChunksEvent to JSON");

        if let Some(channel) = interested_receiver {
            if let Err(send_err) = channel.send(event) {
                warn!(
                    "Failed to send FunaiDB event to WSTS coordinator channel. Miner thread may have exited.";
                    "err" => ?send_err
                );
            }
        }
        ...
        for observer in interested_observers.iter() {
            observer.send_funaidb_chunks(&payload);
        }
    }
    ...
    pub fn register_observer(&mut self, conf: &EventObserverConfig) {
        ...
        for event_key_type in conf.events_keys.iter() {
            match event_key_type {
                ...
                EventKeyType::FunaiDBChunks => {
                    self.funaidb_observers_lookup.insert(observer_index);
                }
...
mod test {
    use clarity::vm::costs::ExecutionCost;
    use funai::burnchains::{PoxConstants, Txid};
    use funai::chainstate::funai::db::FunaiHeaderInfo;
    use funai::chainstate::funai::FunaiBlock;
    use funai_common::bitvec::BitVec;
    use funai_common::types::chainstate::{BurnchainHeaderHash, FunaiBlockId};
    ...
    fn build_block_processed_event() {
        ...
        let block = FunaiBlock::genesis_block();
        let metadata = FunaiHeaderInfo::regtest_genesis();
        let receipts = vec![];
        let parent_index_hash = FunaiBlockId([0; 32]);
