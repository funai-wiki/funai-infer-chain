...
use funai::chainstate::burn::db::sortdb::SortitionDBConn;
use funai::chainstate::funai::db::FunaiChainState;
use funai::chainstate::funai::miner::BlockBuilderSettings;
use funai::chainstate::funai::{
    FunaiBlock, FunaiBlockBuilder, FunaiMicroblock, FunaiPrivateKey, FunaiPublicKey,
    FunaiTransaction,
};
use funai::core::mempool::MemPoolDB;
use funai_common::types::chainstate::VRFSeed;
...
impl<'a> Tenure {
    pub fn new(
        ...
        config: Config,
        mem_pool: MemPoolDB,
        microblock_secret_key: FunaiPrivateKey,
        burnchain_tip: BurnchainTip,
        vrf_proof: VRFProof,
        burn_fee_cap: u64,
    ) -> Tenure {
        let mut microblock_pubkey = FunaiPublicKey::from_private(&microblock_secret_key);
    ...
    pub fn run(&mut self, burn_dbconn: &SortitionDBConn) -> Option<TenureArtifacts> {
        ...
        let (mut chain_state, _) = FunaiChainState::open(
        ...
        let (anchored_block, _, _) = FunaiBlockBuilder::build_anchored_block(
            &mut chain_state,
            burn_dbconn,
            &mut self.mem_pool,
    ...
    #[cfg(test)]
    pub fn open_chainstate(&self) -> FunaiChainState {
        use funai::core::CHAIN_ID_TESTNET;

        let (chain_state, _) = FunaiChainState::open(
            false,
            CHAIN_ID_TESTNET,
            &self.config.get_chainstate_path_str(),
            Some(self.config.node.get_marf_opts()),
