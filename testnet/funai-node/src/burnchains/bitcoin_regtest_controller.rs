...
use funai::burnchains::bitcoin::indexer::{
    BitcoinIndexer, BitcoinIndexerConfig, BitcoinIndexerRuntime,
};
use funai::burnchains::bitcoin::spv::SpvClient;
use funai::burnchains::bitcoin::BitcoinNetworkType;
use funai::burnchains::db::BurnchainDB;
use funai::burnchains::indexer::BurnchainIndexer;
use funai::burnchains::{
...
#[cfg(test)]
use funai::chainstate::burn::Opcodes;
use funai::chainstate::coordinator::comm::CoordinatorChannels;
#[cfg(test)]
use funai::chainstate::funai::address::PoxAddress;
use funai::core::{FunaiEpoch, FunaiEpochId};
use funai::monitoring::{increment_btc_blocks_received_counter, increment_btc_ops_sent_counter};
use funai_common::codec::FunaiMessageCodec;
use funai_common::deps_common::bitcoin::blockdata::opcodes;
use funai_common::deps_common::bitcoin::blockdata::script::{Builder, Script};
use funai_common::deps_common::bitcoin::blockdata::transaction::{
    OutPoint, Transaction, TxIn, TxOut,
};
use funai_common::deps_common::bitcoin::network::encodable::ConsensusEncodable;
#[cfg(test)]
use funai_common::deps_common::bitcoin::network::serialize::deserialize as btc_deserialize;
use funai_common::deps_common::bitcoin::network::serialize::RawEncoder;
...
impl BitcoinRegtestController {
    ...
    pub fn get_all_utxos(&self, public_key: &Secp256k1PublicKey) -> Vec<UTXO> {
        ...
        let address = self.get_miner_address(FunaiEpochId::Epoch21, public_key);
    ...
    pub fn get_utxos(
        ...
        let pubk = if self.config.miner.segwit && epoch_id >= FunaiEpochId::Epoch21 {
            let mut p = public_key.clone();
            p.set_compressed(true);
            p
        } else {
    ...
    pub fn submit_manual(
        ...
        let transaction = match operation {
            ...
            BlockstackOperationType::PreStx(payload) => {
                self.build_pre_funai_tx(epoch_id, payload, op_signer)
            }
            BlockstackOperationType::TransferStx(payload) => {
                self.build_transfer_funai_tx(epoch_id, payload, op_signer, utxo)
            }
        }?;
    ...
    fn build_transfer_funai_tx(
        ...
        info!(
            "Miner node: submitting funai transfer op - {}",
            public_key.to_hex()
        );
    ...
    fn build_delegate_funai_tx(
        ...
        info!(
            "Miner node: submitting funai delegate op - {}",
            public_key.to_hex()
        );
    ...
    fn build_pre_funai_tx(
        ...
        info!(
            "Miner node: submitting pre_funai op - {}",
            public_key.to_hex()
        );
    ...
    pub(crate) fn get_miner_address(
        ...
        if self.config.miner.segwit && epoch_id >= FunaiEpochId::Epoch21 {
    ...
    fn serialize_tx(
        ...
        if value >= DUST_UTXO_LIMIT {
            let change_output = if self.config.miner.segwit && epoch_id >= FunaiEpochId::Epoch21 {
                // p2wpkh
    ...
    pub fn wait_for_sortitions(
        ...
        loop {
            ...
            coord_comms.announce_new_funai_block();
    ...
    pub fn make_operation_tx(
        ...
        let transaction = match operation {
            ...
            BlockstackOperationType::TransferStx(payload) => {
                self.build_transfer_funai_tx(epoch_id, payload, op_signer, None)
            }
            ...
            BlockstackOperationType::DelegateStx(payload) => {
                self.build_delegate_funai_tx(epoch_id, payload, op_signer, None)
            }
...
impl BurnchainController for BitcoinRegtestController {
    ...
    fn connect_dbs(&mut self) -> Result<(), BurnchainControllerError> {
        ...
        burnchain.connect_db(
            ...
            self.indexer.get_funai_epochs(),
        )?;
        Ok(())
    }

    fn get_funai_epochs(&self) -> Vec<FunaiEpoch> {
        self.indexer.get_funai_epochs()
    }
    ...
    fn bootstrap_chain(&mut self, num_blocks: u64) {
        if let Some(ref local_mining_pubkey) = &self.config.burnchain.local_mining_public_key {
            ...
            let address = self.get_miner_address(FunaiEpochId::Epoch21, &local_mining_pubkey);
