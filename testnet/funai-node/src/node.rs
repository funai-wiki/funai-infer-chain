...
use funai::chainstate::funai::db::{
    ChainStateBootData, ChainstateAccountBalance, ChainstateAccountLockup, ChainstateBNSName,
    ChainstateBNSNamespace, ClarityTx, FunaiChainState, FunaiEpochReceipt, FunaiHeaderInfo,
};
use funai::chainstate::funai::events::{
    FunaiTransactionEvent, FunaiTransactionReceipt, TransactionOrigin,
};
use funai::chainstate::funai::{
    CoinbasePayload, FunaiBlock, FunaiMicroblock, FunaiTransaction, FunaiTransactionSigner,
    TransactionAnchorMode, TransactionPayload, TransactionVersion,
};
use funai::core::mempool::MemPoolDB;
use funai::core::{STACKS_EPOCH_2_1_MARKER, STACKS_EPOCH_3_0_MARKER};
use funai::cost_estimates::metrics::UnitMetric;
use funai::cost_estimates::UnitEstimator;
use funai::net::atlas::{AtlasConfig, AtlasDB, AttachmentInstance};
use funai::net::db::PeerDB;
use funai::net::p2p::PeerNetwork;
use funai::net::funaidb::FunaiDBs;
use funai::net::{Error as NetError, RPCHandlerArgs};
use funai::util_lib::strings::UrlString;
use funai_common::types::chainstate::{BlockHeaderHash, BurnchainHeaderHash, TrieHash, VRFSeed};
...
impl ChainTip {
    pub fn genesis(
        ...
        ChainTip {
            metadata: FunaiHeaderInfo::genesis(
                TrieHash([0u8; 32]),
                first_burnchain_block_hash,
                first_burnchain_block_height as u32,
                first_burnchain_block_timestamp,
            ),
            block: FunaiBlock::genesis_block(),
            receipts: vec![],
        }
    }
}

/// Node is a structure modelising an active node working on the funai chain.
pub struct Node {
    pub chain_state: FunaiChainState,
    pub config: Config,
...
pub fn get_account_lockups(
    ...
    Box::new(
        funai_genesis::GenesisData::new(use_test_chainstate_data)
            .read_lockups()
...
fn spawn_peer(
    ...
    let server_thread = thread::spawn(move || {
        ...
        loop {
            ...
            let (mut chainstate, _) = match FunaiChainState::open(
                is_mainnet,
                chain_id,
...
impl Node {
    ...
    pub fn new(config: Config, boot_block_exec: Box<dyn FnOnce(&mut ClarityTx) -> ()>) -> Self {
        ...
        let chain_state_result = FunaiChainState::open_and_exec(
            config.is_mainnet(),
    ...
    pub fn spawn_peer_server(&mut self) {
        ...
        let funaidbs =
            FunaiDBs::connect(&self.config.get_funai_db_file_path(), true).unwrap();
        ...
        let _join_handle = spawn_peer(
            ...
            Sha256Sum::from_hex(funai_genesis::GENESIS_CHAINSTATE_HASH).unwrap(),
            1000,
            self.config.clone(),
        )
        .unwrap();
    ...
    pub fn setup(&mut self, burnchain_controller: &mut Box<dyn BurnchainController>) {
        ...
        let cur_epoch =
            SortitionDB::get_funai_epoch(sortdb.conn(), burnchain_tip.block_snapshot.block_height)
                .expect("FATAL: failed to read sortition DB")
    ...
    pub fn commit_artifacts(
        &mut self,
        anchored_block_from_ongoing_tenure: &FunaiBlock,
        burnchain_tip: &BurnchainTip,
    ...
    pub fn process_tenure(
        ...
        let _parent_consensus_hash = {
            ...
            let parent_consensus_hash = FunaiChainState::get_parent_consensus_hash(
                &ic,
        ...
        let funai_epoch = db
            .index_conn()
            .get_funai_epoch_by_epoch_id(&processed_block.evaluated_epoch)
        ...
        if let Some(estimator) = fee_estimator.as_mut() {
            if let Err(e) = estimator.notify_block(&processed_block, &funai_epoch.block_limit) {
                warn!("FeeEstimator failed to process block receipt";
                      "funai_block" => %processed_block.header.anchored_header.block_hash(),
                      "funai_height" => %processed_block.header.funai_block_height,
                      "error" => %e);
            }
        }
        ...
        let block: FunaiBlock = {
            let block_path = FunaiChainState::get_block_path(
                &self.chain_state.blocks_path,
                &metadata.consensus_hash,
                &metadata.anchored_header.block_hash(),
            )
            .unwrap();
            FunaiChainState::consensus_load(&block_path).unwrap()
        };
    ...
    pub fn get_attachment_instances(
        &self,
        epoch_receipt: &FunaiEpochReceipt,
                        ...
                        for event in receipt.events.iter() {
                                ...
                                event
                            {
                                let res = AttachmentInstance::try_new_from_value(
                                    ...
                                    epoch_receipt.header.funai_block_height,
                                    receipt.transaction.txid(),
                                    self.chain_tip
                                        .as_ref()
                                        .map(|t| t.metadata.funai_block_height),
                                );
    ...
    fn generate_coinbase_tx(&mut self, is_mainnet: bool) -> FunaiTransaction {
