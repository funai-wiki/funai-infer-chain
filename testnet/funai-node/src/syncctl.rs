...
/// Monitor the state of the Funai blockchain as the peer network and relay threads download and
/// proces Funai blocks.  Don't allow the node to process the next PoX reward cycle's sortitions
/// unless it's reasonably sure that it has processed all Funai blocks for this reward cycle.
/// This struct monitors the Funai chainstate to make this determination.
...
impl PoxSyncWatchdog {
    pub fn new(
        ...
        let (chainstate, _) =
            match FunaiChainState::open(mainnet, chain_id, &chainstate_path, Some(marf_opts)) {
                Ok(cs) => cs,
                Err(e) => {
                    return Err(format!(
    ...
    /// How many recently-added Funai blocks are in an attachable state, up to $max_staging?
    fn count_attachable_funai_blocks(&mut self) -> Result<u64, String> {
        // number of staging blocks that have arrived since the last sortition
        let cnt = FunaiChainState::count_attachable_staging_blocks(
            &self.chainstate.db(),
            self.max_staging,
    ...
    /// How many recently-processed Funai blocks are there, up to $max_staging?
    ...
    fn count_processed_funai_blocks(&mut self) -> Result<u64, String> {
        // number of staging blocks that have arrived since the last sortition
        let cnt = FunaiChainState::count_processed_staging_blocks(
            &self.chainstate.db(),
            self.max_staging,
    ...
    fn estimate_block_process_time(
        ...
        let block_wait_times =
            FunaiChainState::measure_block_wait_time(&chainstate.db(), start_height, end_height)
    ...
    fn estimate_block_download_time(
        ...
        let block_download_times = FunaiChainState::measure_block_download_time(
            &chainstate.db(),
            start_height,
            end_height,
        )
    ...
    /// Wait until all of the Funai blocks for the given reward cycle are seemingly downloaded and
    /// processed.  Do so by watching the _rate_ at which attachable Funai blocks arrive and get
    /// processed.
    ...
    pub fn pox_sync_wait(
        ...
        let ibbd = loop {
            ...
            match (
                self.count_attachable_funai_blocks(),
                self.count_processed_funai_blocks(),
            ) {
                (Ok(num_available), Ok(num_processed)) => {
                        ...
                        && get_epoch_time_secs() < expected_first_block_deadline
                    {
                        ...
                        debug!("PoX watchdog: Still warming up: waiting until {}s for first Funai block download (estimated download time: {}s)...", expected_first_block_deadline, self.estimated_block_download_time);
                        sleep_ms(PER_SAMPLE_WAIT_MS);
                        continue;
                    }
