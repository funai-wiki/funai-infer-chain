...
impl PeerThread {
    ...
    /// Check if the FunaiDB config needs to be updated (by looking
    ///  at the signal in `self.globals`), and if so, refresh the
    ///  FunaiDB config
    fn refresh_funaidb(&mut self) {
        if !self.globals.coord_comms.need_funaidb_update() {
            return;
        }

        if let Err(e) = self
            .net
            .refresh_funai_db_configs(&self.sortdb, &mut self.chainstate)
        {
            warn!("Failed to update FunaiDB configs: {e}");
        }

        self.globals.coord_comms.set_funaidb_update(false);
    }
