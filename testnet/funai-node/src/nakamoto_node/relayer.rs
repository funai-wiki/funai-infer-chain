...
use funai::chainstate::nakamoto::coordinator::get_nakamoto_next_recipients;
...
use funai::monitoring::increment_stx_blocks_mined_counter;
use funai::net::db::LocalPeer;
use funai::net::relay::Relayer;
use funai::net::NetworkResult;
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, FunaiBlockId, VRFSeed,
};
...
pub struct RelayerThread {
    ...
    last_committed: Option<(BlockSnapshot, FunaiBlockId)>,
}
...
impl RelayerThread {
    ...
    pub fn process_sortition(
        ...
        committed_index_hash: FunaiBlockId,
    ) -> MinerDirective {
    ...
    pub fn rotate_vrf_and_register(&mut self, burn_block: &BlockSnapshot) {
        ...
        let cur_epoch = SortitionDB::get_funai_epoch(self.sortdb.conn(), burn_block.block_height)
            .expect("FATAL: failed to query sortition DB")
    ...
    fn make_block_commit(
    ...
    ) -> Result<(BlockSnapshot, FunaiEpochId, LeaderBlockCommitOp), NakamotoNodeError> {
        ...
        if parent_block_id != FunaiBlockId::new(target_ch, target_bh) {
            error!("Relayer: Found block header for parent tenure, but mismatched block id";
                   "expected_block_id" => %FunaiBlockId::new(target_ch, target_bh),
        ...
        let Ok(Some(target_epoch)) =
            SortitionDB::get_funai_epoch(self.sortdb.conn(), sort_tip.block_height + 1)
        else {
    ...
    fn issue_block_commit(
        ...
        self.last_committed = Some((
            last_committed_at,
            FunaiBlockId::new(&tenure_start_ch, &tenure_start_bh),
        ));
        ...
        let continue_running = match directive {
            ...
            RelayerDirective::ProcessedBurnBlock(consensus_hash, burn_hash, block_header_hash) => {
                ...
                self.handle_sortition(
                    consensus_hash,
                    burn_hash,
                    FunaiBlockId(block_header_hash.0),
                )
            }
