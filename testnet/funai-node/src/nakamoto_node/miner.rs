...
use funai::chainstate::funai::db::{FunaiChainState, FunaiHeaderInfo};
use funai::chainstate::funai::{
    CoinbasePayload, Error as ChainstateError, FunaiTransaction, FunaiTransactionSigner,
...
pub enum MinerDirective {
    ...
    BeginTenure {
        parent_tenure_start: FunaiBlockId,
...
impl BlockMinerThread {
    ...
    pub fn run_miner(mut self, prior_miner: Option<JoinHandle<()>>) {
        ...
        let mut funaidbs = FunaiDBs::connect(&self.config.get_funai_db_file_path(), true)
        ...
        loop {
            let new_block = loop {
                match self.mine_block(&funaidbs) {
                    Ok(x) => break Some(x),
            ...
            if let Some(mut new_block) = new_block {
                if let Err(e) = self.propose_block(&new_block, &funaidbs) {
                ...
                if let Err(e) = self.broadcast(new_block.clone(), &aggregate_public_key) {
                    ...
                    info!(
                        ...
                        "funai_block_id" => %new_block.header.block_id(),
                        "block_height" => new_block.header.chain_length,
                        "consensus_hash" => %new_block.header.consensus_hash,
                    );
                    self.globals.coord().announce_new_funai_block();
                }
    ...
    fn coordinate_signature(
        ...
        #[cfg(test)]
        {
            ...
            //  channel has been created. This allows integration tests for the funai-node
            //  independent of the funai-signer.
    ...
    fn propose_block(
        ...
        let mut miners_session =
            FunaiDBSession::new(&rpc_socket.to_string(), miners_contract_id.clone());
        let Some(miner_privkey) = self.config.miner.mining_key else {
            return Err(NakamotoNodeError::MinerConfigurationFailed(
                "No mining key configured, cannot mine",
            ));
        ...
        let proposal = match NakamotoBlockBuilder::make_funaidb_block_proposal(
            &sort_db,
            &tip,
            &funaidbs,
            &proposal_msg,
            ...
            Ok(Some(chunk)) => chunk,
            Ok(None) => {
                warn!("Failed to propose block to funaidb: no slot available");
                return Ok(());
            }
            Err(e) => {
                warn!("Failed to propose block to funaidb: {e:?}");
                return Ok(());
            }
        };

        // Propose the block to the observing signers through the .miners funaidb instance
        match miners_session.put_chunk(&proposal) {
            Ok(ack) => {
                info!(
                    "Proposed block to funaidb";
                    "signer_sighash" => %new_block.header.signer_signature_hash(),
                    "ack_msg" => ?ack,
                );
            }
            Err(e) => {
                return Err(NakamotoNodeError::SigningCoordinatorFailure(format!(
                    "Failed to propose block to funaidb {e:?}"
                )));
            }
        }
    ...
    fn get_funaidb_contract_and_slots(
        &self,
        funaidbs: &FunaiDBs,
        msg_id: &MessageSlotID,
        reward_cycle: u64,
    ) -> Result<(QualifiedContractIdentifier, HashMap<u32, FunaiAddress>), NakamotoNodeError> {
        let funaidb_contracts = funaidbs
            .get_funaidb_contract_ids()
            .expect("FATAL: could not get the funai DB contract ids");

        let signers_contract_id =
            msg_id.funai_db_contract(self.config.is_mainnet(), reward_cycle);
        if !funaidb_contracts.contains(&signers_contract_id) {
        ...
        let signers = funaidbs
            .get_signers(&signers_contract_id)
            .expect("FATAL: could not get signers from funai DB");
        let mut slot_ids_addresses = HashMap::with_capacity(signers.len());
        for (slot_id, address) in funaidbs
            .get_signers(&signers_contract_id)
            .expect("FATAL: could not get signers from funai DB")
            .into_iter()
            .enumerate()
    ...
    fn get_signer_transactions(
        ...
        sortdb: &SortitionDB,
        funaidbs: &FunaiDBs,
    ) -> Result<Vec<FunaiTransaction>, NakamotoNodeError> {
        let next_reward_cycle = self
            ...
            .wrapping_add(1);
        let (signers_contract_id, slot_ids_addresses) = self.get_funaidb_contract_and_slots(
            funaidbs,
        ...
        let signer_chunks = funaidbs
            .get_latest_chunks(&signers_contract_id, &slot_ids)
            .expect("FATAL: could not get latest chunks from funai DB");
            ...
            .with_read_only_clarity_tx(&sortdb.index_conn(), &funai_block_id, |clarity_tx| {
                clarity_tx.with_clarity_db_readonly(|clarity_db| {
                    addresses
                        ...
                        .collect::<HashMap<FunaiAddress, u64>>()
                })
            })
            .unwrap_or_default();
        let mut filtered_transactions: HashMap<FunaiAddress, FunaiTransaction> = HashMap::new();
    ...
    fn get_coinbase_recipient(&self, epoch_id: FunaiEpochId) -> Option<PrincipalData> {
        if epoch_id < FunaiEpochId::Epoch21 && self.config.miner.block_reward_recipient.is_some() {
    ...
    fn generate_tenure_change_tx(
        ...
        parent_tenure_blocks: u64,
        miner_pkh: Hash160,
    ) -> Result<FunaiTransaction, NakamotoNodeError> {
        ...
        let mut tx = FunaiTransaction::new(version, tx_auth, tenure_change_tx_payload);
    ...
    fn load_block_parent_info(
    ...
    ) -> Result<ParentFunaiBlockInfo, NakamotoNodeError> {
        let Some(funai_tip) =
            ...
            return Ok(ParentFunaiBlockInfo {
        ...
        let miner_address = self
            ...
            .unwrap();
        match ParentFunaiBlockInfo::lookup(
            chain_state,
            burn_db,
            &self.burn_block,
            miner_address,
    ...
    fn mine_block(&mut self, funaidbs: &FunaiDBs) -> Result<NakamotoBlock, NakamotoNodeError> {
        ...
        let tenure_start_info = if let Some(ref par_tenure_info) = parent_block_info.parent_tenure {
            let parent_block_id = parent_block_info.funai_parent_header.index_block_hash();
        ...
        parent_block_info.funai_parent_header.microblock_tail = None;
        ...
        let signer_transactions =
            self.get_signer_transactions(&mut chain_state, &burn_db, &funaidbs)?;
...
impl ParentFunaiBlockInfo {
    ...
    pub fn lookup(
        ...
        parent_tenure_id: &FunaiBlockId,
        funai_tip_header: FunaiHeaderInfo,
    ) -> Result<ParentFunaiBlockInfo, NakamotoNodeError> {
        ...
        let parent_snapshot = SortitionDB::get_block_snapshot_consensus(
            burn_db.conn(),
            &funai_tip_header.consensus_hash,
        )
        ...
        let parent_tenure_info = if funai_tip_header.consensus_hash
            == parent_tenure_header.consensus_hash
        {
                ...
                .is_some()
            {
                ...
                // the last known tenure block of our parent should be the funai_tip. if not, error.
                if funai_tip_header.index_block_hash()
                    != last_parent_tenure_header.index_block_hash()
                {
                    return Err(NakamotoNodeError::NewParentDiscovered);
                }
                1 + last_parent_tenure_header.funai_block_height
                    - parent_tenure_header.funai_block_height
            } else {
                1
            };
        ...
        debug!(
            ...
            "funai_tip_consensus_hash" => %parent_snapshot.consensus_hash,
            "funai_tip_burn_hash" => %parent_snapshot.burn_header_hash,
            "funai_tip_burn_height" => parent_snapshot.block_height,
        );
        ...
        let coinbase_nonce = {
            ...
            let account = chain_state
                .with_read_only_clarity_tx(
                    &burn_db.index_conn(),
                    &funai_tip_header.index_block_hash(),
