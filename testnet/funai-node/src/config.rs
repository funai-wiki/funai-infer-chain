...
use funai::burnchains::affirmation::AffirmationMap;
use funai::burnchains::bitcoin::BitcoinNetworkType;
use funai::burnchains::{Burnchain, MagicBytes, PoxConstants, BLOCKSTACK_MAGIC_MAINNET};
use funai::chainstate::nakamoto::signer_set::NakamotoSigners;
use funai::chainstate::funai::boot::MINERS_NAME;
use funai::chainstate::funai::index::marf::MARFOpenOpts;
use funai::chainstate::funai::index::storage::TrieHashCalculationMode;
use funai::chainstate::funai::miner::{BlockBuilderSettings, MinerStatus};
use funai::chainstate::funai::MAX_BLOCK_LEN;
use funai::core::mempool::{MemPoolWalkSettings, MemPoolWalkTxTypes};
use funai::core::{
    MemPoolDB, FunaiEpoch, FunaiEpochExtension, FunaiEpochId,
...
use funai::cost_estimates::fee_medians::WeightedMedianFeeRateEstimator;
use funai::cost_estimates::fee_rate_fuzzer::FeeRateFuzzer;
use funai::cost_estimates::fee_scalar::ScalarFeeRateEstimator;
use funai::cost_estimates::metrics::{CostMetric, ProportionalDotProduct, UnitMetric};
use funai::cost_estimates::{CostEstimator, FeeEstimator, PessimisticEstimator, UnitEstimator};
use funai::net::atlas::AtlasConfig;
use funai::net::connection::ConnectionOptions;
use funai::net::{Neighbor, NeighborKey};
use funai::types::chainstate::BurnchainHeaderHash;
use funai::util_lib::boot::boot_code_id;
use funai::util_lib::db::Error as DBError;
...
impl ConfigFile {
    ...
    pub fn xenon() -> ConfigFile {
        let mut burnchain = BurnchainConfigFile {
            ...
            peer_host: Some("bitcoind.testnet.funai.co".to_string()),
    ...
    pub fn mainnet() -> ConfigFile {
        let burnchain = BurnchainConfigFile {
            ...
            password: Some("blockfunaiystem".to_string()),
            magic_bytes: Some("X2".to_string()),
            ..BurnchainConfigFile::default()
        };

        let node = NodeConfigFile {
            bootstrap_node: Some("02196f005965cebe6ddc3901b7b1cc1aa7a88f305bb8c5893456b8f9a605923893@seed.mainnet.hiro.so:20444,02539449ad94e6e6392d8c1deb2b4e61f80ae2a18964349bc14336d8b903c46a8c@cet.funainodes.org:20444,02ececc8ce79b8adf813f13a0255f8ae58d4357309ba0cedd523d9f1a306fcfb79@sgt.funainodes.org:20444,0303144ba518fe7a0fb56a8a7d488f950307a4330f146e1e1458fc63fb33defe96@est.funainodes.org:20444".to_string()),
            miner: Some(false),
            funai: Some(false),
            ..NodeConfigFile::default()
        };
...
impl Config {
    ...
    fn apply_test_settings(&self, burnchain: &mut Burnchain) {
        ...
        if let Some(epochs) = &self.burnchain.epochs {
            if let Some(epoch) = epochs
                .iter()
                .find(|epoch| epoch.epoch_id == FunaiEpochId::Epoch10)
            {
            ...
            if let Some(epoch) = epochs
                .iter()
                .find(|epoch| epoch.epoch_id == FunaiEpochId::Epoch20)
            {
                assert_eq!(
                    epoch.start_height, burnchain.first_block_height,
            ...
            if let Some(epoch) = epochs
                .iter()
                .find(|epoch| epoch.epoch_id == FunaiEpochId::Epoch21)
            {
            ...
            if let Some(epoch) = epochs
                .iter()
                .find(|epoch| epoch.epoch_id == FunaiEpochId::Epoch22)
            {
            ...
            if let Some(epoch) = epochs
                .iter()
                .find(|epoch| epoch.epoch_id == FunaiEpochId::Epoch24)
            {
            ...
            if let Some(epoch) = epochs
                .iter()
                .find(|epoch| epoch.epoch_id == FunaiEpochId::Epoch25)
            {
    ...
    fn check_nakamoto_config(&self, burnchain: &Burnchain) {
        let epochs = FunaiEpoch::get_epochs(
            self.burnchain.get_bitcoin_network().1,
            self.burnchain.epochs.as_ref(),
        );
        let Some(epoch_30) = FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch30)
    ...
    pub fn assert_valid_epoch_settings(burnchain: &Burnchain, epochs: &[FunaiEpoch]) {
        // sanity check: epochs must be contiguous and ordered
        // (this panics if it's not the case)
        test_debug!("Validate epochs: {:#?}", epochs);
        let _ = FunaiEpoch::validate_epochs(epochs);

        // sanity check: v1_unlock_height must happen after pox-2 instantiation
        let epoch21_index = FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch21)
    ...
    fn make_epochs(
        conf_epochs: &[FunaiEpochConfigFile],
        burn_mode: &str,
        bitcoin_network: BitcoinNetworkType,
        pox_2_activation: Option<u32>,
    ) -> Result<Vec<FunaiEpoch>, String> {
        let default_epochs = match bitcoin_network {
            BitcoinNetworkType::Mainnet => {
                Err("Cannot configure epochs in mainnet mode".to_string())
            }
            BitcoinNetworkType::Testnet => Ok(funai::core::STACKS_EPOCHS_TESTNET.to_vec()),
            BitcoinNetworkType::Regtest => Ok(funai::core::STACKS_EPOCHS_REGTEST.to_vec()),
        }?;
        ...
        // Funai 1.0 must start at 0
        if matched_epochs[0].1 != 0 {
            return Err("Funai 1.0 must start at height = 0".into());
        }
        ...
        if let Some(pox_2_activation) = pox_2_activation {
            let last_epoch = out_epochs
                .iter()
                .find(|&e| e.epoch_id == FunaiEpochId::Epoch21)
    ...
    fn from_config_default(config_file: ConfigFile, default: Config) -> Result<Config, String> {
        ...
        if node.funai || node.miner {
            node.add_miner_funaidb(is_mainnet);
            node.add_signers_funaidbs(is_mainnet);
        }
    ...
    pub fn get_funai_db_file_path(&self) -> String {
        let mut path = self.get_chainstate_path();
        path.set_file_name("funai_db.sqlite");
        path.to_str().expect("Unable to produce path").to_string()
    }
...
impl BurnchainConfig {
    ...
    pub fn get_bitcoin_network(&self) -> (String, BitcoinNetworkType) {
        match self.mode.as_str() {
            ...
            other => panic!("Invalid funai-node mode: {other}"),
        }
    }
}

#[derive(Clone, Deserialize, Default, Debug)]
pub struct FunaiEpochConfigFile {
    epoch_name: String,
    start_height: i64,
}
...
pub struct BurnchainConfigFile {
    ...
    pub first_burn_block_timestamp: Option<u32>,
    pub first_burn_block_hash: Option<String>,
    pub epochs: Option<Vec<FunaiEpochConfigFile>>,
    pub pox_prepare_length: Option<u32>,
    pub pox_reward_length: Option<u32>,
    pub pox_2_activation: Option<u32>,
    pub sunset_start: Option<u32>,
...
impl BurnchainConfigFile {
    ...
    ///
    /// The Xenon Testnet Funai 2.4 activation height occurred before the finalized SIP-024 updates and release of the funai-node versioned 2.4.0.0.0.
    /// This caused the Funai Xenon testnet to undergo a deep reorg when 2.4.0.0.0 was finalized. This deep reorg meant that 3 reward cycles were
...
pub struct NodeConfig {
    ...
    /// endpoint to the signer service for verifying inference tasks
...
impl Default for NodeConfig {
    fn default() -> Self {
        ...
        let testnet_id = format!("funai-node-{}", now);
        ...
        NodeConfig {
            ...
            miner_endpoint: "127.0.0.1".to_string(),
...
impl NodeConfig {
    ...
    pub fn add_signers_funaidbs(&mut self, is_mainnet: bool) {
        for signer_set in 0..2 {
            for message_id in 0..SIGNER_SLOTS_PER_USER {
                ...
                if !self.funai_dbs.contains(&contract_id) {
                    debug!("A miner/funai must subscribe to the {contract_id} funai db contract. Forcibly subscribing...");
    ...
    pub fn add_miner_funaidb(&mut self, is_mainnet: bool) {
        let miners_contract_id = boot_code_id(MINERS_NAME, is_mainnet);
        if !self.funai_dbs.contains(&miners_contract_id) {
            debug!("A miner/funai must subscribe to the {miners_contract_id} funai db contract. Forcibly subscribing...");
            self.funai_dbs.push(miners_contract_id);
        }
    }
...
pub struct MinerConfig {
    ...
    pub filter_origins: HashSet<FunaiAddress>,
...
impl Default for MinerConfig {
    fn default() -> MinerConfig {
        MinerConfig {
            ...
            max_reorg_depth: 3,
            // TODO: update to a sane value based on funaidb benchmarking
...
impl NodeConfigFile {
    fn into_config_default(self, default_node_config: NodeConfig) -> Result<NodeConfig, String> {
        ...
        let funai = self.funai.unwrap_or(default_node_config.funai);
        let node_config = NodeConfig {
            ...
            miner_endpoint: self.miner_endpoint.unwrap_or(default_node_config.miner_endpoint),
            ...
            funai_dbs: self
                ...
                .collect(),
            signer_endpoint: self.signer_endpoint.or(default_node_config.signer_endpoint),
        };
        Ok(node_config)
    }
}
        ...
        Ok(MinerConfig {
            ...
            filter_origins: {
                if let Some(filter_origins) = &self.filter_origins {
                    filter_origins
                        .split(',')
                        .map(|origin_str| match FunaiAddress::from_string(origin_str) {
                            Some(addr) => addr,
                            None => {
                                panic!("could not parse '{}' into a Funai address", origin_str);
                            }
                        })
                        .collect()
                } else {
...
impl EventKeyType {
    fn from_string(raw_key: &str) -> Option<EventKeyType> {
        ...
        if raw_key == "funaidb" {
            return Some(EventKeyType::FunaiDBChunks);
        }
