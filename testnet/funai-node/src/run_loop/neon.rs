...
use funai::burnchains::{Burnchain, Error as burnchain_error};
use funai::chainstate::burn::db::sortdb::SortitionDB;
use funai::chainstate::burn::BlockSnapshot;
use funai::chainstate::coordinator::comm::{CoordinatorChannels, CoordinatorReceivers};
use funai::chainstate::coordinator::{
    migrate_chainstate_dbs, static_get_canonical_affirmation_map,
    static_get_heaviest_affirmation_map, static_get_funai_tip_affirmation_map, ChainsCoordinator,
    ChainsCoordinatorConfig, CoordinatorCommunication, Error as coord_error,
};
use funai::chainstate::funai::db::{ChainStateBootData, FunaiChainState};
use funai::chainstate::funai::miner::{signal_mining_blocked, signal_mining_ready, MinerStatus};
use funai::core::FunaiEpochId;
use funai::net::atlas::{AtlasConfig, AtlasDB, Attachment};
use funai::util_lib::db::Error as db_error;
use funai_common::deps_common::ctrlc as termination;
use funai_common::deps_common::ctrlc::SignalId;
...
use crate::neon_node::{FunaiNode, BLOCK_PROCESSOR_STACK_SIZE, RELAYER_MAX_BUFFER};
use crate::node::{
...
impl RunLoop {
    ...
    pub fn instantiate_burnchain_state(
        ...
        let epochs = burnchain_controller.get_funai_epochs();
    ...
    pub fn boot_chainstate(&mut self, burnchain_config: &Burnchain) -> FunaiChainState {
        ...
        let (chain_state_db, receipts) = FunaiChainState::open_and_exec(
    ...
    /// Instantiate the Funai chain state and start the chains coordinator thread.
    ...
    fn get_reward_cycle_sortition_db_height(
        ...
        let (funai_ch, _) = SortitionDB::get_canonical_funai_chain_tip_hash(sortdb.conn())
            .expect("BUG: failed to load canonical funai chain tip hash");

        let sn = match SortitionDB::get_block_snapshot_consensus(sortdb.conn(), &funai_ch)
            .expect("BUG: failed to query sortition DB")
        {
            Some(sn) => sn,
            None => {
                debug!("No canonical funai chain tip hash present");
    ...
    /// Wake up and drive funai block processing if there's been a PoX reorg.
    /// Be careful not to saturate calls to announce new funai blocks, because that will disable
    /// mining (which would prevent a miner attempting to fix a hidden PoX anchor block from making
    /// progress).
    fn drive_pox_reorg_funai_block_processing(
        globals: &Globals,
        config: &Config,
        burnchain: &Burnchain,
        sortdb: &SortitionDB,
        last_funai_pox_reorg_recover_time: &mut u128,
    ) {
        ...
        if *last_funai_pox_reorg_recover_time + (delay as u128) >= get_epoch_time_secs().into() {
            // too soon
            return;
        }

        // compare funai and heaviest AMs
        ...
        .expect("FATAL: could not query funai DB");

        if funai_tip_affirmation_map.len() < heaviest_affirmation_map.len()
                ...
                .is_some()
        {
            ...
            // (i.e. the underlying sortitions) before we can fix the funai fork
            ...
            debug!(
                "Drive funai block processing: possible PoX reorg (funai tip: {}, heaviest: {})",
                &funai_tip_affirmation_map, &heaviest_affirmation_map
            );
            globals.coord().announce_new_funai_block();
        } else {
            debug!(
                "Drive funai block processing: no need (funai tip: {}, heaviest: {})",
                &funai_tip_affirmation_map, &heaviest_affirmation_map
            );

            // announce a new funai block to force the chains coordinator
            //  to wake up anyways. this isn't free, so we have to make sure
            //  the chain-liveness thread doesn't wake up too often
            globals.coord().announce_new_funai_block();
        }

        *last_funai_pox_reorg_recover_time = get_epoch_time_secs().into();
    }
    ...
    fn drive_chain_liveness(
        ...
        let mut last_funai_pox_reorg_recover_time = 0;
        ...
        while globals.keep_running() {
            ...
            Self::drive_pox_reorg_funai_block_processing(
                &globals,
                &config,
                &burnchain,
                &sortdb,
                &mut last_funai_pox_reorg_recover_time,
            );
    ...
    fn spawn_chain_liveness_thread(&self, globals: Globals) -> JoinHandle<()> {
        ...
        let (chain_state_db, _) = FunaiChainState::open(
            config.is_mainnet(),
    ...
    pub fn start(&mut self, burnchain_opt: Option<Burnchain>, mut mine_start: u64) {
        ...
        let mut node = FunaiNode::spawn(self, globals.clone(), relay_recv);
        ...
        loop {
            ...
            debug!("Runloop: Wait until Funai block downloads reach a quiescent state before processing more burnchain blocks"; "remote_chain_height" => remote_chain_height, "local_chain_height" => burnchain_height);
            ...
            // cause the node's p2p and relayer threads to go fetch and download Funai blocks and
            ...
            // obtained all the Funai blocks it can.
            debug!(
            ...
            if sortition_db_height >= burnchain_height && !ibd {
                let canonical_funai_tip_height =
                    SortitionDB::get_canonical_burn_chain_tip(burnchain.sortdb_ref().conn())
                        .map(|snapshot| snapshot.canonical_funai_tip_height)
                        .unwrap_or(0);
                if canonical_funai_tip_height < mine_start {
                    info!(
                        "Runloop: Synchronized full burnchain, but funai tip height is {}, and we are trying to boot to {}, not mining until reaching chain tip",
