...
use funai::chainstate::burn::operations::LeaderKeyRegisterOp;
use funai::chainstate::burn::BlockSnapshot;
use funai::chainstate::coordinator::comm::CoordinatorChannels;
use funai::chainstate::funai::db::unconfirmed::UnconfirmedTxMap;
use funai::chainstate::funai::db::FunaiChainState;
use funai::chainstate::funai::miner::MinerStatus;
use funai::net::NetworkResult;
use funai_common::types::chainstate::{BlockHeaderHash, BurnchainHeaderHash, ConsensusHash};
...
pub struct Globals<T> {
    ...
    /// maps funai height to tip candidate
...
impl<T> Globals<T> {
    ...
    pub fn send_unconfirmed_txs(&self, chainstate: &FunaiChainState) {
    ...
    pub fn recv_unconfirmed_txs(&self, chainstate: &mut FunaiChainState) {
    ...
    /// Record a best-tip
    pub fn add_best_tip(&self, funai_height: u64, tip_candidate: TipCandidate, max_depth: u64) {
        match self.previous_best_tips.lock() {
            Ok(mut tips) => {
                tips.insert(funai_height, tip_candidate);
                let mut stale = vec![];
                for (prev_height, _) in tips.iter() {
                    if *prev_height + max_depth < funai_height {
    ...
    pub fn get_best_tip(&self, funai_height: u64) -> Option<TipCandidate> {
        match self.previous_best_tips.lock() {
            Ok(tips) => tips.get(&funai_height).cloned(),
            Err(_e) => {
                error!("FATAL: failed to lock previous_best_tips");
                panic!();
            }
