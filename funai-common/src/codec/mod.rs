...
impl_funai_message_codec_for_int!(u8; [0; 1]);
impl_funai_message_codec_for_int!(u16; [0; 2]);
impl_funai_message_codec_for_int!(u32; [0; 4]);
impl_funai_message_codec_for_int!(u64; [0; 8]);
impl_funai_message_codec_for_int!(i64; [0; 8]);

impl FunaiMessageCodec for [u8; 20] {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), Error> {
...
impl FunaiMessageCodec for [u8; 32] {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), Error> {
        fd.write_all(self).map_err(Error::WriteError)
    }
...
pub fn read_next<T: FunaiMessageCodec, R: Read>(fd: &mut R) -> Result<T, Error> {
    let item: T = T::consensus_deserialize(fd)?;
    Ok(item)
}

fn read_next_vec<T: FunaiMessageCodec + Sized, R: Read>(
    fd: &mut R,
    num_items: u32,
    max_items: u32,
) -> Result<Vec<T>, Error> {
...
pub fn read_next_at_most<R: Read, T: FunaiMessageCodec + Sized>(
    fd: &mut R,
    max_items: u32,
) -> Result<Vec<T>, Error> {
...
pub fn read_next_exact<R: Read, T: FunaiMessageCodec + Sized>(
    fd: &mut R,
    num_items: u32,
) -> Result<Vec<T>, Error> {
...
impl<T> FunaiMessageCodec for Vec<T>
where
    T: FunaiMessageCodec + Sized,
{
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), Error> {
