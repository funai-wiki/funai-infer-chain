use std::cmp::Ordering;
use std::fmt;

use crate::address::c32::{c32_address, c32_address_decode};
use crate::address::{
    public_keys_to_address_hash, to_bits_p2pkh, AddressHashMode,
    C32_ADDRESS_VERSION_MAINNET_MULTISIG, C32_ADDRESS_VERSION_MAINNET_SINGLESIG,
    C32_ADDRESS_VERSION_TESTNET_MULTISIG, C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
};
use crate::deps_common::bitcoin::blockdata::transaction::TxOut;
use crate::types::chainstate::{FunaiAddress, FunaiPublicKey};
use crate::util::hash::Hash160;
use crate::util::secp256k1::{MessageSignature, Secp256k1PublicKey};

pub mod chainstate;
pub mod net;

/// A container for public keys (compressed secp256k1 public keys)
pub struct FunaiPublicKeyBuffer(pub [u8; 33]);
impl_array_newtype!(FunaiPublicKeyBuffer, u8, 33);
impl_array_hexstring_fmt!(FunaiPublicKeyBuffer);
impl_byte_array_newtype!(FunaiPublicKeyBuffer, u8, 33);
impl_byte_array_message_codec!(FunaiPublicKeyBuffer, 33);
impl_byte_array_serde!(FunaiPublicKeyBuffer);

impl FunaiPublicKeyBuffer {
    pub fn from_public_key(pubkey: &Secp256k1PublicKey) -> FunaiPublicKeyBuffer {
        let pubkey_bytes_vec = pubkey.to_bytes_compressed();
        let mut pubkey_bytes = [0u8; 33];
        pubkey_bytes.copy_from_slice(&pubkey_bytes_vec[..]);
        FunaiPublicKeyBuffer(pubkey_bytes)
    }

    pub fn to_public_key(&self) -> Result<Secp256k1PublicKey, &'static str> {
        Secp256k1PublicKey::from_slice(&self.0)
            .map_err(|_e_str| "Failed to decode Funai public key")
    }
}

pub trait PublicKey: Clone + fmt::Debug + serde::Serialize + serde::de::DeserializeOwned {
    fn to_bytes(&self) -> Vec<u8>;
    fn verify(&self, data_hash: &[u8], sig: &MessageSignature) -> Result<bool, &'static str>;
}

pub trait PrivateKey: Clone + fmt::Debug + serde::Serialize + serde::de::DeserializeOwned {
    fn to_bytes(&self) -> Vec<u8>;
    fn sign(&self, data_hash: &[u8]) -> Result<MessageSignature, &'static str>;
}

pub trait Address: Clone + fmt::Debug + fmt::Display {
    fn to_bytes(&self) -> Vec<u8>;
    fn from_string(from: &str) -> Option<Self>
    where
        Self: Sized;
    fn is_burn(&self) -> bool;
}

pub const PEER_VERSION_EPOCH_1_0: u8 = 0x00;
pub const PEER_VERSION_EPOCH_2_0: u8 = 0x00;
pub const PEER_VERSION_EPOCH_2_05: u8 = 0x05;
pub const PEER_VERSION_EPOCH_2_1: u8 = 0x06;

#[repr(u32)]
#[derive(Debug, Clone, Eq, PartialEq, PartialOrd, Ord, Hash, Copy, Serialize, Deserialize)]
pub enum FunaiEpochId {
    Epoch10 = 0x01000,
    Epoch20 = 0x02000,
    Epoch2_05 = 0x02005,
    Epoch21 = 0x0200a,
    Epoch22 = 0x0200f,
    Epoch23 = 0x02014,
    Epoch24 = 0x02019,
    Epoch25 = 0x0201a,
    Epoch30 = 0x03000,
}

impl FunaiEpochId {
    pub fn latest() -> FunaiEpochId {
        FunaiEpochId::Epoch30
    }

    /// Returns whether or not this Epoch should perform
    ///  memory checks during analysis
    pub fn analysis_memory(&self) -> bool {
        match self {
            FunaiEpochId::Epoch10
            | FunaiEpochId::Epoch20
            | FunaiEpochId::Epoch2_05
            | FunaiEpochId::Epoch21
            | FunaiEpochId::Epoch22
            | FunaiEpochId::Epoch23
            | FunaiEpochId::Epoch24 => false,
            FunaiEpochId::Epoch25 | FunaiEpochId::Epoch30 => true,
        }
    }

    /// Returns whether or not this Epoch should perform
    ///  Clarity value sanitization
    pub fn value_sanitizing(&self) -> bool {
        match self {
            FunaiEpochId::Epoch10
            | FunaiEpochId::Epoch20
            | FunaiEpochId::Epoch2_05
            | FunaiEpochId::Epoch21
            | FunaiEpochId::Epoch22
            | FunaiEpochId::Epoch23 => false,
            FunaiEpochId::Epoch24 | FunaiEpochId::Epoch25 | FunaiEpochId::Epoch30 => true,
        }
    }

    /// Does this epoch support unlocking PoX contributors that miss a slot?
    ///
    /// Epoch 2.0 - 2.05 didn't support this feature, but they weren't epoch-guarded on it. Instead,
    ///  the behavior never activates in those epochs because the Pox1 contract does not provide
    ///  `contibuted_stackers` information. This check maintains that exact semantics by returning
    ///  true for all epochs before 2.5. For 2.5 and after, this returns false.
    pub fn supports_pox_missed_slot_unlocks(&self) -> bool {
        self < &FunaiEpochId::Epoch25
    }
}

impl std::fmt::Display for FunaiEpochId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FunaiEpochId::Epoch10 => write!(f, "1.0"),
            FunaiEpochId::Epoch20 => write!(f, "2.0"),
            FunaiEpochId::Epoch2_05 => write!(f, "2.05"),
            FunaiEpochId::Epoch21 => write!(f, "2.1"),
            FunaiEpochId::Epoch22 => write!(f, "2.2"),
            FunaiEpochId::Epoch23 => write!(f, "2.3"),
            FunaiEpochId::Epoch24 => write!(f, "2.4"),
            FunaiEpochId::Epoch25 => write!(f, "2.5"),
            FunaiEpochId::Epoch30 => write!(f, "3.0"),
        }
    }
}

impl TryFrom<u32> for FunaiEpochId {
    type Error = &'static str;

    fn try_from(value: u32) -> Result<FunaiEpochId, Self::Error> {
        match value {
            x if x == FunaiEpochId::Epoch10 as u32 => Ok(FunaiEpochId::Epoch10),
            x if x == FunaiEpochId::Epoch20 as u32 => Ok(FunaiEpochId::Epoch20),
            x if x == FunaiEpochId::Epoch2_05 as u32 => Ok(FunaiEpochId::Epoch2_05),
            x if x == FunaiEpochId::Epoch21 as u32 => Ok(FunaiEpochId::Epoch21),
            x if x == FunaiEpochId::Epoch22 as u32 => Ok(FunaiEpochId::Epoch22),
            x if x == FunaiEpochId::Epoch23 as u32 => Ok(FunaiEpochId::Epoch23),
            x if x == FunaiEpochId::Epoch24 as u32 => Ok(FunaiEpochId::Epoch24),
            x if x == FunaiEpochId::Epoch25 as u32 => Ok(FunaiEpochId::Epoch25),
            x if x == FunaiEpochId::Epoch30 as u32 => Ok(FunaiEpochId::Epoch30),
            _ => Err("Invalid epoch"),
        }
    }
}

impl PartialOrd for FunaiAddress {
    fn partial_cmp(&self, other: &FunaiAddress) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for FunaiAddress {
    fn cmp(&self, other: &FunaiAddress) -> Ordering {
        match self.version.cmp(&other.version) {
            Ordering::Equal => self.bytes.cmp(&other.bytes),
            inequality => inequality,
        }
    }
}

impl FunaiAddress {
    pub fn new(version: u8, hash: Hash160) -> FunaiAddress {
        FunaiAddress {
            version,
            bytes: hash,
        }
    }

    pub fn is_mainnet(&self) -> bool {
        match self.version {
            C32_ADDRESS_VERSION_MAINNET_MULTISIG | C32_ADDRESS_VERSION_MAINNET_SINGLESIG => true,
            C32_ADDRESS_VERSION_TESTNET_MULTISIG | C32_ADDRESS_VERSION_TESTNET_SINGLESIG => false,
            _ => false,
        }
    }

    pub fn burn_address(mainnet: bool) -> FunaiAddress {
        FunaiAddress {
            version: if mainnet {
                C32_ADDRESS_VERSION_MAINNET_SINGLESIG
            } else {
                C32_ADDRESS_VERSION_TESTNET_SINGLESIG
            },
            bytes: Hash160([0u8; 20]),
        }
    }

    /// Generate an address from a given address hash mode, signature threshold, and list of public
    /// keys.  Only return an address if the combination given is supported.
    /// The version is may be arbitrary.
    pub fn from_public_keys(
        version: u8,
        hash_mode: &AddressHashMode,
        num_sigs: usize,
        pubkeys: &Vec<FunaiPublicKey>,
    ) -> Option<FunaiAddress> {
        // must be sufficient public keys
        if pubkeys.len() < num_sigs {
            return None;
        }

        // address hash mode must be consistent with the number of keys
        match *hash_mode {
            AddressHashMode::SerializeP2PKH | AddressHashMode::SerializeP2WPKH => {
                // must be a single public key, and must require one signature
                if num_sigs != 1 || pubkeys.len() != 1 {
                    return None;
                }
            }
            _ => {}
        }

        // if segwit, then keys must all be compressed
        match *hash_mode {
            AddressHashMode::SerializeP2WPKH | AddressHashMode::SerializeP2WSH => {
                for pubkey in pubkeys {
                    if !pubkey.compressed() {
                        return None;
                    }
                }
            }
            _ => {}
        }

        let hash_bits = public_keys_to_address_hash(hash_mode, num_sigs, pubkeys);
        Some(FunaiAddress::new(version, hash_bits))
    }

    /// Make a P2PKH FunaiAddress
    pub fn p2pkh(mainnet: bool, pubkey: &FunaiPublicKey) -> FunaiAddress {
        let bytes = to_bits_p2pkh(pubkey);
        Self::p2pkh_from_hash(mainnet, bytes)
    }

    /// Make a P2PKH FunaiAddress
    pub fn p2pkh_from_hash(mainnet: bool, hash: Hash160) -> FunaiAddress {
        let version = if mainnet {
            C32_ADDRESS_VERSION_MAINNET_SINGLESIG
        } else {
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG
        };
        Self {
            version,
            bytes: hash,
        }
    }
}

impl std::fmt::Display for FunaiAddress {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        c32_address(self.version, self.bytes.as_bytes())
            .expect("Stacks version is not C32-encodable")
            .fmt(f)
    }
}

impl Address for FunaiAddress {
    fn to_bytes(&self) -> Vec<u8> {
        self.bytes.as_bytes().to_vec()
    }

    fn from_string(s: &str) -> Option<FunaiAddress> {
        let (version, bytes) = match c32_address_decode(s) {
            Ok((v, b)) => (v, b),
            Err(_) => {
                return None;
            }
        };

        if bytes.len() != 20 {
            return None;
        }

        let mut hash_bytes = [0u8; 20];
        hash_bytes.copy_from_slice(&bytes[..]);
        Some(FunaiAddress {
            version,
            bytes: Hash160(hash_bytes),
        })
    }

    fn is_burn(&self) -> bool {
        self.bytes == Hash160([0u8; 20])
    }
}

#[derive(Debug, Clone, PartialEq, Eq, Hash, Deserialize)]
pub struct FunaiEpoch<L> {
    pub epoch_id: FunaiEpochId,
    pub start_height: u64,
    pub end_height: u64,
    pub block_limit: L,
    pub network_epoch: u8,
}

impl<L> FunaiEpoch<L> {
    /// Determine which epoch, if any, in a list of epochs, a given burnchain height falls into.
    /// Returns Some(index) if there is such an epoch in the list.
    /// Returns None if not.
    pub fn find_epoch(epochs: &[FunaiEpoch<L>], height: u64) -> Option<usize> {
        for (i, epoch) in epochs.iter().enumerate() {
            if epoch.start_height <= height && height < epoch.end_height {
                return Some(i);
            }
        }
        None
    }

    /// Find an epoch by its ID
    /// Returns Some(index) if the epoch is in the list
    /// Returns None if not
    pub fn find_epoch_by_id(epochs: &[FunaiEpoch<L>], epoch_id: FunaiEpochId) -> Option<usize> {
        for (i, epoch) in epochs.iter().enumerate() {
            if epoch.epoch_id == epoch_id {
                return Some(i);
            }
        }
        None
    }
}

// FunaiEpochs are ordered by start block height
impl<L: PartialEq> PartialOrd for FunaiEpoch<L> {
    fn partial_cmp(&self, other: &FunaiEpoch<L>) -> Option<Ordering> {
        self.epoch_id.partial_cmp(&other.epoch_id)
    }
}

impl<L: PartialEq + Eq> Ord for FunaiEpoch<L> {
    fn cmp(&self, other: &FunaiEpoch<L>) -> Ordering {
        self.epoch_id.cmp(&other.epoch_id)
    }
}
