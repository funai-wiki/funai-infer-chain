...
use crate::codec::{read_next, write_next, Error as CodecError, FunaiMessageCodec};
...
pub type FunaiPublicKey = Secp256k1PublicKey;
pub type FunaiPrivateKey = Secp256k1PrivateKey;
...
pub struct FunaiAddress {
    pub version: u8,
    pub bytes: Hash160,
}

impl FunaiMessageCodec for FunaiAddress {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiAddress, CodecError> {
...
pub const FUNAI_ADDRESS_ENCODED_SIZE: u32 = 1 + HASH160_ENCODED_SIZE;

/// How much work has gone into this chain so far?
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct FunaiWorkScore {
    pub burn: u64, // number of burn tokens destroyed
    pub work: u64, // in Stacks, "work" == the length of the fork
}

pub struct FunaiBlockId(pub [u8; 32]);
impl_array_newtype!(FunaiBlockId, u8, 32);
impl_array_hexstring_fmt!(FunaiBlockId);
impl_byte_array_newtype!(FunaiBlockId, u8, 32);
impl_byte_array_rusqlite_only!(FunaiBlockId);
impl_byte_array_serde!(FunaiBlockId);
...
impl FunaiBlockId {
    ...
    pub fn first_mined() -> FunaiBlockId {
...
impl FunaiWorkScore {
    ...
    pub fn genesis() -> FunaiWorkScore {
        FunaiWorkScore { burn: 0, work: 0 }
    }
}

impl FunaiMessageCodec for VRFProof {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), CodecError> {
...
impl FunaiMessageCodec for FunaiWorkScore {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), CodecError> {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiWorkScore, CodecError> {
        let burn = read_next(fd)?;
        let work = read_next(fd)?;

        Ok(FunaiWorkScore { burn, work })
    }
}
...
impl_byte_array_message_codec!(FunaiBlockId, 32);
...
impl BlockHeaderHash {
    ...
    pub fn from_serializer<C: FunaiMessageCodec>(
        serializer: &C,
...
impl FunaiMessageCodec for (ConsensusHash, BurnchainHeaderHash) {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), CodecError> {
