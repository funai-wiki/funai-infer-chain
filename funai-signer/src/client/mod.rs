...
/// The funai db module for communicating with the funaidb contract
mod funaidb;
/// The funai node client module for communicating with the funai node
pub(crate) mod funai_client;

use std::time::Duration;

use clarity::vm::errors::Error as ClarityError;
use clarity::vm::types::serialization::SerializationError;
use libfunaidb::Error as FunaiDBError;
use slog::slog_debug;
pub use funaidb::*;
pub use funai_client::*;
use funai_common::codec::Error as CodecError;
...
pub enum ClientError {
    ...
    #[error("Unable to serialize funai-db message: {0}")]
    FunaiDBSerializationError(#[from] CodecError),
    /// Failed to sign funai-db chunk
    #[error("Failed to sign funai-db chunk: {0}")]
    FailToSign(#[from] FunaiDBError),
    /// Funai-db instance rejected the chunk
    #[error("Funai-db rejected the chunk. Reason: {0}")]
    ...
    /// Failed to build and sign a new Funai transaction.
    #[error("Failed to generate transaction from a transaction signer: {0}")]
    TransactionGenerationFailure(String),
    /// Funai node client request failed
    #[error("Funai node client request failed: {0}")]
    ...
    #[error("Backoff retry timeout occurred. Funai node may be down.")]
    RetryTimeout,
    /// Not connected
    #[error("Not connected")]
    NotConnected,
    ...
    /// Funai node does not support a feature we need
    #[error("Funai node does not support a required feature: {0}")]
    UnsupportedFunaiFeature(String),
}
    ...
    E: std::fmt::Debug,
{
    let notify = |err, dur| {
        debug!(
            "Failed to connect to funai node and/or deserialize its response: {err:?}. Next attempt in {dur:?}"
        );
    };
...
pub(crate) mod tests {
    ...
    use funai_common::types::{FunaiEpochId, FunaiPublicKeyBuffer};
    ...
    impl MockServerClient {
        ...
        pub fn new() -> Self {
            ...
            let client = FunaiClient::from(&config);
            Self {
                server,
                client,
                config,
    ...
    /// Build a response for the get_peer_info_with_retry request with a specific funai tip height and consensus hash
    pub fn build_get_peer_info_response(
        ...
        let peer_info = RPCPeerInfoData {
            ...
            funai_tip_height: thread_rng().next_u64(),
            funai_tip: BlockHeaderHash([0x06; 32]),
            funai_tip_consensus_hash: generate_random_consensus_hash(),
            unanchored_tip: None,
    ...
    pub fn generate_signer_config(
        ...
        for signer_id in 0..num_signers {
            ...
            if signer_id == 0 {
                ...
                let address = FunaiAddress::p2pkh(
                    false,
                    &FunaiPublicKey::from_slice(ecdsa_public_key.to_bytes().as_slice())
                        .expect("Failed to create funai public key"),
                );
            ...
            let address = FunaiAddress::p2pkh(
                false,
                &FunaiPublicKey::from_slice(public_key.to_bytes().as_slice())
                    .expect("Failed to create funai public key"),
            );
            signer_slot_ids.push(SignerSlotID(signer_id));
            signer_ids.insert(address, signer_id);
        ...
        SignerConfig {
            ...
            support_models: config.support_models.clone(),
        }
    }
