...
use funailib::net::api::callreadonly::CallReadOnlyResponse;
use funailib::net::api::getaccount::AccountEntryResponse;
use funailib::net::api::getinfo::RPCPeerInfoData;
use funailib::net::api::getpoxinfo::RPCPoxInfoData;
use funailib::net::api::getfunais::GetFunaisResponse;
use funailib::net::api::postblock_proposal::NakamotoBlockProposal;
use funailib::util_lib::boot::{boot_code_addr, boot_code_id};
...
use funai_common::consts::{CHAIN_ID_MAINNET, CHAIN_ID_TESTNET};
use funai_common::debug;
...
/// The Funai signer client used to communicate with the funai node
#[derive(Clone, Debug)]
pub struct FunaiClient {
    /// The funai address of the signer
    funai_address: FunaiAddress,
    /// The private key used in all funai node communications
    funai_private_key: FunaiPrivateKey,
    /// The funai node HTTP base endpoint
    http_origin: String,
    ...
    funai_node_client: reqwest::blocking::Client,
    /// the auth password for the funai node
    auth_password: String,
}

impl From<&GlobalConfig> for FunaiClient {
    fn from(config: &GlobalConfig) -> Self {
        Self {
            funai_private_key: config.funai_private_key,
            funai_address: config.funai_address,
            ...
            funai_node_client: reqwest::blocking::Client::new(),
...
impl FunaiClient {
    /// Create a new signer FunaiClient with the provided private key, funai node host endpoint, version, and auth password
    pub fn new(
        funai_private_key: FunaiPrivateKey,
        ...
        let pubkey = FunaiPublicKey::from_private(&funai_private_key);
        ...
        let funai_address = FunaiAddress::p2pkh(mainnet, &pubkey);
        Self {
            funai_private_key,
            funai_address,
    ...
    pub const fn get_signer_address(&self) -> &FunaiAddress {
        &self.funai_address
    }

    /// Retrieve the signer slots stored within the funaidb contract
    pub fn get_funaidb_signer_slots(
        &self,
        funaidb_contract: &QualifiedContractIdentifier,
        page: u32,
    ) -> Result<Vec<(FunaiAddress, u128)>, ClientError> {
        let function_name_str = "funaidb-get-signer-slots-page";
        ...
        let value = self.read_only_contract_call(
            &funaidb_contract.issuer.clone().into(),
            &funaidb_contract.name,
            &function_name,
            function_args,
        )?;
    ...
    fn parse_signer_slots(
        ...
        for value in values {
            ...
            let signer = if let PrincipalData::Standard(signer) = principal_data {
                ...
                panic!("BUG: Signers funaidb contract is corrupted");
            };
    ...
    /// Determine the funai node current epoch
    pub fn get_node_epoch(&self) -> Result<FunaiEpochId, ClientError> {
        ...
        let epoch_25 = pox_info
            ...
            .ok_or(ClientError::UnsupportedFunaiFeature(
        ...
        let epoch_30 = pox_info
            .epochs
            .iter()
            .find(|epoch| epoch.epoch_id == FunaiEpochId::Epoch30)
            .ok_or(ClientError::UnsupportedFunaiFeature(
    ...
    /// Submit the block proposal to the funai node. The block will be validated and returned via the HTTP endpoint for Block events.
    ...
    pub fn get_account_nonce(&self, address: &FunaiAddress) -> Result<u64, ClientError> {
        let account_entry = self.get_account_entry_with_retry(address)?;
        Ok(account_entry.nonce)
    }

    /// Get the current peer info data from the funai node
    pub fn get_peer_info_with_retry(&self) -> Result<RPCPeerInfoData, ClientError> {
        debug!("Getting funai node info...");
        let send_request = || {
            self.funai_node_client
    ...
    /// Get the reward set signers from the funai node for the given reward cycle
    pub fn get_reward_set_signers_with_retry(
        ...
        let funais_response = response.json::<GetFunaisResponse>()?;
        Ok(funais_response.funai_set.signers)
    }

    /// Retreive the current pox data from the funai node
    ...
    /// Helper function to retrieve the burn tip height from the funai node
    ...
    /// Get the current reward cycle info from the funai node
    ...
    /// Helper function to retrieve the account info from the funai node for a specific address
    ...
    /// Helper function to create a funai transaction for a modifying contract call
    ...
    pub fn submit_transaction_with_retry(
        ...
        let send_request = || {
            self.funai_node_client
                ...
                .send()
                .map_err(|e| {
                    debug!("Failed to submit transaction to the Funai node: {e:?}");
    ...
    /// Makes a read only contract call to a funai contract
    pub fn read_only_contract_call(
        ...
        let body =
            json!({"sender": self.funai_address.to_string(), "arguments": args}).to_string();
    ...
    fn accounts_path(&self, funai_address: &FunaiAddress) -> String {
        format!("{}/v2/accounts/{funai_address}?proof=0", self.http_origin)
    }

    fn reward_set_path(&self, reward_cycle: u64) -> String {
        format!("{}/v2/funai_set/{reward_cycle}", self.http_origin)
    }
    ...
    pub fn build_signed_contract_call_transaction(
        ...
        let public_key = FunaiPublicKey::from_private(funai_private_key);
        ...
        let mut unsigned_tx = FunaiTransaction::new(tx_version, tx_auth, tx_payload);
...
mod tests {
    ...
    fn transaction_contract_call_should_send_bytes_to_node() {
        ...
        let tx = FunaiClient::build_signed_contract_call_transaction(
            &mock.client.funai_address,
            ContractName::from("contract-name"),
            ClarityName::from("function-name"),
            &[],
    ...
    fn get_account_nonce_should_succeed() {
        let mock = MockServerClient::new();
        let address = mock.client.funai_address;
    ...
    fn get_node_epoch_should_succeed() {
        ...
        assert_eq!(epoch, FunaiEpochId::Epoch24);
        ...
        write_response(mock.server, peer_response.as_bytes());
        let epoch = h.join().unwrap().expect("Failed to deserialize response");
        assert_eq!(epoch, FunaiEpochId::Epoch25);
        ...
        write_response(mock.server, peer_response.as_bytes());
        let epoch = h.join().unwrap().expect("Failed to deserialize response");
        assert_eq!(epoch, FunaiEpochId::Epoch30);
    ...
    fn get_reward_set_should_succeed() {
        ...
        let funais_response = GetFunaisResponse {
            funai_set: funai_set.clone(),
        };

        let funais_response_json = serde_json::to_string(&funais_response)
            .expect("Failed to serialize get funai response");
        let response = format!("HTTP/1.1 200 OK\n\n{funais_response_json}");
        let h = spawn(move || mock.client.get_reward_set_signers_with_retry(0));
        write_response(mock.server, response.as_bytes());
        assert_eq!(h.join().unwrap().unwrap(), funai_set.signers);
    }
    ...
    fn get_vote_for_aggregate_public_key_should_succeed() {
        let mock = MockServerClient::new();
        let point = Point::from(Scalar::random(&mut rand::thread_rng()));
        let funai_address = mock.client.funai_address;
        let key_response = build_get_vote_for_aggregate_key_response(Some(point));
        let h = spawn(move || {
            mock.client
                .get_vote_for_aggregate_public_key(0, 0, funai_address)
        });
