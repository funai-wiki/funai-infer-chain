...
use crate::client::{retry_with_exponential_backoff, ClientError, FunaiClient};
...
impl From<GlobalConfig> for RunLoop {
    ...
    fn from(config: GlobalConfig) -> Self {
        ...
        Self {
            config,
            funai_client,
            funai_signers: HashMap::with_capacity(2),
...
impl RunLoop {
    ...
    /// Get the funaidb signer slots for a specific reward cycle
    pub fn get_parsed_signer_slots(
        &self,
        funai_client: &FunaiClient,
        reward_cycle: u64,
    ) -> Result<HashMap<FunaiAddress, SignerSlotID>, ClientError> {
        let signer_set =
            u32::try_from(reward_cycle % 2).expect("FATAL: reward_cycle % 2 exceeds u32::MAX");
        let signer_funaidb_contract_id =
            boot_code_id(SIGNERS_NAME, self.config.network.is_mainnet());
        // Get the signer writers from the funai-db to find the signer slot id
        let funaidb_signer_slots =
            funai_client.get_funaidb_signer_slots(&signer_funaidb_contract_id, signer_set)?;
        let mut signer_slot_ids = HashMap::with_capacity(funaidb_signer_slots.len());
        for (index, (address, _)) in funaidb_signer_slots.into_iter().enumerate() {
    ...
    /// Get a signer configuration for a specific reward cycle from the funai node
    fn get_signer_config(&mut self, reward_cycle: u64) -> Option<SignerConfig> {
        ...
        let signer_slot_ids = self
            .get_parsed_signer_slots(&self.funai_client, reward_cycle)
            .ok()?;
        let current_addr = self.funai_client.get_signer_address();

        let Some(signer_slot_id) = signer_slot_ids.get(current_addr) else {
            warn!(
                    "Signer {current_addr} was not found in funai db. Must not be registered for this reward cycle {reward_cycle}."
                );
            return None;
        };
        let Some(signer_id) = signer_entries.signer_ids.get(current_addr) else {
            warn!(
                "Signer {current_addr} was found in funai db but not the reward set for reward cycle {reward_cycle}."
            );
            return None;
        };
        info!(
    ...
    fn initialize_runloop(&mut self) -> Result<(), ClientError> {
        ...
        if self.funai_signers.is_empty() {
    ...
    fn cleanup_stale_signers(&mut self, current_reward_cycle: u64) {
        ...
        for (idx, signer) in &mut self.funai_signers {
...
impl SignerRunLoop<Vec<OperationResult>, RunLoopCommand> for RunLoop {
    ...
    fn run_one_pass(
        ...
        for signer in self.funai_signers.values_mut() {
            ...
            if signer.approved_aggregate_public_key.is_none() {
                if let Err(e) = retry_with_exponential_backoff(|| {
                    signer
                        .update_dkg(&self.funai_client, current_reward_cycle)
                        .map_err(backoff::Error::transient)
                }) {
            ...
            signer.process_next_command(&self.funai_client, current_reward_cycle);
        }
        None
    }
}
#[cfg(test)]
mod tests {
    use funailib::chainstate::funai::boot::NakamotoSignerEntry;
    ...
    fn parse_nakamoto_signer_entries_test() {
        ...
        for _ in 0..nmb_signers {
            let key = FunaiPublicKey::from_private(&FunaiPrivateKey::new()).to_bytes_compressed();
