...
use funailib::util_lib::signed_structured_data::pox4::Pox4SignatureTopic;
...
/// The CLI arguments for the funai signer
pub struct Cli {
...
/// Subcommands for the funai signer binary
#[derive(clap::Subcommand, Debug)]
pub enum Command {
    /// Get a chunk from the funai-db instance
    GetChunk(GetChunkArgs),
    /// Get the latest chunk from the funai-db instance
    GetLatestChunk(GetLatestChunkArgs),
    /// List chunks from the funai-db instance
    ListChunks(FunaiDBArgs),
    /// Upload a chunk to the funai-db instance
    PutChunk(PutChunkArgs),
    /// Run DKG and sign the message through the funai-db instance
    DkgSign(SignArgs),
    /// Sign the message through the funai-db instance
    Sign(SignArgs),
    /// Run a DKG round through the funai-db instance
    Dkg(RunDkgArgs),
    /// Run the signer, waiting for events from the funai-db instance
    Run(RunSignerArgs),
    ...
    /// Run the inference transaction service to receive and process inference transactions
    RunInferenceService(RunInferenceServiceArgs),
}

/// Basic arguments for all cyrptographic and funai-db functionality
#[derive(Parser, Debug, Clone)]
pub struct FunaiDBArgs {
    /// The Funai node to connect to
    #[arg(long)]
    pub host: String,
    /// The funai-db contract to use. Must be in the format of "STACKS_ADDRESS.CONTRACT_NAME"
...
pub struct PutChunkArgs {
    ...
    #[clap(flatten)]
    pub db_args: FunaiDBArgs,
    /// The Funai private key to use in hexademical format
...
pub struct GenerateFilesArgs {
    ...
    /// A path to a file containing a list of hexadecimal Funai private keys of the signers
    ...
    pub dir: PathBuf,
    /// The number of milliseconds to wait when polling for events from the funai-db instance.
    #[arg(long)]
...
/// Parse the hexadecimal Funai private key
fn parse_private_key(private_key: &str) -> Result<FunaiPrivateKey, String> {
    FunaiPrivateKey::from_hex(private_key).map_err(|e| format!("Invalid private key: {}", e))
}
...
#[derive(Parser, Debug, Clone)]
/// Arguments for the RunInferenceService command
pub struct RunInferenceServiceArgs {
    /// Path to config file
    #[arg(long, short, value_name = "FILE")]
    pub config: PathBuf,
    ...
    pub api_host: String,
    /// Path to SQLite database file for storing inference tasks
    #[arg(long, value_name = "DB_FILE")]
    pub database: Option<PathBuf>,
    /// Enable debug logging for the inference service
...
#[cfg(test)]
mod tests {
    use funailib::chainstate::funai::address::{PoxAddressType20, PoxAddressType32};
    use funailib::util_lib::signed_structured_data::pox4::make_pox_4_signer_key_message_hash;
    ...
    fn test_parse_pox_addr() {
        ...
        match pox_addr {
            PoxAddress::Standard(funai_addr, hash_mode) => {
                assert_eq!(funai_addr.version, 22);
        ...
        match pox_addr {
            PoxAddress::Standard(funai_addr, hash_mode) => {
                assert_eq!(funai_addr.version, 20);
        ...
        match pox_addr {
            PoxAddress::Standard(funai_addr, hash_mode) => {
                assert_eq!(funai_addr.version, C32_ADDRESS_VERSION_TESTNET_SINGLESIG);
                assert_eq!(hash_mode, Some(AddressHashMode::SerializeP2PKH));
            }
            _ => panic!("Invalid parsed address"),
        }
