...
use funai_common::codec::{
    read_next, read_next_at_most, write_next, Error as codec_error, FunaiMessageCodec,
    MAX_MESSAGE_LEN,
};
...
impl FunaiMessageCodec for InferLPString {
...
impl InferLPString {
    pub fn from_str(s: &str) -> Option<InferLPString> {
        if s.len() > CLARITY_MAX_STRING_LENGTH as usize {
            return None;
        }
        Some(InferLPString(s.as_bytes().to_vec()))
    }
}
...
impl fmt::Display for FunaiString {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_str(String::from_utf8_lossy(&self).into_owned().as_str())
    }
}

impl fmt::Debug for FunaiString {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(String::from_utf8_lossy(&self).into_owned().as_str())
    }
}

impl Deref for FunaiString {
    type Target = Vec<u8>;
...
impl DerefMut for FunaiString {
    fn deref_mut(&mut self) -> &mut Vec<u8> {
        &mut self.0
    }
}

impl FunaiMessageCodec for FunaiString {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        write_next(fd, &self.0)
    }

    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiString, codec_error> {
        let bytes: Vec<u8> = {
        ...
        let s = String::from_utf8(bytes.clone()).map_err(|_e| {
            warn!("Invalid FunaiString -- could not build from utf8");
            codec_error::DeserializeError(
                "Invalid Funai string: could not build from utf8".to_string(),
            )
        })?;

        if !FunaiString::is_valid_string(&s) {
            // non-printable ASCII or not ASCII
            warn!("Invalid FunaiString -- non-printable ASCII or non-ASCII");
            return Err(codec_error::DeserializeError(
                "Invalid Funai string: non-printable or non-ASCII string".to_string(),
            ));
        }

        Ok(FunaiString(bytes))
    }
}

impl FunaiMessageCodec for UrlString {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
...
impl From<ClarityName> for FunaiString {
    fn from(clarity_name: ClarityName) -> FunaiString {
        // .unwrap() is safe since FunaiString is less strict
        FunaiString::from_str(&clarity_name).unwrap()
    }
}

impl From<ContractName> for FunaiString {
    fn from(contract_name: ContractName) -> FunaiString {
        // .unwrap() is safe since FunaiString is less strict
        FunaiString::from_str(&contract_name).unwrap()
    }
}
...
impl FunaiString {
    ...
    pub fn is_valid_string(s: &String) -> bool {
        s.is_ascii() && FunaiString::is_printable(s)
    }
    ...
    pub fn from_string(s: &String) -> Option<FunaiString> {
        if !FunaiString::is_valid_string(s) {
            return None;
        }
        Some(FunaiString(s.as_bytes().to_vec()))
    }

    pub fn from_str(s: &str) -> Option<FunaiString> {
        if !FunaiString::is_valid_string(&String::from(s)) {
            return None;
        }
        Some(FunaiString(s.as_bytes().to_vec()))
    }
...
mod test {
    ...
    #[test]
    fn tx_funai_strings_codec() {
        let s = "hello-world";
        let funai_str = FunaiString::from_str(&s).unwrap();
        let clarity_str = ClarityName::try_from(s).unwrap();
        let contract_str = ContractName::try_from(s).unwrap();

        assert_eq!(funai_str[..], s.as_bytes().to_vec()[..]);
        let s2 = funai_str.to_string();
        assert_eq!(s2.to_string(), s.to_string());

        // funai strings have a 4-byte length prefix
        let mut b = vec![];
        funai_str.consensus_serialize(&mut b).unwrap();
        let mut bytes = vec![0x00, 0x00, 0x00, s.len() as u8];
        bytes.extend_from_slice(s.as_bytes());

        check_codec_and_corruption::<FunaiString>(&funai_str, &bytes);
    ...
    #[test]
    fn tx_funai_string_invalid() {
        let s = "hello\rworld";
        assert!(FunaiString::from_str(&s).is_none());
