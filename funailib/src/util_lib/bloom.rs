...
impl FunaiMessageCodec for BitField {
...
impl FunaiMessageCodec for BloomNodeHasher {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
...
impl FunaiMessageCodec for BloomFilter<BloomNodeHasher> {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        write_next(fd, &(BloomHashID::BloomNodeHasher as u8))?;
...
/// (e.g. the mempool).
#[derive(Debug, Clone, PartialEq)]
pub struct BloomCounter<H: BloomHash + Clone + FunaiMessageCodec> {
    hasher: H,
    table_name: String,
    num_bins: u32,
    num_hashes: u32,
...
impl<H: BloomHash + Clone + FunaiMessageCodec> BloomCounter<H> {
