...
use funai_common::types::chainstate::{SortitionId, FunaiAddress, FunaiBlockId, TrieHash};
...
use crate::chainstate::funai::index::marf::{MarfConnection, MarfTransaction, MARF};
use crate::chainstate::funai::index::{Error as MARFError, MARFValue, MarfTrieId};
...
impl FromRow<FunaiAddress> for FunaiAddress {
    fn from_row<'a>(row: &'a Row) -> Result<FunaiAddress, Error> {
        let addr_str: String = row.get(0)?;
        let addr = FunaiAddress::from_string(&addr_str).ok_or(Error::ParseError)?;
        Ok(addr)
    }
}
...
impl_byte_array_from_column_only!(FunaiBlockId);
    ...
    ($thing:ident) => {
        impl rusqlite::types::FromSql for $thing {
            fn column_result(
                ...
                let byte_str = funai_common::util::hash::hex_bytes(hex_str)
