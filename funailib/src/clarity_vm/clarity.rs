...
use funai_common::consts::{CHAIN_ID_TESTNET, SIGNER_SLOTS_PER_USER};
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, SortitionId, FunaiAddress, FunaiBlockId, TrieHash,
};
...
use crate::chainstate::funai::db::{FunaiAccount, FunaiChainState};
use crate::chainstate::funai::events::{FunaiTransactionEvent, FunaiTransactionReceipt};
use crate::chainstate::funai::index::marf::MARF;
use crate::chainstate::funai::index::{ClarityMarfTrieId, MarfTrieId};
use crate::chainstate::funai::{
    Error as ChainstateError, SinglesigHashMode, SinglesigSpendingCondition,
    FunaiMicroblockHeader, FunaiTransaction, TransactionAuth, TransactionPayload,
...
use crate::core::{FunaiEpoch, FunaiEpochId, FIRST_STACKS_BLOCK_ID, GENESIS_EPOCH};
...
/// completion of the Clarity operations in a Funai block from the
...
impl From<ChainstateError> for Error {
    fn from(e: ChainstateError) -> Self {
        match e {
            ChainstateError::InvalidFunaiTransaction(msg, _) => Error::BadTransaction(msg),
...
impl ClarityInstance {
    ...
    /// Returns the Funai epoch of the burn block that elected `funai_block`
    fn get_epoch_of(
        funai_block: &FunaiBlockId,
        header_db: &dyn HeadersDB,
        burn_state_db: &dyn BurnStateDB,
    ) -> FunaiEpoch {
        // Special case the first Funai block -- it is not elected in any burn block
        //  so we specifically set its epoch to GENESIS_EPOCH.
        if funai_block == &*FIRST_STACKS_BLOCK_ID {
            return burn_state_db
                .get_funai_epoch_by_epoch_id(&GENESIS_EPOCH)
                .expect("Failed to obtain the Genesis FunaiEpoch");
        }

        let burn_height = header_db
            .get_burn_block_height_for_block(funai_block)
            .unwrap_or_else(|| panic!("Failed to get burn block height of {}", funai_block));
        burn_state_db
            .get_funai_epoch(burn_height)
            .unwrap_or_else(|| panic!("Failed to get Funai epoch for height = {}", burn_height))
    }

    pub fn begin_block<'a, 'b>(
        &'a mut self,
        current: &FunaiBlockId,
        next: &FunaiBlockId,
        header_db: &'b dyn HeadersDB,
        burn_state_db: &'b dyn BurnStateDB,
    ) -> ClarityBlockConnection<'a, 'b> {
    ...
    pub fn begin_test_genesis_block_2_1<'a, 'b>(
        ...
        let epoch = FunaiEpochId::Epoch21;
    ...
    pub fn drop_unconfirmed_state(&mut self, block: &FunaiBlockId) -> Result<(), Error> {
    ...
    /// Open a read-only connection at `at_block`. This will be evaluated in the Funai epoch that
    ///  was active *during* the evaluation of `at_block`
    pub fn read_only_connection<'a>(
        &'a mut self,
        at_block: &FunaiBlockId,
        header_db: &'a dyn HeadersDB,
        burn_state_db: &'a dyn BurnStateDB,
    ...
    pub fn trie_exists_for_block(&mut self, bhh: &FunaiBlockId) -> Result<bool, DatabaseError> {
        let mut datastore = self.datastore.begin_read_only(None);
        datastore.trie_exists_for_block(bhh)
    }

    /// Evaluate program read-only at `at_block`. This will be evaluated in the Funai epoch that
    ///  was active *during* the evaluation of `at_block`
...
impl<'a, 'b> ClarityBlockConnection<'a, 'b> {
    ...
    pub fn precommit_to_block(self, final_bhh: FunaiBlockId) -> PreCommitClarityBlock<'a> {
        self.cost_track
    ...
    /// time of opening).
    pub fn commit_to_block(self, final_bhh: &FunaiBlockId) -> LimitedCostTracker {
    ...
    pub fn commit_mined_block(self, bhh: &FunaiBlockId) -> Result<LimitedCostTracker, Error> {
    ...
    fn get_boot_code_account(&mut self) -> Result<FunaiAccount, Error> {
    ...
    pub fn initialize_epoch_2_05(&mut self) -> Result<FunaiTransactionReceipt, Error> {
        ...
        using!(self.cost_track, "cost tracker", |old_cost_tracker| {
            ...
            let payload = TransactionPayload::SmartContract(
                TransactionSmartContract {
                    ...
                    code_body: FunaiString::from_str(cost_2_code)
            ...
            let costs_2_contract_tx =
                FunaiTransaction::new(tx_version.clone(), boot_code_auth.clone(), payload);

            let initialization_receipt = self.as_transaction(|tx_conn| {
                // bump the epoch in the Clarity DB
                tx_conn
                    .with_clarity_db(|db| {
                        db.set_clarity_epoch_version(FunaiEpochId::Epoch2_05)?;
                        Ok(())
                    })
                    .unwrap();
    ...
    pub fn initialize_epoch_2_1(&mut self) -> Result<Vec<FunaiTransactionReceipt>, Error> {
        // use the `using!` statement to ensure that the old cost_tracker is placed
        //  back in all branches after initialization
        using!(self.cost_track, "cost tracker", |old_cost_tracker| {
            // epoch initialization is *free*.
            ...
            let payload = TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: ContractName::try_from(POX_2_NAME)
                        .expect("FATAL: invalid boot-code contract name"),
                    code_body: FunaiString::from_str(pox_2_code)
                        .expect("FATAL: invalid boot code body"),
                },
            ...
            // features
            self.epoch = FunaiEpochId::Epoch21;
            let pox_2_initialization_receipt = self.as_transaction(|tx_conn| {
                ...
                tx_conn.epoch = FunaiEpochId::Epoch21;
            ...
            let payload = TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: ContractName::try_from(COSTS_3_NAME)
                        .expect("FATAL: invalid boot-code contract name"),
                    code_body: FunaiString::from_str(cost_3_code)
                        .expect("FATAL: invalid boot code body"),
                },
                None,
            );
    ...
    pub fn initialize_epoch_2_2(&mut self) -> Result<Vec<FunaiTransactionReceipt>, Error> {
        // use the `using!` statement to ensure that the old cost_tracker is placed
        //  back in all branches after initialization
        using!(self.cost_track, "cost tracker", |old_cost_tracker| {
            // epoch initialization is *free*.
            // NOTE: this also means that cost functions won't be evaluated.
            self.cost_track.replace(LimitedCostTracker::new_free());
            self.epoch = FunaiEpochId::Epoch22;
            self.as_transaction(|tx_conn| {
                // bump the epoch in the Clarity DB
                tx_conn
                    .with_clarity_db(|db| {
                        db.set_clarity_epoch_version(FunaiEpochId::Epoch22)?;
                        Ok(())
                    })
                    .unwrap();

                // require 2.2 rules henceforth in this connection as well
                tx_conn.epoch = FunaiEpochId::Epoch22;
            });
    ...
    pub fn initialize_epoch_2_3(&mut self) -> Result<Vec<FunaiTransactionReceipt>, Error> {
        // use the `using!` statement to ensure that the old cost_tracker is placed
        //  back in all branches after initialization
        using!(self.cost_track, "cost tracker", |old_cost_tracker| {
            // epoch initialization is *free*.
            ...
            self.epoch = FunaiEpochId::Epoch23;
            self.as_transaction(|tx_conn| {
                // bump the epoch in the Clarity DB
                tx_conn
                    .with_clarity_db(|db| {
                        db.set_clarity_epoch_version(FunaiEpochId::Epoch23)?;
                        Ok(())
                    })
                    .unwrap();

                // require 2.3 rules henceforth in this connection as well
                tx_conn.epoch = FunaiEpochId::Epoch23;
            });
    ...
    pub fn initialize_epoch_2_4(&mut self) -> Result<Vec<FunaiTransactionReceipt>, Error> {
        // use the `using!` statement to ensure that the old cost_tracker is placed
        //  back in all branches after initialization
        using!(self.cost_track, "cost tracker", |old_cost_tracker| {
            // epoch initialization is *free*.
            // NOTE: this also means that cost functions won't be evaluated.
            self.cost_track.replace(LimitedCostTracker::new_free());
            self.epoch = FunaiEpochId::Epoch24;
            self.as_transaction(|tx_conn| {
                // bump the epoch in the Clarity DB
                tx_conn
                    .with_clarity_db(|db| {
                        db.set_clarity_epoch_version(FunaiEpochId::Epoch24)?;
                        Ok(())
                    })
                    .unwrap();

                // require 2.4 rules henceforth in this connection as well
                tx_conn.epoch = FunaiEpochId::Epoch24;
            });
            ...
            let payload = TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: ContractName::try_from(POX_3_NAME)
                        .expect("FATAL: invalid boot-code contract name"),
                    code_body: FunaiString::from_str(pox_3_code)
                        .expect("FATAL: invalid boot code body"),
                },
                Some(ClarityVersion::Clarity2),
            );
    ...
    pub fn initialize_epoch_2_5(&mut self) -> Result<Vec<FunaiTransactionReceipt>, Error> {
        // use the `using!` statement to ensure that the old cost_tracker is placed
        //  back in all branches after initialization
        using!(self.cost_track, "cost tracker", |old_cost_tracker| {
            // epoch initialization is *free*.
            // NOTE: this also means that cost functions won't be evaluated.
            self.cost_track.replace(LimitedCostTracker::new_free());
            self.epoch = FunaiEpochId::Epoch25;
            self.as_transaction(|tx_conn| {
                // bump the epoch in the Clarity DB
                tx_conn
                    .with_clarity_db(|db| {
                        db.set_clarity_epoch_version(FunaiEpochId::Epoch25)?;
                        Ok(())
                    })
                    .unwrap();

                // require 3.0 rules henceforth in this connection as well
                tx_conn.epoch = FunaiEpochId::Epoch25;
            });
            ...
            let payload = TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: ContractName::try_from(POX_4_NAME)
                        .expect("FATAL: invalid boot-code contract name"),
                    code_body: FunaiString::from_str(pox_4_code)
                        .expect("FATAL: invalid boot code body"),
                },
                Some(ClarityVersion::Clarity2),
            );
            ...
            let payload = TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: ContractName::try_from(SIGNERS_NAME)
                        .expect("FATAL: invalid boot-code contract name"),
                    code_body: FunaiString::from_str(SIGNERS_BODY)
                        .expect("FATAL: invalid boot code body"),
                },
                Some(ClarityVersion::Clarity2),
            );
            ...
            // funaidb contracts for each message type
                    ...
                    let payload = TransactionPayload::SmartContract(
                        TransactionSmartContract {
                            name: ContractName::try_from(signers_name.clone())
                                .expect("FATAL: invalid boot-code contract name"),
                            code_body: FunaiString::from_str(body)
                                .expect("FATAL: invalid boot code body"),
                        },
                        Some(ClarityVersion::Clarity2),
                    );
            ...
            let payload = TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: ContractName::try_from(SIGNERS_VOTING_NAME)
                        .expect("FATAL: invalid boot-code contract name"),
                    code_body: FunaiString::from_str(SIGNERS_VOTING_BODY)
                        .expect("FATAL: invalid boot code body"),
                },
                Some(ClarityVersion::Clarity2),
            );
    ...
    pub fn initialize_epoch_3_0(&mut self) -> Result<Vec<FunaiTransactionReceipt>, Error> {
        // use the `using!` statement to ensure that the old cost_tracker is placed
        //  back in all branches after initialization
        using!(self.cost_track, "cost tracker", |old_cost_tracker| {
            // epoch initialization is *free*.
            // NOTE: this also means that cost functions won't be evaluated.
            self.cost_track.replace(LimitedCostTracker::new_free());
            self.epoch = FunaiEpochId::Epoch30;
            self.as_transaction(|tx_conn| {
                // bump the epoch in the Clarity DB
                tx_conn
                    .with_clarity_db(|db| {
                        db.set_clarity_epoch_version(FunaiEpochId::Epoch30)?;
                        Ok(())
                    })
                    .unwrap();

                // require 3.0 rules henceforth in this connection as well
                tx_conn.epoch = FunaiEpochId::Epoch30;
            });
    ...
    #[cfg(test)]
    pub fn set_epoch(&mut self, epoch_id: FunaiEpochId) {
        self.epoch = epoch_id;
    }
}
...
impl<'a, 'b> TransactionConnection for ClarityTransactionConnection<'a, 'b> {
    fn with_abort_callback<F, A, R, E>(
        ...
        to_do: F,
        abort_call_back: A,
    ) -> Result<(R, AssetMap, Vec<FunaiTransactionEvent>, bool), E>
    where
        A: FnOnce(&AssetMap, &mut ClarityDatabase) -> bool,
        F: FnOnce(&mut OwnedEnvironment) -> Result<(R, AssetMap, Vec<FunaiTransactionEvent>), E>,
                ...
                vm_env
                    .execute_in_env(sender.clone(), None, None, |env| {
                        env.run_as_transaction(|env| {
                            FunaiChainState::handle_poison_microblock(
                                env,
                                mblock_header_1,
                                mblock_header_2,
                            )
...
mod tests {
    ...
    use funai_common::types::chainstate::ConsensusHash;
    ...
    pub fn simple_test() {
        ...
        let mut conn = marf.begin_read_only(Some(&FunaiBlockId([1 as u8; 32])));
    ...
    pub fn test_block_roll_back() {
        ...
        let mut conn = marf.begin(&FunaiBlockId::sentinel(), &FunaiBlockId([0 as u8; 32]));
    ...
    fn test_unconfirmed() {
        ...
        let mut marf = clarity_instance.destroy();
        let mut conn = marf.begin_unconfirmed(&FunaiBlockId([0 as u8; 32]));
    ...
    pub fn test_post_condition_failure_contract_publish() {
        ...
        let mut tx1 = FunaiTransaction::new(
            TransactionVersion::Mainnet,
            TransactionAuth::Standard(spending_cond.clone()),
            TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: "hello-world".into(),
                    code_body: FunaiString::from_str(contract).unwrap(),
                },
                None,
            )
            .into(),
        ...
        let mut tx3 = FunaiTransaction::new(
            TransactionVersion::Mainnet,
            TransactionAuth::Standard(spending_cond.clone()),
    ...
    pub fn test_block_limit() {
        ...
        impl BurnStateDB for BlockLimitBurnStateDB {
            ...
            fn get_funai_epoch(&self, _height: u32) -> Option<FunaiEpoch> {
                // Note: We return this FunaiEpoch for every input, because this test is not exercising
                // this method.
                Some(FunaiEpoch {
