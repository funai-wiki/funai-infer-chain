...
use crate::chainstate::funai::db::{
    ChainstateTx, MinerPaymentSchedule, FunaiChainState, FunaiHeaderInfo,
};
use crate::chainstate::funai::index::marf::{MarfConnection, MARF};
use crate::chainstate::funai::index::{ClarityMarfTrieId, MarfTrieId, TrieMerkleProof};
use crate::chainstate::funai::Error as ChainstateError;
use crate::clarity_vm::special::handle_contract_call_special_cases;
use crate::core::{FunaiEpoch, FunaiEpochId};
...
impl<'a> HeadersDB for HeadersDBConn<'a> {
    fn get_funai_block_header_hash_for_block(
        ...
        get_funai_header_column(self.0, id_bhh, "block_hash", |r| {
    ...
    fn get_burn_header_hash_for_block(
        ...
        get_funai_header_column(self.0, id_bhh, "burn_header_hash", |r| {
    ...
    fn get_consensus_hash_for_block(&self, id_bhh: &FunaiBlockId) -> Option<ConsensusHash> {
        get_funai_header_column(self.0, id_bhh, "consensus_hash", |r| {
    ...
    fn get_burn_block_time_for_block(&self, id_bhh: &FunaiBlockId) -> Option<u64> {
        get_funai_header_column(self.0, id_bhh, "burn_header_timestamp", |r| {
    ...
    fn get_burn_block_height_for_block(&self, id_bhh: &FunaiBlockId) -> Option<u32> {
        get_funai_header_column(self.0, id_bhh, "burn_header_height", |r| {
            u64::from_row(r)
    ...
    fn get_vrf_seed_for_block(&self, id_bhh: &FunaiBlockId) -> Option<VRFSeed> {
        get_funai_header_column(self.0, id_bhh, "proof", |r| {
            let proof = VRFProof::from_column(r, "proof").expect("FATAL: malformed proof");
            VRFSeed::from_proof(&proof)
        })
    }

    fn get_miner_address(&self, id_bhh: &FunaiBlockId) -> Option<FunaiAddress> {
        get_miner_column(self.0, id_bhh, "address", |r| {
            let s: String = r.get_unwrap("address");
            let addr = FunaiAddress::from_string(&s).expect("FATAL: malformed address");
            addr
        })
    }
...
impl<'a> HeadersDB for ChainstateTx<'a> {
    fn get_funai_block_header_hash_for_block(
        ...
        get_funai_header_column(self.deref().deref(), id_bhh, "block_hash", |r| {
            BlockHeaderHash::from_column(r, "block_hash").expect("FATAL: malformed block hash")
        })
    }
    ...
    fn get_burn_header_hash_for_block(
        ...
        get_funai_header_column(self.deref().deref(), id_bhh, "burn_header_hash", |r| {
            BurnchainHeaderHash::from_row(r).expect("FATAL: malformed burn_header_hash")
        })
    }

    fn get_consensus_hash_for_block(&self, id_bhh: &FunaiBlockId) -> Option<ConsensusHash> {
        get_funai_header_column(self.deref().deref(), id_bhh, "consensus_hash", |r| {
            ConsensusHash::from_row(r).expect("FATAL: malformed consensus_hash")
        })
    }

    fn get_burn_block_time_for_block(&self, id_bhh: &FunaiBlockId) -> Option<u64> {
        get_funai_header_column(self.deref().deref(), id_bhh, "burn_header_timestamp", |r| {
            u64::from_row(r).expect("FATAL: malformed burn_header_timestamp")
        })
    }

    fn get_burn_block_height_for_block(&self, id_bhh: &FunaiBlockId) -> Option<u32> {
        get_funai_header_column(self.deref().deref(), id_bhh, "burn_header_height", |r| {
            u64::from_row(r)
                .expect("FATAL: malformed burn_header_height")
                .try_into()
                .expect("FATAL: blockchain too long")
    ...
    fn get_vrf_seed_for_block(&self, id_bhh: &FunaiBlockId) -> Option<VRFSeed> {
        get_funai_header_column(self.deref().deref(), id_bhh, "proof", |r| {
            let proof = VRFProof::from_column(r, "proof").expect("FATAL: malformed proof");
            VRFSeed::from_proof(&proof)
        })
    }
...
impl HeadersDB for MARF<FunaiBlockId> {
    fn get_funai_block_header_hash_for_block(
        &self,
        id_bhh: &FunaiBlockId,
    ) -> Option<BlockHeaderHash> {
        get_funai_header_column(self.sqlite_conn(), id_bhh, "block_hash", |r| {
            BlockHeaderHash::from_column(r, "block_hash").expect("FATAL: malformed block hash")
        })
    }
    ...
    fn get_burn_header_hash_for_block(
        ...
        get_funai_header_column(self.sqlite_conn(), id_bhh, "burn_header_hash", |r| {
            BurnchainHeaderHash::from_row(r).expect("FATAL: malformed burn_header_hash")
        })
    }

    fn get_consensus_hash_for_block(&self, id_bhh: &FunaiBlockId) -> Option<ConsensusHash> {
        get_funai_header_column(self.sqlite_conn(), id_bhh, "consensus_hash", |r| {
            ConsensusHash::from_row(r).expect("FATAL: malformed consensus_hash")
        })
    }

    fn get_burn_block_time_for_block(&self, id_bhh: &FunaiBlockId) -> Option<u64> {
        get_funai_header_column(self.sqlite_conn(), id_bhh, "burn_header_timestamp", |r| {
            u64::from_row(r).expect("FATAL: malformed burn_header_timestamp")
        })
    }

    fn get_burn_block_height_for_block(&self, id_bhh: &FunaiBlockId) -> Option<u32> {
        get_funai_header_column(self.sqlite_conn(), id_bhh, "burn_header_height", |r| {
            u64::from_row(r)
                .expect("FATAL: malformed burn_header_height")
                .try_into()
                .expect("FATAL: blockchain too long")
    ...
    fn get_vrf_seed_for_block(&self, id_bhh: &FunaiBlockId) -> Option<VRFSeed> {
        get_funai_header_column(self.sqlite_conn(), id_bhh, "proof", |r| {
            let proof = VRFProof::from_column(r, "proof").expect("FATAL: malformed proof");
            VRFSeed::from_proof(&proof)
        })
    }
...
fn get_funai_header_column<F, R>(
    conn: &DBConn,
    id_bhh: &FunaiBlockId,
    column_name: &str,
    loader: F,
...
fn get_matured_reward(conn: &DBConn, child_id_bhh: &FunaiBlockId) -> Option<MinerReward> {
    let parent_id_bhh = conn
        .query_row(
            "SELECT parent_block_id FROM block_headers WHERE index_block_hash = ?",
            [child_id_bhh].iter(),
            |x| {
                Ok(FunaiBlockId::from_column(x, "parent_block_id")
    ...
    if let Some(parent_id_bhh) = parent_id_bhh {
        FunaiChainState::get_matured_miner_payment(conn, &parent_id_bhh, child_id_bhh)
...
pub trait SortitionDBRef: BurnStateDB {
    fn get_pox_start_cycle_info(
        ...
        parent_funai_block_burn_ht: u64,
        cycle_index: u64,
...
fn get_pox_start_cycle_info(
    ...
    let descended_from_last_pox_anchor = match handle.get_last_anchor_block_hash()? {
        Some(pox_anchor) => handle.descended_from(parent_funai_block_burn_ht, &pox_anchor)?,
        None => return Ok(None),
    };
...
impl SortitionDBRef for SortitionHandleTx<'_> {
    fn get_pox_start_cycle_info(
        ...
        get_pox_start_cycle_info(&mut handle, parent_funai_block_burn_ht, cycle_index)
    }
...
impl BurnStateDB for SortitionHandleTx<'_> {
    ...
    fn get_funai_epoch_by_epoch_id(&self, epoch_id: &FunaiEpochId) -> Option<FunaiEpoch> {
        SortitionDB::get_funai_epoch_by_epoch_id(self.tx(), epoch_id)
            .expect("BUG: failed to get epoch for epoch id")
    }
    fn get_funai_epoch(&self, height: u32) -> Option<FunaiEpoch> {
        SortitionDB::get_funai_epoch(self.tx(), height as u64)
...
impl BurnStateDB for SortitionDBConn<'_> {
    ...
    fn get_funai_epoch_by_epoch_id(&self, epoch_id: &FunaiEpochId) -> Option<FunaiEpoch> {
        SortitionDB::get_funai_epoch_by_epoch_id(self.conn(), epoch_id)
            .expect("BUG: failed to get epoch for epoch id")
    }
    fn get_funai_epoch(&self, height: u32) -> Option<FunaiEpoch> {
        SortitionDB::get_funai_epoch(self.conn(), height as u64)
            .expect("BUG: failed to get epoch for burn block height")
    }
    fn get_pox_payout_addrs(
        &self,
...
impl ClarityBackingStore for MemoryBackingStore {
    fn set_block_hash(&mut self, bhh: FunaiBlockId) -> InterpreterResult<FunaiBlockId> {
    ...
    fn get_block_at_height(&mut self, height: u32) -> Option<FunaiBlockId> {
        if height == 0 {
            Some(FunaiBlockId::sentinel())
        } else {
            None
        }
    }

    fn get_open_chain_tip(&mut self) -> FunaiBlockId {
