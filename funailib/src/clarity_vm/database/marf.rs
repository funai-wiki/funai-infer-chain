...
use funai_common::types::chainstate::{BlockHeaderHash, FunaiBlockId, TrieHash};

use crate::chainstate::funai::index::marf::{MARFOpenOpts, MarfConnection, MarfTransaction, MARF};
...
pub struct MarfedKV {
    chain_tip: FunaiBlockId,
    marf: MARF<FunaiBlockId>,
}
...
impl MarfedKV {
    fn setup_db(
    ...
    ) -> InterpreterResult<MARF<FunaiBlockId>> {
        ...
        let mut marf: MARF<FunaiBlockId> = if unconfirmed {
    ...
    pub fn temporary() -> MarfedKV {
        ...
        let mut path = PathBuf::from_str("/tmp/funai-node-tests/unit-tests-marf").unwrap();
        ...
        let chain_tip = FunaiBlockId::sentinel();
    ...
    pub fn begin_unconfirmed<'a>(&'a mut self, current: &FunaiBlockId) -> WritableMarfStore<'a> {
        let mut tx = self.marf.begin_tx().unwrap_or_else(|_| {
            panic!(
    ...
    pub fn get_chain_tip(&self) -> &FunaiBlockId {
        &self.chain_tip
    }

    pub fn get_marf(&mut self) -> &mut MARF<FunaiBlockId> {
        &mut self.marf
    }
    ...
    pub fn index_conn<'a, C>(&'a self, context: C) -> IndexDBConn<'a, C, FunaiBlockId> {
        IndexDBConn {
            index: &self.marf,
            context,
        }
...
pub struct WritableMarfStore<'a> {
    chain_tip: FunaiBlockId,
    marf: MarfTransaction<'a, FunaiBlockId>,
}

pub struct ReadOnlyMarfStore<'a> {
    chain_tip: FunaiBlockId,
    marf: &'a mut MARF<FunaiBlockId>,
}
...
impl<'a> ClarityBackingStore for ReadOnlyMarfStore<'a> {
    ...
    fn get_current_block_height(&mut self) -> u32 {
            ...
            .get_block_height_of(&self.chain_tip, &self.chain_tip)
        {
            ...
            Ok(None) => {
                ...
                if self.chain_tip == first_tip || self.chain_tip == FunaiBlockId([0u8; 32]) {
    ...
    fn get_block_at_height(&mut self, block_height: u32) -> Option<FunaiBlockId> {
        self.marf
...
impl<'a> WritableMarfStore<'a> {
    ...
    pub fn commit_to(self, final_bhh: &FunaiBlockId) -> InterpreterResult<()> {
    ...
    pub fn commit_mined_block(self, will_move_to: &FunaiBlockId) -> InterpreterResult<()> {
        debug!(
            "commit_mined_block: ({}->{})",
