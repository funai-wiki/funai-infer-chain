...
fn helper_execute(contract: &str, method: &str) -> (Value, Vec<FunaiTransactionEvent>) {
    helper_execute_epoch(contract, method, None, FunaiEpochId::Epoch21, false)
}

fn helper_execute_epoch(
    contract: &str,
    method: &str,
    set_epoch: Option<FunaiEpochId>,
    epoch: FunaiEpochId,
    use_mainnet: bool,
) -> (Value, Vec<FunaiTransactionEvent>) {
...
fn test_emit_print_ok() {
    ...
    match events.pop() {
        Some(FunaiTransactionEvent::SmartContractEvent(data)) => {
            let contract_id = QualifiedContractIdentifier::local("contract").unwrap();
...
fn test_emit_stx_transfer_ok() {
    ...
    assert_eq!(events.len(), 1);
    match events.pop() {
        Some(FunaiTransactionEvent::STXEvent(STXEventType::STXTransferEvent(data))) => {
...
fn test_emit_stx_burn_ok() {
    ...
    assert_eq!(events.len(), 1);
    match events.pop() {
        Some(FunaiTransactionEvent::STXEvent(STXEventType::STXBurnEvent(data))) => {
            assert_eq!(data.amount, 10u128);
            assert_eq!(
                Value::Principal(data.sender),
                execute("'SZ2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKQ9H6DPR")
...
fn test_emit_ft_transfer_ok() {
    ...
    assert_eq!(events.len(), 1);
    match events.pop() {
        Some(FunaiTransactionEvent::FTEvent(FTEventType::FTTransferEvent(data))) => {
...
fn test_emit_ft_mint_ok() {
    ...
    assert_eq!(events.len(), 1);
    match events.pop() {
        Some(FunaiTransactionEvent::FTEvent(FTEventType::FTMintEvent(data))) => {
            let contract_identifier = QualifiedContractIdentifier::local("contract").unwrap();
            let asset_identifier = AssetIdentifier {
                contract_identifier,
                asset_name: "token".into(),
...
fn test_emit_nft_transfer_ok() {
    ...
    assert_eq!(events.len(), 1);
    match events.pop() {
        Some(FunaiTransactionEvent::NFTEvent(NFTEventType::NFTTransferEvent(data))) => {
            let contract_identifier = QualifiedContractIdentifier::local("contract").unwrap();
            let asset_identifier = AssetIdentifier {
                contract_identifier,
                asset_name: "token".into(),
...
fn test_emit_nft_mint_ok() {
    ...
    assert_eq!(events.len(), 1);
    match events.pop() {
        Some(FunaiTransactionEvent::NFTEvent(NFTEventType::NFTMintEvent(data))) => {
            let contract_identifier = QualifiedContractIdentifier::local("contract").unwrap();
            let asset_identifier = AssetIdentifier {
                contract_identifier,
                asset_name: "token".into(),
