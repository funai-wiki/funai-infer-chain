...
use crate::chainstate::funai::boot::contract_tests::{test_sim_height_to_hash, ClarityTestSim};
...
fn test_get_burn_block_info_eval() {
    ...
    // Advance another block so we get to Funai 2.05.
    ...
    // Advance another block so we get to Funai 2.1. This is the last block in 2.05
    sim.execute_next_block_as_conn(|conn| {
    ...
    // now in Funai 2.1, so this should work!
    sim.execute_next_block_as_conn(|conn| {
        ...
        // * burnchain is 100 blocks ahead of funai
        ...
        // burnchain is 100 blocks ahead of funai chain in this sim
        assert_eq!(
...
fn trait_invocation_205_with_stored_principal() {
    ...
    // Advance two blocks so we get to Funai 2.05.
    sim.execute_next_block(|_env| {});
    sim.execute_next_block(|_env| {});
...
fn trait_invocation_cross_epoch() {
    ...
    let sender = FunaiAddress::burn_address(false).into();
    ...
    // should now be in Funai 2.3, so the invocation should work again!
    info!("Sim height = {}", sim.height);
    sim.execute_next_block_as_conn(|conn| {
        let epoch = conn.get_epoch();
        conn.as_transaction(|clarity_db| {
