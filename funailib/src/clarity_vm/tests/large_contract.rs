...
use crate::chainstate::funai::boot::{BOOT_CODE_COSTS, BOOT_CODE_COSTS_2, BOOT_CODE_COSTS_3};
...
fn test_block_headers(n: u8) -> FunaiBlockId {
    FunaiBlockId([n as u8; 32])
}
...
fn test_simple_token_system(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    ...
    F: FnOnce(&mut OwnedEnvironment, ClarityVersion) -> (),
{
    ...
    let mut owned_env = OwnedEnvironment::new(marf_kv.as_clarity_db(), FunaiEpochId::latest());
...
#[apply(test_clarity_versions)]
fn test_simple_naming_system(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
...
#[apply(test_clarity_versions)]
pub fn let_memory_test(#[case] clarity_version: ClarityVersion, #[case] epoch_id: FunaiEpochId) {
    let marf = MarfedKV::temporary();
    let mut clarity_instance = ClarityInstance::new(false, CHAIN_ID_TESTNET, marf);
    let EXPLODE_N = 100;
    let burn_db = &generate_test_burn_state_db(epoch_id);
...
#[apply(test_clarity_versions)]
pub fn fcall_memory_test(#[case] clarity_version: ClarityVersion, #[case] epoch_id: FunaiEpochId) {
    let marf = MarfedKV::temporary();
    let mut clarity_instance = ClarityInstance::new(false, CHAIN_ID_TESTNET, marf);
...
#[apply(test_clarity_versions)]
pub fn ccall_memory_test(#[case] clarity_version: ClarityVersion, #[case] epoch_id: FunaiEpochId) {
    let marf = MarfedKV::temporary();
    let mut clarity_instance = ClarityInstance::new(false, CHAIN_ID_TESTNET, marf);
...
fn test_deep_tuples() {
        ...
        .enumerate()
    {
        ...
        if *version == ClarityVersion::Clarity2 {
            block.set_epoch(FunaiEpochId::Epoch21);
        } else {
            block.set_epoch(FunaiEpochId::Epoch2_05);
        }
        ...
        let error = block.as_transaction(|tx| {
            if *version == ClarityVersion::Clarity2 {
                assert_eq!(tx.get_epoch(), FunaiEpochId::Epoch21);
            } else {
                assert_eq!(tx.get_epoch(), FunaiEpochId::Epoch2_05);
            }
