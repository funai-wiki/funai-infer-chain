...
fn test_tracked_costs(
...

    {
        let mut conn = clarity_instance.begin_block(
            &FunaiBlockId([3 as u8; 32]),
            &FunaiBlockId([4 + prog_id as u8; 32]),
            &TEST_HEADER_DB,
            &burn_state_db,
        );
...
fn epoch_21_test_all(use_mainnet: bool, version: ClarityVersion) {
    let mut instance = setup_tracked_cost_test(use_mainnet, FunaiEpochId::Epoch21, version);

    let baseline = test_tracked_costs("1", FunaiEpochId::Epoch21, version, 0, &mut instance);
    ...
    for (ix, f) in NativeFunctions::ALL.iter().enumerate() {
        ...
        let cost = test_tracked_costs(test, FunaiEpochId::Epoch21, version, ix + 1, &mut instance);
