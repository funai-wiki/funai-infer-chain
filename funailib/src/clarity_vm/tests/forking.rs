...
fn test_forking_simple(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
...
#[apply(test_clarity_versions)]
fn test_at_block_mutations(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
...
#[apply(test_clarity_versions)]
fn test_at_block_good(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    fn initialize(owned_env: &mut OwnedEnvironment) {
        let c = QualifiedContractIdentifier::local("contract").unwrap();
        let contract =
            "(define-data-var datum int 1)
...
#[apply(test_clarity_versions)]
fn test_at_block_missing_defines(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    ...
    F3: FnOnce(&mut OwnedEnvironment),
{
    ...
    {
        let mut store = marf_kv.begin(&FunaiBlockId::sentinel(), &FunaiBlockId([0 as u8; 32]));
        store
    ...
    {
        let mut store = marf_kv.begin(&FunaiBlockId([0 as u8; 32]), &FunaiBlockId([1 as u8; 32]));
    ...
    {
        let mut store = marf_kv.begin(&FunaiBlockId([1 as u8; 32]), &FunaiBlockId([3 as u8; 32]));
        let mut owned_env = OwnedEnvironment::new(
            store.as_clarity_db(&TEST_HEADER_DB, &TEST_BURN_STATE_DB),
            epoch,
        );
    ...
    {
        let mut store = marf_kv.begin(&FunaiBlockId([2 as u8; 32]), &FunaiBlockId([4 as u8; 32]));
        let mut owned_env = OwnedEnvironment::new(
            store.as_clarity_db(&TEST_HEADER_DB, &TEST_BURN_STATE_DB),
            epoch,
        );
