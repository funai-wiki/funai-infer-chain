...
use funai_common::types::chainstate::{
    BurnchainHeaderHash, ConsensusHash, PoxId, FunaiAddress, TrieHash,
};
pub use funai_common::types::{Address, PrivateKey, PublicKey};
...
impl PoxConstants {
    ...
    /// return false if not.
    pub fn has_pox_sunset(epoch_id: FunaiEpochId) -> bool {
        epoch_id < FunaiEpochId::Epoch21
    }

    /// Returns true if PoX has been fully disabled by the PoX sunset.
    /// Behavior is epoch-specific
    pub fn is_after_pox_sunset_end(&self, burn_block_height: u64, epoch_id: FunaiEpochId) -> bool {
    ...
    /// * `first_block_ht` - the first burn block height that the Funai network monitored
