...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, PoxId, SortitionId, FunaiAddress, TrieHash, VRFSeed,
};
...
fn test_process_block_ops() {
    ...
    for scenario_idx in 0..block_ops_124_possibilities.len() {
        ...
        let mut block_124_snapshot = BlockSnapshot {
            ...
            winning_funai_block_hash: block_124_winners[scenario_idx].block_header_hash.clone(),
        ...
        // TODO: pair up with funai chain state?
        /*
        let winning_header_hashes = {
            let mut tx = db.tx_begin().unwrap();
            BurnDB::get_funai_block_header_inventory(&mut tx, 124).unwrap()
                .iter()
...
fn test_burn_snapshot_sequence() {
    ...
    let mut all_funai_block_hashes = vec![];
    ...
    for i in 0..32 {
        ...
        if i > 0 {
            ...
            all_funai_block_hashes.push(next_block_commit.block_header_hash.clone());
