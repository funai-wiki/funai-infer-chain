...
use funai_common::deps_common::bitcoin::blockdata::transaction::Transaction as BtcTx;
...
use crate::core::{FunaiEpochId, BITCOIN_REGTEST_FIRST_BLOCK_HASH};
...
fn test_store_and_fetch() {
    ...
    for (ix, tx_fixture) in fixtures.iter().enumerate() {
        ...
        let burnchain_tx = parser
            .parse_tx(&tx, ix + 1, FunaiEpochId::Epoch2_05)
            .unwrap();
...
fn test_classify_stack_stx() {
    ...
    let processed_ops_0 = burnchain_db
        .store_new_burnchain_block(&burnchain, &headers, &block_0, FunaiEpochId::Epoch21)
        .unwrap();
    ...
    let processed_ops_1 = burnchain_db
        .store_new_burnchain_block(&burnchain, &headers, &block_1, FunaiEpochId::Epoch21)
        .unwrap();
    ...
    let expected_pre_stack_addr =
        FunaiAddress::from_legacy_bitcoin_address(&LegacyBitcoinAddress {
            addrtype: LegacyBitcoinAddressType::PublicKeyHash,
            network_id: BitcoinNetworkType::Mainnet,
            bytes: Hash160([1; 20]),
        });
...
fn test_classify_delegate_stx() {
    ...
    let expected_pre_delegate_addr = FunaiAddress::from_legacy_bitcoin_address(
    ...
    if let BlockstackOperationType::DelegateStx(op) = &processed_ops_1[0] {
        ...
        assert_eq!(op.delegate_to, FunaiAddress::new(22, Hash160([2u8; 20])));
        assert_eq!(&op.reward_addr, &expected_reward_addr);
