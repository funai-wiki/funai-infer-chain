...
use funai_common::types::chainstate::{BlockHeaderHash, SortitionId, VRFSeed};
...
use crate::funai_common::deps_common::bitcoin::network::serialize::BitcoinHash;
...
pub struct TestMiner {
    pub burnchain: Burnchain,
    pub privks: Vec<FunaiPrivateKey>,
    pub num_sigs: u16,
    pub hash_mode: AddressHashMode,
    pub microblock_privks: Vec<FunaiPrivateKey>,
...
impl TestMiner {
    ...
    pub fn next_microblock_privkey(&mut self) -> FunaiPrivateKey {
        let pk = if self.microblock_privks.len() == 0 {
            ...
            let h = Sha256Sum::from_data(&buf[..]);
            FunaiPrivateKey::from_slice(h.as_bytes()).unwrap()
        } else {
    ...
    pub fn origin_address(&self) -> Option<FunaiAddress> {
    ...
    pub fn sign_as_origin(&mut self, tx_signer: &mut FunaiTransactionSigner) -> () {
    ...
    pub fn sign_as_sponsor(&mut self, tx_signer: &mut FunaiTransactionSigner) -> () {
        let num_keys = if self.privks.len() < self.num_sigs as usize {
            self.privks.len()
        } else {
            self.num_sigs as usize
...
impl TestMinerFactory {
    ...
    pub fn next_private_key(&mut self) -> FunaiPrivateKey {
...
fn process_next_sortition(
    ...
    if prev_keys.len() > 0 {
        ...
        // make a Funai block (hash) for each of the prior block's keys
...
#[test]
fn mine_10_funai_blocks_1_fork() {
...
#[test]
fn mine_10_funai_blocks_2_forks_disjoint() {
    let mut node = TestBurnchainNode::new();
    let mut miner_factory = TestMinerFactory::new();
...
#[test]
fn mine_10_funai_blocks_2_forks_disjoint_same_blocks() {
    let mut node = TestBurnchainNode::new();
    let mut miner_factory = TestMinerFactory::new();
    ...
    // This tests that we can accept two burnchain forks that each contain the same funai
    // block history.
    for i in 5..10 {
        let mut next_block_hashes_1 = vec![];
        for j in 0..miners_1.len() {
