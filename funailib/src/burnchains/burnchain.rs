...
use funai_common::deps_common::bitcoin::util::hash::Sha256dHash as BitcoinSha256dHash;
use funai_common::types::chainstate::{BurnchainHeaderHash, PoxId, FunaiAddress, TrieHash};
...
use crate::chainstate::funai::boot::{POX_2_MAINNET_CODE, POX_2_TESTNET_CODE};
use crate::chainstate::funai::FunaiPublicKey;
use crate::core::{
    FunaiEpoch, FunaiEpochId, MINING_COMMITMENT_WINDOW, NETWORK_ID_MAINNET, NETWORK_ID_TESTNET,
...
impl BurnchainStateTransition {
    ...
    pub fn from_block_ops(
        ...
        let epoch_id = SortitionDB::get_funai_epoch(sort_tx, parent_snapshot.block_height + 1)?
            .unwrap_or_else(|| {
                panic!(
...
impl BurnchainSigner {
    ...
    pub fn mock_parts(
        ...
        num_sigs: usize,
        public_keys: Vec<FunaiPublicKey>,
    ) -> BurnchainSigner {
    ...
    #[cfg(any(test, feature = "testing"))]
    pub fn new_p2pkh(pubk: &FunaiPublicKey) -> BurnchainSigner {
...
impl Burnchain {
    ...
    pub fn default_unittest(
        ...
        let tmp_path = format!("/tmp/funai-node-tests/unit-tests-{}", &to_hex(&byte_tail));
    ...
    ///   *only* to be used by legacy funai node interfaces, like the Helium node
    pub fn process_block_and_sortition_deprecated<B: BurnchainHeaderReader>(
        ...
        let cur_epoch = SortitionDB::get_funai_epoch(db.conn(), block.block_height())?
            .unwrap_or_else(|| {
                panic!(
        ...
        let parse_thread: thread::JoinHandle<Result<(), burnchain_error>> =
            thread::spawn(move || {
                while let Ok(Some(ipc_block)) = parser_recv.recv() {
                    ...
                    let cur_epoch =
                        SortitionDB::get_funai_epoch(parser_sortdb.conn(), ipc_block.height())?
                            .unwrap_or_else(|| {
                                panic!("FATAL: no funai epoch defined for {}", ipc_block.height())
                            });
        ...
        I: BurnchainIndexer + BurnchainHeaderReader + 'static + Send,
    {
        ...
        let epochs = {
            ...
            SortitionDB::get_funai_epochs(sortdb.conn())?
        };
        ...
        let parse_thread: thread::JoinHandle<Result<(), burnchain_error>> = thread::Builder::new()
            ...
            .spawn(move || {
                while let Ok(Some(ipc_block)) = parser_recv.recv() {
                    ...
                    let cur_epoch =
                        SortitionDB::get_funai_epoch(sortdb.conn(), ipc_block.height())?
                            .unwrap_or_else(|| {
                                panic!("FATAL: no funai epoch defined for {}", ipc_block.height())
                            });
            ...
            thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    while let Ok(Some(burnchain_block)) = db_recv.recv() {
                        ...
                        let epoch_index = FunaiEpoch::find_epoch(&epochs, block_height)
                            .unwrap_or_else(|| {
