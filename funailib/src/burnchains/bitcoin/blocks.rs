...
use funai_common::deps_common::bitcoin::blockdata::block::{Block, LoneBlockHeader};
use funai_common::deps_common::bitcoin::blockdata::opcodes::All as btc_opcodes;
use funai_common::deps_common::bitcoin::blockdata::script::{Instruction, Script};
use funai_common::deps_common::bitcoin::blockdata::transaction::Transaction;
use funai_common::deps_common::bitcoin::network::message as btc_message;
use funai_common::deps_common::bitcoin::network::serialize::BitcoinHash;
use funai_common::deps_common::bitcoin::util::hash::bitcoin_merkle_root;
...
impl BitcoinBlockParser {
    ...
    /// Allow raw inputs?
    fn allow_raw_inputs(epoch_id: FunaiEpochId) -> bool {
        epoch_id >= FunaiEpochId::Epoch21
    }

    /// Allow segwit outputs?
    fn allow_segwit_outputs(epoch_id: FunaiEpochId) -> bool {
        epoch_id >= FunaiEpochId::Epoch21
    }
    ...
    fn maybe_burnchain_tx(&self, tx: &Transaction, epoch_id: FunaiEpochId) -> bool {
    ...
    /// (this is the behavior of Funai 2.05 and earlier)
    ...
    /// (this is the behavior of Funai 2.1 and later)
...
mod tests {
    use funai_common::deps_common::bitcoin::blockdata::block::{Block, LoneBlockHeader};
    use funai_common::deps_common::bitcoin::blockdata::transaction::Transaction;
    use funai_common::deps_common::bitcoin::network::encodable::VarInt;
    use funai_common::deps_common::bitcoin::network::serialize::deserialize;
    use funai_common::types::chainstate::BurnchainHeaderHash;
    ...
    fn maybe_burnchain_tx_test_2_05() {
        ...
        for tx_fixture in tx_fixtures {
            ...
            let res = parser.maybe_burnchain_tx(&tx, FunaiEpochId::Epoch2_05);
    ...
    fn parse_tx_test_2_05() {
        ...
        for tx_fixture in tx_fixtures {
            let tx = make_tx(&tx_fixture.txstr).unwrap();
            let burnchain_tx = parser.parse_tx(&tx, vtxindex as usize, FunaiEpochId::Epoch2_05);
    ...
    fn parse_tx_test_2_1() {
        ...
        for tx_fixture in tx_fixtures {
            ...
            let tx = make_tx(&tx_fixture.txstr).unwrap();
            let burnchain_tx = parser.parse_tx(&tx, vtxindex as usize, FunaiEpochId::Epoch21);
            assert!(burnchain_tx.is_some());
            assert_eq!(burnchain_tx, tx_fixture.result);
        }
    }
    ...
    fn parse_block() {
        ...
        for block_fixture in block_fixtures {
            ...
            let parsed_block_opt =
                parser.process_block(&block, &header, height, FunaiEpochId::Epoch2_05);
