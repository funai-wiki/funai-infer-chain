...
// Copyright (C) 2020-2021 Funai Open Internet Foundation
//
...
/// ## The Problem
///
/// There were two related design flaws in the way the Funai blockchain deals with PoX anchor blocks:
///
...
/// processes Funai blocks first without the anchor block in _R_ and then with the anchor block in _R_ will crash
...
/// history.  In doing so, the Funai blockchain now has _three_ levels of forks: the Bitcoin chain, the history of PoX
/// anchor blocks, and the history of Funai blocks.  The canonical Funai fork is the longest history of Funai blocks
...
/// unique history of block-commits (which in turn represent a unique set of possible Funai forks).  This set has
...
/// an ancestor.
///
/// Of course, there are competing block-commits in each Bitcoin block; only one will be chosen as the Funai block.
...
///
/// **The PoX anchor block for reward cycle _R_ is a Funai block that has not yet been chosen to be an anchor block,
...
/// BTC.  If this block-commit doesn't actually correspond to a Funai block, then there is no anchor block for the
/// reward cycle.  Also, if this block-commit has been an anchor block before in some prior reward cycle, then there is
/// no anchor block for this reward cycle.  If Funai miners are honest, and no Funai miner has more than 80% of the
/// mining power, then neither of these two cases arise -- Funai miners will build Funai blocks on top of blocks they
...
/// bootstrapping node does not know which Funai blocks actually exist, since it needs to go and actually download
...
/// block-commit's Funai block confirms.  When processing block-commits, the node will calculate the affirmation map
...
/// re-validated Funai blocks, so they can be downloaded and applied again to the Funai chain state (note that a
/// Funai block will be applied at most once in any case -- it's just that it can be an orphan on one sortition
...
use crate::chainstate::funai::FunaiBlockHeader;
