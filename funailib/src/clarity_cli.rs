...
use serde::Serialize;
use serde_json::json;
use funai_common::address::c32::c32_address;
...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, ConsensusHash, FunaiAddress, FunaiBlockId, VRFSeed, *,
};
use funai_common::util::hash::{bytes_to_hex, Hash160, Sha512Trunc256Sum};
use funai_common::util::{get_epoch_time_ms, log};
...
use crate::core::{FunaiEpochId, BLOCK_LIMIT_MAINNET_205, HELIUM_BLOCK_LIMIT_20};
...
where command is one of:
  ...
  generate_address   to generate a random Funai public address for testing purposes.
",
        invoked_by
    );
    panic_test!()
...
pub const DEFAULT_CLI_EPOCH: FunaiEpochId = FunaiEpochId::Epoch25;
...
fn get_cli_chain_tip(conn: &Connection) -> FunaiBlockId {
    ...
    while let Some(row) = rows.next().expect("FATAL: could not read block hash") {
        let bhh = friendly_expect(
            FunaiBlockId::from_column(&row, "block_hash"),
...
fn get_cli_block_height(conn: &Connection, block_id: &FunaiBlockId) -> Option<u64> {
    let mut stmt = friendly_expect(
    ...
    F: FnOnce(WritableMarfStore) -> (WritableMarfStore, R),
{
    ...
    let to = FunaiBlockId([2u8; 32]); // 0x0202020202 ... (pattern not used anywhere else)
...
{
    // store CLI data alongside the MARF database state
    let from = FunaiBlockId::from_hex(blockhash)
...
impl CLIHeadersDB {
    ...
    pub fn advance_cli_chain_tip(&mut self) -> (FunaiBlockId, FunaiBlockId) {
        let tx = friendly_expect(
            self.conn.transaction(),
        ...
        let next_block_hash = friendly_expect_opt(
            FunaiBlockId::from_bytes(&random_bytes),
