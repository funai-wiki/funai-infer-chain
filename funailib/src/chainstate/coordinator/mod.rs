...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, PoxId, SortitionId, FunaiBlockId,
};
use funai_common::util::get_epoch_time_secs;

pub use self::comm::CoordinatorCommunication;
use super::funai::boot::{RewardSet, RewardSetData};
use super::funai::db::blocks::DummyEventDispatcher;
...
use crate::chainstate::funai::address::PoxAddress;
use crate::chainstate::funai::boot::{POX_3_NAME, POX_4_NAME};
use crate::chainstate::funai::db::accounts::MinerReward;
use crate::chainstate::funai::db::{
    ChainStateBootData, ClarityTx, MinerRewardInfo, FunaiChainState, FunaiEpochReceipt,
    FunaiHeaderInfo,
};
use crate::chainstate::funai::events::{
    FunaiBlockEventData, FunaiTransactionEvent, FunaiTransactionReceipt, TransactionOrigin,
};
use crate::chainstate::funai::index::marf::MARFOpenOpts;
use crate::chainstate::funai::index::MarfTrieId;
use crate::chainstate::funai::miner::{signal_mining_blocked, signal_mining_ready, MinerStatus};
use crate::chainstate::funai::{
    Error as ChainstateError, FunaiBlock, FunaiBlockHeader, TransactionPayload,
};
use crate::core::{
    FunaiEpoch, FunaiEpochId, FIRST_BURNCHAIN_CONSENSUS_HASH, FIRST_STACKS_BLOCK_HASH,
};
...
/// Indicates whether or not we're ready to process Funai blocks, or if not, whether or not we're
/// blocked on a Funai 2.x anchor block or a Nakamoto anchor block
pub enum NewBurnchainBlockStatus {
    /// Ready to process Funai blocks
    Ready,
...
pub trait BlockEventDispatcher {
    fn announce_block(
        &self,
        block: &FunaiBlockEventData,
        metadata: &FunaiHeaderInfo,
        receipts: &[FunaiTransactionReceipt],
        parent: &FunaiBlockId,
        winner_txid: Txid,
...
> {
    ...
    pub chain_state_db: FunaiChainState,
    ...
    burnchain_indexer: B,
    /// Used to tell the P2P thread that the funaidb
...
impl<'a, T: BlockEventDispatcher> RewardSetProvider for OnChainRewardSetProvider<'a, T> {
    fn get_reward_set(
        ...
        let cur_epoch = SortitionDB::get_funai_epoch(sortdb.conn(), cycle_start_burn_height)?
        ...
        let is_nakamoto_reward_set = match SortitionDB::get_funai_epoch_by_epoch_id(
            sortdb.conn(),
...
impl<'a, T: BlockEventDispatcher> OnChainRewardSetProvider<'a, T> {
    fn get_reward_set_epoch2(
        ...
        match cur_epoch.epoch_id {
            ...
            FunaiEpochId::Epoch22 | FunaiEpochId::Epoch23 => {
        ...
        let (threshold, participation) = FunaiChainState::get_reward_threshold_and_participation(
        ...
        Ok(FunaiChainState::make_reward_set(
            threshold,
            registered_addrs,
            cur_epoch.epoch_id,
        ))
    ...
    >
{
    pub fn run(
        config: ChainsCoordinatorConfig,
        chain_state_db: FunaiChainState,
        burnchain: Burnchain,
        dispatcher: &'a T,
        ...
        T: BlockEventDispatcher,
    {
        let funai_blocks_processed = comms.funai_blocks_processed.clone();
        ...
        let mut inst = ChainsCoordinator {
            ...
            config,
            burnchain_indexer,
            refresh_funai_db: comms.refresh_funai_db.clone(),
    ...
    /// This is the Funai 2.x coordinator loop body, which handles communications
    ...
    pub fn handle_comms_epoch2(
        ...
        if (bits & (CoordinatorEvents::NEW_STACKS_BLOCK as u8)) != 0 {
            ...
            debug!("Received new funai block notice");
            match self.handle_new_funai_block() {
                ...
                Err(e) => {
                    warn!("Error processing new funai block: {:?}", e);
                }
            }
            ...
            match self.handle_new_burnchain_block() {
                Ok(burn_block_status) => match burn_block_status {
                    ...
                    NewBurnchainBlockStatus::WaitForPox2x(block_hash) => {
                        debug!("Missing canonical Funai 2.x anchor block {}", &block_hash,);
                    }
    ...
    ChainsCoordinator<'a, T, (), U, (), (), B>
{
    ...
    pub fn test_new_full(
        ...
        let burnchain_blocks_db =
            BurnchainDB::open(&burnchain.get_burnchaindb_path(), false).unwrap();
        let (chain_state_db, _) = FunaiChainState::open_and_exec(
            false,
            chain_id,
        ...
        ChainsCoordinator {
            ...
            burnchain_indexer,
            refresh_funai_db: Arc::new(AtomicBool::new(false)),
            in_nakamoto_epoch: false,
        }
    }
}
...
pub fn get_reward_cycle_info<U: RewardSetProvider>(
    ...
    let epoch_at_height = SortitionDB::get_funai_epoch(sort_db.conn(), burn_height)?
    ...
    let reward_cycle_info = {
        ...
        let burnchain_db_conn_opt =
            if epoch_at_height.epoch_id >= FunaiEpochId::Epoch21 || always_use_affirmation_maps {
    ...
    let reward_cycle_info = if let Some((consensus_hash, funai_block_hash, txid)) =
        reward_cycle_info
    {
        let anchor_block_known = FunaiChainState::is_funai_block_processed(
            &chain_state.db(),
            &consensus_hash,
            &funai_block_hash,
        )?;
        info!(
            ...
            "block_hash" => %funai_block_hash,
            "block_id" => %FunaiBlockId::new(&consensus_hash, &funai_block_hash),
            "is_known" => anchor_block_known,
            "commit_txid" => %txid,
            "cycle_burn_height" => burn_height
        );
        let anchor_status = if anchor_block_known {
            let block_id = FunaiBlockId::new(&consensus_hash, &funai_block_hash);
            let reward_set =
                provider.get_reward_set(burn_height, chain_state, burnchain, sort_db, &block_id)?;
            PoxAnchorBlockStatus::SelectedAndKnown(funai_block_hash, txid, reward_set)
        } else {
            PoxAnchorBlockStatus::SelectedAndUnknown(funai_block_hash, txid)
        };
        RewardCycleInfo {
            reward_cycle,
            anchor_status,
...
/// Forget that all Funai blocks that were mined on descendants of `burn_header` are orphaned.
/// They may be valid again, after a PoX reorg.
fn forget_orphan_funai_blocks(
    sort_conn: &DBConn,
...
pub fn static_get_canonical_affirmation_map<B: BurnchainHeaderReader>(
    ...
    let canonical_am = FunaiChainState::find_canonical_affirmation_map(
        burnchain,
        indexer,
        burnchain_blocks_db,
        chain_state_db,
...
fn inner_static_get_funai_tip_affirmation_map(
    burnchain_blocks_db: &BurnchainDB,
    last_2_05_rc: u64,
    sort_am: &AffirmationMap,
    sortdb_conn: &DBConn,
    ...
    let funai_am = FunaiChainState::find_funai_tip_affirmation_map(
        burnchain_blocks_db,
        sortdb_conn,
        canonical_ch,
        canonical_bhh,
...
/// Get the canonical Funai tip affirmation map, when considering epochs.
/// * In epoch 2.05 and prior, the heaviest AM was the sortition AM.
/// * In epoch 2.1, the reward cycles prior to the 2.1 boundary remain the sortition AM
pub fn static_get_funai_tip_affirmation_map(
    burnchain_blocks_db: &BurnchainDB,
    sortition_db: &SortitionDB,
    sortition_tip: &SortitionId,
    canonical_ch: &ConsensusHash,
    ...
    let sort_am = sortition_db.find_sortition_tip_affirmation_map(sortition_tip)?;
    inner_static_get_funai_tip_affirmation_map(
        burnchain_blocks_db,
        last_2_05_rc,
        &sort_am,
        sortition_db.conn(),
...
{
    /// Process new Funai blocks.  If we get stuck for want of a missing PoX anchor block, return
    /// its hash.
    pub fn handle_new_funai_block(&mut self) -> Result<Option<BlockHeaderHash>, Error> {
        debug!("Handle new Funai block");
    ...
    /// Find the canonical Funai tip at a given sortition, whose affirmation map is compatible
    /// with the heaviest affirmation map.
    fn find_highest_funai_block_with_compatible_affirmation_map(
        ...
        let mut search_height = FunaiChainState::get_max_header_height(chainstate_conn)?;
        ...
        loop {
            let mut search_weight = FunaiChainState::get_max_affirmation_weight_at_height(
                chainstate_conn,
                search_height,
            )? as i64;
            while search_weight >= 0 {
                let all_headers = FunaiChainState::get_all_headers_at_height_and_weight(
                    chainstate_conn,
                    search_height,
                ...
                for hdr in all_headers {
                    ...
                    let am = match inner_static_get_funai_tip_affirmation_map(
                        burnchain_db,
                        last_2_05_rc,
                        &sort_am,
                        sort_tx,
                        ...
                        Err(Error::ChainstateError(ChainstateError::DBError(
                            ...
                            debug!(
                                "Funai tip {}/{} is not on a valid sortition",
                                &hdr.consensus_hash,
                                &hdr.anchored_header.block_hash()
                            );
                            continue;
                    ...
                    match FunaiChainState::is_block_compatible_with_affirmation_map(
                        &am,
                        heaviest_am,
                    ) {
                        Ok(compat) => {
                            if !compat {
                                debug!("Funai tip {}/{} affirmation map {} is incompatible with heaviest affirmation map {}",
                        ...
                        Err(ChainstateError::DBError(DBError::InvalidPoxSortition)) => {
                            debug!(
                                "Funai tip {}/{} affirmation map {} is not on a valid sortition",
                                &hdr.consensus_hash,
                                &hdr.anchored_header.block_hash(),
                                &am
                            );
                    ...
                    let ancestor_sn = match SortitionDB::get_ancestor_snapshot_tx(
                        ...
                        Ok(None) | Err(DBError::InvalidPoxSortition) => {
                            debug!("Funai tip {}/{} affirmation map {} is not on a chain tipped by sortition {}",
                    ...
                    if !ancestor_sn.sortition
                        || ancestor_sn.winning_funai_block_hash != hdr.anchored_header.block_hash()
                        || ancestor_sn.consensus_hash != hdr.consensus_hash
                    {
                        debug!(
                            "Funai tip {}/{} affirmation map {} is not attched to {},{}",
                            &hdr.consensus_hash,
                            &hdr.anchored_header.block_hash(),
                            &am,
                    ...
                    // found it!
                    debug!(
                        "Canonical Funai tip of {} is now {}/{} height {} burn height {} AM `{}` weight {}",
                        sort_tip,
                        &hdr.consensus_hash,
                        &hdr.anchored_header.block_hash(),
                        hdr.funai_block_height,
    ...
    /// funai DB indicate?  This checks both the affirmation map represented by the Funai chain
    /// tip and the affirmation map represented by the sortition tip against the heaviest
    /// affirmation map.  Both checks are necessary, because both Funai and sortition state may
    /// need to be invalidated in order to process the new set of sortitions and Funai blocks that
    ...
    fn check_chainstate_against_burnchain_affirmations(&self) -> Result<Option<u64>, Error> {
        ...
        let (canonical_ch, canonical_bhh) =
            SortitionDB::get_canonical_funai_chain_tip_hash(self.sortition_db.conn())?;
        ...
        let funai_tip_affirmation_map = static_get_funai_tip_affirmation_map(
            &self.burnchain_blocks_db,
            &self.sortition_db,
            &sortition_tip,
            &canonical_ch,
        ...
        debug!(
            "Heaviest anchor block affirmation map is `{}` at height {}, Funai tip is `{}`, sortition tip is `{}`, canonical is `{}`",
            &heaviest_am,
        ...
        let funai_changed_reward_cycle_opt = {
            if heaviest_am.len() <= funai_tip_affirmation_map.len() {
                funai_tip_affirmation_map.find_divergence(&heaviest_am)
            } else {
                heaviest_am.find_divergence(&funai_tip_affirmation_map)
            }
        };
        ...
        let lowest_changed_reward_cycle_opt = match (
            funai_changed_reward_cycle_opt,
        ...
        if let Some(changed_reward_cycle) = lowest_changed_reward_cycle_opt {
            ...
            if changed_reward_cycle < current_reward_cycle {
                info!("Sortition anchor block affirmation map `{}` and/or Funai affirmation map `{}` is no longer compatible with heaviest affirmation map {} in reward cycles {}-{}",
                      &sortition_tip_affirmation_map, &funai_tip_affirmation_map, &heaviest_am, changed_reward_cycle, current_reward_cycle);
    ...
    fn find_invalid_and_revalidated_sortitions(
        ...
        // burn chain height at which we'll re-try orphaned Funai blocks, and
    ...
    /// Forget that funai blocks for now-invalidated sortitions are orphaned, because they might
    /// now be valid.  In particular, this applies to a Funai block that got mined in two PoX
    ...
    /// as valid once again.  In addition, it updates the Funai chainstate DB such that any Funai
    ...
    /// the sortition DB.
    ///
    /// Note that regardless of the affirmation map status, a Funai block will remain processed
    /// once it gets accepted.  Its underlying sortition may become invalidated, in which case, the
    /// Funai block would no longer be considered as part of the canonical Funai fork (since the
    /// canonical Funai chain tip must reside on a valid sortition).  However, a Funai block that
    ...
    /// be treated as invalid.  This is what necessitates retrying Funai blocks that have been
    ...
    fn handle_affirmation_reorg(&mut self) -> Result<(), Error> {
        // find the funai chain's affirmation map
        let canonical_burnchain_tip = self.burnchain_blocks_db.get_canonical_chain_tip()?;
            ...
            self.check_chainstate_against_burnchain_affirmations()?
        {
            ...
            // be invalidated.  Any orphaned Funai blocks in this range will be forgotten, so they
                ...
                match self.find_invalid_and_revalidated_sortitions(
                    ...
                    None => {
                        ...
                        if let Some(x) = revalidation_params {
                            ...
                            // Just update the canonical funai block pointer on the highest valid
                            // sortition.
                            let last_2_05_rc =
                            ...
                            let (canonical_ch, canonical_bhh, canonical_height) =
                                Self::find_highest_funai_block_with_compatible_affirmation_map(
                                    &heaviest_am,
                                    &sortition_tip,
                                    &self.burnchain_blocks_db,
                                    &mut sort_tx,
                            ...
                            let funai_am = inner_static_get_funai_tip_affirmation_map(
                                &self.burnchain_blocks_db,
                                last_2_05_rc,
                            ...
                            debug!("Canonical Funai tip for highest valid sortition {} ({}) is {}/{} height {} am `{}`", &sortition_tip, sortition_height, &canonical_ch, &canonical_bhh, canonical_height, &funai_am);
            ...
            debug!("Invalidate all descendants of {} (after height {}), revalidate some sortitions at and after height {}, and retry all orphaned Funai blocks at or after height {}",
            ...
            let mut funai_blocks_to_unorphan = vec![];
            ...
            self.sortition_db.invalidate_descendants_with_closures(
                ...
                |_sort_tx, burn_header, _invalidate_queue| {
                    ...
                    funai_blocks_to_unorphan.push((burn_header.clone(), invalidation_height));
                },
                |sort_tx| {
                    // no more sortitions to invalidate -- all now-incompatible
                    // sortitions have been invalidated.
                    let (canonical_ch, canonical_bhh, canonical_height) = Self::find_highest_funai_block_with_compatible_affirmation_map(&heaviest_am, &highest_valid_sortition_id, &self.burnchain_blocks_db, sort_tx, &chainstate_db_conn)
                        .expect("FATAL: could not find a valid parent Funai block");

                    let funai_am = inner_static_get_funai_tip_affirmation_map(
                        &self.burnchain_blocks_db,
                        last_2_05_rc,
                        &sort_tx.find_sortition_tip_affirmation_map(&highest_valid_sortition_id).expect("FATAL: failed to query funai DB"),
                        sort_tx,
                        &canonical_ch,
                        &canonical_bhh
                    )
                    .expect("FATAL: failed to query funai DB");

                    debug!("Canonical Funai tip after invalidations is {}/{} height {} am `{}`", &canonical_ch, &canonical_bhh, canonical_height, &funai_am);

                    // Revalidate sortitions, and declare that we have their Funai blocks.
                    for valid_sn in valid_sortitions.iter() {
                        test_debug!("Revalidate snapshot {},{}", valid_sn.block_height, &valid_sn.sortition_id);
                        let block_known = FunaiChainState::is_funai_block_processed(
                            &chainstate_db_conn,
                            &valid_sn.consensus_hash,
                            &valid_sn.winning_funai_block_hash,
                    ...
                    // recalculate highest valid funai tip
                    let (canonical_ch, canonical_bhh, canonical_height) = Self::find_highest_funai_block_with_compatible_affirmation_map(&heaviest_am, &highest_valid_sortition_id, &self.burnchain_blocks_db, sort_tx, &chainstate_db_conn)
                        .expect("FATAL: could not find a valid parent Funai block");
                    ...
                    debug!("Canonical Funai tip after invalidations and revalidations is {}/{} height {} am `{}`", &canonical_ch, &canonical_bhh, canonical_height, &funai_am);
                    ...
                    for dirty_sort_id in dirty_snapshots.iter() {
                        ...
                        let block_known = FunaiChainState::is_funai_block_processed(
                            ...
                            &dirty_sort_sn.winning_funai_block_hash,
                        ).expect("FATAL: failed to query chainstate DB");
                    ...
                    // recalculate highest valid funai tip once more
                    let (canonical_ch, canonical_bhh, canonical_height) = Self::find_highest_funai_block_with_compatible_affirmation_map(&heaviest_am, &highest_valid_sortition_id, &self.burnchain_blocks_db, sort_tx, &chainstate_db_conn)
                        .expect("FATAL: could not find a valid parent Funai block");
                    ...
                    debug!("Canonical Funai tip after invalidations, revalidations, and processed dirty snapshots is {}/{} height {} am `{}`", &canonical_ch, &canonical_bhh, canonical_height, &funai_am);
                    ...
                    let block_known = FunaiChainState::is_funai_block_processed(
                        ...
                        &highest_valid_sn.winning_funai_block_hash,
                    ).expect("FATAL: failed to query chainstate DB");
            ...
            for (burn_header, invalidation_height) in funai_blocks_to_unorphan {
                // permit re-processing of any associated funai blocks if they're
                // orphaned
                forget_orphan_funai_blocks(
                    &ic,
            ...
            Self::undo_funai_block_orphaning(
                &self.burnchain_blocks_db.conn(),
                &ic,
                &mut chainstate_db_tx,
                first_invalidate_start_block,
            ...
            // view of the canonical funai chain tip can't get changed (since no
            // Funai blocks can be processed).
            chainstate_db_tx
                .commit()
            ...
            let funai_tip_affirmation_map = static_get_funai_tip_affirmation_map(
                ...
                &highest_valid_snapshot.canonical_funai_tip_consensus_hash,
                &highest_valid_snapshot.canonical_funai_tip_hash,
            )?;

            debug!(
                "Highest valid sortition (changed) is {} ({} in height {}, affirmation map {}); Funai tip is {}/{} height {} (affirmation map {}); heaviest AM is {}",
                &highest_valid_snapshot.sortition_id,
                ...
                &highest_valid_snapshot.canonical_funai_tip_consensus_hash,
                &highest_valid_snapshot.canonical_funai_tip_hash,
                highest_valid_snapshot.canonical_funai_tip_height,
                &funai_tip_affirmation_map,
                &heaviest_am
            );
            ...
            debug!(
                "Highest valid sortition (not changed) is {} ({} in height {}, affirmation map {}); Funai tip is {}/{} height {} (affirmation map {}); heaviest AM is {}",
                &highest_valid_snapshot.sortition_id,
                &highest_valid_snapshot.burn_header_hash,
                highest_valid_snapshot.block_height,
                &self.sortition_db.find_sortition_tip_affirmation_map(&highest_valid_snapshot.sortition_id)?,
    ...
    fn try_revalidate_sortition(
        ...
        if let Some(sortition) = sortition_opt {
            ...
            info!(
                ...
                &canonical_snapshot.canonical_funai_tip_consensus_hash,
                &canonical_snapshot.canonical_funai_tip_hash,
                canonical_snapshot.canonical_funai_tip_height,
            );
    ...
    fn check_missing_anchor_block(
        ...
        let cur_epoch =
            SortitionDB::get_funai_epoch(self.sortition_db.conn(), header.block_height)?
                .unwrap_or_else(|| {
                    panic!("BUG: no epoch defined at height {}", header.block_height)
                });

        if cur_epoch.epoch_id >= FunaiEpochId::Epoch21 || self.config.always_use_affirmation_maps {
            ...
            // presence.  This only gets checked in Funai 2.1 or later (unless overridden
            // in the config)
    ...
    /// Will call the Funai 2.x or Nakamoto handler, depending on whether or not
    /// Not called internally.
    pub fn handle_new_burnchain_block(&mut self) -> Result<NewBurnchainBlockStatus, Error> {
        let canonical_burnchain_tip = self.burnchain_blocks_db.get_canonical_chain_tip()?;
        let epochs = SortitionDB::get_funai_epochs(self.sortition_db.conn())?;
        let target_epoch_index =
            FunaiEpoch::find_epoch(&epochs, canonical_burnchain_tip.block_height)
                .expect("FATAL: epoch not defined for burnchain height");
        let target_epoch = epochs
            .get(target_epoch_index)
            .expect("FATAL: FunaiEpoch::find_epoch() returned an invalid index");
        if target_epoch.epoch_id < FunaiEpochId::Epoch30 {
        ...
        let target_epoch_index = FunaiEpoch::find_epoch(&epochs, canonical_snapshot.block_height)
    ...
    fn affirmation_maps_active(&self, epoch: &FunaiEpochId) -> bool {
    ...
    pub fn handle_new_epoch2_burnchain_block(
        ...
        let cur_epoch = SortitionDB::get_funai_epoch(
            self.sortition_db.conn(),
        ...
        // if this is set to true, the notify that a funai block has been processed.
        // this wakes up anyone waiting for their block to have been processed.
        let mut revalidated_funai_block = false;
        ...
        for unprocessed_block in sortitions_to_process.into_iter() {
            ...
            if sortition_epoch.epoch_id >= FunaiEpochId::Epoch30 {
                // stop processing
                break;
            }
            ...
            let mut funai_blocks_to_reaccept = vec![];
            ...
            let next_snapshot = {
                // if this sortition exists already, then revalidate it with the canonical Funai
                ...
                if let Some(sortition) = self.try_revalidate_sortition(
                    ...
                    if sortition.sortition {
                        if let Some(funai_block_header) =
                            FunaiChainState::get_funai_block_header_info_by_index_block_hash(
                                &self.chain_state_db.db(),
                                &FunaiBlockId::new(
                                    &sortition.consensus_hash,
                                    &sortition.winning_funai_block_hash,
                                ),
                            )?
                        {
                            // we accepted this block
                            debug!(
                                "Will re-accept Funai block {}/{} height {}",
                                &sortition.consensus_hash,
                                &sortition.winning_funai_block_hash,
                                funai_block_header.anchored_header.height(),
                            );
                            funai_blocks_to_reaccept.push((
                                sortition.consensus_hash.clone(),
                                sortition.winning_funai_block_hash.clone(),
                                funai_block_header.anchored_header.height(),
                            ));
                        } else {
                            debug!(
                                "Will un-orphan Funai block {}/{} if it is orphaned",
                                &sortition.consensus_hash, &sortition.winning_funai_block_hash
                            );
            ...
            let mut compatible_funai_blocks = vec![];
            {
                // get borrow checker to drop sort_tx
                let mut sort_tx = self.sortition_db.tx_begin()?;
                for (ch, bhh, height) in funai_blocks_to_reaccept.into_iter() {
                    debug!(
                        "Check if Funai block {}/{} height {} is compatible with `{}`",
                        &ch, &bhh, height, &heaviest_am
                    );

                    let am = inner_static_get_funai_tip_affirmation_map(
                        &self.burnchain_blocks_db,
                        last_2_05_rc,
                    ...
                    if FunaiChainState::is_block_compatible_with_affirmation_map(
                        &am,
                        &heaviest_am,
                    )? {
                        debug!(
                            "Funai block {}/{} height {} is compatible with `{}`; will reaccept",
                            &ch, &bhh, height, &heaviest_am
                        );
                        compatible_funai_blocks.push((ch, bhh, height));
                    } else {
                        debug!("Funai block {}/{} height {} is NOT compatible with `{}`; will NOT reaccept", &ch, &bhh, height, &heaviest_am);
                    }
                }
            }

            // reaccept any funai blocks
            let mut sortition_db_handle =
                SortitionHandleTx::begin(&mut self.sortition_db, &next_snapshot.sortition_id)?;

            for (ch, bhh, height) in compatible_funai_blocks.into_iter() {
                debug!("Re-accept Funai block {}/{} height {}", &ch, &bhh, height);
                revalidated_funai_block = true;
                sortition_db_handle.set_funai_block_accepted(&ch, &bhh, height)?;
            }
            ...
            if revalidated_funai_block {
                debug!("Bump Funai block(s) reprocessed");
                self.notifier.notify_funai_block_processed();
            }
            ...
            // we may already have the associated Funai block, but linked to a different sortition
            // history.  For example, if an anchor block was selected but PoX was voted disabled or
            // not voted to activate, then the same Funai blocks could be chosen but with
            ...
            if next_snapshot.sortition {
                self.try_replay_funai_block(&canonical_snapshot, &next_snapshot)?;
            }
        ...
        // make sure our memoized canonical funai tip is correct
        let chainstate_db_conn = self.chain_state_db.db();
        let mut sort_tx = self.sortition_db.tx_begin()?;
        ...
        debug!(
            "Canonical Funai tip after burnchain processing is {}/{} height {} am `{}`",
            &canonical_ch, &canonical_bhh, canonical_height, &funai_am
        );
        debug!(
    ...
    pub fn process_atlas_attachment_events(
        ...
        atlas_config: &AtlasConfig,
        block_receipt: &FunaiEpochReceipt,
        canonical_funai_tip_height: u64,
    ) {
        let mut attachments_instances = HashSet::new();
        for receipt in block_receipt.tx_receipts.iter() {
            if let TransactionOrigin::Funai(ref transaction) = receipt.transaction {
                if let TransactionPayload::ContractCall(ref contract_call) = transaction.payload {
                    ...
                    if atlas_config.contracts.contains(&contract_id) {
                        for event in receipt.events.iter() {
                            if let FunaiTransactionEvent::SmartContractEvent(ref event_data) =
                                event
                            {
                                let res = AttachmentInstance::try_new_from_value(
                                    ...
                                    block_receipt.header.funai_block_height,
                                    receipt.transaction.txid(),
                                    Some(canonical_funai_tip_height),
                                );
        ...
        if !attachments_instances.is_empty() {
            info!(
                ...
                "funai_height" => block_receipt.header.funai_block_height,
            );
            if let Some(atlas_db) = atlas_db {
                ...
                warn!("Atlas: attempted to write attachments, but funai-node not configured with Atlas DB");
            }
        }
    }

    /// Replay any existing Funai blocks we have that arose on a different PoX fork.
    /// This is best-effort -- if a block isn't found or can't be loaded, it's skipped.
    fn replay_funai_blocks(
        &mut self,
        tip: &BlockSnapshot,
        blocks: Vec<BlockHeaderHash>,
    ) -> Result<(), Error> {
        for bhh in blocks.into_iter() {
            let staging_block_chs = FunaiChainState::get_staging_block_consensus_hashes(
                self.chain_state_db.db(),
                &bhh,
            )?;
            ...
            for alt_ch in staging_block_chs.into_iter() {
                let alt_id = FunaiBlockHeader::make_index_block_hash(&alt_ch, &bhh);
                if !FunaiChainState::has_block_indexed(&self.chain_state_db.blocks_path, &alt_id)
                    .unwrap_or(false)
                {
                    continue;
                }
                ...
                if let Ok(Some(block)) =
                    FunaiChainState::load_block(&self.chain_state_db.blocks_path, &alt_ch, &bhh)
                {
                    let ic = self.sortition_db.index_conn();
                    if let Some(parent_snapshot) = ic
                        .find_parent_snapshot_for_funai_block(&ch, &bhh)
                        .unwrap_or(None)
                    {
                        // replay in this consensus hash history
                        debug!("Replay Funai block from {} to {}/{}", &alt_ch, &ch, &bhh);
                        let ic = self.sortition_db.index_conn();
    ...
    /// Try and replay a newly-discovered (or re-affirmed) sortition's associated Funai block, if
    /// we have it.
    #[cfg_attr(test, mutants::skip)]
    fn try_replay_funai_block(
        &mut self,
        canonical_snapshot: &BlockSnapshot,
        ...
        let staging_block_chs = FunaiChainState::get_staging_block_consensus_hashes(
            self.chain_state_db.db(),
            &next_snapshot.winning_funai_block_hash,
        )?;
        ...
        if !found && staging_block_chs.len() > 0 {
            ...
            debug!(
                "Sortition re-processes Funai block {}, which is present on a different PoX fork",
                &next_snapshot.winning_funai_block_hash
            );

            self.replay_funai_blocks(
                &canonical_snapshot,
                vec![next_snapshot.winning_funai_block_hash.clone()],
            )?;
        }
        Ok(())
    }
    ...
    fn check_pox_anchor_affirmation(
        ...
        if BurnchainDB::is_anchor_block(
            ...
            let canonical_sortition_tip = self.canonical_sortition_tip.clone().expect(
                "FAIL: processing a new Funai block, but don't have a canonical sortition tip",
            );
            ...
            debug!("Funai block {} received F*w confirmations but is not the heaviest-confirmed burnchain block, so treating as non-anchor block", pox_anchor);
    ...
    /// block."
    ///
    /// Returning None means "we can keep processing Funai blocks"
    #[cfg_attr(test, mutants::skip)]
    fn consider_pox_anchor(
        ...
        match self.check_pox_anchor_affirmation(pox_anchor, &pox_anchor_snapshot) {
            Ok(Some(pox_anchor)) => {
                ...
                let block_id = FunaiBlockId::new(&pox_anchor_snapshot.consensus_hash, &pox_anchor);
                if !FunaiChainState::has_funai_block(&self.chain_state_db.db(), &block_id)? {
                    debug!(
            ...
            Err(Error::NotPoXAnchorBlock) => {
                ...
                if cur_epoch.epoch_id < FunaiEpochId::Epoch21 {
                    panic!("FATAL: found Funai block that 2.0/2.05 rules would treat as an anchor block, but that 2.1+ would not");
                }
                return Ok(None);
            }
            Err(e) => {
    ...
    fn process_ready_blocks(&mut self) -> Result<Option<BlockHeaderHash>, Error> {
        ...
        while let Some(block_result) = processed_blocks.pop() {
            if block_result.0.is_none() && block_result.1.is_none() {
                ...
                let in_sortition_set = self.sortition_db.is_funai_block_in_sortition_set(
                    &canonical_sortition_tip,
                ...
                if in_sortition_set {
                    ...
                    let new_canonical_funai_block =
                        new_canonical_block_snapshot.get_canonical_funai_block_id();

                    debug!("Bump blocks processed ({})", &new_canonical_funai_block);
                    ...
                    Self::process_atlas_attachment_events(
                        ...
                        &self.atlas_config,
                        &block_receipt,
                        new_canonical_block_snapshot.canonical_funai_tip_height,
                    );

                    let block_hash = block_receipt.header.anchored_header.block_hash();
                    let winner_snapshot = SortitionDB::get_block_snapshot_for_winning_funai_block(
                        &self.sortition_db.index_conn(),
                        &canonical_sortition_tip,
                        &block_hash,
                    )
                    ...
                    if let Some(ref mut estimator) = self.cost_estimator {
                        let funai_epoch = self
                            .sortition_db
                            .index_conn()
                            .get_funai_epoch_by_epoch_id(&block_receipt.evaluated_epoch)
                            .expect("Could not find a funai epoch.");
                    ...
                    if let Some(ref mut estimator) = self.fee_estimator {
                        let funai_epoch = self
                            ...
                            .expect("Could not find a funai epoch.");
                        if let Err(e) =
                            estimator.notify_block(&block_receipt, &funai_epoch.block_limit)
                        {
                    ...
                    if let Some(pox_anchor) = self
                        .sortition_db
                        .is_funai_block_pox_anchor(&block_hash, &canonical_sortition_tip)?
                    {
                        debug!(
...
pub fn check_chainstate_db_versions(
    ...
    if fs::metadata(&sortdb_path).is_ok() {
        ...
        let cur_epoch_idx = FunaiEpoch::find_epoch(epochs, max_height)
    ...
    if fs::metadata(&chainstate_path).is_ok() {
        ...
        let db_config = FunaiChainState::get_db_config_from_path(&chainstate_path)?;
...
impl SortitionDBMigrator {
    ...
    pub fn new(
        ...
        let db_config = FunaiChainState::get_db_config_from_path(&chainstate_path)?;
        let (chainstate, _) = FunaiChainState::open(
            db_config.mainnet,
            db_config.chain_id,
            chainstate_path,
            marf_opts,
