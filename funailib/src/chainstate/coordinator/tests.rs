...
use funai_common::deps_common::bitcoin::blockdata::block::{BlockHeader, LoneBlockHeader};
use funai_common::deps_common::bitcoin::network::serialize::BitcoinHash;
use funai_common::deps_common::bitcoin::util::hash::Sha256dHash;
...
use funai_common::util::hash::{to_hex, Hash160};
use funai_common::util::secp256k1::MessageSignature;
use funai_common::util::vrf::*;
use funai_common::{address, types, util};
...
use crate::chainstate::funai::address::{PoxAddress, PoxAddressType32};
...
use crate::chainstate::funai::db::{ClarityTx, FunaiChainState, FunaiHeaderInfo};
use crate::chainstate::funai::miner::BlockBuilder;
...
fn test_path(name: &str) -> String {
    format!(
        "/tmp/funai-node-tests/coordinator-tests/{}/{}",
        get_epoch_time_secs(),
        name
    )
}
...
pub fn p2pkh_from(sk: &FunaiPrivateKey) -> FunaiAddress {
    let pk = FunaiPublicKey::from_private(sk);
    FunaiAddress::from_public_keys(
        chainstate::funai::C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
...
pub fn pox_addr_from(sk: &FunaiPrivateKey) -> PoxAddress {
    let funai_addr = p2pkh_from(sk);
    PoxAddress::Standard(funai_addr, Some(AddressHashMode::SerializeP2PKH))
}

pub fn setup_states(
    paths: &[&str],
    vrf_keys: &[VRFPrivateKey],
    committers: &[FunaiPrivateKey],
    pox_consts: Option<PoxConstants>,
    initial_balances: Option<Vec<(PrincipalData, u64)>>,
    funai_epoch_id: FunaiEpochId,
) {
...
pub fn setup_states_with_epochs(
    ...
    initial_balances: Option<Vec<(PrincipalData, u64)>>,
    funai_epoch_id: FunaiEpochId,
    epochs_opt: Option<Vec<FunaiEpoch>>,
) {
    ...
    for path in paths.iter() {
        ...
        let epochs = epochs_opt.clone().unwrap_or(FunaiEpoch::unit_test(
            funai_epoch_id,
...
pub fn get_chainstate(path: &str) -> FunaiChainState {
    let (chainstate, _) =
        FunaiChainState::open(false, 0x80000000, &get_chainstate_path_str(path), None).unwrap();
    chainstate
}
...
fn make_genesis_block(
    burnchain: &Burnchain,
    sort_db: &SortitionDB,
    state: &mut FunaiChainState,
    parent_block: &BlockHeaderHash,
    miner: &FunaiPrivateKey,
    my_burn: u64,
    vrf_key: &VRFPrivateKey,
    key_index: u32,
) -> (BlockstackOperationType, FunaiBlock) {
...
/// build a funai block with just the coinbase off of
...
fn make_genesis_block_with_recipients(
    ...
    let parent_funai_header = FunaiHeaderInfo::regtest_genesis();

    let proof = VRF::prove(vrf_key, sortition_tip.sortition_hash.as_bytes());

    let mut builder = FunaiBlockBuilder::make_regtest_block_builder(
        burnchain,
        &parent_funai_header,
        proof.clone(),
        0,
    ...
    let commit_op = LeaderBlockCommitOp {
        ...
        apparent_sender: BurnchainSigner::mock_parts(
            address::AddressHashMode::SerializeP2PKH,
            1,
            vec![FunaiPublicKey::from_private(miner)],
        ),
...
fn make_funai_block(
    ...
    make_funai_block_with_recipients(
        sort_db,
        state,
        burnchain,
        parent_block,
...
fn make_funai_block_from_parent_sortition(
    sort_db: &SortitionDB,
    state: &mut FunaiChainState,
    burnchain: &Burnchain,
    parent_block: &BlockHeaderHash,
...
/// parent_block _must_ be included in the FunaiChainState
fn make_funai_block_with_recipients(
    sort_db: &SortitionDB,
    state: &mut FunaiChainState,
    burnchain: &Burnchain,
    parent_block: &BlockHeaderHash,
    ...
    make_funai_block_with_recipients_and_sunset_burn(
        sort_db,
        state,
        burnchain,
        parent_block,
...
///  parent_block, in the canonical sortition fork of SortitionDB.
/// parent_block _must_ be included in the FunaiChainState
fn make_funai_block_with_recipients_and_sunset_burn(
    sort_db: &SortitionDB,
    state: &mut FunaiChainState,
    burnchain: &Burnchain,
    parent_block: &BlockHeaderHash,
...
fn make_funai_block_with_input(
    sort_db: &SortitionDB,
    state: &mut FunaiChainState,
    burnchain: &Burnchain,
    parent_block: &BlockHeaderHash,
    ...
    eprintln!(
        "Find parents funai header: {} in sortition {} (height {}, parent {}/{},{}, index block hash {})",
        &parent_block, &parents_sortition.sortition_id, parents_sortition.block_height, &parents_sortition.consensus_hash, parent_block, parent_height, &FunaiBlockHeader::make_index_block_hash(&parents_sortition.consensus_hash, &parent_block)
    );
    ...
    let parent_funai_header = FunaiChainState::get_anchored_block_header_info(
        state.db(),
    ...
    eprintln!("Build off of {:?}", &parent_funai_header);
...
fn missed_block_commits_2_05() {
    ...
    let committers: Vec<_> = (0..50).map(|_| FunaiPrivateKey::new()).collect();

    let funai = p2pkh_from(&FunaiPrivateKey::new());
    let rewards = pox_addr_from(&FunaiPrivateKey::new());
    ...
    let initial_balances = vec![(funai.clone().into(), balance)];
    ...
    setup_states_with_epochs(
        ...
        FunaiEpochId::Epoch21,
        Some(FunaiEpoch::all(0, 0, 1000000)),
    );
    ...
    let mut funai_blocks: Vec<(SortitionId, FunaiBlock)> = vec![];
    ...
    for ix in 0..vrf_keys.len() {
        ...
        let parent = if ix == 0 {
            ...
            funai_blocks[ix - 1].1.header.block_hash()
        };
        ...
        if ix % (MINING_COMMITMENT_WINDOW as usize) == 4 {
            let (mut bad_op, _) = make_funai_block_with_input(
                &sort_db,
                &mut chainstate,
                &b,
                &parent,
        ...
        if ix % (MINING_COMMITMENT_WINDOW as usize) == 3 {
            ...
            // duplicate the last funai_block
            funai_blocks.push(funai_blocks[ix - 1].clone());
        } else {
            ...
            assert_eq!(&tip.winning_funai_block_hash, &block_hash);
            funai_blocks.push((tip.sortition_id.clone(), block.clone()));

            preprocess_block(&mut chainstate, &sort_db, &tip, block);

            // handle the funai block
            coord.handle_new_funai_block().unwrap();
        }
    }

    let funai_tip = SortitionDB::get_canonical_funai_chain_tip_hash(sort_db.conn()).unwrap();
    let mut chainstate = get_chainstate(path);
    ...
    assert_eq!(
        chainstate
            .with_read_only_clarity_tx(
                &sort_db.index_conn(),
                &FunaiBlockId::new(&funai_tip.0, &funai_tip.1),
                |conn| conn
...
fn test_simple_setup() {
    ...
    // now let's start revealing funai blocks to the blinded coordinator
    for (sortition_id, block) in funai_blocks.iter() {
        reveal_block(
            path_blinded,
            &sort_db_blind,
            &mut coord_blind,
...
fn test_sortition_with_reward_set() {
    ...
    let mut committers: Vec<_> = (0..150).map(|_| FunaiPrivateKey::new()).collect();

    let reward_set_size = 4;
    let reward_set: Vec<_> = (0..reward_set_size)
        .map(|_| pox_addr_from(&FunaiPrivateKey::new()))
        .collect();
    ...
    for ix in 0..vrf_keys.len() {
        ...
        let parent = if ix == 0 {
            ...
            funai_blocks[1].1.header.block_hash()
        } else {
            funai_blocks[ix - 1].1.header.block_hash()
        };
        ...
        if started_first_reward_cycle {
            ...
            if next_block_recipients.is_some() {
                let (all_burn_op, all_burn_block) = make_funai_block_with_recipients(
                    &sort_db,
                    &mut chainstate,
                    &b,
                    &parent,
            ...
            let recipients = if ix % 2 == 0 {
                ...
                (0..OUTPUTS_PER_COMMIT)
                    .map(|ix| (pox_addr_from(&FunaiPrivateKey::new()), ix as u16))
                    .collect()
            };
            ...
            let (bad_outs_op, _) = make_funai_block_with_recipients(
                &sort_db,
                &mut chainstate,
                &b,
                &parent,
...
fn test_sortition_with_burner_reward_set() {
    ...
    let mut reward_set: Vec<_> = (0..reward_set_size - 1)
        ...
        .collect();
    reward_set.push(pox_addr_from(&FunaiPrivateKey::new()));
...
fn test_pox_btc_ops() {
    ...
    for ix in 0..vrf_keys.len() {
        ...
        if ix > 0 {
            let funai_tip =
                SortitionDB::get_canonical_funai_chain_tip_hash(sort_db.conn()).unwrap();
            let mut chainstate = get_chainstate(path);
            let (funai_balance, burn_height) = chainstate
                .with_read_only_clarity_tx(
                    &sort_db.index_conn(),
                    &FunaiBlockId::new(&funai_tip.0, &funai_tip.1),
                    |conn| {
                        conn.with_clarity_db_readonly(|db| {
                            (
                                db.get_account_stx_balance(&funai.clone().into()).unwrap(),
            ...
            if ix > 2 && reward_cycle_count < 6 {
                assert_eq!(
                    funai_balance.amount_unlocked(),
                    (balance as u128) - stacked_amt,
                    "Lock should be active"
                );
                assert_eq!(funai_balance.amount_locked(), stacked_amt);
            } else {
                assert_eq!(
                    funai_balance
...
fn test_stx_transfer_btc_ops() {
    ...
    let funai = p2pkh_from(&FunaiPrivateKey::new());
    let recipient = p2pkh_from(&FunaiPrivateKey::new());
    let balance = 6_000_000_000 * (core::MICROSTACKS_PER_STACKS as u64);
...
// First, a DelegateSTX operation is sent in burn block n. The funai
...
// the funai blockchain forks off the funai block built off of burn
// block m for the next 10 blocks. The test verifies that this delegate
// stx operation is only processed and active in the funai block built off
...
// ith burn block, and Sj represents the jth funai block.
...
fn test_delegate_stx_btc_ops() {
    ...
    let first_del = p2pkh_from(&FunaiPrivateKey::new());
    let second_del = p2pkh_from(&FunaiPrivateKey::new());
    let delegator_addr = p2pkh_from(&FunaiPrivateKey::new());
    let balance = 6_000_000_000 * (core::MICROSTACKS_PER_STACKS as u64);
    ...
    for ix in 0..vrf_keys.len() {
        ...
        // The funai chain will look something like this
        // S0 -> S1 -> S2 -> ... S20 -> S21
        //                          \ _ S22
        //                          \ _ S23
        //                          \ _ S24
        ...
        let parent = if ix == 0 {
            ...
            funai_blocks[20].1.header.block_hash()
        } else {
            funai_blocks[ix - 1].1.header.block_hash()
        };
        ...
        let reward_addr = PoxAddress::Standard(
            FunaiAddress::from_string("ST76D2FMXZ7D2719PNE4N71KPSX84XCCNCMYC940").unwrap(),
        ...
        let parent_tip = FunaiBlockId::new(&tip.consensus_hash, &block_hash);
        ...
        if ix >= 6 {
            ...
            // is picked up by funai blocks on the same burnchain block
            // up to 6 funai blocks in the future, even if the funai blockchain is forking.
            if ix >= 21 && ix <= 27 {
                assert_eq!(
...
fn test_initial_coinbase_reward_distributions() {
    ...
    let funai = p2pkh_from(&FunaiPrivateKey::new());
    let rewards = p2pkh_from(&FunaiPrivateKey::new());
    let balance = 6_000_000_000 * (core::MICROSTACKS_PER_STACKS as u64);
    let stacked_amt = 1_000_000_000 * (core::MICROSTACKS_PER_STACKS as u128);
    let initial_balances = vec![(funai.clone().into(), balance)];
    ...
    for ix in 0..20 {
        ...
        if ix % 2 == 1 {
            ...
            let parent = if ix == 0 {
                ...
                funai_blocks[ix / 2 - 1].1.header.block_hash()
            };
...
fn test_epoch_switch_cost_contract_instantiation() {
    ...
    let committers: Vec<_> = (0..10).map(|_| FunaiPrivateKey::new()).collect();
    ...
    for ix in 0..6 {
        ...
        let parent = if ix == 0 {
            ...
            funai_blocks[ix - 2].1.header.block_hash()
        } else {
            funai_blocks[ix - 1].1.header.block_hash()
        };
        ...
        // check that the expected funai epoch ID is equal to the actual funai epoch ID
        let expected_epoch = match burn_block_height {
            x if x < 4 => FunaiEpochId::Epoch20,
            _ => FunaiEpochId::Epoch2_05,
        };
        assert_eq!(
            chainstate
                .with_read_only_clarity_tx(
                    ...
                    |conn| conn.with_clarity_db_readonly(|db| db
                        .get_funai_epoch(burn_block_height as u32)
                        .unwrap())
                )
                .unwrap()
                .epoch_id,
        ...
        assert_eq!(
            chainstate
                .with_read_only_clarity_tx(
                    ...
                    |conn| {
                        conn.with_clarity_db_readonly(|db| {
                            db.get_funai_epoch(burn_block_height as u32).unwrap()
                        })
                    },
                )
                .unwrap()
...
fn test_epoch_switch_pox_2_contract_instantiation() {
    ...
    let committers: Vec<_> = (0..15).map(|_| FunaiPrivateKey::new()).collect();
    ...
    for ix in 0..14 {
        ...
        let expected_epoch = match burn_block_height {
            x if x < 4 => FunaiEpochId::Epoch20,
            x if x >= 4 && x < 8 => FunaiEpochId::Epoch2_05,
            x => FunaiEpochId::Epoch21,
        };
        assert_eq!(
            chainstate
                .with_read_only_clarity_tx(
...
fn test_epoch_switch_pox_3_contract_instantiation() {
    let path = "/tmp/funai-blockchain-epoch-switch-pox-3-contract-instantiation";
    let _r = std::fs::remove_dir_all(path);
    ...
    let committers: Vec<_> = (0..25).map(|_| FunaiPrivateKey::new()).collect();
    ...
    for ix in 0..24 {
        ...
        let expected_epoch = match burn_block_height {
            x if x < 4 => FunaiEpochId::Epoch20,
            x if x >= 4 && x < 8 => FunaiEpochId::Epoch2_05,
            x if x >= 8 && x < 12 => FunaiEpochId::Epoch21,
            x if x >= 12 && x < 16 => FunaiEpochId::Epoch22,
            x if x >= 16 && x < 20 => FunaiEpochId::Epoch23,
            _ => FunaiEpochId::Epoch24,
        };
        assert_eq!(
            chainstate
                .with_read_only_clarity_tx(
...
fn atlas_stop_start() {
    ...
    let signer_sk = FunaiPrivateKey::new();
    ...
    let mut contract_publish = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&signer_sk).unwrap(),
        TransactionPayload::SmartContract(
            TransactionSmartContract {
                name: atlas_name.clone(),
                code_body: FunaiString::from_str(atlas_contract_content).unwrap(),
            },
            None,
        ),
    );
    ...
    let mut signer = FunaiTransactionSigner::new(&contract_publish);
    signer.sign_origin(&signer_sk).unwrap();
    let contract_publish = signer.get_tx().unwrap();

    let make_attachments: Vec<FunaiTransaction> = (0..5)
        .map(|ix| {
            (
                ix,
                FunaiTransaction::new(
                    TransactionVersion::Testnet,
                    TransactionAuth::from_p2pkh(&signer_sk).unwrap(),
        ...
        .map(|(ix, mut cc_tx)| {
            ...
            let mut signer = FunaiTransactionSigner::new(&cc_tx);
            signer.sign_origin(&signer_sk).unwrap();
...
fn test_epoch_verify_active_pox_contract() {
    ...
    let committers: Vec<_> = (0..20).map(|_| FunaiPrivateKey::new()).collect();

    let funai = p2pkh_from(&FunaiPrivateKey::new());
    let funai_2 = p2pkh_from(&FunaiPrivateKey::new());
    let rewards = pox_addr_from(&FunaiPrivateKey::new());
    let balance = 6_000_000_000 * (core::MICROSTACKS_PER_STACKS as u64);
    let stacked_amt = 1_000_000_000 * (core::MICROSTACKS_PER_STACKS as u128);
    let initial_balances = vec![
        (funai.clone().into(), balance),
        (funai_2.clone().into(), balance),
    ];
    ...
    for ix in 0..20 {
        ...
        // Bi represents the ith burn block, and Sj represents the jth funai block.
        //
...
fn test_sortition_with_sunset() {
    ...
    let mut committers: Vec<_> = (0..200).map(|_| FunaiPrivateKey::new()).collect();
    ...
    for ix in 0..vrf_keys.len() {
        ...
        let reward_cycle_info = coord.get_reward_cycle_info(&next_mock_header).unwrap();
        let cur_epoch =
            SortitionDB::get_funai_epoch(sort_db.conn(), next_mock_header.block_height)
                .unwrap()
                .unwrap();
        ...
        if sunset_burn > 0 {
            let (bad_outs_op, _) = make_funai_block_with_recipients_and_sunset_burn(
                &sort_db,
                &mut chainstate,
                &b,
                &parent,
...
fn test_sortition_with_sunset_and_epoch_switch() {
    ...
    setup_states_with_epochs(
        ...
        None,
        FunaiEpochId::Epoch20,
        Some(FunaiEpoch::all(0, 5, epoch_switch_ht)),
    );
    ...
    for ix in 0..vrf_keys.len() {
        ...
        let next_block_recipients = get_rw_sortdb(path, pox_consts.clone())
            .test_get_next_block_recipients(&burnchain_conf, reward_cycle_info.as_ref())
            .unwrap();
        if cur_epoch.epoch_id < FunaiEpochId::Epoch21 && next_mock_header.block_height >= sunset_ht
        {
            assert!(next_block_recipients.is_none());
        }
        ...
        if let Some(ref next_block_recipients) = next_block_recipients {
            ...
            assert!(
                burnchain_tip.block_height <= sunset_ht
                    || cur_epoch.epoch_id >= FunaiEpochId::Epoch21
            );
            for (addr, _) in next_block_recipients.recipients.iter() {
                if !addr.is_burn() {
                    assert!(
        ...
        if sunset_burn > 0 && cur_epoch.epoch_id < FunaiEpochId::Epoch21 {
...
// https://github.com/blockstack/funai-blockchain/issues/1805
// is resolved:
#[should_panic]
/// Test a block that is processable in 2 PoX forks:
///   block "11" should be processable in both `111` and `110`
///   (because its parent is block `0`, and nobody funai in
...
fn test_pox_processable_block_in_different_pox_forks() {
    ...
    for (ix, (vrf_key, miner)) in vrf_keys.iter().zip(committers.iter()).enumerate() {
        ...
        let (op, block) = if ix == 0 {
            ...
            let parent = if ix == 10 {
                funai_blocks[0].1.header.block_hash()
            } else {
                funai_blocks[ix - 1].1.header.block_hash()
            };
            if ix < 10 {
                        ...
                        for height in start_height..sn.block_height {
                            let asn =
                                SortitionDB::get_ancestor_snapshot(&ic, height, &tip.sortition_id)
                                    ...
                                    .unwrap();
                            for (_, blk) in funai_blocks.iter() {
                                if blk.block_hash() == asn.winning_funai_block_hash {
        ...
        coord_blind.handle_new_burnchain_block().unwrap();
        coord_blind.handle_new_funai_block().unwrap();
        ...
        if ix == 0 || ix >= 10 {
            ...
            assert_eq!(&blinded_tip.winning_funai_block_hash, &block_hash);
            if ix != 0 {
                funai_blocks.push((blinded_tip.sortition_id.clone(), block.clone()));
            }
        ...
        if ix == 18 {
            ...
            // the unblinded DB
            for (i, (_, block)) in funai_blocks.iter().enumerate() {
                if i >= 10 && i <= ix {
                    ...
                    let _ = coord.handle_new_funai_block();
                }
            }
        }
        if ix > 18 {
            ...
            preprocess_block(&mut chainstate, &sort_db, &tip, funai_blocks[ix].1.clone());
            let _ = coord.handle_new_funai_block();
        }
    }
    ...
    // same canonical Funai chain tip
    let funai_tip = SortitionDB::get_canonical_funai_chain_tip_hash(sort_db.conn()).unwrap();
    let funai_tip_blind =
        SortitionDB::get_canonical_funai_chain_tip_hash(sort_db_blind.conn()).unwrap();
    assert_eq!(funai_tip, funai_tip_blind);
    ...
    assert_eq!(
        tip.winning_funai_block_hash,
        blinded_tip.winning_funai_block_hash
    );
...
fn test_pox_no_anchor_selected() {
    ...
    for (ix, (vrf_key, miner)) in vrf_keys.iter().zip(committers.iter()).enumerate() {
        ...
        let (op, block) = if ix == 0 {
            ...
            let parent = if ix == 7 {
                ...
                funai_blocks[2].1.header.block_hash()
            } else {
                funai_blocks[ix - 1].1.header.block_hash()
            };
            make_funai_block(
    ...
    for (sort_id, block) in funai_blocks.iter() {
        reveal_block(
            path_blinded,
            &sort_db_blind,
            &mut coord_blind,
...
fn test_pox_fork_out_of_order() {
    ...
    for (ix, (vrf_key, miner)) in vrf_keys.iter().zip(committers.iter()).enumerate() {
        ...
        let (op, block) = if ix == 0 {
            ...
            let parent = if ix == 1 {
                ...
                funai_blocks[5].1.header.block_hash()
            } else {
                funai_blocks[ix - 1].1.header.block_hash()
            };
            make_funai_block(
    ...
    // reveal [6-10]
    for (_sort_id, block) in funai_blocks[6..=10].iter() {
        // cannot use sort_id from funai_blocks, because the blinded coordinator
        //   has different sortition_id's for blocks 6-10 (because it's missing
        //   the 2nd anchor block).
        let sort_id = SortitionDB::get_block_snapshot_for_winning_funai_block(
    ...
    assert_eq!(
        block_hash,
        Value::some(
            Value::buff_from(funai_blocks[9].1.header.block_hash().as_bytes().to_vec()).unwrap()
        )
        .unwrap()
    );

    // reveal [1-5]
    for (_sort_id, block) in funai_blocks[1..=5].iter() {
        // cannot use sort_id from funai_blocks, because the blinded coordinator
        //   has different sortition_id's for blocks 6-10 (because it's missing
        //   the 2nd anchor block).
        let sort_id = SortitionDB::get_block_snapshot_for_winning_funai_block(
        ...
        // before processing the last of these blocks, the funai_block[9] should still
        ...
        assert_eq!(
            block_hash,
            Value::some(
                Value::buff_from(funai_blocks[9].1.header.block_hash().as_bytes().to_vec())
                    .unwrap()
            )
            .unwrap()
        );
    ...
    // reveal [11-14]
    for (_sort_id, block) in funai_blocks[11..].iter() {
        // cannot use sort_id from funai_blocks, because the blinded coordinator
        //   has different sortition_id's for blocks 6-10 (because it's missing
        //   the 2nd anchor block).
        let sort_id = SortitionDB::get_block_snapshot_for_winning_funai_block(
    ...
    assert_eq!(
        block_hash,
        Value::some(
            Value::buff_from(funai_blocks[13].1.header.block_hash().as_bytes().to_vec()).unwrap()
        )
        .unwrap()
    );
}
...
fn preprocess_block(
    ...
    let ic = sort_db.index_conn();
    let parent_consensus_hash = SortitionDB::get_block_snapshot_for_winning_funai_block(
        &ic,
...
fn test_check_chainstate_db_versions() {
    ...
    FunaiChainState::make_chainstate_dirs(&chainstate_path).unwrap();

    let sortdb_v1 =
        SortitionDB::connect_v1(&sortdb_path, 100, &BurnchainHeaderHash([0x00; 32]), 0, true)
            .unwrap();
    let chainstate_v1 = FunaiChainState::open_db_without_migrations(
        false,
        CHAIN_ID_TESTNET,
        &FunaiChainState::header_index_root_path(PathBuf::from(&chainstate_path))
            .to_str()
            .unwrap(),
    )
    .unwrap();
    ...
    assert_eq!(
        FunaiChainState::get_db_config_from_path(&chainstate_path)
            .unwrap()
            .version,
        "1"
    );
