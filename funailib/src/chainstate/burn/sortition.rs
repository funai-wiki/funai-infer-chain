...
use funai_common::types::chainstate::{
    BlockHeaderHash, PoxId, SortitionId, FunaiBlockId, TrieHash, VRFSeed,
};
use funai_common::util::hash::Hash160;
use funai_common::util::log;
use funai_common::util::uint::{BitArray, Uint256, Uint512};
...
use crate::chainstate::funai::index::{ClarityMarfTrieId, MarfTrieId, TrieHashExtension};
use crate::core::*;
...
impl BlockSnapshot {
    ...
    pub fn empty() -> BlockSnapshot {
        BlockSnapshot {
            ...
            winning_funai_block_hash: BlockHeaderHash([0; 32]),
            index_root: TrieHash([0; 32]),
            num_sortitions: 0,
            funai_block_accepted: true,
            funai_block_height: 0,
            arrival_index: 0,
            canonical_funai_tip_height: 0,
            canonical_funai_tip_hash: BlockHeaderHash([0; 32]),
            canonical_funai_tip_consensus_hash: ConsensusHash([0; 20]),
    ...
    pub fn initial(
        ...
        BlockSnapshot {
            ...
            winning_funai_block_hash: FIRST_STACKS_BLOCK_HASH.clone(),
            ...
            arrival_index: 0,
            canonical_funai_tip_height: 0,
            canonical_funai_tip_hash: FIRST_STACKS_BLOCK_HASH.clone(),
            canonical_funai_tip_consensus_hash: FIRST_BURNCHAIN_CONSENSUS_HASH.clone(),
    ...
    pub fn get_canonical_funai_block_id(&self) -> FunaiBlockId {
        FunaiBlockId::new(
            &self.canonical_funai_tip_consensus_hash,
            &self.canonical_funai_tip_hash,
        )
    }
    ...
    /// Select the next Funai block header hash using cryptographic sortition.
    ...
    fn make_snapshot_no_sortition(
        ...
        Ok(BlockSnapshot {
            ...
            winning_funai_block_hash: non_winning_block_hash,
            index_root: TrieHash::from_empty_data(), // will be overwritten
            num_sortitions: parent_snapshot.num_sortitions,
            funai_block_accepted: false,
            funai_block_height: 0,
            arrival_index: 0,
            canonical_funai_tip_height: parent_snapshot.canonical_funai_tip_height,
            canonical_funai_tip_hash: parent_snapshot.canonical_funai_tip_hash.clone(),
            canonical_funai_tip_consensus_hash: parent_snapshot
    ...
    pub fn make_snapshot(
        ...
        let accumulated_coinbase_ustx = if parent_snapshot.total_burn == 0 {
            ...
            let missed_coinbase = FunaiChainState::get_coinbase_reward(
        ...
        Ok(BlockSnapshot {
            ...
            winning_funai_block_hash: winning_block.block_header_hash,
            index_root: TrieHash::from_empty_data(), // will be overwritten,
...
mod test {
    ...
    use funai_common::types::chainstate::{BlockHeaderHash, BurnchainHeaderHash, VRFSeed};
    use funai_common::util::get_epoch_time_secs;
    use funai_common::util::hash::hex_bytes;
    use funai_common::util::vrf::{VRFPrivateKey, VRFPublicKey};
