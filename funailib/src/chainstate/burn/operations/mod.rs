...
pub struct TransferStxOp {
    pub sender: FunaiAddress,
    pub recipient: FunaiAddress,
...
pub struct StackStxOp {
    ...
    pub num_cycles: u8,
    pub signer_key: Option<FunaiPublicKeyBuffer>,
...
pub struct LeaderBlockCommitOp {
    pub block_header_hash: BlockHeaderHash, // hash of Funai block header (sha512/256)
...
pub struct VoteForAggregateKeyOp {
    pub sender: FunaiAddress,
    pub aggregate_key: FunaiPublicKeyBuffer,
    pub round: u32,
    pub reward_cycle: u64,
    pub signer_index: u16,
    pub signer_key: FunaiPublicKeyBuffer,
...
pub fn funai_addr_serialize(addr: &FunaiAddress) -> serde_json::Value {
...
impl BlockstackOperationType {
    ...
    pub fn pre_stx_to_json(op: &PreStxOp) -> serde_json::Value {
        json!({
            "pre_stx": {
                ...
                "output": funai_addr_serialize(&op.output),
                "burn_txid": op.txid,
    ...
    pub fn stack_stx_to_json(op: &StackStxOp) -> serde_json::Value {
        json!({
            "stack_stx": {
                ...
                "sender": funai_addr_serialize(&op.sender),
    ...
    pub fn transfer_stx_to_json(op: &TransferStxOp) -> serde_json::Value {
        json!({
            "transfer_stx": {
                ...
                "recipient": funai_addr_serialize(&op.recipient),
                "sender": funai_addr_serialize(&op.sender),
    ...
    pub fn delegate_stx_to_json(op: &DelegateStxOp) -> serde_json::Value {
        json!({
            "delegate_stx": {
                "burn_block_height": op.block_height,
                "burn_header_hash": &op.burn_header_hash.to_hex(),
                "delegate_to": funai_addr_serialize(&op.delegate_to),
    ...
    pub fn blockstack_op_to_json(&self) -> serde_json::Value {
        match self {
            ...
            // is currently only used to json-ify burnchain ops executed as Funai transactions (so,
