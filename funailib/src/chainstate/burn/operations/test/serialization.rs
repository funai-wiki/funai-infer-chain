...
use funai_common::address::C32_ADDRESS_VERSION_MAINNET_SINGLESIG;
...
fn test_serialization_transfer_stx_op() {
    ...
    let recipient = FunaiAddress::from_string(recipient_addr).unwrap();
...
fn test_serialization_pre_stx_op() {
    ...
    let output = FunaiAddress::from_string(output_addr).unwrap();
...
fn test_serialization_delegate_stx_op() {
    ...
    let delegate_to = FunaiAddress::from_string(delegate_to_addr).unwrap();
...
fn test_serialization_vote_for_aggregate_key_op() {
    ...
    let op = VoteForAggregateKeyOp {
        ...
        round: 1,
        signer_index: 12,
        signer_key: FunaiPublicKeyBuffer([0x01; 33]),
        aggregate_key: FunaiPublicKeyBuffer([0x02; 33]),
        txid: Txid([10u8; 32]),
        vtxindex: 10,
        block_height: 10,
        burn_header_hash: BurnchainHeaderHash([0x10; 32]),
