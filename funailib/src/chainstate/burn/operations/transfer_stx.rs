...
impl TransferStxOp {
    ...
    pub fn get_sender_txid(tx: &BurnchainTransaction) -> Result<&Txid, op_error> {
        match tx.get_input_tx_ref(0) {
            Some((ref txid, vout)) => {
                if *vout != 1 {
                    warn!(
                        "Invalid tx: TransferStxOp must spend the second output of the PreFunaiOp"
                    );
    ...
    pub fn parse_from_tx(
        ...
        let output = outputs[0]
            ...
            .ok_or_else(|| {
                warn!("Invalid tx: output must be representable as a FunaiAddress");
                op_error::InvalidInput
            })?;
...
impl FunaiMessageCodec for TransferStxOp {
