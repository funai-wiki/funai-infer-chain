...
use funai_common::types::chainstate::{BurnchainHeaderHash, FunaiAddress};
...
struct ParsedData {
    signer_index: u16,
    aggregate_key: FunaiPublicKeyBuffer,
    round: u32,
    reward_cycle: u64,
}
...
impl VoteForAggregateKeyOp {
    ...
    fn parse_data(data: &Vec<u8>) -> Option<ParsedData> {
        ...
        */
        ...
        let aggregate_key = FunaiPublicKeyBuffer::from(&data[2..35]);
    ...
    /// `Secp256k1PublicKey` from `funai_common` as well as `Point` from wsts.
...
impl FunaiMessageCodec for VoteForAggregateKeyOp {
...
mod tests {
    ...
    use funai_common::types::chainstate::{BurnchainHeaderHash, FunaiAddress};
    use funai_common::types::{Address, FunaiPublicKeyBuffer};
    ...
    fn test_parse_vote_tx_signer_key() {
        let aggregate_key = FunaiPublicKeyBuffer([0x01; 33]);
        let signer_key = FunaiPublicKeyBuffer([0x02; 33]);
    ...
    fn test_key_validation() {
        ...
        let op = VoteForAggregateKeyOp {
            ...
            round: 1,
            signer_index: 12,
            signer_key: FunaiPublicKeyBuffer([0x00; 33]),
            aggregate_key: FunaiPublicKeyBuffer([0x00; 33]),
            txid: Txid([10u8; 32]),
            vtxindex: 10,
            block_height: 10,
