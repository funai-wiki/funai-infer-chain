...
use funai_common::codec::{write_next, Error as codec_error, FunaiMessageCodec};
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, FunaiAddress, FunaiBlockId, TrieHash, VRFSeed,
};
use funai_common::util::hash::to_hex;
use funai_common::util::log;
use funai_common::util::vrf::{VRFPrivateKey, VRFPublicKey, VRF};
...
use crate::core::{
    FunaiEpoch, FunaiEpochId, STACKS_EPOCH_2_05_MARKER, STACKS_EPOCH_2_1_MARKER,
...
impl LeaderBlockCommitOp {
    ...
    pub fn last_tenure_id(&self) -> FunaiBlockId {
        FunaiBlockId(self.block_header_hash.0.clone())
    }
...
impl FunaiMessageCodec for LeaderBlockCommitOp {
    /*
        Wire format:
...
impl LeaderBlockCommitOp {
    ...
    fn check_epoch_commit(&self, epoch_id: FunaiEpochId) -> Result<(), op_error> {
        match epoch_id {
            FunaiEpochId::Epoch10 => {
                panic!("FATAL: processed block-commit pre-Funai 2.0");
            }
            ...
            FunaiEpochId::Epoch2_05 => self.check_epoch_commit_marker(STACKS_EPOCH_2_05_MARKER),
            FunaiEpochId::Epoch21 => self.check_epoch_commit_marker(STACKS_EPOCH_2_1_MARKER),
            FunaiEpochId::Epoch22 => self.check_epoch_commit_marker(STACKS_EPOCH_2_2_MARKER),
            FunaiEpochId::Epoch23 => self.check_epoch_commit_marker(STACKS_EPOCH_2_3_MARKER),
            FunaiEpochId::Epoch24 => self.check_epoch_commit_marker(STACKS_EPOCH_2_4_MARKER),
            FunaiEpochId::Epoch25 => self.check_epoch_commit_marker(STACKS_EPOCH_2_5_MARKER),
            FunaiEpochId::Epoch30 => self.check_epoch_commit_marker(STACKS_EPOCH_3_0_MARKER),
        }
    }
    ...
    fn check_common(
        ...
        let is_already_committed = tx.expects_funai_block_in_fork(&self.block_header_hash)?;

        // in Epoch3.0+, block commits can include Funai blocks already accepted in the fork.
        if is_already_committed && epoch_id < FunaiEpochId::Epoch30 {
            warn!(
        ...
        /////////////////////////////////////////////////////////////////////////////////////
        // If we are in Funai 2.05 or later, then the memo field *must* have the appropriate epoch
    ...
    pub fn check(
        ...
        let epoch = SortitionDB::get_funai_epoch(tx, self.block_height)?.unwrap_or_else(|| {
            panic!(
...
mod tests {
    ...
    use funai_common::address::AddressHashMode;
    use funai_common::deps_common::bitcoin::blockdata::transaction::{Transaction, TxOut};
    use funai_common::deps_common::bitcoin::network::serialize::{deserialize, serialize_hex};
    use funai_common::types::chainstate::{BlockHeaderHash, SortitionId, FunaiAddress, VRFSeed};
    ...
    use crate::core::{
        FunaiEpoch, FunaiEpochExtension, FunaiEpochId, PEER_VERSION_EPOCH_1_0,
    ...
    fn funai_address_to_bitcoin_tx_out(addr: &FunaiAddress, value: u64) -> TxOut {
        let btc_version = to_b58_version_byte(addr.version)
            .expect("BUG: failed to decode Funai version byte to Bitcoin version byte");
    ...
    fn test_parse() {
        ...
        let tx_fixtures = vec![
            OpFixture {
                ...
                result: Some(LeaderBlockCommitOp {
                    ...
                    commit_outs: vec![
                        PoxAddress::Standard( FunaiAddress { version: 26, bytes: Hash160::empty() }, None ),
                        PoxAddress::Standard( FunaiAddress { version: 26, bytes: Hash160::empty() }, None ),
                    ],
        ...
        for tx_fixture in tx_fixtures {
            ...
            if is_first {
                ...
                tx.output.insert(
                    2,
                    funai_address_to_bitcoin_tx_out(&FunaiAddress::burn_address(false), 12345),
                );
                is_first = false;
            ...
            let burnchain_tx = BurnchainTransaction::Bitcoin(
                parser
                    .parse_tx(&tx, vtxindex as usize, FunaiEpochId::Epoch2_05)
                    .unwrap(),
            );
    ...
    fn test_check_2_1() {
        ...
        let mut db = SortitionDB::connect_test_with_epochs(
            first_block_height,
            &first_burn_hash,
            FunaiEpoch::all(0, 0, first_block_height),
        )
        .unwrap();
        let block_ops = vec![
            // 122
