...
//! can last for the duration of one or more burnchain blocks, and may be extended by Funais.  As
...
//! guaranteed to be globally unique across all Funai chain histories and burnchain histories.
//!
...
//! child tenure.  The first-ever Nakamoto tenure's parent block is the last epoch2 Funai block.
...
//!
//! * An `Extended` tenure change, which is induced by Funais. This resets the tenure's ongoing
//! execution budget, thereby allowing the miner to continue producing blocks.
//!
//! A tenure may be extended at any time by Funais, and may span multiple Bitcoin blocks (such
...
//! * The _sortition consensus hash: this is the tip of the sortition history that Funais knew
...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, ConsensusHash, FunaiBlockId, FunaiPrivateKey,
    FunaiPublicKey, TrieHash, VRFSeed,
};
...
use crate::chainstate::funai::db::{
    ...
    MinerPaymentTxFees, MinerRewardInfo, FunaiBlockHeaderTypes, FunaiChainState, FunaiDBTx,
    FunaiEpochReceipt, FunaiHeaderInfo,
};
...
pub static NAKAMOTO_TENURES_SCHEMA: &'static str = r#"
    CREATE TABLE nakamoto_tenures (
        ...
        -- block ID of this start block (this is the FunaiBlockId of the above tenure_id_consensus_hash and block_hash)
...
impl NakamotoChainState {
    ...
    pub(crate) fn get_matured_miner_reward_schedules(
        ...
        let latest_miners = FunaiChainState::get_scheduled_block_rewards_at_block(
    ...
    ///  Funai block height of `block` if it was an epoch-2 block
    ///
    /// In Funai 2.x, the coinbase height and block height are the
    /// same. A miner's tenure in Funai 2.x is entirely encompassed
    /// in the single Bitcoin-anchored Funai block they produce, as
    ...
    /// Only works for Nakamoto blocks, not Funai epoch2 blocks.
    ...
    pub fn get_highest_nakamoto_tenure(
        ...
        // find the tenure for the Funai chain tip
        let (tip_ch, tip_bhh) = SortitionDB::get_canonical_funai_chain_tip_hash(sortdb_conn)?;
    ...
    pub(crate) fn check_first_nakamoto_tenure_change(
        ...
        let Some(epoch2_header_info) = parent_header.anchored_header.as_funai_epoch2() else {
    ...
    pub(crate) fn advance_nakamoto_tenure(
        ...
        let Some(highest_processed_tenure) =
            ...
            return Err(ChainstateError::InvalidFunaiTransaction(
