...
use funai_common::types::chainstate::{ConsensusHash, FunaiBlockId};
...
use crate::chainstate::funai::{Error as ChainstateError, FunaiBlock, FunaiBlockHeader};
use crate::funai_common::codec::FunaiMessageCodec;
...
pub const NAKAMOTO_STAGING_DB_SCHEMA_1: &'static [&'static str] = &[
  ...
  CREATE TABLE nakamoto_staging_blocks (
                 ...
                 -- used internally -- this is the FunaiBlockId of this block's consensus hash and block hash
...
impl<'a> NakamotoStagingBlocksConnRef<'a> {
    ...
    fn has_processed_nakamoto_block<SH: SortitionHandle>(
        ...
        let ibh = FunaiBlockId::new(&ch, &bhh);
            ...
            .or_else(|e| {
                ...
                )) = e
                {
                    ...
                    while let Some(row) = qry.next()? {
                        let index_block_hash : FunaiBlockId = row.get(0)?;
                        let parent_block_id : FunaiBlockId = row.get(1)?;
...
impl<'a> NakamotoStagingBlocksTx<'a> {
    /// Notify the staging database that a given funai block has been processed.
    /// This will update the attachable status for children blocks, as well as marking the funai
    ///  block itself as processed.
    pub fn set_block_processed(&self, block: &FunaiBlockId) -> Result<(), ChainstateError> {
    ...
    /// Modify the staging database that a given funai block can never be processed.
    /// This will update the attachable status for children blocks, as well as marking the funai
    /// block itself as orphaned.
    pub fn set_block_orphaned(&self, block: &FunaiBlockId) -> Result<(), ChainstateError> {
