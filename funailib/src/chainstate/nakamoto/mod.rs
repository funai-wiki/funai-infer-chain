...
use clarity::vm::types::{PrincipalData, FunaiAddressExtensions, TupleData};
...
use funai_common::codec::{
    read_next, write_next, Error as CodecError, FunaiMessageCodec, MAX_MESSAGE_LEN,
    MAX_PAYLOAD_LEN,
};
...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, ConsensusHash, FunaiAddress, FunaiBlockId,
    FunaiPrivateKey, FunaiPublicKey, TrieHash, VRFSeed,
};
use funai_common::types::{PrivateKey, FunaiEpochId};
use funai_common::util::hash::{to_hex, Hash160, MerkleHashFunc, MerkleTree, Sha512Trunc256Sum};
use funai_common::util::retry::BoundReader;
use funai_common::util::secp256k1::MessageSignature;
use funai_common::util::vrf::{VRFProof, VRFPublicKey, VRF};
use funai_common::util::{get_epoch_time_secs, sleep_ms};
...
use super::funai::db::accounts::MinerReward;
use super::funai::db::{
    ChainstateTx, ClarityTx, MinerPaymentSchedule, MinerPaymentTxFees, MinerRewardInfo,
    FunaiBlockHeaderTypes, FunaiDBTx, FunaiEpochReceipt, FunaiHeaderInfo,
};
use super::funai::events::{FunaiTransactionReceipt, TransactionOrigin};
use super::funai::{
    Error as ChainstateError, FunaiBlock, FunaiBlockHeader, FunaiMicroblock, FunaiTransaction,
...
use crate::chainstate::funai::boot::{POX_4_NAME, SIGNERS_UPDATE_STATE};
use crate::chainstate::funai::db::{DBConfig as ChainstateConfig, FunaiChainState};
use crate::chainstate::funai::index::marf::MarfConnection;
...
use crate::net::funaidb::{FunaiDBConfig, MINER_SLOT_COUNT};
...
lazy_static! {
    pub static ref FIRST_STACKS_BLOCK_ID: FunaiBlockId = FunaiBlockId::new(&FIRST_BURNCHAIN_CONSENSUS_HASH, &FIRST_STACKS_BLOCK_HASH);
    ...
    r#"
      ...
      CREATE TABLE nakamoto_block_headers (
          -- The following fields all correspond to entries in the FunaiHeaderInfo struct
          ...
          -- The following fields all correspond to entries in the NakamotoBlockHeader struct
                     ...
                     -- bitvec capturing funai participation in signature
                     signer_bitvec TEXT NOT NULL,
          -- The following fields are not part of either the FunaiHeaderInfo struct
          --   or its contained NakamotoBlockHeader struct, but are used for querying
                     -- what kind of header this is (nakamoto or funai 2.x)
...
pub struct SetupBlockResult<'a, 'b> {
    ...
    /// Transaction receipts from any Funai-on-Bitcoin transactions and epoch transition events
    ...
    /// stack-stx Funai-on-Bitcoin txs
    pub burn_stack_stx_ops: Vec<StackStxOp>,
    /// transfer-stx Funai-on-Bitcoin txs
    pub burn_transfer_stx_ops: Vec<TransferStxOp>,
    /// delegate-stx Funai-on-Bitcoin txs
    ...
    /// vote-for-aggregate-key Funai-on-Bitcoin txs
...
pub struct NakamotoBlockHeader {
    pub version: u8,
    /// The total number of FunaiBlock and NakamotoBlocks preceding
...
pub struct NakamotoBlock {
    ...
    pub txs: Vec<FunaiTransaction>,
}

pub struct NakamotoChainState;

impl FunaiMessageCodec for NakamotoBlockHeader {
    fn consensus_serialize<W: std::io::Write>(&self, fd: &mut W) -> Result<(), CodecError> {
...
impl NakamotoBlockHeader {
    ...
    /// Inner calculation of the message digest for funais to sign.
    /// This includes all fields _except_ the funai signature.
    ...
    pub fn recover_miner_pk(&self) -> Option<FunaiPublicKey> {
        let signed_hash = self.miner_signature_hash();
        let recovered_pk =
            FunaiPublicKey::recover_to_pubkey(signed_hash.bits(), &self.miner_signature).ok()?;
    ...
    pub fn block_id(&self) -> FunaiBlockId {
        FunaiBlockId::new(&self.consensus_hash, &self.block_hash())
    }

    pub fn is_first_mined(&self) -> bool {
        self.parent_block_id == FunaiBlockId::first_mined()
    }

    /// Sign the block header by the miner
    pub fn sign_miner(&mut self, privk: &FunaiPrivateKey) -> Result<(), ChainstateError> {
    ...
    pub fn empty() -> NakamotoBlockHeader {
        NakamotoBlockHeader {
            ...
            parent_block_id: FunaiBlockId([0u8; 32]),
            tx_merkle_root: Sha512Trunc256Sum([0u8; 32]),
            state_index_root: TrieHash([0u8; 32]),
            miner_signature: MessageSignature::empty(),
            signer_signature: ThresholdSignature::empty(),
    ...
    pub fn genesis() -> NakamotoBlockHeader {
        NakamotoBlockHeader {
            ...
            parent_block_id: FunaiBlockId(BOOT_BLOCK_HASH.0.clone()),
            tx_merkle_root: Sha512Trunc256Sum([0u8; 32]),
            state_index_root: TrieHash([0u8; 32]),
            miner_signature: MessageSignature::empty(),
            signer_signature: ThresholdSignature::empty(),
...
impl NakamotoBlock {
    ...
    pub fn get_coinbase_tx(&self) -> Option<&FunaiTransaction> {
        if self.is_wellformed_tenure_start_block() != Ok(true) {
    ...
    pub fn validate_vrf_seed(
        ...
        if !block_commit.new_seed.is_from_proof(&parent_vrf_proof) {
            ...
            return Err(ChainstateError::InvalidFunaiBlock(
    ...
    pub(crate) fn recover_miner_pubkh(&self) -> Result<Hash160, ChainstateError> {
        let recovered_miner_pubk = self.header.recover_miner_pk().ok_or_else(|| {
            warn!(
                "Nakamoto Funai block downloaded with unrecoverable miner public key";
                "block_hash" => %self.header.block_hash(),
                "block_id" => %self.header.block_id(),
            );
            return ChainstateError::InvalidFunaiBlock("Unrecoverable miner public key".into());
        })?;
    ...
    pub(crate) fn check_miner_signature(
        ...
        if &recovered_miner_hash160 != miner_pubkey_hash160 {
            warn!(
                "Nakamoto Funai block signature mismatch: {recovered_miner_hash160} != {miner_pubkey_hash160} from leader-key";
                "block_hash" => %self.header.block_hash(),
                "block_id" => %self.header.block_id(),
            );
            return Err(ChainstateError::InvalidFunaiBlock(
    ...
    pub(crate) fn check_coinbase_tx(
        ...
        if let Some(coinbase_tx) = self.get_coinbase_tx() {
            ...
            let vrf_proof = vrf_proof_opt.ok_or(ChainstateError::InvalidFunaiBlock(
            ...
            let valid = match VRF::verify(leader_vrf_key, vrf_proof, sortition_hash.as_bytes()) {
                ...
                Err(e) => {
                    warn!(
                        "Invalid Funai block header {}: failed to verify VRF proof: {}",
    ...
    pub fn validate_against_burnchain(
        ...
        // * funai signature  (validated on accept_block())
        Ok(())
    }
    ...
    pub fn validate_transactions_static(
        ...
        if !FunaiBlock::validate_transactions_unique(&self.txs) {
            return false;
        }
        if !FunaiBlock::validate_transactions_network(&self.txs, mainnet) {
            return false;
        }
        if !FunaiBlock::validate_transactions_chain_id(&self.txs, chain_id) {
            return false;
        }
        ...
        if !FunaiBlock::validate_transactions_static_epoch(&self.txs, epoch_id) {
            return false;
        }
        return true;
    }
...
impl NakamotoChainState {
    ...
    fn infallible_set_block_processed(
        funai_chain_state: &mut FunaiChainState,
        block_id: &FunaiBlockId,
    ) {
        loop {
            let Ok(staging_block_tx) = funai_chain_state.staging_db_tx_begin().map_err(|e| {
    ...
    pub fn process_next_nakamoto_block<'a, T: BlockEventDispatcher>(
    ...
    ) -> Result<Option<FunaiEpochReceipt>, ChainstateError> {
        let nakamoto_blocks_db = funai_chain_state.nakamoto_blocks_db();
        let Some((next_ready_block, block_size)) =
            nakamoto_blocks_db.next_ready_nakamoto_block(funai_chain_state.db(), sort_tx)?
        else {
            // no more blocks
        ...
        let (mut chainstate_tx, clarity_instance) = funai_chain_state.chainstate_tx_begin()?;
        ...
        let parent_block_id = FunaiBlockId::new(
        ...
        // mutable reference to `funai_chain_state` to start it).  This means ensuring that, in the
        ...
        // to access `funai_chain_state` again.  In the `Ok(..)` case, it's instead sufficient so
        ...
        if let Some(e) = err_opt {
            ...
            Self::infallible_set_block_orphaned(funai_chain_state, &block_id);
            return Err(e);
        };
        ...
        // within the same, single thread.  Also, it's *very important* that this update
        // succeeds, since *we have already processed* the block.
        Self::infallible_set_block_processed(funai_chain_state, &block_id);
    ...
    /// * the funais during the tenure didn't sign it
    /// * a DB error occurs
    /// Does nothing if:
    ...
    pub fn accept_block(
        ...
        block.is_wellformed_tenure_start_block().map_err(|_| {
            ...
            ChainstateError::InvalidFunaiBlock("Not a well-formed first-tenure block".into())
        })?;
        ...
        block.is_wellformed_tenure_extend_block().map_err(|_| {
            ...
            ChainstateError::InvalidFunaiBlock("Not a well-formed tenure-extend block".into())
        })?;
        ...
        if !db_handle.expects_signer_signature(
            ...
            return Err(ChainstateError::InvalidFunaiBlock(msg));
        }
    ...
    pub(crate) fn load_aggregate_public_key<SH: SortitionHandle>(
        ...
        match chainstate.get_aggregate_public_key_pox_4(sortdb, at_block_id, rc)? {
            Some(key) => Ok(key),
            None => {
                // FALLBACK: The aggregate public key for the current cycle may not exist
                // (e.g., if the chain was offline and DKG votes were never processed on-chain).
                // Walk backwards through previous cycles to find a valid aggregate key.
    ...
    /// See https://github.com/funai-network/funai-core/issues/4109
    ...
    /// Return a Nakamoto FunaiHeaderInfo at a given coinbase height in the fork identified by `tip_index_hash`.
    /// * For Funai 2.x, this is the Funai block's header
    /// * For Funai 3.x (Nakamoto), this is the first block in the miner's tenure.
    pub fn get_header_by_coinbase_height(
        tx: &mut FunaiDBTx,
        tip_index_hash: &FunaiBlockId,
        coinbase_height: u64,
    ) -> Result<Option<FunaiHeaderInfo>, ChainstateError> {
        ...
        for candidate_ch in candidate_chs.into_iter() {
            ...
            let Ok(Some(ancestor_at_height)) =
                tx.get_ancestor_block_hash(candidate.funai_block_height, tip_index_hash)
            else {
    ...
    /// Load the canonical Funai block header (either epoch-2 rules or Nakamoto)
    pub fn get_canonical_block_header(
        ...
        Self::get_block_header(
            chainstate_conn,
            &FunaiBlockId::new(&consensus_hash, &block_hash),
        )
    }
    ...
    pub fn get_block_header_by_consensus_hash(
        ...
        let epoch2_header_info = FunaiChainState::get_funai_block_header_info_by_consensus_hash(
            chainstate_conn,
            consensus_hash,
        )?;
    ...
    /// Get the VRF proof for a Funai block.
    ...
    pub fn get_block_vrf_proof(
        ...
        let vrf_proof = match start_header.anchored_header {
            FunaiBlockHeaderTypes::Epoch2(epoch2_header) => Some(epoch2_header.proof),
            FunaiBlockHeaderTypes::Nakamoto(..) => {
    ...
    ///
    /// Returns InvalidFunaiBlock if the sortition for `consensus_hash` does not exist, or if its
    ...
    pub fn get_parent_vrf_proof(
        ...
        let sn = SortitionDB::get_block_snapshot_consensus(sortdb_conn, consensus_hash)?.ok_or(
            ChainstateError::InvalidFunaiBlock("No sortition for consensus hash".into()),
        )?;
        ...
        .ok_or(ChainstateError::InvalidFunaiBlock(
        ...
        let parent_sn = SortitionDB::get_block_snapshot(sortdb_conn, &parent_sortition_id)?.ok_or(
            ChainstateError::InvalidFunaiBlock(
    ...
    pub(crate) fn insert_funai_block_header(
        chainstate_tx: &Connection,
        tip_info: &FunaiHeaderInfo,
        ...
        assert_eq!(tip_info.funai_block_height, header.chain_length,);
        ...
        assert!(*funai_block_height < u64::try_from(i64::MAX).unwrap());
    ...
    fn advance_tip(
        headers_tx: &mut FunaiDBTx,
        parent_tip: &FunaiBlockHeaderTypes,
        ...
        new_tenure: bool,
        block_fees: u128,
    ) -> Result<FunaiHeaderInfo, ChainstateError> {
        if new_tip.parent_block_id
            != FunaiBlockId::new(&FIRST_BURNCHAIN_CONSENSUS_HASH, &FIRST_STACKS_BLOCK_HASH)
        {
            // not the first-ever block, so linkage must occur
            match parent_tip {
                FunaiBlockHeaderTypes::Epoch2(..) => {
                    assert_eq!(
                        new_tip.parent_block_id,
                        FunaiBlockId::new(&parent_consensus_hash, &parent_tip.block_hash())
                    );
                }
                FunaiBlockHeaderTypes::Nakamoto(nakamoto_header) => {
        ...
        let new_tip_info = FunaiHeaderInfo {
            ...
            funai_block_height: new_tip.chain_length,
        ...
        Self::insert_funai_block_header(
        ...
        if let Some(matured_miner_payouts) = mature_miner_payouts_opt {
            let rewarded_miner_block_id = FunaiBlockId::new(
                &matured_miner_payouts.reward_info.from_block_consensus_hash,
                &matured_miner_payouts.reward_info.from_funai_block_hash,
            );
            let rewarded_parent_miner_block_id = FunaiBlockId::new(
                &matured_miner_payouts
                    .reward_info
                    .from_parent_block_consensus_hash,
                &matured_miner_payouts
                    .reward_info
                    .from_parent_funai_block_hash,
            );
    ...
    /// * Process any new Funai-on-Bitcoin transactions
    ///
    ...
    /// * parent_consensus_hash, parent_header_hash, parent_funai_height, parent_burn_height:
    /// pointer to the already-processed parent Funai block
    ...
    pub fn setup_block<'a, 'b>(
        ...
        let tip_index_hash = FunaiBlockId::new(&parent_consensus_hash, &parent_header_hash);
        ...
        let initial_cost = if new_tenure || tenure_extend {
            ...
            let parent_cost_total =
                Self::get_total_tenure_cost_at(&chainstate_tx.deref().deref(), &parent_index_hash)?
                    ...
                    "Failed to load total tenure cost from parent. parent_funai_block_id = {}",
                    &parent_index_hash
                ))
                    })?;
            parent_cost_total
        ...
        let (applied_epoch_transition, mut tx_receipts) =
            FunaiChainState::process_epoch_transition(&mut clarity_tx, burn_header_height)?;
        ...
        let auto_unlock_events = if evaluated_epoch >= FunaiEpochId::Epoch21 {
            let unlock_events = FunaiChainState::check_and_handle_reward_start(
                burn_header_height.into(),
                sortition_dbconn.as_burn_state_db(),
                sortition_dbconn,
                &mut clarity_tx,
    ...
    /// Append a Nakamoto Funai block to the Funai chain state.
    ...
    > {
        ...
        let parent_block_id = FunaiBlockId::new(&parent_ch, &parent_block_hash);
        ...
        let expected_burn_opt = Self::get_expected_burns(burn_dbconn, chainstate_tx, block)
            .map_err(|e| {
                ...
                ChainstateError::InvalidFunaiBlock("Invalid Nakamoto block: could not find sortition burns".into())
            })?;
        ...
        let SetupBlockResult {
            ...
            parent_ch,
            parent_block_hash,
            parent_chain_tip.funai_block_height,
            parent_chain_tip.burn_header_height,
            burn_header_hash,
            burn_header_height.try_into().map_err(|_| {
                ChainstateError::InvalidFunaiBlock("Burn block height exceeded u32".into())
            })?,
            new_tenure,
            coinbase_height,
            tenure_extend,
        ...
        debug!(
            ...
            "funai_height" => next_block_height,
        ...
        block_execution_cost.sub(&starting_cost).map_err(|_e| {
            ChainstateError::InvalidFunaiBlock("Block execution cost was negative".into())
        })?;
        ...
        let mut lockup_events =
            match Self::finish_block(&mut clarity_tx, matured_miner_rewards_opt.as_ref()) {
                Err(ChainstateError::InvalidFunaiBlock(e)) => {
                    clarity_tx.rollback_block();
                    return Err(ChainstateError::InvalidFunaiBlock(e));
                }
                Err(e) => return Err(e),
            ...
            if block.is_first_mined() {
                ...
                let sn = SortitionDB::get_block_snapshot_consensus(burn_dbconn, &parent_ch)?
                    .ok_or_else(|| {
                        ...
                        ChainstateError::InvalidFunaiBlock("No parent consensus hash".into())
                    })?;
                (
                    sn.burn_header_hash,
                    sn.block_height,
    ...
    /// Create a FunaiDB config for the .miners contract.
    /// It has two slots -- one for the past two sortition winners.
    pub fn make_miners_funaidb_config(
        sortdb: &SortitionDB,
        tip: &BlockSnapshot,
    ) -> Result<FunaiDBConfig, ChainstateError> {
        ...
        // the funaidb signer list -- if a miner was in slot 0, then it should stay in slot 0
        ...
        let signers = miner_key_hash160s
            ...
            .map(|hash160|
                ...
                (
                    FunaiAddress {
                        version: 1, // NOTE: the version is ignored in funaidb; we only care about the hashbytes
                        bytes: hash160
                    },
                    MINER_SLOT_COUNT,
                ))
        ...
        Ok(FunaiDBConfig {
            ...
            hint_replicas: vec![], // TODO: is there a way to get the IP addresses of funais' preferred nodes?
        })
    }

    /// Get the slot range for the given miner's public key.
    /// Returns Some(Range<u32>) if the miner is in the FunaiDB config, where the range of slots for the miner is [start, end).
    ///   i.e., inclusive of `start`, exclusive of `end`.
    /// Returns None if the miner is not in the FunaiDB config.
    /// Returns an error if the miner is in the FunaiDB config but the slot number is invalid.
    pub fn get_miner_slot(
        sortdb: &SortitionDB,
        tip: &BlockSnapshot,
        miner_pubkey: &FunaiPublicKey,
    ) -> Result<Option<Range<u32>>, ChainstateError> {
        let miner_hash160 = Hash160::from_node_public_key(&miner_pubkey);
        let funaidb_config = Self::make_miners_funaidb_config(sortdb, &tip)?;

        // find out which slot we're in
        let mut slot_index = 0;
        let mut slot_id_result = None;
        for (addr, slot_count) in funaidb_config.signers.iter() {
        ...
        let Some(slot_id_range) = slot_id_result else {
            ...
            warn!("Miner is not in the miners FunaiDB config";
                  "miner" => %miner_hash160,
                  "funaidb_slots" => format!("{:?}", &funaidb_config.signers));
    ...
    /// TODO: This should be removed once it's possible for funais to vote on the aggregate
    /// public key
...
impl FunaiMessageCodec for NakamotoBlock {
    fn consensus_serialize<W: std::io::Write>(&self, fd: &mut W) -> Result<(), CodecError> {
    ...
    fn consensus_deserialize<R: std::io::Read>(fd: &mut R) -> Result<Self, CodecError> {
        ...
        if !FunaiBlock::validate_transactions_unique(&txs) {
