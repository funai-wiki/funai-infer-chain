...
use funai_common::types::chainstate::{
    ...
    FunaiPrivateKey, FunaiPublicKey,
};
use funai_common::types::{FunaiEpoch, FunaiEpochId};
...
use crate::chainstate::funai::boot::{RewardSet, SIGNERS_NAME};
use crate::chainstate::funai::db::{FunaiBlockHeaderTypes, FunaiChainState};
...
/// In Nakamoto, the PoX anchor block for reward cycle _R_ is the _first_ Funai block mined in the
...
///
/// If this method returns None, the caller should try again when there are more Funai blocks.  In
...
pub fn get_nakamoto_reward_cycle_info<U: RewardSetProvider>(
    ...
    assert!(
        epoch_at_height >= FunaiEpochId::Epoch25,
    ...
    // Find the first Funai block in this reward cycle's preceding prepare phase.
    // This block will have invoked `.signers.funaidb-set-signer-slots()` with the reward set.
    // Note that we may not have processed it yet. But, if we do find it, then it's
    // unique (and since Nakamoto Funai blocks are processed in order, the anchor block
    // cannot change later).
    ...
    let anchor_block_header = match prepare_phase_sortitions
        ...
        .find_map(|sn| {
            ...
            match FunaiChainState::get_funai_block_header_info_by_consensus_hash(
                chain_state.db(),
                &sn.consensus_hash,
            ) {
                Ok(Some(x)) => return Some(Ok(x)),
        ...
        .transpose()?
    {
        ...
        None => {
            ...
            // This can happen when the node was offline during the entire prepare phase
            // and Bitcoin blocks accumulated. In this case, fall back to the canonical
            // Stacks block as the anchor block, and reuse the previous cycle's reward set.
            warn!(
    ...
    let (block_id, funai_block_hash) = match anchor_block_header.anchored_header {
        FunaiBlockHeaderTypes::Epoch2(ref header) => (
            FunaiBlockId::new(&anchor_block_header.consensus_hash, &header.block_hash()),
            header.block_hash(),
        ),
        FunaiBlockHeaderTypes::Nakamoto(ref header) => {
    ...
    let reward_set = match provider.get_reward_set_nakamoto(
        ...
        Ok(rs) => rs,
        Err(initial_err) => {
            // FALLBACK: The reward set for the target cycle may not have been written
            // to .signers (e.g., no Stacks block was processed during the prepare phase).
            // Walk backwards through previous cycles until we find a valid reward set.
            // This handles multi-cycle gaps where the node was offline for extended periods.
    ...
    let anchor_status = PoxAnchorBlockStatus::SelectedAndKnown(funai_block_hash, txid, reward_set);
...
pub fn get_nakamoto_next_recipients(
    ...
    let reward_cycle_info = if burnchain.is_reward_cycle_start(sortition_tip.block_height + 1) {
        ...
        // NOTE: this must panic because Nakamoto's first reward cycle has funais
        ...
        // NOTE: don't panic here. The only caller of this method is a funai-node miner,
    ...
    > ChainsCoordinator<'a, T, N, U, CE, FE, B>
{
    ...
    /// to ensure that the PoX funais have been selected for this cycle.  This means that we
    /// don't proceed to process Nakamoto blocks until the reward cycle has begun.  Also, the last
    /// reward cycle of epoch2 _must_ be PoX so we have funais who can sign.
    pub fn can_process_nakamoto(&mut self) -> Result<bool, Error> {
        ...
        let cur_epoch =
            SortitionDB::get_funai_epoch(self.sortition_db.conn(), canonical_sn.block_height)?
                .unwrap_or_else(|| {
                    panic!(
        ...
        // in epoch3
        let all_epochs = SortitionDB::get_funai_epochs(self.sortition_db.conn())?;
        let epoch_3_idx = FunaiEpoch::find_epoch_by_id(&all_epochs, FunaiEpochId::Epoch30)
    ...
    pub fn handle_comms_nakamoto(
        ...
        if (bits & (CoordinatorEvents::NEW_STACKS_BLOCK as u8)) != 0 {
            ...
            debug!("Received new Nakamoto funai block notice");
            ...
            if !self.in_nakamoto_epoch {
                ...
                if let Ok(Some(canonical_header)) = NakamotoChainState::get_canonical_block_header(
                    ...
                    if canonical_header.is_nakamoto_block() {
                        ...
                        debug!(
                            "The canonical Funai tip ({}/{}) is a Nakamoto block!",
                        ...
                        debug!("Received new epoch 2.x Funai block notice");
            ...
            match self.handle_new_nakamoto_funai_block() {
    ...
    /// Handle one or more new Nakamoto Funai blocks.
    ...
    /// DB.
    pub fn handle_new_nakamoto_funai_block(&mut self) -> Result<Option<BlockHeaderHash>, Error> {
        ...
        loop {
            ...
            let mut processed_block_receipt = match NakamotoChainState::process_next_nakamoto_block(
                ...
                Err(ChainstateError::InvalidFunaiBlock(msg)) => {
            ...
            let (
                canonical_funai_block_id,
                canonical_funai_block_height,
                canonical_funai_consensus_hash,
            ) = {
            ...
            debug!("Bump blocks processed ({})", &canonical_funai_block_id);
            ...
            if let Some(ref mut estimator) = self.fee_estimator {
                let funai_epoch = self
                    ...
                    .get_funai_epoch_by_epoch_id(&block_receipt.evaluated_epoch)
                    .expect("Could not find a funai epoch.");
                if let Err(e) = estimator.notify_block(&block_receipt, &funai_epoch.block_limit) {
            ...
            let funai_sn = SortitionDB::get_block_snapshot_consensus(
            ...
            if !self.burnchain.is_in_prepare_phase(funai_sn.block_height) {
                // next ready funai block
                continue;
            }
            ...
            let current_reward_cycle = self
                .burnchain
                .block_height_to_reward_cycle(funai_sn.block_height)
                .unwrap_or_else(|| {
            ...
            // This is the first Funai block in the prepare phase for the next reward cycle.
    ...
    pub fn handle_new_nakamoto_burnchain_block(&mut self) -> Result<bool, Error> {
        ...
        // Unlike in Funai 2.x, there can be neither chain reorgs nor PoX reorgs unless Bitcoin itself
        ...
        for unprocessed_block in sortitions_to_process.into_iter() {
            ...
            let reward_cycle_info = if self.burnchain.is_reward_cycle_start(header.block_height) {
                ...
                if let Some(rc_info) = reward_cycle_info.as_ref() {
                    // in nakamoto, if we have any reward cycle info at all, it will be known.
                    // otherwise, we may have to process some more Funai blocks
