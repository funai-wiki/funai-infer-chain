...
use funai_common::types::chainstate::{
    FunaiAddress, FunaiBlockId, FunaiPrivateKey, FunaiPublicKey,
};
use funai_common::types::{Address, FunaiEpoch};
...
use crate::chainstate::funai::boot::signers_tests::{readonly_call, readonly_call_with_sortdb};
use crate::chainstate::funai::boot::test::{
    key_to_funai_addr, make_pox_4_lockup, make_signer_key_signature,
    make_signers_vote_for_aggregate_public_key, make_signers_vote_for_aggregate_public_key_value,
    with_sortdb,
};
use crate::chainstate::funai::boot::{MINERS_NAME, SIGNERS_NAME};
use crate::chainstate::funai::db::{MinerPaymentTxFees, FunaiAccount, FunaiChainState};
use crate::chainstate::funai::{
    CoinbasePayload, FunaiTransaction, FunaiTransactionSigner, TenureChangeCause,
...
use crate::net::funaidb::FunaiDBConfig;
...
fn advance_to_nakamoto(
    ...
    for sortition_height in 0..11 {
        ...
        let txs = if sortition_height == 6 {
            // Make all the test Funais stack
            test_funais
                .iter()
                .map(|test_funai| {
                    ...
                    let signing_key =
                        FunaiPublicKey::from_private(&test_funai.signer_private_key);
                    make_pox_4_lockup(
                        &test_funai.funai_private_key,
                        0,
                        test_funai.amount,
                        &pox_addr,
                        12,
...
pub fn make_all_signers_vote_for_aggregate_key(
    ...
    for test_funai in test_funais {
        let addr = key_to_funai_addr(&test_funai.signer_private_key);
...
pub fn boot_nakamoto<'a>(
    ...
    peer_config.epochs = Some(FunaiEpoch::unit_test_3_0_only(37));
    peer_config.initial_balances = vec![(addr.to_account_principal(), 1_000_000_000_000_000_000)];

    // Create some balances for test Funais
    let mut funai_balances = test_funais
        .iter()
        .map(|test_funai| {
            (
                PrincipalData::from(key_to_funai_addr(&test_funai.funai_private_key)),
                u64::try_from(test_funai.amount + 10000).expect("Stacking amount too large"),
            )
        })
        .collect();

    // Create some balances for test Signers
    let mut signer_balances = test_funais
        .iter()
        .map(|funai| {
            (
                PrincipalData::from(p2pkh_from(&funai.signer_private_key)),
                1000,
            )
        })
        .collect();

    peer_config.initial_balances.append(&mut funai_balances);
...
pub fn make_replay_peer<'a>(peer: &mut TestPeer<'a>) -> TestPeer<'a> {
    ...
    replay_config.test_funais = peer.config.test_funais.clone();

    let test_funais = replay_config.test_funais.clone().unwrap_or(vec![]);
...
pub fn make_token_transfer(
    ...
    let mut tx_signer = FunaiTransactionSigner::new(&stx_transfer);
...
fn replay_reward_cycle(
    ...
    funai_blocks: &[NakamotoBlock],
) {
    eprintln!("\n\n=============================================\nBegin replay\n==============================================\n");
    let reward_cycle_length = peer.config.burnchain.pox_constants.reward_cycle_length as usize;
    let reward_cycle_indices: Vec<usize> = (0..funai_blocks.len())
        .step_by(reward_cycle_length)
        .collect();

    let mut indexes: Vec<_> = (0..funai_blocks.len()).collect();
    ...
    let mut blocks_to_process = funai_blocks.to_vec();
    ...
    while let Some(block) = blocks_to_process.pop() {
        ...
        if accepted {
            ...
            peer.coord.handle_new_nakamoto_funai_block().unwrap();
        } else {
...
fn test_simple_nakamoto_coordinator_bootup() {
    ...
    assert_eq!(
        tip.anchored_header.as_funai_nakamoto().unwrap(),
...
fn test_simple_nakamoto_coordinator_1_tenure_10_blocks() {
    let private_key = FunaiPrivateKey::from_seed(&[2]);
    let addr = FunaiAddress::from_public_keys(
        C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
        &AddressHashMode::SerializeP2PKH,
        1,
    ...
    let tip = {
        let chainstate = &mut replay_peer.funai_node.as_mut().unwrap().chainstate;
...
fn test_nakamoto_chainstate_getters() {
    ...
    };
    {
        ...
        let chainstate = &peer.funai_node.as_mut().unwrap().chainstate;
        let sort_db = peer.sortdb.as_mut().unwrap();
    ...
    };
    {
        ...
        let (mut funai_db_tx, _) = chainstate.chainstate_tx_begin().unwrap();
            ...
            + 1)
        {
                    ...
                    - 10
            {
                ...
                assert!(header.anchored_header.as_funai_epoch2().is_some());
            } else {
                // last tenure is nakamoto
                assert!(header.anchored_header.as_funai_nakamoto().is_some());
            }
        }
    }
    ...
    debug!("\n======================================\nBegin tests\n===========================================\n");
    {
        ...
        let (cur_funai_ch, cur_funai_bhh, cur_funai_height) =
            SortitionDB::get_canonical_funai_chain_tip_hash_and_height(sort_tx.sqlite()).unwrap();
        sort_tx
            .test_update_canonical_funai_tip(
    ...
    let parent_vrf_proof = NakamotoChainState::get_parent_vrf_proof(
        &peer.funai_node.as_ref().unwrap().chainstate.db(),
...
pub fn simple_nakamoto_coordinator_10_tenures_10_sortitions<'a>() -> TestPeer<'a> {
    ...
    let stx_miner_addr = FunaiAddress::from_public_keys(
        C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
        &AddressHashMode::SerializeP2PKH,
        1,
        &vec![FunaiPublicKey::from_private(&private_key)],
    ...
    for (i, ch) in consensus_hashes.into_iter().enumerate() {
        ...
        let block_id = FunaiBlockId(sn.winning_funai_block_hash.0);
