...
use clarity::vm::types::{
    PrincipalData, QualifiedContractIdentifier, FunaiAddressExtensions, TupleData,
};
...
impl RawRewardSetEntry {
    pub fn from_pox_4_tuple(is_mainnet: bool, tuple: TupleData) -> Result<Self, ChainstateError> {
        ...
        let funai = tuple_data
            .remove("funai")
            .expect("FATAL: no 'funai' in return value from (pox-4.get-reward-set-pox-address)")
            .expect_optional()?
...
impl NakamotoSigners {
    ...
    pub fn handle_signer_funaidb_update(
        clarity: &mut ClarityTransactionConnection,
        ...
        let reward_set =
            FunaiChainState::make_reward_set(threshold, reward_slots, FunaiEpochId::Epoch30);

        let funaidb_list = if participation == 0 {
            vec![]
        } else {
            reward_set
                .signers
                ...
                .map(|signer| {
                    ...
                    let signing_address = FunaiAddress::p2pkh_from_hash(is_mainnet, signer_hash);
                    Value::Tuple(
        ...
        let set_funaidb_args = [
            SymbolicExpression::atom_value(Value::cons_list_unsanitized(funaidb_list).expect(
        ...
        let (value, _, events, _) = clarity
            ...
            .expect("FATAL: failed to update signer funaidb");
    ...
    pub fn check_and_handle_prepare_phase_start(
        ...
        if current_epoch < FunaiEpochId::Epoch25 {
    ...
    pub fn get_signers_weights(
        ...
        reward_cycle: u64,
    ) -> Result<HashMap<FunaiAddress, u64>, ChainstateError> {
    ...
    pub fn valid_vote_transaction(
        account_nonces: &HashMap<FunaiAddress, u64>,
    ...
    pub fn update_filtered_transactions(
        filtered_transactions: &mut HashMap<FunaiAddress, FunaiTransaction>,
        account_nonces: &HashMap<FunaiAddress, u64>,
        mainnet: bool,
