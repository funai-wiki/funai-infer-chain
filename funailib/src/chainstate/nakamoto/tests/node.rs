...
use funai_common::types::chainstate::{BlockHeaderHash, SortitionId, FunaiBlockId, VRFSeed};
...
use crate::chainstate::funai::tests::TestFunaiNode;
use crate::chainstate::funai::{
    Error as ChainstateError, FunaiBlock, C32_ADDRESS_VERSION_TESTNET_SINGLESIG, *,
};
...
#[derive(Debug, Clone)]
pub struct TestFunai {
    pub funai_private_key: FunaiPrivateKey,
    pub signer_private_key: FunaiPrivateKey,
    pub amount: u128,
}

impl TestFunai {
    pub const DEFAULT_STACKER_AMOUNT: u128 = 1_000_000_000_000_000_000;
    pub fn from_seed(seed: &[u8]) -> TestFunai {
        let funai_private_key = FunaiPrivateKey::from_seed(seed);
        let mut signer_seed = seed.to_vec();
        signer_seed.append(&mut vec![0xff, 0x00, 0x00, 0x00]);
        let signer_private_key = FunaiPrivateKey::from_seed(signer_seed.as_slice());
        TestFunai {
            funai_private_key,
            signer_private_key,
    ...
    pub fn signer_public_key(&self) -> FunaiPublicKey {
        FunaiPublicKey::from_private(&self.signer_private_key)
    }

    /// make a set of funais who will share a single signing key and stack with
    /// `Self::DEFAULT_STACKER_AMOUNT`
    pub fn common_signing_set(test_signers: &TestSigners) -> Vec<TestFunai> {
        let mut signing_key_seed = test_signers.num_keys.to_be_bytes().to_vec();
        signing_key_seed.extend_from_slice(&[1, 1, 1, 1]);
        let signing_key = FunaiPrivateKey::from_seed(signing_key_seed.as_slice());
        (0..test_signers.num_keys)
            .map(|index| TestFunai {
                signer_private_key: signing_key.clone(),
                funai_private_key: FunaiPrivateKey::from_seed(&index.to_be_bytes()),
...
impl TestMiner {
    pub fn nakamoto_miner_key(&self) -> FunaiPrivateKey {
        self.privks[0].clone()
    }

    pub fn nakamoto_miner_hash160(&self) -> Hash160 {
        let pubk = FunaiPublicKey::from_private(&self.nakamoto_miner_key());
    ...
    pub fn make_nakamoto_tenure_change(
        ...
        let mut tx_tenure_change = FunaiTransaction::new(
            TransactionVersion::Testnet,
            self.as_transaction_auth().unwrap(),
        ...
        let mut tx_signer = FunaiTransactionSigner::new(&tx_tenure_change);
        self.sign_as_origin(&mut tx_signer);
...
impl TestFunaiNode {
    pub fn add_nakamoto_tenure_commit(
        ...
        miner: &mut TestMiner,
        last_tenure_start: &FunaiBlockId,
        burn_amount: u64,
    ...
    pub fn make_nakamoto_tenure_commitment(
        ...
        let block_commit_op = TestFunaiNode::add_nakamoto_tenure_commit(
            sortdb,
            burn_block,
            miner,
            &last_tenure_id,
    ...
    pub fn begin_nakamoto_tenure(
        ...
        burn_block: &mut TestBurnchainBlock,
        miner_key: &LeaderKeyRegisterOp,
        // parent Funai block, if this is the first Nakamoto tenure
        ...
        let (last_tenure_id, parent_block_snapshot) =
            if let Some(parent_blocks) = parent_nakamoto_tenure {
                ...
                let parent_tenure_id = FunaiBlockId::new(
            ...
            } else if let Some(parent_funai_block) = parent_funai_block {
                ...
                test_debug!(
                    "Work in {} {} for Funai 2.x parent: {},{}",
                    burn_block.block_height,
                    burn_block.parent_snapshot.burn_header_hash,
                    parent_funai_block_snapshot.total_burn,
                    parent_chain_tip.anchored_header.height(),
                );

                (parent_tenure_id, parent_funai_block_snapshot)
            } else {
        ...
        let (previous_tenure_end, previous_tenure_consensus_hash, previous_tenure_blocks) = {
            let hdr = NakamotoChainState::get_canonical_block_header(self.chainstate.db(), &sortdb)
                .unwrap()
                .unwrap();
            if hdr.anchored_header.as_funai_nakamoto().is_some() {
    ...
    pub fn make_nakamoto_tenure_blocks<'a, F>(
        ...
        mut tenure_change: Option<FunaiTransaction>,
        mut coinbase: Option<FunaiTransaction>,
        ...
        ) -> Vec<FunaiTransaction>,
    {
        ...
        loop {
            ...
            if accepted {
                ...
                coord.handle_new_nakamoto_funai_block().unwrap();
                ...
                let funai_chain_tip =
                    NakamotoChainState::get_canonical_block_header(chainstate.db(), &sortdb)
                        .unwrap()
                        .unwrap();
                let nakamoto_chain_tip = funai_chain_tip
                    .anchored_header
                    .as_funai_nakamoto()
            ...
            match builder.try_mine_tx_with_len(
                ...
                | TransactionResult::ProcessingError(TransactionError { error, .. }) => {
                    match error {
                        ...
                        ChainstateError::InvalidFunaiTransaction(_emsg, true) => {
...
impl<'a> TestPeer<'a> {
    ...
    fn get_nakamoto_parent(
        ...
        if let Some(parent_blocks) = funai_node.get_last_nakamoto_tenure(miner) {
            ...
            let last_tenure_id = FunaiBlockId::new(
                &first_parent.header.consensus_hash,
                &first_parent.header.block_hash(),
            );
            (
            ...
            let (parent_opt, parent_sortition_opt) = if let Some(parent_block) =
                funai_node.get_last_anchored_block(miner)
            {
                debug!("Parent will be a Funai 2.x block");
                let ic = sortdb.index_conn();
                let sort_opt = SortitionDB::get_block_snapshot_for_winning_funai_block(
                    &ic,
                    &tip.sortition_id,
            ...
            let last_tenure_id = if let Some(last_epoch2_block) = parent_opt.as_ref() {
                ...
                FunaiBlockId(BOOT_BLOCK_HASH.0.clone())
            };
    ...
    pub fn begin_nakamoto_tenure(
        ...
        let (last_tenure_id, parent_block_opt, parent_tenure_opt, parent_sortition_opt) =
            Self::get_nakamoto_parent(&self.miner, &funai_node, &sortdb);
        ...
        let (mut block_commit_op, tenure_change_payload) = funai_node.begin_nakamoto_tenure(
            &sortdb,
            &mut self.miner,
            &mut burn_block,
            &last_key,
        ...
        if last_tenure_id == FunaiBlockId(BOOT_BLOCK_HASH.0.clone()) {
        ...
        let miner_account = get_account(&mut funai_node.chainstate, &sortdb, &miner_addr);
        ...
        ) -> Vec<FunaiTransaction>,
    {
        ...
        let blocks = TestFunaiNode::make_nakamoto_tenure_blocks(
        ...
        let just_blocks = blocks
            ...
            .map(|(block, _, _)| block)
            .collect();
        funai_node.add_nakamoto_tenure_blocks(just_blocks);
    ...
    pub fn make_nakamoto_tenure_extension<F>(
        &mut self,
        tenure_extend_tx: FunaiTransaction,
        signers: &mut TestSigners,
        block_builder: F,
    ) -> Vec<(NakamotoBlock, u64, ExecutionCost)>
    where
        ...
        ) -> Vec<FunaiTransaction>,
    {
        ...
        let just_blocks = blocks
            ...
            .map(|(block, _, _)| block)
            .collect();
        funai_node.add_nakamoto_extended_blocks(just_blocks);
