...
use clarity::types::chainstate::{PoxId, SortitionId, FunaiBlockId};
...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, ConsensusHash, FunaiAddress, FunaiPrivateKey,
    FunaiPublicKey, FunaiWorkScore, TrieHash, VRFSeed,
};
use funai_common::types::{Address, PrivateKey, FunaiEpoch, FunaiEpochId};
use funai_common::util::get_epoch_time_secs;
use funai_common::util::hash::{hex_bytes, Hash160, MerkleTree, Sha512Trunc256Sum};
use funai_common::util::secp256k1::{MessageSignature, Secp256k1PublicKey};
use funai_common::util::vrf::{VRFPrivateKey, VRFProof, VRFPublicKey, VRF};
...
use crate::chainstate::funai::db::{
    ...
    ChainstateBNSNamespace, FunaiAccount, FunaiBlockHeaderTypes, FunaiChainState,
    FunaiHeaderInfo,
};
use crate::chainstate::funai::{
    CoinbasePayload, Error as ChainstateError, FunaiBlock, FunaiBlockHeader, FunaiTransaction,
    FunaiTransactionSigner, TenureChangeCause, TenureChangePayload, ThresholdSignature,
...
use crate::core;
use crate::core::{FunaiEpochExtension, STACKS_EPOCH_3_0_MARKER};
use crate::net::codec::test::check_codec_and_corruption;
use crate::net::funaidb::MINER_SLOT_COUNT;
...
pub fn get_account(
    ...
    chainstate
        .with_read_only_clarity_tx(
            ...
            &tip.index_block_hash(),
            |clarity_conn| {
                FunaiChainState::get_account(clarity_conn, &addr.to_account_principal())
            },
        )
        .unwrap()
}
...
fn test_path(name: &str) -> String {
    format!("/tmp/funai-node-tests/nakamoto-tests/{}", name)
}
...
fn codec_nakamoto_header() {
    let header = NakamotoBlockHeader {
        ...
        parent_block_id: FunaiBlockId([0x05; 32]),
    ...
    let mut bytes = vec![
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, // funai signature (mocked)
...
pub fn test_nakamoto_first_tenure_block_syntactic_validation() {
    ...
    let mut tenure_change_tx = FunaiTransaction::new(
    ...
    let mut tenure_extend_tx = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
    ...
    let mut invalid_tenure_change_tx = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
    ...
    let mut invalid_coinbase_tx = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
    ...
    let recipient_addr =
        FunaiAddress::from_string("ST2YM3J4KQK09V670TD6ZZ1XYNYCNGCWCVTASN5VM").unwrap();
    let mut stx_transfer = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
    ...
    assert_eq!(
        block.validate_transactions_static(false, 0x80000000, FunaiEpochId::Epoch30),
        false
...
pub fn test_load_store_update_nakamoto_blocks() {
    ...
    let epochs = FunaiEpoch::unit_test_3_0_only(1);
    ...
    let epoch2_header = FunaiBlockHeader {
        version: 0,
    ...
    let epoch2_parent_block_id =
        FunaiBlockId::new(&epoch2_consensus_hash, &epoch2_header.block_hash());

    let epoch2_header_info = FunaiHeaderInfo {
        anchored_header: FunaiBlockHeaderTypes::Epoch2(epoch2_header.clone()),
        microblock_tail: None,
        funai_block_height: epoch2_header.total_work.work,
    ...
    let recipient_addr =
        FunaiAddress::from_string("ST2YM3J4KQK09V670TD6ZZ1XYNYCNGCWCVTASN5VM").unwrap();
    let mut stx_transfer_tx = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
    ...
    let mut stx_transfer_tx_3 = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
    ...
    let nakamoto_header_info = FunaiHeaderInfo {
        anchored_header: FunaiBlockHeaderTypes::Nakamoto(nakamoto_header.clone()),
        microblock_tail: None,
        funai_block_height: nakamoto_header.chain_length,
    ...
    let nakamoto_header_info_2 = FunaiHeaderInfo {
        anchored_header: FunaiBlockHeaderTypes::Nakamoto(nakamoto_header_2.clone()),
        microblock_tail: None,
        funai_block_height: nakamoto_header_2.chain_length,
        index_root: TrieHash([0x67; 32]),
    ...
    let nakamoto_header_info_3 = FunaiHeaderInfo {
        anchored_header: FunaiBlockHeaderTypes::Nakamoto(nakamoto_header_3.clone()),
        microblock_tail: None,
        funai_block_height: nakamoto_header_2.chain_length,
        index_root: TrieHash([0x67; 32]),
        consensus_hash: nakamoto_header_2.consensus_hash.clone(),
    ...
    // store epoch2 and nakamoto headers
    {
        ...
        // this succeeds now
        NakamotoChainState::insert_funai_block_header(
            &tx,
    ...
    // next ready nakamoto block is None unless both the burn block and funai parent block have
    // been processed
    {
        let (tx, staging_tx) = chainstate.headers_and_staging_tx_begin().unwrap();
...
fn test_nakamoto_block_static_verification() {
    let private_key = FunaiPrivateKey::new();
    let private_key_2 = FunaiPrivateKey::new();
    ...
    let tenure_change_payload = TenureChangePayload {
        ...
        prev_tenure_consensus_hash: ConsensusHash([0x01; 20]),
        burn_view_consensus_hash: ConsensusHash([0x04; 20]),
        previous_tenure_end: FunaiBlockId([0x03; 32]),
        previous_tenure_blocks: 1,
        cause: TenureChangeCause::BlockFound,
        pubkey_hash: Hash160::from_node_public_key(&FunaiPublicKey::from_private(&private_key)),
    };
    ...
    let mut tenure_change_tx_bad_ch = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
    ...
    let mut tenure_change_tx_bad_miner_sig = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
    ...
    let mut nakamoto_header = NakamotoBlockHeader {
        ...
        burn_spent: 126,
        consensus_hash: tenure_change_payload.tenure_consensus_hash.clone(),
        parent_block_id: FunaiBlockId([0x03; 32]),
        tx_merkle_root: nakamoto_tx_merkle_root,
        state_index_root: TrieHash([0x07; 32]),
        miner_signature: MessageSignature::empty(),
        signer_signature: ThresholdSignature::empty(),
    ...
    assert_eq!(
        ...
        FunaiPublicKey::from_private(&private_key)
    );
    assert_eq!(
    ...
    assert!(nakamoto_block
        .check_miner_signature(&Hash160::from_node_public_key(
            &FunaiPublicKey::from_private(&private_key_2)
        ))
        .is_err());
...
fn make_fork_run_with_arrivals(
    ...
    for i in last_snapshot.block_height..(last_snapshot.block_height + length) {
        let snapshot = BlockSnapshot {
            ...
            arrival_index: 0,
            canonical_funai_tip_height: last_snapshot.canonical_funai_tip_height + 10,
            canonical_funai_tip_hash: BlockHeaderHash([((i + 1) as u8) | bit_pattern; 32]),
            canonical_funai_tip_consensus_hash: ConsensusHash([((i + 1) as u8) | bit_pattern; 20]),
            miner_pk_hash: None,
        };
...
pub fn test_get_highest_nakamoto_tenure() {
    ...
    let test_funais = TestFunai::common_signing_set(&test_signers);
    ...
    let chainstate = &mut peer.funai_node.as_mut().unwrap().chainstate;
    ...
    for (i, sn) in snapshots.iter().enumerate() {
        let block_header = NakamotoBlockHeader {
            version: 0,
            chain_length: sn.canonical_funai_tip_height,
            burn_spent: i as u64,
    ...
    let (funai_ch, funai_bhh, funai_height) =
        SortitionDB::get_canonical_funai_chain_tip_hash_and_height(sort_db.conn()).unwrap();
    debug!("tip = {:?}", &tip);
    debug!(
        "funai tip = {},{},{}",
        &funai_ch, &funai_bhh, funai_height
    );
    let highest_tenure =
...
/// Test that we can generate a .miners funaidb config.
...
#[test]
fn test_make_miners_funaidb_config() {
    let mut test_signers = TestSigners::default();
    let test_funais = TestFunai::common_signing_set(&test_signers);
    let mut peer = boot_nakamoto(
        function_name!(),
    ...
    let miner_keys: Vec<_> = (0..10).map(|_| FunaiPrivateKey::new()).collect();
    let miner_hash160s: Vec<_> = miner_keys
        .iter()
        .map(|miner_privkey| {
            let miner_pubkey = FunaiPublicKey::from_private(miner_privkey);
    ...
    let miner_addrs: Vec<_> = miner_hash160s
        .iter()
        .map(|miner_hash160| FunaiAddress {
            version: 1,
    ...
    // extract chainstate, sortdb, and funaidbs -- we don't need the peer anymore
    let chainstate = &mut peer.funai_node.as_mut().unwrap().chainstate;
    let sort_db = peer.sortdb.as_mut().unwrap();
    let mut last_snapshot = SortitionDB::get_canonical_burn_chain_tip(sort_db.conn()).unwrap();
    let funaidbs = peer.network.funaidbs;
    ...
    let mut funaidb_configs = vec![];
    let mut funaidb_chunks = vec![];
    ...
    for (i, miner) in miners.iter().enumerate() {
        ...
        let snapshot = BlockSnapshot {
            ...
            winning_funai_block_hash: winning_block_hash.clone(),
            index_root: TrieHash([0u8; 32]),
            num_sortitions: last_snapshot.num_sortitions + if sortition { 1 } else { 0 },
            funai_block_accepted: false,
            funai_block_height: last_snapshot.funai_block_height,
            arrival_index: 0,
            canonical_funai_tip_height: last_snapshot.canonical_funai_tip_height + 10,
            canonical_funai_tip_hash: BlockHeaderHash([id; 32]),
            canonical_funai_tip_consensus_hash: ConsensusHash([id; 20]),
            miner_pk_hash: None,
        };
        ...
        let tip = SortitionDB::get_canonical_burn_chain_tip(sort_db.conn()).unwrap();
        // check the funaidb config as of this chain tip
        let funaidb_config =
            NakamotoChainState::make_miners_funaidb_config(sort_db, &tip).unwrap();
        eprintln!(
            "funaidb_config at i = {} (sorition? {}): {:?}",
            &i, sortition, &funaidb_config
        );

        funaidb_configs.push(funaidb_config);

        // make a funaidb chunk for a hypothetical block
        let header = NakamotoBlockHeader {
            version: 1,
            chain_length: 2,
            burn_spent: 3,
        ...
        let tip = SortitionDB::get_canonical_burn_chain_tip(sort_db.conn()).unwrap();
        if sortition {
            let chunk = NakamotoBlockBuilder::make_funaidb_block_proposal(
                &sort_db,
                &tip,
                &funaidbs,
                &block,
            ...
            funaidb_chunks.push(chunk);
        } else {
            assert!(NakamotoBlockBuilder::make_funaidb_block_proposal(
                &sort_db,
                &tip,
                &funaidbs,
                &block,
    ...
    let miner_hashbytes: Vec<_> = funaidb_configs
        .iter()
        .map(|config| {
            (
    ...
    assert_eq!(funaidb_chunks[0].slot_id, first_miner_slot);
    assert_eq!(funaidb_chunks[1].slot_id, second_miner_slot);
    assert_eq!(funaidb_chunks[2].slot_id, first_miner_slot);
    assert_eq!(funaidb_chunks[3].slot_id, second_miner_slot);
    assert_eq!(funaidb_chunks[4].slot_id, first_miner_slot);
    assert_eq!(funaidb_chunks[5].slot_id, second_miner_slot);

    assert!(funaidb_chunks[0].verify(&miner_addrs[1]).unwrap());
    assert!(funaidb_chunks[1].verify(&miner_addrs[2]).unwrap());
    assert!(funaidb_chunks[2].verify(&miner_addrs[4]).unwrap());
    assert!(funaidb_chunks[3].verify(&miner_addrs[5]).unwrap());
    assert!(funaidb_chunks[4].verify(&miner_addrs[7]).unwrap());
    assert!(funaidb_chunks[5].verify(&miner_addrs[8]).unwrap());
...
fn parse_vote_for_aggregate_public_key_invalid() {
    ...
    let chainid = CHAIN_ID_TESTNET;
    let vote_contract_id = boot_code_id(SIGNERS_VOTING_NAME, mainnet);
    let contract_addr: FunaiAddress = vote_contract_id.issuer.into();
    let contract_name = vote_contract_id.name.clone();
    ...
    let mut invalid_contract_address = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
        ...
        post_conditions: vec![],
        payload: TransactionPayload::ContractCall(TransactionContractCall {
            address: FunaiAddress::p2pkh(
                false,
                &FunaiPublicKey::from_private(&signer_private_key),
            ),
    ...
    let mut invalid_contract_name = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut invalid_signers_vote_function = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut invalid_function_arg_signer_index = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut invalid_function_arg_key = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut invalid_function_arg_round = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut invalid_function_arg_reward_cycle = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
...
fn valid_vote_transaction() {
    ...
    let mut valid_tx = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
...
fn valid_vote_transaction_malformed_transactions() {
    ...
    let mut invalid_not_contract_call = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut invalid_network = FunaiTransaction {
    ...
    let mut invalid_nonce = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
...
fn filter_one_transaction_per_signer_multiple_addresses() {
    let signer_private_key_1 = FunaiPrivateKey::new();
    let signer_private_key_2 = FunaiPrivateKey::new();
    let mainnet = false;
    let chainid = CHAIN_ID_TESTNET;
    let vote_contract_id = boot_code_id(SIGNERS_VOTING_NAME, mainnet);
    let contract_addr: FunaiAddress = vote_contract_id.issuer.into();
    ...
    let mut valid_tx_1_address_1 = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
    ...
    let mut valid_tx_2_address_1 = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key_1).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut valid_tx_3_address_1 = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key_1).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut valid_tx_1_address_2 = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
    ...
    let mut valid_tx_2_address_2 = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key_2).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
...
fn filter_one_transaction_per_signer_duplicate_nonces() {
    ...
    let mut valid_tx_1 = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut valid_tx_2 = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
    ...
    let mut valid_tx_3 = FunaiTransaction {
        version: TransactionVersion::Testnet,
        chain_id: CHAIN_ID_TESTNET,
        auth: TransactionAuth::from_p2pkh(&signer_private_key).unwrap(),
        anchor_mode: TransactionAnchorMode::Any,
