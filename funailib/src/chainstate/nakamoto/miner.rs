...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, ConsensusHash, FunaiAddress, FunaiBlockId, TrieHash,
};
use funai_common::types::FunaiPublicKeyBuffer;
use funai_common::util::get_epoch_time_ms;
use funai_common::util::hash::{Hash160, MerkleTree, Sha512Trunc256Sum};
...
use crate::chainstate::funai::db::{
    ChainstateTx, ClarityTx, MinerRewardInfo, FunaiBlockHeaderTypes, FunaiChainState,
    FunaiHeaderInfo, MINER_REWARD_MATURITY,
};
...
use crate::chainstate::funai::{Error, FunaiBlockHeader, *};
...
pub struct NakamotoTenureInfo {
    ...
    pub coinbase_tx: Option<FunaiTransaction>,
    /// Tenure change transaction from Funais
    pub tenure_change_tx: Option<FunaiTransaction>,
}
...
impl NakamotoTenureInfo {
    ...
    pub fn tenure_change_tx(&self) -> Option<&FunaiTransaction> {
        self.tenure_change_tx.as_ref()
    }

    pub fn coinbase_tx(&self) -> Option<&FunaiTransaction> {
...
pub struct NakamotoBlockBuilder {
    ...
    parent_header: Option<FunaiHeaderInfo>,
    /// Signed coinbase tx, if starting a new tenure
    coinbase_tx: Option<FunaiTransaction>,
    /// Tenure change tx, if starting or extending a tenure
    tenure_tx: Option<FunaiTransaction>,
...
pub struct MinerTenureInfo<'a> {
    ...
    pub parent_funai_block_height: u64,
...
impl NakamotoBlockBuilder {
    ...
    ///
    /// * `parent_funai_header` - the funai header this builder's block will build off
    ///
    ...
    ///
    pub fn new(
        parent_funai_header: &FunaiHeaderInfo,
        tenure_id_consensus_hash: &ConsensusHash,
        total_burn: u64,
        tenure_change: Option<&FunaiTransaction>,
        coinbase: Option<&FunaiTransaction>,
    ) -> Result<NakamotoBlockBuilder, Error> {
        let next_height = parent_funai_header
            .anchored_header
            .height()
            .checked_add(1)
            .ok_or_else(|| Error::InvalidFunaiBlock("Block height exceeded u64".into()))?;
        if matches!(
            parent_funai_header.anchored_header,
            FunaiBlockHeaderTypes::Epoch2(_)
        ) {
            // building atop a funai 2.x block.
            // we are necessarily starting a new tenure
            if tenure_change.is_none() || coinbase.is_none() {
                // not allowed
                warn!("Failed to start a Nakamoto tenure atop a Funai 2.x block -- missing a coinbase and/or tenure");
    ...
    pub fn load_tenure_info<'a>(
        ...
        let parent_block_id = FunaiBlockId::new(&parent_consensus_hash, &parent_header_hash);
        ...
        Ok(MinerTenureInfo {
            ...
            parent_header_hash,
            parent_funai_block_height: chain_tip.funai_block_height,
    ...
    pub fn tenure_begin<'a, 'b>(
        ...
        let SetupBlockResult {
            ...
            info.parent_funai_block_height,
    ...
    pub fn build_nakamoto_block(
        ...
        // Funai header we're building off of.
        parent_funai_header: &FunaiHeaderInfo,
        ...
        signer_transactions: Vec<FunaiTransaction>,
        ...
        let (blocked, tx_events) = match FunaiBlockBuilder::select_and_apply_transactions(
            &mut tenure_tx,
            &mut builder,
            mempool,
            parent_funai_header.funai_block_height,
    ...
    /// Make a FunaiDB chunk message containing a proposed block.
    /// Sign it with the miner's private key.
    /// Automatically determine which FunaiDB slot and version number to use.
    ...
    pub fn make_funaidb_block_proposal<T: FunaiMessageCodec>(
        sortdb: &SortitionDB,
        tip: &BlockSnapshot,
        funaidbs: &FunaiDBs,
        block: &T,
    ...
    ) -> Result<Option<FunaiDBChunkData>, Error> {
        ...
        let mut chunk = FunaiDBChunkData::new(slot_id, slot_version, block_bytes);
        chunk
            .sign(miner_privkey)
            .map_err(|_| net_error::SigningError("Failed to sign FunaiDB chunk".into()))?;
        Ok(Some(chunk))
    }
}
