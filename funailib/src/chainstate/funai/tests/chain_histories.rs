...
/// blocks and microblocks in which either the Funai or burnchain histories fork (or both), and
/// that the Funai chain state can still process all blocks and microblocks on each fork correctly
...
/// Simplest end-to-end test: create 1 fork of N Funai epochs, mined on 1 burn chain fork,
/// all from the same miner.
fn mine_funai_blocks_1_fork_1_miner_1_burnchain<F, G>(
    test_name: &String,
    rounds: usize,
    mut block_builder: F,
    mut check_oracle: G,
...
where
    F: FnMut(
        &mut ClarityTx,
        &mut FunaiBlockBuilder,
        &mut TestMiner,
        usize,
        Option<&FunaiMicroblockHeader>,
    ) -> (FunaiBlock, Vec<FunaiMicroblock>),
    G: FnMut(&FunaiBlock, &Vec<FunaiMicroblock>) -> bool,
{
    ...
    let mut node = TestFunaiNode::new(
        false,
        0x80000000,
        &full_test_name,
    ...
    let mut first_burn_block = TestFunaiNode::next_burn_block(&mut burn_node.sortdb, &mut fork);
    ...
    // next, build up some funai blocks
    for i in 0..rounds {
        let mut burn_block = {
        ...
        let (funai_block, microblocks, block_commit_op) = node.mine_funai_block(
            ...
            |mut builder, ref mut miner, ref sortdb| {
                test_debug!("Produce anchored funai block");
                ...
                let all_prev_mining_rewards = get_all_mining_rewards(
                    ...
                    &builder.chain_tip,
                    builder.chain_tip.funai_block_height,
                );
                ...
                let mut epoch = builder
                    ...
                    .unwrap()
                    .0;
                let (funai_block, microblocks) = block_builder(
                    &mut epoch,
                    &mut builder,
                    miner,
                    i,
        ...
        // "discover" the funai block and its microblocks
        preprocess_funai_block_data(
            &mut node,
            &mut burn_node,
            &fork_snapshot,
            &funai_block,
        ...
        // process all blocks
        test_debug!(
            "Process Funai block {} and {} microblocks",
        ...
        let expect_success = check_oracle(&funai_block, &microblocks);
        if expect_success {
            ...
            assert!(check_block_state_index_root(
                ...
                chain_tip.anchored_header.as_funai_epoch2().unwrap(),
            ));
        }
...
fn mine_funai_blocks_1_fork_2_miners_1_burnchain<F>(
    test_name: &String,
    rounds: usize,
    ...
    ) -> (FunaiBlock, Vec<FunaiMicroblock>),
{
        ...
        let (funai_block, microblocks, block_commit_op) = node.mine_funai_block(
            ...
            |mut builder, ref mut miner, ref sortdb| {
                ...
                let mut epoch = builder
                    ...
                    .unwrap()
                    .0;
                let (funai_block, microblocks) = miner_1_block_builder(
                    &mut epoch,
                    &mut builder,
                    miner,
                    i,
    ...
    for i in rounds / 2..rounds {
        ...
        let parent_block_opt = Some(
            node.get_anchored_block(&last_winning_snapshot.winning_funai_block_hash)
        ...
        let (funai_block_1, microblocks_1, block_commit_op_1) = node.mine_funai_block(
            &mut burn_node.sortdb,
            &mut miner_1,
            &mut burn_block,
            &last_key_1,
            ...
            |mut builder, ref mut miner, ref sortdb| {
                test_debug!(
                    "Produce anchored funai block in funai fork 1 via {}",
        ...
        let (funai_block_2, microblocks_2, block_commit_op_2) = node.mine_funai_block(
            &mut burn_node.sortdb,
            &mut miner_2,
            &mut burn_block,
            &last_key_2,
            ...
            |mut builder, ref mut miner, ref sortdb| {
                test_debug!(
                    "Produce anchored funai block in funai fork 2 via {}",
                    miner.origin_address().unwrap().to_string()
                );
                ...
                let mut epoch = builder
                    ...
                    .unwrap()
                    .0;
                let (funai_block, microblocks) = miner_2_block_builder(
                    &mut epoch,
                    &mut builder,
                    miner,
                    i,
        ...
        // "discover" the funai blocks
        let res_1 = preprocess_funai_block_data(
            &mut node,
            &mut burn_node,
            &fork_snapshot,
            &funai_block_1,
        ...
        let res_2 = preprocess_funai_block_data(
            &mut node,
            &mut burn_node,
            &fork_snapshot,
            &funai_block_2,
        ...
        // exactly one funai block will have been queued up, since sortition picks only one.
        match (res_1, res_2) {
            (Some(res), None) => {}
            (None, Some(res)) => {}
            (_, _) => assert!(false),
        ...
        test_debug!(
            "Process Funai block {}",
            &fork_snapshot.winning_funai_block_hash
        );
        let tip_info_list = node
            .chainstate
            .process_blocks_at_tip(
        ...
        let mut next_miner_trace = TestMinerTracePoint::new();
        if fork_snapshot.winning_funai_block_hash == funai_block_1.block_hash() {
            test_debug!(
...
/// two miners begin working on the same funai chain, and then the funai chain forks
...
fn mine_funai_blocks_2_forks_2_miners_1_burnchain<F>(
    test_name: &String,
    rounds: usize,
...
{
    mine_funai_blocks_2_forks_at_height_2_miners_1_burnchain(
        test_name,
        rounds,
        rounds / 2,
...
fn mine_funai_blocks_2_forks_at_height_2_miners_1_burnchain<F>(
    test_name: &String,
    rounds: usize,
    fork_height: usize,
    mut miner_1_block_builder: F,
    ...
    ) -> (FunaiBlock, Vec<FunaiMicroblock>),
{
    ...
    for i in 0..fork_height {
        ...
        test_debug!(
            "Process Funai block {} and {} microblocks",
            &funai_block_1.block_hash(),
            microblocks_1.len()
        );
        test_debug!(
            "Process Funai block {} and {} microblocks",
            &funai_block_2.block_hash(),
            microblocks_2.len()
        );
        let tip_info_list = node
            .chainstate
    ...
    for i in fork_height..rounds {
        ...
        let mut last_winning_snapshot_1 = {
            let ic = burn_node.sortdb.index_conn();
            let tip = fork.get_tip(&ic);
            match TestFunaiNode::get_last_winning_snapshot(&ic, &tip, &miner_1) {
                Some(sn) => sn,
        ...
        let mut last_winning_snapshot_2 = {
            let ic = burn_node.sortdb.index_conn();
            let tip = fork.get_tip(&ic);
            match TestFunaiNode::get_last_winning_snapshot(&ic, &tip, &miner_2) {
                Some(sn) => sn,
                None => SortitionDB::get_first_block_snapshot(&ic).unwrap(),
            }
        };
        ...
        let parent_block_opt_1 =
            node.get_anchored_block(&last_winning_snapshot_1.winning_funai_block_hash);
        let parent_block_opt_2 =
            node_2.get_anchored_block(&last_winning_snapshot_2.winning_funai_block_hash);
        ...
        let (funai_block_1, microblocks_1, block_commit_op_1) = node.mine_funai_block(
            ...
            |mut builder, ref mut miner, ref sortdb| {
                test_debug!(
                    "Miner {}: Produce anchored funai block in funai fork 1 via {}",
                    miner.id,
                    miner.origin_address().unwrap().to_string()
                );
        ...
        let (funai_block_2, microblocks_2, block_commit_op_2) = node_2.mine_funai_block(
            &mut burn_node.sortdb,
            &mut miner_2,
            &mut burn_block,
            &last_key_2,
            ...
            |mut builder, ref mut miner, ref sortdb| {
                test_debug!(
                    "Miner {}: Produce anchored funai block in funai fork 2 via {}",
                    miner.id,
                    miner.origin_address().unwrap().to_string()
                );
...
fn mine_funai_blocks_1_fork_2_miners_2_burnchains<F>(
    test_name: &String,
    rounds: usize,
    mut miner_1_block_builder: F,
    mut miner_2_block_builder: F,
    ...
    ) -> (FunaiBlock, Vec<FunaiMicroblock>),
{
    ...
    let mut first_burn_block = TestFunaiNode::next_burn_block(&mut burn_node.sortdb, &mut fork_1);
    ...
    // next, build up some funai blocks, cooperatively
    for i in 0..rounds / 2 {
        let mut burn_block = {
            let ic = burn_node.sortdb.index_conn();
        ...
        let (funai_block_1, microblocks_1, block_commit_op_1) = node.mine_funai_block(
            ...
            1000,
            |mut builder, ref mut miner, ref sortdb| {
                test_debug!("Produce anchored funai block from miner 1");
        ...
        let (funai_block_2, microblocks_2, block_commit_op_2) = node.mine_funai_block(
            ...
            1000,
            |mut builder, ref mut miner, ref sortdb| {
                test_debug!("Produce anchored funai block from miner 2");
    ...
    // next, build up some funai blocks on two separate burnchain forks.
    ...
    for i in rounds / 2..rounds {
        ...
        let parent_block_opt_1 =
            node.get_anchored_block(&block_1_snapshot.winning_funai_block_hash);
        let parent_block_opt_2 =
            node.get_anchored_block(&block_2_snapshot.winning_funai_block_hash);
        ...
        // "discover" the funai block
        test_debug!("preprocess fork 1 {}", funai_block_1.block_hash());
        preprocess_funai_block_data(
            &mut node,
            &mut burn_node,
            &fork_snapshot_1,
        ...
        test_debug!("preprocess fork 2 {}", funai_block_1.block_hash());
        preprocess_funai_block_data(
            &mut node,
            &mut burn_node,
            &fork_snapshot_2,
        ...
        // processed all funai blocks -- one on each burn chain fork
...
fn mine_funai_blocks_2_forks_2_miners_2_burnchains<F>(
    test_name: &String,
    rounds: usize,
    mut miner_1_block_builder: F,
    mut miner_2_block_builder: F,
    ...
    ) -> (FunaiBlock, Vec<FunaiMicroblock>),
{
    ...
    // next, build up some funai blocks. miners cooperate
    for i in 0..rounds / 2 {
        let mut burn_block = {
            let ic = burn_node.sortdb.index_conn();
            fork_1.next_block(&ic)
        ...
        let (block_1_snapshot_opt, block_2_snapshot_opt) = {
            ...
            let block_1_snapshot_opt =
                TestFunaiNode::get_last_winning_snapshot(&ic, &chain_tip, &miner_1);
            let block_2_snapshot_opt =
                TestFunaiNode::get_last_winning_snapshot(&ic, &chain_tip, &miner_2);
            (block_1_snapshot_opt, block_2_snapshot_opt)
        };

        let parent_block_opt_1 = match block_1_snapshot_opt {
            Some(sn) => node.get_anchored_block(&sn.winning_funai_block_hash),
            None => None,
        };
...
fn assert_chainstate_blocks_eq(test_name_1: &str, test_name_2: &str) {
    ...
    let mut all_blocks_1 = FunaiChainState::list_blocks(&ch1.db()).unwrap();
    let mut all_blocks_2 = FunaiChainState::list_blocks(&ch2.db()).unwrap();
    ...
    let mut all_microblocks_1 =
        FunaiChainState::list_microblocks(&ch1.db(), &ch1.blocks_path).unwrap();
    let mut all_microblocks_2 =
        FunaiChainState::list_microblocks(&ch2.db(), &ch2.blocks_path).unwrap();
    ...
    for i in 0..all_blocks_1.len() {
        let staging_1_opt = FunaiChainState::load_staging_block(
            &ch1.db(),
            &ch2.blocks_path,
            &all_blocks_1[i].0,
            &all_blocks_1[i].1,
        ...
        .unwrap();
        let staging_2_opt = FunaiChainState::load_staging_block(
            &ch2.db(),
            &ch2.blocks_path,
            &all_blocks_2[i].0,
            &all_blocks_2[i].1,
        ...
        let chunk_1_opt =
            FunaiChainState::load_block(&ch1.blocks_path, &all_blocks_1[i].0, &all_blocks_1[i].1)
                .unwrap();
        let chunk_2_opt =
            FunaiChainState::load_block(&ch2.blocks_path, &all_blocks_2[i].0, &all_blocks_2[i].1)
                .unwrap();
    ...
    for i in 0..all_microblocks_1.len() {
        ...
        let chunk_1_opt = FunaiChainState::load_descendant_staging_microblock_stream(
            &ch1.db(),
        ...
        .unwrap();
        let chunk_2_opt = FunaiChainState::load_descendant_staging_microblock_stream(
            &ch1.db(),
            &FunaiBlockHeader::make_index_block_hash(
        ...
        for j in 0..all_microblocks_1[i].2.len() {
            ...
            let staging_1_opt = FunaiChainState::load_staging_microblock(
                &ch1.db(),
                &all_microblocks_1[i].0,
                &all_microblocks_1[i].1,
            ...
            .unwrap();
            let staging_2_opt = FunaiChainState::load_staging_microblock(
                &ch2.db(),
                &all_microblocks_2[i].0,
                &all_microblocks_2[i].1,
...
/// produce all funai blocks, but don't process them in order.  Instead, queue them all up and
/// process them in randomized order.
/// This works by running mine_funai_blocks_1_fork_1_miner_1_burnchain, extracting the blocks,
...
fn miner_trace_replay_randomized(miner_trace: &mut TestMinerTrace) {
    ...
    for (i, test_name) in test_names.iter().enumerate() {
        ...
        let next_node = TestFunaiNode::new(
            false,
            0x80000000,
            &rnd_test_name,
            miner_trace
    ...
    for point in miner_trace.points.drain(..) {
        ...
        for miner_id in miner_ids {
            ...
            let funai_block_opt = point.get_funai_block(miner_id);
            ...
            match funai_block_opt {
                Some(funai_block) => {
                    let mut microblocks = microblocks_opt.unwrap_or(vec![]);

                    // "discover" the funai block and its microblocks in all nodes
                    ...
                    for (node_name, mut node) in nodes.iter_mut() {
                        ...
                        if microblocks.len() > 0 {
                            for mblock in microblocks.iter() {
                                ...
                                test_debug!(
                                    "Process Funai block {} and microblock {} {}",
                                    &funai_block.block_hash(),
                                    mblock.block_hash(),
                            ...
                            // process all the blocks we can
                            test_debug!(
                                "Process Funai block {} and {} microblocks in {}",
                                &funai_block.block_hash(),
                                microblocks.len(),
                                &node_name
                            );
                ...
                None => {
                    ...
                    test_debug!(
                        "Miner {} did not produce a Funai block for {:?} (commit {:?})",
                        miner_id,
                        &fork_snapshot,
                        &block_commit_op
                    );
...
pub fn mine_empty_anchored_block(
    ...
    builder: &mut FunaiBlockBuilder,
    miner: &mut TestMiner,
    burnchain_height: usize,
    parent_microblock_header: Option<&FunaiMicroblockHeader>,
) -> (FunaiBlock, Vec<FunaiMicroblock>) {
    let miner_account = FunaiChainState::get_account(
        clarity_tx,
    ...
    let funai_block = builder.mine_anchored_block(clarity_tx);

    test_debug!(
        "Produce anchored funai block at burnchain height {} funai height {}",
        burnchain_height,
...
pub fn mine_empty_anchored_block_with_burn_height_pubkh(
    ...
    test_debug!(
        "Produce anchored funai block at burnchain height {} funai height {} pubkeyhash {}",
        burnchain_height,
        funai_block.header.total_work.work,
        &funai_block.header.microblock_pubkey_hash
    );
    (funai_block, vec![])
}

pub fn mine_empty_anchored_block_with_funai_height_pubkh(
    clarity_tx: &mut ClarityTx,
    builder: &mut FunaiBlockBuilder,
    miner: &mut TestMiner,
    burnchain_height: usize,
...
pub fn mine_invalid_token_transfers_block(
    ...
    let recipient = FunaiAddress::new(C32_ADDRESS_VERSION_TESTNET_SINGLESIG, Hash160([0xff; 20]));
    ...
    test_debug!("Produce anchored funai block {} with invalid token transfers at burnchain height {} funai height {}", funai_block.block_hash(), burnchain_height, funai_block.header.total_work.work);
...
pub fn mine_smart_contract_contract_call_block(
    ...
    test_debug!("Produce anchored funai block {} with smart contract and contract call at burnchain height {} funai height {}", funai_block.block_hash(), burnchain_height, funai_block.header.total_work.work);
    (funai_block, vec![])
}
...
pub fn mine_smart_contract_block_contract_call_microblock(
    ...
    if burnchain_height > 0 && builder.chain_tip.anchored_header.height() > 0 {
        ...
        for i in (0..burnchain_height).rev() {
            ...
            let contract = FunaiChainState::get_contract(clarity_tx, &prev_contract_id).unwrap();
            if contract.is_none() {
                continue;
            }

            let prev_bar_value =
                FunaiChainState::get_data_var(clarity_tx, &prev_contract_id, "bar").unwrap();
    ...
    test_debug!("Produce anchored funai block {} with smart contract and {} microblocks with contract call at burnchain height {} funai height {}",
                funai_block.block_hash(), microblocks.len(), burnchain_height, funai_block.header.total_work.work);
...
fn mine_anchored_empty_blocks_single() {
    mine_funai_blocks_1_fork_1_miner_1_burnchain(
...
fn mine_anchored_empty_blocks_random() {
    let mut miner_trace = mine_funai_blocks_1_fork_1_miner_1_burnchain(
...
fn mine_anchored_empty_blocks_multiple_miners() {
    mine_funai_blocks_1_fork_2_miners_1_burnchain(
...
fn mine_anchored_empty_blocks_multiple_miners_random() {
    let mut miner_trace = mine_funai_blocks_1_fork_2_miners_1_burnchain(
...
#[test]
fn mine_anchored_empty_blocks_funai_fork() {
    mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"empty-anchored-blocks-funai-fork".to_string(),
        10,
        mine_empty_anchored_block,
        mine_empty_anchored_block,
    );
...
#[test]
fn mine_anchored_empty_blocks_funai_fork_random() {
    let mut miner_trace = mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"empty-anchored-blocks-funai-fork-random".to_string(),
        10,
        mine_empty_anchored_block,
        mine_empty_anchored_block,
    );
...
fn mine_anchored_empty_blocks_burnchain_fork() {
    mine_funai_blocks_1_fork_2_miners_2_burnchains(
...
fn mine_anchored_empty_blocks_burnchain_fork_random() {
    let mut miner_trace = mine_funai_blocks_1_fork_2_miners_2_burnchains(
...
#[test]
fn mine_anchored_empty_blocks_burnchain_fork_funai_fork() {
    mine_funai_blocks_2_forks_2_miners_2_burnchains(
        &"empty-anchored-blocks-burnchain-funai-fork".to_string(),
        10,
        mine_empty_anchored_block,
        mine_empty_anchored_block,
    );
...
#[test]
fn mine_anchored_empty_blocks_burnchain_fork_funai_fork_random() {
    let mut miner_trace = mine_funai_blocks_2_forks_2_miners_2_burnchains(
        &"empty-anchored-blocks-burnchain-funai-fork-random".to_string(),
        10,
        mine_empty_anchored_block,
        mine_empty_anchored_block,
    );
...
#[test]
fn mine_anchored_smart_contract_contract_call_blocks_funai_fork() {
    mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"smart-contract-contract-call-anchored-blocks-funai-fork".to_string(),
        10,
        mine_smart_contract_contract_call_block,
        mine_smart_contract_contract_call_block,
    );
...
#[test]
fn mine_anchored_smart_contract_contract_call_blocks_funai_fork_random() {
    let mut miner_trace = mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"smart-contract-contract-call-anchored-blocks-funai-fork-random".to_string(),
        10,
        mine_smart_contract_contract_call_block,
        mine_smart_contract_contract_call_block,
    );
...
#[test]
fn mine_anchored_smart_contract_contract_call_blocks_burnchain_fork_funai_fork() {
    mine_funai_blocks_2_forks_2_miners_2_burnchains(
        &"smart-contract-contract-call-anchored-blocks-burnchain-funai-fork".to_string(),
        10,
        mine_smart_contract_contract_call_block,
        mine_smart_contract_contract_call_block,
    );
...
#[test]
fn mine_anchored_smart_contract_contract_call_blocks_burnchain_fork_funai_fork_random() {
    let mut miner_trace = mine_funai_blocks_2_forks_2_miners_2_burnchains(
        &"smart-contract-contract-call-anchored-blocks-burnchain-funai-fork-random".to_string(),
        10,
        mine_smart_contract_contract_call_block,
        mine_smart_contract_contract_call_block,
    );
...
#[test]
fn mine_anchored_smart_contract_block_contract_call_microblock_funai_fork() {
    mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"smart-contract-block-contract-call-microblock-funai-fork".to_string(),
        10,
        mine_smart_contract_block_contract_call_microblock,
        mine_smart_contract_block_contract_call_microblock,
    );
...
#[test]
#[ignore]
fn mine_anchored_smart_contract_block_contract_call_microblock_funai_fork_random() {
    let mut miner_trace = mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"smart-contract-block-contract-call-microblock-funai-fork-random".to_string(),
        10,
        mine_smart_contract_block_contract_call_microblock,
        mine_smart_contract_block_contract_call_microblock,
    );
...
#[test]
fn mine_anchored_smart_contract_block_contract_call_microblock_burnchain_fork_funai_fork() {
    mine_funai_blocks_2_forks_2_miners_2_burnchains(
        &"smart-contract-block-contract-call-microblock-burnchain-funai-fork".to_string(),
        10,
        mine_smart_contract_block_contract_call_microblock,
        mine_smart_contract_block_contract_call_microblock,
    );
...
#[test]
fn mine_anchored_smart_contract_block_contract_call_microblock_burnchain_fork_funai_fork_random() {
    let mut miner_trace = mine_funai_blocks_2_forks_2_miners_2_burnchains(
        &"smart-contract-block-contract-call-microblock-burnchain-funai-fork-random".to_string(),
        10,
        mine_smart_contract_block_contract_call_microblock,
        mine_smart_contract_block_contract_call_microblock,
    );
...
#[test]
fn mine_anchored_smart_contract_block_contract_call_microblock_exception_funai_fork() {
    mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"smart-contract-block-contract-call-microblock-exception-funai-fork".to_string(),
        10,
        mine_smart_contract_block_contract_call_microblock_exception,
        mine_smart_contract_block_contract_call_microblock_exception,
    );
...
#[test]
#[ignore]
fn mine_anchored_smart_contract_block_contract_call_microblock_exception_funai_fork_random() {
    let mut miner_trace = mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"smart-contract-block-contract-call-microblock-exception-funai-fork-random".to_string(),
        10,
        mine_smart_contract_block_contract_call_microblock_exception,
        mine_smart_contract_block_contract_call_microblock_exception,
    );
...
#[test]
fn mine_anchored_smart_contract_block_contract_call_microblock_exception_burnchain_fork_funai_fork(
) {
    mine_funai_blocks_2_forks_2_miners_2_burnchains(
        &"smart-contract-block-contract-call-microblock-exception-burnchain-funai-fork"
            .to_string(),
        10,
        mine_smart_contract_block_contract_call_microblock_exception,
        mine_smart_contract_block_contract_call_microblock_exception,
...
#[test]
fn mine_anchored_smart_contract_block_contract_call_microblock_exception_burnchain_fork_funai_fork_random(
) {
    let mut miner_trace = mine_funai_blocks_2_forks_2_miners_2_burnchains(
        &"smart-contract-block-contract-call-microblock-exception-burnchain-funai-fork-random"
            .to_string(),
        10,
        mine_smart_contract_block_contract_call_microblock_exception,
        mine_smart_contract_block_contract_call_microblock_exception,
...
#[test]
fn mine_empty_anchored_block_deterministic_pubkeyhash_funai_fork() {
    mine_funai_blocks_2_forks_2_miners_1_burnchain(
        &"mine_empty_anchored_block_deterministic_pubkeyhash_funai_fork".to_string(),
        10,
        mine_empty_anchored_block_with_funai_height_pubkh,
        mine_empty_anchored_block_with_funai_height_pubkh,
    );
}

#[test]
fn mine_empty_anchored_block_deterministic_pubkeyhash_funai_fork_at_genesis() {
    mine_funai_blocks_2_forks_at_height_2_miners_1_burnchain(
        &"mine_empty_anchored_block_deterministic_pubkeyhash_funai_fork_at_genesis".to_string(),
        10,
        0,
        mine_empty_anchored_block_with_funai_height_pubkh,
        mine_empty_anchored_block_with_funai_height_pubkh,
...
fn mine_anchored_invalid_token_transfer_blocks_single() {
    let miner_trace = mine_funai_blocks_1_fork_1_miner_1_burnchain(
    ...
    for point in miner_trace.points.iter() {
        for (height, bc) in point.block_commits.iter() {
            ...
            .unwrap()
            .unwrap();
            assert!(FunaiChainState::is_block_orphaned(
                &chainstate.db(),
                &sn.consensus_hash,
...
// TODO: missing blocks
// TODO: no-sortition
// TODO: burnchain forks, and we mine the same anchored funai block in the beginnings of the two descendent
