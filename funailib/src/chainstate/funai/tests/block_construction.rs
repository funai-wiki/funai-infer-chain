...
use funai_common::util::{get_epoch_time_ms, sleep_ms};
...
fn test_build_anchored_blocks_empty() {
    ...
    let mut last_block: Option<FunaiBlock> = None;
    for tenure_id in 0..num_blocks {
        ...
        if let Some(block) = last_block {
            assert_eq!(tip.winning_funai_block_hash, block.block_hash());
        }
        ...
        last_block = Some(funai_block.clone());
        ...
        let (burn_ops, funai_block, microblocks) = peer.make_tenure(
            |ref mut miner,
             ...
             ref parent_microblock_header_opt| {
                ...
                if tenure_id > 0 {
                    let stx_transfer = make_user_funai_transfer(
                        &privk,
                        sender_nonce,
                        200,
...
fn test_build_anchored_blocks_empty_with_builder_timeout() {
    ...
    for tenure_id in 0..num_blocks {
        ...
        if tenure_id > 0 {
            ...
            assert_eq!(funai_block.txs.len(), 1);
        }
    }
}
...
fn test_build_anchored_blocks_stx_transfers_multi() {
    ...
    for tenure_id in 0..num_blocks {
        ...
        if tenure_id > 0 {
            ...
            assert_eq!(funai_block.txs.len(), 11);
            for i in 1..11 {
                if let TransactionPayload::TokenTransfer(ref addr, ref amount, ref memo) =
                    funai_block.txs[i].payload
                {
                    assert_eq!(*addr, recipient.to_account_principal());
                    assert_eq!(*amount, 1);
                } else {
...
fn test_build_anchored_blocks_connected_by_microblocks_across_epoch() {
    ...
    for tenure_id in 0..num_blocks {
        ...
        let acct = get_funai_account(&mut peer, &addr.to_account_principal());
             ...
             ref parent_microblock_header_opt| {
                ...
                let (parent_mblock_stream, mblock_pubkey_hash) = {
                    if tenure_id > 0 {
                        ...
                        let mblock_tx = make_user_funai_transfer(
                            &privk,
                            acct.nonce,
                            200,
                            &recipient.to_account_principal(),
                        ...
                        let mblock_pubkey_hash = Hash160::from_node_public_key(
                            &FunaiPublicKey::from_private(&parent_microblock_privkey),
                        );
        ...
        peer.next_burnchain_block(burn_ops.clone());
        peer.process_funai_epoch_at_tip_checked(&funai_block, &vec![])
            .unwrap();
    }
...
fn test_build_anchored_blocks_connected_by_microblocks_across_epoch_invalid() {
    ...
    for tenure_id in 0..num_blocks {
        ...
        let (burn_ops, funai_block, microblocks) = peer.make_tenure(
            |ref mut miner,
             ...
             ref parent_microblock_header_opt| {
                ...
                let mut anchored_block = FunaiBlockBuilder::build_anchored_block(
                    chainstate,
                    &sort_ic,
                    &mut mempool,
                    &parent_tip,
        ...
        if tenure_id != 5 {
            ...
            if let Err(e) = peer.process_funai_epoch_at_tip_checked(&funai_block, &vec![]) {
                match e {
                    CoordinatorError::ChainstateError(ChainstateError::InvalidFunaiBlock(_)) => {}
                    x => {
            ...
            assert!(FunaiChainState::block_crosses_epoch_boundary(
                peer.chainstate().db(),
                &last_block_ch,
                &funai_block.header.parent_block
            )
            .unwrap());
...
fn test_build_anchored_blocks_incrementing_nonces() {
    let private_keys: Vec<_> = (0..10).map(|_| FunaiPrivateKey::new()).collect();
    ...
    assert_eq!(funai_block.txs.len(), 251);
    ...
    for (i, tx) in funai_block.txs.iter().enumerate() {
        if i == 0 {
...
fn test_build_anchored_blocks_skip_too_expensive() {
    ...
    .unwrap();
    let privk_extra = FunaiPrivateKey::from_hex(
    ...
    for i in 0..num_blocks {
        let pk = FunaiPrivateKey::new();
        let addr = FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(&pk)],
        )
        .unwrap()
    ...
    .unwrap();
    let addr_extra = FunaiAddress::from_public_keys(
        C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
        &AddressHashMode::SerializeP2PKH,
        1,
        &vec![FunaiPublicKey::from_private(&privk_extra)],
    )
    .unwrap();
    ...
    peer_config.initial_balances = initial_balances;
    peer_config.epochs = Some(vec![FunaiEpoch {
        epoch_id: FunaiEpochId::Epoch20,
        start_height: 0,
    ...
    for tenure_id in 0..num_blocks {
        ...
        if tenure_id > 0 {
            // expensive transaction was not mined, but the two stx-transfers were
            assert_eq!(funai_block.txs.len(), 3);
            for tx in funai_block.txs.iter() {
                match tx.payload {
...
fn test_build_anchored_blocks_multiple_chaintips() {
    ...
    for tenure_id in 0..num_blocks {
            ...
            |ref mut miner,
             ...
             ref parent_microblock_header_opt| {
                ...
                let anchored_block = {
                    ...
                    FunaiBlockBuilder::build_anchored_block(
                        chainstate,
                        &sortdb.index_conn(),
                        mempool_to_use,
                        &parent_tip,
        ...
        if tenure_id < num_blocks - 1 {
            ...
            assert_eq!(funai_block.txs.len(), num_blocks);
        }
    }
}
...
fn test_build_anchored_blocks_invalid() {
    ...
    let mut last_block: Option<FunaiBlock> = None;
    let mut last_valid_block: Option<FunaiBlock> = None;
    let mut last_tip: Option<BlockSnapshot> = None;
    let mut last_parent: Option<FunaiBlock> = None;
    let mut last_parent_tip: Option<FunaiHeaderInfo> = None;
    ...
    let mut bad_block_parent: Option<FunaiBlock> = None;
    let mut bad_block_parent_tip: Option<FunaiHeaderInfo> = None;
    ...
    for tenure_id in 0..num_blocks {
        ...
        let (mut burn_ops, funai_block, microblocks) = peer.make_tenure(|ref mut miner, ref mut sortdb, ref mut chainstate, vrf_proof, ref parent_opt, ref parent_microblock_header_opt| {
            let parent_opt =
                    ...
                    if let Some(tip) = &last_parent_tip {
                        ...
                        match parent_opt {
                            ...
                            Some(ref block) => {
                                ...
                                let snapshot = SortitionDB::get_block_snapshot_for_winning_funai_block(&ic, &tip.sortition_id, &parent_block_hash).unwrap().unwrap();      // succeeds because we don't fork
                                FunaiChainState::get_anchored_block_header_info(chainstate.db(), &snapshot.consensus_hash, &snapshot.winning_funai_block_hash).unwrap().unwrap()
                            }
                        }
                    }
                }
        ...
        peer.process_funai_epoch(&funai_block, &consensus_hash, &microblocks);
    }
}
...
fn test_build_microblock_stream_forks() {
    ...
    for tenure_id in 0..num_blocks {
        ...
        let (burn_ops, funai_block, microblocks) = peer.make_tenure(
            |ref mut miner,
             ...
             ref parent_microblock_header_opt| {
                let parent_tip = match parent_opt {
                    None => FunaiChainState::get_genesis_header_info(chainstate.db())
                        .unwrap(),
                    Some(block) => {
                        let ic = sortdb.index_conn();
                        let snapshot =
                ...
                let parent_header_hash = parent_tip.anchored_header.block_hash();
                let parent_consensus_hash = parent_tip.consensus_hash.clone();
                let parent_index_hash = FunaiBlockHeader::make_index_block_hash(&parent_consensus_hash, &parent_header_hash);
                ...
                let expected_parent_microblock_opt =
                    if tenure_id > 0 {
                        ...
                        let parent_mblock_stream = {
                            let parent_cost = FunaiChainState::get_funai_block_anchored_cost(chainstate.db(), &FunaiBlockHeader::make_index_block_hash(&parent_consensus_hash, &parent_header_hash)).unwrap().unwrap();
                            ...
                            let mut microblock_builder = FunaiMicroblockBuilder::new(parent_header_hash.clone(), parent_consensus_hash.clone(), chainstate, &sort_ic, BlockBuilderSettings::max_value()).unwrap();
                        ...
                        let (_, poison_opt) = match FunaiChainState::load_descendant_staging_microblock_stream_with_poison(
                            &chainstate.db(),
                            &parent_index_hash,
                            0,
                            u16::MAX
                ...
                let mblock_pubkey_hash = Hash160::from_node_public_key(&FunaiPublicKey::from_private(&mblock_privks[tenure_id]));

                let (anchored_block, block_size, block_execution_cost) = FunaiBlockBuilder::build_anchored_block(
                    chainstate,
                    &sortdb.index_conn(),
                    &mut mempool,
                    &parent_tip,
    ...
    for (i, addr) in addrs.iter().enumerate() {
        let account = get_funai_account(&mut peer, &addr.to_account_principal());
...
fn test_build_microblock_stream_forks_with_descendants() {
    ...
    let mut microblock_tail_1: Option<FunaiMicroblockHeader> = None;
    let mut microblock_tail_2: Option<FunaiMicroblockHeader> = None;
    ...
    for tenure_id in 0..num_blocks {
        ...
        let (mut burn_ops, funai_block, microblocks) = peer.make_tenure(
            |ref mut miner,
             ref mut sortdb,
             ref mut chainstate,
             vrf_proof,
             ...
             ref parent_microblock_header_opt| {
                ...
                let (anchored_block, block_size, block_execution_cost) = FunaiBlockBuilder::build_anchored_block(
                    ...
                    &mut mempool,
                    &parent_tip,
                    parent_tip.anchored_header.as_funai_epoch2().unwrap().total_work.burn + 1000,
                    vrf_proof,
                    mblock_pubkey_hash,
                    &coinbase_tx,
                    BlockBuilderSettings::max_value(),
        ...
        for burn_op in burn_ops.iter_mut() {
            if let BlockstackOperationType::LeaderBlockCommit(ref mut op) = burn_op {
                ...
                op.parent_block_ptr = (*parent_block_ptrs
                    .borrow()
                    .get(&funai_block.header.parent_block)
                    .unwrap()) as u32;
            }
        }
        ...
        if tenure_id >= 1 {
            let next_tip = FunaiChainState::get_anchored_block_header_info(
                peer.chainstate().db(),
                &consensus_hash,
            ...
            let new_tip_hash = FunaiBlockHeader::make_index_block_hash(
        ...
        peer.with_db_state(|ref mut sortdb, ref mut chainstate, _, _| {
            chainstate
                .with_read_only_clarity_tx(&sortdb.index_conn(), &chain_tip, |clarity_tx| {
                    ...
                    let report = FunaiChainState::get_poison_microblock_report(clarity_tx, 1)
                        .unwrap()
                        .unwrap();
...
fn test_contract_call_across_clarity_versions() {
    ...
    .unwrap();
    let privk_anchored = FunaiPrivateKey::from_hex(
        "f67c7437f948ca1834602b28595c12ac744f287a4efaf70d437042a6afed81bc01",
    )
    .unwrap();
    ...
    let addr_anchored = FunaiAddress::from_public_keys(
        C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
        &AddressHashMode::SerializeP2PKH,
        1,
        &vec![FunaiPublicKey::from_private(&privk_anchored)],
    )
    .unwrap();
    ...
    let funai_block_id = FunaiBlockHeader::make_index_block_hash(&consensus_hash, &block_bhh);
...
fn test_is_tx_problematic() {
    ...
    for tenure_id in 0..num_blocks {
             ...
             ref parent_microblock_header_opt| {
                ...
                if tenure_id == 2 {
                    ...
                    let block_builder = FunaiBlockBuilder::make_regtest_block_builder(
                        ...
                        vrf_proof.clone(),
                        tip.total_burn,
                        Hash160::from_node_public_key(&FunaiPublicKey::from_private(&miner.next_microblock_privkey()))
                    )
                    .unwrap();

                    if let Err(ChainstateError::ProblematicTransaction(txid)) = FunaiBlockBuilder::make_anchored_block_from_txs(
                        block_builder,
                        chainstate,
                        &sortdb.index_conn(),
                ...
                if tenure_id == 4 {
                    ...
                    let err = FunaiBlockBuilder::make_anchored_block_from_txs(
                        block_builder,
                        chainstate,
                        &sortdb.index_conn(),
        ...
        last_block = Some(FunaiBlockHeader::make_index_block_hash(
            &consensus_hash,
            &funai_block.block_hash(),
        ));
    }
...
fn mempool_incorporate_pox_unlocks() {
    ...
    let first_pox_cycle = pox_constants
        .block_height_to_reward_cycle(first_block_height, first_funai_block_height)
        .unwrap();
    ...
    let num_blocks = 3 + lockup_end - first_funai_block_height;
    info!(
        "Starting test";
        "num_blocks" => num_blocks,
        "first_funai_block_height" => first_funai_block_height,
        ...
        let (burn_ops, funai_block, microblocks) = peer.make_tenure(
            |ref mut miner,
             ...
             ref parent_microblock_header_opt| {
                 ...
                 let my_height = first_funai_block_height + 1 + tenure_id;
                 ...
                 if tenure_id == 1 {
                     ...
                     let stx_transfer = make_user_funai_transfer(
                         ...
                         1,
                         10_000,
                         &FunaiAddress::burn_address(false).into(),
    ...
    let (burn_ops, funai_block, microblocks) = peer.make_tenure(
        |ref mut miner,
         ...
         ref parent_microblock_header_opt| {
            ...
            let stx_transfer0 =
                make_user_funai_transfer(&privk, 0, 200, &recipient.to_account_principal(), 1);
            let stx_transfer1 =
                make_user_funai_transfer(&privk, 1, 400, &recipient.to_account_principal(), 1);
...
fn paramaterized_mempool_walk_test(
    ...
    let key_address_pairs: Vec<(Secp256k1PrivateKey, FunaiAddress)> = (0..num_users)
        .map(|_user_index| {
            let privk = FunaiPrivateKey::new();
            let addr = FunaiAddress::from_public_keys(
                C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
    ...
    for round_index in 0..num_rounds {
        for user_index in 0..num_users {
            ...
            let mut tx = make_user_funai_transfer(
    ...
    chainstate.with_read_only_clarity_tx(
        &TEST_BURN_STATE_DB,
        &FunaiBlockHeader::make_index_block_hash(&b_2.0, &b_2.1),
        |clarity_conn| {
            let mut count_txs = 0;
                        ...
                        |_, available_tx, _| {
                            ...
                            Ok(Some(
                                ...
                                TransactionResult::success(
                                    ...
                                    FunaiTransactionReceipt::from_stx_transfer(
