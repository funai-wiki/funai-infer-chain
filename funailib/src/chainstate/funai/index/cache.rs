...
use crate::chainstate::funai::index::{trie_sql, ClarityMarfTrieId, Error, MarfTrieId, TrieLeaf};
...
pub struct TrieCacheState<T: MarfTrieId> {
    ...
    /// never evicted, since the size of this map grows only at the rate of new Funai blocks.
...
pub mod test {
    ...
    fn test_marf_with_cache(
        ...
        let test_file = if test_name == ":memory:" {
            ...
            let test_dir = format!("/tmp/funai-marf-tests/{}", test_name);
