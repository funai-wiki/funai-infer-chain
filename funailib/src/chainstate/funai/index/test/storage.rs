...
fn load_store_trie_m_n_same(m: u64, n: u64, same: bool) {
    ...
    let confirmed_marf_storage =
        TrieFileStorage::<FunaiBlockId>::open(&test_name, marf_opts).unwrap();
    let mut confirmed_marf = MARF::<FunaiBlockId>::from_storage(confirmed_marf_storage);

    confirmed_marf
        .begin(&FunaiBlockId::sentinel(), &FunaiBlockId([0x02; 32]))
        .unwrap();
    ...
    let confirmed_tip = FunaiBlockId([0x01; 32]);
    confirmed_marf.commit_to(&confirmed_tip).unwrap();

    let marf_opts = MARFOpenOpts::default();
    let marf_storage =
        TrieFileStorage::<FunaiBlockId>::open_unconfirmed(&test_name, marf_opts).unwrap();
