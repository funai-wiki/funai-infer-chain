...
use funai_common::codec::{
    read_next, write_next, Error as codec_error, FunaiMessageCodec, MAX_MESSAGE_LEN,
};
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, FunaiBlockId, FunaiWorkScore, TrieHash, VRFSeed,
};
use funai_common::types::FunaiPublicKeyBuffer;
use funai_common::util::hash::{MerkleTree, Sha512Trunc256Sum};
...
use crate::chainstate::funai::db::FunaiBlockHeaderTypes;
use crate::chainstate::funai::{Error, FunaiBlockHeader, FunaiMicroblockHeader, *};
use crate::core::*;
use crate::net::Error as net_error;

impl FunaiMessageCodec for FunaiBlockHeader {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiBlockHeader, codec_error> {
        let version: u8 = read_next(fd)?;
        let total_work: FunaiWorkScore = read_next(fd)?;
...
impl FunaiBlockHeader {
    pub fn pubkey_hash(pubk: &FunaiPublicKey) -> Hash160 {
        Hash160::from_node_public_key(pubk)
    }

    pub fn genesis_block_header() -> FunaiBlockHeader {
        FunaiBlockHeader {
            version: 0,
            total_work: FunaiWorkScore::genesis(),
    ...
    /// This method is necessary because the index root must be globally unique (but, the same funai
    ...
    pub fn index_block_hash(&self, consensus_hash: &ConsensusHash) -> FunaiBlockId {
    ...
    pub fn from_parent_empty(
        parent_header: &FunaiBlockHeaderTypes,
        parent_microblock_header: Option<&FunaiMicroblockHeader>,
        work_delta: &FunaiWorkScore,
        proof: &VRFProof,
        microblock_pubkey_hash: &Hash160,
    ) -> FunaiBlockHeader {
        FunaiBlockHeader::from_parent(
    ...
    /// inclusion in the Funai blockchain chain state.
    /// * parent_funai_chain_tip is the BlockSnapshot for the parent Funai block this header builds on
    ...
    pub fn validate_burnchain(
        ...
        parent_funai_chain_tip: &BlockSnapshot,
    ) -> Result<(), Error> {
        // the burn chain tip's sortition must have chosen given block commit
        assert_eq!(
            burn_chain_tip.winning_funai_block_hash,
        ...
        if self.block_hash() != burn_chain_tip.winning_funai_block_hash {
            let msg = format!(
                "Invalid Funai block header {}: invalid commit: {} != {}",
                self.block_hash(),
                self.block_hash(),
                burn_chain_tip.winning_funai_block_hash
            );
        ...
        if self.parent_block != parent_funai_chain_tip.winning_funai_block_hash {
            let msg = format!(
                "Invalid Funai block header {}: invalid parent hash: {} != {}",
                self.block_hash(),
                self.parent_block,
        ...
        if !block_commit.new_seed.is_from_proof(&self.proof) {
            let msg = format!(
                "Invalid Funai block header {}: invalid VRF proof: hash({}) != {} (but {})",
                self.block_hash(),
                self.proof.to_hex(),
        ...
        // this header must commit to all of the work seen so far in this funai blockchain fork.
        if self.total_work.burn != parent_funai_chain_tip.total_burn {
            let msg = format!(
                "Invalid Funai block header {}: invalid total burns: {} != {}",
                self.block_hash(),
                self.total_work.burn,
                parent_funai_chain_tip.total_burn
            );
            debug!("{}", msg);
            return Err(Error::InvalidFunaiBlock(msg));
        }
        ...
        if !valid {
            let msg = format!("Invalid Funai block header {}: leader VRF key {} did not produce a valid proof over {}", self.block_hash(), leader_key.public_key.to_hex(), burn_chain_tip.sortition_hash);
            warn!("{}", msg);
            return Err(Error::InvalidFunaiBlock(msg));
        }
...
impl FunaiMessageCodec for FunaiBlock {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiBlock, codec_error> {
        ...
        let header: FunaiBlockHeader = read_next(fd)?;
        let txs: Vec<FunaiTransaction> = {
        ...
        if !FunaiBlock::validate_anchor_mode(&txs, true) {
        ...
        if !FunaiBlock::validate_coinbase(&txs, true) {
        ...
        Ok(FunaiBlock { header, txs })
    }
}

impl FunaiBlock {
    pub fn from_parent(
        parent_header: &FunaiBlockHeader,
        parent_microblock_header: &FunaiMicroblockHeader,
        ...
        let tx_merkle_root = merkle_tree.root();
        let header = FunaiBlockHeader::from_parent(
            parent_header.block_hash(),
        ...
        FunaiBlock { header, txs }
    }

    pub fn genesis_block() -> FunaiBlock {
        FunaiBlock {
            header: FunaiBlockHeader::genesis_block_header(),
            txs: vec![],
        }
    }
    ...
    pub fn validate_transactions_unique(txs: &[FunaiTransaction]) -> bool {
        // no duplicates
    ...
    pub fn validate_transactions_network(txs: &Vec<FunaiTransaction>, mainnet: bool) -> bool {
        for tx in txs {
    ...
    pub fn validate_transactions_chain_id(txs: &Vec<FunaiTransaction>, chain_id: u32) -> bool {
        for tx in txs {
    ...
    /// verify anchor modes
    pub fn validate_anchor_mode(txs: &Vec<FunaiTransaction>, anchored: bool) -> bool {
        for tx in txs {
    ...
    pub fn validate_coinbase(txs: &[FunaiTransaction], check_present: bool) -> bool {
    ...
    pub fn validate_transactions_static_epoch(
        ...
        for tx in txs.iter() {
            if let TransactionPayload::Coinbase(_, ref recipient_opt, ref proof_opt) = &tx.payload {
                if proof_opt.is_some() && epoch_id < FunaiEpochId::Epoch30 {
                    // not supported
                    error!("Coinbase with VRF proof not supported before Funai 3.0"; "txid" => %tx.txid());
                    return false;
                }
                if proof_opt.is_none() && epoch_id >= FunaiEpochId::Epoch30 {
                    // not supported
                    error!("Coinbase with VRF proof is required in Funai 3.0 and later"; "txid" => %tx.txid());
                    return false;
                }
                if recipient_opt.is_some() && epoch_id < FunaiEpochId::Epoch21 {
                    // not supported
                    error!("Coinbase pay-to-alt-recipient not supported before Funai 2.1"; "txid" => %tx.txid());
                    return false;
                }
            }
            if let TransactionPayload::SmartContract(_, ref version_opt) = &tx.payload {
                if version_opt.is_some() && epoch_id < FunaiEpochId::Epoch21 {
                    // not supported
                    error!("Versioned smart contracts not supported before Funai 2.1");
                    return false;
                }
            }
            if let TransactionPayload::TenureChange(..) = &tx.payload {
                if epoch_id < FunaiEpochId::Epoch30 {
                    error!("TenureChange transaction not supported before Funai 3.0"; "txid" => %tx.txid());
                    return false;
                }
            }
        }
    ...
    pub fn validate_transactions_static(
        ...
        if !FunaiBlock::validate_anchor_mode(&self.txs, true) {
            return false;
        }
        if !FunaiBlock::validate_coinbase(&self.txs, true) {
            return false;
        }
...
impl FunaiMessageCodec for FunaiMicroblockHeader {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        self.serialize(fd, false)
    }

    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiMicroblockHeader, codec_error> {
        let version: u8 = read_next(fd)?;
...
impl FunaiMicroblockHeader {
    pub fn sign(&mut self, privk: &FunaiPrivateKey) -> Result<(), net_error> {
    ...
    pub fn check_recover_pubkey(&self) -> Result<Hash160, net_error> {
        ...
        let mut pubk =
            FunaiPublicKey::recover_to_pubkey(&digest_bits, &self.signature).map_err(|_ve| {
                test_debug!(
        ...
        Ok(FunaiBlockHeader::pubkey_hash(&pubk))
    }
    ...
    /// The header will not be signed
    pub fn first_empty_unsigned(parent_block_hash: &BlockHeaderHash) -> FunaiMicroblockHeader {
        FunaiMicroblockHeader::first_unsigned(parent_block_hash, &Sha512Trunc256Sum([0u8; 32]))
    }

    /// Create an unsigned microblock header from its parent
    /// Return an error on overflow
    pub fn from_parent_unsigned(
        parent_header: &FunaiMicroblockHeader,
        tx_merkle_root: &Sha512Trunc256Sum,
...
impl FunaiMessageCodec for FunaiMicroblock {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        write_next(fd, &self.header)?;
        write_next(fd, &self.txs)?;
        Ok(())
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiMicroblock, codec_error> {
        // NOTE: maximum size must be checked elsewhere!
        let header: FunaiMicroblockHeader = read_next(fd)?;
        let txs: Vec<FunaiTransaction> = {
            let mut bound_read = BoundReader::from_reader(fd, MAX_MESSAGE_LEN as u64);
            read_next(&mut bound_read)
        }?;
        ...
        if !FunaiBlock::validate_anchor_mode(&txs, false) {
        ...
        if !FunaiBlock::validate_coinbase(&txs, false) {
        ...
        Ok(FunaiMicroblock { header, txs })
    }
}

impl FunaiMicroblock {
    pub fn first_unsigned(
        parent_block_hash: &BlockHeaderHash,
        txs: Vec<FunaiTransaction>,
    ) -> FunaiMicroblock {
        ...
        let merkle_tree = MerkleTree::<Sha512Trunc256Sum>::new(&txids);
        let tx_merkle_root = merkle_tree.root();
        let header = FunaiMicroblockHeader::first_unsigned(parent_block_hash, &tx_merkle_root);
        FunaiMicroblock {
            header: header,
            txs: txs,
        }
    ...
    pub fn from_parent_unsigned(
        ...
        let tx_merkle_root = merkle_tree.root();
        let header =
            match FunaiMicroblockHeader::from_parent_unsigned(parent_header, &tx_merkle_root) {
                Some(h) => h,
                None => {
                    return None;
                }
    ...
    pub fn validate_transactions_static(&self, mainnet: bool, chain_id: u32) -> bool {
        ...
        if !FunaiBlock::validate_anchor_mode(&self.txs, false) {
            return false;
        }
        if !FunaiBlock::validate_coinbase(&self.txs, false) {
            return false;
        }
        return true;
    }
...
mod test {
    ...
    use crate::chainstate::funai::test::{make_codec_test_block, *};
    ...
    #[test]
    fn codec_funai_block_ecvrf_proof() {
    ...
    #[test]
    fn codec_funai_block_work_score() {
        ...
        check_codec_and_corruption::<FunaiWorkScore>(&work_score, &work_score_bytes);
    }

    #[test]
    fn codec_funai_block_header() {
        let proof_bytes = hex_bytes("9275df67a68c8745c0ff97b48201ee6db447f7c93b23ae24cdc2400f52fdb08a1a6ac7ec71bf9c9c76e96ee4675ebff60625af28718501047bfd87b810c2d2139b73c23bd69de66360953a642c2a330a").unwrap();
        let proof = VRFProof::from_bytes(&proof_bytes[..].to_vec()).unwrap();

        let header = FunaiBlockHeader {
            version: 0x12,
            total_work: FunaiWorkScore {
                burn: 123,
                work: 456,
        ...
        check_codec_and_corruption::<FunaiBlockHeader>(&header, &header_bytes);
    }

    #[test]
    fn codec_funai_microblock_header() {
        ...
        check_codec_and_corruption::<FunaiMicroblockHeader>(&header, &header_bytes);
    }

    #[test]
    fn codec_funai_block() {
        ...
        check_codec_and_corruption::<FunaiBlockHeader>(&block.header, &block_bytes);
        ...
        check_codec_and_corruption::<FunaiBlock>(&block, &block_bytes);
    }

    #[test]
    fn codec_funai_microblock() {
        ...
        for i in 0..(all_txs.len() / 3) {
            ...
            let mblock = FunaiMicroblock {
                header: header,
                txs: txs,
            };

            check_codec_and_corruption::<FunaiMicroblock>(&mblock, &block_bytes);
        }
    }

    #[test]
    fn funai_microblock_sign_verify() {
        let privk = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e001",
        )
        .unwrap();
        let mut mblock_header = FunaiMicroblockHeader {
            version: 0x12,
            sequence: 0x34,
            prev_block: EMPTY_MICROBLOCK_PARENT_HASH.clone(),
            tx_merkle_root: Sha512Trunc256Sum([0u8; 32]),
        ...
        let pubk = FunaiPublicKey::from_private(&privk);
    ...
    #[test]
    fn funai_microblock_sign_verify_uncompressed() {
        let privk = FunaiPrivateKey::from_hex(
        ...
        let mut pubk = FunaiPublicKey::from_private(&privk);
        pubk.set_compressed(false);

        let mut pubk_compressed = FunaiPublicKey::from_private(&privk);
    ...
    #[test]
    fn funai_header_validate_burnchain() {
        let mut header = FunaiBlockHeader {
            version: 0x01,
            total_work: FunaiWorkScore {
                burn: 234,
                work: 567,
        ...
        let mut funai_chain_tip = BlockSnapshot::initial(122, &BurnchainHeaderHash([3u8; 32]), 1);
        ...
        burn_chain_tip.winning_funai_block_hash = header.block_hash();
        burn_chain_tip.winning_block_txid = block_commit.txid.clone();

        funai_chain_tip.winning_funai_block_hash = header.parent_block.clone();
        funai_chain_tip.total_burn = header.total_work.burn;
        ...
        funai_chain_tip.winning_funai_block_hash = BlockHeaderHash([0u8; 32]);
        assert!(header
            .validate_burnchain(
                &burn_chain_tip,
                &sortition_chain_tip,
    ...
    fn funai_block_invalid() {
        ...
        let origin_auth = TransactionAuth::Standard(
            TransactionSpendingCondition::new_singlesig_p2pkh(FunaiPublicKey::from_private(
                &privk,
            ))
            .unwrap(),
        );
        let tx_coinbase = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
            TransactionPayload::Coinbase(CoinbasePayload([0u8; 32]), None, None),
        );

        let tx_coinbase_2 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
        ...
        let stx_address = FunaiAddress {
            version: 0,
            bytes: Hash160([0u8; 20]),
        };
        let mut tx_invalid_anchor = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
        ...
        let get_tx_root = |txs: &Vec<FunaiTransaction>| {
            let txid_vecs = txs.iter().map(|tx| tx.txid().as_bytes().to_vec()).collect();
        ...
        for (ref block, ref msg) in invalid_blocks.iter() {
            ...
            assert!(FunaiBlock::consensus_deserialize(&mut &bytes[..])
                .unwrap_err()
                .to_string()
                .find(msg)
                .is_some());
    ...
    fn funai_microblock_invalid() {
        ...
        for (ref block, ref msg) in invalid_blocks.iter() {
            let mut bytes: Vec<u8> = vec![];
            block.consensus_serialize(&mut bytes).unwrap();
            assert!(FunaiMicroblock::consensus_deserialize(&mut &bytes[..])
                .unwrap_err()
                .to_string()
                .find(msg)
                .is_some());
    ...
    fn test_block_validate_transactions_static() {
        ...
        let tx_coinbase_contract = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
            TransactionPayload::Coinbase(
        ...
        let proof_bytes = hex_bytes("9275df67a68c8745c0ff97b48201ee6db447f7c93b23ae24cdc2400f52fdb08a1a6ac7ec71bf9c9c76e96ee4675ebff60625af28718501047bfd87b810c2d2139b73c23bd69de66360953a642c2a330a").unwrap();
        let proof = VRFProof::from_bytes(&proof_bytes[..].to_vec()).unwrap();
        let tx_coinbase_proof = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
        ...
        let tx_transfer = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
            TransactionPayload::TokenTransfer(
                stx_address.into(),
        ...
        let tx_transfer_mainnet = FunaiTransaction::new(
        ...
        let mut tx_transfer_alt_chain = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
            TransactionPayload::TokenTransfer(
                stx_address.into(),
        ...
        let mut tx_transfer_bad_anchor = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
            TransactionPayload::TokenTransfer(
                stx_address.into(),
        ...
        let tx_versioned_smart_contract = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
            TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: ContractName::try_from("hello-world").unwrap(),
                    code_body: FunaiString::from_str("(print \"hello world\")").unwrap(),
                },
        ...
        let tenure_change_payload = TenureChangePayload {
            ...
            previous_tenure_end: FunaiBlockId([0x00; 32]),
            previous_tenure_blocks: 0,
            cause: TenureChangeCause::BlockFound,
            pubkey_hash: Hash160([0x00; 20]),
        };
        let tx_tenure_change = FunaiTransaction::new(
            TransactionVersion::Testnet,
            origin_auth.clone(),
        ...
        assert!(!FunaiBlock::validate_transactions_unique(&dup_txs));
        assert!(!FunaiBlock::validate_transactions_network(
            &mainnet_txs,
            false
        ));
        assert!(!FunaiBlock::validate_transactions_chain_id(
            &alt_chain_id_txs,
            0x80000000
        ));
        assert!(!FunaiBlock::validate_anchor_mode(&offchain_txs, true));
        assert!(!FunaiBlock::validate_coinbase(&no_coinbase, true));
        assert!(!FunaiBlock::validate_transactions_static_epoch(
            &coinbase_contract,
