...
(define-map stacking-rejectors
    { funai: principal, reward-cycle: uint }
    { amount: uint }
)

;; Getter for stacking-rejectors
(define-read-only (get-pox-rejection (funai principal) (reward-cycle uint))
    (map-get? stacking-rejectors { funai: funai, reward-cycle: reward-cycle }))
...
(define-private (append-reward-cycle-pox-addr (pox-addr (tuple (version (buff 1)) (hashbytes (buff 32))))
    ...
    (let ((sz (get-reward-set-size reward-cycle)))
        (map-set reward-cycle-pox-address-list
            ...
            { pox-addr: pox-addr, total-ustx: amount-ustx, funai: funai })
...
(define-private (fold-unlock-reward-cycle (set-index uint)
    ...
    (let ((data (try! data-res))
         ...
         (let ((cycle-entry (unwrap-panic (map-get? reward-cycle-pox-address-list { reward-cycle: cycle, index: set-index })))
               (cycle-entry-u (get funai cycle-entry))
            ...
            (asserts! (is-eq cycle-entry-u (some (get funai data))) (err ERR_STACKING_CORRUPTED_STATE))
            (if (not (is-eq cycle-last-entry-ix set-index))
                ...
                (let ((move-entry (unwrap-panic (map-get? reward-cycle-pox-address-list { reward-cycle: cycle, index: cycle-last-entry-ix }))))
                    (map-set reward-cycle-pox-address-list
                             ...
                             move-entry)
                    (match (get funai move-entry) moved-funai
                     ;; if the moved entry had an associated funai, update its state
                     (let ((moved-state (unwrap-panic (map-get? stacking-state { funai: moved-funai })))
                          ...
                          (map-set stacking-state { funai: moved-funai }
...
;; This method is called by the Funai block processor directly in order to handle the contract state mutations
...
(define-private (handle-unlock (user principal) (amount-locked uint) (cycle-to-unlock uint))
    (let ((user-stacking-state (unwrap-panic (map-get? stacking-state { funai: user })))
        ...
        (try! (fold fold-unlock-reward-cycle reward-set-indexes (ok { cycle: first-cycle-locked, first-unlocked-cycle: cycle-to-unlock, funai: user })))
        ;; Now that we've cleaned up all the reward set entries for the user, delete the user's stacking-state
        (map-delete stacking-state { funai: user })
        (ok true)))
  ...
  (let ((cycle-indexes (list u0 u1 u2 u3 u4 u5 u6 u7 u8 u9 u10 u11))
        (results (fold add-pox-addr-to-ith-reward-cycle cycle-indexes
                         { pox-addr: pox-addr, first-reward-cycle: first-reward-cycle, num-cycles: num-cycles,
                           reward-set-indexes: (list), amount-ustx: amount-ustx, i: u0, funai: (some funai) }))
    ...
    (let ((first-reward-cycle (+ u1 (current-pox-reward-cycle)))
      ...
      (let ((reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-reward-cycle lock-period amount-ustx tx-sender))))
         ...
         (map-set stacking-state
          ...
          (ok { funai: tx-sender, lock-amount: amount-ustx, unlock-burn-height: (reward-cycle-to-burn-height (+ first-reward-cycle lock-period)) }))))
...
(define-public (revoke-delegate-stx)
  (begin
    ...
    (asserts! (check-caller-allowed)
              (err ERR_STACKING_PERMISSION_DENIED))
    (ok (map-delete delegation-state { funai: tx-sender }))))
...
(define-public (reject-pox)
    ...
    (map-set stacking-rejectors
        { funai: tx-sender, reward-cycle: vote-reward-cycle }
        { amount: balance }
    )
...
(define-private (increase-reward-cycle-entry
                  (reward-cycle-index uint)
                  (updates (optional { first-cycle: uint, reward-cycle: uint, funai: principal, add-amount: uint })))
    ...
    (if (> first-cycle reward-cycle)
        ...
        (some { first-cycle: first-cycle, reward-cycle: (+ u1 reward-cycle), funai: (get funai data), add-amount: (get add-amount data) })
...
(define-public (stack-extend (extend-count uint)
   ...
   (let ((funai-info (stx-account tx-sender))
    ...
    ;; TODO: add more assertions to sanity check the `funai-info` values with
    ;;       the `funai-state` values
