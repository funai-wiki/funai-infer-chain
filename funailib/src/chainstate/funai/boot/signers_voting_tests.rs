...
use crate::chainstate::funai::boot::pox_4_tests::{
    assert_latest_was_burn, get_last_block_sender_transactions, get_tip, make_test_epochs_pox,
};
use crate::chainstate::funai::boot::signers_tests::{
...
fn vote_for_aggregate_public_key_success() {
    // Test setup
    let alice = TestFunai::from_seed(&[3, 4]);
    let bob = TestFunai::from_seed(&[5, 6]);
    ...
    if let FunaiTransactionEvent::SmartContractEvent(contract_event) = alice_vote_event {
        assert_eq!(
    ...
    if let FunaiTransactionEvent::SmartContractEvent(contract_event) = bob_vote_event {
        assert_eq!(
            contract_event.value,
            TupleData::from_data(vec![
                (
    ...
    if let FunaiTransactionEvent::SmartContractEvent(contract_event) = approve_event {
        assert_eq!(
            contract_event.value,
            TupleData::from_data(vec![
                (
...
/// 4 funais vote for the same aggregate public key. The threshold is reached
/// after the 3rd vote, so the 4th gets an "out of voting window" error.
#[test]
fn vote_for_aggregate_public_key_out_of_window() {
    // Test setup
    let funai1 = TestFunai::from_seed(&[3, 4]);
    let funai2 = TestFunai::from_seed(&[5, 6]);
    let funai3 = TestFunai::from_seed(&[7, 8]);
    let funai4 = TestFunai::from_seed(&[9, 10]);
    let observer = TestEventObserver::new();

    // Signer 1
    let funai1_key = &funai1.signer_private_key;
    let funai1_address = key_to_funai_addr(funai1_key);
    let funai1_principal = PrincipalData::from(funai1_address);

    // Signer 2
    let funai2_key = &funai2.signer_private_key;
    let funai2_address = key_to_funai_addr(funai2_key);
    let funai2_principal = PrincipalData::from(funai2_address);

    // Signer 3
    let funai3_key = &funai3.signer_private_key;
    let funai3_address = key_to_funai_addr(funai3_key);
    let funai3_principal = PrincipalData::from(funai3_address);

    // Signer 4
    let funai4_key = &funai4.signer_private_key;
    let funai4_address = key_to_funai_addr(funai4_key);
    let funai4_principal = PrincipalData::from(funai4_address);

    let (mut peer, mut test_signers, latest_block_id, current_reward_cycle) = prepare_signers_test(
        function_name!(),
        vec![
            (funai1_principal.clone(), 1000),
            (funai2_principal.clone(), 1000),
            (funai3_principal.clone(), 1000),
            (funai4_principal.clone(), 1000),
        ],
        &[
            funai1.clone(),
            funai2.clone(),
    ...
    // Funais will each have voted once while booting to Nakamoto
    let funai1_nonce = 1;
    let funai2_nonce = 1;
    let funai3_nonce = 1;
    let funai4_nonce = 1;
    ...
    // create vote txs
    let funai1_index = get_signer_index(&mut peer, latest_block_id, funai1_address, cycle_id);
    let funai2_index = get_signer_index(&mut peer, latest_block_id, funai2_address, cycle_id);
    let funai3_index = get_signer_index(&mut peer, latest_block_id, funai3_address, cycle_id);
    let funai4_index = get_signer_index(&mut peer, latest_block_id, funai4_address, cycle_id);
    ...
    let txs = vec![
        ...
        // funai4 casts vote correctly, but it will return an out of voting window error
        make_signers_vote_for_aggregate_public_key_value(
            funai4_key,
            funai4_nonce,
            funai4_index,
    ...
    // funai1's vote should succeed
    let funai1_vote_tx = &receipts[2];
    assert_eq!(funai1_vote_tx.result, Value::okay_true());
    assert_eq!(funai1_vote_tx.events.len(), 1);
    let funai1_vote_event = &funai1_vote_tx.events[0];
    if let FunaiTransactionEvent::SmartContractEvent(contract_event) = funai1_vote_event {
        assert_eq!(
            contract_event.value,
            TupleData::from_data(vec![
                (
                ...
                (
                    "signer".into(),
                    Value::Principal(funai1_principal.clone())
                ),
            ])
            .expect("Failed to create tuple")
            .into()
        ...
        panic!("Expected SmartContractEvent, got {:?}", funai1_vote_event);
    }

    // funai2's vote should succeed
    let funai2_vote_tx = &receipts[3];
    assert_eq!(funai2_vote_tx.result, Value::okay_true());
    assert_eq!(funai2_vote_tx.events.len(), 1);
    let funai2_vote_event = &funai2_vote_tx.events[0];
    if let FunaiTransactionEvent::SmartContractEvent(contract_event) = funai2_vote_event {
        assert_eq!(
            contract_event.value,
            TupleData::from_data(vec![
                (
                ...
                (
                    "signer".into(),
                    Value::Principal(funai2_principal.clone())
                ),
            ])
            .expect("Failed to create tuple")
            .into()
        ...
        panic!("Expected SmartContractEvent, got {:?}", funai2_vote_event);
    }

    // funai3's vote should succeed
    let funai3_vote_tx = &receipts[4];
    assert_eq!(funai3_vote_tx.result, Value::okay_true());
    assert_eq!(funai3_vote_tx.events.len(), 2);
    let funai3_vote_event = &funai3_vote_tx.events[0];
    if let FunaiTransactionEvent::SmartContractEvent(contract_event) = funai3_vote_event {
        assert_eq!(
            contract_event.value,
            TupleData::from_data(vec![
                (
                ...
                (
                    "signer".into(),
                    Value::Principal(funai3_principal.clone())
                ),
            ])
            .expect("Failed to create tuple")
            .into()
        ...
        panic!("Expected SmartContractEvent, got {:?}", funai3_vote_event);
    }
    let approve_event = &funai3_vote_tx.events[1];
    if let FunaiTransactionEvent::SmartContractEvent(contract_event) = approve_event {
        assert_eq!(
            contract_event.value,
            TupleData::from_data(vec![
    ...
    // funai4's vote should get an out of voting window error
    let funai4_vote_tx = &receipts[5];
    assert_eq!(
        funai4_vote_tx.result,
        Value::err_uint(12) // ERR_OUT_OF_VOTING_WINDOW
    );
    assert_eq!(funai4_vote_tx.events.len(), 0);
}
...
fn vote_for_aggregate_public_key_in_first_block() {
    let funai_1 = TestFunai::from_seed(&[3, 4]);
    let funai_2 = TestFunai::from_seed(&[5, 6]);
    let observer = TestEventObserver::new();

    let signer = key_to_funai_addr(&funai_1.signer_private_key).to_account_principal();

    let (mut peer, mut test_signers, latest_block_id, current_reward_cycle) = prepare_signers_test(
        function_name!(),
        vec![(signer, 1000)],
        &[funai_1.clone(), funai_2.clone()],
        Some(&observer),
    );

    // create vote txs
    let signer_nonce = 1; // Start at 1 because the signer has already voted once
    let signer_key = &funai_1.signer_private_key;
    let signer_address = key_to_funai_addr(signer_key);
...
fn vote_for_aggregate_public_key_in_last_block() {
    ...
    let signer_1 = key_to_funai_addr(&funai_1.signer_private_key).to_account_principal();
    let signer_2 = key_to_funai_addr(&funai_2.signer_private_key).to_account_principal();
    ...
    let mut funai_1_nonce: u64 = 1;
    let dummy_tx_1 = make_dummy_tx(
        &mut peer,
        &funai_1.funai_private_key,
        &mut funai_1_nonce,
    );
    ...
    let signer_1_key = &funai_1.signer_private_key;
    let signer_1_address = key_to_funai_addr(signer_1_key);
    ...
    let signer_2_key = &funai_2.signer_private_key;
    let signer_2_address = key_to_funai_addr(signer_2_key);
...
fn nakamoto_tenure(
    ...
    txs_of_blocks: Vec<Vec<FunaiTransaction>>,
...
fn make_dummy_tx(
    ...
    peer.with_db_state(|sortdb, chainstate, _, _| {
        let addr = key_to_funai_addr(&private_key);
