...
use clarity::vm::eval;
use clarity::vm::events::{STXEventType, STXLockEventData, FunaiTransactionEvent};
...
use clarity::vm::types::{
    ...
    FunaiAddressExtensions, StandardPrincipalData, TupleData, TupleTypeSignature, TypeSignature,
    Value, NONE,
};
use clarity::vm::Value::Optional;
use funai_common::address::AddressHashMode;
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, FunaiAddress, FunaiBlockId, VRFSeed,
};
use funai_common::types::{Address, PrivateKey};
use funai_common::util::hash::{hex_bytes, to_hex, Sha256Sum, Sha512Trunc256Sum};
...
use crate::chainstate::funai::address::{PoxAddress, PoxAddressType20, PoxAddressType32};
use crate::chainstate::funai::boot::pox_2_tests::{
...
use crate::chainstate::funai::db::{
    MinerPaymentSchedule, FunaiChainState, FunaiHeaderInfo, MINER_REWARD_MATURITY,
};
use crate::chainstate::funai::events::{FunaiTransactionReceipt, TransactionOrigin};
use crate::chainstate::funai::index::marf::MarfConnection;
use crate::chainstate::funai::index::MarfTrieId;
use crate::chainstate::funai::tests::make_coinbase;
...
pub fn make_test_epochs_pox() -> (Vec<FunaiEpoch>, PoxConstants) {
...
fn pox_extend_transition() {
    ...
    let alice_address = key_to_funai_addr(&alice);
    let alice_principal = PrincipalData::from(alice_address.clone());
    let bob_address = key_to_funai_addr(&bob);
    ...
    let alice_rewards_to_v2_start_checks = |tip_index_block, peer: &mut TestPeer| {
        ...
        //  Alice is the only Funai, so check that.
        let (amount_ustx, pox_addr, lock_period, first_reward_cycle) =
            get_funai_info(peer, &key_to_funai_addr(&alice).into()).unwrap();
        eprintln!(
    ...
    let alice_account = get_account(&mut peer, &key_to_funai_addr(&alice).into());
    ...
    let alice_balance = get_balance(&mut peer, &key_to_funai_addr(&alice).into());
    ...
    while get_tip(peer.sortdb.as_ref()).block_height < epochs[4].start_height {
        ...
        let alice_balance = get_balance(&mut peer, &key_to_funai_addr(&alice).into());
        assert_eq!(alice_balance, 0);
        let bob_balance = get_balance(&mut peer, &key_to_funai_addr(&bob).into());
    ...
    for cycle_number in first_v4_cycle..(first_v4_cycle + 4) {
        ...
        assert_eq!(
            ...
            key_to_funai_addr(&alice).bytes.0.to_vec()
        );
    ...
    let bob_lockup = make_pox_4_lockup(
        ...
        &bob_pox_addr,
        3,
        &FunaiPublicKey::from_private(&bob_signer_private),
        tip.block_height,
        Some(bob_signature),
        u128::MAX,
        2,
    ...
    let alice_signer_key = FunaiPublicKey::from_private(&alice_signer_private);
    ...
    for cycle_number in (first_v4_cycle + 1)..(first_v4_cycle + 4) {
        ...
        assert_eq!(
            reward_set_entries[0].reward_address.bytes(),
            key_to_funai_addr(&bob).bytes.0.to_vec()
        );
    ...
    for b in blocks.into_iter() {
        for r in b.receipts.into_iter() {
            if let TransactionOrigin::Funai(ref t) = r.transaction {
    ...
    let common_data = PoxPrintFields {
        ...
        funai: Value::Principal(alice_principal.clone()),
...
fn get_burn_pox_addr_info(peer: &mut TestPeer) -> (Vec<PoxAddress>, u128) {
    let tip = get_tip(peer.sortdb.as_ref());
    let tip_index_block = tip.get_canonical_funai_block_id();
...
fn pox_lock_unlock() {
    ...
    let funais: Vec<_> = keys
        ...
        .map(|(ix, (key, hash_mode))| {
            let pox_addr = PoxAddress::from_legacy(hash_mode, key_to_funai_addr(key).bytes);
            ...
            txs.push(make_pox_4_lockup(
                ...
                &pox_addr,
                lock_period,
                &FunaiPublicKey::from_private(&signer_key),
                tip_height,
                Some(signature),
                u128::MAX,
                1,
    ...
    // Advance to start of rewards cycle funais are participating in
    ...
    for _ in 0..lock_period {
        ...
        info!("Checking that funais have STX locked for cycle {cycle}");
        ...
        info!("Checking we have 2 funais for cycle {cycle}");
        ...
        for funai in funais.iter() {
            assert!(
                rewarded.contains(funai),
                "Reward cycle should include {funai}"
            );
        }
    ...
    assert!(
        rewarded.contains(&funais[3]),
        "Reward set should include the index-3 funai"
    );

    info!("Checking that funais[0..2] have no STX locked");
    let balances = balances_from_keys(&mut peer, &latest_block, &keys);
...
fn pox_3_defunct() {
    ...
    info!("Checking that funais have no STX locked");
    let balances = balances_from_keys(&mut peer, &latest_block, &keys);
    assert_eq!(balances[0].amount_locked(), 0);
    assert_eq!(balances[1].amount_locked(), 0);
    ...
    for _ in 0..=lock_period {
        ...
        info!("Checking that funais have no STX locked for cycle {cycle}");
        let balances = balances_from_keys(&mut peer, &latest_block, &keys);
        assert_eq!(balances[0].amount_locked(), 0);
        assert_eq!(balances[1].amount_locked(), 0);

        info!("Checking no funais for cycle {cycle}");
        for _ in 0..burnchain.pox_constants.reward_cycle_length {
            latest_block = peer.tenure_with_txs(&[], &mut coinbase_nonce);
            // Should all be burn because no funais
            assert_latest_was_burn(&mut peer);
        }
    }
}
...
fn pox_3_unlocks() {
    ...
    for _ in 0..3 {
        ...
        info!("Checking that funais have no STX locked after cycle {cycle}");
        let balances = balances_from_keys(&mut peer, &latest_block, &keys);
        assert_eq!(balances[0].amount_locked(), 0);
        assert_eq!(balances[1].amount_locked(), 0);
    }
...
// In this set up, Steph is a solo funai and invokes `stack-stx`, `stack-increase` and `stack-extend` functions
...
fn pox_4_check_cycle_id_range_in_print_events_pool() {
    ...
    // steph the solo funai funai stx so nakamoto signer set stays stacking.
    let steph_key = keys.pop().unwrap();
    let steph_address = key_to_funai_addr(&steph_key);
    ...
    let tip = get_tip(peer.sortdb.as_ref());
    let tipId = FunaiBlockId::new(&tip.consensus_hash, &tip.canonical_funai_tip_hash);
    ...
    let common_data = PoxPrintFields {
        op_name: "stack-stx".to_string(),
        funai: steph_principal.clone().into(),
        balance: Value::UInt(10240000000000),
        locked: Value::UInt(0),
        burnchain_unlock_height: Value::UInt(0),
    };
    ...
    let common_data = PoxPrintFields {
        ...
        funai: alice_principal.clone().into(),
        balance: Value::UInt(10240000000000),
        locked: Value::UInt(0),
        burnchain_unlock_height: Value::UInt(0),
    };
    ...
    let common_data = PoxPrintFields {
        ...
        funai: bob_principal.clone().into(),
        balance: Value::UInt(10240000000000),
        locked: Value::UInt(0),
        burnchain_unlock_height: Value::UInt(0),
    };
...
// In this setup, Steph solo funai in the prepare phase
#[test]
fn pox_4_check_cycle_id_range_in_print_events_before_prepare_phase() {
    ...
    let txs: HashMap<_, _> = observer
        ...
        .filter_map(|r| match r.transaction {
            TransactionOrigin::Funai(ref t) => Some((t.txid(), r.clone())),
            _ => None,
        })
        .collect();
...
fn pox_4_delegate_stack_increase_events() {
    ...
    let alice_address = key_to_funai_addr(&alice_key);
    let alice_principal = PrincipalData::from(alice_address.clone());
    let alice_pox_addr = pox_addr_from(&alice_key);

    let bob_key = keys.pop().unwrap();
    let bob_address = key_to_funai_addr(&bob_key);
    let bob_principal = PrincipalData::from(bob_address.clone());
...
fn pox_4_revoke_delegate_stx_events() {
    ...
    let steph_address = key_to_funai_addr(&steph);
    let steph_principal = PrincipalData::from(steph_address.clone());
    let steph_pox_addr =
        make_pox_addr(AddressHashMode::SerializeP2PKH, steph_address.bytes.clone());
...
fn verify_signer_key_signatures() {
    ...
    let bob = keys.pop().unwrap();
    let bob_address = key_to_funai_addr(&bob);
    let bob_public_key = FunaiPublicKey::from_private(&bob);
...
fn stack_stx_verify_signer_sig() {
    ...
    let mut funai_nonce = 0;
    let funai_key = &keys[0];
    let min_ustx = get_stacking_minimum(&mut peer, &latest_block);
    let funai_addr = key_to_funai_addr(&funai_key);
    let signer_key = &keys[1];
    let signer_public_key = FunaiPublicKey::from_private(signer_key);
    let pox_addr = pox_addr_from(&funai_key);

    let second_funai = &keys[2];
    let second_funai_addr = key_to_funai_addr(second_funai);
    let second_funai_pox_addr = PoxAddress::from_legacy(
        AddressHashMode::SerializeP2PKH,
        second_funai_addr.bytes.clone(),
    );
    ...
    let invalid_cycle_nonce = funai_nonce;
    ...
    let invalid_pox_addr_nonce = funai_nonce;
    ...
    let invalid_key_nonce = funai_nonce;
    ...
    let invalid_topic_nonce = funai_nonce;
    ...
    let invalid_period_nonce = funai_nonce;
    ...
    let invalid_auth_id_nonce = funai_nonce;
    ...
    let invalid_amount_nonce = funai_nonce;
    ...
    let invalid_max_amount_nonce = funai_nonce;
    ...
    let funai_txs = get_last_block_sender_transactions(&observer, funai_addr);
    let expected_error = Value::error(Value::Int(35)).unwrap();

    assert_eq!(funai_txs.len(), (valid_nonce + 1) as usize);
    let tx_result =
        |nonce: u64| -> Value { funai_txs.get(nonce as usize).unwrap().result.clone() };
...
fn stack_agg_commit_verify_sig() {
    ...
    let funai_key = &keys[0];
    let funai_addr = PrincipalData::from(key_to_funai_addr(&funai_key));

    let signer_sk = &keys[1];
    let signer_pk = FunaiPublicKey::from_private(signer_sk);

    let delegate_key = &keys[2];
    let delegate_addr = key_to_funai_addr(&delegate_key);
...
// Helper struct to hold information about funais and signers
#[derive(Debug, Clone)]
struct FunaiSignerInfo {
    private_key: FunaiPrivateKey,
    public_key: FunaiPublicKey,
    principal: PrincipalData,
    address: FunaiAddress,
    pox_address: PoxAddress,
    nonce: u64,
}

impl FunaiSignerInfo {
    fn new() -> Self {
        let private_key = FunaiPrivateKey::new();
        let public_key = FunaiPublicKey::from_private(&private_key);
        let address = key_to_funai_addr(&private_key);
        let pox_address =
...
fn advance_to_block_height(
    ...
    target_height: u64,
) -> (FunaiBlockId, TestEventObserverBlock) {
...
#[test]
/// Test for verifying that the funai aggregation works as expected
...
fn stack_agg_increase() {
    ...
    let alice = FunaiSignerInfo::new();
    // Bob pool operator
    let mut bob = FunaiSignerInfo::new();
    // Carl pool 1 delegate
    let mut carl = FunaiSignerInfo::new();
    // Dave pool 1 delegate
    let mut dave = FunaiSignerInfo::new();
    // Eve late 1 pool delegate
    let mut eve = FunaiSignerInfo::new();
    // Frank pool 2 delegate
    let mut frank = FunaiSignerInfo::new();
    // Grace pool 2 delegate
    let mut grace = FunaiSignerInfo::new();
    ...
    let addr = FunaiAddress::from_public_keys(
        ...
        &AddressHashMode::SerializeP2PKH,
        1,
        &vec![FunaiPublicKey::from_private(&private_key)],
    )
    .unwrap();
    ...
    peer_config.epochs = Some(FunaiEpoch::unit_test_3_0_only(1000)); // Let us not activate nakamoto to make life easier
    ...
    let epoch_3 = &epochs[FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch30).unwrap()];
    ...
    // Carl pool funai timely delegating STX to Bob
    ...
    // Dave pool funai timely delegating STX to Bob
    ...
    // Eve Late Functions
    // Eve pool funai late delegating STX to Bob
    ...
    bob.nonce += 1;
    // Frank pool funai delegating STX to Bob
    ...
    frank.nonce += 1;
    // Grace pool funai delegating STX to Bob
...
fn stack_increase_verify_signer_key() {
    ...
    let signature = make_signer_key_signature(
        &pox_addr,
        &funai_key, // different than signer
        reward_cycle,
        &topic,
        lock_period,
        u128::MAX,
    ...
    let txs = get_last_block_sender_transactions(&observer, funai_addr);
    let tx_result = |nonce: u64| -> Value { txs.get(nonce as usize).unwrap().result.clone() };
...
fn stack_increase_different_signer_keys() {
    ...
    let second_signer_pk = FunaiPublicKey::from_private(second_signer_sk);
    ...
    let increase_nonce = funai_nonce;
    let stack_increase = make_pox_4_stack_increase(
        &funai_key,
        funai_nonce,
        min_ustx,
    ...
    let txs = get_last_block_sender_transactions(&observer, funai_addr.clone());
...
fn stack_stx_signer_key() {
    ...
    let stacking_state = get_stacking_state_pox_4(
        &mut peer,
        &latest_block,
        &key_to_funai_addr(funai_key).to_account_principal(),
    )
    .expect("No stacking state, stack-stx failed")
    .expect_tuple();

    let funai_txs =
        get_last_block_sender_transactions(&observer, key_to_funai_addr(&funai_key));

    let stacking_tx = funai_txs.get(0).unwrap();
    let events: Vec<&STXLockEventData> = stacking_tx
        .events
        .iter()
        .filter_map(|e| match e {
            FunaiTransactionEvent::STXEvent(STXEventType::STXLockEvent(data)) => Some(data),
            _ => None,
        })
        .collect();
...
fn stack_stx_signer_auth() {
    ...
    let failed_stack_nonce = funai_nonce;
    ...
    funai_nonce += 1;
    let successful_stack_nonce = funai_nonce;
    ...
    assert_eq!(funai_txs.len(), (funai_nonce + 1) as usize);
    let funai_tx_result =
        |nonce: u64| -> Value { funai_txs.get(nonce as usize).unwrap().result.clone() };

    // First stack-stx failed
    assert_eq!(funai_tx_result(failed_stack_nonce), expected_error);

    let successful_stack_result = funai_tx_result(successful_stack_nonce);
    // second stack-stx worked
    successful_stack_result
        .expect_result_ok()
        .expect("Expected ok result from stack-stx tx");

    let signer_txs = get_last_block_sender_transactions(&observer, key_to_funai_addr(&signer_key));
...
fn stack_extend_signer_auth() {
    ...
    funai_nonce += 1;
    let invalid_extend_nonce = funai_nonce;
    let invalid_cycle_tx = make_pox_4_extend(
        &funai_key,
        funai_nonce,
        pox_addr.clone(),
    ...
    funai_nonce += 1;
    let valid_extend_nonce = funai_nonce;
    let valid_tx = make_pox_4_extend(
        &funai_key,
        funai_nonce,
        pox_addr,
...
fn test_set_signer_key_auth() {
    ...
    let min_ustx = get_stacking_minimum(&mut peer, &latest_block);
    let alice_addr = key_to_funai_addr(&alice_key);
    let mut signer_nonce = 0;
    let signer_key = &keys[1];
    let signer_public_key = FunaiPublicKey::from_private(signer_key);
    let signer_addr = key_to_funai_addr(&signer_key);
    let pox_addr = pox_addr_from(&signer_key);
    ...
    signer_nonce += 1;
    let invalid_tx_period: FunaiTransaction = make_pox_4_set_signer_key_auth(
        &pox_addr,
        &signer_key,
    ...
    let invalid_tx_cycle: FunaiTransaction = make_pox_4_set_signer_key_auth(
        &pox_addr,
        &signer_key,
        1,
        &Pox4SignatureTopic::StackStx,
    ...
    let disable_auth_tx: FunaiTransaction = make_pox_4_set_signer_key_auth(
        &pox_addr,
        &signer_key,
        current_reward_cycle,
        &Pox4SignatureTopic::StackStx,
...
fn delegate_stack_stx_signer_key() {
    ...
    let delegate_principal = PrincipalData::from(key_to_funai_addr(delegate_key));
    ...
    let txs = vec![
        ...
        make_pox_4_contract_call(
            ...
            "delegate-stack-stx",
            vec![
                PrincipalData::from(key_to_funai_addr(funai_key)).into(),
                Value::UInt(min_ustx + 1),
                pox_addr_val.clone(),
                Value::UInt(block_height as u128),
...
//  Bob then funai the delegated stx for one cycle with an
...
fn delegate_stack_stx_extend_signer_key() {
    ...
    let alice_nonce = 0;
    let alice_funai_key = &keys[0];
    let mut bob_nonce = 0;
    let bob_delegate_private_key = &keys[1];
    let bob_delegate_principal = PrincipalData::from(key_to_funai_addr(bob_delegate_private_key));
    ...
    let pox_addr = PoxAddress::from_legacy(
        AddressHashMode::SerializeP2PKH,
        key_to_funai_addr(bob_delegate_private_key).bytes,
    );
    ...
    let alice_principal = PrincipalData::from(key_to_funai_addr(alice_funai_key));

    let delegate_stack_stx = make_pox_4_delegate_stack_stx(
        bob_delegate_private_key,
        bob_nonce,
        key_to_funai_addr(alice_funai_key).into(),
        min_ustx + 1,
        pox_addr.clone(),
    ...
    let partially_stacked_0 = get_partially_stacked_state_pox_4(
        ...
        &pox_addr,
        next_reward_cycle,
        &key_to_funai_addr(bob_delegate_private_key),
    );
...
// In this test case, Alice is a solo funai-signer.
//  Alice funai the stacking minimum for two cycles.
...
fn stack_increase() {
    ...
    let alice_address = key_to_funai_addr(alice_stacking_private_key);
    let signing_sk = FunaiPrivateKey::from_seed(&[1]);
    let signing_pk = FunaiPublicKey::from_private(&signing_sk);
    let signing_bytes = signing_pk.to_bytes_compressed();

    let min_ustx = get_stacking_minimum(&mut peer, &latest_block);
    let pox_addr = PoxAddress::from_legacy(
        AddressHashMode::SerializeP2PKH,
        key_to_funai_addr(alice_stacking_private_key).bytes,
    );
    let reward_cycle = get_current_reward_cycle(&peer, &burnchain);
    ...
    let stacking_state = get_stacking_state_pox_4(
        &mut peer,
        &latest_block,
        &key_to_funai_addr(alice_stacking_private_key).to_account_principal(),
    )
    .expect("No stacking state, stack-stx failed")
    .expect_tuple();
    ...
    let latest_block = peer.tenure_with_txs(&txs, &mut coinbase_nonce);
    let funai_transactions = get_last_block_sender_transactions(&observer, alice_address);

    let actual_result = funai_transactions.first().cloned().unwrap().result;

    let increase_event = &funai_transactions.first().cloned().unwrap().events[0];
    ...
    let common_data = PoxPrintFields {
        op_name: "stack-increase".to_string(),
        funai: Value::Principal(PrincipalData::from(alice_address.clone())),
        balance: Value::UInt(10234866375000),
        locked: Value::UInt(5133625000),
...
//  Bob funai half of Alice's funds. In the next cycle,
//  Bob funai Alice's remaining funds.
//
// This test asserts that Alice's total-locked is equal to
...
fn delegate_stack_increase() {
    ...
    let alice_nonce = 0;
    let alice_key = &keys[0];
    let alice_address = PrincipalData::from(key_to_funai_addr(alice_key));
    let mut bob_nonce = 0;
    let bob_delegate_key = &keys[1];
    let bob_delegate_address = PrincipalData::from(key_to_funai_addr(bob_delegate_key));
    let min_ustx = get_stacking_minimum(&mut peer, &latest_block);
    let signer_sk = FunaiPrivateKey::from_seed(&[1, 3, 3, 7]);
    let signer_pk = FunaiPublicKey::from_private(&signer_sk);
    ...
    let pox_addr = PoxAddress::from_legacy(
        AddressHashMode::SerializeP2PKH,
        key_to_funai_addr(bob_delegate_key).bytes,
    );
    ...
    let alice_principal = PrincipalData::from(key_to_funai_addr(alice_key));
    ...
    let delegate_transactions =
        get_last_block_sender_transactions(&observer, key_to_funai_addr(bob_delegate_key));
...
// In this test case, Alice delegates twice the stacking minimum to Bob.
//  Bob funai Alice's funds, and then immediately tries to funai-aggregation-increase.
...
pub fn get_stacking_state_pox_4(
    ...
    with_clarity_db_ro(peer, tip, |db| {
        let lookup_tuple = Value::Tuple(
            TupleData::from_data(vec![("funai".into(), account.clone().into())]).unwrap(),
        );
...
pub fn get_stacking_minimum(peer: &mut TestPeer, latest_block: &FunaiBlockId) -> u128 {
...
/// In this test case, two Funais, Alice and Bob stack in PoX 4. Alice funai enough
...
fn missed_slots_no_unlock() {
    ...
    for b in blocks.into_iter() {
        if let Some(ref reward_set_data) = b.reward_set_data {
            ...
            assert_eq!(
                FunaiPublicKey::from_private(&alice).to_bytes_compressed(),
