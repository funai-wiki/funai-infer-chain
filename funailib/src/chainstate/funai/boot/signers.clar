...
(define-data-var funaidb-signer-slots-0 (list 4000 { signer: principal, num-slots: uint }) (list))
(define-data-var funaidb-signer-slots-1 (list 4000 { signer: principal, num-slots: uint }) (list))
...
;; Called internally by the Funai node.
;; Stores the funaidb signer slots for a given reward cycle.
;; Since there is one funaidb per signer message, the `num-slots` field will always be u1.
(define-private (funaidb-set-signer-slots 
	...
	(let ((cycle-mod (mod reward-cycle u2)))
        ...
        (if (is-eq cycle-mod u0)
            (ok (var-set funaidb-signer-slots-0 signer-slots))
            (ok (var-set funaidb-signer-slots-1 signer-slots)))))
...
(define-read-only (funaidb-get-signer-slots-page (page uint))
    (if (is-eq page u0)     (ok (var-get funaidb-signer-slots-0))
        (if (is-eq page u1)  (ok (var-get funaidb-signer-slots-1))
...
;; NOTE: the node may ignore `write-freq`, since not all funaidbs will be needed at a given time
