...
The tokens will unlock and be returned to the Funai (tx-sender) automatically."),
        ("revoke-delegate-stx", "Revoke a Stacking delegate relationship. A particular Funai may only have one delegate,
so this method does not take any parameters, and just revokes the Funai's current delegate (if one exists)."),
...
a given reward cycle may be higher than this"),
        ...
        ("get-funai-info", "Returns the _current_ stacking information for `funai.  If the information
