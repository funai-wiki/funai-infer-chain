...
use funai_common::types::chainstate::{
    BurnchainHeaderHash, FunaiAddress, FunaiBlockId, FunaiPrivateKey, FunaiPublicKey,
};
...
use crate::chainstate::funai::boot::pox_2_tests::with_clarity_db_ro;
...
use crate::chainstate::funai::boot::test::{
    instantiate_pox_peer_with_epoch, key_to_funai_addr, make_pox_4_lockup, with_sortdb,
};
use crate::chainstate::funai::boot::{NakamotoSignerEntry, SIGNERS_NAME, SIGNERS_VOTING_NAME};
...
use crate::chainstate::funai::{
    FunaiTransaction, FunaiTransactionSigner, TenureChangeCause, TransactionAuth,
...
use crate::net::funaidb::{STACKERDB_CONFIG_FUNCTION, STACKERDB_INV_MAX};
...
fn make_signer_units() {
    assert_eq!(FunaiChainState::make_signer_set(100, &[]), None);
    ...
    fn perform_test(threshold: u128, input: &[(u64, u128)], expected: &[(u64, u128, u32)]) {
        ...
        let expected: Vec<_> = expected
            ...
            .collect();
        assert_eq!(
            FunaiChainState::make_signer_set(threshold, &in_entries),
            Some(expected)
        );
    }
...
fn make_signer_sanity_panic_0() {
    ...
    FunaiChainState::make_signer_set(5, &bad_set);
}
...
#[test]
fn signers_get_signer_keys_from_funaidb() {
    ...
    let signers = readonly_call(
        ...
        &latest_block_id,
        "signers".into(),
        "funaidb-get-signer-slots-page".into(),
        vec![Value::UInt(1)],
    )
    .expect_result_ok()
    .unwrap();

    assert_eq!(signers, expected_funaidb_slots);
}
...
) -> (TestPeer<'a>, TestSigners, FunaiBlockId, u128) {
    ...
    let blocks_and_sizes = peer.make_nakamoto_tenure(
        ...
        |miner, chainstate, sortdb, blocks| {
            if blocks.len() < num_blocks as usize {
                let addr = key_to_funai_addr(&funai_private_key);
