...
use crate::chainstate::funai::events::TransactionOrigin;
...
/// Get the reward set entries if evaluated at the given FunaiBlock
...
/// Perform `check_funai_link_invariants` on cycles [first_cycle_number, max_cycle_number]
pub fn check_all_funai_link_invariants(
    peer: &mut TestPeer,
    tip: &FunaiBlockId,
    ...
    for cycle in first_cycle_number..(max_cycle_number + 1) {
        ...
        if active_pox_contract == POX_3_NAME && epoch < FunaiEpochId::Epoch24 {
            info!(
        ...
        check_funai_link_invariants(peer, tip, cycle);
    }
}
...
// This function takes in a FunaiTransactionEvent for a print statement from a pox function that modifies
// a funai's state. It verifies that the values in the print statement are as expected.
pub fn check_pox_print_event(
    event: &FunaiTransactionEvent,
    common_data: PoxPrintFields,
    op_data: HashMap<&str, Value>,
) {
    if let FunaiTransactionEvent::SmartContractEvent(data) = event {
        test_debug!(
        ...
        assert_eq!(
            outer_tuple.data_map.get("funai"),
            Some(&common_data.funai)
        );
        assert_eq!(
...
/// Check the stacking-state invariants of `funai`
...
pub fn check_stacking_state_invariants(
    ...
    let account_state = with_clarity_db_ro(peer, tip, |db| {
        db.get_stx_balance_snapshot(funai)
            .unwrap()
            .canonical_balance_repr()
            .unwrap()
    });

    let tip_burn_height = FunaiChainState::get_funai_block_header_info_by_index_block_hash(
        peer.chainstate().db(),
        tip,
    )
    .unwrap()
    ...
    let stacking_state_entry = get_stacking_state_pox(peer, tip, funai, active_pox_contract)
        .expect(&format!(
            "Invariant violated: reward-cycle entry has funai field set, but not present in funai-state (pox_contract = {})",
            active_pox_contract,
        ))
        .expect_tuple().unwrap();
    ...
    if reward_indexes.len() > 0 || expect_indexes {
        ...
        for i in 0..lock_period {
            ...
            let entry_funai = entry_value.get("funai")
                .unwrap()
                .clone()
                .expect_optional().unwrap()
                .expect("Invariant violated: stacking-state.reward-set-indexes pointed at an entry without a funai set")
                .expect_principal().unwrap();

            assert_eq!(
                &entry_funai, funai,
                "Invariant violated: reward-set-index points to different funai's entry"
            );
...
/// Check that:
///  (1) `reward-cycle-pox-address-list.funai` points to a real `stacking-state`
...
pub fn check_funai_link_invariants(peer: &mut TestPeer, tip: &FunaiBlockId, cycle_number: u64) {
    let current_burn_height = FunaiChainState::get_funai_block_header_info_by_index_block_hash(
        peer.chainstate().db(),
        tip,
    )
    .unwrap()
    ...
    let tip_epoch = SortitionDB::get_funai_epoch(peer.sortdb().conn(), current_burn_height as u64)
        .unwrap()
        .unwrap();

    let cycle_start_epoch = SortitionDB::get_funai_epoch(peer.sortdb().conn(), cycle_start)
        .unwrap()
        .unwrap();
    ...
    if cycle_start_epoch.epoch_id == FunaiEpochId::Epoch22
        || cycle_start_epoch.epoch_id == FunaiEpochId::Epoch23
    {
        info!(
    ...
    if cycle_start_epoch.epoch_id == FunaiEpochId::Epoch24 && active_pox_contract != POX_3_NAME {
        info!(
    ...
    for (actual_index, entry) in reward_set_entries.iter().enumerate() {
        debug!(
            "Cycle {}: Check {:?} (stacked={}, funai={}, tip_epoch={})",
            cycle_number,
        ...
        if let Some(funai) = &entry.funai {
            if tip_cycle > cycle_number {
                // if the checked cycle is before the tip's cycle,
                // the reward-set-entrie's funai links are no longer necessarily valid
                // (because the reward cycles for those entries has passed)
                // so we continue here to skip the funai reference checks
                continue;
            }

            if tip_epoch.epoch_id == FunaiEpochId::Epoch22
                || tip_epoch.epoch_id == FunaiEpochId::Epoch23
            {
                // if the current tip is epoch-2.2 or epoch-2.3, the funai invariant checks
                // no longer make sense: the funai has unlocked, even though a reward cycle
                // is still active (i.e., the last active cycle from epoch-2.1).
                continue;
            }

            if tip_epoch.epoch_id >= FunaiEpochId::Epoch24
                && current_burn_height
            ...
            if tip_epoch.epoch_id >= FunaiEpochId::Epoch25
                && current_burn_height
            ...
            let StackingStateCheckData {
                ...
                cycle_indexes,
                ..
            } = check_stacking_state_invariants(peer, tip, funai, true, active_pox_contract);

            assert_eq!(&entry.reward_address, &pox_addr, "Invariant violated: reward-cycle entry has a different PoX addr than in funai-state");
            assert_eq!(
                cycle_indexes.get(&(cycle_number as u128)).cloned().unwrap(),
                actual_index as u128,
                "Invariant violated: stacking-state.reward-set-indexes entry at cycle_number must point to this funai's entry"
            );
        }
    }
...
pub fn get_reward_cycle_total(peer: &mut TestPeer, tip: &FunaiBlockId, cycle_number: u64) -> u128 {
    let active_pox_contract = peer.config.burnchain.pox_constants.active_pox_contract(
        peer.config
            .burnchain
            .reward_cycle_to_block_height(cycle_number),
...
pub fn with_clarity_db_ro<F, R>(peer: &mut TestPeer, tip: &FunaiBlockId, todo: F) -> R
where
...
/// In this test case, two Funais, Alice and Bob stack and interact with the
///  PoX v1 contract and PoX v2 contract across the epoch transition.
///
/// Alice: funai via PoX v1 for 4 cycles. The third of these cycles occurs after
///        the PoX v1 -> v2 transition, and so Alice gets "early unlocked".
///        After the early unlock, Alice re-funai in PoX v2
...
/// Bob:   funai via PoX v2 for 6 cycles. He attempted to stack via PoX v1 as well,
...
fn test_simple_pox_lockup_transition_pox_2() {
    ...
    let epochs = FunaiEpoch::all(0, 0, EMPTY_SORTITIONS as u64 + 10);
    ...
    let test = make_pox_2_contract_call(
        ...
        vec![
            ...
            PrincipalData::from(key_to_funai_addr(&charlie)).into(),
            Value::none(),
            Value::none(),
        ],
    );
    ...
    for b in blocks.into_iter() {
        for r in b.receipts.into_iter() {
            if let TransactionOrigin::Funai(ref t) = r.transaction {
                ...
                } else if addr == key_to_funai_addr(&charlie) {
                    assert!(
...
fn test_simple_pox_2_auto_unlock(alice_first: bool) {
    ...
    let (bob_bal, _) = get_stx_account_at(
        &mut peer,
        &latest_block,
        &key_to_funai_addr(&bob).to_account_principal(),
    )
    ...
    let alice_state = get_stacking_state_pox_2(
        &mut peer,
        &latest_block,
        &key_to_funai_addr(&alice).to_account_principal(),
    )
    ...
    let alice_address = key_to_funai_addr(&alice);
    let bob_address = key_to_funai_addr(&bob);
    let charlie_address = key_to_funai_addr(&charlie);
    let blocks = observer.get_blocks();
    ...
    let common_data = PoxPrintFields {
        ...
        funai: Value::Principal(
            FunaiAddress::from_string("ST1GCB6NH3XR67VT4R5PKVJ2PYXNVQ4AYQATXNP4P")
                .unwrap()
...
///  Bob funai Alice's funds via PoX v2 for 6 cycles. In the third cycle,
...
fn delegate_stack_increase() {
    ...
    let common_data = PoxPrintFields {
        ...
        funai: Value::Principal(
            FunaiAddress::from_string("ST2Q1B4S2DY2Y96KYNZTVCCZZD1V9AGWCS5MFXM4C")
                .unwrap()
                .to_account_principal(),
        ),
...
/// In this test case, Alice funai and interacts with the
///  PoX v2 contract after the epoch transition.
///
/// Alice: funai via PoX v2 for 6 cycles. In the third cycle, Alice invokes
/// `stack-increase`
///
#[test]
fn stack_increase() {
...
///  covers the two different ways a Funai can validly extend via `stack-extend` --
...
///        Alice tries to stack again via PoX v1, which is allowed by the contract,
///        but forbidden by the VM (because PoX has transitioned to v2)
/// Bob:   funai via PoX v2 for 3 cycles.
...
/// In this test case, two Funais, Alice and Bob delegate stack and interact with the
///  PoX v1 contract and PoX v2 contract across the epoch transition. This test
///  covers the two different ways a Funai can be validly extended via `delegate-stack-extend` --
///  extending from a V1 lockup and extending from a V2 lockup.
///
/// Alice: delegate-funai via PoX v1 for 4 cycles. The third of these cycles occurs after
///        the PoX v1 -> v2 transition, and so Alice gets "early unlocked".
...
///        Alice tries to stack again via PoX v1, which is allowed by the contract,
///        but forbidden by the VM (because PoX has transitioned to v2)
/// Bob:   delegate-funai via PoX v2 for 3 cycles.
///        Bob extends 1 cycles
#[test]
fn test_delegate_extend_transition_pox_2() {
    ...
    let delegate_op_data = HashMap::from([
        ...
        (
            "delegator",
            Value::Principal(
                FunaiAddress::from_string("ST9DJEQ7PRF5PZCGBJ2R53A343KW48FM6DJS5VNY")
                    .unwrap()
                    .to_account_principal(),
            ),
        ),
...
fn test_get_pox_addrs() {
    ...
    let epochs = FunaiEpoch::all(1, 2, 3);
                ...
                if tenure_id == 1 {
                    ...
                    for (key, hash_mode) in keys.iter().zip(
                        ...
                        let lockup = make_pox_2_lockup(
                            ...
                            0,
                            1024 * POX_THRESHOLD_STEPS_USTX,
                            PoxAddress::from_legacy(*hash_mode, key_to_funai_addr(key).bytes),
                            2,
                            tip.block_height,
                        );
...
fn stack_aggregation_increase() {
    ...
    let dan_address = key_to_funai_addr(&dan);
...
fn stack_in_both_pox1_and_pox2() {
    ...
    // alice funai some of her STX in pox1
    ...
    // alice funai some of her STX in pox2
    ...
    // bob funai some of his STX in pox2
    ...
    // bob funai some of her STX in pox1
