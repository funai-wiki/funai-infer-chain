...
(define-public (stack-extend (extend-count uint)
                             (pox-addr { version: (buff 1), hashbytes: (buff 32) }))
   (let ((funai-info (stx-account tx-sender))
         (funai-state (get-funai-info tx-sender))
         ...
         ;;  whether there is an existing entry for the funai in the state
         ;; this would be the case if the funai is extending a lockup from PoX-1
         ;;  to PoX-2
         (first-reward-cycle (match (get first-reward-cycle funai-state)
                                       ;; if we've stacked in PoX2, then max(cur-cycle, funai-state.first-reward-cycle) is valid
    ...
    (let ((last-extend-cycle  (- (+ first-extend-cycle extend-count) u1))
      ...
      (let ((extended-reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-extend-cycle extend-count amount-ustx tx-sender)))
            (reward-set-indexes (match funai-state
                                       ;; if there's active funai state, we need to extend the existing reward-set-indexes
...
(define-public (delegate-stack-extend
                    ...
                    (extend-count uint))
    (let ((funai-info (stx-account funai))
          (funai-state (get-funai-info funai))
          (amount-ustx (get locked funai-info))
          (unlock-height (get unlock-height funai-info))
          (unlock-in-cycle (burn-height-to-reward-cycle unlock-height))
          ;; if the account unlocks *during* this cycle (should only occur during testing),
          ...
          ;;  to PoX-2
          (first-reward-cycle (match (get first-reward-cycle funai-state)
                                       ;; if funai stacked in PoX2, then max(cur-cycle, funai-state.first-reward-cycle) is valid
                                       old-first-cycle (if (> cur-cycle old-first-cycle) cur-cycle old-first-cycle)
                                       ;; otherwise, there aren't PoX2 entries until first-extend-cycle
                                       first-extend-cycle)))
