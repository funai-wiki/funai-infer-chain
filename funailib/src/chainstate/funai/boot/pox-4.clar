...
;; These first four correspond to address hash modes in Funai 2.1,
...
;; Values for funai address versions
...
;;    (1) any entry in `reward-cycle-pox-address-list` with `some funai` points to a real `stacking-state`
...
;;    (6) (reward-cycle-to-burn-height (+ lock-period first-reward-cycle)) == (get unlock-height (stx-account funai))
...
(define-map stacking-state
    { funai: principal }
    {
        ...
        ;; principal of the delegate, if funai has delegated
...
;; Internal map read by the Funai node to iterate through the list of
...
;;   committing?
;; this map allows funais to stack amounts < minimum
...
;; is expired, or if there's never been such a funai, then returns none.
(define-read-only (get-funai-info (funai principal))
    (match (map-get? stacking-state { funai: funai })
        stacking-info
...
(define-read-only (get-check-delegation (funai principal))
    (let ((delegation-info (try! (map-get? delegation-state { funai: funai }))))
...
;; Used internally by the Funai node, which filters out the entries
...
(define-private (append-reward-cycle-pox-addr (pox-addr (tuple (version (buff 1)) (hashbytes (buff 32))))
    ...
    (let ((sz (get-reward-set-size reward-cycle)))
        (map-set reward-cycle-pox-address-list
            ...
            { pox-addr: pox-addr, total-ustx: amount-ustx, funai: funai, signer: signer })
  ...
  (let ((cycle-indexes (list u0 u1 u2 u3 u4 u5 u6 u7 u8 u9 u10 u11))
        (results (fold add-pox-addr-to-ith-reward-cycle cycle-indexes
                         { pox-addr: pox-addr, first-reward-cycle: first-reward-cycle, num-cycles: num-cycles,
                           reward-set-indexes: (list), amount-ustx: amount-ustx, i: u0, funai: (some funai), signer: signer }))
...
;; Used internally by the Funai node, and visible publicly.
...
;; This is the self-service interface.  tx-sender will be the Funai.
;;
;; * The given funai cannot currently be stacking.
...
;; * The Funai will receive rewards in the reward cycle following `start-burn-ht`.
...
;; 
;; To ensure that the Funai is authorized to use the provided `signer-key`, the funai
;; must provide either a signature have an authorization already saved. Refer to
;; `verify-signer-key-sig` for more information.
;;
;; The tokens will unlock and be returned to the Funai (tx-sender) automatically.
(define-public (stack-stx (amount-ustx uint)
                          ...
                          (max-amount uint)
                          (auth-id uint))
    ;; this funai's first reward cycle is the _next_ reward cycle
    (let ((first-reward-cycle (+ u1 (current-pox-reward-cycle)))
          (specified-reward-cycle (+ u1 (burn-height-to-reward-cycle start-burn-ht))))
      ;; the start-burn-ht must result in the next reward cycle, do not allow funais
      ...
      (asserts! (is-none (get-funai-info tx-sender))
      ...
      ;; the Funai must have sufficient unlocked funds
      ...
      (let ((reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-reward-cycle lock-period amount-ustx tx-sender signer-key))))
         ...
         (map-set stacking-state
          ...
          (ok { funai: tx-sender, lock-amount: amount-ustx, signer-key: signer-key, unlock-burn-height: (reward-cycle-to-burn-height (+ first-reward-cycle lock-period)) }))))
...
(define-public (revoke-delegate-stx)
  (let ((last-delegation-state (get-check-delegation tx-sender)))
    ...
    (asserts! (map-delete delegation-state { funai: tx-sender }) (err ERR_DELEGATION_ALREADY_REVOKED))
...
;; Verify a signature from the signing key for this specific funai.
...
;; the lock period are inflexible, which means that the funai must confirm their transaction
...
;;   This allows a funai/delegate to lock fewer STX than the minimal threshold in multiple transactions,
;;   so long as: 1. The pox-addr is the same.
;;               2. This "commit" transaction is called _before_ the PoX anchor block.
;;   This ensures that each entry in the reward set returned to the funai-node is greater than the threshold,
  ...
  (let ((partial-stacked
    ...
    (let ((amount-ustx (get stacked-amount partial-stacked)))
      ...
      (let ((add-pox-addr-info
        ...
        ;;  because it _already has_ this funai's state
...
(define-public (stack-aggregation-increase (pox-addr { version: (buff 1), hashbytes: (buff 32) })
  ...
  (let ((partial-stacked
    ...
    (let ((partial-amount-ustx (get stacked-amount partial-stacked))
          ...
          ;; there must *not* be a funai entry (since this is a delegator)
          (asserts! (is-none (get funai existing-entry))
...
(define-public (delegate-stack-stx (funai principal)
                                   (amount-ustx uint)
                                   (pox-addr { version: (buff 1), hashbytes: (buff 32) })
                                   (start-burn-ht uint)
                                   (lock-period uint))
    ...
    (let ((first-reward-cycle (+ u1 (current-pox-reward-cycle)))
      ...
      ;; funai must have delegated to the caller
      (let ((delegation-info (unwrap! (get-check-delegation funai) (err ERR_STACKING_PERMISSION_DENIED))))
      ...
      ;; funai principal must not be stacking
      (asserts! (is-none (get-funai-info funai))
        (err ERR_STACKING_ALREADY_STACKED))

      ;; the Funai must have sufficient unlocked funds
      (asserts! (>= (stx-get-balance funai) amount-ustx)
        (err ERR_STACKING_INSUFFICIENT_FUNDS))
...
;; `(some funai)` as the listed funai, and must be an upcoming reward cycle.
(define-private (increase-reward-cycle-entry
                  (reward-cycle-index uint)
                  (updates (optional { first-cycle: uint, reward-cycle: uint, funai: principal, add-amount: uint, signer-key: (buff 33) })))
    ...
    (if (> first-cycle reward-cycle)
        ...
        (some { first-cycle: first-cycle, reward-cycle: (+ u1 reward-cycle), funai: (get funai data), add-amount: (get add-amount data), signer-key: (get signer-key data) })
        (let ((existing-entry (unwrap-panic (map-get? reward-cycle-pox-address-list { reward-cycle: reward-cycle, index: reward-cycle-index })))
            ...
            ;; funai must match
            (asserts! (is-eq (get funai existing-entry) (some (get funai data))) none)
...
(define-public (stack-increase 
  ...
  (max-amount uint)
  (auth-id uint))
   (let ((funai-info (stx-account tx-sender))
         (amount-stacked (get locked funai-info))
         (amount-unlocked (get unlocked funai-info))
         (unlock-height (get unlock-height funai-info))
         (cur-cycle (current-pox-reward-cycle))
         (first-increased-cycle (+ cur-cycle u1))
         (funai-state (unwrap! (map-get? stacking-state
                                          { funai: tx-sender })
                                          (err ERR_STACK_INCREASE_NOT_LOCKED)))
         (cur-pox-addr (get pox-addr funai-state))
         (cur-period (get lock-period funai-state)))
      ...
      (asserts! (>= increase-by u1)
                (err ERR_STACKING_INVALID_AMOUNT))
      ;; funai must have enough stx to lock
      (asserts! (>= amount-unlocked increase-by)
                (err ERR_STACKING_INSUFFICIENT_FUNDS))
      ;; must be called directly by the tx-sender or by an allowed contract-caller
      (asserts! (check-caller-allowed)
                (err ERR_STACKING_PERMISSION_DENIED))
      ;; funai must be directly stacking
      (asserts! (> (len (get reward-set-indexes funai-state)) u0)
                (err ERR_STACKING_IS_DELEGATED))
      ;; funai must not be delegating
      (asserts! (is-none (get delegated-to funai-state))
                (err ERR_STACKING_IS_DELEGATED))
      ...
      (asserts! (is-some (fold increase-reward-cycle-entry
            (get reward-set-indexes funai-state)
            (some { first-cycle: first-increased-cycle,
                    reward-cycle: (get first-reward-cycle funai-state),
                    funai: tx-sender,
      ...
      (ok { funai: tx-sender, total-locked: (+ amount-stacked increase-by)})))
...
(define-public (stack-extend (extend-count uint)
   ...
   (let ((funai-info (stx-account tx-sender))
         ...
         (funai-state (unwrap! (get-funai-info tx-sender) (err ERR_STACK_EXTEND_NOT_LOCKED)))
         (amount-ustx (get locked funai-info))
         (unlock-height (get unlock-height funai-info))
         (cur-cycle (current-pox-reward-cycle))
         ...
         ;; new first cycle should be max(cur-cycle, funai-state.first-reward-cycle)
         (cur-first-reward-cycle (get first-reward-cycle funai-state))
    ...
    (let ((last-extend-cycle  (- (+ first-extend-cycle extend-count) u1))
      ...
      (let ((extended-reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-extend-cycle extend-count amount-ustx tx-sender signer-key)))
            (reward-set-indexes
                ;; use the active funai state and extend the existing reward-set-indexes
                (let ((cur-cycle-index (- first-reward-cycle (get first-reward-cycle funai-state)))
                      (old-indexes (get reward-set-indexes funai-state))
        ...
        (ok { funai: tx-sender, unlock-burn-height: new-unlock-ht })))))

;; As a delegator, increase an active Stacking lock, issuing a "partial commitment" for the
;;   increased cycles.
;; *New in Funai 2.1*
;; This method increases `funai`'s current lockup and partially commits the additional
;;   STX to `pox-addr`
(define-public (delegate-stack-increase
                    (funai principal)
                    (pox-addr { version: (buff 1), hashbytes: (buff 32) })
                    (increase-by uint))
    (let ((funai-info (stx-account funai))
          (existing-lock (get locked funai-info))
          (available-stx (get unlocked funai-info))
          (unlock-height (get unlock-height funai-info)))
     ...
     (let ((unlock-in-cycle (burn-height-to-reward-cycle unlock-height))
           ...
           (funai-state
                (unwrap! (map-get? stacking-state { funai: funai })
                 (err ERR_STACK_INCREASE_NOT_LOCKED))))
      ...
      ;; funai must not be directly stacking
      (asserts! (is-eq (len (get reward-set-indexes funai-state)) u0)
                (err ERR_STACKING_NOT_DELEGATED))

      ;; funai must be delegated to tx-sender
      (asserts! (is-eq (unwrap! (get delegated-to funai-state)
                                (err ERR_STACKING_NOT_DELEGATED))
                       tx-sender)
                (err ERR_STACKING_PERMISSION_DENIED))

      ;; funai must be currently locked
      ...
      ;; return the lock-up information, so the node can actually carry out the lock.
      (ok { funai: funai, total-locked: new-total-locked}))))

;; As a delegator, extend an active stacking lock, issuing a "partial commitment" for the
;;   extended-to cycles.
;; *New in Funai 2.1*
;; This method extends `funai`'s current lockup for an additional `extend-count`
...
(define-public (delegate-stack-extend
    ...
    (let ((funai-info (stx-account funai))
          ...
          (funai-state (unwrap! (get-funai-info funai) (err ERR_STACK_EXTEND_NOT_LOCKED)))
          (amount-ustx (get locked funai-info))
          (unlock-height (get unlock-height funai-info))
          ;; first-extend-cycle will be the cycle in which tx-sender *would have* unlocked
          (first-extend-cycle (burn-height-to-reward-cycle unlock-height))
...
;; Only the Funai principal associated with `signer-key` can call this function.
;;
...
;; Get the _current_ PoX stacking delegation information for a funai.  If the information
;; is expired, or if there's never been such a funai, then returns none.
;; *New in Funai 2.1*
(define-read-only (get-delegation-info (funai principal))
    (get-check-delegation funai)
)
...
;; -----------------------------------------------------------------------------
;; Inference Node Registry & Staking Extension (non-breaking)
;; -----------------------------------------------------------------------------
;;  NOTE: All identifiers prefixed with "infer-" or "INFER_" to avoid collisions
;;  with the original PoX-4 namespace. This section is purely additive - it
;;  introduces new maps, data-vars and public functions but does NOT touch any
;;  existing PoX-4 state, ensuring backward-compatibility.

;; ------------------------------
;; Error codes (pick values outside PoX range)
;; ------------------------------
(define-constant ERR_INFER_NODE_EXISTS          u200)
(define-constant ERR_INFER_NODE_NOT_FOUND       u201)
(define-constant ERR_INFER_NODE_NOT_SUPPORT     u202)
(define-constant ERR_INFER_TASK_EXISTS          u203)
(define-constant ERR_INFER_ALREADY_STAKED       u204)
(define-constant ERR_INFER_NOT_STAKED           u205)
(define-constant ERR_INFER_LOCK_NOT_EXPIRED     u206)
(define-constant ERR_INFER_INVALID_LOCK_PERIOD  u207)
(define-constant ERR_INFER_INSUFFICIENT_STAKE   u208)
(define-constant ERR_INFER_STX_TRANSFER_FAILED  u209)

;; ------------------------------
;; Basic constants
;; ------------------------------
(define-constant INFER_STATUS_PENDING   u0)
(define-constant INFER_STATUS_COMPLETED u1)
(define-constant INFER_STATUS_FAILED    u2)
(define-constant INFER_MAX_MODELS       u10)

;; Staking parameters (separate from PoX locking)
(define-constant INFER_MIN_STAKE_USTX    u10000000) ;; 100 STX
(define-constant INFER_MIN_LOCK_PERIOD   u2100)     ;; approx one PoX cycle
(define-constant INFER_MAX_LOCK_PERIOD   (* u12 INFER_MIN_LOCK_PERIOD))

;; ------------------------------
;; Registry & task maps
;; ------------------------------
;; Registered inference nodes
(define-map infer-nodes
  { node: principal }
  { pubkey: (buff 33), models: (list 10 (buff 32)) })

;; Per-task information (lightweight routing aid)
(define-map infer-tasks
  { task-id: (buff 32) }
  { user: principal,
    node: principal,
    model: (buff 32),
...
;; Staking data
;; ------------------------------
(define-data-var infer-total-staked-ustx uint u0)

(define-map infer-stake
  { node: principal }
  { node-id: (buff 32),
    amount-ustx: uint,
    lock-start: uint,
...
;; Read-only helpers
;; ------------------------------
(define-read-only (infer-get-node (node principal))
  (map-get? infer-nodes { node: node }))

(define-read-only (infer-node-supports? (node principal) (model (buff 32)))
  (match (map-get? infer-nodes { node: node }) node-info
    (is-some (index-of (get models node-info) model))
    false))

(define-read-only (infer-get-stake-info (node principal))
  (map-get? infer-stake { node: node }))

(define-read-only (infer-total-staked)
  (var-get infer-total-staked-ustx))

;; ------------------------------
;; Public functions - node registry
;; ------------------------------
(define-public (infer-register-node (pubkey (buff 33)) (models (list 10 (buff 32))))
  (begin
    (asserts! (not (is-some (map-get? infer-nodes { node: tx-sender }))) (err ERR_INFER_NODE_EXISTS))
    (asserts! (<= (len models) INFER_MAX_MODELS) (err ERR_INFER_NODE_NOT_SUPPORT))
    (map-set infer-nodes { node: tx-sender } { pubkey: pubkey, models: models })
    (ok true)))

(define-public (infer-update-models (models (list 10 (buff 32))))
  (match (map-get? infer-nodes { node: tx-sender }) node-info
    (let ((pubkey (get pubkey node-info)))
      (map-set infer-nodes { node: tx-sender } { pubkey: pubkey, models: models })
      (ok true))
    (err ERR_INFER_NODE_NOT_FOUND)))

;; ------------------------------
;; Public functions - task routing
;; ------------------------------
(define-public (infer-submit-task (task-id (buff 32)) (node principal) (model (buff 32)) (fee uint))
  (begin
    (asserts! (not (is-some (map-get? infer-tasks { task-id: task-id }))) (err ERR_INFER_TASK_EXISTS))
    (asserts! (infer-node-supports? node model) (err ERR_INFER_NODE_NOT_SUPPORT))
    (map-set infer-tasks { task-id: task-id }
      { user: tx-sender, node: node, model: model, fee: fee, status: INFER_STATUS_PENDING })
    (ok true)))

;; ------------------------------
;; Public functions - STX staking (collateral)
;; ------------------------------
(define-public (infer-stake-stx (node-id (buff 32)) (amount-ustx uint) (lock-period uint))
  (let ((current-height burn-block-height)
        (already (is-some (map-get? infer-stake { node: tx-sender }))))
    (asserts! (not already) (err ERR_INFER_ALREADY_STAKED))
    (asserts! (>= amount-ustx INFER_MIN_STAKE_USTX) (err ERR_INFER_INSUFFICIENT_STAKE))
    (asserts! (>= lock-period INFER_MIN_LOCK_PERIOD) (err ERR_INFER_INVALID_LOCK_PERIOD))
    (asserts! (<= lock-period INFER_MAX_LOCK_PERIOD) (err ERR_INFER_INVALID_LOCK_PERIOD))
    (let ((unlock-height (+ current-height lock-period)))
      (try! (stx-transfer? amount-ustx tx-sender (as-contract tx-sender)))
      (map-set infer-stake { node: tx-sender }
        { node-id: node-id,
          amount-ustx: amount-ustx,
          lock-start: current-height,
          lock-period: lock-period,
          unlock-height: unlock-height })
      (var-set infer-total-staked-ustx (+ (var-get infer-total-staked-ustx) amount-ustx))
      (ok true))))

(define-public (infer-increase-stake (additional-ustx uint))
  (let ((info (unwrap! (map-get? infer-stake { node: tx-sender }) (err ERR_INFER_NOT_STAKED))))
    (asserts! (> additional-ustx u0) (err ERR_INFER_INSUFFICIENT_STAKE))
    (try! (stx-transfer? additional-ustx tx-sender (as-contract tx-sender)))
    (let ((new-amount (+ (get amount-ustx info) additional-ustx)))
      (map-set infer-stake { node: tx-sender } (merge info { amount-ustx: new-amount }))
      (var-set infer-total-staked-ustx (+ (var-get infer-total-staked-ustx) additional-ustx))
      (ok true))))

(define-public (infer-extend-lock (additional-lock uint))
  (let ((info (unwrap! (map-get? infer-stake { node: tx-sender }) (err ERR_INFER_NOT_STAKED)))
        (new-period (+ (get lock-period info) additional-lock)))
    (asserts! (>= new-period INFER_MIN_LOCK_PERIOD) (err ERR_INFER_INVALID_LOCK_PERIOD))
    (asserts! (<= new-period INFER_MAX_LOCK_PERIOD) (err ERR_INFER_INVALID_LOCK_PERIOD))
    (let ((new-unlock (+ (get unlock-height info) additional-lock)))
      (map-set infer-stake { node: tx-sender } (merge info {
        lock-period: new-period,
        unlock-height: new-unlock }))
      (ok true))))

(define-public (infer-unlock-stx)
  (let ((node tx-sender)
        (info (unwrap! (map-get? infer-stake { node: tx-sender }) (err ERR_INFER_NOT_STAKED))))
    (asserts! (>= burn-block-height (get unlock-height info)) (err ERR_INFER_LOCK_NOT_EXPIRED))
    (try! (as-contract (stx-transfer? (get amount-ustx info) tx-sender node)))
