...
use funai_common::types;
use funai_common::types::chainstate::{BlockHeaderHash, FunaiAddress, FunaiBlockId};
use funai_common::util::hash::{hex_bytes, to_hex, Hash160};
...
use crate::core::{
    FunaiEpochId, BITCOIN_REGTEST_FIRST_BLOCK_HASH, CHAIN_ID_MAINNET, POX_MAXIMAL_SCALING,
...
pub fn make_contract_id(addr: &FunaiAddress, name: &str) -> QualifiedContractIdentifier {
...
pub struct RawRewardSetEntry {
    ...
    pub funai: Option<PrincipalData>,
...
impl FunaiChainState {
    ...
    ///  cycle's "start" has been handled by the Funai fork yet. This
    ///  is used in Funai 2.1 to help process unlocks.
    ...
    ///  Funai fork in the opened `clarity_db`.
            ...
            .with_readonly_clarity_env(
                ...
                |vm_env| {
                    vm_env.eval_read_only_with_rules(
                        &pox_contract,
                        &format!(r#"
                            (unwrap-panic (map-get? stacking-state {{ funai: '{unlocked_principal} }}))
                            "#,
    ...
    fn synthesize_unlock_event_data(
        ...
        let user_first_cycle_locked = user_data
            ...
            .expect("FATAL: missing funai info")
            .to_owned();
                ...
                |vm_env| {
                    vm_env.eval_read_only_with_rules(
                        ...
                        &format!(
                            r#"
                            (let (
                                (funai-info (stx-account '{unlocked_principal}))
    ...
    /// Currently, this just means applying any auto-unlocks to Funais who qualified.
    ///
    ...
    fn handle_pox_cycle_missed_unlocks(
        ...
        if !matches!(pox_contract_ver, PoxVersions::Pox2 | PoxVersions::Pox3) {
            return Err(Error::InvalidFunaiBlock(format!(
        ...
        for (principal, amount_locked) in cycle_info.missed_reward_slots.iter() {
            ...
            // 1. lookup their Funai account and accelerate their unlock
            ...
            clarity.with_clarity_db(|db| {
                // lookup the Funai account and alter their unlock height to next block
    ...
    pub fn get_liquid_ustx(&mut self, funai_block_id: &FunaiBlockId) -> u128 {
        ...
        let mut signer_set: Vec<_> = signer_set
            ...
            .filter_map(|(signing_key, stacked_amt)| {
                let weight = u32::try_from(stacked_amt / threshold)
                    .expect("CORRUPTION: Funai claimed > u32::max() reward slots");
                if weight == 0 {
                    return None;
                }
    ...
    pub fn make_reward_set(
        ...
        {
            let mut contributed_funais = vec![];
            if let Some(funai) = funai.as_ref() {
                contributed_funais.push((funai.clone(), stacked_amt));
            }
            ...
            while addresses.last().map(|x| &x.reward_address) == Some(&address) {
                ...
                if let Some(funai) = next_contrib.funai {
                    contributed_funais.push((funai.clone(), additional_amt));
                }

                stacked_amt = stacked_amt
                    .checked_add(additional_amt)
                    .expect("CORRUPTION: Funai stacked > u128 max amount");
            }
            ...
            // if funai did not qualify for a slot *and* they have a funai
            //   pointer set by the PoX contract, then add them to auto-unlock list
            if slots_taken == 0 && !contributed_funais.is_empty() {
                info!(
                    "{}",
                    if epoch_id.supports_pox_missed_slot_unlocks() {
                        "Funai missed reward slot, added to unlock list"
                    } else {
                        "Funai missed reward slot"
                    };
                ...
                contributed_funais
                    .sort_by_cached_key(|(funai, ..)| to_hex(&funai.serialize_to_vec()));
                while let Some((contributor, amt)) = contributed_funais.pop() {
                    let mut total_amount = amt;
                    while contributed_funais.last().map(|(funai, ..)| funai)
                        == Some(&contributor)
                    {
                        let (add_funai, additional) = contributed_funais
                            .pop()
                            .expect("BUG: last() returned some, but pop() is none.");
                        assert_eq!(&add_funai, &contributor);
    ...
    fn get_reward_addresses_pox_2(
        ...
        for i in 0..num_addrs {
            ...
            let funai = tuple
                .get("funai")
                .unwrap_or_else(|_| panic!("FATAL: no 'funai' in return value from (get-reward-set-pox-address u{} u{})",
                    reward_cycle, i))
                .to_owned()
                .expect_optional()
                .expect("FATAL: unexpected PoX structure")
    ...
    /// TODO: also return their funai signer keys (as part of `RawRewardSetEntry`
    ...
    /// lost in the conversion to FunaiAddress)
    ...
    pub fn get_reward_addresses_in_cycle(
        ...
        // there hasn't yet been a Funai block.
        match result {
...
pub mod test {
    ...
    fn make_reward_set_units() {
        ...
        let addresses = vec![
            RawRewardSetEntry {
                reward_address: PoxAddress::Standard(
                    FunaiAddress::from_string("STVK1K405H6SK9NKJAP32GHYHDJ98MMNP8Y6Z9N0").unwrap(),
        ...
        assert_eq!(
            FunaiChainState::make_reward_set(threshold, addresses, FunaiEpochId::Epoch2_05)
                .rewarded_addresses
                .len(),
            3
        );
    ...
    fn get_reward_threshold_units() {
        ...
        assert_eq!(
            FunaiChainState::get_reward_threshold_and_participation(
                &test_pox_constants,
                &[],
                liquid,
            )
    ...
    fn rand_addr() -> FunaiAddress {
        key_to_funai_addr(&FunaiPrivateKey::new())
    }

    pub fn key_to_funai_addr(key: &FunaiPrivateKey) -> FunaiAddress {
        FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(key)],
        )
        .unwrap()
    }
    ...
    pub fn instantiate_pox_peer<'a>(
        burnchain: &Burnchain,
        test_name: &str,
    ) -> (TestPeer<'a>, Vec<FunaiPrivateKey>) {
    ...
    pub fn instantiate_pox_peer_with_epoch<'a>(
        ...
        let addrs: Vec<FunaiAddress> = keys.iter().map(|pk| key_to_funai_addr(pk)).collect();
    ...
    pub fn eval_at_tip(peer: &mut TestPeer, boot_contract: &str, expr: &str) -> Value {
        ...
        let funai_block_id = FunaiBlockId::new(&consensus_hash, &block_bhh);
        let iconn = sortdb.index_conn();
    ...
    fn contract_id(addr: &FunaiAddress, name: &str) -> QualifiedContractIdentifier {
        QualifiedContractIdentifier::new(
            StandardPrincipalData::from(addr.clone()),
            ContractName::try_from(name.to_string()).unwrap(),
        )
    ...
    pub fn get_funai_info(
        ...
        let value_opt = eval_at_tip(
            peer,
            "pox",
            &format!("(get-funai-info '{})", addr.to_string()),
        );
    ...
    where
        F: FnOnce(&mut FunaiChainState, &SortitionDB) -> R,
    {
        let sortdb = peer.sortdb.take().unwrap();
    ...
    pub fn get_account(peer: &mut TestPeer, addr: &PrincipalData) -> FunaiAccount {
    ...
    fn get_contract(peer: &mut TestPeer, addr: &QualifiedContractIdentifier) -> Option<Contract> {
        let contract_opt = with_sortdb(peer, |ref mut chainstate, ref mut sortdb| {
            ...
            chainstate
                .with_read_only_clarity_tx(&sortdb.index_conn(), &funai_block_id, |clarity_tx| {
                    FunaiChainState::get_contract(clarity_tx, addr).unwrap()
                })
                .unwrap()
        });
        contract_opt
    ...
    pub fn make_pox_4_revoke_delegate_stx(key: &FunaiPrivateKey, nonce: u64) -> FunaiTransaction {
        let payload = TransactionPayload::new_contract_call(
            boot_code_test_addr(),
            POX_4_NAME,
            "revoke-delegate-stx",
    ...
    pub fn make_pox_4_set_signer_key_auth(
        ...
        enabled: bool,
        nonce: u64,
        sender_key: Option<&FunaiPrivateKey>,
        max_amount: u128,
        auth_id: u128,
    ) -> FunaiTransaction {
        let signer_pubkey = FunaiPublicKey::from_private(signer_key);
        let payload = TransactionPayload::new_contract_call(
            boot_code_test_addr(),
            POX_4_NAME,
    ...
    fn make_tx(
        ...
        let mut tx = FunaiTransaction::new(TransactionVersion::Testnet, auth, payload);
    ...
    fn make_invalid_pox_lockups(key: &FunaiPrivateKey, mut nonce: u64) -> Vec<FunaiTransaction> {
        let mut ret = vec![];
    ...
    fn make_pox_lockup_contract(
        ...
        (define-public (do-contract-lockup (amount-ustx uint) (pox-addr (tuple (version (buff 1)) (hashbytes (buff 20)))) (lock-period uint))
            ...
            (begin
                ...
                ;; this contract funai the stx given to it
                (as-contract
    ...
    fn make_pox_reject(key: &FunaiPrivateKey, nonce: u64) -> FunaiTransaction {
    ...
    fn test_lockups() {
        ...
        let alice = FunaiAddress::from_string("STVK1K405H6SK9NKJAP32GHYHDJ98MMNP8Y6Z9N0").unwrap();
        let bob = FunaiAddress::from_string("ST76D2FMXZ7D2719PNE4N71KPSX84XCCNCMYC940").unwrap();
    ...
    fn test_hook_special_contract_call() {
        ...
        for tenure_id in 0..num_blocks {
            ...
            let (burn_ops, funai_block, microblocks) = peer.make_tenure(|ref mut miner, ref mut sortdb, ref mut chainstate, vrf_proof, ref parent_opt, ref parent_microblock_header_opt| {
                let parent_tip = get_parent_tip(parent_opt, chainstate, sortdb);
                let coinbase_tx = make_coinbase(miner, tenure_id);
                ...
                if tenure_id == 1 {
                    let alice_lockup_1 = make_pox_lockup(&alice, 0, 512 * POX_THRESHOLD_STEPS_USTX, AddressHashMode::SerializeP2PKH, key_to_funai_addr(&alice).bytes, 1, tip.block_height);
                    block_txs.push(alice_lockup_1);
                }
                if tenure_id == 2 {
                    let alice_test_tx = make_bare_contract(&alice, 1, 0, "nested-funai", &format!(
                ...
                if tenure_id == 8 {
                    ...
                    let cc_payload = TransactionPayload::new_contract_call(key_to_funai_addr(&alice),
                                                                           "nested-funai",
                                                                           "nested-stack-stx",
                                                                           vec![]).unwrap();
                    ...
                    let alice_contract: Value = contract_id(&key_to_funai_addr(&alice), "nested-funai").into();
                ...
                let block_builder = FunaiBlockBuilder::make_regtest_block_builder(&burnchain,
                    &parent_tip, vrf_proof, tip.total_burn, microblock_pubkeyhash).unwrap();
                let (anchored_block, _size, _cost) = FunaiBlockBuilder::make_anchored_block_from_txs(block_builder, chainstate, &sortdb.index_conn(), block_txs).unwrap();
                (anchored_block, vec![])
            });
    ...
    pub fn get_par_burn_block_height(
        ...
        let parent_block_id = FunaiChainState::get_parent_block_id(state.db(), block_id)
            .unwrap()
            .unwrap();
    ...
    fn test_pox_lockup_single_tx_sender() {
        ...
        for tenure_id in 0..num_blocks {
            ...
            let tip_index_block = FunaiBlockId::new(&consensus_hash, &funai_block.block_hash());
            ...
            if tenure_id <= 1 {
                ...
                if cur_reward_cycle >= alice_reward_cycle {
                    ...
                    let (amount_ustx, pox_addr, lock_period, first_reward_cycle) =
                        get_funai_info(&mut peer, &key_to_funai_addr(&alice).into()).unwrap();
                    ...
                    // Lock-up is consistent with funai state
                    let alice_account = get_account(&mut peer, &key_to_funai_addr(&alice).into());
            ...
            let (burn_ops, funai_block, microblocks) = peer.make_tenure(
                |ref mut miner,
                 ...
                 ref parent_microblock_header_opt| {
                    ...
                    let block_builder = FunaiBlockBuilder::make_block_builder(
                        &burnchain,
                        false,
                        &parent_tip,
                        vrf_proof,
            ...
            if tenure_id <= 1 {
                if tenure_id < 1 {
                    ...
                    for key in keys.iter() {
                        ...
                        let balance = get_balance(&mut peer, &key_to_funai_addr(&key).into());
                        assert_eq!(balance, 1024 * POX_THRESHOLD_STEPS_USTX);

                        let account = get_account(&mut peer, &key_to_funai_addr(&key).into());
                        assert_eq!(
                ...
                if cur_reward_cycle >= lockup_reward_cycle {
                    ...
                    for key in keys.iter() {
                        all_addrbytes.insert(key_to_funai_addr(&key).bytes);
                    }

                    for key in keys.iter() {
                        let (amount_ustx, pox_addr, lock_period, first_reward_cycle) =
                            get_funai_info(&mut peer, &key_to_funai_addr(&key).into()).unwrap();
                        ...
                        assert!(all_addrbytes.contains(&key_to_funai_addr(&key).bytes));
                        all_addrbytes.remove(&key_to_funai_addr(&key).bytes);
                        assert_eq!(reward_addrs[0].1, 1024 * POX_THRESHOLD_STEPS_USTX);
            ...
            if tenure_id <= 1 {
                ...
                if cur_reward_cycle >= alice_reward_cycle {
                    ...
                    if cur_reward_cycle == alice_reward_cycle {
                        ...
                        // Alice is _not_ a funai -- Bob's contract is!
                        let alice_info =
                            get_funai_info(&mut peer, &key_to_funai_addr(&alice).into());
                        assert!(alice_info.is_none());

                        // Bob is _not_ a funai either.
                        let bob_info =
                            get_funai_info(&mut peer, &key_to_funai_addr(&alice).into());
                        assert!(bob_info.is_none());

                        // Bob's contract is a funai
                        let (amount_ustx, pox_addr, lock_period, first_reward_cycle) =
                            get_funai_info(
                                &mut peer,
                                &make_contract_id(&key_to_funai_addr(&bob), "do-lockup").into(),
                            )
                            .unwrap();
        ...
        for tenure_id in 0..num_blocks {
            ...
            let (burn_ops, funai_block, microblocks) = peer.make_tenure(|ref mut miner, ref mut sortdb, ref mut chainstate, vrf_proof, ref parent_opt, ref parent_microblock_header_opt| {
                ...
                if tenure_id == 1 {
                    ...
                    let alice_lockup_1 = make_pox_lockup(&alice, 0, 512 * POX_THRESHOLD_STEPS_USTX, AddressHashMode::SerializeP2PKH, key_to_funai_addr(&alice).bytes, 12, tip.block_height);
                    block_txs.push(alice_lockup_1);

                    // will be rejected
                    let alice_lockup_2 = make_pox_lockup(&alice, 1, 512 * POX_THRESHOLD_STEPS_USTX, AddressHashMode::SerializeP2PKH, key_to_funai_addr(&alice).bytes, 12, tip.block_height);
                    ...
                    let alice_test = contract_id(&key_to_funai_addr(&alice), "alice-test").into();
                    let alice_allowance = make_pox_contract_call(&alice, 2, "allow-contract-caller", vec![alice_test, Value::none()]);

                    let bob_test = contract_id(&key_to_funai_addr(&bob), "bob-test").into();
                    let bob_allowance = make_pox_contract_call(&bob, 0, "allow-contract-caller", vec![bob_test, Value::none()]);

                    let charlie_test = contract_id(&key_to_funai_addr(&charlie), "charlie-test").into();
            ...
            if tenure_id <= 1 {
                ...
                if cur_reward_cycle >= alice_reward_cycle {
                    ...
                    if cur_reward_cycle == alice_reward_cycle {
                        ...
                        // Lock-up is consistent with funai state
                        let alice_account =
                            get_account(&mut peer, &key_to_funai_addr(&alice).into());
                        assert_eq!(alice_account.stx_balance.amount_unlocked(), 0);
                        assert_eq!(
                            alice_account.stx_balance.amount_locked(),
                            1024 * POX_THRESHOLD_STEPS_USTX
                        ...
                        // alice shouldn't be a funai
                        let info = get_funai_info(&mut peer, &key_to_funai_addr(&alice).into());
                        assert!(
                            get_funai_info(&mut peer, &key_to_funai_addr(&alice).into())
                                .is_none()
                        );
    ...
    fn test_pox_lockup_unlock_relock() {
        ...
        for tenure_id in 0..num_blocks {
                 ...
                 ref parent_microblock_header_opt| {
                    ...
                    if tenure_id == 1 {
                        ...
                        let charlie_stack = make_pox_lockup_contract_call(
                            ...
                            1024 * POX_THRESHOLD_STEPS_USTX,
                            AddressHashMode::SerializeP2PKH,
                            key_to_funai_addr(&charlie).bytes,
                            1,
                        );
            ...
            let charlie_balance = get_balance(&mut peer, &key_to_funai_addr(&charlie).into());
            ...
            if first_reward_cycle > 0 && second_reward_cycle == 0 {
                if cur_reward_cycle == first_reward_cycle {
                    ...
                    // alice shouldn't be a funai
                    assert!(
                        get_funai_info(&mut peer, &key_to_funai_addr(&alice).into()).is_none()
                    );
                    assert!(get_funai_info(
                        &mut peer,
                        &make_contract_id(&key_to_funai_addr(&bob), "do-lockup").into(),
                    )
                    .is_none());
                ...
                if cur_reward_cycle == second_reward_cycle {
                    ...
                    // alice and charlie shouldn't be funais
                    assert!(
                        get_funai_info(&mut peer, &key_to_funai_addr(&alice).into()).is_none()
                    );
                    assert!(get_funai_info(
    ...
    fn test_pox_lockup_unlock_on_spend() {
        ...
        for tenure_id in 0..num_blocks {
                 ...
                 ref parent_microblock_header_opt| {
                    ...
                    if tenure_id == 1 {
                        ...
                        let danielle_lockup = make_pox_lockup(
                            ...
                            1024 * POX_THRESHOLD_STEPS_USTX,
                            AddressHashMode::SerializeP2PKH,
                            key_to_funai_addr(&danielle).bytes,
                            1,
                            tip.block_height,
                        );
                        ...
                        let bob_tx =
                            make_token_transfer(&bob, 2, 0, key_to_funai_addr(&alice).into(), 1);
            ...
            let funai_addrs: Vec<PrincipalData> = vec![
                key_to_funai_addr(&alice).into(),
                key_to_funai_addr(&bob).into(),
                key_to_funai_addr(&charlie).into(),
                key_to_funai_addr(&danielle).into(),
                make_contract_id(&key_to_funai_addr(&bob), "do-lockup").into(),
            ];
            ...
            let balances: Vec<u128> = funai_addrs
                .iter()
            ...
            if tenure_id <= 1 {
                ...
                assert!(get_contract(
                    &mut peer,
                    &make_contract_id(&key_to_funai_addr(&alice), "alice-try-spend").into(),
                )
                .is_none());
            }
            ...
            if reward_cycle > 0 {
                if cur_reward_cycle == reward_cycle {
                    ...
                    // all funais are present
                    for addr in funai_addrs.iter() {
                        let (amount_ustx, pox_addr, lock_period, pox_reward_cycle) =
                            get_funai_info(&mut peer, addr).unwrap();
                    ...
                    for (addr, expected_balance) in
                        funai_addrs.iter().zip(balances_stacked.iter())
                    {
                    ...
                    // no one's a funai
                    for addr in funai_addrs.iter() {
                        assert!(get_funai_info(&mut peer, addr).is_none());
                    }
            ...
            if tenure_id >= 11 {
                ...
                for (addr, expected_balance) in
                    funai_addrs.iter().zip(balances_after_spending.iter())
                {
                    let account = get_account(&mut peer, addr);
                ...
                for (addr, (expected_locked, expected_balance)) in funai_addrs
                    .iter()
    ...
    fn test_pox_lockup_reject() {
        ...
        for tenure_id in 0..num_blocks {
            ...
            let (burn_ops, funai_block, microblocks) = peer.make_tenure(|ref mut miner, ref mut sortdb, ref mut chainstate, vrf_proof, ref parent_opt, ref parent_microblock_header_opt| {
                ...
                if tenure_id == 1 {
                    // Alice locks up exactly 25% of the liquid STX supply, so this should succeed.
                    let alice_lockup = make_pox_lockup(&alice, 0, 1024 * POX_THRESHOLD_STEPS_USTX, AddressHashMode::SerializeP2PKH, key_to_funai_addr(&alice).bytes, 12, tip.block_height);
                    block_txs.push(alice_lockup);
                    ...
                    // this _should_ be included in the block
                    let charlie_contract: Value = contract_id(&key_to_funai_addr(&charlie), "charlie-try-stack").into();
                ...
                let block_builder = FunaiBlockBuilder::make_regtest_block_builder(&burnchain, &parent_tip, vrf_proof, tip.total_burn, microblock_pubkeyhash).unwrap();
                let (anchored_block, _size, _cost) = FunaiBlockBuilder::make_anchored_block_from_txs(block_builder, chainstate, &sortdb.index_conn(), block_txs).unwrap();
