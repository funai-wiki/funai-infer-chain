...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, SortitionId, FunaiAddress, FunaiBlockId, VRFSeed,
};
use funai_common::util::hash::{to_hex, Sha256Sum, Sha512Trunc256Sum};
...
use crate::chainstate::funai::db::{MinerPaymentSchedule, FunaiHeaderInfo};
...
use crate::core::{
    FunaiEpoch, FunaiEpochId, BITCOIN_REGTEST_FIRST_BLOCK_HASH,
...
lazy_static! {
    pub static ref FIRST_INDEX_BLOCK_HASH: FunaiBlockId = FunaiBlockHeader::make_index_block_hash(
    ...
    pub static ref USER_KEYS: Vec<FunaiPrivateKey> =
        (0..50).map(|_| FunaiPrivateKey::new()).collect();
    pub static ref POX_ADDRS: Vec<Value> = (0..50u64)
        ...
        .collect();
    pub static ref MINER_KEY: FunaiPrivateKey = FunaiPrivateKey::new();
    pub static ref MINER_ADDR: FunaiAddress = FunaiAddress::from_public_keys(
        C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
        &AddressHashMode::SerializeP2PKH,
        1,
        &vec![FunaiPublicKey::from_private(&MINER_KEY.clone())],
    )
    .unwrap();
...
impl ClarityTestSim {
    pub fn new() -> ClarityTestSim {
        let mut marf = MarfedKV::temporary();
        {
            let mut store = marf.begin(
                ...
                &FunaiBlockId(test_sim_height_to_hash(0, 0)),
            );
        ...
        F: FnOnce(&mut ClarityBlockConnection) -> R,
    {
        let r = {
            let mut store = self.marf.begin(
                &FunaiBlockId(test_sim_height_to_hash(self.height, self.fork)),
                &FunaiBlockId(test_sim_height_to_hash(self.height + 1, self.fork)),
            );
    ...
    fn check_and_bump_epoch(
        ...
        let sortition_epoch = clarity_db
            .get_funai_epoch(headers_db.height as u32)
            .unwrap()
            .epoch_id;
    ...
    {
        let mut store = self.marf.begin(
            &FunaiBlockId(test_sim_height_to_hash(parent_height, self.fork)),
            &FunaiBlockId(test_sim_height_to_hash(parent_height + 1, self.fork + 1)),
        );
...
fn check_arithmetic_only(contract: &str, version: ClarityVersion) {
    let analysis = mem_type_check(contract, version, FunaiEpochId::latest())
        .unwrap()
        .1;
...
impl BurnStateDB for TestSimBurnStateDB {
    ...
    fn get_sortition_id_from_consensus_hash(
        ...
        // consensus hashes are constructed as the leading 20 bytes of the funai block ID from
        // whence it came.
    ...
    fn get_funai_epoch(&self, height: u32) -> Option<FunaiEpoch> {
        ...
        let epoch_id = match epoch_begin_index {
            0 => FunaiEpochId::Epoch20,
            1 => FunaiEpochId::Epoch2_05,
            2 => FunaiEpochId::Epoch21,
            3 => FunaiEpochId::Epoch22,
            4 => FunaiEpochId::Epoch23,
...
impl HeadersDB for TestSimHeadersDB {
    ...
    fn get_consensus_hash_for_block(&self, bhh: &FunaiBlockId) -> Option<ConsensusHash> {
...
fn pox_2_contract_caller_units() {
    ...
    let delegator = FunaiPrivateKey::new();
    ...
    sim.execute_next_block(|env| {
        ...
        assert_eq!(
            ...
            .0,
            execute(&format!(
                "(ok {{ funai: '{}, lock-amount: {}, unlock-burn-height: {} }})",
    ...
    sim.execute_next_block(|env| {
        ...
        for cycle in 0..20 {
            ...
            if !empty_set {
                ...
                let expected_funai = Value::from(&USER_KEYS[1]);
                assert_eq!(
                    env.eval_read_only(
                        &POX_2_CONTRACT_TESTNET,
                        ...
                        .0,
                    execute(&format!(
                        "{{ pox-addr: {}, total-ustx: u{}, funai: (some '{}) }}",
                        expected_pox_addr,
                        1_000_000,
                        &expected_funai,
                    ))
    ...
    sim.execute_next_block(|env| {
        ...
        assert_eq!(
            env.execute_transaction(
                ...
                .to_string(),
            "(err 11)".to_string(),
            "At cycle 12, delegate cannot aggregate commit because only one funai was extended by 10"
        );
            ...
            if !empty_set {
                ...
                assert_eq!(
                    env.eval_read_only(
                        ...
                        .0,
                    execute(&format!(
                        "{{ pox-addr: {}, total-ustx: u{}, funai: none }}",
                        expected_pox_addr,
                        MIN_THRESHOLD.deref(),
                    ))
                );
...
#[test]
// test that the maximum funaidb list size will fit in a value
fn max_funaidb_list() {
    let signers_list: Vec<_> = (0..SIGNERS_MAX_LIST_SIZE)
        .into_iter()
        .map(|signer_ix| {
            let signer_address = FunaiAddress {
                version: 0,
...
fn delegation_tests() {
    ...
    sim.execute_next_block(|env| {
        ...
        // 1. we already locked user[0] up for round 2, so let's add some more funai for round 2 from
        //    user[3]. in the process, this will add more funai for lockup in round 1, so lets commit
        //    that as well.
...
fn test_vote_fail() {
    ...
    sim.execute_next_block(|env| {
        ...
        // Commit all liquid funai to vote
