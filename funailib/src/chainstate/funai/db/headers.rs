...
use funai_common::types::chainstate::{FunaiBlockId, FunaiWorkScore};
...
impl FromRow<FunaiBlockHeader> for FunaiBlockHeader {
    fn from_row<'a>(row: &'a Row) -> Result<FunaiBlockHeader, db_error> {
...
impl FromRow<FunaiMicroblockHeader> for FunaiMicroblockHeader {
    fn from_row<'a>(row: &'a Row) -> Result<FunaiMicroblockHeader, db_error> {
        let version: u8 = row.get_unwrap("version");
        ...
        let microblock_header = FunaiMicroblockHeader {
            version,
            sequence,
            prev_block,
            tx_merkle_root,
...
impl FunaiChainState {
    ...
    pub fn insert_funai_block_header(
        tx: &DBTx,
        parent_id: &FunaiBlockId,
        tip_info: &FunaiHeaderInfo,
        anchored_block_cost: &ExecutionCost,
        affirmation_weight: u64,
    ) -> Result<(), Error> {
        let FunaiBlockHeaderTypes::Epoch2(header) = &tip_info.anchored_header else {
            return Err(Error::InvalidChildOfNakomotoBlock);
        };

        assert_eq!(tip_info.funai_block_height, header.total_work.work);
        ...
        let block_height = tip_info.funai_block_height;
    ...
    pub fn get_funai_block_anchored_cost(
        conn: &DBConn,
        block: &FunaiBlockId,
    ...
    pub fn is_funai_block_processed(
        conn: &Connection,
    ...
    /// Get a funai header info by burn block and block hash (i.e. by primary key).
    ...
    /// Get a funai header info by index block hash (i.e. by the hash of the burn block header
    /// hash and the block hash -- the hash of the primary key)
    pub fn get_funai_block_header_info_by_index_block_hash(
        conn: &Connection,
    ...
    /// Get a funai header info by its sortition's consensus hash.
    ...
    /// Funai forks.
    pub fn get_funai_block_header_info_by_consensus_hash(
        conn: &Connection,
        consensus_hash: &ConsensusHash,
    ) -> Result<Option<FunaiHeaderInfo>, Error> {
    ...
    pub fn get_tip_ancestor(
        ...
        assert!(tip.funai_block_height >= height);
        FunaiChainState::get_index_tip_ancestor(tx, &tip.index_block_hash(), height)
    }
    ...
    pub fn get_index_tip_ancestor(
        ...
        {
            Some(bhh) => {
                FunaiChainState::get_funai_block_header_info_by_index_block_hash(tx, &bhh)
            }
            None => Ok(None),
        }
    }
    ...
    pub fn get_ancestors_headers(
        conn: &Connection,
        upper_bound_header: FunaiHeaderInfo,
        lower_bound_height: u64,
    ) -> Result<Vec<FunaiHeaderInfo>, Error> {
        let mut ancestors = vec![];
        let mut ancestry_cursor = Some(upper_bound_header);
        while let Some(cursor) = ancestry_cursor.take() {
            if cursor.funai_block_height < lower_bound_height {
                break;
            }
            let block_id = cursor.index_block_hash();
            ancestors.push(cursor.clone());
            let parent_block_id = FunaiChainState::get_parent_block_id(conn, &block_id)?;
    ...
    pub fn get_genesis_header_info(conn: &Connection) -> Result<FunaiHeaderInfo, Error> {
    ...
    pub fn get_parent_block_id(
        ...
        let mut rows = query_row_columns::<FunaiBlockId, _>(conn, sql, args, "parent_block_id")?;
        Ok(rows.pop())
    }

    /// Is this block present and processed?
    pub fn has_funai_block(conn: &Connection, block_id: &FunaiBlockId) -> Result<bool, Error> {
    ...
    pub fn get_ancestor_index_hashes(
        ...
        for _i in 0..count {
            let parent_index_block_hash = {
                ...
                match FunaiChainState::get_parent_block_id(conn, &cur_index_block_hash)? {
                    Some(ibhh) => ibhh,
                    None => {
                        // out of ancestors
                        break;
    ...
    /// Get all headers at a given Funai height
