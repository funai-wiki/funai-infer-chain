...
pub type UnconfirmedTxMap = HashMap<Txid, (FunaiTransaction, BlockHeaderHash, u16)>;
...
pub struct ProcessedUnconfirmedState {
    ...
    // for that microblock
    pub receipts: Vec<(u16, FunaiMicroblockHeader, Vec<FunaiTransactionReceipt>)>,
...
pub struct UnconfirmedState {
    pub confirmed_chain_tip: FunaiBlockId,
    pub unconfirmed_chain_tip: FunaiBlockId,
    pub clarity_inst: ClarityInstance,
    pub mined_txs: UnconfirmedTxMap,
    pub cost_so_far: ExecutionCost,
    pub bytes_so_far: u64,

    pub last_mblock: Option<FunaiMicroblockHeader>,
...
impl UnconfirmedState {
    ...
    fn new(chainstate: &FunaiChainState, tip: FunaiBlockId) -> Result<UnconfirmedState, Error> {
        ...
        let cost_so_far = FunaiChainState::get_funai_block_anchored_cost(chainstate.db(), &tip)?
    ...
    fn append_microblocks(
        ...
        if mblocks.len() > 0 {
            ...
            let mut clarity_tx = FunaiChainState::chainstate_begin_unconfirmed(
                db_config,
                &headers_db_conn,
            ...
            for mblock in mblocks.into_iter() {
                ...
                let (stx_fees, stx_burns, receipts) =
                    match FunaiChainState::process_microblocks_transactions(
                        ...
                        Err((e, _)) => {
                            // absorb the error
                            warn!("Encountered invalid funai microblock: {}", &e);
                            break;
                        }
                    };
    ...
    /// Load up the Funai microblock stream to process, composed of only the new microblocks
    fn load_child_microblocks(
        ...
        FunaiChainState::load_descendant_staging_microblock_stream(
            &chainstate.db(),
            &FunaiBlockId::new(&consensus_hash, &anchored_block_hash),
            0,
            u16::MAX,
        )
    }
    ...
    pub fn get_unconfirmed_transaction(
        &self,
        txid: &Txid,
    ) -> Option<(FunaiTransaction, BlockHeaderHash, u16)> {
    ...
    pub fn get_unconfirmed_state_if_exists(&mut self) -> Result<Option<FunaiBlockId>, String> {
        if self.is_readable() {
                ...
                .trie_exists_for_block(&self.unconfirmed_chain_tip)
            {
                ...
                Err(e) => {
                    let err_str = format!(
                        "Failed to load Funai chain tip; error checking underlying trie: {}",
                        e
                    );
                    warn!("{}", err_str);
                    return Err(err_str);
...
impl FunaiChainState {
    ...
    fn make_unconfirmed_state(
        &self,
        burn_dbconn: &dyn BurnStateDB,
        anchored_block_id: FunaiBlockId,
...
mod test {
    ...
    fn test_unconfirmed_refresh_one_microblock_stx_transfer() {
        ...
        for tenure_id in 0..num_blocks {
            ...
            let (burn_ops, funai_block, _) = peer.make_tenure(
                |ref mut miner,
                 ref mut sortdb,
                 ref mut chainstate,
                 vrf_proof,
            ...
            peer.process_funai_epoch_at_tip(&funai_block, &vec![]);

            let canonical_tip = FunaiBlockId::new(&consensus_hash, &funai_block.block_hash());
            ...
            let (canonical_burn, canonical_block) =
                SortitionDB::get_canonical_funai_chain_tip_hash(peer.sortdb().conn()).unwrap();
            ...
            eprintln!("\nrecv_balance: {}\nconfirmed_recv_balance: {}\nblock header {}: {:?}\ntip: {}/{}\n", recv_balance.amount_unlocked(), confirmed_recv_balance.amount_unlocked(), &funai_block.block_hash(), &funai_block.header, &canonical_burn, &canonical_block);
        }
    }
    ...
    fn test_unconfirmed_refresh_invalid_microblock() {
        ...
        for tenure_id in 0..num_blocks {
            ...
            let mut inner_node = peer.funai_node.take().unwrap();
            ...
            for i in 0..num_microblocks {
                ...
                let microblock = {
                    let sort_iconn = sortdb.index_conn();
                    let mut microblock_builder = FunaiMicroblockBuilder::resume_unconfirmed(
            ...
            peer.sortdb = Some(sortdb);
            peer.funai_node = Some(inner_node);
        }

        let (consensus_hash, canonical_block) =
            SortitionDB::get_canonical_funai_chain_tip_hash(peer.sortdb().conn()).unwrap();
        let canonical_tip =
            FunaiBlockHeader::make_index_block_hash(&consensus_hash, &canonical_block);
