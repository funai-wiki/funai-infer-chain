...
use crate::clarity::vm::types::FunaiAddressExtensions;
...
impl FromRow<MinerPaymentSchedule> for MinerPaymentSchedule {
    fn from_row(row: &Row) -> Result<MinerPaymentSchedule, db_error> {
        let address = FunaiAddress::from_column(row, "address")?;
...
impl MinerReward {
    ...
    pub fn genesis(mainnet: bool) -> MinerReward {
        MinerReward {
            address: FunaiAddress::burn_address(mainnet),
            recipient: FunaiAddress::burn_address(mainnet).to_account_principal(),
            coinbase: 0,
            tx_fees_anchored: 0,
...
impl FunaiChainState {
    ...
    pub fn insert_miner_payment_schedule(
        ...
        let index_block_hash =
            FunaiBlockId::new(&block_reward.consensus_hash, &block_reward.block_hash);
    ...
    fn inner_insert_matured_miner_reward<'a>(
        ...
        // the only time it's okay to re-insert the same reward is if there are two Funai forks
        // trying to store the same matured rewards for a common ancestor block.
        let cur_rewards = FunaiChainState::inner_get_matured_miner_payments(
            tx,
            parent_block_id,
            child_block_id,
        )?;
    ...
    pub fn insert_matured_parent_miner_reward<'a>(
        ...
        FunaiChainState::inner_insert_matured_miner_reward(
            tx,
            parent_block_id,
            child_block_id,
            parent_reward,
    ...
    pub fn get_poison_microblock_report<T: ClarityConnection>(
        clarity_tx: &mut T,
        height: u64,
    ) -> Result<Option<(FunaiAddress, u16)>, Error> {
    ...
    /// Get the scheduled miner rewards in a particular Funai fork at a particular height.
    pub fn get_scheduled_block_rewards_in_fork_at_height<'a>(
        tx: &mut FunaiDBTx<'a>,
        tip: &FunaiHeaderInfo,
        block_height: u64,
    ) -> Result<Vec<MinerPaymentSchedule>, Error> {
        let ancestor_info = match FunaiChainState::get_tip_ancestor(tx, tip, block_height)? {
            Some(info) => info,
            None => {
    ...
    pub fn get_scheduled_block_rewards(
        ...
        if tip.funai_block_height < MINER_REWARD_MATURITY {
            return Ok(vec![]);
        }

        let block_height = tip.funai_block_height - MINER_REWARD_MATURITY;
        FunaiChainState::get_scheduled_block_rewards_in_fork_at_height(tx, tip, block_height)
    }

    /// Get the miner info at a particular burn/funai block
    pub fn get_miner_info(
        ...
        match len {
            ...
            _ => {
                panic!(
                    "Multiple miners for {}/{}",
                    consensus_hash, funai_block_hash
                );
            }
        }
    }
            ...
            if let Some(reporter_address) = poison_reporter_opt {
                if participant.miner {
                    ...
                    debug!(
                        ...
                        FunaiChainState::poison_microblock_commission(coinbase_reward)
                    );
                    (
                    ...
                    (
                        ...
                        FunaiAddress::burn_address(mainnet).to_account_principal(),
                        0,
                        false,
                    )
                }
            ...
            if participant.miner {
                ...
                match participant.tx_fees {
                    MinerPaymentTxFees::Epoch2 {
                        ...
                        let parent_streamed_fees = if parent_block_epoch < FunaiEpochId::Epoch21 {
    ...
    pub fn find_mature_miner_rewards(
        ...
        if tip_funai_height <= MINER_REWARD_MATURITY {
            // no mature rewards exist
            return Ok(None);
        }

        let reward_height = tip_funai_height - MINER_REWARD_MATURITY;
        ...
        let reward_info = MinerRewardInfo {
            from_funai_block_hash: miner.block_hash.clone(),
            from_block_consensus_hash: miner.consensus_hash.clone(),
            from_parent_funai_block_hash: parent_miner.block_hash.clone(),
        ...
        let parent_evaluated_epoch =
            SortitionDB::get_funai_epoch(sortdb_conn, parent_evaluated_snapshot.block_height)?
        ...
        let poison_recipient_opt =
            FunaiChainState::get_poison_microblock_report(clarity_tx, reward_height)?
        ...
        let (parent_miner_reward, miner_reward) = FunaiChainState::calculate_miner_reward(
            mainnet,
        ...
        for user_reward in users.iter() {
            let (parent_reward, reward) = FunaiChainState::calculate_miner_reward(
                mainnet,
                parent_evaluated_epoch.epoch_id,
                user_reward,
                &miner,
...
mod test {
    ...
    fn advance_tip(
        ...
        parent_header_info: &FunaiHeaderInfo,
        block_reward: &mut MinerPaymentSchedule,
    ) -> FunaiHeaderInfo {
        let mut new_tip = parent_header_info.clone();

        let mut anchored_header = new_tip.anchored_header.as_funai_epoch2().unwrap().clone();
        ...
        new_tip.funai_block_height = parent_header_info.funai_block_height + 1;
        ...
        let tip = FunaiChainState::advance_tip(
            &mut tx,
            parent_header_info
                .anchored_header
                .as_funai_epoch2()
            ...
            new_tip.anchored_header.as_funai_epoch2().unwrap(),
    ...
    fn get_tip_ancestor() {
        ...
        let miner_1 =
            FunaiAddress::from_string(&"SP1A2K3ENNA6QQ7G8DVJXM24T6QMBDVS7D0TRTAR5".to_string())
                .unwrap();
        let user_1 =
            FunaiAddress::from_string(&"SP2837ZMC89J40K4YTS64B00M7065C6X46JX6ARG0".to_string())
                .unwrap();
        ...
        {
            let mut tx = chainstate.index_tx_begin().unwrap();
            let ancestor_0 = FunaiChainState::get_tip_ancestor(
                &mut tx,
                &FunaiHeaderInfo::regtest_genesis(),
                0,
            )
            .unwrap();
        ...
        {
            let mut tx = chainstate.index_tx_begin().unwrap();
            let ancestor_0 = FunaiChainState::get_tip_ancestor(&mut tx, &parent_tip, 0).unwrap();
            let ancestor_1 = FunaiChainState::get_tip_ancestor(&mut tx, &parent_tip, 1).unwrap();

            assert!(ancestor_1.is_some());
            assert!(ancestor_0.is_some());
            assert_eq!(ancestor_0.unwrap().funai_block_height, 0); // block 0 is the boot block
            assert_eq!(ancestor_1.unwrap().funai_block_height, 1);
        }
        ...
        {
            let mut tx = chainstate.index_tx_begin().unwrap();
            let ancestor_2 = FunaiChainState::get_tip_ancestor(&mut tx, &tip, 2).unwrap();
            let ancestor_1 = FunaiChainState::get_tip_ancestor(&mut tx, &tip, 1).unwrap();
            let ancestor_0 = FunaiChainState::get_tip_ancestor(&mut tx, &tip, 0).unwrap();

            assert!(ancestor_2.is_some());
            assert_eq!(ancestor_2.unwrap().funai_block_height, 2);
            assert!(ancestor_1.is_some());
            assert_eq!(ancestor_1.unwrap().funai_block_height, 1);
            assert!(ancestor_0.is_some());
            assert_eq!(ancestor_0.unwrap().funai_block_height, 0); // block 0 is the boot block
    ...
    fn load_store_miner_payment_schedule() {
        ...
        let initial_tip = FunaiHeaderInfo::regtest_genesis();
...

        {
            let mut tx = chainstate.index_tx_begin().unwrap();
            let payments_0 =
                FunaiChainState::get_scheduled_block_rewards_in_fork_at_height(&mut tx, &tip, 0)
                    .unwrap();
            let payments_1 =
                FunaiChainState::get_scheduled_block_rewards_in_fork_at_height(&mut tx, &tip, 1)
                    .unwrap();
            let payments_2 =
                FunaiChainState::get_scheduled_block_rewards_in_fork_at_height(&mut tx, &tip, 2)
                    .unwrap();
    ...
    fn miner_reward_one_miner_no_tx_fees_no_users() {
        ...
        let (parent_reward, miner_reward) = FunaiChainState::calculate_miner_reward(
            false,
    ...
    fn miner_reward_one_miner_one_user_no_tx_fees() {
        ...
        let (parent_miner_1, reward_miner_1) = FunaiChainState::calculate_miner_reward(
            false,
            FunaiEpochId::Epoch2_05,
            &miner,
            &miner,
        ...
        let (parent_user_1, reward_user_1) = FunaiChainState::calculate_miner_reward(
            false,
            FunaiEpochId::Epoch2_05,
            &user,
            &miner,
    ...
    fn miner_reward_tx_fees() {
        ...
        let parent_miner_1 =
            FunaiAddress::from_string(&"SP2QDF700V0FWXVNQJJ4XFGBWE6R2Y4APTSFQNBVE".to_string())
                .unwrap();
