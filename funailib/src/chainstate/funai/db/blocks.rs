...
use clarity::vm::types::{
    ...
    FunaiAddressExtensions as ClarityFunaiAddressExtensions, StandardPrincipalData, TupleData,
    TypeSignature, Value,
};
...
use funai_common::codec::{read_next, write_next, MAX_MESSAGE_LEN};
use funai_common::types::chainstate::{
    BurnchainHeaderHash, SortitionId, FunaiAddress, FunaiBlockId,
};
use funai_common::util::hash::to_hex;
use funai_common::util::retry::BoundReader;
use funai_common::util::{get_epoch_time_ms, get_epoch_time_secs};
...
use crate::chainstate::funai::address::{PoxAddress, FunaiAddressExtensions};
use crate::chainstate::funai::db::accounts::MinerReward;
use crate::chainstate::funai::db::transactions::TransactionNonceMismatch;
use crate::chainstate::funai::db::*;
use crate::chainstate::funai::events::FunaiBlockEventData;
use crate::chainstate::funai::index::MarfTrieId;
...
use crate::util_lib::strings::FunaiString;
...
pub struct SetupBlockResult<'a, 'b> {
    ...
    pub microblock_txs_receipts: Vec<FunaiTransactionReceipt>,
    ...
    pub auto_unlock_events: Vec<FunaiTransactionEvent>,
...
impl BlockEventDispatcher for DummyEventDispatcher {
    fn announce_block(
        &self,
        _block: &FunaiBlockEventData,
        _metadata: &FunaiHeaderInfo,
        _receipts: &[FunaiTransactionReceipt],
        _parent: &FunaiBlockId,
        _winner_txid: Txid,
...
impl StagingMicroblock {
    #[cfg(test)]
    pub fn try_into_microblock(self) -> Result<FunaiMicroblock, StagingMicroblock> {
        FunaiMicroblock::consensus_deserialize(&mut &self.block_data[..]).map_err(|_e| self)
    }
}

impl FunaiChainState {
    fn get_index_block_pathbuf(blocks_dir: &str, index_block_hash: &FunaiBlockId) -> PathBuf {
    ...
    pub fn get_index_block_path(
        ...
        let block_path = FunaiChainState::get_index_block_pathbuf(blocks_dir, index_block_hash);
    ...
    pub fn get_block_path(
        ...
        FunaiChainState::get_index_block_path(blocks_dir, &index_block_hash)
    }
    ...
    fn make_block_dir(
        ...
        let _ = FunaiChainState::mkdirs(&block_path)?;
    ...
    pub fn atomic_file_write(path: &str, bytes: &[u8]) -> Result<(), Error> {
        FunaiChainState::atomic_file_store(path, false, |ref mut fd| {
            fd.write_all(bytes)
                .map_err(|e| Error::DBError(db_error::IOError(e)))
        })
    }
    ...
    pub fn consensus_load<T: FunaiMessageCodec>(path: &str) -> Result<T, Error> {
        let mut fd = fs::OpenOptions::new()
            .read(true)
            .write(false)
            .open(path)
    ...
    pub fn has_block_indexed(
        ...
        let block_path = FunaiChainState::get_index_block_path(blocks_dir, index_block_hash)?;
    ...
    pub fn has_stored_block(
        ...
        let staging_status_opt =
            FunaiChainState::get_staging_block_status(blocks_db, consensus_hash, block_hash)?
                .map(|processed| !processed);

        match staging_status_opt {
            Some(staging_status) => {
                let index_block_hash =
                    FunaiBlockHeader::make_index_block_hash(consensus_hash, block_hash);
                if staging_status {
                    // not processed yet
                    test_debug!(
                        "Block {}/{} ({}) is staging",
                    ...
                    FunaiChainState::has_block_indexed(blocks_dir, &index_block_hash)
                }
            }
            None => {
    ...
    pub fn store_block(
        ...
        let block_path = FunaiChainState::make_block_dir(blocks_dir, consensus_hash, &block_hash)?;
        ...
        FunaiChainState::atomic_file_store(&block_path, true, |ref mut fd| {
    ...
    fn store_empty_block(
        ...
        let block_path =
            FunaiChainState::make_block_dir(blocks_path, consensus_hash, &block_hash)?;
        FunaiChainState::atomic_file_write(&block_path, &vec![])
    }
    ...
    fn free_block(
        ...
        let block_path =
            FunaiChainState::make_block_dir(blocks_dir, consensus_hash, &block_header_hash)
        ...
        if sz > 0 {
            ...
            let index_block_hash = FunaiBlockId::new(consensus_hash, block_header_hash);
            let mut invalid_path =
                FunaiChainState::get_index_block_pathbuf(blocks_dir, &index_block_hash);
            invalid_path
                .file_name()
    ...
    fn free_block_state(
        blocks_path: &str,
        consensus_hash: &ConsensusHash,
        block_header: &FunaiBlockHeader,
    ) -> () {
        FunaiChainState::free_block(blocks_path, consensus_hash, &block_header.block_hash())
    }
    ...
    /// Get all funai block headers.  Great for testing!
    ...
    pub fn list_microblocks(
        ...
        let mut blocks = FunaiChainState::list_blocks(blocks_conn)?;
        let mut ret = vec![];
    ...
    pub fn load_block_bytes(
        ...
        let block_path = FunaiChainState::get_block_path(blocks_dir, consensus_hash, block_hash)?;
        let sz = FunaiChainState::get_file_size(&block_path)?;
        if sz == 0 {
    ...
    pub fn load_block(
        ...
        consensus_hash: &ConsensusHash,
        block_hash: &BlockHeaderHash,
    ) -> Result<Option<FunaiBlock>, Error> {
        let block_path = FunaiChainState::get_block_path(blocks_dir, consensus_hash, block_hash)?;
        let sz = FunaiChainState::get_file_size(&block_path)?;
        if sz == 0 {
            debug!("Zero-sized block {}", &block_hash);
        ...
        let block: FunaiBlock = FunaiChainState::consensus_load(&block_path)?;
        Ok(Some(block))
    }

    fn inner_load_block_header(block_path: &str) -> Result<Option<FunaiBlockHeader>, Error> {
        let sz = FunaiChainState::get_file_size(block_path)?;
        if sz == 0 {
        ...
        let block_header: FunaiBlockHeader = FunaiChainState::consensus_load(block_path)?;
    ...
    pub fn load_block_header(
        ...
        consensus_hash: &ConsensusHash,
        block_hash: &BlockHeaderHash,
    ) -> Result<Option<FunaiBlockHeader>, Error> {
        let block_path = FunaiChainState::get_block_path(blocks_dir, consensus_hash, block_hash)?;
        FunaiChainState::inner_load_block_header(&block_path)
    }
    ...
    fn empty_stream(e: Error) -> Result<Option<Vec<FunaiMicroblock>>, Error> {
        match e {
    ...
    fn inner_load_staging_block_bytes(
        ...
        let args = [&block_hash];
        let mut blobs = FunaiChainState::load_block_data_blobs(block_conn, &sql, &args)?;
        let len = blobs.len();
        match len {
            0 => Ok(None),
            1 => {
    ...
    fn load_staging_microblock_bytes(
        ...
        FunaiChainState::inner_load_staging_block_bytes(
            block_conn,
        ...
        match len {
            ...
            1 => {
                ...
                staging_block.block_data =
                    FunaiChainState::load_block_bytes(blocks_path, consensus_hash, block_hash)?
                        .unwrap_or(vec![]);
    ...
    fn load_staging_block_data(
        ...
        match FunaiChainState::load_staging_block(
            block_conn,
            blocks_path,
            consensus_hash,
            block_hash,
            ...
            Some(staging_block) => {
                ...
                match FunaiBlock::consensus_deserialize(&mut &staging_block.block_data[..]) {
    ...
    fn load_block_pubkey_hash(
        ...
        let pubkey_hash = match FunaiChainState::load_staging_block_pubkey_hash(
            block_conn,
            consensus_hash,
            block_hash,
        )? {
            ...
            None => {
                ...
                let header = match FunaiChainState::load_block_header(
                    block_path,
                    consensus_hash,
                    block_hash,
                )? {
    ...
    pub fn load_staging_microblock_info(
        blocks_conn: &DBConn,
        parent_index_block_hash: &FunaiBlockId,
    ...
    pub fn load_staging_microblock_info_indexed(
        blocks_conn: &DBConn,
        index_microblock_hash: &FunaiBlockId,
    ) -> Result<Option<StagingMicroblock>, Error> {
    ...
    pub fn load_staging_microblock(
        ...
        let parent_index_hash =
            FunaiBlockHeader::make_index_block_hash(parent_consensus_hash, parent_block_hash);
    ...
    pub fn load_staging_microblock_indexed(
        blocks_conn: &DBConn,
        parent_index_hash: &FunaiBlockId,
        microblock_hash: &BlockHeaderHash,
    ) -> Result<Option<StagingMicroblock>, Error> {
        match FunaiChainState::load_staging_microblock_info(
            blocks_conn,
            &parent_index_hash,
            microblock_hash,
        )? {
            Some(mut staging_microblock) => {
                ...
                staging_microblock.block_data =
                    FunaiChainState::load_staging_microblock_bytes(blocks_conn, microblock_hash)?
                        .unwrap_or(vec![]);
    ...
    fn inner_load_microblock_stream_fork(
        ...
        processed_only: bool,
    ) -> Result<Option<Vec<FunaiMicroblock>>, Error> {
        let mut ret = vec![];
        ...
        loop {
            let microblock =
                match FunaiChainState::load_staging_microblock_bytes(blocks_conn, &mblock_hash)? {
                    Some(mblock_data) => {
                        FunaiMicroblock::consensus_deserialize(&mut &mblock_data[..])
                            .unwrap_or_else(|_| {
                                panic!(
            ...
            if processed_only {
                if !FunaiChainState::has_processed_microblocks_indexed(
                    blocks_conn,
    ...
    pub fn load_microblock_stream_fork(
        ...
        FunaiChainState::inner_load_microblock_stream_fork(
            blocks_conn,
            parent_consensus_hash,
            parent_anchored_block_hash,
            tip_microblock_hash,
    ...
    pub fn load_descendant_staging_microblock_stream(
        ...
        let res = FunaiChainState::load_descendant_staging_microblock_stream_with_poison(
            blocks_conn,
    ...
    pub fn load_descendant_staging_microblock_stream_with_poison(
        ...
        start_seq: u16,
        last_seq: u16,
    ) -> Result<Option<(Vec<FunaiMicroblock>, Option<TransactionPayload>)>, Error> {
        ...
        let mut ret: Vec<FunaiMicroblock> = vec![];
        let mut tip: Option<FunaiMicroblock> = None;
        ...
        for i in 0..staging_microblocks.len() {
            let mblock_data = FunaiChainState::load_staging_microblock_bytes(
                blocks_conn,
            ...
            let mblock = match FunaiMicroblock::consensus_deserialize(&mut &mblock_data[..]) {
                Ok(mb) => mb,
                Err(e) => {
    ...
    pub fn load_next_descendant_microblock(
        ...
        parent_index_block_hash: &FunaiBlockId,
        seq: u16,
    ) -> Result<Option<FunaiMicroblock>, Error> {
        FunaiChainState::load_descendant_staging_microblock_stream(
            blocks_conn,
            parent_index_block_hash,
            seq,
            seq,
    ...
    /// funai_block _must_ have been committed, or this will return an error
    pub fn get_parent(&self, funai_block: &FunaiBlockId) -> Result<FunaiBlockId, Error> {
        let sql = "SELECT parent_block_id FROM block_headers WHERE index_block_hash = ?";
        self.db()
            .query_row(sql, &[funai_block], |row| row.get(0))
            .map_err(|e| Error::from(db_error::from(e)))
    }

    /// only used in integration tests with funai-node
    pub fn get_parent_consensus_hash(
        ...
        let sql = "SELECT * FROM snapshots WHERE winning_funai_block_hash = ?1";
    ...
    pub fn load_parent_block_header(
    ...
    ) -> Result<Option<(FunaiBlockHeader, ConsensusHash)>, Error> {
        let header = match FunaiChainState::load_block_header(
            blocks_path,
            consensus_hash,
            anchored_block_hash,
        ...
        for possible_parent in possible_parent_snapshots.into_iter() {
            ...
            if let Some(ancestor) = burn_ancestor {
                // found!
                let ret = FunaiChainState::load_block_header(
                    blocks_path,
                    &ancestor.consensus_hash,
                    &ancestor.winning_funai_block_hash,
                )?
    ...
    fn store_staging_block<'a>(
        ...
        FunaiChainState::store_block(blocks_path, consensus_hash, block)?;
    ...
    pub fn store_staging_microblock<'a>(
        ...
        let index_microblock_hash = FunaiBlockHeader::make_index_block_hash(
            parent_consensus_hash,
            &microblock.block_hash(),
        );
    ...
    pub fn get_staging_block_status(
        ...
        FunaiChainState::read_i64s(blocks_conn, "SELECT processed FROM staging_blocks WHERE anchored_block_hash = ?1 AND consensus_hash = ?2", &[block_hash, consensus_hash])
            .and_then(|processed| {
                if processed.len() == 0 {
                    Ok(None)
                }
    ...
    /// Do we have a given Funai block in any PoX fork or sortition fork?
    ...
    pub fn is_block_orphaned(
        ...
        FunaiChainState::read_i64s(blocks_conn, "SELECT orphaned FROM staging_blocks WHERE anchored_block_hash = ?1 AND consensus_hash = ?2", &[block_hash, consensus_hash])
            .and_then(|orphaned| {
                if orphaned.len() == 0 {
                    Ok(false)
                }
    ...
    pub fn get_microblock_status(
        ...
        FunaiChainState::read_i64s(&self.db(), "SELECT processed FROM staging_microblocks WHERE anchored_block_hash = ?1 AND microblock_hash = ?2 AND consensus_hash = ?3", &[&parent_block_hash, microblock_hash, &parent_consensus_hash])
            .and_then(|processed| {
                if processed.len() == 0 {
                    Ok(None)
                }
    ...
    pub fn has_processed_microblocks(
        &self,
        child_index_block_hash: &FunaiBlockId,
    ) -> Result<bool, Error> {
        let (parent_consensus_hash, parent_block_hash) =
            match FunaiChainState::get_parent_block_header_hashes(
                &self.db(),
        ...
        let parent_microblock_hash =
            match FunaiChainState::get_staging_block_parent_microblock_hash(
                &self.db(),
                child_index_block_hash,
            )? {
                Some(x) => x,
    ...
    pub fn has_processed_microblocks_at_tail(
        ...
        FunaiChainState::read_i64s(self.db(), "SELECT staging_microblocks.processed
    ...
    /// SortitionDB::get_funai_header_hashes().  Note that header_hashes must be less than or equal to
    ...
    pub fn get_blocks_inventory(
        ...
        for (consensus_hash, funai_header_hash_opt) in header_hashes.iter() {
            match funai_header_hash_opt {
                None => {
                    test_debug!(
                        "Do not have any Funai block for consensus hash {} in {}",
                        &consensus_hash,
                        &self.blocks_path
                    );
                ...
                Some(ref funai_header_hash) => {
                    ...
                    match FunaiChainState::load_block_header(
                        &self.blocks_path,
                        &consensus_hash,
                        &funai_header_hash,
                    ) {
                        Ok(Some(hdr)) => {
                            ...
                            if self.fault_injection.hide_blocks {
                                if let Some(header_info) = FunaiChainState::get_funai_block_header_info_by_index_block_hash(self.db(), &index_block_hash)? {
    ...
    /// Find the minimum and maximum Funai block heights for a reward cycle
    pub fn get_min_max_funai_block_heights_in_reward_cycle(
        &self,
        burnchain: &Burnchain,
        reward_cycle: u64,
        ...
        test_debug!(
            "Search for min/max Funai blocks between burn blocks [{},{})",
            burn_height_start,
            burn_height_end
        );
    ...
    /// Generate a blocks inventory message for a range of Funai block heights.
    ...
    pub fn get_blocks_inventory_for_reward_cycle(
        ...
        let (start_height, end_height) =
            self.get_min_max_funai_block_heights_in_reward_cycle(burnchain, reward_cycle)?;
    ...
    pub fn has_staging_block(
        ...
        match FunaiChainState::get_staging_block_status(blocks_conn, consensus_hash, block_hash)? {
    ...
    fn has_funai_block_for(chainstate_conn: &DBConn, block_commit: LeaderBlockCommitOp) -> bool {
        FunaiChainState::get_known_consensus_hashes_for_block(
            chainstate_conn,
    ...
    pub fn find_canonical_affirmation_map<B: BurnchainHeaderReader>(
        ...
        BurnchainDB::get_canonical_affirmation_map(
            ...
            |anchor_block_commit, _anchor_block_metadata| {
                ...
                FunaiChainState::has_funai_block_for(chainstate.db(), anchor_block_commit)
            },
        )
        .map_err(|e| e.into())
    }

    /// Get the affirmation map represented by the Funai chain tip.
    /// This is the private interface, to avoid having a public function take two db connections of the
    /// same type.
    fn inner_find_funai_tip_affirmation_map(
        ...
        if let Some(leader_block_commit) =
            SortitionDB::get_block_commit_for_funai_block(sort_db_conn, tip_ch, tip_bhh)?
        {
            if let Some(am_id) =
                BurnchainDB::get_block_commit_affirmation_id(burnchain_conn, &leader_block_commit)?
            {
                if let Some(am) = BurnchainDB::get_affirmation_map(burnchain_conn, am_id)? {
                    debug!(
                        "Funai tip {}/{} (txid {}) has affirmation map '{}'",
                        tip_ch, tip_bhh, &leader_block_commit.txid, &am
                    );
                    return Ok(am);
                } else {
                    debug!(
                        "Funai tip {}/{} (txid {}) affirmation map ID {} has no corresponding map",
                        tip_ch, tip_bhh, &leader_block_commit.txid, am_id
                    );
                }
            } else {
                debug!(
                    "No affirmation map for funai tip {}/{} (txid {})",
                    tip_ch, tip_bhh, &leader_block_commit.txid
                );
            }
        } else {
            debug!("No block-commit for funai tip {}/{}", tip_ch, tip_bhh);
        }
    ...
    pub fn find_funai_tip_affirmation_map(
        burnchain_db: &BurnchainDB,
        sort_db_conn: &DBConn,
        tip_ch: &ConsensusHash,
        tip_bhh: &BlockHeaderHash,
    ) -> Result<AffirmationMap, Error> {
        Self::inner_find_funai_tip_affirmation_map(
            burnchain_db.conn(),
            sort_db_conn,
            tip_ch,
            tip_bhh,
    ...
    pub fn is_block_compatible_with_affirmation_map(
        funai_tip_affirmation_map: &AffirmationMap,
        heaviest_am: &AffirmationMap,
    ) -> Result<bool, Error> {
        // NOTE: a.find_divergence(b) will be `Some(..)` even if a and b have the same prefix,
        // but b happens to be longer.  So, we need to check both `funai_tip_affirmation_map`
        // and `heaviest_am` against each other depending on their lengths.
        if (funai_tip_affirmation_map.len() > heaviest_am.len()
            && funai_tip_affirmation_map
                .find_divergence(&heaviest_am)
                .is_some())
            || (funai_tip_affirmation_map.len() <= heaviest_am.len()
                && heaviest_am
                    .find_divergence(&funai_tip_affirmation_map)
                    .is_some())
        {
            return Ok(false);
        } else {
    ...
    fn delete_orphaned_epoch_data<'a>(
        ...
        for mblock_hash in orphaned_microblock_hashes {
            FunaiChainState::delete_microblock_data(tx, &mblock_hash)?;
        }

        // mark the block as invalid if we haven't already
        let block_path =
            FunaiChainState::get_block_path(blocks_path, consensus_hash, anchored_block_hash)?;
        match fs::metadata(&block_path) {
            Ok(_) => {
                FunaiChainState::free_block(blocks_path, consensus_hash, anchored_block_hash);
            }
            Err(_) => {
                FunaiChainState::atomic_file_write(&block_path, &[])?;
            }
        }
    ...
    fn set_block_processed<'a, 'b>(
        ...
        let has_stored_block = FunaiChainState::has_stored_block(
            tx,
            blocks_path,
            consensus_hash,
            anchored_block_hash,
        ...
        if accept {
            ...
            match sort_tx_opt {
                Some(ref mut sort_tx) => {
                    sort_tx.set_funai_block_accepted(
                        consensus_hash,
            ...
            FunaiChainState::delete_orphaned_epoch_data(
                tx,
                blocks_path,
                consensus_hash,
                anchored_block_hash,
    ...
    fn set_microblocks_processed<'a>(
        ...
        let child_index_block_hash = FunaiBlockHeader::make_index_block_hash(
            child_consensus_hash,
            child_anchored_block_hash,
        );
        let (parent_consensus_hash, parent_block_hash) =
            match FunaiChainState::get_parent_block_header_hashes(tx, &child_index_block_hash)? {
                Some(x) => x,
                None => {
                    return Ok(());
                }
        ...
        loop {
            ...
            let mblock_info_opt = FunaiChainState::load_staging_microblock_info(
                tx,
                &parent_index_hash,
                &mblock_hash,
            )?;
    ...
    pub fn has_staging_microblock_indexed(
        ...
        let parent_index_block_hash =
            FunaiBlockHeader::make_index_block_hash(&parent_consensus_hash, &parent_block_hash);
        FunaiChainState::read_i64s(&self.db(), "SELECT processed FROM staging_microblocks WHERE index_block_hash = ?1 AND sequence = ?2", &[&parent_index_block_hash, &seq])
            .and_then(|processed| {
                if processed.len() == 0 {
                    Ok(false)
                }
    ...
    pub fn get_confirmed_microblock_index_hash(
        &self,
        child_index_block_hash: &FunaiBlockId,
    ) -> Result<Option<FunaiBlockId>, Error> {
        // get parent's consensus hash and block hash
        let (parent_consensus_hash, _) = match FunaiChainState::get_parent_block_header_hashes(
            &self.db(),
            child_index_block_hash,
        )? {
            Some(x) => x,
        ...
        let child_block_info =
            match FunaiChainState::load_staging_block_info(&self.db(), child_index_block_hash)? {
                Some(hdr) => hdr,
                None => {
                    test_debug!("No such block: {:?}", &child_index_block_hash);
                    return Ok(None);
        ...
        Ok(Some(FunaiBlockHeader::make_index_block_hash(
            &parent_consensus_hash,
    ...
    pub fn has_any_staging_microblock_indexed(
        ...
        FunaiChainState::read_i64s(&self.db(), "SELECT processed FROM staging_microblocks WHERE index_block_hash = ?1 AND sequence >= ?2 LIMIT 1", &[&parent_index_block_hash, &min_seq])
    ...
    pub fn has_descendant_microblock_indexed(
        ...
        FunaiChainState::read_i64s(&self.db(), "SELECT processed FROM staging_microblocks WHERE index_block_hash = ?1 AND microblock_hash = ?2 LIMIT 1", &[parent_index_block_hash, microblock_hash])
            .and_then(|processed| Ok(processed.len() > 0))
    }
    ...
    pub fn get_block_header_hashes(
        ...
        FunaiChainState::inner_get_block_header_hashes(
            &self.db(),
            index_block_hash,
            "consensus_hash",
            "anchored_block_hash",
    ...
    pub fn read_extended_header(
        ...
        blocks_path: &str,
        index_block_hash: &FunaiBlockId,
    ) -> Result<ExtendedFunaiHeader, Error> {
        let header = FunaiChainState::load_block_header_indexed(blocks_path, index_block_hash)?
            .ok_or(Error::NoSuchBlockError)?;

        let header_info = FunaiChainState::load_staging_block_info(db, index_block_hash)?
            .ok_or(Error::NoSuchBlockError)?;

        let parent_index_block_hash = FunaiBlockHeader::make_index_block_hash(
            &header_info.parent_consensus_hash,
            &header_info.parent_anchored_block_hash,
        );

        let extended_header = ExtendedFunaiHeader {
    ...
    /// Check whether or not there exists a Funai block at or higher
    ...
    fn extract_signed_microblocks(
        parent_anchored_block_header: &FunaiBlockHeader,
    ...
    pub fn validate_parent_microblock_stream(
        parent_anchored_block_header: &FunaiBlockHeader,
        anchored_block_header: &FunaiBlockHeader,
        microblocks: &[FunaiMicroblock],
        ...
        let signed_microblocks = if verify_signatures {
            FunaiChainState::extract_signed_microblocks(&parent_anchored_block_header, microblocks)
        } else {
        ...
        // miner equivocated.
        let mut parent_hashes: HashMap<BlockHeaderHash, FunaiMicroblockHeader> = HashMap::new();
    ...
    pub fn block_crosses_epoch_boundary(
        ...
        let args: &[&dyn ToSql] = &[&FunaiBlockHeader::make_index_block_hash(
            parent_consensus_hash,
            parent_block_hash,
        )];
        let res = block_conn
    ...
    /// Validate an anchored block against the burn chain state.  Determines if this given Funai
    ...
    fn validate_anchored_block_burnchain(
        ...
        let block_hash = block.block_hash();
        let (block_commit, parent_funai_chain_tip) = match db_handle
            .get_block_snapshot_of_parent_funai_block(consensus_hash, &block_hash)
        {
            Ok(Some(bc)) => bc,
            Ok(None) => {
                // unsoliciated
        ...
        let cur_epoch =
            SortitionDB::get_funai_epoch(db_handle.deref(), burn_chain_tip.block_height)?
                .expect("FATAL: no epoch defined for current Funai block");
        ...
        if FunaiChainState::block_crosses_epoch_boundary(
            blocks_conn,
            &parent_funai_chain_tip.consensus_hash,
            &parent_funai_chain_tip.winning_funai_block_hash,
        )? {
    ...
    pub fn has_anchored_block(
        ...
        if FunaiChainState::has_stored_block(
            ...
            return Ok(true);
        } else if FunaiChainState::has_staging_block(conn, consensus_hash, &block.block_hash())? {
            debug!(
            ...
            return Ok(true);
        } else if FunaiChainState::has_valid_block_indexed(&blocks_path, &index_block_hash)? {
            debug!(
    ...
    ///
    /// Caller must have called SortitionDB::expects_funai_block() to determine if this block belongs
    /// to the blockchain.  The consensus_hash is the hash of the burnchain block whose sortition
    /// elected the given Funai block.
    ///
    /// If we find the same Funai block in two or more burnchain forks, insert it there too.
    ///
    ...
    /// block: the actual block data for this anchored Funai block
    ...
    pub fn preprocess_anchored_block(
        ...
        if FunaiChainState::has_anchored_block(
            self.db(),
            &self.blocks_path,
            consensus_hash,
            block,
        ...
        if FunaiChainState::has_anchored_block(&block_tx, &blocks_path, consensus_hash, block)? {
            return Ok(false);
        }

        // does this block match the burnchain state? skip if not
        let validation_res = FunaiChainState::validate_anchored_block_burnchain(
            &block_tx,
            &sort_handle,
            consensus_hash,
            block,
        ...
        let (commit_burn, sortition_burn) = match validation_res {
            ...
            None => {
                ...
                match FunaiChainState::set_block_processed(
                    &mut block_tx,
                    None,
                    &blocks_path,
                    consensus_hash,
        ...
        FunaiChainState::store_staging_block(
            &mut block_tx,
            &blocks_path,
            consensus_hash,
            &block,
    ...
    pub fn preprocess_streamed_microblock(
        ...
        let parent_index_hash = FunaiBlockHeader::make_index_block_hash(
            parent_consensus_hash,
            parent_anchored_block_hash,
        );
        ...
        let pubkey_hash = if let Some(pubkh) = FunaiChainState::load_block_pubkey_hash(
            &blocks_tx,
            &blocks_path,
            parent_consensus_hash,
            parent_anchored_block_hash,
        ...
        if let Err(e) = dup.verify(&pubkey_hash) {
            ...
            warn!("{}", &msg);
            return Err(Error::InvalidFunaiMicroblock(msg, microblock.block_hash()));
        }
        ...
        // add to staging
        FunaiChainState::store_staging_microblock(
            &mut blocks_tx,
            parent_consensus_hash,
            parent_anchored_block_hash,
            microblock,
    ...
    /// Given a burnchain snapshot, a Funai block and a microblock stream, preprocess them all.
    /// This does not work when forking
    #[cfg(test)]
    pub fn preprocess_funai_epoch(
        &mut self,
        sort_ic: &SortitionDBConn,
        ...
        let parent_sn = match SortitionDB::get_block_snapshot_for_winning_funai_block(
            sort_ic,
    ...
    pub fn make_scheduled_miner_reward(
        mainnet: bool,
        epoch_id: FunaiEpochId,
        parent_block_hash: &BlockHeaderHash,
        parent_consensus_hash: &ConsensusHash,
        block_hash: &BlockHeaderHash,
        coinbase_tx: &FunaiTransaction,
        ...
        let recipient = if epoch_id >= FunaiEpochId::Epoch21 {
        ...
        // per the above check (and moreover, a Funai block with a pay-to-alt-recipient coinbase would
    ...
    pub fn find_parent_microblock_stream(
        ...
        match FunaiChainState::load_microblock_stream_fork(
            blocks_conn,
    ...
    fn find_next_staging_block<'a>(
        blocks_tx: &mut FunaiDBTx<'a>,
        blocks_path: &str,
        sort_tx: &mut SortitionHandleTx,
    ) -> Result<Option<(Vec<FunaiMicroblock>, StagingBlock)>, Error> {
            ...
            while let Some(row) = rows.next().map_err(|e| db_error::SqliteError(e))? {
                ...
                let can_attach = {
                    if candidate.parent_anchored_block_hash == FIRST_STACKS_BLOCK_HASH {
                        ...
                        let hdr_row = query_row_panic::<FunaiHeaderInfo, _, _>(
                            blocks_tx,
                            &hdr_sql,
                            hdr_args,
                            || {
                ...
                if can_attach {
                    ...
                    candidate.block_data = match FunaiChainState::load_block_bytes(
                        blocks_path,
                        &candidate.consensus_hash,
                        &candidate.anchored_block_hash,
                    )? {
                    ...
                    match FunaiChainState::find_parent_microblock_stream(blocks_tx, &candidate)? {
        ...
        for (consensus_hash, anchored_block_hash) in to_delete.into_iter() {
            ...
            let _ = FunaiChainState::set_block_processed(
                blocks_tx,
                None,
                blocks_path,
                &consensus_hash,
    ...
    /// If an epoch transition occurs at this Funai block,
    ...
    pub fn process_epoch_transition(
    ...
    ) -> Result<(bool, Vec<FunaiTransactionReceipt>), Error> {
        // is this funai block the first of a new epoch?
        let (funai_parent_epoch, sortition_epoch) = clarity_tx
            .with_clarity_db_readonly::<_, Result<_, clarity::vm::errors::Error>>(|db| {
                Ok((
                    db.get_clarity_epoch_version()?,
                    db.get_funai_epoch(chain_tip_burn_header_height),
                ))
            })?;
        ...
        if let Some(sortition_epoch) = sortition_epoch {
            // check if the parent funai block has a different epoch than what the Sortition DB
            //  thinks should be in place, and apply epoch transitions
            let mut current_epoch = funai_parent_epoch;
            while current_epoch != sortition_epoch.epoch_id {
                ...
                assert!(current_epoch < sortition_epoch.epoch_id, "The SortitionDB believes the epoch is earlier than this Funai block's parent: sortition db epoch = {}, current epoch = {}", sortition_epoch.epoch_id, current_epoch);
    ...
    ///  that haven't been processed in this Funai fork yet.
    pub fn process_stacking_ops(
        clarity_tx: &mut ClarityTx,
        operations: Vec<StackStxOp>,
        active_pox_contract: &str,
    ) -> Vec<FunaiTransactionReceipt> {
        ...
        for stack_stx_op in operations.into_iter() {
            ...
            if active_pox_contract == PoxVersions::Pox4.get_name() {
                match FunaiChainState::collect_pox_4_stacking_args(&stack_stx_op) {
                    Ok(pox_4_args) => {
    ...
    pub fn process_matured_miner_rewards<'a, 'b>(
        ...
        FunaiChainState::process_matured_miner_reward(clarity_tx, miner_share)?;
        for reward in users_share.iter() {
            coinbase_reward += reward.coinbase;
            FunaiChainState::process_matured_miner_reward(clarity_tx, reward)?;
        }
        ...
        FunaiChainState::process_matured_miner_reward(clarity_tx, parent_share)?;
        Ok(coinbase_reward)
    }
    ...
    pub fn process_stx_unlocks<'a, 'b>(
        clarity_tx: &mut ClarityTx<'a, 'b>,
    ) -> Result<(u128, Vec<FunaiTransactionEvent>), Error> {
        let mainnet = clarity_tx.config.mainnet;
            ...
            .as_transaction(|tx_connection| {
                ...
                for entry in entries.into_iter() {
                    ...
                    let recipient = schedule
                        ...
                        .expect_principal()?;
                    total_minted += amount;
                    FunaiChainState::account_credit(
                        tx_connection,
                        &recipient,
                    ...
                    events.push(FunaiTransactionEvent::STXEvent(event));
                }
    ...
    fn get_stacking_and_transfer_and_delegate_burn_ops_v210(
        ...
        Error,
    > {
        // only consider transactions in Funai 2.1
        ...
        debug!(
            "Search the last {} sortitions for burnchain-hosted funai operations before {} ({})",
        ...
        let processed_burnchain_txids = FunaiChainState::get_burnchain_txids_in_ancestors(
            chainstate_tx.deref().deref(),
            parent_index_hash,
            search_window.into(),
        )?;

        // Find the *new* transactions -- the ones that we *haven't* seen in this Funai
        // fork yet.  Note that we search for the ones that we have seen by searching back
        // `BURNCHAIN_TX_SEARCH_WINDOW` *Funai* blocks, whose sortitions may span more
    ...
    /// the Funai block that was selected for this burnchain block.
    /// The rules are different for different epochs:
    ///
    /// * In Funai 2.0/2.05, only the operations in the burnchain block will be considered.
    /// So if a transaction was mined in burnchain block N, it will be processed in the Funai
    /// block mined in burnchain block N (if there is one).
    ///
    /// * In Funai 2.1+, the operations in the last K burnchain blocks that have not yet been
    /// considered in this Funai block's fork will be processed in the order in which they are
    /// mined in the burnchain.  So if a transaction was mined in an burnchain block between N and
    /// N-K inclusive, it will be processed in each Funai fork that contains at least one Funai
    /// block mined in the same burnchain interval.
    ///
    /// The rationale for the new behavior in Funai 2.1+ is that burnchain-hosted STX operations
    /// can get picked up in Funai blocks that only live on short-lived forks, or get mined in
    /// burnchain blocks in which there was no sortiton.  In either case, the operation does not
    /// materialize on the canonical Funai chain.  This is a bad user
    ...
    ///
    /// The change in Funai 2.1+ makes it so that it's overwhelmingly likely to work
    ...
    pub fn get_stacking_and_transfer_and_delegate_burn_ops(
        ...
        Error,
    > {
        let cur_epoch = SortitionDB::get_funai_epoch(sortdb_conn, burn_tip_height)?
        ...
        match cur_epoch.epoch_id {
            ...
            FunaiEpochId::Epoch20 | FunaiEpochId::Epoch2_05 => {
                let (stack_ops, transfer_ops) =
                    FunaiChainState::get_stacking_and_transfer_burn_ops_v205(
                        sortdb_conn,
                        burn_tip,
                    )?;
            ...
            | FunaiEpochId::Epoch24 => {
                let (stack_ops, transfer_ops, delegate_ops, _) =
                    FunaiChainState::get_stacking_and_transfer_and_delegate_burn_ops_v210(
                        chainstate_tx,
                        parent_index_hash,
                        sortdb_conn,
                        burn_tip,
            ...
            FunaiEpochId::Epoch25 | FunaiEpochId::Epoch30 => {
    ...
    pub fn check_and_handle_reward_start(
        ...
        let events = clarity_tx.block.as_free_transaction(|clarity_tx| {
            match active_epoch {
                ...
                FunaiEpochId::Epoch24 => Self::handle_pox_cycle_start_pox_3(
                    clarity_tx,
                    pox_reward_cycle,
                    pox_start_cycle_info,
                ),
    ...
    /// miner rewards tuples, the funai epoch id, and a boolean that
    ...
    pub fn setup_block<'a, 'b>(
        ...
        let (latest_matured_miners, matured_miner_parent) = {
            let latest_miners = FunaiChainState::get_scheduled_block_rewards(
        ...
        let parent_block_cost = if miner_id_opt.is_none() || !parent_microblocks.is_empty() {
            let cost = FunaiChainState::get_funai_block_anchored_cost(
                &chainstate_tx.deref().deref(),
                &parent_index_hash,
            )?
            .ok_or_else(|| {
                Error::InvalidFunaiBlock(format!(
                    "Failed to load parent block cost. parent_funai_block_id = {}",
                    &parent_index_hash
                ))
            })?;
        ...
        let mut clarity_tx = FunaiChainState::chainstate_block_begin(
            chainstate_tx,
            clarity_instance,
            burn_dbconn,
            &parent_consensus_hash,
        ...
        let matured_miner_rewards_opt = match FunaiChainState::find_mature_miner_rewards(
            &mut clarity_tx,
            conn,
            chain_tip.funai_block_height,
        ...
        let (microblock_fees, microblock_burns, microblock_txs_receipts) =
            match FunaiChainState::process_microblocks_transactions(
                &mut clarity_tx,
                &parent_microblocks,
                ...
                Err((e, mblock_header_hash)) => {
                    let msg = format!(
                        "Invalid Funai microblocks {},{} (offender {}): {:?}",
                    ...
                    return Err(Error::InvalidFunaiMicroblock(msg, mblock_header_hash));
                }
            };
        ...
        let (applied_epoch_transition, mut tx_receipts) =
            FunaiChainState::process_epoch_transition(&mut clarity_tx, burn_tip_height)?;
        ...
        let signer_set_calc;
        if evaluated_epoch >= FunaiEpochId::Epoch25 {
            signer_set_calc = NakamotoSigners::check_and_handle_prepare_phase_start(
                ...
                chain_tip.funai_block_height + 1,
            )?;
        } else {
        ...
        tx_receipts.extend(FunaiChainState::process_stacking_ops(
            &mut clarity_tx,
        ...
        tx_receipts.extend(FunaiChainState::process_transfer_ops(
            &mut clarity_tx,
        ...
        if evaluated_epoch >= FunaiEpochId::Epoch21 {
            tx_receipts.extend(FunaiChainState::process_delegate_ops(
                &mut clarity_tx,
        ...
        if evaluated_epoch >= FunaiEpochId::Epoch25 {
            tx_receipts.extend(FunaiChainState::process_vote_for_aggregate_key_ops(
                &mut clarity_tx,
    ...
    pub fn finish_block(
        ...
        // process unlocks
        let (new_unlocked_ustx, lockup_events) = FunaiChainState::process_stx_unlocks(clarity_tx)?;

        clarity_tx.increment_ustx_liquid_supply(new_unlocked_ustx);

        // mark microblock public key as used
        match FunaiChainState::insert_microblock_pubkey_hash(
            clarity_tx,
            block_height,
            &mblock_pubkey_hash,
        ) {
    ...
    /// processed yet.
    ///
    /// Returns a `FunaiEpochReceipt` containing receipts and events from the transactions executed
    /// in the block, and a `PreCommitClarityBlock` struct.
    ///
    /// The `FunaiEpochReceipts` contains the list of transaction
    ...
    pub fn append_block<'a>(
        ...
        pox_constants: &PoxConstants,
        parent_chain_tip: &FunaiHeaderInfo,
        ...
        block: &FunaiBlock,
        block_size: u64,
        microblocks: &Vec<FunaiMicroblock>, // parent microblocks
        burnchain_commit_burn: u64,
        burnchain_sortition_burn: u64,
    ...
    > {
        ...
        if FunaiChainState::block_crosses_epoch_boundary(
            ...
            if block.has_microblock_parent() {
                ...
                warn!("{}", &msg);
                return Err(Error::InvalidFunaiBlock(msg.to_string()));
            }
        }
        ...
        let (
            ...
            match FunaiChainState::has_microblock_pubkey_hash(
                &mut clarity_tx,
                &block.header.microblock_pubkey_hash,
            ) {
                Ok(Some(height)) => {
                    // already used
                    let msg = format!(
                        "Invalid funai block {}/{} -- already used microblock pubkey hash {} at height {}",
                        chain_tip_consensus_hash,
                        block.block_hash(),
                        &block.header.microblock_pubkey_hash,
                        height
            ...
            debug!("Append block";
                   ...
                   "funai_height" => %block.header.total_work.work,
            ...
            let mut lockup_events = match FunaiChainState::finish_block(
                &mut clarity_tx,
                miner_payouts_opt.as_ref(),
                u32::try_from(block.header.total_work.work).expect("FATAL: more than 2^32 blocks"),
                block.header.microblock_pubkey_hash,
            ) {
                Err(Error::InvalidFunaiBlock(e)) => {
                    clarity_tx.rollback_block();
                    return Err(Error::InvalidFunaiBlock(e));
                }
                Err(e) => return Err(e),
            ...
            let coinbase_at_block = FunaiChainState::get_coinbase_reward(
            ...
            let scheduled_miner_reward = FunaiChainState::make_scheduled_miner_reward(
                ...
                block
                    .get_coinbase_tx()
                    .as_ref()
                    .ok_or(Error::InvalidFunaiBlock("No coinbase transaction".into()))?,
                chain_tip_consensus_hash,
                next_block_height,
                block_fees,
                microblock_fees,
        ...
        if do_not_advance {
            let epoch_receipt = FunaiEpochReceipt {
                header: FunaiHeaderInfo::regtest_genesis(),
                tx_receipts,
                matured_rewards,
        ...
        let new_tip = FunaiChainState::advance_tip(
        ...
        //  it depends on knowing the FunaiBlockId.
    ...
    /// Verify that a Funai anchored block attaches to its parent anchored block.
    /// * checks .header.total_work.work
    /// * checks .header.parent_block
    pub fn check_block_attachment(
        parent_block_header: &FunaiBlockHeader,
        block_header: &FunaiBlockHeader,
    ) -> bool {
    ...
    ) -> Result<Option<FunaiHeaderInfo>, Error> {
        let parent_block_header_info = match FunaiChainState::get_anchored_block_header_info(
            &chainstate_tx.tx,
            ...
            None => {
                if next_staging_block.is_first_mined() {
                    ...
                    FunaiChainState::get_anchored_block_header_info(
                        &chainstate_tx.tx,
    ...
    pub fn extract_funai_block(next_staging_block: &StagingBlock) -> Result<FunaiBlock, Error> {
        let block = {
            FunaiBlock::consensus_deserialize(&mut &next_staging_block.block_data[..])
                .map_err(Error::CodecError)?
        };
    ...
    pub fn extract_connecting_microblocks(
        parent_block_header_info: &FunaiHeaderInfo,
        next_staging_block: &StagingBlock,
        block: &FunaiBlock,
        mut next_microblocks: Vec<FunaiMicroblock>,
    ) -> Result<Vec<FunaiMicroblock>, Error> {
        ...
        let microblock_terminus = match FunaiChainState::validate_parent_microblock_stream(
    ...
    pub fn process_next_staging_block<'a, T: BlockEventDispatcher>(
    ...
    ) -> Result<(Option<FunaiEpochReceipt>, Option<TransactionPayload>), Error> {
        let blocks_path = self.blocks_path.clone();
        ...
        let (next_microblocks, next_staging_block) =
            match FunaiChainState::find_next_staging_block(
                &mut chainstate_tx.tx,
                &blocks_path,
                sort_tx,
            )? {
        ...
        if microblocks_disabled_by_epoch_25 {
            ...
            {
                let msg = format!(
                    "Invalid funai block {}/{} ({}). Confirms microblocks after Epoch 2.5 start.",
                    &next_staging_block.consensus_hash,
                ...
                // clear out
                FunaiChainState::set_block_processed(
                    chainstate_tx.deref_mut(),
                    None,
                    &blocks_path,
                    &next_staging_block.consensus_hash,
        ...
        let parent_header_info = match FunaiChainState::get_parent_header_info(
            &mut chainstate_tx,
            &next_staging_block,
        )? {
            Some(hinfo) => hinfo,
        ...
        let block = FunaiChainState::extract_funai_block(&next_staging_block)?;
        ...
        if FunaiChainState::has_funai_block(
            chainstate_tx.tx.deref().deref(),
            &FunaiBlockHeader::make_index_block_hash(
                &next_staging_block.consensus_hash,
                &next_staging_block.anchored_block_hash,
            ),
        )? || FunaiChainState::has_stored_block(
            chainstate_tx.tx.deref().deref(),
            &blocks_path,
            &next_staging_block.consensus_hash,
            &next_staging_block.anchored_block_hash,
        ...
        if !FunaiChainState::check_block_attachment(
            parent_header_info
                .anchored_header
                .as_funai_epoch2()
                .ok_or_else(|| Error::InvalidChildOfNakomotoBlock)?,
            ...
            let msg = format!(
                "Invalid funai block {}/{} -- does not attach to parent {}/{}",
                &next_staging_block.consensus_hash,
                block.block_hash(),
        ...
        let next_microblocks = FunaiChainState::extract_connecting_microblocks(
            &parent_header_info,
            &next_staging_block,
            &block,
            next_microblocks,
        ...
        let block_am = FunaiChainState::inner_find_funai_tip_affirmation_map(
            burnchain_dbconn,
            sort_tx.tx(),
            &next_staging_block.consensus_hash,
            &next_staging_block.anchored_block_hash,
        ...
        let (epoch_receipt, clarity_commit, reward_set_data) = match FunaiChainState::append_block(
            &mut chainstate_tx,
            clarity_instance,
            sort_tx,
            &pox_constants,
            ...
            Err(e) => {
                ...
                FunaiChainState::free_block_state(
                    &blocks_path,
                    &next_staging_block.consensus_hash,
                    &block.header,
                );
                ...
                match e {
                    Error::InvalidFunaiMicroblock(ref msg, ref header_hash) => {
                        ...
                        FunaiChainState::drop_staging_microblocks(
                            chainstate_tx.deref_mut(),
                            &parent_header_info.consensus_hash,
                            &parent_header_info.anchored_header.block_hash(),
                            header_hash,
            ...
            || next_staging_block.parent_microblock_seq != 0
        {
            ...
            FunaiChainState::set_microblocks_processed(
                chainstate_tx.deref_mut(),
                &next_staging_block.consensus_hash,
                &next_staging_block.anchored_block_hash,
                &next_staging_block.parent_microblock_hash,
        ...
        if let Some(dispatcher) = dispatcher_opt {
            let parent_id = FunaiBlockId::new(
                &next_staging_block.parent_consensus_hash,
                &next_staging_block.parent_anchored_block_hash,
            );
    ...
    ///  PoX aware (i.e., unit tests, and old funai-node loops),
    ...
    pub fn process_blocks_at_tip(
        ...
        max_blocks: usize,
    ) -> Result<Vec<(Option<FunaiEpochReceipt>, Option<TransactionPayload>)>, Error> {
    ...
    pub fn process_blocks<'a, T: BlockEventDispatcher>(
        ...
        loop {
            ...
            let deleted =
                FunaiChainState::process_next_orphaned_staging_block(&mut block_tx, &blocks_path)?;
            if !deleted {
                break;
            }
            num_orphans += 1;
        ...
        for i in 0..max_blocks {
            ...
            match self.process_next_staging_block(burnchain_db_conn, &mut sort_tx, dispatcher_opt) {
                ...
                Err(Error::InvalidFunaiBlock(msg)) => {
                    warn!("Encountered invalid block: {}", &msg);
                    ret.push((None, None));
                    continue;
                }
                Err(Error::InvalidFunaiMicroblock(msg, hash)) => {
    ...
    /// in.
    pub fn get_funai_chain_tip(
        &self,
        sortdb: &SortitionDB,
    ) -> Result<Option<StagingBlock>, Error> {
        let (consensus_hash, block_bhh) =
            SortitionDB::get_canonical_funai_chain_tip_hash(sortdb.conn())?;
    ...
    pub fn get_funai_chain_tips(&self, sortdb: &SortitionDB) -> Result<Vec<StagingBlock>, Error> {
        let (consensus_hash, block_bhh) =
            SortitionDB::get_canonical_funai_chain_tip_hash(sortdb.conn())?;
        let sql = "SELECT * FROM staging_blocks WHERE processed = 1 AND orphaned = 0 AND consensus_hash = ?1 AND anchored_block_hash = ?2";
        let args: &[&dyn ToSql] = &[&consensus_hash, &block_bhh];
        ...
        self.get_funai_chain_tips_at_height(staging_block.height)
    }

    /// Get all Funai blocks at a given height
    pub fn get_funai_chain_tips_at_height(&self, height: u64) -> Result<Vec<StagingBlock>, Error> {
        let sql =
    ...
    fn can_admit_mempool_semantic(
        ...
        match tx.payload {
            TransactionPayload::TokenTransfer(ref recipient, amount, ref _memo) => {
                ...
                if !FunaiChainState::is_valid_address_version(is_mainnet, recipient.version()) {
    ...
    pub fn will_admit_mempool_tx(
        ...
        FunaiChainState::can_admit_mempool_semantic(tx, is_mainnet)?;
        ...
        let current_tip =
            FunaiChainState::get_parent_index_block(current_consensus_hash, current_block);
        match self.with_read_only_clarity_tx(burn_state_db, &current_tip, |conn| {
            FunaiChainState::can_include_tx(conn, &conf, false, tx, tx_size)
        }) {
            Some(r) => r,
    ...
    fn can_include_tx<T: ClarityConnection>(
        ...
        FunaiChainState::process_transaction_precheck(&chainstate_config, &tx)
        ...
        let (origin, payer) =
            match FunaiChainState::check_transaction_nonces(clarity_connection, &tx, true) {
                Ok(x) => x,
        ...
        if !FunaiChainState::is_valid_address_version(
            chainstate_config.mainnet,
            origin.principal.version(),
        ) || !FunaiChainState::is_valid_address_version(
            chainstate_config.mainnet,
        ...
        match &tx.payload {
            ...
            TransactionPayload::SmartContract(
                ...
                if let Some(_version) = version_opt.as_ref() {
                    if clarity_connection.get_epoch() < FunaiEpochId::Epoch21 {
                        return Err(MemPoolRejection::Other(
...
pub mod test {
    ...
    use serde_json;
    use funai_common::types::chainstate::{BlockHeaderHash, FunaiWorkScore};
    use funai_common::util::hash::*;
    use funai_common::util::retry::*;

    use super::*;
    use crate::burnchains::*;
    use crate::chainstate::burn::db::sortdb::*;
    use crate::chainstate::burn::*;
    use crate::chainstate::funai::boot::test::eval_at_tip;
    use crate::chainstate::funai::db::test::*;
    use crate::chainstate::funai::db::*;
    use crate::chainstate::funai::miner::*;
    use crate::chainstate::funai::test::*;
    use crate::chainstate::funai::tests::*;
    use crate::chainstate::funai::{Error as chainstate_error, *};
    ...
    pub fn make_empty_coinbase_block(mblock_key: &FunaiPrivateKey) -> FunaiBlock {
        ...
        let work_score = FunaiWorkScore {
            burn: 123,
            work: 456,
        };

        let parent_header = FunaiBlockHeader {
            version: 0x01,
            total_work: FunaiWorkScore {
                burn: 234,
                work: 567,
            },
            proof: proof.clone(),
        ...
        let parent_microblock_header = FunaiMicroblockHeader {
            version: 0x12,
            sequence: 0x34,
        ...
        let mblock_pubkey_hash =
            Hash160::from_node_public_key(&FunaiPublicKey::from_private(mblock_key));
        let mut block = FunaiBlock::from_parent(
            &parent_header,
    ...
    pub fn make_16k_block(mblock_key: &FunaiPrivateKey) -> FunaiBlock {
        let privk = FunaiPrivateKey::from_hex(
            "59e4d5e18351d6027a37920efe53c2f1cbadc50dca7d77169b7291dff936ed6d01",
        )
        .unwrap();
        ...
        let mut tx_big_contract = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
        ...
        let mut tx_signer = FunaiTransactionSigner::new(&tx_big_contract);
        tx_signer.sign_origin(&privk).unwrap();
    ...
    pub fn make_sample_microblock_stream_fork(
        privk: &FunaiPrivateKey,
        base: &BlockHeaderHash,
        initial_seq: u16,
    ) -> Vec<FunaiMicroblock> {
        let mut all_txs = vec![];
        let mut microblocks: Vec<FunaiMicroblock> = vec![];
        ...
        for i in 0..(all_txs.len() / 3) {
            ...
            let header = FunaiMicroblockHeader {
                version: 0x12,
            ...
            let mut mblock = FunaiMicroblock {
                header: header,
                txs: txs,
            };
    ...
    fn resign_microblocks(
        microblocks: &mut Vec<FunaiMicroblock>,
        privk: &FunaiPrivateKey,
    ) -> BlockHeaderHash {
    ...
    fn assert_block_staging_not_processed(
        ...
        assert!(FunaiChainState::load_staging_block_data(
            &chainstate.db(),
        ...
        .is_some());
        assert_eq!(
            FunaiChainState::load_staging_block_data(
                &chainstate.db(),
                &chainstate.blocks_path,
                consensus_hash,
                &block.block_hash()
        ...
        assert_eq!(
            FunaiChainState::get_staging_block_status(
                &chainstate.db(),
                consensus_hash,
                &block.block_hash()
            )
        ...
        let index_block_hash =
            FunaiBlockHeader::make_index_block_hash(consensus_hash, &block.block_hash());
        assert!(
            FunaiChainState::has_block_indexed(&chainstate.blocks_path, &index_block_hash)
                .unwrap()
        );
    }
    ...
    fn assert_block_not_stored(
        ...
        assert!(!FunaiChainState::has_stored_block(
            &chainstate.db(),
            &chainstate.blocks_path,
            consensus_hash,
            &block.block_hash()
        ...
        .unwrap());
        assert_eq!(
            FunaiChainState::load_staging_block_pubkey_hash(
                &chainstate.db(),
                consensus_hash,
                &block.block_hash()
            )
    ...
    fn assert_block_stored_rejected(
        ...
        .unwrap());
        assert!(FunaiChainState::load_block(
            &chainstate.blocks_path,
            consensus_hash,
            &block.block_hash()
        )
        .unwrap()
        .is_none());
        assert!(FunaiChainState::load_block_header(
            &chainstate.blocks_path,
            consensus_hash,
            &block.block_hash()
        )
        .unwrap()
        .is_none());
        assert!(FunaiChainState::load_staging_block_pubkey_hash(
            &chainstate.db(),
            consensus_hash,
            &block.block_hash()
        )
    ...
    fn assert_block_stored_not_staging(
        ...
        assert_eq!(
            FunaiChainState::load_block_header(
                &chainstate.blocks_path,
                consensus_hash,
                &block.block_hash()
            )
    ...
    pub fn set_block_orphaned(
        ...
        let mut tx = chainstate.db_tx_begin().unwrap();
        FunaiChainState::set_block_orphaned(
            &mut tx,
            &blocks_path,
            consensus_hash,
            anchored_block_hash,
    ...
    pub fn set_microblocks_processed(
        ...
        let (parent_consensus_hash, parent_block_hash) =
            FunaiChainState::get_parent_block_header_hashes(
                &chainstate.db(),
                &child_index_block_hash,
            )
            .unwrap()
        ...
        let parent_microblock_index_hash =
            FunaiBlockHeader::make_index_block_hash(&parent_consensus_hash, &tail_microblock_hash);
        ...
        assert!(chainstate
            .has_microblocks_indexed(&parent_index_block_hash)
            .unwrap());
        assert!(FunaiChainState::has_processed_microblocks_indexed(
            chainstate.db(),
    ...
    fn process_next_orphaned_staging_block(chainstate: &mut FunaiChainState) -> bool {
        let blocks_path = chainstate.blocks_path.clone();
        let mut tx = chainstate.db_tx_begin().unwrap();
        let res =
            FunaiChainState::process_next_orphaned_staging_block(&mut tx, &blocks_path).unwrap();
        tx.commit().unwrap();
        res
    }
    ...
    #[test]
    fn funai_db_block_load_store_empty() {
        let chainstate = instantiate_chainstate(false, 0x80000000, function_name!());

        let path = FunaiChainState::get_block_path(
            &chainstate.blocks_path,
        ...
        FunaiChainState::store_empty_block(
            &chainstate.blocks_path,
            &ConsensusHash([1u8; 20]),
            &BlockHeaderHash([2u8; 32]),
        )
    ...
    fn funai_db_block_load_store() {
        ...
        FunaiChainState::forget_orphaned_epoch_data(
            &mut dbtx,
            &ConsensusHash([1u8; 20]),
            &block.block_hash(),
        )
    ...
    #[test]
    fn funai_db_staging_block_load_store_accept() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
    ...
    #[test]
    fn funai_db_staging_block_load_store_reject() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
    ...
    #[test]
    fn funai_db_load_store_microblock_stream() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
        ...
        assert!(FunaiChainState::load_microblock_stream_fork(
            &chainstate.db(),
            &ConsensusHash([2u8; 20]),
            &block.block_hash(),
        ...
        assert_eq!(
            FunaiChainState::load_microblock_stream_fork(
                &chainstate.db(),
                &ConsensusHash([2u8; 20]),
                &block.block_hash(),
                &microblocks.last().as_ref().unwrap().block_hash(),
        ...
        // not processed
        assert!(FunaiChainState::load_processed_microblock_stream_fork(
            &chainstate.db(),
            &ConsensusHash([2u8; 20]),
            &block.block_hash(),
            &microblocks.last().as_ref().unwrap().block_hash(),
    ...
    #[test]
    fn funai_db_staging_microblock_stream_load_store_confirm_all() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
        ...
        assert!(FunaiChainState::load_staging_microblock(
            &chainstate.db(),
            &ConsensusHash([2u8; 20]),
            &block.block_hash(),
            &microblocks[0].block_hash()
        ...
        assert!(FunaiChainState::load_descendant_staging_microblock_stream(
            &chainstate.db(),
            &FunaiBlockHeader::make_index_block_hash(
                &ConsensusHash([2u8; 20]),
                &block.block_hash()
        ...
        assert_eq!(
            FunaiChainState::load_staging_microblock(
                &chainstate.db(),
                &ConsensusHash([2u8; 20]),
                &block.block_hash(),
                &microblocks[0].block_hash()
    ...
    #[test]
    fn funai_db_staging_microblock_stream_load_store_partial_confirm() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
        ...
        assert_eq!(
            FunaiChainState::load_processed_microblock_stream_fork(
                &chainstate.db(),
                &ConsensusHash([2u8; 20]),
                &block.block_hash(),
    ...
    #[test]
    fn funai_db_staging_microblock_stream_load_continuous_streams() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
    ...
    #[test]
    fn funai_db_validate_parent_microblock_stream() {
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
        .unwrap();
        ...
        let child_block_header = FunaiBlockHeader {
            version: 0x01,
            total_work: FunaiWorkScore {
                burn: 234,
                work: 567,
        ...
        {
            let res = FunaiChainState::validate_parent_microblock_stream(
                &block.header,
                &child_block_header,
                &microblocks,
                true,
        ...
        // deliberate miner fork
        {
            ...
            for i in 0..broken_microblocks.len() {
                ...
                if i == num_mblocks / 2 {
                    ...
                    let extra_tx = {
                        let auth = TransactionAuth::from_p2pkh(&privk).unwrap();
                        let tx_smart_contract = FunaiTransaction::new(
                            TransactionVersion::Testnet,
                            auth.clone(),
                            TransactionPayload::new_smart_contract(
                        ...
                        let mut tx_signer = FunaiTransactionSigner::new(&tx_smart_contract);
                        tx_signer.sign_origin(&privk).unwrap();
    ...
    #[test]
    fn funai_db_staging_block_load_store_accept_attachable() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
        ...
        assert_eq!(
            FunaiChainState::load_staging_block(
                &chainstate.db(),
                &chainstate.blocks_path,
                &consensus_hashes[0],
                &block_1.block_hash()
    ...
    #[test]
    fn funai_db_staging_block_load_store_accept_attachable_reversed() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
    ...
    #[test]
    fn funai_db_staging_block_load_store_accept_attachable_fork() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
    ...
    #[test]
    fn funai_db_staging_microblocks_multiple_descendants() {
        ...
        for i in 1..len {
            ...
            let mblocks_confirmed = FunaiChainState::load_processed_microblock_stream_fork(
                &chainstate.db(),
                &consensus_hashes[0],
                &blocks[0].block_hash(),
                &blocks[i].header.parent_microblock,
    ...
    #[test]
    fn funai_db_staging_blocks_orphaned() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
        ...
        for i in 0..blocks.len() {
            ...
            for mblock in microblocks[i].iter() {
                ...
                assert!(FunaiChainState::load_staging_microblock_bytes(
                    &chainstate.db(),
                    &mblock.block_hash()
                )
                .unwrap()
            ...
            if i + 1 < blocks.len() {
                ...
                assert!(FunaiChainState::load_staging_block(
                    &chainstate.db(),
                    &chainstate.blocks_path,
                ...
                .is_none());
                assert!(
                    FunaiChainState::load_block_bytes(
                        &chainstate.blocks_path,
                        &consensus_hashes[i + 1],
                        &blocks[i + 1].block_hash()
                    )
                ...
                for mblock in microblocks[i + 1].iter() {
                    let staging_mblock = FunaiChainState::load_staging_microblock(
                        &chainstate.db(),
                        &consensus_hashes[i + 1],
                        &blocks[i + 1].block_hash(),
                        &mblock.block_hash(),
    ...
    #[test]
    fn funai_db_drop_staging_microblocks() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
    ...
    #[test]
    fn funai_db_has_blocks_and_microblocks() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());
        let privk = FunaiPrivateKey::from_hex(
            "eb05c83546fdd2c79f10f5ad5434a90dd28f7e3acb7c092157aa1bc3656b012c01",
        )
        ...
        let index_block_header =
            FunaiBlockHeader::make_index_block_hash(&consensus_hash, &block.block_hash());
        assert!(
            !FunaiChainState::has_block_indexed(&chainstate.blocks_path, &index_block_header)
                .unwrap()
        );
        assert!(!chainstate
        ...
        let child_index_block_header = FunaiBlockHeader::make_index_block_hash(
            &child_consensus_hash,
            &child_block.block_hash(),
        );
        assert!(!FunaiChainState::has_block_indexed(
            &chainstate.blocks_path,
        ...
        assert_eq!(
            FunaiChainState::stream_microblock_get_info(&chainstate.db(), &index_block_header)
                .unwrap()
                .len(),
            0
        );
        ...
        for (i, mblock) in mblocks.iter().enumerate() {
            assert!(FunaiChainState::stream_microblock_get_rowid(
                &chainstate.db(),
                &index_block_header,
    ...
    pub fn decode_microblock_stream(mblock_bytes: &Vec<u8>) -> Vec<FunaiMicroblock> {
        // decode stream
        ...
        loop {
            test_debug!("decoded {}", mblocks.len());
            {
                ...
                let next_mblock = FunaiMicroblock::consensus_deserialize(&mut debug_reader)
                    .map_err(|e| {
    ...
    #[test]
    fn funai_db_get_blocks_inventory() {
        let mut chainstate = instantiate_chainstate(false, 0x80000000, function_name!());

        let mut blocks: Vec<FunaiBlock> = vec![];
        let mut privks = vec![];
        let mut microblocks = vec![];
    ...
    #[test]
    fn funai_db_get_blocks_inventory_for_reward_cycle() {
        let mut peer_config = TestPeerConfig::new(function_name!(), 21313, 21314);

        let privk = FunaiPrivateKey::new();
        let addr = FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(&privk)],
        )
        .unwrap();
        ...
        let recv_addr =
            FunaiAddress::from_string("ST1H1B54MY50RMBRRKS7GV2ZWG79RZ1RQ1ETW4E01").unwrap();

        let mut peer = TestPeer::new(peer_config.clone());

        let chainstate_path = peer.chainstate_path.clone();

        // NOTE: first_funai_block_height is the burnchain height at which the node starts mining.
        // The burnchain block at this height will have the VRF key register, but no block-commit.
        // The first burnchain block with a Funai block is at first_funai_block_height + 1.
        let (first_funai_block_height, canonical_sort_id) = {
            let sn =
        ...
        for i in 0..(first_funai_block_height + 1) {
            ...
            header_hashes.push((
                sn.consensus_hash,
                if sn.sortition {
                    Some(sn.winning_funai_block_hash)
                } else {
                    None
                },
            ));
        ...
        let last_funai_block_height = first_funai_block_height
        ...
        for tenure_id in 0..(last_funai_block_height - first_funai_block_height) {
            let tip =
                SortitionDB::get_canonical_burn_chain_tip(&peer.sortdb.as_ref().unwrap().conn())
                    .unwrap();
            ...
            assert_eq!(
                tip.block_height,
                first_funai_block_height + (tenure_id as u64)
            );

            let (burn_ops, funai_block, microblocks) = peer.make_tenure(
                |ref mut miner,
                 ref mut sortdb,
                 ref mut chainstate,
                 vrf_proof,
                 ...
                 ref parent_microblock_header_opt| {
                    let parent_tip = match parent_opt {
                        None => FunaiChainState::get_genesis_header_info(chainstate.db()).unwrap(),
                        Some(block) => {
                            ...
                            FunaiChainState::get_anchored_block_header_info(
                                chainstate.db(),
                                &snapshot.consensus_hash,
                                &snapshot.winning_funai_block_hash,
                            )
                            .unwrap()
                            .unwrap()
                        }
                    ...
                    let microblock_privkey = FunaiPrivateKey::new();
                    let microblock_pubkeyhash = Hash160::from_node_public_key(
                        &FunaiPublicKey::from_private(&microblock_privkey),
                    );
                    let anchored_block = FunaiBlockBuilder::build_anchored_block(
                        chainstate,
                        &sortdb.index_conn(),
                        &mut mempool,
                        &parent_tip,
                    ...
                    for i in 0..2 {
                        ...
                        let tx = {
                            let auth = TransactionAuth::Standard(
                                TransactionSpendingCondition::new_singlesig_p2pkh(
                                    FunaiPublicKey::from_private(&privk),
                                )
                                .unwrap(),
                            );
                            ...
                            let mut signer = FunaiTransactionSigner::new(&tx_stx_transfer);
                        ...
                        mblock_txs.push(tx);
                        let microblock = FunaiMicroblockBuilder::make_next_microblock_from_txs(
                            mblock_txs,
                            &microblock_privkey,
            ...
            peer.process_funai_epoch_at_tip(&funai_block, &microblocks);

            header_hashes.push((consensus_hash, Some(funai_block.block_hash())));
        }

        let total_reward_cycles = peer_config
            .burnchain
            .block_height_to_reward_cycle(last_funai_block_height)
            .unwrap();
        let mut chainstate = FunaiChainState::open(false, 0x80000000, &chainstate_path, None)
            .unwrap()
            .0;

        test_debug!(
            "first, last block heights are {}, {}. Total reward cycles: {}",
            first_funai_block_height,
            last_funai_block_height,
            total_reward_cycles
        );
        ...
        for i in 0..total_reward_cycles {
            ...
            for block_height in start_range..end_range {
                ...
                if block_height > first_funai_block_height
                    && block_height <= last_funai_block_height
                {
                    assert!(blocks_inv.has_ith_block((block_height - start_range) as u16));
                    if block_height > first_funai_block_height + 1 {
    ...
    fn test_get_parent_block_header() {
        ...
        let mut last_parent_opt: Option<FunaiBlock> = None;
        for tenure_id in 0..num_blocks {
            ...
            if tenure_id == 0 {
                let parent_header_opt = FunaiChainState::load_parent_block_header(
                ...
                let upper_bound_header =
                    FunaiChainState::get_funai_block_header_info_by_index_block_hash(
                        peer.chainstate().db(),
                        &chain_tip_index_hash,
                    )
                    .unwrap()
                    .unwrap();
                let ancestors = FunaiChainState::get_ancestors_headers(
                    peer.chainstate().db(),
                    upper_bound_header,
                    0,
                )
    ...
    #[test]
    fn funai_db_staging_microblocks_fork() {
    ...
    #[test]
    fn funai_db_staging_microblocks_multiple_forks() {
    ...
    /// Funai 2.1.  That is, they're up for consideration in the 6 subsequent sortiitons after
    /// they are mined (including the one they are in).  This test verifies that TransferSTX & DelegateSTX
    /// operations are picked up and applied as expected in the given Funai fork, even though
    ...
    fn test_get_stacking_and_transfer_and_delegate_burn_ops_v210() {
        ...
        let del_addrs: Vec<_> = (0..num_blocks)
            .map(|_| {
                FunaiAddress::from_public_keys(
                    C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
                    &AddressHashMode::SerializeP2PKH,
                    1,
                    &vec![FunaiPublicKey::from_private(&FunaiPrivateKey::new())],
                )
                .unwrap()
            })
            .collect();

        let recipient_privk = FunaiPrivateKey::new();
        let recipient_addr = FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(&recipient_privk)],
        )
        .unwrap();
        ...
        let mut epochs = FunaiEpoch::unit_test_2_1(0);
        ...
        let mut last_block_id = FunaiBlockId([0x00; 32]);
        for tenure_id in 0..num_blocks {
            ...
            // For the first 5 burn blocks, sortition a Funai block.
            // For sortitions 6 and 8, don't sortition any Funai block.
            // For sortitions 7 and 9, do sortition a Funai block, and verify that it includes all
            // burnchain STX operations that got skipped by the missing sortition.
            let process_funai_block = tenure_id <= 5 || tenure_id % 2 != 0;

            let (mut burn_ops, funai_block_opt, microblocks_opt) = if process_funai_block {
                let (burn_ops, funai_block, microblocks) = peer.make_tenure(
                    |ref mut miner,
                     ref mut sortdb,
                     ref mut chainstate,
                     ...
                     ref parent_microblock_header_opt| {
                        let parent_tip = match parent_opt {
                            None => {
                                FunaiChainState::get_genesis_header_info(chainstate.db()).unwrap()
                            }
                            Some(block) => {
                                let ic = sortdb.index_conn();
                                let snapshot =
                ...
                (burn_ops, Some(funai_block), Some(microblocks))
            } else {
                (vec![], None, None)
            };
            ...
            match (funai_block_opt, microblocks_opt) {
                (Some(funai_block), Some(microblocks)) => {
                    peer.process_funai_epoch_at_tip(&funai_block, &microblocks);
                    last_block_id = FunaiBlockHeader::make_index_block_hash(
                        &consensus_hash,
                        &funai_block.block_hash(),
                    );
                }
        ...
        let funai_tip =
            NakamotoChainState::get_canonical_block_header(peer.chainstate().db(), &sortdb)
                .unwrap()
                .unwrap();
        assert_eq!(funai_tip.anchored_header.height(), 8);

        // but we did process all burnchain operations
        let (consensus_hash, block_bhh) =
            SortitionDB::get_canonical_funai_chain_tip_hash(sortdb.conn()).unwrap();
        let tip_hash = FunaiBlockHeader::make_index_block_hash(&consensus_hash, &block_bhh);
        let account = peer
            .chainstate()
            .with_read_only_clarity_tx(&sortdb.index_conn(), &tip_hash, |conn| {
                FunaiChainState::get_account(conn, &addr.to_account_principal())
            })
            .unwrap();
        peer.sortdb.replace(sortdb);
    ...
    /// between when they are mined and when the next Funai block is mined.
    #[test]
    fn test_get_stacking_and_transfer_and_delegate_burn_ops_v210_expiration() {
        ...
        for tenure_id in 0..num_blocks {
            ...
            // For the first 5 burn blocks, sortition a Funai block.
            let process_funai_block = tenure_id <= 5 || tenure_id >= 13;
        ...
        let funai_tip =
            NakamotoChainState::get_canonical_block_header(peer.chainstate().db(), &sortdb)
                .unwrap()
                .unwrap();
        assert_eq!(funai_tip.anchored_header.height(), 13);
