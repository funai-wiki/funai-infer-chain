...
use clarity::vm::types::{
    ...
    FunaiAddressExtensions as ClarityFunaiAddressExt, StandardPrincipalData, TupleData,
    TypeSignature, Value,
};
...
use crate::chainstate::funai::{Error, FunaiMicroblockHeader, *};
...
use crate::util_lib::strings::{FunaiString, VecDisplay};
...
impl FunaiTransactionReceipt {
    pub fn from_stx_transfer(
        tx: FunaiTransaction,
        events: Vec<FunaiTransactionEvent>,
        result: Value,
        cost: ExecutionCost,
    ) -> FunaiTransactionReceipt {
        FunaiTransactionReceipt {
            events,
            result,
            stx_burned: 0,
    ...
    pub fn from_coinbase(tx: FunaiTransaction) -> FunaiTransactionReceipt {
        FunaiTransactionReceipt {
            transaction: tx.into(),
            events: vec![],
            post_condition_aborted: false,
    ...
    pub fn from_tenure_change(tx: FunaiTransaction) -> FunaiTransactionReceipt {
        FunaiTransactionReceipt {
            transaction: tx.into(),
            events: vec![],
            post_condition_aborted: false,
    ...
    pub fn is_coinbase_tx(&self) -> bool {
        if let TransactionOrigin::Funai(ref transaction) = self.transaction {
...
impl<T> From<(TransactionNonceMismatch, T)> for Error {
    fn from(e: (TransactionNonceMismatch, T)) -> Error {
        Error::InvalidFunaiTransaction(e.0.to_string(), e.0.quiet)
    }
}
...
pub enum ClarityRuntimeTxError {
    ...
    AbortedByCallback(Option<Value>, AssetMap, Vec<FunaiTransactionEvent>),
...
impl FunaiChainState {
    ...
    fn get_payer_account<T: ClarityConnection>(
        ...
        let payer_account = if let Some(sponsor_address) = tx.sponsor_address() {
            let payer_account = FunaiChainState::get_account(clarity_tx, &sponsor_address.into());
            payer_account
        } else {
            let origin_account =
                FunaiChainState::get_account(clarity_tx, &tx.origin_address().into());
            origin_account
        };
    ...
    /// Check the account nonces for the supplied funai transaction,
    ...
    pub fn check_transaction_nonces<T: ClarityConnection>(
        ...
        (FunaiAccount, FunaiAccount),
        (TransactionNonceMismatch, (FunaiAccount, FunaiAccount)),
    > {
        // who's sending it?
        let origin = tx.get_origin();
        let origin_account = FunaiChainState::get_account(clarity_tx, &tx.origin_address().into());
    ...
    fn pay_transaction_fee(
        ...
        FunaiChainState::account_debit(clarity_tx, &payer_account.principal, fee);
        Ok(fee)
    }
    ...
    fn check_microblock_header_signer(
        mblock_hdr_1: &FunaiMicroblockHeader,
        mblock_hdr_2: &FunaiMicroblockHeader,
    ) -> Result<Hash160, Error> {
        let pkh1 = mblock_hdr_1.check_recover_pubkey().map_err(|e| {
            Error::InvalidFunaiTransaction(
    ...
    pub fn handle_poison_microblock(
        env: &mut Environment,
        mblock_header_1: &FunaiMicroblockHeader,
        mblock_header_2: &FunaiMicroblockHeader,
    ) -> Result<Value, Error> {
        ...
        let pubkh =
            FunaiChainState::check_microblock_header_signer(mblock_header_1, mblock_header_2)?;
        ...
        let mblock_pubk_height = match microblock_height_opt {
            None => {
                ...
                let msg = format!(
                    "Invalid Funai transaction: microblock public key hash {} never seen in this fork",
                    &pubkh
                );
                warn!("{}", &msg;
            ...
            Some(height) => {
                if height
                    ...
                    < current_height
                {
                    let msg = format!("Invalid Funai transaction: microblock public key hash from height {} has matured relative to current height {}", height, current_height);
                    warn!("{}", &msg;
                          "microblock_pubkey_hash" => %pubkh
                    );
    ...
    ///
    /// NOTE: this does not verify that the transaction can be processed in the clarity_tx's Funai
    /// epoch.  This check must be performed by the caller before processing the block, e.g. via
    /// FunaiBlock::validate_transactions_static().
    ///
    /// Returns the funai transaction receipt
        ...
        match tx.payload {
            TransactionPayload::TokenTransfer(ref addr, ref amount, ref memo) => {
                ...
                if tx.post_conditions.len() > 0 {
                    let msg = format!("Invalid Funai transaction: TokenTransfer transactions do not support post-conditions");
                    warn!("{}", &msg);
                ...
                let receipt = FunaiTransactionReceipt::from_stx_transfer(
                    tx.clone(),
                    events,
                    value,
                    total_cost,
            ...
            TransactionPayload::ContractCall(ref contract_call) => {
                ...
                let contract_call_resp = clarity_tx.run_contract_call(
                    ...
                    |asset_map, _| {
                        !FunaiChainState::check_transaction_postconditions(
                            &tx.post_conditions,
                    ...
                    Err(e) => match handle_clarity_runtime_error(e) {
                        ...
                        ClarityRuntimeTxError::AbortedByCallback(value, assets, events) => {
                            info!("Contract-call aborted by post-condition";
                                      ...
                                      "function_name" => %contract_call.function_name,
                                      "function_args" => %VecDisplay(&contract_call.function_args));
                            let receipt = FunaiTransactionReceipt::from_condition_aborted_contract_call(
                                    tx.clone(),
                                    events,
                        ...
                        ClarityRuntimeTxError::AnalysisError(check_error) => {
                            if epoch_id >= FunaiEpochId::Epoch21 {
                                ...
                                let receipt =
                                    FunaiTransactionReceipt::from_runtime_failure_contract_call(
                                        tx.clone(),
                                        total_cost,
                                        check_error,
                                    );
                ...
                let receipt = FunaiTransactionReceipt::from_contract_call(
                    tx.clone(),
                    events,
                    result,
            ...
            TransactionPayload::SmartContract(ref smart_contract, ref version_opt) => {
                ...
                if FunaiChainState::get_contract(clarity_tx, &contract_id)?.is_some() {
                    ...
                    Err(e) => {
                        match e {
                            ...
                            other_error => {
                                ...
                                let receipt = FunaiTransactionReceipt::from_analysis_failure(
                                    tx.clone(),
                                    analysis_cost,
                                    other_error,
                                );
                ...
                let (asset_map, events) = match initialize_resp {
                    ...
                    Err(e) => match handle_clarity_runtime_error(e) {
                        ...
                        ClarityRuntimeTxError::AbortedByCallback(_, assets, events) => {
                            let receipt =
                                FunaiTransactionReceipt::from_condition_aborted_smart_contract(
                                    tx.clone(),
                                    events,
                                    assets.get_stx_burned_total()?,
                                    contract_analysis,
                        ...
                        ClarityRuntimeTxError::AnalysisError(check_error) => {
                            if epoch_id >= FunaiEpochId::Epoch21 {
                                ...
                                let receipt =
                                    FunaiTransactionReceipt::from_runtime_failure_smart_contract(
                                        tx.clone(),
                                        total_cost,
                                        contract_analysis,
                                        check_error,
                ...
                let receipt = FunaiTransactionReceipt::from_smart_contract(
                    tx.clone(),
                    events,
                    asset_map.get_stx_burned_total()?,
                    contract_analysis,
            ...
            TransactionPayload::PoisonMicroblock(ref mblock_header_1, ref mblock_header_2) => {
                ...
                // Their presence in this variant makes the transaction invalid.
                if tx.post_conditions.len() > 0 {
                    let msg = format!("Invalid Funai transaction: PoisonMicroblock transactions do not support post-conditions");
                    warn!("{}", &msg);
                ...
                let receipt =
                    FunaiTransactionReceipt::from_poison_microblock(tx.clone(), res, cost);
            ...
            TransactionPayload::Coinbase(..) => {
                ...
                let receipt = FunaiTransactionReceipt::from_coinbase(tx.clone());
                Ok(receipt)
            }
            TransactionPayload::TenureChange(ref payload) => {
                ...
                // Their presence in this variant makes the transaction invalid.
                if tx.post_conditions.len() > 0 {
                    let msg = format!("Invalid Funai transaction: TenureChange transactions do not support post-conditions");
                    warn!("{msg}");
                ...
                match payload.cause {
                    ...
                    TenureChangeCause::Extended => {
                        // the funais granted a tenure extension.
            ...
            TransactionPayload::Infer(ref from, ref amount, _, _, ref node_principal, _, ref output_hash) => {
                ...
                // Check if the sender has enough balance for the amount
                let total_spent = u128::from(*amount) + u128::from(tx.get_tx_fee());
                
                // Get current block height and unlock heights for balance check
                let (block_height, v1_unlock_height, v2_unlock_height, v3_unlock_height) = clarity_tx
                    .with_clarity_db_readonly(|ref mut db| {
                        Ok((
                ...
                if !origin_account.stx_balance.can_transfer_at_burn_block(
                    total_spent,
                    block_height,
                    v1_unlock_height,
                    v2_unlock_height,
                    ...
                    let msg = format!("Invalid Infer: insufficient balance for amount {}", amount);
                    warn!("{}", &msg);
                    return Err(Error::InvalidFunaiTransaction(msg, false));
                }
            ...
            TransactionPayload::RegisterModel(_, _) => {
                // No Clarity state change; just accept to persist in libllm DB at POST time.
                // Keep receipt minimal.
                let cost_before = clarity_tx.cost_so_far();
                let total_cost = clarity_tx.cost_so_far();
                let mut delta = total_cost.clone();
                delta.sub(&cost_before).expect("BUG: cost decreased");
                let receipt = FunaiTransactionReceipt::from_tenure_change(tx.clone());
                Ok(receipt)
            }
        }
    ...
    pub fn process_transaction(
        ...
        FunaiChainState::process_transaction_precheck(&clarity_block.config, tx)?;

        // what version of Clarity did the transaction caller want? And, is it valid now?
        let clarity_version = FunaiChainState::get_tx_clarity_version(clarity_block, tx)?;
        if clarity_version == ClarityVersion::Clarity2 {
            // requires 2.1 and higher
            if clarity_block.get_epoch() < FunaiEpochId::Epoch21 {
                let msg = format!("Invalid transaction {}: asks for Clarity2, but not in Funai epoch 2.1 or later", tx.txid());
                warn!("{}", &msg);
                return Err(Error::InvalidFunaiTransaction(msg, false));
            }
        }
        ...
        let tx_receipt = if epoch >= FunaiEpochId::Epoch21 {
            ...
            let (_origin_account, payer_account) =
                FunaiChainState::check_transaction_nonces(&mut transaction, tx, quiet)?;

            let payer_address = payer_account.principal.clone();
            let payer_nonce = payer_account.nonce;
            FunaiChainState::pay_transaction_fee(&mut transaction, fee, payer_account)?;

            // origin balance may have changed (e.g. if the origin paid the tx fee), so reload the account
            let origin_account =
                FunaiChainState::get_account(&mut transaction, &tx.origin_address().into());

            let tx_receipt = FunaiChainState::process_transaction_payload(
                &mut transaction,
                tx,
                &origin_account,
                ast_rules,
            ...
            FunaiChainState::update_account_nonce(
                &mut transaction,
                &origin_account.principal,
            ...
            FunaiChainState::pay_transaction_fee(&mut transaction, fee, new_payer_account)?;
...
pub mod test {
    ...
    use crate::chainstate::funai::index::storage::*;
    use crate::chainstate::funai::index::*;
    use crate::chainstate::funai::{Error, *};
    ...
    fn contract_publish_runtime_error() {
        ...
        genesis.as_transaction(|tx_conn| {
            ...
            tx_conn
                .with_clarity_db(|db| {
                    db.set_clarity_epoch_version(FunaiEpochId::Epoch21)
                        .unwrap();
                    Ok(())
                })
                .unwrap();
        ...
        let receipt = FunaiChainState::process_transaction_payload(
            &mut tx_conn,
            &tx,
            &FunaiAccount {
    ...
    fn process_token_transfer_stx_transaction() {
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            let _account = FunaiChainState::get_account(&mut conn, &addr.to_account_principal());
            let recv_account =
                FunaiChainState::get_account(&mut conn, &recv_addr.to_account_principal());
            ...
            let recv_account = FunaiChainState::get_account(&mut conn, &recv_addr);
            ...
            let recv_account_after = FunaiChainState::get_account(&mut conn, &recv_addr);
            assert_eq!(recv_account_after.nonce, 0);
    ...
    fn process_token_transfer_stx_transaction_invalid() {
        ...
        .unwrap();
        let privk_sponsor = FunaiPrivateKey::from_hex(
        ...
        let auth = TransactionAuth::from_p2pkh(&privk).unwrap();
        let addr = auth.origin().address_testnet();
        let sponsor_addr = FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(&privk_sponsor)],
        )
        .unwrap();
        ...
        let mut tx_stx_transfer_same_receiver = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::TokenTransfer(
                recv_addr.clone().into(),
        ...
        let mut tx_stx_transfer_wrong_network = FunaiTransaction::new(
            TransactionVersion::Mainnet,
            auth.clone(),
            TransactionPayload::TokenTransfer(
        ...
        let mut tx_stx_transfer_wrong_chain_id = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::TokenTransfer(
                sponsor_addr.clone().into(),
        ...
        let mut tx_stx_transfer_postconditions = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::TokenTransfer(
                sponsor_addr.clone().into(),
        ...
        let mut tx_stx_transfer_wrong_nonce = FunaiTransaction::new(
            TransactionVersion::Testnet,
            wrong_nonce_auth,
            TransactionPayload::TokenTransfer(
                recv_addr.clone().into(),
        ...
        let mut tx_stx_transfer_wrong_nonce_sponsored = FunaiTransaction::new(
            TransactionVersion::Testnet,
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            .zip(error_frags.clone())
            {
                ...
                match res {
                    Err(Error::InvalidFunaiTransaction(msg, false)) => {
    ...
    fn process_token_transfer_stx_sponsored_transaction() {
        ...
        let privk_origin = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e001",
        )
        .unwrap();
        let privk_sponsor = FunaiPrivateKey::from_hex(
    ...
    fn process_smart_contract_transaction() {
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_contract_call);
        signer.sign_origin(&privk).unwrap();
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            let contract_before_res =
                FunaiChainState::get_contract(&mut conn, &contract_id).unwrap();
            ...
            let contract_res = FunaiChainState::get_contract(&mut conn, &contract_id);
    ...
    fn process_smart_contract_transaction_invalid() {
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            for i in 0..contracts.len() {
                ...
                let mut signer = FunaiTransactionSigner::new(&tx_contract);
                signer.sign_origin(&privk).unwrap();
    ...
    fn process_smart_contract_transaction_syntax_error() {
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            for i in 0..contracts.len() {
                ...
                let expected_error =
                    if burn_db.get_funai_epoch(0).unwrap().epoch_id == FunaiEpochId::Epoch21 {
    ...
    fn process_smart_contract_contract_call_transaction() {
        ...
        // contract-call
        let privk_2 = FunaiPrivateKey::from_hex(
        ...
        let mut signer_2 = FunaiTransactionSigner::new(&tx_contract_call);
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            let contract_res = FunaiChainState::get_contract(&mut conn, &contract_id).unwrap();
            let var_res = FunaiChainState::get_data_var(&mut conn, &contract_id, "bar").unwrap();
    ...
    fn process_smart_contract_contract_call_sponsored_transaction() {
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            let account_publisher =
                FunaiChainState::get_account(&mut conn, &addr_publisher.to_account_principal());
            assert_eq!(account_publisher.nonce, 0);

            let account_origin =
                FunaiChainState::get_account(&mut conn, &addr_origin.to_account_principal());
    ...
    fn process_post_conditions_tokens() {
        ...
        .unwrap();
        let privk_recipient = FunaiPrivateKey::from_hex(
            "7e3af4db6af6b3c67e2c6c6d7d5983b519f4d9b3a6e00580ae96dcace3bde8bc01",
        )
        .unwrap();
        let auth_origin = TransactionAuth::from_p2pkh(&privk_origin).unwrap();
        ...
        let recv_addr = FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(&privk_recipient)],
        )
        .unwrap();
        ...
        let mut tx_contract_call_stackaroos = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth_origin.clone(),
            TransactionPayload::new_contract_call(
                addr_publisher.clone(),
        ...
        .iter()
        {
            ...
            let mut signer = FunaiTransactionSigner::new(&tx_contract_call_pass);
            signer.sign_origin(&privk_origin).unwrap();
        ...
        let mut tx_contract_call_user_stackaroos = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth_recv.clone(),
            TransactionPayload::new_contract_call(
                addr_publisher.clone(),
        ...
        for (_i, pass_condition) in [NonfungibleConditionCode::Sent].iter().enumerate() {
            ...
            let mut tx_contract_call_names = FunaiTransaction::new(
                TransactionVersion::Testnet,
                auth_origin.clone(),
                TransactionPayload::new_contract_call(
                    addr_publisher.clone(),
            ...
            let mut signer = FunaiTransactionSigner::new(&tx_contract_call_names);
            signer.sign_origin(&privk_origin).unwrap();
        ...
        for fail_condition in [FungibleConditionCode::SentLt, FungibleConditionCode::SentGt].iter()
        {
            ...
            let mut signer = FunaiTransactionSigner::new(&tx_contract_call_fail);
            signer.sign_origin(&privk_origin).unwrap();
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            let _ = FunaiChainState::get_account_ft(
                &mut conn,
                &contract_id,
                "stackaroos",
                &recv_principal,
            ...
            let account_stackaroos_balance = FunaiChainState::get_account_ft(
                &mut conn,
                &contract_id,
                "stackaroos",
                &recv_principal,
            ...
            for tx_pass in post_conditions_pass.iter() {
                ...
                let account_recipient_stackaroos_after = FunaiChainState::get_account_ft(
                    &mut conn,
                    &contract_id,
                    "stackaroos",
                    &recv_principal,
                ...
                let account_publisher_after = FunaiChainState::get_account(
                    &mut conn,
            ...
            for tx_pass in post_conditions_pass_payback.iter() {
                ...
                let account_pub_stackaroos_after = FunaiChainState::get_account_ft(
                    &mut conn,
                    &contract_id,
                    "stackaroos",
                    &addr_principal,
            ...
            for (_i, tx_pass) in post_conditions_pass_nft.iter().enumerate() {
                ...
                let account_recipient_names_after = FunaiChainState::get_account_nft(
                    &mut conn,
                    &contract_id,
                    "names",
                    &expected_value,
            ...
            for (_i, tx_fail) in post_conditions_fail_nft.iter().enumerate() {
                ...
                let res = FunaiChainState::get_account_nft(
                    &mut conn,
                    &contract_id,
                    "names",
                    &expected_value,
    ...
    fn process_post_conditions_tokens_deny() {
        ...
        .enumerate()
        {
            ...
            let mut tx_contract_call_both = FunaiTransaction::new(
                TransactionVersion::Testnet,
                auth_origin.clone(),
                TransactionPayload::new_contract_call(
                    addr_publisher.clone(),
            ...
            let mut signer = FunaiTransactionSigner::new(&tx_contract_call_both);
            signer.sign_origin(&privk_origin).unwrap();
            post_conditions_pass.push(signer.get_tx().unwrap());
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            for (_i, tx_pass) in post_conditions_pass_payback.iter().enumerate() {
                ...
                let account_publisher_names_after = FunaiChainState::get_account_nft(
                    &mut conn,
                    &contract_id,
                    "names",
                    &expected_value,
...
"
    ...
    fn make_account(principal: &PrincipalData, nonce: u64, balance: u128) -> FunaiAccount {
    ...
    fn test_check_postconditions_multiple_fts() {
        ...
        let contract_addr = FunaiAddress {
            version: 1,
        ...
        for test in tests {
            ...
            let result = FunaiChainState::check_transaction_postconditions(
                post_conditions,
                mode,
                origin,
                &ft_transfer_2,
    ...
    fn process_smart_contract_fee_check() {
        ...
        let mut tx_contract_create = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"hello-world".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_contract_create);
        signer.sign_origin(&privk).unwrap();
            ...
            TransactionPayload::new_contract_call(
                ...
                vec![
                    ...
                    Value::Principal(PrincipalData::from(
                        FunaiAddress::from_string("ST1H1B54MY50RMBRRKS7GV2ZWG79RZ1RQ1ETW4E01")
                            .unwrap(),
                    )),
                ],
            )
        ...
        assert_eq!(fee, 1);
        assert_eq!(
            FunaiChainState::get_account(&mut conn, &addr.into())
                .stx_balance
    ...
    fn process_poison_microblock_same_block() {
        ...
        let block_privk = FunaiPrivateKey::from_hex(
            "2f90f1b148207a110aa58d1b998510407420d7a8065d4fdfc0bbe22c5d9f1c6a01",
        )
        .unwrap();

        let block_pubkh =
            Hash160::from_node_public_key(&FunaiPublicKey::from_private(&block_privk));

        let reporter_privk = FunaiPrivateKey::from_hex(
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            FunaiChainState::insert_microblock_pubkey_hash(&mut conn, 1, &block_pubkh).unwrap();

            let height_opt =
                FunaiChainState::has_microblock_pubkey_hash(&mut conn, &block_pubkh).unwrap();
            ...
            let mut tx_poison_microblock = FunaiTransaction::new(
                TransactionVersion::Testnet,
            ...
            let mut signer = FunaiTransactionSigner::new(&tx_poison_microblock);
            ...
            // sequence.
            let report_opt = FunaiChainState::get_poison_microblock_report(&mut conn, 1).unwrap();
    ...
    fn process_poison_microblock_multiple_same_block() {
        ...
        let reporter_privk_1 = FunaiPrivateKey::from_hex(
            "e606e944014b2a9788d0e3c8defaf6bc44b1e3ab881aaba32faa6e32002b7e1f01",
        )
        .unwrap();
        let reporter_privk_2 = FunaiPrivateKey::from_hex(
        ...
        for (dbi, burn_db) in ALL_BURN_DBS.iter().enumerate() {
            ...
            // report poison (in the same block)
            let mut tx_poison_microblock_1 = FunaiTransaction::new(
                TransactionVersion::Testnet,
            ...
            let mut signer = FunaiTransactionSigner::new(&tx_poison_microblock_1);
            ...
            // report poison (in the same block)
            let mut tx_poison_microblock_2 = FunaiTransaction::new(
                TransactionVersion::Testnet,
            ...
            let mut signer = FunaiTransactionSigner::new(&tx_poison_microblock_2);
    ...
    fn test_get_tx_clarity_version_v205() {
        ...
        impl BurnStateDB for MockedBurnDB {
            ...
            fn get_funai_epoch(&self, height: u32) -> Option<FunaiEpoch> {
                Some(match height {
                    0 => FunaiEpoch {
                        epoch_id: FunaiEpochId::Epoch2_05,
                        start_height: 1,
            ...
            fn get_funai_epoch_by_epoch_id(
                &self,
                epoch_id: &FunaiEpochId,
            ) -> Option<FunaiEpoch> {
                match epoch_id {
                    FunaiEpochId::Epoch10 => Some(FunaiEpoch {
                    ...
                    FunaiEpochId::Epoch20 => self.get_funai_epoch(0),
                    FunaiEpochId::Epoch2_05 => self.get_funai_epoch(1),
                    FunaiEpochId::Epoch21 => self.get_funai_epoch(2),
                    FunaiEpochId::Epoch22 => self.get_funai_epoch(3),
                    FunaiEpochId::Epoch23 => self.get_funai_epoch(4),
                    FunaiEpochId::Epoch24 => self.get_funai_epoch(5),
                    FunaiEpochId::Epoch25 => self.get_funai_epoch(6),
                    FunaiEpochId::Epoch30 => self.get_funai_epoch(7),
                }
            }
        ...
        let smart_contract = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::SmartContract(
                TransactionSmartContract {
                    name: ContractName::try_from("hello-world").unwrap(),
                    code_body: FunaiString::from_str("(print \"hello\")").unwrap(),
                },
                None,
            ),
        );
        let smart_contract_v1 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::SmartContract(
                TransactionSmartContract {
        ...
        let smart_contract_v2 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::SmartContract(
                TransactionSmartContract {
        ...
        let token_transfer = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::TokenTransfer(
                recv_addr.clone().into(),
        ...
        for mut tx in txs.into_iter() {
            ...
            let mut signer = FunaiTransactionSigner::new(&tx);
            signer.sign_origin(&privk).unwrap();
            let signed_tx = signer.get_tx().unwrap();
        ...
        assert_eq!(conn.get_epoch(), FunaiEpochId::Epoch2_05);
        assert_eq!(
            ClarityVersion::Clarity1,
            FunaiChainState::get_tx_clarity_version(&mut conn, &smart_contract).unwrap()
        );
        assert_eq!(
            ClarityVersion::Clarity1,
            FunaiChainState::get_tx_clarity_version(&mut conn, &smart_contract_v1).unwrap()
        );
        assert_eq!(
            ClarityVersion::Clarity2,
            FunaiChainState::get_tx_clarity_version(&mut conn, &smart_contract_v2).unwrap()
        );
        assert_eq!(
            ClarityVersion::Clarity1,
            FunaiChainState::get_tx_clarity_version(&mut conn, &token_transfer).unwrap()
        );

        // verify that 2.1 gating is applied for clarity2
        if let Err(Error::InvalidFunaiTransaction(msg, ..)) = FunaiChainState::process_transaction(
            &mut conn,
            &smart_contract_v2,
            false,
            ASTRules::PrecheckSize,
    ...
    fn test_get_tx_clarity_version_v210() {
        ...
        assert_eq!(conn.get_epoch(), FunaiEpochId::Epoch21);
        assert_eq!(
            ClarityVersion::Clarity2,
            FunaiChainState::get_tx_clarity_version(&mut conn, &smart_contract).unwrap()
        );
    ...
    fn process_fee_gating() {
        ...
        let privk_recv = FunaiPrivateKey::from_hex(
    ...
    fn test_checkerrors_at_runtime() {
        ...
        let mut tx_runtime_checkerror_trait = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"foo".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_runtime_checkerror_trait);
        signer.sign_origin(&privk).unwrap();

        let signed_runtime_checkerror_trait_tx = signer.get_tx().unwrap();

        let mut tx_runtime_checkerror_impl = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"foo-impl".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_runtime_checkerror_impl);
        signer.sign_origin(&privk).unwrap();

        let signed_runtime_checkerror_impl_tx = signer.get_tx().unwrap();

        let mut tx_runtime_checkerror_clar1 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_runtime_checkerror_clar1);
        signer.sign_origin(&privk).unwrap();

        let signed_runtime_checkerror_tx_clar1 = signer.get_tx().unwrap();

        let mut tx_runtime_checkerror_clar2 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"trait-checkerror".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_runtime_checkerror_clar2);
        signer.sign_origin(&privk).unwrap();

        let signed_runtime_checkerror_tx_clar2 = signer.get_tx().unwrap();

        let mut tx_test_trait_checkerror = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_contract_call(
                addr.clone(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_test_trait_checkerror);
        signer.sign_origin(&privk).unwrap();

        let signed_test_trait_checkerror_tx = signer.get_tx().unwrap();

        let mut tx_runtime_checkerror_cc_contract_clar1 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_runtime_checkerror_cc_contract_clar1);
        signer.sign_origin(&privk).unwrap();

        let signed_runtime_checkerror_cc_contract_tx_clar1 = signer.get_tx().unwrap();

        let mut tx_runtime_checkerror_cc_contract_clar2 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"trait-checkerror-cc".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_runtime_checkerror_cc_contract_clar2);
        signer.sign_origin(&privk).unwrap();
        ...
        let acct = FunaiChainState::get_account(&mut conn, &addr.into());
        ...
        let (fee, tx_receipt) = FunaiChainState::process_transaction(
            &mut conn,
            &signed_test_trait_checkerror_tx,
            false,
            ASTRules::PrecheckSize,
        ...
        let executed_var =
            FunaiChainState::get_data_var(&mut conn, &contract_id, "executed").unwrap();
    ...
    fn test_embedded_trait() {
        ...
        let mut tx_foo_trait = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"foo".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_foo_trait);
        signer.sign_origin(&privk).unwrap();

        let signed_foo_trait_tx = signer.get_tx().unwrap();

        let mut tx_foo_impl = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"foo-impl".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_foo_impl);
        signer.sign_origin(&privk).unwrap();

        let signed_foo_impl_tx = signer.get_tx().unwrap();

        let mut tx_call_foo_clar1 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"call-foo".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_call_foo_clar1);
        signer.sign_origin(&privk).unwrap();

        let signed_call_foo_tx_clar1 = signer.get_tx().unwrap();

        let mut tx_call_foo_clar2 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"call-foo".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_call_foo_clar2);
        signer.sign_origin(&privk).unwrap();

        let signed_call_foo_tx_clar2 = signer.get_tx().unwrap();

        let mut tx_test_call_foo = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_contract_call(
                addr.clone(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_test_call_foo);
        signer.sign_origin(&privk).unwrap();
    ...
    fn test_transitive_trait() {
        ...
        let mut tx_transitive_trait_clar1 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_transitive_trait_clar1);
        signer.sign_origin(&privk).unwrap();

        let signed_transitive_trait_clar1_tx = signer.get_tx().unwrap();

        let mut tx_transitive_trait_clar2 = FunaiTransaction::new(
            TransactionVersion::Testnet,
            auth.clone(),
            TransactionPayload::new_smart_contract(
                &"transitive".to_string(),
        ...
        let mut signer = FunaiTransactionSigner::new(&tx_transitive_trait_clar2);
        signer.sign_origin(&privk).unwrap();
