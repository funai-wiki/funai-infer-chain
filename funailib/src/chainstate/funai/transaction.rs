...
use funai_common::codec::{read_next, write_next, Error as codec_error, FunaiMessageCodec};
use funai_common::types::chainstate::FunaiAddress;
use funai_common::types::FunaiPublicKeyBuffer;
use funai_common::util::hash::{to_hex, MerkleHashFunc, MerkleTree, Sha512Trunc256Sum};
...
use crate::chainstate::funai::{TransactionPayloadID, *};
use crate::codec::Error as CodecError;
use crate::core::*;
use crate::net::Error as net_error;

impl FunaiMessageCodec for TransactionContractCall {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<TransactionContractCall, codec_error> {
        let address: FunaiAddress = read_next(fd)?;
        ...
        if !FunaiString::from(function_name.clone()).is_clarity_variable() {
...
impl FunaiMessageCodec for TransactionSmartContract {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<TransactionSmartContract, codec_error> {
        ...
        let code_body: FunaiString = read_next(fd)?;
...
impl FunaiMessageCodec for TenureChangeCause {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        let byte = (*self) as u8;
        write_next(fd, &byte)
    }
...
impl FunaiMessageCodec for ThresholdSignature {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
...
impl FunaiMessageCodec for TenureChangePayload {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
...
impl FunaiMessageCodec for TransactionPayload {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        match self {
            ...
            TransactionPayload::Coinbase(buf, recipient_opt, vrf_opt) => {
                match (recipient_opt, vrf_opt) {
                    (None, None) => {
                        // funai 2.05 and earlier only use this path
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<TransactionPayload, codec_error> {
        ...
        let payload = match type_id {
            ...
            TransactionPayloadID::PoisonMicroblock => {
                let h1: FunaiMicroblockHeader = read_next(fd)?;
                let h2: FunaiMicroblockHeader = read_next(fd)?;
            ...
            TransactionPayloadID::RegisterModel => {
                let model_name: InferLPString = read_next(fd)?;
                let model_params: InferLPString = read_next(fd)?;
                TransactionPayload::RegisterModel(model_name, model_params)
            }
        };
...
impl<'a, H> FromIterator<&'a FunaiTransaction> for MerkleTree<H>
where
    H: MerkleHashFunc + Clone + PartialEq + fmt::Debug,
{
    fn from_iter<T: IntoIterator<Item = &'a FunaiTransaction>>(iter: T) -> Self {
        let txid_vec = iter
            .into_iter()
...
impl FunaiMessageCodec for AssetInfo {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<AssetInfo, codec_error> {
        let contract_address: FunaiAddress = read_next(fd)?;
        let contract_name: ContractName = read_next(fd)?;
...
impl FunaiMessageCodec for PostConditionPrincipal {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        match *self {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<PostConditionPrincipal, codec_error> {
        ...
        let principal = match principal_id {
            ...
            x if x == PostConditionPrincipalID::Standard as u8 => {
                let addr: FunaiAddress = read_next(fd)?;
...
impl FunaiMessageCodec for TransactionPostCondition {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        match *self {
...
impl FunaiTransaction {
    ...
    pub fn consensus_deserialize_with_len<R: Read>(
        fd: &mut R,
    ) -> Result<(FunaiTransaction, u64), codec_error> {
...
impl FunaiMessageCodec for FunaiTransaction {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiTransaction, codec_error> {
        FunaiTransaction::consensus_deserialize_with_len(fd).map(|(result, _)| result)
    }
}
...
impl FunaiTransaction {
    ...
    pub fn sign_next_origin(
        ...
        let next_sighash = match self.auth {
            ...
            | TransactionAuth::Sponsored(ref mut origin_condition, _) => {
                FunaiTransaction::sign_and_append(
                    origin_condition,
                    cur_sighash,
    ...
    pub fn append_next_origin(&mut self, pubk: &FunaiPublicKey) -> Result<(), net_error> {
        match self.auth {
            TransactionAuth::Standard(ref mut origin_condition) => {
                FunaiTransaction::append_pubkey(origin_condition, pubk)
            }
            TransactionAuth::Sponsored(ref mut origin_condition, _) => {
                FunaiTransaction::append_pubkey(origin_condition, pubk)
            }
    ...
    pub fn append_next_sponsor(&mut self, pubk: &FunaiPublicKey) -> Result<(), net_error> {
        match self.auth {
            ...
            TransactionAuth::Sponsored(_, ref mut sponsor_condition) => {
                FunaiTransaction::append_pubkey(sponsor_condition, pubk)
            }
        }
    }
    ...
    pub fn origin_address(&self) -> FunaiAddress {
    ...
    pub fn sponsor_address(&self) -> Option<FunaiAddress> {
        match (&self.version, &self.auth) {
...
impl FunaiTransactionSigner {
    pub fn new(tx: &FunaiTransaction) -> FunaiTransactionSigner {
        FunaiTransactionSigner {
            tx: tx.clone(),
    ...
    ) -> Result<FunaiTransactionSigner, Error> {
    ...
    pub fn resume(&mut self, tx: &FunaiTransaction) -> () {
        self.tx = tx.clone()
    }
    ...
    pub fn sign_origin(&mut self, privk: &FunaiPrivateKey) -> Result<(), net_error> {
    ...
    pub fn append_origin(&mut self, pubk: &FunaiPublicKey) -> Result<(), net_error> {
        if self.check_overlap && self.origin_done {
    ...
    pub fn sign_sponsor(&mut self, privk: &FunaiPrivateKey) -> Result<(), net_error> {
        match self.tx.auth {
    ...
    pub fn append_sponsor(&mut self, pubk: &FunaiPublicKey) -> Result<(), net_error> {
    ...
    pub fn pop_origin_auth_field(&mut self) -> Option<TransactionAuthField> {
        match self.tx.auth {
            TransactionAuth::Standard(ref mut origin_condition) => {
                FunaiTransaction::pop_auth_field(origin_condition)
            }
            TransactionAuth::Sponsored(ref mut origin_condition, _) => {
                FunaiTransaction::pop_auth_field(origin_condition)
            }
    ...
    pub fn pop_sponsor_auth_field(&mut self) -> Option<TransactionAuthField> {
        match self.tx.auth {
            TransactionAuth::Sponsored(_, ref mut sponsor_condition) => {
                FunaiTransaction::pop_auth_field(sponsor_condition)
            }
            _ => None,
        }
    }
    ...
    pub fn get_tx_incomplete(&self) -> FunaiTransaction {
        self.tx.clone()
    }

    pub fn get_tx(&self) -> Option<FunaiTransaction> {
        if self.complete() {
            Some(self.tx.clone())
        } else {
            None
...
mod test {
    ...
    use funai_common::util::retry::{BoundReader, LogReader};

    use super::*;
    use crate::chainstate::funai::test::codec_all_transactions;
    use crate::chainstate::funai::{
        FunaiPublicKey as PubKey, C32_ADDRESS_VERSION_MAINNET_MULTISIG,
                        ...
                        TransactionSpendingCondition::Multisig(ref mut data) => {
                            let corrupt_field = match data.fields[i] {
                                TransactionAuthField::PublicKey(ref pubkey) => {
                                    TransactionAuthField::PublicKey(FunaiPublicKey::from_hex("0270790e675116a63a75008832d82ad93e4332882ab0797b0f156de9d739160a0b").unwrap())
                                },
    ...
    fn test_signature_and_corruption(
        ...
        corrupt_tx_payload.payload = match corrupt_tx_payload.payload {
            ...
            TransactionPayload::ContractCall(_) => TransactionPayload::SmartContract(
                TransactionSmartContract {
                    ...
                    code_body: FunaiString::from_str("corrupt body").unwrap(),
                },
                None,
            ),
        ...
        for i in 0..tx_bytes.len() {
            ...
            match FunaiTransaction::consensus_deserialize(&mut reader) {
                Ok(corrupt_tx) => {
    ...
    #[test]
    fn tx_funai_transaction_payload_tokens() {
        let addr = PrincipalData::from(FunaiAddress {
            version: 1,
            bytes: Hash160([0xff; 20]),
        });
    ...
    #[test]
    fn tx_funai_transaction_payload_contracts() {
        ...
        let smart_contract = TransactionSmartContract {
            ...
            code_body: FunaiString::from_str(hello_contract_body).unwrap(),
        };
    ...
    #[test]
    fn tx_funai_transaction_payload_coinbase() {
    ...
    #[test]
    fn tx_funai_transaction_payload_nakamoto_coinbase() {
    ...
    #[test]
    fn tx_funai_transaction_payload_nakamoto_coinbase_alt_recipient() {
        let proof_bytes = hex_bytes("9275df67a68c8745c0ff97b48201ee6db447f7c93b23ae24cdc2400f52fdb08a1a6ac7ec71bf9c9c76e96ee4675ebff60625af28718501047bfd87b810c2d2139b73c23bd69de66360953a642c2a330a").unwrap();
        let proof = VRFProof::from_bytes(&proof_bytes[..].to_vec()).unwrap();
    ...
    #[test]
    fn tx_funai_transaction_payload_microblock_poison() {
        let header_1 = FunaiMicroblockHeader {
            version: 0x12,
            sequence: 0x34,
        ...
        let header_2 = FunaiMicroblockHeader {
            version: 0x12,
            sequence: 0x34,
            prev_block: EMPTY_MICROBLOCK_PARENT_HASH.clone(),
    ...
    #[test]
    fn tx_funai_transaction_payload_invalid() {
        let hello_contract_call = "hello-contract-call";
        let hello_contract_name = "hello-contract-name";
        let hello_function_name = "hello-function-name";
    ...
    #[test]
    fn tx_funai_transaction_payload_invalid_contract_name() {
        // test invalid contract name
        let address = FunaiAddress {
            version: 1,
            bytes: Hash160([0xff; 20]),
        };
    ...
    #[test]
    fn tx_funai_transaction_payload_invalid_function_name() {
        // test invalid contract name
        let address = FunaiAddress {
            version: 1,
            bytes: Hash160([0xff; 20]),
    ...
    #[test]
    fn tx_funai_postcondition() {
        let tx_post_condition_principals = vec![
            PostConditionPrincipal::Origin,
            PostConditionPrincipal::Standard(FunaiAddress {
                version: 1,
    ...
    #[test]
    fn tx_funai_postcondition_invalid() {
        let addr = FunaiAddress {
            version: 1,
            bytes: Hash160([0xff; 20]),
        };
    ...
    fn tx_funai_transaction_codec() {
        ...
        for tx in all_txs.iter() {
            ...
            check_codec_and_corruption::<FunaiTransaction>(&tx, &tx_bytes);
        }
    }

    fn tx_funai_transaction_test_txs(auth: &TransactionAuth) -> Vec<FunaiTransaction> {
        let header_1 = FunaiMicroblockHeader {
            version: 0x12,
            sequence: 0x34,
            prev_block: EMPTY_MICROBLOCK_PARENT_HASH.clone(),
        ...
        let token_name = FunaiString::from_str(hello_token_name).unwrap();

        let asset_value = FunaiString::from_str("asset-value").unwrap();
        ...
        let tx_contract_call = FunaiTransaction::new(
            TransactionVersion::Mainnet,
            auth.clone(),
        ...
        let tx_stx = FunaiTransaction::new(
            TransactionVersion::Mainnet,
            auth.clone(),
        ...
        let tx_poison = FunaiTransaction::new(
            TransactionVersion::Mainnet,
            auth.clone(),
    ...
    fn check_oversign_origin_singlesig(signed_tx: &mut FunaiTransaction) -> () {
        let txid_before = signed_tx.txid();
        match signed_tx.append_next_origin(
            &FunaiPublicKey::from_hex(
    ...
    fn check_sign_no_sponsor(signed_tx: &mut FunaiTransaction) -> () {
        let txid_before = signed_tx.txid();
    ...
    fn check_oversign_sponsor_singlesig(signed_tx: &mut FunaiTransaction) -> () {
        let txid_before = signed_tx.txid();
        match signed_tx.append_next_sponsor(
            &FunaiPublicKey::from_hex(
                "03442a63b6d312710b1d6b24d803120dc6f5714352ba57907863b78de55974123c",
    ...
    fn check_oversign_origin_multisig(signed_tx: &FunaiTransaction) -> () {
    ...
    fn check_oversign_origin_multisig_uncompressed(signed_tx: &FunaiTransaction) -> () {
        let tx = signed_tx.clone();
        let privk = FunaiPrivateKey::from_hex(
        ...
        match tx_signer.pop_origin_auth_field().unwrap() {
            ...
            TransactionAuthField::PublicKey(_) => {
                tx_signer
                    .append_origin(&FunaiPublicKey::from_private(&privk))
                    .unwrap();
            }
        };
    ...
    fn check_oversign_sponsor_multisig(signed_tx: &FunaiTransaction) -> () {
        let tx = signed_tx.clone();
        let privk = FunaiPrivateKey::from_hex(
            "c6ebf45dabca8cac9a25ae39ab690743b96eb2b0960066e98ba6df50d6f9293b01",
        )
    ...
    fn check_oversign_sponsor_multisig_uncompressed(signed_tx: &FunaiTransaction) -> () {
        let tx = signed_tx.clone();
        let privk = FunaiPrivateKey::from_hex(
            "c6ebf45dabca8cac9a25ae39ab690743b96eb2b0960066e98ba6df50d6f9293b",
        )
        ...
        match tx_signer.pop_sponsor_auth_field().unwrap() {
            ...
            TransactionAuthField::PublicKey(_) => {
                tx_signer
                    .append_sponsor(&FunaiPublicKey::from_private(&privk))
                    .unwrap();
            }
        };
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_standard_p2pkh() {
        let privk = FunaiPrivateKey::from_hex(
        ...
        let txs = tx_funai_transaction_test_txs(&origin_auth);
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_sponsored_p2pkh() {
        let privk = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e001",
        )
        .unwrap();
        ...
        .unwrap();
        let privk_diff_sponsor = FunaiPrivateKey::from_hex(
        ...
        let diff_sponsor_address = FunaiAddress {
            version: C32_ADDRESS_VERSION_MAINNET_SINGLESIG,
            bytes: Hash160::from_hex("a139de6733cef9e4663c4a093c1a7390a1dcc297").unwrap(),
        };

        let txs = tx_funai_transaction_test_txs(&auth);
        ...
        for mut tx in txs {
            ...
            let mut sponsor_auth = TransactionSpendingCondition::new_singlesig_p2pkh(
                FunaiPublicKey::from_private(&privk_diff_sponsor),
            )
            .unwrap();
            ...
            let mut tx_sponsor_signer =
                FunaiTransactionSigner::new_sponsor(&origin_tx, sponsor_auth).unwrap();
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_standard_p2pkh_uncompressed() {
        let privk = FunaiPrivateKey::from_hex(
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_sponsored_p2pkh_uncompressed() {
        let privk = FunaiPrivateKey::from_hex(
            "807bbe9e471ac976592cc35e3056592ecc0f778ee653fced3b491a122dd8d59701",
        )
        .unwrap();
        let privk_sponsored = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e0",
        )
        .unwrap();

        let mut random_sponsor = FunaiPrivateKey::new(); // what the origin sees
        ...
        let real_sponsor = TransactionSpendingCondition::new_singlesig_p2pkh(
            FunaiPublicKey::from_private(&privk_sponsored),
        )
        .unwrap();
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_standard_p2sh() {
        let privk_1 = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e001",
        )
        .unwrap();
        ...
        .unwrap();
        let privk_3 = FunaiPrivateKey::from_hex(
            "d5200dee706ee53ae98a03fba6cf4fdcc5084c30cfa9e1b3462dcdeaa3e0f1d201",
        )
        .unwrap();

        let pubk_1 = FunaiPublicKey::from_private(&privk_1);
        let pubk_2 = FunaiPublicKey::from_private(&privk_2);
        let pubk_3 = FunaiPublicKey::from_private(&privk_3);
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_sponsored_p2sh() {
        let origin_privk = FunaiPrivateKey::from_hex(
            "807bbe9e471ac976592cc35e3056592ecc0f778ee653fced3b491a122dd8d59701",
        )
        .unwrap();
        ...
        let random_sponsor = FunaiPrivateKey::new(); // what the origin sees
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_standard_p2sh_uncompressed() {
        let privk_1 = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e0",
        )
        .unwrap();
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_sponsored_p2sh_uncompressed() {
        let origin_privk = FunaiPrivateKey::from_hex(
            "807bbe9e471ac976592cc35e3056592ecc0f778ee653fced3b491a122dd8d59701",
        )
        .unwrap();
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_standard_p2sh_mixed() {
        let privk_1 = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e001",
        )
        .unwrap();
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_sponsored_p2sh_mixed() {
        let origin_privk = FunaiPrivateKey::from_hex(
            "807bbe9e471ac976592cc35e3056592ecc0f778ee653fced3b491a122dd8d59701",
        )
        .unwrap();
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_standard_p2wpkh() {
        let privk = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e001",
        )
        .unwrap();
        let origin_auth = TransactionAuth::Standard(
            TransactionSpendingCondition::new_singlesig_p2wpkh(FunaiPublicKey::from_private(
                &privk,
            ))
            .unwrap(),
        );
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_sponsored_p2wpkh() {
        let origin_privk = FunaiPrivateKey::from_hex(
            "807bbe9e471ac976592cc35e3056592ecc0f778ee653fced3b491a122dd8d59701",
        )
        .unwrap();
        ...
        let random_sponsor = FunaiPrivateKey::new();
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_standard_p2wsh() {
        let privk_1 = FunaiPrivateKey::from_hex(
            "6d430bb91222408e7706c9001cfaeb91b08c2be6d5ac95779ab52c6b431950e001",
        )
        .unwrap();
    ...
    #[test]
    fn tx_funai_transaction_sign_verify_sponsored_p2wsh() {
        let origin_privk = FunaiPrivateKey::from_hex(
            "807bbe9e471ac976592cc35e3056592ecc0f778ee653fced3b491a122dd8d59701",
        )
        .unwrap();
