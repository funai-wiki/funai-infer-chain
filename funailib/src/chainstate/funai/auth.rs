...
use funai_common::address::{public_keys_to_address_hash, AddressHashMode};
...
use funai_common::util::secp256k1::{MessageSignature, MESSAGE_SIGNATURE_ENCODED_SIZE};
...
use crate::chainstate::funai::{
    ...
    SinglesigSpendingCondition, FunaiPrivateKey, FunaiPublicKey, TransactionAuth,
...
impl FunaiMessageCodec for TransactionAuthField {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        match *self {
            TransactionAuthField::PublicKey(ref pubk) => {
                ...
                let pubkey_buf = FunaiPublicKeyBuffer::from_public_key(pubk);
    ...
    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<TransactionAuthField, codec_error> {
        ...
        let field = match field_id {
            x if x == TransactionAuthFieldID::PublicKeyCompressed as u8 => {
                let pubkey_buf: FunaiPublicKeyBuffer = read_next(fd)?;
...
impl FunaiMessageCodec for MultisigSpendingCondition {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
...
impl MultisigSpendingCondition {
    ...
    pub fn push_public_key(&mut self, public_key: FunaiPublicKey) -> () {
        self.fields
    ...
    pub fn address_mainnet(&self) -> FunaiAddress {
        FunaiAddress {
    ...
    pub fn address_testnet(&self) -> FunaiAddress {
        FunaiAddress {
    ...
    pub fn verify(
        ...
        let addr_bytes = match FunaiAddress::from_public_keys(
            0,
...
impl FunaiMessageCodec for SinglesigSpendingCondition {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        write_next(fd, &(self.hash_mode.clone() as u8))?;
        write_next(fd, &self.signer)?;
        write_next(fd, &self.nonce)?;
...
impl FunaiMessageCodec for TransactionSpendingCondition {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        match *self {
...
impl TransactionSpendingCondition {
    pub fn new_singlesig_p2pkh(pubkey: FunaiPublicKey) -> Option<TransactionSpendingCondition> {
        ...
        let signer_addr =
            FunaiAddress::from_public_keys(0, &AddressHashMode::SerializeP2PKH, 1, &vec![pubkey])?;
    ...
    pub fn new_singlesig_p2wpkh(pubkey: FunaiPublicKey) -> Option<TransactionSpendingCondition> {
        let signer_addr = FunaiAddress::from_public_keys(
            0,
    ...
    pub fn get_address(&self, mainnet: bool) -> FunaiAddress {
        if mainnet {
    ...
    /// Used by FunaiTransaction::verify*
    pub fn next_verification(
        ...
        key_encoding: &TransactionPublicKeyEncoding,
        sig: &MessageSignature,
    ) -> Result<(FunaiPublicKey, Txid), net_error> {
        let sighash_presign = TransactionSpendingCondition::make_sighash_presign(
            cur_sighash,
            cond_code,
            tx_fee,
        ...
        let mut pubk = FunaiPublicKey::recover_to_pubkey(sighash_presign.as_bytes(), sig)
...
impl FunaiMessageCodec for TransactionAuth {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        match *self {
...
impl TransactionAuth {
    pub fn from_p2pkh(privk: &FunaiPrivateKey) -> Option<TransactionAuth> {
        match TransactionSpendingCondition::new_singlesig_p2pkh(FunaiPublicKey::from_private(
            privk,
        )) {
    ...
    pub fn from_p2sh(privks: &[FunaiPrivateKey], num_sigs: u16) -> Option<TransactionAuth> {
        let mut pubks = vec![];
        for privk in privks.iter() {
            pubks.push(FunaiPublicKey::from_private(privk));
        }
    ...
    pub fn from_p2wpkh(privk: &FunaiPrivateKey) -> Option<TransactionAuth> {
        match TransactionSpendingCondition::new_singlesig_p2wpkh(FunaiPublicKey::from_private(
            privk,
        )) {
            Some(auth) => Some(TransactionAuth::Standard(auth)),
            None => None,
    ...
    pub fn from_p2wsh(privks: &[FunaiPrivateKey], num_sigs: u16) -> Option<TransactionAuth> {
        let mut pubks = vec![];
        for privk in privks.iter() {
            pubks.push(FunaiPublicKey::from_private(privk));
        }
...
mod test {
    ...
    #[test]
    fn tx_funai_spending_condition_p2pkh() {
        // p2pkh
    ...
    #[test]
    fn tx_funai_spending_condition_p2sh() {
        // p2sh
    ...
    #[test]
    fn tx_funai_spending_condition_p2wpkh() {
    ...
    #[test]
    fn tx_funai_spending_condition_p2wsh() {
    ...
    #[test]
    fn tx_funai_invalid_spending_conditions() {
    ...
    fn tx_funai_signature() {
        ...
        for i in 0..4 {
            ...
            assert_eq!(next_pubkey, FunaiPublicKey::from_private(&keys[i]));
        }
    }
}
