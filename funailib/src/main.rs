...
use funailib::burnchains::bitcoin::indexer::{
    BitcoinIndexer, BitcoinIndexerConfig, BitcoinIndexerRuntime,
};
use funailib::burnchains::bitcoin::{spv, BitcoinNetworkType};
use funailib::burnchains::db::{BurnchainBlockData, BurnchainDB};
use funailib::burnchains::{
    Address, Burnchain, PoxConstants, Txid, BLOCKSTACK_MAGIC_MAINNET,
};
use funailib::chainstate::burn::db::sortdb::SortitionDB;
use funailib::chainstate::burn::ConsensusHash;
use funailib::chainstate::nakamoto::NakamotoChainState;
use funailib::chainstate::funai::db::blocks::{DummyEventDispatcher, StagingBlock};
use funailib::chainstate::funai::db::{
    ChainStateBootData, FunaiBlockHeaderTypes, FunaiChainState, FunaiHeaderInfo,
};
use funailib::chainstate::funai::index::marf::{MARFOpenOpts, MarfConnection, MARF};
use funailib::chainstate::funai::index::ClarityMarfTrieId;
use funailib::chainstate::funai::miner::*;
use funailib::chainstate::funai::{FunaiBlockHeader, *};
use funailib::clarity::vm::costs::ExecutionCost;
use funailib::clarity::vm::types::FunaiAddressExtensions;
use funailib::clarity::vm::ClarityVersion;
use funailib::clarity_cli;
use funailib::clarity_cli::vm_execute;
use funailib::core::{MemPoolDB, *};
use funailib::cost_estimates::metrics::UnitMetric;
use funailib::cost_estimates::UnitEstimator;
use funailib::net::db::LocalPeer;
use funailib::net::p2p::PeerNetwork;
use funailib::net::relay::Relayer;
use funailib::net::FunaiMessage;
use funailib::util_lib::db::sqlite_open;
use funailib::util_lib::strings::UrlString;
...
fn main() {
    ...
    if argv[1] == "decode-tx" {
        ...
        let tx = FunaiTransaction::consensus_deserialize(&mut debug_cursor)
            .map_err(|e| {
    ...
    if argv[1] == "decode-block" {
        ...
        let block = FunaiBlock::consensus_deserialize(&mut io::Cursor::new(&block_data))
            .map_err(|_e| {
    ...
    if argv[1] == "decode-net-message" {
        ...
        match read_next::<FunaiMessage, _>(&mut &buf[..]) {
            Ok(msg) => {
                println!("{:#?}", &msg);
                process::exit(0);
            }
            Err(_) => {
                ...
                let _ = read_next::<FunaiMessage, _>(&mut debug_cursor);
                process::exit(1);
            }
        }
    }
    ...
    if argv[1] == "get-tenure" {
        ...
        let index_block_hash = FunaiBlockId::from_hex(&index_block_hash).unwrap();
        let chain_state_path = format!("{}/mainnet/chainstate/", &argv[2]);

        let (chainstate, _) =
            FunaiChainState::open(true, CHAIN_ID_MAINNET, &chain_state_path, None).unwrap();
        ...
        let mut block_info =
            FunaiChainState::load_staging_block_info(chainstate.db(), &index_block_hash)
                .unwrap()
                .expect("No such block");
        block_info.block_data = FunaiChainState::load_block_bytes(
        ...
        let block =
            FunaiBlock::consensus_deserialize(&mut io::Cursor::new(&block_info.block_data))
                .map_err(|_e| {
                    eprintln!("Failed to decode block");
                    process::exit(1);
                })
        ...
        let microblocks =
            FunaiChainState::find_parent_microblock_stream(chainstate.db(), &block_info)
                .unwrap()
                .unwrap_or(vec![]);
...
",
        ...
        let (chain_state, _) = FunaiChainState::open(true, chain_id, &chain_state_path, None)
            .expect("Failed to open funai chain state");
        ...
        let header_hashes = {
            ...
            ic.get_funai_header_hashes(2100, &chain_tip.consensus_hash, &HashMap::new())
                .unwrap()
        };
...
",
        ...
        for (consensus_hash, block_hash_opt) in header_hashes.iter() {
            ...
            let parent_header_opt = {
                let child_block_info = match FunaiChainState::load_staging_block_info(
                    &chain_state.db(),
                    &index_block_hash,
                ) {
                    Ok(Some(hdr)) => hdr,
...
",
        ...
        let header_tip = NakamotoChainState::get_canonical_block_header(chain_state.db(), &sort_db)
            .unwrap()
            .unwrap();
        let parent_header = FunaiChainState::get_anchored_block_header_info(
            chain_state.db(),
        ...
        let mut coinbase_tx = FunaiTransaction::new(
        ...
        let mut tx_signer = FunaiTransactionSigner::new(&coinbase_tx);
        ...
        let result = FunaiBlockBuilder::build_anchored_block(
            &chain_state,
            &sort_db.index_conn(),
            &mut mempool_db,
            &parent_header,
        ...
        println!(
            ...
            parent_header.funai_block_height + 1,
    ...
    if argv[1] == "decode-microblocks" {
        ...
        let mut debug_cursor = LogReader::from_reader(&mut cursor);
        let mblocks: Vec<FunaiMicroblock> = Vec::consensus_deserialize(&mut debug_cursor)
            .map_err(|e| {
    ...
    if argv[1] == "header-indexed-get" {
        ...
        let marf_bhh = FunaiBlockId::from_hex(marf_tip).expect("Bad MARF block hash");
    ...
    if argv[1] == "marf-get" {
        ...
        let itip = FunaiBlockHeader::make_index_block_hash(&consensustip, &tip);
        let key = &argv[5];
    ...
    if argv[1] == "docgen" {
        println!(
            "{}",
            funailib::clarity::vm::docs::make_json_api_reference()
        );
        return;
    }
    ...
    if argv[1] == "docgen_boot" {
        println!(
            "{}",
            funailib::chainstate::funai::boot::docs::make_json_boot_contracts_reference()
        );
        return;
    }
    ...
    if argv[1] == "replay-block" {
        ...
        let funai_path = &argv[2];
        let mode = argv.get(3).map(String::as_str);
        let staging_blocks_db_path = format!("{funai_path}/mainnet/chainstate/vm/index.sqlite");
        let conn =
        ...
        for (i, index_block_hash) in index_block_hashes.iter().enumerate() {
            ...
            replay_block(funai_path, index_block_hash);
        }
        println!("Finished!");
        process::exit(0);
    }
    ...
    if argv[1] == "post-funaidb" {
        if argv.len() < 4 {
            eprintln!(
                "Usage: {} post-funaidb slot_id slot_version privkey data",
                &argv[0]
            );
            process::exit(1);
        }
        ...
        let mut chunk = FunaiDBChunkData::new(slot_id, slot_version, buf);
        let privk = FunaiPrivateKey::from_hex(&privkey).unwrap();
    ...
    if argv[1] == "replay-chainstate" {
        ...
        let (old_chainstate, _) =
            FunaiChainState::open(false, 0x80000000, old_chainstate_path, None).unwrap();
        let old_sortition_db =
            SortitionDB::open(old_sort_path, true, PoxConstants::mainnet_default()).unwrap();

        // initial argon balances -- see testnet/funai-node/conf/testnet-follower-conf.toml
        let initial_balances = vec![
            (
                FunaiAddress::from_string("ST2QKZ4FKHAH1NQKYKYAYZPY440FEPK7GZ1R5HBP2")
                    .unwrap()
                    .to_account_principal(),
                10000000000000000,
            ),
            (
                FunaiAddress::from_string("ST319CF5WV77KYR1H3GT0GZ7B8Q4AQPY42ETP1VPF")
                    .unwrap()
                    .to_account_principal(),
                10000000000000000,
            ),
            (
                FunaiAddress::from_string("ST221Z6TDTC5E0BYR2V624Q2ST6R0Q71T78WTAX6H")
                    .unwrap()
                    .to_account_principal(),
                10000000000000000,
            ),
            (
                FunaiAddress::from_string("ST2TFVBMRPS5SSNP98DQKQ5JNB2B6NZM91C4K3P7B")
                    .unwrap()
                    .to_account_principal(),
                10000000000000000,
            ),
        ...
        let epochs = FunaiEpoch::all(first_burnchain_block_height, u64::MAX, u64::MAX);
        ...
        let (mut new_chainstate, _) = FunaiChainState::open_and_exec(
            false,
            0x80000000,
            new_chainstate_path,
            Some(&mut boot_data),
        ...
        let all_funai_blocks =
            FunaiChainState::get_all_staging_block_headers(&old_chainstate.db()).unwrap();

        // order block hashes by arrival index
        let mut funai_blocks_arrival_indexes = vec![];
        for snapshot in all_snapshots.iter() {
            ...
            funai_blocks_arrival_indexes.push((index_hash, snapshot.arrival_index));
        }
        funai_blocks_arrival_indexes.sort_by(|ref a, ref b| a.1.partial_cmp(&b.1).unwrap());
        let funai_blocks_arrival_order: Vec<FunaiBlockId> = funai_blocks_arrival_indexes
            .into_iter()
            .map(|(h, _)| h)
            .collect();

        let mut funai_blocks_available: HashMap<FunaiBlockId, StagingBlock> = HashMap::new();
        let num_staging_blocks = all_funai_blocks.len();
        for staging_block in all_funai_blocks.into_iter() {
            if !staging_block.orphaned {
                ...
                funai_blocks_available.insert(index_hash, staging_block);
            }
        }

        eprintln!(
            "\nWill replay {} funai epochs out of {}\n",
            &funai_blocks_available.len(),
            num_staging_blocks
        );

        let mut known_funai_blocks = HashSet::new();
        ...
        let (mut p2p_chainstate, _) =
            FunaiChainState::open(false, 0x80000000, new_chainstate_path, None).unwrap();
        ...
        for old_snapshot in all_snapshots.into_iter() {
            ...
            assert_eq!(
                old_snapshot.winning_funai_block_hash,
                new_snapshot.winning_funai_block_hash
            );
            ...
            if new_snapshot.sortition {
                let mut funai_block_id = FunaiBlockHeader::make_index_block_hash(
                    &new_snapshot.consensus_hash,
                    &new_snapshot.winning_funai_block_hash,
                );
                known_funai_blocks.insert(funai_block_id.clone());

                if next_arrival >= funai_blocks_arrival_order.len() {
                    // all blocks should have been queued up
                    continue;
                }

                if funai_block_id == funai_blocks_arrival_order[next_arrival] {
                    while next_arrival < funai_blocks_arrival_order.len()
                        && known_funai_blocks.contains(&funai_block_id)
                    {
                        if let Some(_) = funai_blocks_available.get(&funai_block_id) {
                            // load up the block
                            let funai_block_opt = FunaiChainState::load_block(
                                &old_chainstate.blocks_path,
                                &new_snapshot.consensus_hash,
                                &new_snapshot.winning_funai_block_hash,
                            )
                            .unwrap();
                            if let Some(funai_block) = funai_block_opt {
                        ...
                        funai_block_id = funai_blocks_arrival_order[next_arrival].clone();
                    }
                }
        ...
        eprintln!(
            ...
            next_arrival,
            funai_blocks_arrival_order.len()
        );
        return;
    }
...
",
    ...
    let sort_db = SortitionDB::open(&sort_db_path, false, PoxConstants::mainnet_default())
        .unwrap_or_else(|_| panic!("Failed to open {sort_db_path}"));
    let chain_id = CHAIN_ID_MAINNET;
    let mut chain_state = FunaiChainState::open(true, chain_id, &chain_state_path, None)
        .expect("Failed to open funai chain state")
        .0;
    let chain_tip = SortitionDB::get_canonical_burn_chain_tip(sort_db.conn())
        .expect("Failed to get sortition chain tip");
    ...
    // Find ancestor block
    let mut funai_header = header_tip.to_owned();
    loop {
        let parent_block_id = match funai_header.anchored_header {
            FunaiBlockHeaderTypes::Nakamoto(ref nakamoto_header) => {
                nakamoto_header.parent_block_id.clone()
            }
            FunaiBlockHeaderTypes::Epoch2(ref epoch2_header) => {
                let block_info = FunaiChainState::load_staging_block(
                    chain_state.db(),
                    &chain_state.blocks_path,
                    &funai_header.consensus_hash,
        ...
        let funai_parent_header =
            NakamotoChainState::get_block_header(chain_state.db(), &parent_block_id)
                .unwrap()
                .unwrap();
        if funai_parent_header.anchored_header.height() < mine_tip_height {
            break;
        }
        funai_header = funai_parent_header;
    }
    info!(
        "Found funai_chain_tip with height {}",
        ...
        match path {
            ...
            "new_mempool_tx" => {
                ...
                for item in payload {
                    ...
                    let raw_tx = FunaiTransaction::consensus_deserialize(&mut cursor).unwrap();
                    if found_block_height {
                        ...
                        let result = mempool_db.submit(
                            ...
                            &sort_db,
                            &funai_header.consensus_hash,
                            &funai_header.anchored_header.block_hash(),
                            &raw_tx,
                            None,
...
fn replay_block(funai_path: &str, index_block_hash_hex: &str) {
    let index_block_hash = FunaiBlockId::from_hex(index_block_hash_hex).unwrap();
    let chain_state_path = format!("{funai_path}/mainnet/chainstate/");
    let sort_db_path = format!("{funai_path}/mainnet/burnchain/sortition");
    let burn_db_path = format!("{funai_path}/mainnet/burnchain/burnchain.sqlite");
    ...
    let mut next_staging_block =
        FunaiChainState::load_staging_block_info(&chainstate_tx.tx, &index_block_hash)
            .expect("Failed to load staging block data")
            .expect("No such index block hash in block database");

    next_staging_block.block_data = FunaiChainState::load_block_bytes(
        &blocks_path,
    ...
    let Some(next_microblocks) =
        FunaiChainState::find_parent_microblock_stream(&chainstate_tx.tx, &next_staging_block)
            .unwrap()
    else {
    ...
    let Some(parent_header_info) =
        FunaiChainState::get_parent_header_info(&mut chainstate_tx, &next_staging_block).unwrap()
    else {
        println!("Failed to load parent head info for block: {index_block_hash_hex}");
        return;
    };

    let block = FunaiChainState::extract_funai_block(&next_staging_block).unwrap();
    let block_size = next_staging_block.block_data.len() as u64;

    let parent_block_header = match &parent_header_info.anchored_header {
        FunaiBlockHeaderTypes::Epoch2(bh) => bh,
        FunaiBlockHeaderTypes::Nakamoto(_) => panic!("Nakamoto blocks not supported yet"),
    };

    if !FunaiChainState::check_block_attachment(&parent_block_header, &block.header) {
        let msg = format!(
    ...
    let block_am = FunaiChainState::find_funai_tip_affirmation_map(
        &burnchain_blocks_db,
        sort_tx.tx(),
        &next_staging_block.consensus_hash,
        &next_staging_block.anchored_block_hash,
    ...
    match FunaiChainState::append_block(
        &mut chainstate_tx,
        clarity_instance,
        &mut sort_tx,
        &pox_constants,
