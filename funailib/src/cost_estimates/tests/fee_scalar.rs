...
fn make_dummy_coinbase_tx() -> FunaiTransaction {
...
fn make_dummy_transfer_tx(fee: u64) -> FunaiTransactionReceipt {
...
fn make_dummy_cc_tx(fee: u64) -> FunaiTransactionReceipt {
    let mut tx = FunaiTransaction::new(
        TransactionVersion::Mainnet,
        TransactionAuth::Standard(TransactionSpendingCondition::new_initial_sighash()),
...
fn test_fee_estimator() {
    ...
    let coinbase_only_receipt = make_block_receipt(vec![FunaiTransactionReceipt::from_coinbase(
    ...
    let single_tx_receipt = make_block_receipt(vec![
        FunaiTransactionReceipt::from_coinbase(make_dummy_coinbase_tx()),
        make_dummy_cc_tx(1),
    ]);
