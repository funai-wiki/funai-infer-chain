...
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, FunaiAddress, FunaiWorkScore, TrieHash,
};
...
use crate::core::{FunaiEpochId, BLOCK_LIMIT_MAINNET_20};
...
fn test_empty_pessimistic_estimator() {
    ...
    assert_eq!(
        estimator
            .estimate_cost(&make_dummy_transfer_payload(), &FunaiEpochId::Epoch20)
            .expect_err("Empty pessimistic estimator should error."),
        EstimatorError::NoEstimateAvailable
    );
}

fn make_dummy_coinbase_tx() -> FunaiTransactionReceipt {
    FunaiTransactionReceipt::from_coinbase(FunaiTransaction::new(
...
fn make_dummy_transfer_tx() -> FunaiTransactionReceipt {
    let tx = FunaiTransaction::new(
        TransactionVersion::Mainnet,
        TransactionAuth::Standard(TransactionSpendingCondition::new_initial_sighash()),
        TransactionPayload::TokenTransfer(
            PrincipalData::Standard(StandardPrincipalData(0, [0; 20])),
...
fn make_dummy_cc_tx(
    ...
    FunaiTransactionReceipt::from_contract_call(
        tx,
        vec![],
        Value::okay(Value::Bool(true)).unwrap(),
        0,
...
fn make_dummy_cc_payload(contract_name: &str, function_name: &str) -> TransactionPayload {
    TransactionPayload::ContractCall(TransactionContractCall {
        address: FunaiAddress::new(0, Hash160([0; 20])),
...
fn test_cost_estimator_notify_block() {
    ...
    estimator.notify_block(&block, &BLOCK_LIMIT_MAINNET_20, &FunaiEpochId::Epoch20);
...
fn pessimistic_estimator_contract_owner_separation() {
    ...
    let cc_payload_1 = TransactionPayload::ContractCall(TransactionContractCall {
        address: FunaiAddress::new(0, Hash160([1; 20])),
        contract_name: "contract-1".into(),
        function_name: "func1".into(),
        function_args: vec![],
    });
    ...
    assert_eq!(
        estimator.estimate_cost(&cc_payload_1, &FunaiEpochId::Epoch20,),
        Err(EstimatorError::NoEstimateAvailable)
    );

    assert_eq!(
        estimator
            .estimate_cost(&cc_payload_0, &FunaiEpochId::Epoch20,)
            .expect("Should be able to provide cost estimate now"),
        ExecutionCost {
            write_length: 1,
            write_count: 1,
    ...
    assert_eq!(
        estimator
            .estimate_cost(&cc_payload_1, &FunaiEpochId::Epoch20,)
            .expect("Should be able to provide cost estimate now"),
        ExecutionCost {
            write_length: 5,
            write_count: 5,
...
/// Test that we forget the "learnings" from previous Funai epoch on next epoch.
#[test]
...
fn test_cost_estimator_dont_affect_previous() {
    ...
    estimator.notify_block(&block, &BLOCK_LIMIT_MAINNET_20, &FunaiEpochId::Epoch2_05);
...
/// Test that updates to Funai 2.0 and Funai 2.05 can be recorded and recovered independently.
#[test]
