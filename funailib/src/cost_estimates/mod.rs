...
use crate::chainstate::funai::db::FunaiEpochReceipt;
use crate::chainstate::funai::events::{FunaiTransactionReceipt, TransactionOrigin};
use crate::chainstate::funai::{FunaiBlock, TransactionPayload};
...
use crate::chainstate::funai::FunaiTransaction;
...
pub trait FeeEstimator {
    /// This method is invoked by the `funai-node` to update the fee estimator with a new
    ///  block receipt.
    fn notify_block(
        &mut self,
        receipt: &FunaiEpochReceipt,
...
pub fn estimate_fee_rate<CE: CostEstimator + ?Sized, CM: CostMetric + ?Sized>(
    ...
    let cost_estimate = estimator.estimate_cost(&tx.payload, funai_epoch_id)?;
...
pub trait CostEstimator: Send {
    /// This method is invoked by the `funai-node` to update the cost estimator with a new
    ...
    /// This method is used by a funai-node to obtain an estimate for a given transaction payload.
    ...
    /// This method is invoked by the `funai-node` to notify the estimator of all the transaction
    ...
    fn notify_block(
        ...
        for current_receipt in receipts.iter() {
            let current_txid = match current_receipt.transaction {
                ...
                TransactionOrigin::Funai(ref tx) => tx.txid(),
            };
            let tx_payload = match current_receipt.transaction {
                TransactionOrigin::Burn(_) => continue,
                TransactionOrigin::Funai(ref tx) => &tx.payload,
            };
