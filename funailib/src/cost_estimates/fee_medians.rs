...
use crate::cost_estimates::FunaiTransactionReceipt;
...
fn fee_rate_and_weight_from_receipt(
    ...
    tx_receipt: &FunaiTransactionReceipt,
    block_limit: &ExecutionCost,
) -> Option<FeeRateAndWeight> {
    let (payload, fee, tx_size) = match tx_receipt.transaction {
        TransactionOrigin::Funai(ref tx) => Some((&tx.payload, tx.get_tx_fee(), tx.tx_len())),
