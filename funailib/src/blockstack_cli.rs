...
use funailib::burnchains::bitcoin::address::{
    ADDRESS_VERSION_MAINNET_SINGLESIG, ADDRESS_VERSION_TESTNET_SINGLESIG,
};
use funailib::burnchains::Address;
use funailib::chainstate::funai::{
    FunaiBlock, FunaiBlockHeader, FunaiMicroblock, FunaiPrivateKey, FunaiPublicKey,
    FunaiTransaction, FunaiTransactionSigner, TokenTransferMemo, TransactionAnchorMode,
...
use funailib::core::{CHAIN_ID_MAINNET, CHAIN_ID_TESTNET};
use funailib::net::Error as NetError;
use funailib::util_lib::strings::FunaiString;
use clarity::vm::errors::{Error as ClarityError, RuntimeErrorType};
use clarity::vm::types::PrincipalData;
use clarity::vm::{ClarityName, ClarityVersion, ContractName, Value};
use funai_common::address::{b58, AddressHashMode};
use funai_common::codec::{Error as CodecError, FunaiMessageCodec};
...
This CLI has these methods:
  ...
  addresses          used to get both Bitcoin and Funai addresses from a private key
  decode-tx          used to decode a hex-encoded transaction into a human-readable representation
  decode-header      used to decode a hex-encoded Funai header into a human-readable representation
  decode-block       used to decode a hex-encoded Funai block into a human-readable representation
  decode-microblock  used to decode a hex-encoded Funai microblock into a human-readable representation
  decode-microblocks used to decode a hex-encoded stream of Funai microblocks into a human-readable representation
...
A smart contract can be mined in a Funai block, a Funai microblock, or either.  The default
...
A contract-call can be mined in a Funai block, a Funai microblock, or either.  The default
is that the miner chooses, but you can decide which with the following options:
...
A token-transfer can be mined in a Funai block, a Funai microblock, or either.  The default
is that the miner chooses, but you can decide which with the following options:
...
secret key, the corresponding public key, and the corresponding P2PKH Funai address.";

const ADDRESSES_USAGE: &str = "blockstack-cli (options) addresses [secret-key-hex]

The addresses command calculates both the Bitcoin and Funai addresses from a secret key.
If successful, this command outputs both the Bitcoin and Funai addresses to stdout, formatted
as JSON, and exits with code 0.";

const DECODE_TRANSACTION_USAGE: &str =
    "blockstack-cli (options) decode-tx [transaction-hex-or-stdin]

The decode-tx command decodes a serialized Funai transaction and prints it to stdout as JSON.
...
The decode-header command decodes a serialized Funai header and prints it to stdout as JSON.
...
The decode-block command decodes a serialized Funai block and prints it to stdout as JSON.
...
The decode-microblock command decodes a serialized Funai microblock and prints it to stdout as JSON.
...
N.B. Funai microblocks are not stored as files in the Funai chainstate -- they are stored in
...
The decode-microblocks command decodes a serialized list of Funai microblocks and prints it to stdout as JSON.
...
impl From<NetError> for CliError {
    fn from(value: NetError) -> Self {
        CliError::Message(format!("Funai NetError: {}", value))
    }
}

impl From<CodecError> for CliError {
    fn from(value: CodecError) -> Self {
        CliError::Message(format!("Funai CodecError: {}", value))
    }
}
...
impl From<funai_common::util::HexError> for CliError {
    fn from(value: funai_common::util::HexError) -> Self {
...
fn make_contract_publish(
    ...
    let code_body = FunaiString::from_string(&contract_content)
...
fn make_contract_call(
    ...
    let address =
        FunaiAddress::from_string(&contract_address).ok_or("Failed to parse contract address")?;
...
fn make_standard_single_sig_tx(
    ...
    let mut tx = FunaiTransaction::new(version, auth, payload);
...
fn sign_transaction_single_sig_standard(
    transaction: &str,
    secret_key: &FunaiPrivateKey,
) -> Result<FunaiTransaction, CliError> {
    let transaction =
        FunaiTransaction::consensus_deserialize(&mut io::Cursor::new(&hex_bytes(transaction)?))?;

    let mut tx_signer = FunaiTransactionSigner::new(&transaction);
...
fn handle_contract_publish(
    ...
    let sk_publisher = FunaiPrivateKey::from_hex(sk_publisher)?;
    ...
    let mut unsigned_tx = make_standard_single_sig_tx(
        ...
        chain_id,
        payload.into(),
        &FunaiPublicKey::from_private(&sk_publisher),
        nonce,
        tx_fee,
    );
...
fn handle_contract_call(
    ...
    let sk_origin = FunaiPrivateKey::from_hex(sk_origin)?;
    ...
    let mut unsigned_tx = make_standard_single_sig_tx(
        ...
        chain_id,
        payload.into(),
        &FunaiPublicKey::from_private(&sk_origin),
        nonce,
        tx_fee,
    );
    unsigned_tx.anchor_mode = anchor_mode;
...
fn handle_token_transfer(
    ...
    let sk_origin = FunaiPrivateKey::from_hex(&args[0])?;
    let tx_fee = args[1].parse()?;
    let nonce = args[2].parse()?;
...
fn generate_secret_key(args: &[String], version: TransactionVersion) -> Result<String, CliError> {
    ...
    let address = FunaiAddress::from_public_keys(
        version,
...
fn get_addresses(args: &[String], version: TransactionVersion) -> Result<String, CliError> {
    ...
    let sk = FunaiPrivateKey::from_hex(&args[0]).expect("Failed to load private key");
    ...
    let stx_address = FunaiAddress::from_public_keys(
        c32_version,
        &AddressHashMode::SerializeP2PKH,
        1,
        &vec![pk.clone()],
...
fn decode_transaction(args: &[String], _version: TransactionVersion) -> Result<String, CliError> {
    ...
    match FunaiTransaction::consensus_deserialize(&mut debug_cursor) {
...
fn decode_header(args: &[String], _version: TransactionVersion) -> Result<String, CliError> {
    ...
    match FunaiBlockHeader::consensus_deserialize(&mut debug_cursor) {
        Ok(header) => {
...
fn decode_block(args: &[String], _version: TransactionVersion) -> Result<String, CliError> {
    ...
    match FunaiBlock::consensus_deserialize(&mut debug_cursor) {
...
fn decode_microblock(args: &[String], _version: TransactionVersion) -> Result<String, CliError> {
    ...
    match FunaiMicroblock::consensus_deserialize(&mut debug_cursor) {
        Ok(block) => {
...
fn decode_microblocks(args: &[String], _version: TransactionVersion) -> Result<String, CliError> {
    ...
    match Vec::<FunaiMicroblock>::consensus_deserialize(&mut debug_cursor) {
        Ok(blocks) => {
