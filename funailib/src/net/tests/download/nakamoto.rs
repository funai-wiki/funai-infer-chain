...
use funai_common::types::chainstate::{
    ConsensusHash, FunaiAddress, FunaiBlockId, FunaiPrivateKey, TrieHash,
};
use funai_common::types::net::PeerAddress;
use funai_common::types::FunaiEpochId;
use funai_common::util::get_epoch_time_secs;
use funai_common::util::hash::{hex_bytes, Sha512Trunc256Sum};
...
use crate::chainstate::funai::{
    CoinbasePayload, FunaiTransaction, TenureChangeCause, TenureChangePayload, ThresholdSignature,
...
fn test_nakamoto_tenure_downloader() {
    ...
    let mut next_tenure_change_tx = FunaiTransaction::new(
        TransactionVersion::Testnet,
        TransactionAuth::from_p2pkh(&private_key).unwrap(),
...
fn test_nakamoto_unconfirmed_tenure_downloader() {
    ...
    let tip_block_id = FunaiBlockId::new(&peer.network.funai_tip.0, &peer.network.funai_tip.1);

    let tip_ch = peer.network.funai_tip.0.clone();
    let parent_tip_ch = peer.network.parent_funai_tip.0.clone();
    ...
    let highest_confirmed_wanted_tenure = WantedTenure {
        tenure_id_consensus_hash: peer.network.parent_funai_tip.0.clone(),
    ...
    let unconfirmed_wanted_tenure = WantedTenure {
        tenure_id_consensus_hash: peer.network.funai_tip.0.clone(),
    ...
    // we've processed the tip already, so we transition straight to the Done state
    {
        ...
        let tenure_tip = RPCGetTenureInfo {
            consensus_hash: peer.network.funai_tip.0.clone(),
            tenure_start_block_id: peer.network.tenure_start_block_id.clone(),
            parent_consensus_hash: peer.network.parent_funai_tip.0.clone(),
            parent_tenure_start_block_id: FunaiBlockId::new(
                &peer.network.parent_funai_tip.0,
                &peer.network.parent_funai_tip.1,
            ),
            tip_block_id: FunaiBlockId::new(
                &peer.network.funai_tip.0,
                &peer.network.funai_tip.1,
            ),
            tip_height: peer.network.funai_tip.2,
            reward_cycle: tip_rc,
        };
    ...
    // test load_wanted_tenures()
    {
        ...
        for i in (tip.block_height - rc_len)..tip.block_height {
            ...
            assert_eq!(
                ...
                all_sortitions[i].winning_funai_block_hash.0
            );
...
fn test_nakamoto_download_run_2_peers() {
    ...
    let privk = FunaiPrivateKey::from_seed(&[0, 1, 2, 3, 4]);
    ...
    let mut all_block_headers: HashMap<ConsensusHash, FunaiHeaderInfo> = HashMap::new();
    ...
    let (canonical_funai_tip_ch, canonical_funai_tip_bhh) =
    ...
    thread::scope(|s| {
        s.spawn(move || {
            ...
            let (mut last_funai_tip_ch, mut last_funai_tip_bhh) =
                SortitionDB::get_canonical_funai_chain_tip_hash(boot_peer.sortdb().conn())
                    .unwrap();
            ...
            loop {
                ...
                let (funai_tip_ch, funai_tip_bhh) =
                    SortitionDB::get_canonical_funai_chain_tip_hash(boot_peer.sortdb().conn())
                        .unwrap();
                ...
                if burnchain_ptr < all_burn_block_ops.len() {
                    ...
                    let header = all_block_headers
                        ...
                        .unwrap();
                    debug!(
                        "Waiting for Funai block {} (sortition {} height {} burn height {})",
                    ...
                    if funai_tip_ch != last_funai_tip_ch
                        || funai_tip_ch == header.consensus_hash
                ...
                last_funai_tip_ch = funai_tip_ch;
                last_funai_tip_bhh = funai_tip_bhh;

                debug!(
                    "Booting peer's funai tip is now {:?}",
                    &boot_peer.network.funai_tip
                );
                if funai_tip_ch == canonical_funai_tip_ch {
                    break;
                }
            }
