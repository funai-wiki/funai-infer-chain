...
use crate::net::test::*;
use crate::net::*;
use crate::funai_common::types::PublicKey;
...
fn test_get_block_availability() {
    with_timeout(600, || {
        ...
        for i in 0..num_blocks {
            let (mut burn_ops, funai_block, microblocks) = peer_2.make_default_tenure();
            ...
            let sn =
                SortitionDB::get_canonical_burn_chain_tip(&peer_2.sortdb.as_ref().unwrap().conn())
                    .unwrap();
            block_data.push((sn.consensus_hash.clone(), funai_block, microblocks));
        }
            ...
            inv_1_count = match peer_1.network.inv_state {
                Some(ref inv) => {
                    ...
                    let peer_1_availability = get_peer_availability(
                        ...
                        first_funai_block_height + reward_cycle_length,
                    );
        ...
        for (
            (sn_consensus_hash, funai_block, microblocks),
            (consensus_hash, funai_block_hash_opt, neighbors),
        ) in block_data.iter().zip(availability.iter())
        {
            assert_eq!(*consensus_hash, *sn_consensus_hash);
            assert!(funai_block_hash_opt.is_some());
            assert_eq!(*funai_block_hash_opt, Some(funai_block.block_hash()));
        }
    })
}
    ...
    D: FnMut(&mut Vec<TestPeer>) -> bool,
{
        ...
        for i in 0..peers.len() {
            ...
            peer.with_peer_state(|peer, sortdb, chainstate, mempool| {
                for i in 0..(result.blocks.len() + result.confirmed_microblocks.len() + 1) {
                    ...
                    test_debug!(
                        "\n\n{:?}: after funai block, new tip PoX ID is {:?}\n\n",
            ...
            for i in 0..num_peers {
                for b in 0..num_blocks {
                    if !peer_invs[i].has_ith_block(
                        ((b as u64) + first_funai_block_height - first_sortition_height) as u16,
                    ) {
                        if block_data[b].1.is_some() {
                            test_debug!(
                                ...
                                i,
                                b,
                                (b as u64) + first_funai_block_height - first_sortition_height,
                                first_funai_block_height - first_sortition_height,
                                first_funai_block_height - first_sortition_height
                                    + (num_blocks as u64),
                            );
                            done = false;
    ...
    for peer in peers.iter_mut() {
        ...
        let availability = get_peer_availability(
            peer,
            first_funai_block_height - first_sortition_height,
            first_funai_block_height - first_sortition_height + (num_blocks as u64),
        );
        ...
        for (
            (sn_consensus_hash, funai_block_opt, microblocks_opt),
            (consensus_hash, funai_block_hash_opt, neighbors),
        ) in block_data.iter().zip(availability.iter())
        {
            assert_eq!(*consensus_hash, *sn_consensus_hash);

            if funai_block_hash_opt.is_some() {
                assert!(funai_block_opt.is_some());
                assert_eq!(
                    *funai_block_hash_opt,
                    Some(funai_block_opt.as_ref().unwrap().block_hash())
                );
            } else {
                assert!(funai_block_opt.is_none());
            }
        }
    }
        ...
        run_get_blocks_and_microblocks(
            ...
            |num_blocks, ref mut peers| {
                ...
                for _ in 0..num_blocks {
                    let (mut burn_ops, funai_block, microblocks) = peers[1].make_default_tenure();

                    let (_, burn_header_hash, consensus_hash) =
                        peers[1].next_burnchain_block(burn_ops.clone());
                    peers[1].process_funai_epoch_at_tip(&funai_block, &microblocks);
            ...
            |peer| {
                ...
                let _ = peer.for_each_convo_p2p(|event_id, convo| {
                    let cnt = *(convo
                        .stats
                        .msg_rx_counts
                        .get(&FunaiMessageID::BlocksAvailable)
                        .unwrap_or(&0));
                    assert_eq!(
                        cnt, 0,
...
fn make_contract_call_transaction(
    ...
    let tx_cc = {
        ...
        let chain_tip = FunaiBlockHeader::make_index_block_hash(consensus_hash, block_hash);
    ...
    with_timeout(600, || {
        run_get_blocks_and_microblocks(
            ...
            |num_blocks, ref mut peers| {
                ...
                // function to make a tenure in which a the peer's miner funai its STX
                ...
                >| {
                    ...
                    let mblock_privkey = FunaiPrivateKey::new();

                    let mblock_pubkey_hash_bytes = Hash160::from_data(
                        &FunaiPublicKey::from_private(&mblock_privkey).to_bytes(),
                    );
                    ...
                    let (anchored_block, _size, _cost, microblock_opt) =
                        FunaiBlockBuilder::make_anchored_block_and_microblock_from_txs(
                            builder,
                            chainstate,
                            &sortdb.index_conn(),
                ...
                for i in 0..50 {
                    let (mut burn_ops, funai_block, microblocks) = if i == 1 {
    ...
    with_timeout(600, || {
        run_get_blocks_and_microblocks(
            ...
            |num_blocks, ref mut peers| {
                ...
                let mut block_data = vec![];
                for _ in 0..num_blocks {
                    let (mut burn_ops, funai_block, microblocks) = peers[0].make_default_tenure();
    ...
    with_timeout(600, || {
        run_get_blocks_and_microblocks(
            ...
            |num_blocks, ref mut peers| {
                ...
                for i in 0..num_blocks {
                    if i == 0 {
                        let (mut burn_ops, funai_block, mut microblocks) =
                            peers[1].make_default_tenure();
                        ...
                        while microblocks.len() != num_blocks {
                            let next_microblock_payload = TransactionPayload::SmartContract(
                                TransactionSmartContract {
                                    ...
                                    code_body: FunaiString::from_str(
                        ...
                        peers[1].process_funai_epoch(&funai_block, &consensus_hash, &microblocks);
                        ...
                        let (mut burn_ops, funai_block, _) = peers[1].make_tenure(
                            |ref mut miner,
                             ref mut sortdb,
                             ref mut chainstate,
                             vrf_proof,
                        ...
                        for burn_op in burn_ops.iter_mut() {
                            if let BlockstackOperationType::LeaderBlockCommit(ref mut op) = burn_op
                            {
                                ...
                                op.block_header_hash = funai_block.block_hash();
                            }
                        }
                        ...
                        peers[1].process_funai_epoch(&funai_block, &consensus_hash, &vec![]);
