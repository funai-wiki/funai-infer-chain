        ...
        match peer_1.network.inv_state {
            Some(ref mut inv) => {
                assert!(!inv
                    ...
                    .unwrap()
                    .inv
                    .has_ith_block(i + first_funai_block_height + 1));
                assert!(!inv
                    .block_stats
                    .get(&nk)
                    .unwrap()
                    .inv
                    .has_ith_microblock_stream(i + first_funai_block_height + 1));

                let sn = {
                    let ic = sortdb.index_conn();
                    let sn = SortitionDB::get_ancestor_snapshot(
                        &ic,
                        i + first_funai_block_height + 1,
                        &tip.sortition_id,
                    )
                    .unwrap()
                    .unwrap();
                ...
                assert_eq!(
                    Some(i + first_funai_block_height - sortdb.first_block_height + 1),
                    sh
                );
                assert!(inv
                    .block_stats
...
fn test_sync_inv_make_inv_messages() {
    ...
    for i in 0..num_blocks {
        let (burn_ops, funai_block, microblocks) = peer_1.make_default_tenure();
    ...
    match reply {
        FunaiMessageType::PoxInv(poxinv) => {
    ...
    match reply {
        FunaiMessageType::Nack(nack_data) => {
    ...
    let getblocksinv_request = peer_1
        .with_network_state(|sortdb, _chainstate, network, _relayer, _mempool| {
            let height = network.burnchain.reward_cycle_to_block_height(
                network
                    .burnchain
                    .block_height_to_reward_cycle(first_funai_block_height)
                    .unwrap(),
            );
            let sn = {
                let ic = sortdb.index_conn();
    ...
    match reply {
        FunaiMessageType::BlocksInv(blocksinv) => {
    ...
    // ask for a getblocksinv, right at the first Funai block height
    let getblocksinv_request = peer_1
        .with_network_state(|sortdb, _chainstate, network, _relayer, _mempool| {
            let height = network.burnchain.reward_cycle_to_block_height(
                network
    ...
    // ask for a getblocksinv, prior to the first Funai block height
    let getblocksinv_request = peer_1
        .with_network_state(|sortdb, _chainstate, network, _relayer, _mempool| {
            let height = network.burnchain.reward_cycle_to_block_height(
                network
...
fn test_sync_inv_2_peers_plain() {
    with_timeout(600, || {
        ...
        for i in 0..num_blocks {
            assert!(
                peer_2_inv.has_ith_block(i + first_funai_block_height),
                "Missing block {} (+ {})",
                i,
        ...
        for i in 1..(num_blocks - 1) {
            assert!(
                peer_2_inv.has_ith_microblock_stream(i + first_funai_block_height),
                "Missing microblock {} (+ {})",
                i,
                first_funai_block_height
            );
        ...
        for i in 0..num_blocks {
            assert!(
                peer_1_inv.has_ith_block(i + first_funai_block_height),
                "Missing block {} (+ {})",
                i,
                first_funai_block_height
            );
...
fn test_sync_inv_2_peers_stale() {
    with_timeout(600, || {
        ...
        while !peer_1_check || !peer_2_check {
            ...
            match peer_1.network.inv_state {
                Some(ref inv) => {
                    ...
                    if let Some(ref peer_2_inv) = inv.block_stats.get(&peer_2.to_neighbor().addr) {
                        ...
                        {
                            for i in 0..first_funai_block_height {
            ...
            test_debug!("\n\npeer_1_check = {}, peer_2_check = {}, inv_1_count = {}, inv_2_count = {}, first_funai_block_height = {}\n\n", peer_1_check, peer_2_check, inv_1_count, inv_2_count, first_funai_block_height);
        }
    ...
    with_timeout(600, || {
        ...
        for i in 0..num_blocks {
            ...
            if i + 1 < num_blocks {
                ...
                // peer 1 diverges
                test_debug!("Peer 1 diverges at {}", i + first_funai_block_height);
        ...
        for i in 0..(num_blocks - stable_confs) {
            assert!(peer_2_inv.has_ith_block(i + first_funai_block_height));
            if i > 0 {
                assert!(peer_2_inv.has_ith_microblock_stream(i + first_funai_block_height));
            } else {
                assert!(!peer_2_inv.has_ith_microblock_stream(i + first_funai_block_height));
            }
        }
        ...
        for i in 0..(num_blocks - stable_confs) {
            assert!(peer_1_inv.has_ith_block(i + first_funai_block_height));
        }
    ...
    with_timeout(600, || {
        ...
        for i in 0..(num_blocks - 2 * reward_cycle_length) {
            ...
            if i > 0 && i != num_blocks - 2 * reward_cycle_length - 1 {
                ...
                assert!(peer_1_inv.has_ith_microblock_stream(i + first_funai_block_height));
            }
        }
