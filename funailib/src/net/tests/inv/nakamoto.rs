...
use funai_common::types::FunaiEpoch;
...
use crate::net::{
    ...
    PeerNetworkComms, FunaiMessage, FunaiMessageType,
};
...
pub fn peer_get_nakamoto_invs<'a>(
    mut peer: TestPeer<'a>,
    reward_cycles: &[u64],
) -> (TestPeer<'a>, Vec<FunaiMessageType>) {
    ...
    for reward_cycle in reward_cycles {
        ...
        let get_nakamoto_inv =
            FunaiMessageType::GetNakamotoInv(GetNakamotoInvData { consensus_hash });
    ...
    let join_handle = thread::spawn(move || {
        ...
        // first, handshake
        let handshake_data =
            FunaiMessageType::Handshake(HandshakeData::from_local_peer(&client_peer));
        ...
        let msg: FunaiMessage = read_next(&mut tcp_socket).unwrap();
        match msg.payload {
        ...
        for get_nakamoto_inv in get_nakamoto_invs {
            ...
            loop {
                ...
                let msg: FunaiMessage = read_next(&mut tcp_socket).unwrap();
                let is_inv_reply = if let FunaiMessageType::NakamotoInv(..) = &msg.payload {
                    true
                } else {
                    false
                };
...
fn test_nakamoto_inv_10_tenures_10_sortitions() {
    ...
    assert_eq!(
        peer.config.epochs,
        Some(FunaiEpoch::unit_test_3_0_only(37))
    );
    ...
    for (rc, inv) in reward_cycle_invs.into_iter().enumerate() {
        ...
        let FunaiMessageType::NakamotoInv(inv) = inv else {
...
fn check_inv_messages(
    ...
    messages: Vec<FunaiMessageType>,
) {
    for (msg_idx, msg) in messages.into_iter().enumerate() {
        let FunaiMessageType::NakamotoInv(inv) = msg else {
