...
fn test_step_walk_1_neighbor_bad_epoch() {
    with_timeout(600, || {
        ...
        peer_1_config.epochs = Some(vec![FunaiEpoch {
        ...
        peer_2_config.epochs = Some(vec![FunaiEpoch {
...
fn funai_db_id(i: usize) -> QualifiedContractIdentifier {
...
fn make_funai_db_ids(i: usize) -> Vec<QualifiedContractIdentifier> {
    let mut dbs = vec![];
    for j in 0..i {
        dbs.push(funai_db_id(j));
    }
    dbs
}
        ...
        for (_, convo) in peers[i].network.peers.iter() {
            ...
            assert!(
                *convo
                    .stats
                    .msg_rx_counts
                    .get(&FunaiMessageID::HandshakeReject)
                    .unwrap_or(&0)
                    == 0
            );
        }
    ...
    F: FnMut(&Vec<TestPeer>) -> bool,
{
    ...
    // each peer learns each other peer's funai DBs
    for (i, peer) in peers.iter().enumerate() {
        ...
        for (j, other_peer) in peers.iter().enumerate() {
            ...
            // what does the other peer see as this peer's funai DBs?
            let mut other_peer_dbs = other_peer
                .network
                .peerdb
                .get_peer_funai_dbs(&peer.config.to_neighbor())
                .unwrap();
            other_peer_dbs.sort();

            if j % 2 == 0 {
                test_debug!(
                    "Compare funai DBs of {} vs {}",
                    &peer.config.to_neighbor(),
                    &other_peer.config.to_neighbor()
                );
                assert_eq!(expected_dbs, other_peer_dbs);
            } else {
                // this peer doesn't support Funai DBs
