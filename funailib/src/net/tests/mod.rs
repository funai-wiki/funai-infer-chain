...
use crate::chainstate::funai::boot::test::{
    key_to_funai_addr, make_pox_4_lockup, make_signer_key_signature, with_sortdb,
};
...
use crate::core::{FunaiEpoch, FunaiEpochExtension};
...
pub enum NakamotoBootStep {
    Block(Vec<FunaiTransaction>),
    TenureExtend(Vec<FunaiTransaction>),
}
...
pub struct NakamotoBootPlan {
    ...
    pub private_key: FunaiPrivateKey,
    pub initial_balances: Vec<(PrincipalData, u64)>,
    pub test_funais: Vec<TestFunai>,
...
impl NakamotoBootPlan {
    pub fn new(test_name: &str) -> Self {
        ...
        Self {
            ...
            private_key: FunaiPrivateKey::from_seed(&[2]),
            initial_balances: vec![],
            test_funais: TestFunai::common_signing_set(&test_signers),
            test_signers,
    ...
    pub fn with_private_key(mut self, privk: FunaiPrivateKey) -> Self {
    ...
    pub fn with_test_funais(mut self, test_funais: Vec<TestFunai>) -> Self {
        self.test_funais = test_funais;
        self
    }
    ...
    fn boot_nakamoto<'a>(
        ...
        let addr = FunaiAddress::from_public_keys(
            ...
            1,
            &vec![FunaiPublicKey::from_private(&self.private_key)],
        )
        .unwrap();
        ...
        peer_config.epochs = Some(FunaiEpoch::unit_test_3_0_only(
        ...
        let funai_balances = self.test_funais.iter().map(|test_funai| {
            (
                PrincipalData::from(key_to_funai_addr(&test_funai.funai_private_key)),
                u64::try_from(test_funai.amount).expect("Stacking amount too large"),
            )
        });
        let signer_balances = self.test_funais.iter().map(|test_funai| {
            (
                PrincipalData::from(key_to_funai_addr(&test_funai.signer_private_key)),
                fee_payment_balance,
            )
        });

        peer_config.initial_balances.extend(funai_balances);
        peer_config.initial_balances.extend(signer_balances);
        peer_config.test_signers = Some(self.test_signers.clone());
        peer_config.test_funais = Some(self.test_funais.clone());
        ...
        for i in 0..self.num_peers {
            ...
            other_config.test_funais = peer.config.test_funais.clone();
            other_config.private_key = FunaiPrivateKey::from_seed(&(i as u128).to_be_bytes());
    ...
    fn advance_to_nakamoto(&mut self, peer: &mut TestPeer, other_peers: &mut [TestPeer]) {
        ...
        let addr = FunaiAddress::p2pkh(false, &FunaiPublicKey::from_private(&self.private_key));
        ...
        let mut funai_block = peer.tenure_with_txs(&stack_txs, &mut peer_nonce);
        for (other_peer, other_peer_nonce) in
            other_peers.iter_mut().zip(other_peer_nonces.iter_mut())
        {
        ...
        {
            funai_block = peer.tenure_with_txs(&[], &mut peer_nonce);
            other_peers
                .iter_mut()
    ...
    pub fn boot_into_nakamoto_peers<'a>(
        ...
        let test_funais = self.test_funais.clone();
                ...
                NakamotoBootTenure::Sortition(boot_steps) => {
                    ...
                    let blocks_and_sizes = peer.make_nakamoto_tenure(
                        ...
                        |miner, chainstate, sortdb, blocks_so_far| {
                            ...
                            let mut txs = vec![];
                            // check if the funai/signers need to vote for an aggregate key. if so, append those transactions
        ...
        if let Some(observer) = observer {
            ...
            for tenure in all_blocks {
                for block in tenure {
                    ...
                    let mut funai_receipts = vec![];
                    for receipt in observed_block.receipts.iter() {
                        match &receipt.transaction {
                            TransactionOrigin::Funai(..) => {
                                funai_receipts.push(receipt);
                            }
                            TransactionOrigin::Burn(..) => burn_receipts.push(receipt),
                        }
                    }

                    assert_eq!(funai_receipts.len(), block.txs.len());
                    for (receipt, tx) in funai_receipts.iter().zip(block.txs.iter()) {
        ...
        for other_peer in other_peers.iter_mut() {
            let (other_highest_tenure, other_sort_tip) = {
                let chainstate = &mut other_peer.funai_node.as_mut().unwrap().chainstate;
