...
use funai_common::types::chainstate::{FunaiAddress, FunaiBlockId, FunaiPrivateKey};
...
use crate::chainstate::funai::db::blocks::test::make_sample_microblock_stream;
use crate::chainstate::funai::test::make_codec_test_block;
use crate::chainstate::funai::{
    FunaiTransaction, TokenTransferMemo, TransactionAuth, TransactionPayload,
...
use crate::net::httpcore::{
    HttpPreambleExtensions, HttpRequestContentsExtensions, FunaiHttp, FunaiHttpMessage,
    FunaiHttpPreamble, FunaiHttpRequest, FunaiHttpResponse,
};
use crate::net::rpc::ConversationHttp;
use crate::net::{ProtocolFamily, TipRequest};

#[test]
fn test_parse_funai_http_preamble_request_err() {
    let tests = vec![
        (
            "GET /foo HTTP/1.1\r\n",
    ...
    for (data, errstr) in tests.iter() {
        let sres = FunaiHttpPreamble::consensus_deserialize(&mut data.as_bytes());
...
#[test]
fn test_parse_funai_http_preamble_response_err() {
    let tests = vec![
        ("HTTP/1.1 200",
        "Not enough bytes to form a HTTP request or response"),
        ("HTTP/1.1 200 OK\r\nfoo: \u{2764}\r\n\r\n",
...
fn make_test_transaction() -> FunaiTransaction {
...
fn test_http_request_type_codec() {
    ...
    let fixtures = vec![
        (
            FunaiHttpRequest::new_getneighbors(convo.get_peer_host()),
        ...
        (
            FunaiHttpRequest::new_getinfo(convo.get_peer_host(), Some(1234)),
            HttpRequestPreamble::new(
                HttpVersion::Http11,
                "GET".to_string(),
                "/v2/info".to_string(),
        ...
        (
            FunaiHttpRequest::new_getinfo(convo.get_peer_host(), None),
            HttpRequestPreamble::new(
                HttpVersion::Http11,
                "GET".to_string(),
                "/v2/info".to_string(),
        ...
        (
            FunaiHttpRequest::new_getpoxinfo(convo.get_peer_host(), TipRequest::UseLatestUnconfirmedTip),
            HttpRequestPreamble::new(
                HttpVersion::Http11,
                "GET".to_string(),
        ...
        (
            FunaiHttpRequest::new_getpoxinfo(convo.get_peer_host(), TipRequest::UseLatestAnchoredTip),
            HttpRequestPreamble::new(
                HttpVersion::Http11,
                "GET".to_string(),
                "/v2/pox".to_string(),
        ...
        (
            FunaiHttpRequest::new_getheaders(convo.get_peer_host(), 2100, TipRequest::SpecificTip(FunaiBlockId([0x80; 32]))),
            HttpRequestPreamble::new(
                HttpVersion::Http11,
                "GET".to_string(),
        ...
        (
            FunaiHttpRequest::new_getblock(convo.get_peer_host(), FunaiBlockId([2u8; 32])),
            HttpRequestPreamble::new(
                HttpVersion::Http11,
                "GET".to_string(),
                format!("/v2/blocks/{}", FunaiBlockId([2u8; 32]).to_hex()),
                "localhost".to_string(),
                12345,
                true,
            ),
        ...
        (
            FunaiHttpRequest::new_getmicroblocks_indexed(convo.get_peer_host(), FunaiBlockId([3u8; 32])),
            HttpRequestPreamble::new(
                HttpVersion::Http11,
                "GET".to_string(),
                format!("/v2/microblocks/{}", FunaiBlockId([3u8; 32]).to_hex()),
                "localhost".to_string(),
                12345,
                true,
            ),
        ...
        (
            FunaiHttpRequest::new_post_transaction(convo.get_peer_host(), tx.clone()),
            HttpRequestPreamble::new(
                HttpVersion::Http11,
                "POST".to_string(),
    ...
    for (mut test, mut expected_http_preamble, expected_http_body) in fixtures.into_iter() {
        ...
        if let Some(h) = test.preamble().get_canonical_funai_tip_height() {
            expected_http_preamble.set_canonical_funai_tip_height(Some(h));
        }
        ...
        http.write_message(&mut bytes, &FunaiHttpMessage::Request(test.clone()))
            .unwrap();
...
fn test_http_response_type_codec() {
    ...
    let tests = vec![
        // length is known
        (
            FunaiHttpResponse::new_getneighbors(test_neighbors_info.clone(), true),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_getblock(test_block_info.clone(), true),
            "GET".to_string(),
            format!("/v2/blocks/{}", test_block_info.block_hash().to_hex()),
        ),
        (
            FunaiHttpResponse::new_getmicroblocks_indexed(test_microblock_info.clone(), true),
            "GET".to_string(),
            format!(
                "/v2/microblocks/{}",
        ...
        (
            FunaiHttpResponse::new_posttransaction(Txid([0x01; 32]), true),
            "POST".to_string(),
            "/v2/transactions".to_string(),
        ),
        // length is unknown
        (
            FunaiHttpResponse::new_getneighbors(test_neighbors_info.clone(), false),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_getblock(test_block_info.clone(), false),
            "GET".to_string(),
            format!("/v2/blocks/{}", test_block_info.block_hash().to_hex()),
        ),
        (
            FunaiHttpResponse::new_getmicroblocks_indexed(test_microblock_info.clone(), false),
            "GET".to_string(),
            format!(
                "/v2/microblocks/{}",
                test_microblock_info[0].block_hash().to_hex()
        ...
        (
            FunaiHttpResponse::new_posttransaction(Txid([0x01; 32]), false),
            "POST".to_string(),
            "/v2/transactions".to_string(),
        ),
        // errors without error messages
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(400, "".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(401, "".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(402, "".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(403, "".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(404, "".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(500, "".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(503, "".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(502, "".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        // errors with specific messages
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(400, "foo".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(401, "foo".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(402, "foo".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(403, "foo".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(404, "foo".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(500, "foo".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(503, "foo".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
        (
            FunaiHttpResponse::new_empty_error(&*http_error_from_code_and_text(502, "foo".into())),
            "GET".to_string(),
            "/v2/neighbors".to_string(),
        ),
    ];
        ...
        )
    {
        ...
        http.write_message(&mut bytes, &FunaiHttpMessage::Response((*test).clone()))
            .unwrap();
        ...
        match preamble {
            FunaiHttpPreamble::Response(ref mut req) => {
        ...
        assert_eq!(
            preamble,
            FunaiHttpPreamble::Response((*expected_http_preamble).clone())
        );

        // note that message's headers contain cors headers and the like, which we don't synthesize
        // here
        match message {
            FunaiHttpMessage::Response(ref mut response) => response.clear_headers(),
            _ => {
                panic!("Not an HTTP response");
            }
        }
        assert_eq!(message, FunaiHttpMessage::Response((*test).clone()));
...
fn test_http_duplicate_concurrent_streamed_response_fails() {
    ...
    let msg = http
        ...
        .unwrap();
    match msg {
        (Some((FunaiHttpMessage::Response(response), _)), _) => assert_eq!(
...
fn test_metrics_identifiers() {
    ...
    for (input, output) in fixtures {
        ...
        let mut request = FunaiHttpRequest::new(preamble, HttpRequestContents::new());
