...
use super::FunaiDBEventDispatcher;
use crate::chainstate::funai::address::PoxAddress;
use crate::net::funaidb::{FunaiDBConfig, FunaiDBTx, FunaiDBs, STACKERDB_INV_MAX};
use crate::net::{Error as net_error, FunaiDBChunkData, FunaiDBHandshakeData};
...
const STACKER_DB_SCHEMA: &'static [&'static str] = &[
    ...
    CREATE TABLE databases(
        -- internal numeric identifier for this funaidb's smart contract identifier
        -- (so we don't have to copy it into each chunk row)
        funaidb_id INTEGER NOT NULL,
        -- smart contract ID for this funaidb
        smart_contract_id TEXT UNIQUE NOT NULL,
        PRIMARY KEY(funaidb_id)
    );
    "#,
    r#"
    ...
    CREATE TABLE chunks(
        ...
        -- secp256k1 recoverable signature from the funai over the above columns
        ...
        PRIMARY KEY(funaidb_id,slot_id),
        FOREIGN KEY(funaidb_id) REFERENCES databases(funaidb_id) ON DELETE CASCADE
    );
    "#,
    r#"
    CREATE INDEX on_funai_db_slots ON chunks(funaidb_id,slot_id,version);
    "#,
];
...
impl FromRow<SlotValidation> for SlotValidation {
    fn from_row(row: &Row) -> Result<SlotValidation, db_error> {
        let signer = FunaiAddress::from_column(row, "signer")?;
...
impl FromRow<FunaiDBChunkData> for FunaiDBChunkData {
    fn from_row(row: &Row) -> Result<FunaiDBChunkData, db_error> {
        let slot_id: u32 = row.get_unwrap("slot_id");
        let slot_version: u32 = row.get_unwrap("version");
...
/// Get the local numeric ID of a funai DB.
/// Returns Err(NoSuchFunaiDB(..)) if it doesn't exist
fn inner_get_funaidb_id(
    conn: &DBConn,
    ...
    Ok(query_row(conn, sql, args)?.ok_or(net_error::NoSuchFunaiDB(smart_contract.clone()))?)
}
...
fn inner_get_slot_metadata(
    ...
    let funaidb_id = inner_get_funaidb_id(conn, smart_contract)?;
    let sql = "SELECT slot_id,version,data_hash,signature FROM chunks WHERE funaidb_id = ?1 AND slot_id = ?2";
    let args: &[&dyn ToSql] = &[&funaidb_id, &slot_id];
...
fn inner_get_slot_validation(
    ...
    let funaidb_id = inner_get_funaidb_id(conn, smart_contract)?;
    let sql =
        "SELECT signer,write_time,version FROM chunks WHERE funaidb_id = ?1 AND slot_id = ?2";
    let args: &[&dyn ToSql] = &[&funaidb_id, &slot_id];
    query_row(conn, &sql, args).map_err(|e| e.into())
}
...
impl<'a> FunaiDBTx<'a> {
    ...
    /// Delete a funai DB table and its contents.
    /// Idempotent.
    pub fn delete_funaidb(
        &self,
    ...
    /// List all funai DB smart contracts we have available
    pub fn get_funaidb_contract_ids(
        &self,
    ...
    /// Get the Funai DB ID for a smart contract
    pub fn get_funaidb_id(
        &self,
        smart_contract: &QualifiedContractIdentifier,
    ) -> Result<i64, net_error> {
        inner_get_funaidb_id(&self.conn(), smart_contract)
    }
    ...
    pub fn create_funaidb(
        &self,
        smart_contract: &QualifiedContractIdentifier,
        slots: &[(FunaiAddress, u32)],
    ) -> Result<(), net_error> {
        if slots.len() > (STACKERDB_INV_MAX as usize) {
            return Err(net_error::ArrayTooLong);
        }

        if self.get_funaidb_id(smart_contract).is_ok() {
            return Err(net_error::FunaiDBExists(smart_contract.clone()));
        }
        ...
        let funaidb_id = self.get_funaidb_id(smart_contract)?;

        let qry = "INSERT OR REPLACE INTO chunks (funaidb_id,signer,slot_id,version,write_time,data,data_hash,signature) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)";
        let mut stmt = self.sql_tx.prepare(&qry)?;
        let mut slot_id = 0u32;

        for (principal, slot_count) in slots.iter() {
            test_debug!("Create FunaiDB slots: ({}, {})", &principal, slot_count);
    ...
    pub fn clear_funaidb_slots(
        &self,
        smart_contract: &QualifiedContractIdentifier,
    ) -> Result<(), net_error> {
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let qry = "DELETE FROM chunks WHERE funaidb_id = ?1";
        let args: &[&dyn ToSql] = &[&funaidb_id];
        let mut stmt = self.sql_tx.prepare(&qry)?;
        stmt.execute(args)?;
        Ok(())
    }
    ...
    fn insert_chunk(
        ...
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let sql = "UPDATE chunks SET version = ?1, data_hash = ?2, signature = ?3, data = ?4, write_time = ?5 WHERE funaidb_id = ?6 AND slot_id = ?7";
    ...
    /// Try to upload a chunk to the FunaiDB instance, notifying
    ///  and subscribed listeners via the `dispatcher`
    pub fn put_chunk<ED: FunaiDBEventDispatcher>(
        self,
        contract: &QualifiedContractIdentifier,
        chunk: FunaiDBChunkData,
        dispatcher: &ED,
        miner_endpoint: Option<String>,
    ) -> Result<(), net_error> {
        self.try_replace_chunk(contract, &chunk.get_slot_metadata(), &chunk.data)?;
        self.commit()?;
        dispatcher.new_funaidb_chunks(contract.clone(), vec![chunk], miner_endpoint);
        Ok(())
    }
    ...
    pub fn try_replace_chunk(
        ...
        if chunk.len() > STACKERDB_MAX_CHUNK_SIZE as usize {
            return Err(net_error::FunaiDBChunkTooBig(chunk.len()));
        }
...
impl FunaiDBs {
    /// Instantiate the DB
    fn instantiate(path: &str, readwrite: bool) -> Result<FunaiDBs, net_error> {
        ...
        if create_flag {
            let db_tx = db.tx_begin(FunaiDBConfig::noop())?;
    ...
    /// Connect to a funai DB, creating it if it doesn't exist and if readwrite is true.
    /// Readwrite is enforced by the underling sqlite connection.
    pub fn connect(path: &str, readwrite: bool) -> Result<FunaiDBs, net_error> {
        Self::instantiate(path, readwrite)
    }

    #[cfg(test)]
    pub fn connect_memory() -> FunaiDBs {
        Self::instantiate(":memory:", true).unwrap()
    }

    /// Open the FunaiDBs again
    pub fn reopen(&self) -> Result<FunaiDBs, net_error> {
        Self::instantiate(&self.path, true)
    }

    /// Open a transaction on the Funai DB.
    /// The config would be obtained from a DBSelector instance
    pub fn tx_begin<'a>(
        &'a mut self,
        config: FunaiDBConfig,
    ) -> Result<FunaiDBTx<'a>, db_error> {
        let sql_tx = tx_begin_immediate(&mut self.conn)?;
        Ok(FunaiDBTx { sql_tx, config })
    }
    ...
    /// Get the principal who signs a particular slot in a particular funai DB.
    ...
    pub fn get_slot_signer(
        ...
        smart_contract: &QualifiedContractIdentifier,
        slot_id: u32,
    ) -> Result<Option<FunaiAddress>, net_error> {
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let sql = "SELECT signer FROM chunks WHERE funaidb_id = ?1 AND slot_id = ?2";
        let args: &[&dyn ToSql] = &[&funaidb_id, &slot_id];
        query_row(&self.conn, &sql, args).map_err(|e| e.into())
    }

    /// Get all principals who can write to a particular funai DB.
    /// Returns Ok(list of addr) if this contract exists in the DB
    /// Returns Err(..) if the DB doesn't exist of some other DB error happens
    pub fn get_signers(
        &self,
        smart_contract: &QualifiedContractIdentifier,
    ) -> Result<Vec<FunaiAddress>, net_error> {
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let sql = "SELECT signer FROM chunks WHERE funaidb_id = ?1 GROUP BY signer";
        let args: &[&dyn ToSql] = &[&funaidb_id];
    ...
    pub fn get_db_slot_metadata(
        ...
        let funaidb_id = inner_get_funaidb_id(&self.conn, smart_contract)?;
        let sql = "SELECT slot_id,version,data_hash,signature FROM chunks WHERE funaidb_id = ?1 ORDER BY slot_id ASC";
        let args: &[&dyn ToSql] = &[&funaidb_id];
        query_rows(&self.conn, &sql, args).map_err(|e| e.into())
    }
    ...
    pub fn get_slot_version(
        ...
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let qry = "SELECT version FROM chunks WHERE funaidb_id = ?1 AND slot_id = ?2";
        let args: &[&dyn ToSql] = &[&funaidb_id, &slot_id];
    ...
    pub fn get_slot_versions(
        ...
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let sql = "SELECT version FROM chunks WHERE funaidb_id = ?1 ORDER BY slot_id";
        let args: &[&dyn ToSql] = &[&funaidb_id];
        query_rows(&self.conn, &sql, args).map_err(|e| e.into())
    }
    ...
    pub fn get_slot_write_timestamps(
        ...
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let sql = "SELECT write_time FROM chunks WHERE funaidb_id = ?1 ORDER BY slot_id";
        let args: &[&dyn ToSql] = &[&funaidb_id];
        query_rows(&self.conn, &sql, args).map_err(|e| e.into())
    }
    ...
    pub fn get_latest_chunk(
        ...
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let qry = "SELECT data FROM chunks WHERE funaidb_id = ?1 AND slot_id = ?2";
        let args: &[&dyn ToSql] = &[&funaidb_id, &slot_id];
    ...
    pub fn get_chunk(
        ...
        slot_id: u32,
        slot_version: u32,
    ) -> Result<Option<FunaiDBChunkData>, net_error> {
        let funaidb_id = self.get_funaidb_id(smart_contract)?;
        let qry = "SELECT slot_id,version,signature,data FROM chunks WHERE funaidb_id = ?1 AND slot_id = ?2 AND version = ?3";
        let args: &[&dyn ToSql] = &[&funaidb_id, &slot_id, &slot_version];
