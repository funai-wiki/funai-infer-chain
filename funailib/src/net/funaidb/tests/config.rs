...
use funai_common::types::chainstate::{
    BurnchainHeaderHash, FunaiAddress, FunaiPrivateKey, FunaiPublicKey,
};
...
use crate::chainstate::funai::{
    FunaiTransaction, FunaiTransactionSigner, TransactionAuth, TransactionPayload,
    TransactionVersion,
};
use crate::core::{FunaiEpochExtension, BITCOIN_REGTEST_FIRST_BLOCK_HASH};
use crate::net::test::TestEventObserver;
use crate::net::{Error as net_error, NeighborAddress, PeerAddress, FunaiDBConfig};
...
#[test]
fn test_valid_and_invalid_funaidb_configs() {
    ...
    let (mut peer, mut keys) = instantiate_pox_peer_with_epoch(
        &burnchain,
        "test_valid_and_invalid_funaidb_configs",
        Some(epochs.clone()),
        Some(&observer),
    );
    ...
    let testcases = vec![
        (
            // valid
            r#"
            (define-public (funaidb-get-signer-slots)
        ...
        (
            ...
            r#"
            (define-public (funaidb-get-signer-slots (bad-arg uint))
                (ok (list { signer: 'ST2TFVBMRPS5SSNP98DQKQ5JNB2B6NZM91C4K3P7B, num-slots: u3 })))
    ...
    for (i, (code, result)) in testcases.iter().enumerate() {
        let contract_id = QualifiedContractIdentifier::new(
            FunaiAddress::from_public_keys(
                ...
                1,
                &vec![FunaiPublicKey::from_private(&contract_owner)],
            )
            .unwrap()
            .into(),
        ...
        peer.with_db_state(|sortdb, chainstate, _, _| {
            match FunaiDBConfig::from_smart_contract(chainstate, sortdb, &contract_id) {
                Ok(config) => {
                    let expected = result
                        .clone()
                ...
                Err(net_error::InvalidFunaiDBContract(..)) => {
                    assert!(
                        result.is_none(),
