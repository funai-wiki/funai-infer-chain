...
use funai_common::types::chainstate::{
    ConsensusHash, FunaiAddress, FunaiPrivateKey, FunaiPublicKey,
};
...
/// Test that we can instantiate a funai DB instance
#[test]
fn test_funaidb_connect() {
    let path = "/tmp/funai-node-tests/test_funaidb_connect.sqlite";
    setup_test_path(path);

    let _ = FunaiDBs::connect(path, true).unwrap();
}

/// Test that we can create, enumerate, and drop FunaiDB tables.
#[test]
fn test_funaidb_create_list_delete() {
    let path = "/tmp/funai-node-tests/test_funaidb_create_list_delete.sqlite";
    setup_test_path(path);

    let mut db = FunaiDBs::connect(path, true).unwrap();
    let tx = db.tx_begin(FunaiDBConfig::noop()).unwrap();
    ...
    let mut dbs = db.get_funaidb_contract_ids().unwrap();
    dbs.sort();
    ...
    let tx = db.tx_begin(FunaiDBConfig::noop()).unwrap();
    if let net_error::FunaiDBExists(..) = tx
        .create_funaidb(
            &QualifiedContractIdentifier::new(
                FunaiAddress {
                    version: 0x01,
        ...
        .unwrap_err()
    {
        ...
        panic!("Did not error on creating the same funai DB twice");
    }
    tx.commit().unwrap();
    ...
    // remove a db
    let tx = db.tx_begin(FunaiDBConfig::noop()).unwrap();
    tx.delete_funaidb(&QualifiedContractIdentifier::new(
        FunaiAddress {
            version: 0x01,
            bytes: Hash160([0x01; 20]),
        }
...
/// Test that we can set up a FunaiDB with a given config, and clear it.
#[test]
fn test_funaidb_prepare_clear_slots() {
    let path = "/tmp/test_funaidb_prepare_clear_slots.sqlite";
    setup_test_path(path);
    ...
    let tx = db.tx_begin(FunaiDBConfig::noop()).unwrap();
    tx.clear_funaidb_slots(&sc).unwrap();
    tx.commit().unwrap();
...
/// Test that we can insert and query chunks to a FunaiDB.
...
#[test]
fn test_funaidb_insert_query_chunks() {
    let path = "/tmp/test_funaidb_insert_query_chunks.sqlite";
    setup_test_path(path);
    ...
    let mut db_config = FunaiDBConfig::noop();
    db_config.max_writes = 3;
    db_config.write_freq = 120;

    let tx = db.tx_begin(db_config.clone()).unwrap();

    let pks: Vec<_> = (0..10).map(|_| FunaiPrivateKey::new()).collect();
    let addrs: Vec<_> = pks
        .iter()
        .map(|pk| {
    ...
    for (i, pk) in pks.iter().enumerate() {
        let mut chunk_data = FunaiDBChunkData {
            slot_id: i as u32,
            slot_version: 1,
        ...
        if let Err(net_error::BadSlotSigner(funai, slot_id)) =
            tx.try_replace_chunk(&sc, &chunk_data.get_slot_metadata(), &chunk_data.data)
        {
            assert_eq!(funai, addrs[i]);
...
#[test]
fn test_reconfigure_funaidb() {
    let path = "/tmp/test_funaidb_reconfigure.sqlite";
    setup_test_path(path);
    ...
    let new_pks: Vec<_> = (0..10).map(|_| FunaiPrivateKey::new()).collect();
    ...
    for (i, pk) in new_pks.iter().enumerate() {
        if i < 5 {
            ...
            let chunk_data = FunaiDBChunkData {
                slot_id: i as u32,
                slot_version: 1,
                sig: MessageSignature::empty(),
                data: vec![i as u8; 128],
