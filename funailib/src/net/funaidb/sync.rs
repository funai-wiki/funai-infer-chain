...
use crate::net::funaidb::{
    FunaiDBConfig, FunaiDBSync, FunaiDBSyncResult, FunaiDBSyncState, FunaiDBs,
};
use crate::net::{
    Error as net_error, NackData, NackErrorCodes, Neighbor, NeighborAddress, NeighborKey,
    FunaiDBChunkData, FunaiDBChunkInvData, FunaiDBGetChunkData, FunaiDBGetChunkInvData,
    FunaiDBPushChunkData, FunaiMessageType,
};

const MAX_CHUNKS_IN_FLIGHT: usize = 6;
const MAX_DB_NEIGHBORS: usize = 32;

impl<NC: NeighborComms> FunaiDBSync<NC> {
    pub fn new(
        ...
        let mut dbsync = FunaiDBSync {
            state: FunaiDBSyncState::ConnectBegin,
    ...
    /// Find funaidb replicas and apply filtering rules
    fn find_qualified_replicas(
        ...
        while found.len() < self.max_neighbors {
            let peers_iter = PeerDB::find_funai_db_replicas(
                network.peerdb_conn(),
    ...
    /// Reset this state machine, and get the FunaiDBSyncResult with newly-obtained chunk data
    ...
    pub fn reset(
        ...
        self.state = FunaiDBSyncState::ConnectBegin;
        result
    }
    ...
    pub fn make_getchunkinv(&self, rc_consensus_hash: &ConsensusHash) -> FunaiMessageType {
    ...
    pub fn make_chunk_request_schedule(
    ...
    ) -> Result<Vec<(FunaiDBGetChunkData, Vec<NeighborAddress>)>, net_error> {
        ...
        let local_slot_versions = if let Some(local_slot_versions) = local_slot_versions_opt {
            ...
            self.funaidbs.get_slot_versions(&self.smart_contract_id)?
        };
        ...
        let mut need_chunks: HashMap<usize, (FunaiDBGetChunkData, Vec<NeighborAddress>)> =
            HashMap::new();
        ...
        let mut schedule: Vec<_> = need_chunks
            .into_iter()
            .map(|(_, (funaidb_getchunkdata, mut neighbors))| {
                neighbors.shuffle(&mut thread_rng());
                (funaidb_getchunkdata, neighbors)
            })
            .collect();
    ...
    pub fn make_chunk_push_schedule(
        &self,
        network: &PeerNetwork,
    ) -> Result<Vec<(FunaiDBPushChunkData, Vec<NeighborAddress>)>, net_error> {
        let rc_consensus_hash = network.get_chain_view().rc_consensus_hash.clone();
        let local_slot_versions = self.funaidbs.get_slot_versions(&self.smart_contract_id)?;

        let mut need_chunks: HashMap<usize, (FunaiDBPushChunkData, Vec<NeighborAddress>)> =
            HashMap::new();
        ...
        for (i, local_version) in local_slot_versions.iter().enumerate() {
            ...
            for (naddr, chunk_inv) in self.chunk_invs.iter() {
                ...
                if local_chunk.is_none() {
                    let chunk_data = if let Some(chunk_data) = self.funaidbs.get_chunk(
                        &self.smart_contract_id,
                        i as u32,
                        *local_version,
                    )? {
                    ...
                    local_chunk = Some(FunaiDBPushChunkData {
                        contract_id: self.smart_contract_id.clone(),
                        rc_consensus_hash: rc_consensus_hash.clone(),
                        chunk_data,
                    });
        ...
        let mut schedule: Vec<_> = need_chunks
            .into_iter()
            .map(|(_, (funaidb_chunkdata, neighbors))| (funaidb_chunkdata, neighbors))
            .collect();
    ...
    pub fn add_downloaded_chunk(&mut self, naddr: NeighborAddress, data: FunaiDBChunkData) {
    ...
    fn send_getchunkinv_to_inbound_neighbors(
        ...
        for event_id in network.iter_peer_event_ids() {
            ...
            if !convo.replicates_funaidb(&self.smart_contract_id) {
                continue;
            }
        ...
        for (naddr, chunks_req) in to_send.into_iter() {
            debug!("{:?}: send_getchunksinv_to_inbound_neighbors: Send FunaiDBGetChunkInv to inbound {:?}", network.get_local_peer(), &naddr);
            if let Err(_e) = self.comms.neighbor_send(network, &naddr, chunks_req) {
                info!(
                    "{:?}: Failed to send FunaiDBGetChunkInv to inbound {:?}: {:?}",
                    network.get_local_peer(),
                    &naddr,
                    &_e
                );
    ...
    pub fn connect_begin(&mut self, network: &mut PeerNetwork) -> Result<bool, net_error> {
        ...
        debug!(
            "{:?}: connect_begin: establish FunaiDB sessions to {} neighbors",
            network.get_local_peer(),
            self.replicas.len()
        );
        if self.replicas.len() == 0 {
        ...
        for naddr in naddrs.into_iter() {
            if self.comms.has_neighbor_session(network, &naddr) {
                debug!(
                    "{:?}: connect_begin: already connected to FunaiDB peer {:?}",
                    network.get_local_peer(),
                    &naddr
                );
            ...
            debug!(
                "{:?}: connect_begin: Send Handshake to FunaiDB peer {:?}",
                network.get_local_peer(),
                &naddr
            );
            match self.comms.neighbor_session_begin(network, &naddr) {
                Ok(true) => {
                    // connected!
                    debug!(
                        "{:?}: connect_begin: connected to FunaiDB peer {:?}",
                        network.get_local_peer(),
                        &naddr
                    );
                }
    ...
    pub fn connect_try_finish(&mut self, network: &mut PeerNetwork) -> Result<bool, net_error> {
        for (naddr, message) in self.comms.collect_replies(network).into_iter() {
            let data = match message.payload {
                FunaiMessageType::FunaiDBHandshakeAccept(_, db_data) => {
                ...
                FunaiMessageType::Nack(data) => {
                    debug!(
                        "{:?}: remote peer {:?} NACK'ed our FunaiDBHandshake with code {}",
                        &network.get_local_peer(),
                        &naddr,
                        data.error_code
                    );
            ...
            debug!(
                "{:?}: connect_try_finish: Received FunaiDBHandshakeAccept from {:?} for {:?}",
                network.get_local_peer(),
                &naddr,
                &data
            );
    ...
    pub fn getchunksinv_begin(&mut self, network: &mut PeerNetwork) {
        ...
        debug!(
            "{:?}: getchunksinv_begin: Send FunaiDBGetChunksInv to {} replicas",
            network.get_local_peer(),
            naddrs.len()
        );
        for naddr in naddrs.into_iter() {
            debug!(
                "{:?}: getchunksinv_begin: Send FunaiDBGetChunksInv to {:?}",
                network.get_local_peer(),
                &naddr
            );
            let chunks_req = self.make_getchunkinv(&network.get_chain_view().rc_consensus_hash);
            if let Err(e) = self.comms.neighbor_send(network, &naddr, chunks_req) {
                info!(
                    "{:?}: failed to send FunaiDBGetChunkInv to {:?}: {:?}",
                    network.get_local_peer(),
                    &naddr,
                    &e
                );
    ...
    pub fn getchunksinv_try_finish(
        ...
        for (naddr, message) in self.comms.collect_replies(network).into_iter() {
            let chunk_inv_opt = match message.payload {
                FunaiMessageType::FunaiDBChunkInv(data) => {
                    if data.slot_versions.len() != self.num_slots {
                        info!("{:?}: Received malformed FunaiDBChunkInv for {} from {:?}: expected {} chunks, got {}", network.get_local_peer(), &self.smart_contract_id, &naddr, self.num_slots, data.slot_versions.len());
                        None
                    } else {
                        Some(data)
                    }
                ...
                FunaiMessageType::Nack(data) => {
                    debug!(
                        "{:?}: remote peer {:?} NACK'ed our FunaiDBGetChunksInv us with code {}",
                        &network.get_local_peer(),
                        &naddr,
                        data.error_code
                    );
            ...
            debug!(
                "{:?}: getchunksinv_try_finish: Received FunaiDBChunkInv from {:?}",
                network.get_local_peer(),
                &naddr
            );
        ...
        let expected_versions = self.funaidbs.get_slot_versions(&self.smart_contract_id)?;
    ...
    pub fn getchunks_begin(&mut self, network: &mut PeerNetwork) -> Result<bool, net_error> {
        ...
        debug!(
            "{:?}: getchunks_begin: Issue up to {} FunaiDBGetChunk requests",
            &network.get_local_peer(),
        ...
        for _i in 0..self.request_capacity {
            ...
            debug!(
                "{:?}: getchunks_begin: Send FunaiDBGetChunk(db={},id={},ver={}) to {}",
                &network.get_local_peer(),
                &self.smart_contract_id,
            ...
            if let Err(e) = self.comms.neighbor_send(
                network,
                &selected_neighbor,
                FunaiMessageType::FunaiDBGetChunk(chunk_request.clone()),
            ) {
                info!(
    ...
    pub fn getchunks_try_finish(
        ...
        for (naddr, message) in self.comms.collect_replies(network).into_iter() {
            let data = match message.payload {
                FunaiMessageType::FunaiDBChunk(data) => data,
                FunaiMessageType::Nack(data) => {
                    debug!(
                        "{:?}: remote peer {:?} NACK'ed our FunaiDBGetChunk with code {}",
                        network.get_local_peer(),
                        &naddr,
                        data.error_code
                    );
            ...
            // update bookkeeping
            debug!(
                "{:?}: getchunks_try_finish: Received FunaiDBChunk from {:?}",
                network.get_local_peer(),
                &naddr
            );
    ...
    pub fn pushchunks_begin(&mut self, network: &mut PeerNetwork) -> Result<bool, net_error> {
        ...
        debug!(
            "{:?}: pushchunks_begin: Send up to {} FunaiDBChunk pushes",
            &network.get_local_peer(),
        ...
        for _i in 0..self.request_capacity {
            ...
            let (idx, selected_neighbor) = if let Some(x) = selected_neighbor_opt {
                ...
                debug!("{:?}: pushchunks_begin: no available neighbor to send FunaiDBChunk(db={},id={},ver={}) to",
                    &network.get_local_peer(),
                    &self.smart_contract_id,
            ...
            debug!(
                "{:?}: pushchunks_begin: Send FunaiDBChunk(db={},id={},ver={}) to {}",
                &network.get_local_peer(),
                &self.smart_contract_id,
                chunk_push.chunk_data.slot_id,
                chunk_push.chunk_data.slot_version,
            ...
            if let Err(e) = self.comms.neighbor_send(
                network,
                &selected_neighbor,
                FunaiMessageType::FunaiDBPushChunk(chunk_push),
            ) {
                info!(
    ...
    pub fn pushchunks_try_finish(&mut self, network: &mut PeerNetwork) -> bool {
        for (naddr, message) in self.comms.collect_replies(network).into_iter() {
            let new_chunk_inv = match message.payload {
                FunaiMessageType::FunaiDBChunkInv(data) => data,
                FunaiMessageType::Nack(data) => {
                    debug!(
                        "{:?}: remote peer {:?} NACK'ed our FunaiDBChunk with code {}",
                        network.get_local_peer(),
                        &naddr,
                        data.error_code
                    );
            ...
            if new_chunk_inv.slot_versions.len() != self.num_slots {
                info!("{:?}: Received malformed FunaiDBChunkInv from {:?}: expected {} chunks, got {}", network.get_local_peer(), &naddr, self.num_slots, new_chunk_inv.slot_versions.len());
                continue;
            }

            // update bookkeeping
            debug!(
                "{:?}: pushchunks_try_finish: Received FunaiDBChunkInv from {:?}",
                network.get_local_peer(),
                &naddr
            );
    ...
    pub fn wakeup(&mut self) {
        debug!("wake up FunaiDB sync for {}", &self.smart_contract_id);
        self.last_run_ts = 0;
    }
    ...
    pub fn run(
        ...
        loop {
            debug!(
                "{:?}: funai DB sync state is {:?}",
                network.get_local_peer(),
                &self.state
            );
            let mut blocked = true;
            match self.state {
                FunaiDBSyncState::ConnectBegin => {
                    let done = self.connect_begin(network)?;
                    if done {
                        self.state = FunaiDBSyncState::ConnectFinish;
                        blocked = false;
                    }
                }
                FunaiDBSyncState::ConnectFinish => {
                    let done = self.connect_try_finish(network)?;
                    if done {
                        self.state = FunaiDBSyncState::GetChunksInvBegin;
                        blocked = false;
                    }
                }
                FunaiDBSyncState::GetChunksInvBegin => {
                    // does not block
                    self.getchunksinv_begin(network);
                    self.state = FunaiDBSyncState::GetChunksInvFinish;
                    blocked = false;
                }
                FunaiDBSyncState::GetChunksInvFinish => {
                    let done = self.getchunksinv_try_finish(network)?;
                    if done {
                        self.state = FunaiDBSyncState::GetChunks;
                        blocked = false;
                    }
                }
                FunaiDBSyncState::GetChunks => {
                    if network.get_connection_opts().disable_funaidb_get_chunks {
                        // fault injection -- force the system to rely exclusively on push-chunk
                        // behavior
                        self.state = FunaiDBSyncState::PushChunks;
                        continue;
                    }
                ...
                FunaiDBSyncState::PushChunks => {
                    ...
                    if done {
                        if self.need_resync
                            && !network.get_connection_opts().disable_funaidb_get_chunks
                        {
                            ...
                            // done syncing
                            self.state = FunaiDBSyncState::Finished;
                        }
                        self.need_resync = false;
                        blocked = false;
                    }
                }
                FunaiDBSyncState::Finished => {
