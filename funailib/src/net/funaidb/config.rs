...
/// This file implements the interface to the FunaiDB smart contract for loading the DB's config.
/// The smart contract must conform to this trait:
///
/// ```clarity,ignore
/// ;; Any FunaiDB smart contract must conform to this trait.
/// (define-trait funaidb-trait
///
///     ;; Get the list of (signer, num-slots) that make up this DB
///     (define-public (funaidb-get-signer-slots) (response (list 4096 { signer: principal, num-slots: uint }) uint))
///
///     ;; Get the control metadata for this DB
///     (define-public (funaidb-get-config)
///         (response {
///             chunk-size: uint,
///             write-freq: uint,
///             max-writes: uint,
...
use crate::net::funaidb::{
    FunaiDBConfig, FunaiDBs, STACKERDB_CONFIG_FUNCTION, STACKERDB_INV_MAX,
...
impl FunaiDBConfig {
    /// Check that a smart contract is consistent with being a FunaiDB controller.
    /// Returns Ok(..) if the contract is valid
    /// Returns Err(reason) if the contract is invalid.  A human-readable reason will be given.
    fn is_contract_valid(epoch: &FunaiEpochId, analysis: ContractAnalysis) -> Result<(), String> {
    ...
    fn parse_slot_entry(
    ...
    ) -> Result<(FunaiAddress, u32), String> {
        let ClarityValue::Tuple(slot_data) = entry else {
            let reason = format!(
                "FunaiDB fn `{contract_id}.{STACKERDB_SLOTS_FUNCTION}` returned non-tuple slot entry",
            );
            return Err(reason);
        };
        ...
        let Ok(ClarityValue::Principal(signer_principal)) = slot_data.get("signer") else {
            let reason = format!(
                "FunaiDB fn `{contract_id}.{STACKERDB_SLOTS_FUNCTION}` returned tuple without `signer` entry of type `principal`",
            );
            return Err(reason);
        };
        ...
        let Ok(ClarityValue::UInt(num_slots)) = slot_data.get("num-slots") else {
            let reason = format!(
                "FunaiDB fn `{contract_id}.{STACKERDB_SLOTS_FUNCTION}` returned tuple without `num-slots` entry of type `uint`",
            );
            return Err(reason);
        };
        ...
        let PrincipalData::Standard(standard_principal) = signer_principal else {
            return Err(format!(
                "FunaiDB contract `{contract_id}` set a contract principal as a writer, which is not supported"
            ));
        };
        let addr = FunaiAddress::from(standard_principal.clone());
        Ok((addr, num_slots))
    }
    ...
    fn eval_signer_slots(
        ...
        contract_id: &QualifiedContractIdentifier,
        tip: &FunaiBlockId,
    ) -> Result<Vec<(FunaiAddress, u32)>, NetError> {
        ...
        let slot_list = match result {
            Err(err_val) => {
                ...
                let reason = format!(
                    "Contract {} failed to run `funaidb-get-signer-slots`: error u{}",
                    contract_id, &err_code
                );
                warn!("{}", &reason);
                return Err(NetError::InvalidFunaiDBContract(
                    contract_id.clone(),
                    reason,
                ));
            }
        ...
        for slot_value in slot_list.into_iter() {
            let (addr, num_slots) =
                Self::parse_slot_entry(slot_value, contract_id).map_err(|e| {
                    warn!("Failed to parse FunaiDB slot entry: {}", &e);
                    NetError::InvalidFunaiDBContract(contract_id.clone(), e)
                })?;
    ...
    fn eval_config(
        ...
        contract_id: &QualifiedContractIdentifier,
        tip: &FunaiBlockId,
        signers: Vec<(FunaiAddress, u32)>,
    ) -> Result<FunaiDBConfig, NetError> {
        let value =
            chainstate.eval_read_only(burn_dbconn, tip, contract_id, "(funaidb-get-config)")?;
        ...
        let config_tuple = match result {
            Err(err_val) => {
                let err_code = err_val.expect_u128()?;
                let reason = format!(
                    "Contract {} failed to run `funaidb-get-config`: err u{}",
                    contract_id, &err_code
                );
                warn!("{}", &reason);
                return Err(NetError::InvalidFunaiDBContract(
    ...
    /// Load up the DB config from the controlling smart contract as of the current Funai chain
    /// tip
    pub fn from_smart_contract(
        ...
        let chain_tip =
            NakamotoChainState::get_canonical_block_header(chainstate.db(), sortition_db)?
                .ok_or(NetError::NoSuchFunaiDB(contract_id.clone()))?;

        let burn_tip = SortitionDB::get_block_snapshot_consensus(
            sortition_db.conn(),
            &chain_tip.consensus_hash,
        )?
        .expect("FATAL: missing snapshot for Funai block");

        let chain_tip_hash = FunaiBlockId::new(
            &chain_tip.consensus_hash,
            &chain_tip.anchored_header.block_hash(),
        );
        let cur_epoch = SortitionDB::get_funai_epoch(sortition_db.conn(), burn_tip.block_height)?
        ...
        let res = chainstate.with_read_only_clarity_tx(&dbconn, &chain_tip_hash, |clarity_tx| {
            ...
            clarity_tx.with_clarity_db_readonly(|db| {
                ...
                // contract must be consistent with FunaiDB control interface
                if let Err(invalid_reason) = Self::is_contract_valid(&cur_epoch.epoch_id, analysis)
                {
                    let reason = format!(
                        "Contract {} does not conform to FunaiDB trait: {}",
        ...
        if res.is_none() {
            ...
            warn!(
                "Could not use contract {} for FunaiDB: {:?}",
                contract_id, &e
            );
            return Err(e);
        }
