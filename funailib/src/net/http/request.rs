...
use serde_json;
use funai_common::codec::{write_next, Error as CodecError, FunaiMessageCodec};
use funai_common::deps_common::httparse;
...
impl HttpRequestPreamble {
    ...
    /// Create a request destined for another Funai node.
    /// Funai nodes support HTTP/1.1 and keep-alive
    pub fn new_for_peer(
        peerhost: PeerHost,
        verb: String,
        path_and_query_str: String,
...
impl FunaiMessageCodec for HttpRequestPreamble {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), CodecError> {
        ...
        // "User-Agent: $agent\r\nHost: $host\r\n"
        fd.write_all("User-Agent: funai/3.0\r\nHost: ".as_bytes())
            .map_err(CodecError::WriteError)?;
...
impl HttpRequestContents {
    ...
    pub fn payload_funai<T: FunaiMessageCodec>(mut self, payload: &T) -> Self {
