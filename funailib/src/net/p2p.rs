...
use funai_common::types::chainstate::{PoxId, SortitionId};
use funai_common::types::net::{PeerAddress, PeerHost};
...
use crate::chainstate::coordinator::{
    ...
    static_get_funai_tip_affirmation_map,
};
use crate::chainstate::funai::boot::MINERS_NAME;
use crate::chainstate::funai::db::FunaiChainState;
use crate::chainstate::funai::{FunaiBlockHeader, MAX_BLOCK_LEN, MAX_TRANSACTION_LEN};
use crate::core::FunaiEpoch;
...
use crate::net::httpcore::FunaiHttpRequest;
...
use crate::net::funaidb::{FunaiDBConfig, FunaiDBSync, FunaiDBTx, FunaiDBs};
...
pub enum NetworkRequest {
    Ban(Vec<NeighborKey>),
    AdvertizeBlocks(BlocksAvailableMap, HashMap<ConsensusHash, FunaiBlock>), // announce to all wanting neighbors that we have these blocks
    ...
    Relay(NeighborKey, FunaiMessage),
    Broadcast(Vec<RelayData>, FunaiMessageType),
}
...
impl NetworkHandle {
    ...
    pub fn advertize_blocks(
        ...
        block_data: HashMap<ConsensusHash, FunaiBlock>,
    ) -> Result<(), net_error> {
    ...
    pub fn advertize_microblocks(
        ...
        microblock_data: HashMap<ConsensusHash, (FunaiBlockId, Vec<FunaiMicroblock>)>,
    ) -> Result<(), net_error> {
...
pub struct PeerNetwork {
    ...
    /// Current Funai tip -- the highest block's consensus hash, block hash, and height
    pub funai_tip: (ConsensusHash, BlockHeaderHash, u64),
    /// Sortition that corresponds to the current Funai tip, if known
    pub funai_tip_sn: Option<BlockSnapshot>,
    /// Parent tenure Funai tip -- the last block in the current tip's parent tenure.
    /// In epoch 2.x, this is the parent block.
    /// In nakamoto, this is the last block in the parent tenure
    pub parent_funai_tip: (ConsensusHash, BlockHeaderHash, u64),
    ...
    pub tenure_start_block_id: FunaiBlockId,
    /// The aggregate public keys of each witnessed reward cycle.
    /// Only active during epoch 3.x and beyond.
    /// Gets refreshed on each new Funai block arrival, which deals with burnchain forks.
    ...
    pub funai_tip_affirmation_map: AffirmationMap,
    ...
    // peer funai DB state machines
    pub funai_db_syncs:
        Option<HashMap<QualifiedContractIdentifier, FunaiDBSync<PeerNetworkComms>>>,
    // configuration state for funai DBs (loaded at runtime from smart contracts)
    pub funai_db_configs: HashMap<QualifiedContractIdentifier, FunaiDBConfig>,
    // handle to all funai DB state
    pub funaidbs: FunaiDBs,
    ...
    // when did we send it?
    antientropy_blocks: HashMap<NeighborKey, HashMap<FunaiBlockId, u64>>,
    antientropy_microblocks: HashMap<NeighborKey, HashMap<FunaiBlockId, u64>>,
    ...
    pub pending_messages: HashMap<usize, Vec<FunaiMessage>>,
...
impl PeerNetwork {
    pub fn new(
        ...
        funai_db_syncs: HashMap<
            QualifiedContractIdentifier,
            (FunaiDBConfig, FunaiDBSync<PeerNetworkComms>),
        >,
        ...
        let mut funai_db_configs = HashMap::new();
        let mut funai_db_sync_map = HashMap::new();
        for (contract_id, (funai_db_config, funai_db_sync)) in funai_db_syncs.into_iter() {
            funai_db_configs.insert(contract_id.clone(), funai_db_config);
            funai_db_sync_map.insert(contract_id.clone(), funai_db_sync);
        }
        ...
        let mut network = PeerNetwork {
            ...
            funai_tip_affirmation_map: AffirmationMap::empty(),
            ...
            funai_tip: (ConsensusHash([0x00; 20]), BlockHeaderHash([0x00; 32]), 0),
            funai_tip_sn: None,
            parent_funai_tip: (ConsensusHash([0x00; 20]), BlockHeaderHash([0x00; 32]), 0),
            tenure_start_block_id: FunaiBlockId([0x00; 32]),
            ...
            funai_db_syncs: Some(funai_db_sync_map),
            funai_db_configs: funai_db_configs,
            funaidbs: funaidbs,
    ...
    pub fn get_current_epoch(&self) -> FunaiEpoch {
        self.get_epoch_at_burn_height(self.chain_view.burn_block_height)
    }

    /// Get an epoch at a burn block height
    pub fn get_epoch_at_burn_height(&self, burn_height: u64) -> FunaiEpoch {
        let epoch_index = FunaiEpoch::find_epoch(&self.epochs, burn_height)
    ...
    pub fn get_epoch_by_epoch_id(&self, epoch_id: FunaiEpochId) -> FunaiEpoch {
        let epoch_index = FunaiEpoch::find_epoch_by_id(&self.epochs, epoch_id)
    ...
    /// Get FunaiDBs link
    pub fn get_funaidbs(&self) -> &FunaiDBs {
        &self.funaidbs
    }

    /// Get FunaiDBs transaction
    pub fn funaidbs_tx_begin<'a>(
        &'a mut self,
        funaidb_contract_id: &QualifiedContractIdentifier,
    ) -> Result<FunaiDBTx<'a>, net_error> {
        if let Some(config) = self.funai_db_configs.get(funaidb_contract_id) {
            return self
                .funaidbs
                .tx_begin(config.clone())
                .map_err(net_error::from);
        }
        Err(net_error::NoSuchFunaiDB(funaidb_contract_id.clone()))
    }
    ...
    /// Count up the number of outbound FunaiDB replicas we talk to,
    /// given the contract ID that controls it.
    pub fn count_outbound_funaidb_replicas(
        &self,
        ...
        for (_, convo) in self.peers.iter() {
            ...
            if !convo.replicates_funaidb(contract_id) {
                continue;
            }
            count += 1;
        }
    ...
    pub fn broadcast_message(
        ...
        relay_hints: Vec<RelayData>,
        message_payload: FunaiMessageType,
    ) -> () {
        debug!(
        ...
        match request {
            ...
            NetworkRequest::Broadcast(relay_hints, msg) => {
                ...
                let neighbor_keys = match msg {
                    FunaiMessageType::Blocks(ref data) => {
                    ...
                    FunaiMessageType::Microblocks(ref data) => {
                    ...
                    FunaiMessageType::Transaction(ref data) => {
    ...
    fn process_p2p_conversation(
        ...
        ibd: bool,
    ) -> Result<(Vec<FunaiMessage>, bool), net_error> {
        self.with_p2p_convo(event_id, |network, convo, client_sock| {
    ...
    fn process_ready_sockets(
        ...
        ibd: bool,
    ) -> (Vec<usize>, HashMap<usize, Vec<FunaiMessage>>) {
        let mut to_remove = vec![];
        let mut unhandled: HashMap<usize, Vec<FunaiMessage>> = HashMap::new();
    ...
    pub fn queue_ping_heartbeats(&mut self) -> () {
        ...
        for (_, convo) in self.peers.iter_mut() {
                    ...
                    < now
            {
                ...
                let payload = FunaiMessageType::Ping(PingData::new());
                let ping_res =
    ...
    fn prune_connections(&mut self) -> () {
        ...
        for (nk, event_id) in self.events.iter() {
            ...
            // if we're running funai DBs, then don't prune any outbound connections it
            // established
            if let Some(funai_db_syncs) = self.funai_db_syncs.as_ref() {
                for (_, funai_db_sync) in funai_db_syncs.iter() {
                    if funai_db_sync.get_pinned_connections().contains(event_id) {
                        safe.insert(*event_id);
                    }
                }
            }
    ...
    fn rekey(&mut self, old_local_peer_opt: Option<&LocalPeer>) {
        ...
        for (event_id, convo) in self.peers.iter_mut() {
            ...
            let handshake = FunaiMessageType::Handshake(handshake_data);
        ...
        if let Some(mut rh) = rh_opt {
            ...
            match rh.try_send_recv() {
                Ok(message) => match message.payload {
                    FunaiMessageType::NatPunchReply(data) => {
    ...
    fn find_next_push_block(
    ...
    ) -> Option<(ConsensusHash, FunaiBlock)> {
        ...
        if block_stats.inv.get_block_height() >= height && !block_stats.inv.has_ith_block(height) {
            ...
            let index_block_hash = FunaiBlockHeader::make_index_block_hash(
                ...
                &ancestor_sn.winning_funai_block_hash,
            );
            let block = match FunaiChainState::load_block(
            ...
            debug!(
                "{:?}: AntiEntropy: Peer {:?} is missing Funai block {} from height {}, which we have",
    ...
    fn find_next_push_microblocks(
        ...
        local_blocks_inv: &BlocksInvData,
        block_stats: &NeighborBlockStats,
    ) -> Option<(ConsensusHash, BlockHeaderHash, Vec<FunaiMicroblock>)> {
        let start_block_height = self.burnchain.reward_cycle_to_block_height(reward_cycle);
            ...
            && !block_stats.inv.has_ith_microblock_stream(height)
        {
            ...
            let block_info = match FunaiChainState::load_staging_block_info(
                &chainstate.db(),
            ...
            let microblocks = match FunaiChainState::load_processed_microblock_stream_fork(
                &chainstate.db(),
            ...
            debug!(
                "{:?}: AntiEntropy: Peer {:?} is missing Funai microblocks {} from height {}, which we have",
                &self.local_peer, nk, &index_block_hash, height
            );
            return Some((
                block_info.parent_consensus_hash,
    ...
    fn try_push_local_data_epoch2x(&mut self, sortdb: &SortitionDB, chainstate: &FunaiChainState) {
        ...
        for reward_cycle in (reward_cycle_finish..reward_cycle_start + 1).rev() {
            ...
            for (nk, blocks) in blocks_to_broadcast.into_iter() {
                ...
                for block in blocks.iter() {
                    let ibh =
                        FunaiBlockHeader::make_index_block_hash(&block.0, &block.1.block_hash());
                    debug!(
                ...
                self.broadcast_message(
                    vec![nk.clone()],
                    vec![],
                    FunaiMessageType::Blocks(blocks_data),
                );
            }
            ...
            for (nk, microblock_datas) in microblocks_to_broadcast.into_iter() {
                for (anchor_block_id, microblocks) in microblock_datas.into_iter() {
                    ...
                    self.broadcast_message(
                        vec![nk.clone()],
                        vec![],
                        FunaiMessageType::Microblocks(microblocks_data),
                    );
                }
            }
        }
    ...
    fn mempool_sync_recv_response(
        &mut self,
        event_id: usize,
    ) -> Result<(bool, Option<Txid>, Option<Vec<FunaiTransaction>>), net_error> {
    ...
    fn do_mempool_sync(
        ...
        dns_client_opt: &mut Option<&mut DNSClient>,
        mempool: &MemPoolDB,
    ) -> (bool, Option<Vec<FunaiTransaction>>) {
    ...
    fn do_network_work(
        ...
        let prune = if cur_epoch.epoch_id >= FunaiEpochId::Epoch30 {
    ...
    fn handle_unsolicited_inv_update_epoch2x(
        ...
        if epoch.epoch_id >= FunaiEpochId::Epoch30 {
            info!(
    ...
    fn buffer_data_message(&mut self, event_id: usize, msg: FunaiMessage) -> () {
        if let Some(msgs) = self.pending_messages.get_mut(&event_id) {
            ...
            for msg in msgs.iter() {
                match &msg.payload {
                    FunaiMessageType::BlocksAvailable(_) => {
                        blocks_available += 1;
                    }
                    FunaiMessageType::MicroblocksAvailable(_) => {
            ...
            if let FunaiMessageType::BlocksAvailable(_) = &msg.payload {
            ...
            if let FunaiMessageType::MicroblocksAvailable(_) = &msg.payload {
            ...
            if let FunaiMessageType::Blocks(_) = &msg.payload {
            ...
            if let FunaiMessageType::Microblocks(_) = &msg.payload {
    ...
    fn handle_unsolicited_BlocksData(
        ...
        for BlocksDatum(consensus_hash, block) in new_blocks.blocks.iter() {
            ...
            if sn.winning_funai_block_hash != block.block_hash() {
                info!(
    ...
    fn handle_unsolicited_message(
        ...
        match payload {
            ...
            FunaiMessageType::Blocks(ref new_blocks) => {
            ...
            FunaiMessageType::Microblocks(ref new_mblocks) => {
            ...
            FunaiMessageType::FunaiDBPushChunk(ref data) => {
                match self.handle_unsolicited_FunaiDBPushChunk(event_id, preamble, data) {
                    Ok(x) => {
    ...
    fn handle_unsolicited_messages(
        ...
        sortdb: &SortitionDB,
        chainstate: &FunaiChainState,
        unsolicited: HashMap<usize, Vec<FunaiMessage>>,
        ibd: bool,
        buffer: bool,
    ) -> HashMap<NeighborKey, Vec<FunaiMessage>> {
        let mut unhandled: HashMap<NeighborKey, Vec<FunaiMessage>> = HashMap::new();
    ...
    /// Set the funai DB configs
    pub fn set_funai_db_configs(
        &mut self,
        configs: HashMap<QualifiedContractIdentifier, FunaiDBConfig>,
    ) {
        self.funai_db_configs = configs;
    }

    /// Obtain a copy of the funai DB configs
    pub fn get_funai_db_configs_owned(
        &self,
    ) -> HashMap<QualifiedContractIdentifier, FunaiDBConfig> {
        self.funai_db_configs.clone()
    }

    /// Obtain a ref to the funai DB configs
    pub fn get_funai_db_configs(&self) -> &HashMap<QualifiedContractIdentifier, FunaiDBConfig> {
        &self.funai_db_configs
    }

    /// Reload FunaiDB configs from chainstate
    pub fn refresh_funai_db_configs(
        &mut self,
        sortdb: &SortitionDB,
        chainstate: &mut FunaiChainState,
    ) -> Result<(), net_error> {
        let funai_db_configs = mem::replace(&mut self.funai_db_configs, HashMap::new());
        self.funai_db_configs = self.funaidbs.create_or_reconfigure_funaidbs(
            chainstate,
            sortdb,
            funai_db_configs,
        )?;
    ...
    /// Load up the parent funai tip.
    /// For epoch 2.x, this is the pointer to the parent block of the current funai tip
    ...
    /// that it builds atop.
    pub(crate) fn get_parent_funai_tip(
        cur_epoch: FunaiEpochId,
        chainstate: &FunaiChainState,
        funai_tip_block_id: &FunaiBlockId,
    ) -> Result<(ConsensusHash, BlockHeaderHash, u64), net_error> {
        let header = NakamotoChainState::get_block_header(chainstate.db(), funai_tip_block_id)?
            .ok_or(net_error::DBError(db_error::NotFoundError))?;

        let parent_header = if cur_epoch < FunaiEpochId::Epoch30 {
            // prior to epoch 3.0, the self.prev_funai_tip field is just the parent block
            let parent_block_id =
                FunaiChainState::get_parent_block_id(chainstate.db(), &header.index_block_hash())?
                    .ok_or(net_error::DBError(db_error::NotFoundError))?;

            NakamotoChainState::get_block_header(chainstate.db(), &parent_block_id)?
                .ok_or(net_error::DBError(db_error::NotFoundError))?
        } else {
            // in epoch 3.0 and later, self.prev_funai_tip is the first tenure block of the
    ...
    fn find_new_aggregate_public_keys(
        ...
        let mut new_agg_pubkeys: Vec<_> = (next_agg_pubkey_rc..=sort_tip_rc)
            .filter_map(|key_rc| {
                ...
                let agg_pubkey_opt = if self.get_current_epoch().epoch_id < FunaiEpochId::Epoch25 {
                    None
                } else {
                    test_debug!(
    ...
    pub fn refresh_burnchain_view<B: BurnchainHeaderReader>(
        ...
        chainstate: &mut FunaiChainState,
        ibd: bool,
    ) -> Result<HashMap<NeighborKey, Vec<FunaiMessage>>, net_error> {
        ...
        let funai_tip =
            SortitionDB::get_canonical_funai_chain_tip_hash_and_height(sortdb.conn())?;

        let burnchain_tip_changed = canonical_sn.block_height != self.chain_view.burn_block_height
            || self.num_state_machine_passes == 0;
        let funai_tip_changed = self.funai_tip != funai_tip;
        let new_funai_tip_block_id = FunaiBlockId::new(&funai_tip.0, &funai_tip.1);
        let need_funaidb_refresh = canonical_sn.canonical_funai_tip_consensus_hash
            != self.burnchain_tip.canonical_funai_tip_consensus_hash
            || burnchain_tip_changed
            || funai_tip_changed;
        let mut ret: HashMap<NeighborKey, Vec<FunaiMessage>> = HashMap::new();

        let aggregate_public_keys = self.find_new_aggregate_public_keys(
            sortdb,
            &canonical_sn,
            chainstate,
            &new_funai_tip_block_id,
        )?;
        let (parent_funai_tip, tenure_start_block_id, funai_tip_sn) = if funai_tip_changed {
            let funai_tip_sn =
                SortitionDB::get_block_snapshot_consensus(sortdb.conn(), &funai_tip.0)?;
            let tenure_start_block_id = if let Some(header) =
                ...
                new_funai_tip_block_id.clone()
            };
            let parent_tip_id = match Self::get_parent_funai_tip(
            ...
            (parent_tip_id, tenure_start_block_id, funai_tip_sn)
        } else {
            (
                self.parent_funai_tip.clone(),
                self.tenure_start_block_id.clone(),
                self.funai_tip_sn.clone(),
            )
        };

        if burnchain_tip_changed || funai_tip_changed {
        ...
        if burnchain_tip_changed {
            ...
            if self.get_current_epoch().epoch_id < FunaiEpochId::Epoch30 {
        ...
        if need_funaidb_refresh {
            // refresh funaidb configs -- canonical funai tip has changed
            debug!("{:?}: Refresh all funaidbs", &self.get_local_peer());
            self.refresh_funai_db_configs(sortdb, chainstate)?;
        }

        if funai_tip_changed && self.get_current_epoch().epoch_id < FunaiEpochId::Epoch30 {
            // update funai tip affirmation map view
            // (NOTE: this check has to happen _after_ self.chain_view gets updated!)
            let burnchain_db = self.burnchain.open_burnchain_db(false)?;
            self.funai_tip_affirmation_map = static_get_funai_tip_affirmation_map(
                &burnchain_db,
                sortdb,
                &canonical_sn.sortition_id,
                &canonical_sn.canonical_funai_tip_consensus_hash,
                &canonical_sn.canonical_funai_tip_hash,
            )
            .map_err(|_| {
                net_error::Transient("Unable to query funai tip affirmation map".to_string())
            })?;
        }
        ...
        // update cached funai chain view for /v2/info and /v3/tenures/info
        self.burnchain_tip = canonical_sn;
        self.funai_tip = funai_tip;
        self.funai_tip_sn = funai_tip_sn;
        self.parent_funai_tip = parent_funai_tip;
    ...
    fn dispatch_network(
        ...
        // synchronize funai DBs
        if !ibd {
            match self.run_funai_db_sync() {
                Ok(funai_db_sync_results) => {
                    network_result.consume_funai_db_sync_results(funai_db_sync_results);
                }
                Err(e) => {
                    warn!("Failed to run Funai DB sync: {:?}", &e);
                }
            }
        } else {
            debug!("{}: skip FunaiDB sync in IBD", self.get_local_peer());
        }
    ...
    fn store_transaction(
        ...
        let funai_epoch = match sortdb
            .index_conn()
            .get_funai_epoch(burnchain_tip.block_height as u32)
        {
            Some(epoch) => epoch,
            None => {
                warn!(
                        "Failed to store transaction because could not load Funai epoch for canonical burn height = {}",
    ...
    pub fn store_transactions(
        ...
        let mut ret: HashMap<NeighborKey, Vec<(Vec<RelayData>, FunaiTransaction)>> =
            HashMap::new();
    ...
    pub fn run<B: BurnchainHeaderReader>(
        ...
        let mut network_result = NetworkResult::new(
            ...
            self.get_funai_db_configs_owned(),
        );
        ...
        PeerNetwork::with_network_state(self, |ref mut network, ref mut network_state| {
            let http_funai_msgs = PeerNetwork::with_http(network, |ref mut net, ref mut http| {
                let mut node_state =
                    FunaiNodeState::new(net, sortdb, chainstate, mempool, handler_args);
                http.run(network_state, &mut node_state, http_poll_state)
            });
            network_result.consume_http_uploads(http_funai_msgs);
            Ok(())
        })
...
mod test {
    ...
    use rand;
    use rand::RngCore;
    use funai_common::types::chainstate::BurnchainHeaderHash;
    use funai_common::util::secp256k1::Secp256k1PrivateKey;
    use funai_common::util::{log, sleep_ms};
    ...
    fn make_test_p2p_network(initial_neighbors: &Vec<Neighbor>) -> PeerNetwork {
        ...
        let funai_db = FunaiDBs::connect_memory();
    ...
    fn test_dispatch_requests_connect_and_message_relay() {
        with_timeout(100, || {
            ...
            let mut ping = FunaiMessage::new(
                ...
                FunaiMessageType::Ping(PingData::new()),
            );
    ...
    fn test_mempool_sync_2_peers() {
        ...
        let num_txs = 10;
        let pks: Vec<_> = (0..num_txs).map(|_| FunaiPrivateKey::new()).collect();
        ...
        for i in 0..(num_blocks / 2) {
            let (burn_ops, funai_block, microblocks) = peer_2.make_default_tenure();

            peer_1.next_burnchain_block(burn_ops.clone());
            peer_2.next_burnchain_block(burn_ops.clone());

            peer_1.process_funai_epoch_at_tip(&funai_block, &microblocks);
            peer_2.process_funai_epoch_at_tip(&funai_block, &microblocks);
        }

        let addr = FunaiAddress {
