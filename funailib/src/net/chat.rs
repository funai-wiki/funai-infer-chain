...
use funai_common::types::chainstate::PoxId;
...
use funai_common::util::{get_epoch_time_ms, get_epoch_time_secs, log};
...
use crate::core::{FunaiEpoch, PEER_VERSION_EPOCH_2_2, PEER_VERSION_EPOCH_2_3};
...
use crate::net::relay::*;
use crate::net::funaidb::FunaiDBs;
use crate::net::{
    Error as net_error, GetBlocksInv, GetPoxInv, Neighbor, NeighborKey, FunaiMessage, FunaiP2P,
...
/// The max number of data points to gather for block/microblock/transaction/funaidb push messages from a neighbor
...
/// Statistics on relayer hints in Funai messages.  Used to deduce network choke points.
#[derive(Debug, Clone)]
...
pub struct NeighborStats {
    ...
    pub msg_rx_counts: HashMap<FunaiMessageID, u64>,
    /// (timestamp, num bytes)
    ...
    /// (timestamp, num bytes)
    pub funaidb_push_rx_counts: VecDeque<(u64, u64)>,
...
impl NeighborStats {
    pub fn new(outbound: bool) -> NeighborStats {
        NeighborStats {
            ...
            funaidb_push_rx_counts: VecDeque::new(),
    ...
    /// Record that we recently received a funaidb chunk push of the given size.
    /// Keeps track of the last `NUM_BANDWIDTH_POINTS` such events, so we can estimate the current
    /// bandwidth consumed by funaidb chunk pushes.
    pub fn add_funaidb_push(&mut self, message_size: u64) -> () {
        self.funaidb_push_rx_counts
            .push_back((get_epoch_time_secs(), message_size));
        while self.funaidb_push_rx_counts.len() > NUM_BANDWIDTH_POINTS {
            self.funaidb_push_rx_counts.pop_front();
        }
    }
    ...
    /// Get a peer's total funaidb-push bandwidth usage
    pub fn get_funaidb_push_bandwidth(&self) -> f64 {
        NeighborStats::get_bandwidth(&self.funaidb_push_rx_counts, BANDWIDTH_POINT_LIFETIME)
    }

    /// Determine how many of a particular message this peer has received
    pub fn get_message_recv_count(&self, msg_id: FunaiMessageID) -> u64 {
        *(self.msg_rx_counts.get(&msg_id).unwrap_or(&0))
    }
}

/// P2P ongoing conversation with another Funai peer
pub struct ConversationP2P {
    ...
    /// which funai DBs this peer replicates
...
impl ConversationP2P {
    ...
    pub fn new(
        ...
        ConversationP2P {
            ...
            network_id: network_id,
            version: version,
            connection: ConnectionP2P::new(FunaiP2P::new(), conn_opts, None),
            conn_id: conn_id,
    ...
    pub fn get_public_key(&self) -> Option<FunaiPublicKey> {
    ...
    pub fn ref_public_key(&self) -> Option<&FunaiPublicKey> {
    ...
    /// Does the given services bitfield support funai DBs?  It will if it has the STACKERDB bit set
    pub fn supports_funaidb(peer_services: u16) -> bool {
        (peer_services & (ServiceFlags::STACKERDB as u16)) != 0
    }

    /// Does this remote neighbor support a particular FunaiDB?
    pub fn replicates_funaidb(&self, db: &QualifiedContractIdentifier) -> bool {
    ...
    fn get_current_epoch(&self, cur_burn_height: u64) -> FunaiEpoch {
        let epoch_index = FunaiEpoch::find_epoch(&self.epochs, cur_burn_height)
    ...
    pub fn sign_message_seq(
        ...
        seq: u32,
        payload: FunaiMessageType,
    ) -> Result<FunaiMessage, net_error> {
        let mut msg =
            FunaiMessage::from_chain_view(self.version, self.network_id, chain_view, payload);
    ...
    fn sign_and_reply(
        ...
        reply_message: FunaiMessageType,
    ...
    pub fn sign_and_forward(
        ...
        forward_message: FunaiMessageType,
    ) -> Result<ReplyHandleP2P, net_error> {
    ...
    fn reply_nack(
        ...
        let nack_payload = FunaiMessageType::Nack(NackData::new(nack_code));
    ...
    fn validate_handshake(
        ...
        local_peer: &LocalPeer,
        chain_view: &BurnchainView,
        message: &mut FunaiMessage,
    ) -> Result<(), net_error> {
        let handshake_data = match message.payload {
            FunaiMessageType::Handshake(ref mut data) => data.clone(),
        ...
        if handshake_data.node_public_key
            == FunaiPublicKeyBuffer::from_public_key(&Secp256k1PublicKey::from_private(
                &local_peer.private_key,
            ))
        {
            debug!(
    ...
    /// Update connection state from funai DB handshake data.
    /// Just synchronizes the announced smart contracts for which this node replicates data.
    pub fn update_from_funai_db_handshake_data(
        &mut self,
        funai_db_data: &FunaiDBHandshakeData,
    ) {
        self.db_smart_contracts = funai_db_data.smart_contracts.clone();
    }

    /// Forget about this peer's funai DB replication state
    pub fn clear_funai_db_handshake_data(&mut self) {
        self.db_smart_contracts.clear();
    }

    /// Getter for funai DB contracts
    pub fn get_funaidb_contract_ids(&self) -> &[QualifiedContractIdentifier] {
    ...
    fn handle_natpunch_request(&self, chain_view: &BurnchainView, nonce: u32) -> FunaiMessage {
        ...
        let msg = FunaiMessage::from_chain_view(
            self.version,
            self.network_id,
            chain_view,
            FunaiMessageType::NatPunchReply(natpunch_data),
        );
        msg
    }
    ...
    fn handle_handshake(
        ...
        authenticated: bool,
        ibd: bool,
    ) -> Result<(Option<FunaiMessage>, bool), net_error> {
        ...
        match res {
            ...
            Err(net_error::InvalidHandshake) => {
                let reject = FunaiMessage::from_chain_view(
                    self.version,
                    self.network_id,
        ...
        let funai_message =
            if ConversationP2P::supports_funaidb(network.get_local_peer().services)
                && ConversationP2P::supports_funaidb(self.peer_services)
            {
                // participate in funaidb protocol, but only announce funaidbs if we're no
                ...
                FunaiMessageType::FunaiDBHandshakeAccept(
                    ...
                    FunaiDBHandshakeData {
                        ...
                        smart_contracts: if ibd {
                            ...
                            network.get_local_peer().funai_dbs.clone()
                        },
                    },
                )
            } else {
                FunaiMessageType::HandshakeAccept(accept_data)
            };

        let accept = FunaiMessage::from_chain_view(
            self.version,
            self.network_id,
            network.get_chain_view(),
            funai_message,
    ...
    fn handle_handshake_accept(
        ...
        funaidb_accept: Option<&FunaiDBHandshakeData>,
    ) -> Result<(), net_error> {
        ...
        if let Some(funaidb_accept) = funaidb_accept {
            test_debug!(
                "{} =?= {}",
                &funaidb_accept.rc_consensus_hash,
                &burnchain_view.rc_consensus_hash
            );
            if funaidb_accept.rc_consensus_hash == burnchain_view.rc_consensus_hash {
                // remote peer is in the same reward cycle as us.
                self.update_from_funai_db_handshake_data(funaidb_accept);
            } else {
                ...
                // seconds as nodes begin processing the next Funai blocks, but it's harmless -- at worst, it
                // just means that no funai DB replication happens between this peer and
                // localhost during this time.
                self.clear_funai_db_handshake_data();
            }
        } else {
    ...
    fn handle_getneighbors(
        ...
        let payload = FunaiMessageType::Neighbors(NeighborsData {
    ...
    fn validate_consensus_hash_reward_cycle_start(
    ...
    ) -> Result<Result<BlockSnapshot, FunaiMessageType>, net_error> {
        ...
        let Some(base_snapshot) =
            ...
            return Ok(Err(FunaiMessageType::Nack(NackData::new(
    ...
    pub fn make_getblocksinv_response(
    ...
    ) -> Result<FunaiMessageType, net_error> {
        ...
        {
            return Ok(FunaiMessageType::Nack(NackData::new(
        ...
        let tip_snapshot = {
            ...
            // NOTE: need the '- 1' here because get_funai_header_hashes includes
        ...
        let block_hashes = {
            ...
            let ic = sortdb.index_conn();
            let res = ic.get_funai_header_hashes(
                num_headers,
        ...
        Ok(FunaiMessageType::BlocksInv(blocks_inv_data))
    }
    ...
    fn handle_getblocksinv(
        ...
        if let FunaiMessageType::BlocksInv(ref mut blocks_inv_data) = &mut response {
            debug!(
    ...
    pub fn make_getnakamotoinv_response(
        ...
        Ok(FunaiMessageType::NakamotoInv(nakamoto_inv))
    }
    ...
    fn handle_getnakamotoinv(
        ...
        if let FunaiMessageType::NakamotoInv(ref mut tenure_inv_data) = &mut response {
            debug!(
            ...
            Ok(Some(sn)) => {
                ...
                match burnchain.block_height_to_reward_cycle(sn.block_height) {
                    Some(reward_cycle) => {
                        ...
                        Ok(FunaiMessageType::PoxInv(poxinvdata))
                    }
                    None => {
                        ...
                        Ok(FunaiMessageType::Nack(NackData::new(
                            NackErrorCodes::InvalidPoxFork,
                        )))
                    }
                }
    ...
    /// Handle an inbound FunaiDBGetChunkInv request.
    /// Generates a FunaiDBChunkInv response from the target database table, if we have it.
    /// Generates a Nack if we don't have this DB, or if the request's consensus hash is invalid.
    fn make_funai_db_getchunkinv_response(
        network: &PeerNetwork,
        getchunkinv: &FunaiDBGetChunkInvData,
    ) -> Result<FunaiMessageType, net_error> {
        let local_peer = network.get_local_peer();
        let burnchain_view = network.get_chain_view();

        // remote peer's Funai chain tip is different from ours, meaning it might have a different
        // funaidb configuration view (and we won't be able to authenticate their chunks, and
        // vice versa)
        if burnchain_view.rc_consensus_hash != getchunkinv.rc_consensus_hash {
            debug!(
                "{:?}: NACK FunaiDBGetChunkInv; {} != {}",
        ...
        Ok(network.make_FunaiDBChunksInv_or_Nack(&getchunkinv.contract_id))
    }

    /// Handle an inbound FunaiDBGetChunkInv request.
    /// Retrns a reply handle to the generated message (possibly a nack)
    fn handle_funai_db_getchunkinv(
        &mut self,
        network: &PeerNetwork,
        preamble: &Preamble,
        getchunkinv: &FunaiDBGetChunkInvData,
    ) -> Result<ReplyHandleP2P, net_error> {
        let response = ConversationP2P::make_funai_db_getchunkinv_response(network, getchunkinv)?;
        self.sign_and_reply(
            network.get_local_peer(),
            network.get_chain_view(),
            preamble,
    ...
    /// Handle an inbound FunaiDBGetChunk request.
    /// Generates a FunaiDBChunk response from the target database table, if we have it.
    /// Generates a NACK if we don't have this DB, or if the request's consensus hash or version
    /// are stale or invalid.
    fn make_funai_db_getchunk_response(
        network: &PeerNetwork,
        getchunk: &FunaiDBGetChunkData,
    ) -> Result<FunaiMessageType, net_error> {
        let local_peer = network.get_local_peer();
        let burnchain_view = network.get_chain_view();
        let funai_dbs = network.get_funaidbs();

        if burnchain_view.rc_consensus_hash != getchunk.rc_consensus_hash {
            debug!(
                "{:?}: NACK FunaiDBGetChunk; {} != {}",
        ...
        let chunk = match funai_dbs.get_chunk(
        ...
        Ok(FunaiMessageType::FunaiDBChunk(chunk))
    }

    /// Handle an inbound FunaiDBGetChunk request
    /// Returns a reply handle to the generated message (possibly a nack)
    fn handle_funai_db_getchunk(
        &mut self,
        network: &PeerNetwork,
        preamble: &Preamble,
        getchunk: &FunaiDBGetChunkData,
    ) -> Result<ReplyHandleP2P, net_error> {
        let response = ConversationP2P::make_funai_db_getchunk_response(network, getchunk)?;
        self.sign_and_reply(
            network.get_local_peer(),
            network.get_chain_view(),
            preamble,
    ...
    /// Validate a pushed funaidb chunk.
    /// Update bandwidth accounting, but forward the funaidb chunk along if we can accept it.
    /// Possibly return a reply handle for a NACK if we throttle the remote sender
    fn validate_funaidb_push(
        &mut self,
        network: &PeerNetwork,
        preamble: &Preamble,
        relayers: Vec<RelayData>,
        ...
        if !self.process_relayers(local_peer, preamble, &relayers) {
            warn!(
                "Drop pushed funaidb chunk -- invalid relayers {:?}",
                &relayers
            );
            self.stats.msgs_err += 1;
            return Err(net_error::InvalidMessage);
        ...
        self.stats
            .add_funaidb_push((preamble.payload_len as u64) - 1);

        if self.connection.options.max_funaidb_push_bandwidth > 0
            && self.stats.get_funaidb_push_bandwidth()
                > (self.connection.options.max_funaidb_push_bandwidth as f64)
        {
            debug!("Neighbor {:?} exceeded max funaidb-push bandwidth of {} bytes/sec (currently at {})", &self.to_neighbor_key(), self.connection.options.max_funaidb_push_bandwidth, self.stats.get_funaidb_push_bandwidth());
            return self
                .reply_nack(local_peer, chain_view, preamble, NackErrorCodes::Throttled)
                .and_then(|handle| Ok(Some(handle)));
        }
    ...
    fn handle_data_message(
        ...
        let res = match msg.payload {
            FunaiMessageType::GetNeighbors => self.handle_getneighbors(network, &msg.preamble),
            FunaiMessageType::GetPoxInv(ref getpoxinv) => {
                self.handle_getpoxinv(network, sortdb, &msg.preamble, getpoxinv)
            }
            FunaiMessageType::GetBlocksInv(ref get_blocks_inv) => {
                self.handle_getblocksinv(network, sortdb, chainstate, &msg.preamble, get_blocks_inv)
            }
            FunaiMessageType::GetNakamotoInv(ref get_nakamoto_inv) => self.handle_getnakamotoinv(
                network,
                sortdb,
                chainstate,
                &msg.preamble,
            ...
            FunaiMessageType::Microblocks(_) => {
            ...
            FunaiMessageType::Transaction(_) => {
            ...
            FunaiMessageType::FunaiDBGetChunkInv(ref getchunkinv) => {
                self.handle_funai_db_getchunkinv(network, &msg.preamble, getchunkinv)
            }
            FunaiMessageType::FunaiDBGetChunk(ref getchunk) => {
                self.handle_funai_db_getchunk(network, &msg.preamble, getchunk)
            }
            FunaiMessageType::FunaiDBChunk(_) | FunaiMessageType::FunaiDBPushChunk(_) => {
                // not handled here, but do some accounting -- we can't receive too many
                // funaidb chunks per second
                match self.validate_funaidb_push(network, &msg.preamble, msg.relayers.clone())? {
                    Some(handle) => Ok(handle),
                    None => {
                        // will forward upstream
                        return Ok(Some(msg));
    ...
    fn handle_authenticated_control_message(
        ...
        let reply_opt = match msg.payload {
            ...
            FunaiMessageType::HandshakeAccept(ref data) => {
                test_debug!("{:?}: Got HandshakeAccept", &self);
                self.handle_handshake_accept(network.get_chain_view(), &msg.preamble, data, None)
                    .and_then(|_| Ok(None))
            }
            FunaiMessageType::FunaiDBHandshakeAccept(ref data, ref db_data) => {
                test_debug!("{:?}: Got FunaiDBHandshakeAccept", &self);
            ...
            FunaiMessageType::Pong(_) => {
                test_debug!("{:?}: Got Pong", &self);
                Ok(None)
            }
            FunaiMessageType::NatPunchRequest(ref nonce) => {
            ...
            FunaiMessageType::NatPunchReply(ref _m) => {
                if cfg!(test) && self.connection.options.disable_natpunch {
                    return Err(net_error::InvalidMessage);
                }
    ...
    fn handle_unauthenticated_control_message(
        ...
        let reply_opt = match msg.payload {
            ...
            FunaiMessageType::FunaiDBHandshakeAccept(ref data, ref db_data) => {
                if solicited {
                    test_debug!("{:?}: Got unauthenticated FunaiDBHandshakeAccept", &self);
                    self.handle_handshake_accept(
                        network.get_chain_view(),
                        &msg.preamble,
                        data,
                    ...
                    test_debug!(
                        "{:?}: Unsolicited unauthenticated FunaiDBHandshakeAccept",
                        &self
                    );
            ...
            _ => {
                ...
                let nack_payload =
                    FunaiMessageType::Nack(NackData::new(NackErrorCodes::HandshakeRequired));
                let nack = FunaiMessage::from_chain_view(
                    self.version,
                    self.network_id,
                    network.get_chain_view(),
                    nack_payload,
    ...
    fn update_stats(&mut self, msg: &FunaiMessage, solicited: bool) {
        if solicited {
            let now = get_epoch_time_secs();
    ...
    /// Returns the list of unfulfilled Funai messages we received -- messages not destined for
    ...
    pub fn chat(
        ...
        dns_client_opt: &mut Option<&mut DNSClient>,
        ibd: bool,
    ) -> Result<Vec<FunaiMessage>, net_error> {
...
mod test {
    ...
    use funai_common::types::chainstate::{BlockHeaderHash, BurnchainHeaderHash, SortitionId};
    use funai_common::util::pipe::*;
    ...
    use crate::chainstate::funai::db::ChainStateBootData;
    ...
    fn make_test_chain_dbs(
        ...
        services: u16,
    ) -> (PeerDB, SortitionDB, FunaiDBs, PoxId, FunaiChainState) {
        let test_path = format!("/tmp/funai-test-databases-{}", testname);
        ...
        let funaidb_path = format!("{}/funaidb.sqlite", &test_path);
        ...
        let sortdb = SortitionDB::connect(
            ...
            get_epoch_time_secs(),
            &FunaiEpoch::unit_test_pre_2_05(burnchain.first_block_height),
        ...
        let funaidb = FunaiDBs::connect(&funaidb_path, true).unwrap();
        ...
        (peerdb, sortdb, funaidb, pox_id, chainstate)
    }
    ...
    fn db_setup(
        ...
        let test_path = format!("/tmp/funai-test-databases-{}", test_name);
        {
            let mut tx = peerdb.tx_begin().unwrap();
        ...
        for i in prev_snapshot.block_height..chain_view.burn_block_height + 1 {
            ...
            next_snapshot.winning_funai_block_hash = BlockHeaderHash(big_i_bytes_32.clone());
        ...
        let funaidbs = FunaiDBs::connect(&funaidb_path, true).unwrap();
        ...
        let network = PeerNetwork::new(
            ...
            HashMap::new(),
            FunaiEpoch::unit_test_pre_2_05(0),
        );
        network
    }
    ...
    /// Inner function for testing various kinds of funaidb/non-funaidb peer interactions
    fn inner_convo_handshake_accept_funaidb(
        peer_1_services: u16,
        ...
        with_timeout(100, move || {
            ...
            let (mut peerdb_1, mut sortdb_1, funaidbs_1, pox_id_1, mut chainstate_1) =
                make_test_chain_dbs(
                    &test_name_1,
                    &burnchain,
                    0x9abcdef0,
            ...
            let (mut peerdb_2, mut sortdb_2, funaidbs_2, pox_id_2, mut chainstate_2) =
                make_test_chain_dbs(
                    &test_name_2,
                    &burnchain,
                    0x9abcdef0,
            ...
            let handshake_1 = convo_1
                .sign_message(
                    ...
                    FunaiMessageType::Handshake(handshake_data_1.clone()),
                )
                .unwrap();
            ...
            match unhandled_2[0].payload {
                FunaiMessageType::Handshake(ref data) => {
            ...
            {
                // received a valid FunaiDBHandshakeAccept from peer 2?
                match reply_1.payload {
                    FunaiMessageType::FunaiDBHandshakeAccept(ref data, ref db_data) => {
                        ...
                        assert_eq!(
                            ...
                            FunaiPublicKeyBuffer::from_public_key(
                        ...
                        if peer_1_rc_consensus_hash == peer_2_rc_consensus_hash {
                            ...
                            assert!(convo_1.replicates_funaidb(
    ...
    #[test]
    /// Two funaidb peers handshake
    fn convo_handshake_accept_funaidb() {
        inner_convo_handshake_accept_funaidb(
            STACKERDB_SERVICES,
    ...
    #[test]
    /// A funaidb peer handshakes with a legacy peer
    fn convo_handshake_accept_funaidb_legacy() {
        inner_convo_handshake_accept_funaidb(
            STACKERDB_SERVICES,
    ...
    #[test]
    /// Two funaidb peers handshake, but with different reward cycle consensus hashes
    fn convo_handshake_accept_funaidb_bad_consensus_hash() {
        inner_convo_handshake_accept_funaidb(
            STACKERDB_SERVICES,
            ConsensusHash([0x33; 20]),
            STACKERDB_SERVICES,
    ...
    fn convo_handshake_badsignature() {
        ...
        match handshake_1.payload {
            FunaiMessageType::Handshake(ref mut data) => {
    ...
    fn convo_handshake_update_key() {
        ...
        // received a valid HandshakeAccept from peer 2
        match reply_1.payload {
            FunaiMessageType::HandshakeAccept(..) => {}
            _ => {
                assert!(false);
            }
        };
    ...
    fn convo_ping() {
        ...
        let ping_1 = convo_1
            .sign_message(
                &chain_view,
                &local_peer_1.private_key,
                FunaiMessageType::Ping(ping_data_1.clone()),
            )
            .unwrap();
        ...
        match reply_ping_1.payload {
            FunaiMessageType::Pong(ref data) => {
    ...
    fn convo_handshake_ping_loop() {
        ...
        for i in 0..5 {
            ...
            match reply_handshake_1.payload {
                FunaiMessageType::HandshakeAccept(ref data)
                | FunaiMessageType::FunaiDBHandshakeAccept(ref data, ..) => {
                    assert_eq!(data.handshake.addrbytes, local_peer_2.addrbytes);
                    assert_eq!(data.handshake.port, local_peer_2.port);
                    assert_eq!(data.handshake.services, local_peer_2.services);
                    assert_eq!(
    ...
    fn convo_nack_unsolicited() {
        ...
        // convo_1 got a NACK
        match reply_1.payload {
            FunaiMessageType::Nack(ref data) => {
        ...
        let unauthed_messages = {
            ...
            let accept_1 = convo_1
                .sign_message(
                    &chain_view,
                    &local_peer_1.private_key,
                    FunaiMessageType::HandshakeAccept(accept_data_1.clone()),
                )
                .unwrap();

            let funaidb_accept_data_1 = FunaiMessageType::FunaiDBHandshakeAccept(
                accept_data_1.clone(),
                FunaiDBHandshakeData {
            ...
            let funaidb_accept_1 = convo_1
                .sign_message(
                    &chain_view,
                    &local_peer_1.private_key,
                    funaidb_accept_data_1,
                )
                .unwrap();

            vec![accept_1, funaidb_accept_1]
        };
        ...
        with_timeout(100, || {
            ...
            let getblocksdata_1_msg = convo_1
                .sign_message(
                    &chain_view,
                    &local_peer_1.private_key,
                    FunaiMessageType::GetBlocksInv(getblocksdata_1.clone()),
                )
                .unwrap();
            ...
            match reply_1.payload {
                FunaiMessageType::BlocksInv(ref data) => {
                    assert_eq!(data.bitlen, 10);
                    test_debug!("data: {:?}", data);

                    // all burn blocks had sortitions, but we have no Funai blocks :(
            ...
            let getblocksdata_diverged_1_msg = convo_1
                .sign_message(
                    &chain_view,
                    &local_peer_1.private_key,
                    FunaiMessageType::GetBlocksInv(getblocksdata_diverged_1.clone()),
                )
                .unwrap();
            let mut rh_1 = convo_1
        ...
        with_timeout(100, || {
            ...
            let getnakamotodata_1_msg = convo_1
                .sign_message(
                    &chain_view,
                    &local_peer_1.private_key,
                    FunaiMessageType::GetNakamotoInv(getnakamotodata_1.clone()),
                )
                .unwrap();
            let mut rh_1 = convo_1
            ...
            match reply_1.payload {
                FunaiMessageType::NakamotoInv(ref data) => {
            ...
            let getnakamotodata_diverged_1_msg = convo_1
                .sign_message(
                    &chain_view,
                    &local_peer_1.private_key,
                    FunaiMessageType::GetNakamotoInv(getnakamotodata_diverged_1.clone()),
                )
                .unwrap();
            let mut rh_1 = convo_1
    ...
    fn convo_natpunch() {
        ...
        match natpunch_reply_1.payload {
            FunaiMessageType::NatPunchReply(ref data) => {
    ...
    fn convo_is_preamble_valid() {
        ...
        let (mut peerdb_1, mut sortdb_1, funaidbs_1, pox_id_1, chainstate_1) =
            make_test_chain_dbs(
                test_name_1,
                &burnchain,
                0x9abcdef0,
        ...
        // network ID check
        {
            ...
            let ping_bad = convo_bad
                .sign_message(
                    &chain_view,
                    &local_peer_1.private_key,
                    FunaiMessageType::Ping(ping_data.clone()),
                )
                .unwrap();
            convo_bad.network_id -= 1;
        ...
        // stale peer version max-epoch
        {
            ...
            let epochs = FunaiEpoch::unit_test_2_05(chain_view.burn_block_height - 4);
            let cur_epoch_idx =
                FunaiEpoch::find_epoch(&epochs, chain_view.burn_block_height).unwrap();
    ...
    fn convo_process_relayers() {
        ...
        let payload = FunaiMessageType::Nack(NackData { error_code: 123 });
        let msg = convo
        ...
        let self_sent = vec![RelayData {
            peer: NeighborAddress {
                ...
                public_key_hash: Hash160::from_node_public_key(&FunaiPublicKey::from_private(
                    &local_peer.private_key,
                )),
            },
            seq: 789,
    ...
    #[test]
    fn test_neighbor_stats_funaidb_push_bandwidth() {
        let mut stats = NeighborStats::new(false);

        assert_eq!(stats.get_funaidb_push_bandwidth(), 0.0);

        stats.add_funaidb_push(100);
        assert_eq!(stats.get_funaidb_push_bandwidth(), 0.0);
        ...
        assert_eq!(
            bw_stats.get_funaidb_push_bandwidth(),
            (NUM_BANDWIDTH_POINTS as f64) * 100.0
        );
        ...
        // 100 bytes/sec
        assert_eq!(bw_stats.get_funaidb_push_bandwidth(), 110.0);
    }
    ...
    fn test_sign_relay_forward_message() {
        ...
        let (mut peerdb_1, mut sortdb_1, funaidbs_1, pox_id_1, _) = make_test_chain_dbs(
            test_name_1,
            &burnchain,
            0x9abcdef0,
            12352,
    ...
    #[test]
    fn test_validate_funaidb_push() {
        let mut conn_opts = ConnectionOptions::default();
        conn_opts.max_funaidb_push_bandwidth = 100;
        ...
        let test_name_1 = "validate_funaidb_push_1";
        let (mut peerdb_1, mut sortdb_1, funaidbs_1, pox_id_1, _) = make_test_chain_dbs(
            test_name_1,
            &burnchain,
            0x9abcdef0,
        ...
        let err_before = convo_1.stats.msgs_err;
        match convo_1
            .validate_funaidb_push(&net_1, &bad_msg.preamble, bad_msg.relayers.clone())
            .unwrap_err()
        {
            net_error::InvalidMessage => {}
            e => {
        ...
        msg.preamble.payload_len = 106;
        assert!(convo_1
            .validate_funaidb_push(&net_1, &msg.preamble, msg.relayers.clone())
            .unwrap()
            .is_none());
        assert_eq!(convo_1.stats.msgs_err, err_before);
