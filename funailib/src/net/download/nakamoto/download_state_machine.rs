...
pub struct NakamotoDownloadStateMachine {
    ...
    tenure_start_blocks: HashMap<FunaiBlockId, NakamotoBlock>,
...
impl NakamotoDownloadStateMachine {
    ...
    pub(crate) fn load_wanted_tenures(
        ...
        while cursor.block_height >= first_block_height {
            test_debug!(
                ...
                &cursor.winning_funai_block_hash,
                cursor.block_height
            );
            wanted_tenures.push(WantedTenure::new(
                cursor.consensus_hash,
                FunaiBlockId(cursor.winning_funai_block_hash.0),
                cursor.block_height,
            ));
    ...
    pub(crate) fn load_tenure_start_blocks(
        ...
        chainstate: &FunaiChainState,
        tenure_start_blocks: &mut HashMap<FunaiBlockId, NakamotoBlock>,
    ) -> Result<(), NetError> {
    ...
    pub(crate) fn make_unconfirmed_tenure_downloaders(
        ...
        highest_processed_block_id: Option<FunaiBlockId>,
    ) {
    ...
    pub(crate) fn run_unconfirmed_downloaders(
        ...
        let highest_processed_block_id =
            FunaiBlockId::new(&network.funai_tip.0, &network.funai_tip.1);
        let highest_processed_block_height = network.funai_tip.2;
    ...
    fn download_unconfirmed_tenures(
        ...
        let mut coalesced_blocks: HashMap<ConsensusHash, HashMap<FunaiBlockId, NakamotoBlock>> =
            HashMap::new();
