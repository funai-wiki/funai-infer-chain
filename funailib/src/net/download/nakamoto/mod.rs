...
//! loop of the p2p network.  On each pass, the downloader state machine inspects the Funai chain
...
//! Unlike in Funai 2.x, the downloader is consistently running, and can act on newly-discovered
//! tenures once a peer's inventory reports their availability.  This is because Nakamoto is more
//! latency-sensitive than Funai 2.x, and nodes need to obtain blocks as quickly as possible.
//!
...
impl PeerNetwork {
    ...
    pub fn init_nakamoto_block_downloader(&mut self) {
        ...
        let epoch = self.get_epoch_by_epoch_id(FunaiEpochId::Epoch30);
