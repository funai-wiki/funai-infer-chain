...
use funai_common::types::chainstate::{
    BlockHeaderHash, ConsensusHash, PoxId, SortitionId, FunaiBlockId,
};
...
use crate::chainstate::funai::{
    Error as chainstate_error, FunaiBlockHeader, TenureChangePayload,
};
use crate::core::{
...
pub enum NakamotoTenureDownloadState {
    /// Getting the tenure-start block (the given FunaiBlockId is it's block ID).
    GetTenureStartBlock(FunaiBlockId),
    ...
    WaitForTenureEndBlock(FunaiBlockId, Instant),
    ...
    GetTenureEndBlock(FunaiBlockId),
...
pub struct NakamotoTenureDownloader {
    ...
    /// Funai block ID of the tenure-start block.  Learned from the inventory state machine and
    /// sortition DB.
    pub tenure_start_block_id: FunaiBlockId,
    /// Funai block ID of the last block in this tenure (this will be the tenure-start block ID
    /// for some other tenure).  Learned from the inventory state machine and sortition DB.
    pub tenure_end_block_id: FunaiBlockId,
...
impl NakamotoTenureDownloader {
    pub fn new(
        ...
        tenure_start_block_id: FunaiBlockId,
        tenure_end_block_id: FunaiBlockId,
    ...
    pub fn make_next_download_request(
        ...
        let request = match self.state {
            ...
            NakamotoTenureDownloadState::GetTenureEndBlock(end_block_id) => {
                ...
                FunaiHttpRequest::new_get_nakamoto_block(peerhost, end_block_id.clone())
            }
            NakamotoTenureDownloadState::GetTenureBlocks(end_block_id) => {
                test_debug!("Downloading tenure ending at {}", &end_block_id);
                FunaiHttpRequest::new_get_nakamoto_tenure(peerhost, end_block_id.clone(), None)
            }
    ...
    /// Handle a received FunaiHttpResponse and advance the state machine.
