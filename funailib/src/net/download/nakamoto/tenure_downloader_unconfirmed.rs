...
pub enum NakamotoUnconfirmedDownloadState {
    ...
    GetUnconfirmedTenureBlocks(FunaiBlockId),
...
pub struct NakamotoUnconfirmedTenureDownloader {
    ...
    pub highest_processed_block_id: Option<FunaiBlockId>,
...
impl NakamotoUnconfirmedTenureDownloader {
    ...
    pub fn new(naddr: NeighborAddress, highest_processed_block_id: Option<FunaiBlockId>) -> Self {
        Self {
    ...
    pub fn set_highest_processed_block(
        &mut self,
        highest_processed_block_id: FunaiBlockId,
    ...
    ///     the winning Funai block hash of the snapshot for the ongoing tenure.
    ///  
    ...
    pub fn try_accept_tenure_info(
        ...
        // snapshot
        if local_tenure_sn.winning_funai_block_hash.0
            != remote_tenure_tip.parent_tenure_start_block_id.0
        {
            warn!("Ongoing tenure does not commit to highest complete tenure's start block";
                  ...
                  "local_tenure_sn.winning_funai_block_hash" => %local_tenure_sn.winning_funai_block_hash);
            return Err(NetError::InvalidMessage);
        }
    ...
    pub fn try_accept_unconfirmed_tenure_start_block(
        ...
        // funai signature has to match the current aggregate public key
    ...
    pub fn make_next_download_request(&self, peerhost: PeerHost) -> Option<FunaiHttpRequest> {
        match &self.state {
            NakamotoUnconfirmedDownloadState::GetTenureInfo => {
                // need to get the tenure tip
                return Some(FunaiHttpRequest::new_get_nakamoto_tenure_info(peerhost));
            }
            NakamotoUnconfirmedDownloadState::GetTenureStartBlock(block_id) => {
                return Some(FunaiHttpRequest::new_get_nakamoto_block(
                    peerhost,
                    block_id.clone(),
                ));
            }
            NakamotoUnconfirmedDownloadState::GetUnconfirmedTenureBlocks(tip_block_id) => {
                return Some(FunaiHttpRequest::new_get_nakamoto_tenure(
                    peerhost,
                    tip_block_id.clone(),
    ...
    /// Handle a received FunaiHttpResponse and advance this machine's state
