...
use funai_common::types::chainstate::{BlockHeaderHash, PoxId, SortitionId, FunaiBlockId};
...
use crate::chainstate::funai::{Error as chainstate_error, FunaiBlockHeader};
use crate::core::{
...
use crate::net::{
    Error as net_error, GetBlocksInv, Neighbor, NeighborKey, FunaiMessage, FunaiP2P, *,
};
...
pub struct BlockRequestKey {
    ...
    pub parent_block_header: Option<FunaiBlockHeader>, // only used if asking for a microblock; used to confirm the stream's continuity
    ...
    pub canonical_funai_tip_height: u64,
}
...
impl BlockRequestKey {
    pub fn new(
        ...
        parent_block_header: Option<FunaiBlockHeader>,
    ...
    fn make_getblock_request(&self, peer_host: PeerHost) -> FunaiHttpRequest {
    ...
    fn make_confirmed_microblocks_request(&self, peer_host: PeerHost) -> FunaiHttpRequest {
        FunaiHttpRequest::new_for_peer(
            peer_host,
            "GET".into(),
...
pub struct BlockDownloader {
    ...
    blocks: HashMap<BlockRequestKey, FunaiBlock>,
    microblocks: HashMap<BlockRequestKey, Vec<FunaiMicroblock>>,
    ...
    requested_blocks: HashMap<FunaiBlockId, u64>,
    requested_microblocks: HashMap<FunaiBlockId, u64>,
}
...
impl BlockDownloader {
                ...
                match http.get_conversation(event_id) {
                    ...
                    Some(ref mut convo) => {
                        match convo.try_get_response() {
                            ...
                            Some(http_response) => {
                                match FunaiHttpResponse::decode_block(http_response) {
                                    Ok(block) => {
                                        if FunaiBlockHeader::make_index_block_hash(
                ...
                match http.get_conversation(event_id) {
                    ...
                    Some(ref mut convo) => {
                        match convo.try_get_response() {
                            ...
                            Some(http_response) => {
                                match FunaiHttpResponse::decode_microblocks(http_response) {
                                    Ok(microblocks) => {
    ...
    fn get_microblock_stream_availability(
        ...
        if sn.winning_funai_block_hash != *block_hash {
            test_debug!(
    ...
    fn is_inflight(&self, index_hash: &FunaiBlockId, microblocks: bool) -> bool {
        if microblocks {
            // being requested now?
...
impl PeerNetwork {
    ...
    fn need_anchored_block(
        ...
        if FunaiChainState::has_block_indexed(&chainstate.blocks_path, &index_block_hash)? {
            test_debug!(
    ...
    pub fn can_download_microblock_stream(
        ...
        if let Ok(Some(true)) = FunaiChainState::get_staging_block_status(
            &chainstate.db(),
        ...
        let _parent_header = match FunaiChainState::load_block_header(
        ...
        let child_header = match FunaiChainState::load_block_header(
            &chainstate.blocks_path,
            child_consensus_hash,
            child_block_hash,
        ) {
    ...
    fn make_requests(
            ...
            availability.drain(..).enumerate()
        {
            ...
            let (target_consensus_hash, target_block_hash) = if !microblocks {
                ...
                let block_header = match FunaiChainState::load_block_header(
                    &chainstate.blocks_path,
                    &consensus_hash,
                    &block_hash,
                ) {
            ...
            let target_index_block_hash = FunaiBlockHeader::make_index_block_hash(
            ...
            for nk in neighbors.drain(..) {
                ...
                let request = BlockRequestKey::new(
                    ...
                    self.burnchain_tip.canonical_funai_tip_height,
                );
    ...
    fn finish_downloads(
        ...
        (
            ...
            bool,
            Option<PoxId>,
            Vec<(ConsensusHash, FunaiBlock, u64)>,
            Vec<(ConsensusHash, Vec<FunaiMicroblock>, u64)>,
        ),
        net_error,
    > {
        let mut blocks = vec![];
        ...
        PeerNetwork::with_downloader_state(self, |ref mut network, ref mut downloader| {
            ...
            for (request_key, mut microblock_stream) in downloader.microblocks.drain() {
                ...
                let block_header = match FunaiChainState::load_block_header(
                    ...
                    Ok(Some(hdr)) => hdr,
                    Ok(None) => {
                        warn!("Missing Funai blcok header for {}/{}.  Possibly invalidated due to PoX reorg", &request_key.consensus_hash, &request_key.anchor_block_hash);
                ...
                if FunaiChainState::validate_parent_microblock_stream(
                    &parent_block_header,
                    &block_header,
                    &microblock_stream,
                    true,
