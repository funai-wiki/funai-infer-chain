...
impl Neighbor {
    ...
    pub fn save_update<'a>(
        &mut self,
        tx: &DBTx<'a>,
        funai_dbs: Option<&[QualifiedContractIdentifier]>,
    ) -> Result<(), net_error> {
        self.last_contact_time = get_epoch_time_secs();
        PeerDB::update_peer(tx, &self).map_err(net_error::DBError)?;
        if let Some(funai_dbs) = funai_dbs {
            PeerDB::update_peer_funai_dbs(tx, &self, funai_dbs).map_err(net_error::DBError)?;
        }
        Ok(())
    }
    ...
    pub fn save<'a>(
        ...
        self.last_contact_time = get_epoch_time_secs();
        PeerDB::try_insert_peer(tx, &self, funai_dbs.unwrap_or(&[])).map_err(net_error::DBError)
    }
