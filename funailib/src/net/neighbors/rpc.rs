...
use crate::net::{
    ...
    PeerHostExtensions, FunaiHttpRequest, FunaiHttpResponse, FunaiMessage, FunaiMessageType,
    NUM_NEIGHBORS,
};
...
pub struct NeighborRPC {
    state: HashMap<NeighborAddress, (usize, Option<FunaiHttpRequest>)>,
...
impl NeighborRPC {
    ...
    ) -> Vec<(NeighborAddress, FunaiHttpResponse)> {
    ...
    /// Find the PeerHost to use when creating a Funai HTTP request.
    ...
    fn poll_next_reply(
        network: &mut PeerNetwork,
        event_id: usize,
        request_opt: &mut Option<FunaiHttpRequest>,
    ) -> Result<Option<FunaiHttpResponse>, NetError> {
