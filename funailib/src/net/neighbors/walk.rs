...
use crate::net::{
    ...
    NeighborAddress, NeighborKey, PeerAddress, Preamble, FunaiDBHandshakeData, FunaiMessage,
    FunaiMessageType, NUM_NEIGHBORS,
};
...
pub struct NeighborPingback {
    ...
    pub pubkey: FunaiPublicKey, // public key of neighbor to ping back
}
...
impl<DB: NeighborWalkDB, NC: NeighborComms> NeighborWalk<DB, NC> {
    ...
    fn handle_handshake_accept(
        ...
        db_data: Option<&FunaiDBHandshakeData>,
    ...
    pub fn handshake_try_finish(&mut self, network: &mut PeerNetwork) -> Result<bool, net_error> {
        ...
        let (data, db_data) = match message.payload {
            FunaiMessageType::HandshakeAccept(ref data) => (data, None),
    ...
    pub fn getneighbors_try_finish(
        ...
        let mut neighbor_addrs_to_resolve = match message.payload {
            FunaiMessageType::Neighbors(ref data) => {
                debug!(
                    "{:?}: Got Neighbors from {:?}: {:?}",
                    network.get_local_peer(),
                    &self.cur_neighbor.addr,
    ...
    pub fn getneighbors_neighbors_begin(
        ...
        for naddr in handshake_neighbor_addrs.into_iter() {
            ...
            if let Err(e) =
                self.comms
                    .neighbor_send(network, &naddr, FunaiMessageType::GetNeighbors)
            {
                debug!(
                    "{:?}: Could not send to {:?}: {:?}",
                    network.get_local_peer(),
    ...
    /// * The Funai peer network is a _directed_ graph, whereas MHRWDA is desigend to operate
    ...
    pub fn pingback_handshakes_try_finish(
        ...
        for (naddr, message) in self.comms.collect_replies(network).into_iter() {
            ...
            let (data, db_data) = match message.payload {
                ...
                FunaiMessageType::FunaiDBHandshakeAccept(ref data, ref db_data) => {
                    debug!("{:?}: received FunaiDBHandshakeAccept from peer {:?}; now known to be routable from us", network.get_local_peer(), &message.to_neighbor_key(&data.handshake.addrbytes, data.handshake.port));
                    (data, Some(db_data))
                }
                _ => {
                    let nkey = naddr.to_neighbor_key(network);
    ...
    pub fn ping_existing_neighbors_begin(
        ...
        for (naddr, _slot) in replaced_neighbors.iter_slots() {
            ...
            if let Err(e) = self.comms.neighbor_send(
                network,
                &naddr,
                FunaiMessageType::Handshake(HandshakeData::from_local_peer(
                    network.get_local_peer(),
                )),
            ) {
                debug!(
    ...
    /// If it was a FunaiDBHandshakeAccept, then also handle the newly-announced DBs
