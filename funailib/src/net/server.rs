...
impl HttpPeer {
    ...
    pub fn connect_http(
        ...
        data_url: UrlString,
        addr: SocketAddr,
        request: Option<FunaiHttpRequest>,
    ...
    fn register_http(
        &mut self,
        network_state: &mut NetworkState,
        node_state: &mut FunaiNodeState,
        event_id: usize,
        mut socket: mio_net::TcpStream,
        outbound_url: Option<UrlString>,
        initial_request: Option<FunaiHttpRequest>,
    ) -> Result<(), net_error> {
    ...
    fn process_http_conversation(
        ...
        convo: &mut ConversationHttp,
    ) -> Result<(bool, Vec<FunaiMessageType>), net_error> {
            ...
            Err(e) => {
                match e {
                    ...
                    net_error::InvalidMessage => {
                        ...
                        match convo.reply_error(FunaiHttpResponse::new_empty_error(
                            &HttpBadRequest::new(
    ...
    fn process_ready_sockets(
        ...
        poll_state: &mut NetworkPollState,
        node_state: &mut FunaiNodeState,
    ) -> (Vec<FunaiMessageType>, Vec<usize>) {
        let mut to_remove = vec![];
        let mut msgs = vec![];
    ...
    pub fn run(
        ...
        let (funai_msgs, error_events) = self.process_ready_sockets(&mut poll_state, node_state);
...
mod test {
    ...
    use funai_common::codec::MAX_MESSAGE_LEN;
    ...
    where
        F: FnMut(usize, &mut FunaiChainState) -> Vec<u8>,
    ...
    fn test_http_getinfo() {
        test_http_server(
            ...
            0,
            |client_id, _| {
                let mut request = FunaiHttpRequest::new_for_peer(
            ...
            |client_id, http_response_bytes_res| {
                ...
                let response =
                    FunaiHttp::parse_response("GET", "/v2/info", &http_response_bytes).unwrap();
                true
            },
        );
    }
    ...
    fn test_http_getblock() {
        test_http_server(
            ...
            |client_id, http_response_bytes_res| {
                ...
                let response =
                    FunaiHttp::parse_response("GET", &request_path, &http_response_bytes).unwrap();
                match response {
                    FunaiHttpMessage::Response(funai_http_response) => {
                        if let Ok(block) = FunaiHttpResponse::decode_block(funai_http_response) {
            ...
            |client_id, http_response_bytes_res| {
                match http_response_bytes_res {
                    Ok(http_response_bytes) => {
                        // should be a PeerInfo
                        let response = match FunaiHttp::parse_response(
                            "GET",
                            "/v2/info",
                            &http_response_bytes,
                        ) {
        ...
        test_http_server(
            ...
            |client_id, ref mut chainstate| {
                ...
                let mut request = FunaiHttpRequest::new_for_peer(
                    ...
                    HttpRequestContents::new().payload_funai(&signed_contract_tx),
                )
                .unwrap();
                request.preamble_mut().keep_alive = false;
