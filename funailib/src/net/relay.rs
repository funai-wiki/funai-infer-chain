...
use clarity::vm::types::{QualifiedContractIdentifier, FunaiAddressExtensions};
use clarity::vm::ClarityVersion;
use rand::prelude::*;
use rand::{thread_rng, Rng};
use funai_common::codec::MAX_PAYLOAD_LEN;
use funai_common::types::chainstate::{BurnchainHeaderHash, PoxId, SortitionId, FunaiBlockId};
...
use crate::chainstate::funai::db::unconfirmed::ProcessedUnconfirmedState;
use crate::chainstate::funai::db::{FunaiChainState, FunaiEpochReceipt, FunaiHeaderInfo};
use crate::chainstate::funai::events::FunaiTransactionReceipt;
use crate::chainstate::funai::{FunaiBlockHeader, TransactionPayload};
use crate::clarity_vm::clarity::Error as clarity_error;
use crate::core::mempool::{MemPoolDB, *};
use crate::monitoring::update_funai_tip_height;
use crate::net::chat::*;
...
use crate::net::poll::*;
use crate::net::rpc::*;
use crate::net::funaidb::{
    FunaiDBConfig, FunaiDBEventDispatcher, FunaiDBSyncResult, FunaiDBs,
};
...
pub struct ProcessedNetReceipts {
    pub mempool_txs_added: Vec<FunaiTransaction>,
...
/// A trait for implementing both mempool event observer methods and funaidb methods.
...
pub trait RelayEventDispatcher:
    ...
    + FunaiDBEventDispatcher
    + AsMemPoolEventDispatcher
    + AsFunaiDBEventDispatcher
{
}
impl<T: MemPoolEventDispatcher + FunaiDBEventDispatcher> RelayEventDispatcher for T {}
...
/// Trait for upcasting to FunaiDBEventDispatcher
pub trait AsFunaiDBEventDispatcher {
    fn as_funaidb_event_dispatcher(&self) -> &dyn FunaiDBEventDispatcher;
}
...
impl<T: RelayEventDispatcher> AsFunaiDBEventDispatcher for T {
    fn as_funaidb_event_dispatcher(&self) -> &dyn FunaiDBEventDispatcher {
        self
    }
}
...
impl RelayPayload for FunaiBlock {
    fn get_digest(&self) -> Sha512Trunc256Sum {
        let h = self.block_hash();
        Sha512Trunc256Sum(h.0)
    }
    fn get_id(&self) -> String {
        format!("FunaiBlock({})", self.block_hash())
    }
}

impl RelayPayload for FunaiMicroblock {
    fn get_digest(&self) -> Sha512Trunc256Sum {
        let h = self.block_hash();
        Sha512Trunc256Sum(h.0)
    }
    fn get_id(&self) -> String {
        format!("FunaiMicroblock({})", self.block_hash())
    }
}

impl RelayPayload for FunaiTransaction {
    fn get_digest(&self) -> Sha512Trunc256Sum {
        let h = self.txid();
        Sha512Trunc256Sum(h.0)
    }
...
impl Relayer {
    pub fn new(handle: NetworkHandle, funai_dbs: FunaiDBs) -> Relayer {
        Relayer {
            p2p: handle,
            funai_dbs,
        }
    ...
    pub fn from_p2p(network: &mut PeerNetwork, funai_dbs: FunaiDBs) -> Relayer {
        let handle = network.new_handle(1024);
        Relayer::new(handle, funai_dbs)
    }
    ...
    pub fn validate_blocks_push(
        ...
        for BlocksDatum(consensus_hash, block) in blocks_data.blocks.iter() {
            ...
            // is this the right Funai block for this sortition?
            ...
            if !sn.sortition || sn.winning_funai_block_hash != block_hash {
                info!(
    ...
    /// Get the snapshot of the parent of a given Funai block
    pub fn get_parent_funai_block_snapshot(
    ...
    pub fn process_new_anchored_block(
        ...
        let parent_block_snapshot = match sort_ic
            .find_parent_snapshot_for_funai_block(consensus_hash, &block.block_hash())?
        {
            Some(sn) => sn,
            None => {
        ...
        let epoch_id = SortitionDB::get_funai_epoch(sort_ic, block_sn.block_height)?
        ...
        if !Relayer::static_check_problematic_relayed_block(
            ...
            warn!(
                ...
                "funai_block_hash" => %block.block_hash(),
    ...
    pub fn process_new_nakamoto_block(
        ...
        let epoch_id = SortitionDB::get_funai_epoch(sort_handle, block_sn.block_height + 1)?
            .expect("FATAL: no epoch defined")
            .epoch_id;

        if epoch_id < FunaiEpochId::Epoch30 {
            error!("Nakamoto blocks are not supported in this epoch");
            return Err(chainstate_error::InvalidFunaiBlock(
        ...
        if !Relayer::static_check_problematic_relayed_nakamoto_block(
            ...
            warn!(
                ...
                "funai_block_hash" => %block.header.block_hash(),
        ...
        if accepted {
            ...
            if let Some(coord_comms) = coord_comms {
                if !coord_comms.announce_new_funai_block() {
    ...
    fn make_microblocksdata_messages(
        new_microblocks: HashMap<
            FunaiBlockId,
            (Vec<RelayData>, HashMap<BlockHeaderHash, FunaiMicroblock>),
        >,
    ) -> Vec<(Vec<RelayData>, MicroblocksData)> {
        let mut mblocks_data: HashMap<FunaiBlockId, Vec<(Vec<RelayData>, MicroblocksData)>> =
            HashMap::new();
        let mut mblocks_sizes: HashMap<FunaiBlockId, usize> = HashMap::new();
    ...
    fn preprocess_downloaded_blocks(
        ...
        chainstate: &mut FunaiChainState,
    ) -> HashMap<ConsensusHash, FunaiBlock> {
        ...
        for (consensus_hash, block, download_time) in network_result.blocks.iter() {
            ...
            match Relayer::process_new_anchored_block(
                ...
                *download_time,
                ...
                Err(chainstate_error::InvalidFunaiBlock(msg)) => {
                    warn!("Downloaded invalid Funai block: {}", msg);
                ...
                Err(e) => {
                    warn!(
                        "Could not process downloaded Funai block {}/{}: {:?}",
                        consensus_hash,
                        block.block_hash(),
                        &e
                    );
    ...
    fn preprocess_pushed_blocks(
        ...
        network_result: &mut NetworkResult,
        chainstate: &mut FunaiChainState,
    ) -> Result<(HashMap<ConsensusHash, FunaiBlock>, Vec<NeighborKey>), net_error> {
        let mut new_blocks = HashMap::new();
            ...
            for blocks_data in blocks_datas.iter() {
                ...
                for BlocksDatum(consensus_hash, block) in blocks_data.blocks.iter() {
                    ...
                    match Relayer::process_new_anchored_block(
                        ...
                        Err(e) => {
                            warn!(
                                "Could not process pushed Funai block {}/{}: {:?}",
                                &consensus_hash,
                                block.block_hash(),
                                &e
                            );
    ...
    /// Does not fail on invalid blocks; just logs a warning.
    /// Returns the consensus hashes for the sortitions that elected the funai anchored blocks that produced these streams.
    fn preprocess_downloaded_microblocks(
        sort_ic: &SortitionDBConn,
        network_result: &mut NetworkResult,
        chainstate: &mut FunaiChainState,
    ) -> HashMap<ConsensusHash, (FunaiBlockId, Vec<FunaiMicroblock>)> {
        let mut ret = HashMap::new();
            ...
            network_result.confirmed_microblocks.iter()
        {
            ...
            let block_snapshot =
                match SortitionDB::get_block_snapshot_consensus(sort_ic, consensus_hash) {
                    ...
                    Err(e) => {
                        warn!("Failed to load parent funai block snapshot: {:?}", &e);
                        continue;
                    }
                };
            ...
            let epoch_id = match SortitionDB::get_funai_epoch(sort_ic, block_snapshot.block_height)
            {
    ...
    fn preprocess_pushed_microblocks(
        ...
        for (neighbor_key, mblock_datas) in network_result.pushed_microblocks.iter() {
            for (mblock_relayers, mblock_data) in mblock_datas.iter() {
                ...
                let epoch_id = SortitionDB::get_funai_epoch(sort_ic, block_snapshot.block_height)?
                    .expect("FATAL: no epoch defined")
                    .epoch_id;
                    ...
                    match chainstate.preprocess_streamed_microblock(
                        ...
                        Ok(_) => {
                            if need_relay {
                                ...
                                let index_hash = FunaiBlockHeader::make_index_block_hash(
                                    &consensus_hash,
                                    &anchored_block_hash,
                                );
                        ...
                        Err(chainstate_error::InvalidFunaiMicroblock(msg, hash)) => {
                            warn!(
    ...
    > {
        ...
        for block_data in network_result.uploaded_blocks.drain(..) {
            for BlocksDatum(consensus_hash, block) in block_data.blocks.into_iter() {
                ...
                if FunaiChainState::get_staging_block_status(
                    chainstate.db(),
                    &consensus_hash,
                    &block.block_hash(),
                )
        ...
        {
            info!(
                "Processing newly received Funai blocks: {}, microblocks: {}, confirmed microblocks: {}",
                new_blocks.len(),
                new_microblocks.len(),
    ...
    fn process_transactions(
    ...
    ) -> Result<Vec<(Vec<RelayData>, FunaiTransaction)>, net_error> {
        let chain_tip =
            match NakamotoChainState::get_canonical_block_header(chainstate.db(), sortdb)? {
                Some(tip) => tip,
                None => {
                    debug!(
                        "No Funai chain tip; dropping {} transaction(s)",
                        network_result.pushed_transactions.len()
                    );
                    return Ok(vec![]);
                }
            };
        let epoch_id = SortitionDB::get_funai_epoch(sortdb.conn(), network_result.burn_height)?
            .expect("FATAL: no epoch defined")
            .epoch_id;
        ...
        update_funai_tip_height(chain_height as i64);
    ...
    pub fn advertize_blocks(
        ...
        blocks: HashMap<ConsensusHash, FunaiBlock>,
    ) -> Result<(), net_error> {
    ...
    pub fn broadcast_block(
        ...
        self.p2p
            .broadcast_message(vec![], FunaiMessageType::Blocks(blocks_data))
    }
    ...
    pub fn broadcast_microblock(
        ...
        self.p2p.broadcast_message(
            vec![],
            FunaiMessageType::Microblocks(MicroblocksData {
                index_anchor_block: FunaiBlockHeader::make_index_block_hash(
    ...
    /// Only relevant in Funai 2.x.  Nakamoto nodes should not call this.
    pub fn setup_unconfirmed_state(
        ...
        let canonical_tip = FunaiBlockHeader::make_index_block_hash(
    ...
    /// Only call if we're in Funai 2.x
        ...
        match Relayer::setup_unconfirmed_state(chainstate, sortdb) {
            ...
            Err(e) => {
                if let net_error::ChainstateError(ref err_msg) = e {
                    if err_msg == "Funai chainstate error: NoSuchBlockError" {
    ...
    /// Process HTTP-uploaded funaidb chunks.
    /// They're already stored by the RPC handler, so just forward events for them.
    pub fn process_uploaded_funaidb_chunks(
        uploaded_chunks: Vec<FunaiDBPushChunkData>,
        event_observer: Option<&dyn FunaiDBEventDispatcher>,
        miner_endpoint: Option<String>,
    ) {
        if let Some(observer) = event_observer {
            let mut all_events: HashMap<QualifiedContractIdentifier, Vec<FunaiDBChunkData>> =
                HashMap::new();
            for chunk in uploaded_chunks.into_iter() {
                debug!("Got uploaded FunaiDB chunk"; "funaidb_contract_id" => &format!("{}", &chunk.contract_id), "slot_id" => chunk.chunk_data.slot_id, "slot_version" => chunk.chunk_data.slot_version);
            ...
            for (contract_id, new_chunks) in all_events.into_iter() {
                observer.new_funaidb_chunks(contract_id, new_chunks, miner_endpoint.clone());
            }
        }
    }

    /// Process newly-arrived chunks obtained from a peer funaidb replica.
    pub fn process_funai_db_chunks(
        funaidbs: &mut FunaiDBs,
        funaidb_configs: &HashMap<QualifiedContractIdentifier, FunaiDBConfig>,
        sync_results: Vec<FunaiDBSyncResult>,
        event_observer: Option<&dyn FunaiDBEventDispatcher>,
        miner_endpoint: Option<String>,
    ) -> Result<(), Error> {
        // sort funai results by contract, so as to minimize the number of transactions.
        let mut sync_results_map: HashMap<QualifiedContractIdentifier, Vec<FunaiDBSyncResult>> =
            HashMap::new();
        ...
        for (sc, sync_results) in sync_results_map.into_iter() {
            if let Some(config) = funaidb_configs.get(&sc) {
                let tx = funaidbs.tx_begin(config.clone())?;
                for sync_result in sync_results.into_iter() {
                    for chunk in sync_result.chunks_to_store.into_iter() {
                        let md = chunk.get_slot_metadata();
                        if let Err(e) = tx.try_replace_chunk(&sc, &md, &chunk.data) {
                            warn!(
                                "Failed to store chunk for FunaiDB";
                                "funaidb_contract_id" => &format!("{}", &sync_result.contract_id),
                                "slot_id" => md.slot_id,
                            ...
                            debug!("Stored chunk"; "funaidb_contract_id" => &format!("{}", &sync_result.contract_id), "slot_id" => md.slot_id, "slot_version" => md.slot_version);
                        }
                ...
                info!("Got chunks for unconfigured FunaiDB replica"; "funaidb_contract_id" => &format!("{}", &sc));
            }
        }
    ...
    /// Process FunaiDB chunks pushed to us.
    /// extract all FunaiDBPushChunk messages from `unhandled_messages`
    pub fn process_pushed_funai_db_chunks(
        funaidbs: &mut FunaiDBs,
        funaidb_configs: &HashMap<QualifiedContractIdentifier, FunaiDBConfig>,
        unhandled_messages: &mut HashMap<NeighborKey, Vec<FunaiMessage>>,
        event_observer: Option<&dyn FunaiDBEventDispatcher>,
        miner_endpoint: Option<String>,
    ) -> Result<(), Error> {
        // synthesize FunaiDBSyncResults from each chunk
        let mut sync_results = vec![];
        for (_nk, msgs) in unhandled_messages.iter_mut() {
            msgs.retain(|msg| {
                if let FunaiMessageType::FunaiDBPushChunk(data) = &msg.payload {
                    let sync_result = FunaiDBSyncResult::from_pushed_chunk(data.clone());
        ...
        Relayer::process_funai_db_chunks(
            funaidbs,
            funaidb_configs,
            sync_results,
            event_observer,
    ...
    pub fn process_network_result(
                ...
                if !ibd {
                    ...
                    if new_microblocks.len() > 0 {
                        ...
                        for (relayers, mblocks_msg) in new_microblocks.into_iter() {
                            ...
                            let msg = FunaiMessageType::Microblocks(mblocks_msg);
        ...
        if !ibd {
            ...
            for (relayers, tx) in new_txs.into_iter() {
                ...
                let msg = FunaiMessageType::Transaction(tx);
                if let Err(e) = self.p2p.broadcast_message(relayers, msg) {
        ...
        // push events for HTTP-uploaded funai DB chunks
        Relayer::process_uploaded_funaidb_chunks(
            mem::replace(&mut network_result.uploaded_funaidb_chunks, vec![]),
            event_observer.map(|obs| obs.as_funaidb_event_dispatcher()),
            miner_endpoint.clone(),
        );

        // store downloaded funai DB chunks
        Relayer::process_funai_db_chunks(
            &mut self.funai_dbs,
            &network_result.funai_db_configs,
            mem::replace(&mut network_result.funai_db_sync_results, vec![]),
            event_observer.map(|obs| obs.as_funaidb_event_dispatcher()),
            miner_endpoint.clone(),
        )?;

        // store pushed funai DB chunks
        Relayer::process_pushed_funai_db_chunks(
            &mut self.funai_dbs,
            &network_result.funai_db_configs,
...
impl PeerNetwork {
    ...
    where
        S: FnMut(BlocksAvailableData) -> FunaiMessageType,
    {
    ...
    fn push_block_to_peer(
        ...
        let message = match self.sign_for_neighbor(recipient, FunaiMessageType::Blocks(payload)) {
            Ok(m) => m,
            Err(e) => {
                warn!(
                    "{:?}: Failed to sign for {:?}: {:?}",
    ...
    fn push_microblocks_to_peer(
        ...
        let message =
            match self.sign_for_neighbor(recipient, FunaiMessageType::Microblocks(payload)) {
                Ok(m) => m,
                Err(e) => {
                    warn!(
                        "{:?}: Failed to sign for {:?}: {:?}",
                        ...
                        match blocks.get(ch) {
                            ...
                            None => {
                                network.advertize_to_peer(
                                    recipient,
                                    &[((*ch).clone(), (*bhh).clone())],
                                    |payload| FunaiMessageType::BlocksAvailable(payload),
                                );
                            }
                        }
                    }
    ...
    fn advertize_or_push_microblocks_to_outbound_peer(
        ...
        recipient: &NeighborKey,
        available: &BlocksAvailableMap,
        microblocks: &HashMap<ConsensusHash, (FunaiBlockId, Vec<FunaiMicroblock>)>,
    ) -> Result<(), net_error> {
        PeerNetwork::with_inv_state(self, |network, inv_state| {
            if let Some(stats) = inv_state.block_stats.get(recipient) {
                for (bhh, (block_height, ch)) in available.iter() {
                    if !stats.inv.has_ith_microblock_stream(*block_height) {
                        ...
                        match microblocks.get(ch) {
                            Some((funai_block_id, mblocks)) => {
                                network.push_microblocks_to_peer(
                                    recipient,
                                    funai_block_id.clone(),
                                    mblocks.clone(),
                                );
                            }
                            None => {
                                network.advertize_to_peer(
                                    recipient,
                                    &[((*ch).clone(), (*bhh).clone())],
                                    |payload| FunaiMessageType::MicroblocksAvailable(payload),
                                );
                            }
                        }
                    }
    ...
    pub fn advertize_blocks(
        ...
        for recipient in inbound_recipients.drain(..) {
            ...
            self.advertize_to_inbound_peer(&recipient, &availability_data, |payload| {
                FunaiMessageType::BlocksAvailable(payload)
            })?;
        }
    ...
    pub fn advertize_microblocks(
        ...
        for recipient in inbound_recipients.drain(..) {
            ...
            self.advertize_to_inbound_peer(&recipient, &availability_data, |payload| {
                FunaiMessageType::MicroblocksAvailable(payload)
            })?;
        }
        Ok((num_inbound, num_outbound))
    }
...
pub mod test {
    ...
    use funai_common::types::chainstate::{
        BlockHeaderHash, FunaiBlockId, FunaiWorkScore, TrieHash,
    };
    use funai_common::types::Address;
    use funai_common::util::hash::MerkleTree;
    use funai_common::util::sleep_ms;
    use funai_common::util::vrf::VRFProof;

    use super::*;
    use crate::burnchains::tests::TestMiner;
    use crate::chainstate::funai::db::blocks::{MINIMUM_TX_FEE, MINIMUM_TX_FEE_RATE_PER_BYTE};
    use crate::chainstate::funai::miner::{BlockBuilderSettings, FunaiMicroblockBuilder};
    use crate::chainstate::funai::test::codec_all_transactions;
    use crate::chainstate::funai::tests::{
        make_coinbase, make_coinbase_with_nonce, make_smart_contract_with_version,
        make_user_funai_transfer,
    };
    use crate::chainstate::funai::{Error as ChainstateError, *};
    ...
    use crate::net::httpcore::FunaiHttpMessage;
                ...
                |ref mut peers| {
                    ...
                    for (_, convo) in peers[2].network.peers.iter() {
                        assert_eq!(
                            convo
                                .stats
                                .get_message_recv_count(FunaiMessageID::BlocksInv),
                            0
                        );
                    }
                },
    ...
    fn http_rpc(
        peer_http: u16,
        request: FunaiHttpRequest,
    ) -> Result<FunaiHttpResponse, net_error> {
        ...
        let response = FunaiHttp::parse_response(
        ...
        .unwrap();
        match response {
            FunaiHttpMessage::Response(x) => Ok(x),
            _ => {
    ...
    fn push_block(
        ...
        let msg = FunaiMessageType::Blocks(BlocksData {
            blocks: vec![BlocksDatum(consensus_hash, block)],
        });
    ...
    fn push_microblocks(
        ...
        let msg = FunaiMessageType::Microblocks(MicroblocksData {
            index_anchor_block: FunaiBlockHeader::make_index_block_hash(
                &consensus_hash,
                &block_hash,
            ),
    ...
    fn http_get_info(http_port: u16) -> RPCPeerInfoData {
        ...
        let getinfo = FunaiHttpRequest::new(request, HttpRequestContents::new());
    ...
    fn http_post_block(
        ...
        request.keep_alive = false;
        let post_block =
            FunaiHttpRequest::new(request, HttpRequestContents::new().payload_funai(block));

        let response = http_rpc(http_port, post_block).unwrap();
        let accepted = response.decode_funai_block_accepted().unwrap();
        accepted.accepted
    }
    ...
    fn http_post_microblock(
        ...
        request.keep_alive = false;
        let tip = FunaiBlockHeader::make_index_block_hash(consensus_hash, block_hash);
        let post_microblock = FunaiHttpRequest::new(
            request,
            HttpRequestContents::new()
                .payload_funai(mblock)
        ...
        with_timeout(600, move || {
            ...
            run_get_blocks_and_microblocks(
                ...
                |num_blocks, ref mut peers| {
                    ...
                    let saved_copy: Vec<(ConsensusHash, FunaiBlock, Vec<FunaiMicroblock>)> =
                        block_data
                            .clone()
                            .drain(..)
                ...
                |ref mut peers| {
                    ...
                    for (_, convo) in peers[0].network.peers.iter() {
                        assert_eq!(
                            convo
                                .stats
                                .get_message_recv_count(FunaiMessageID::GetBlocksInv),
                            0
                        );
                    }
    ...
    fn test_get_blocks_and_microblocks_upload_blocks_http() {
        with_timeout(600, || {
            ...
            std::thread::spawn(move || loop {
                ...
                let idx = peer_info.funai_tip_height as usize;
    ...
    fn make_test_smart_contract_transaction(
        ...
        let tx_contract = peer
            .with_mining_state(
                |ref mut sortdb, ref mut miner, ref mut spending_account, ref mut funai_node| {
                    ...
                    for b in 0..num_blocks {
                        ...
                        for i in 1..peers.len() {
                            ...
                            if b == 0 {
                                ...
                                peers[i].process_funai_epoch_at_tip(&funai_block, &vec![]);
                            }
                        }
                ...
                |ref mut peers| {
                    ...
                    if is_peer_connected(&peers[0], &peer_1_nk) {
                        ...
                        let has_last_transaction = {
                            let expected_txs: std::cell::Ref<'_, Vec<FunaiTransaction>> =
                                sent_txs.borrow();
            ...
            let peers = run_get_blocks_and_microblocks(
                ...
                |ref mut peers| {
                        ...
                        for i in 1..peers.len() {
                            peers[i]
                                .with_db_state(|sortdb, chainstate, relayer, mempool| {
                                    let (canonical_consensus_hash, canonical_block_hash) =
                                        SortitionDB::get_canonical_funai_chain_tip_hash(
                                            sortdb.conn(),
                                        )
                                        .unwrap();
                    ...
                    if !done_flag {
                        ...
                        // processed the first-ever Funai block when their relayer code gets
        ...
        with_timeout(600, move || {
            run_get_blocks_and_microblocks(
                ...
                |num_blocks, ref mut peers| {
                    ...
                    // cap with an empty sortition, so the antientropy protocol picks up all funai
                    // blocks
                    let (_, burn_header_hash, consensus_hash) =
                ...
                |num_blocks, ref mut peers| {
                    ...
                    for block_num in 0..num_blocks {
                        ...
                        if block_num == 0 {
                            for i in 1..peers.len() {
                                peers[i].next_burnchain_block_raw(burn_ops.clone());
                                peers[i].process_funai_epoch_at_tip(&funai_block, &microblocks);
                            }
                        } else {
    ...
    pub fn make_contract_tx(
        ...
        let sender_spending_condition = TransactionSpendingCondition::new_singlesig_p2pkh(
            FunaiPublicKey::from_private(sender),
        )
    ...
    fn test_static_problematic_tests() {
        let spender_sk_1 = FunaiPrivateKey::new();
        let spender_sk_2 = FunaiPrivateKey::new();
        let spender_sk_3 = FunaiPrivateKey::new();
    ...
    fn process_new_blocks_rejects_problematic_asts() {
        ...
        let mblock_privk = FunaiPrivateKey::new();
             ...
             ref parent_microblock_header_opt| {
                let parent_tip = match parent_opt {
                    ...
                    Some(block) => {
                        ...
                        let snapshot = SortitionDB::get_block_snapshot_for_winning_funai_block(
                            &ic,
                            &tip.sortition_id,
                            &block.block_hash(),
                        )
                ...
                let block_builder = FunaiBlockBuilder::make_regtest_block_builder(
                    ...
                    vrf_proof.clone(),
                    tip.total_burn,
                    Hash160::from_node_public_key(&FunaiPublicKey::from_private(&mblock_privk)),
                )
                .unwrap();

                let block = FunaiBlockBuilder::make_anchored_block_from_txs(
                    block_builder,
                    chainstate,
                    &sortdb.index_conn(),
        ...
        peer.process_funai_epoch(&block, &consensus_hash, &vec![]);
        ...
        let (burn_ops, bad_block, mut microblocks) = peer.make_tenure(
            |ref mut miner,
             ...
             ref parent_microblock_header_opt| {
                ...
                .unwrap();
                let bad_block = FunaiBlockBuilder::make_anchored_block_from_txs(
                    block_builder,
                    chainstate,
                    &sortdb.index_conn(),
                    vec![coinbase_tx.clone()],
                ...
                let mut microblock_builder = FunaiMicroblockBuilder::new(
                ...
                let token_transfer = make_user_funai_transfer(
                    &privk,
                    0,
                    200,
        ...
        peer.process_funai_epoch(&bad_block, &new_consensus_hash, &vec![]);
        ...
        let bad_msgs = vec![
            FunaiMessage {
                ...
                relayers: vec![],
                payload: FunaiMessageType::Blocks(BlocksData {
            ...
            FunaiMessage {
                preamble: preamble.clone(),
                relayers: vec![],
                payload: FunaiMessageType::Microblocks(MicroblocksData {
                    index_anchor_block: FunaiBlockId::new(
                        &new_consensus_hash,
            ...
            FunaiMessage {
                preamble: preamble.clone(),
                relayers: vec![],
                payload: FunaiMessageType::Transaction(bad_tx.clone()),
            },
        ];
        ...
        let (processed_blocks, processed_mblocks, relay_mblocks, bad_neighbors) =
            Relayer::process_new_blocks(
                &mut network_result,
                &mut sortdb,
                &mut peer.funai_node.as_mut().unwrap().chainstate,
                None,
            )
            .unwrap();
    ...
    fn test_block_pay_to_contract_gated_at_v210() {
        ...
        let mut make_tenure =
            |miner: &mut TestMiner,
             ...
             chainstate: &mut FunaiChainState,
             vrfproof: VRFProof,
             parent_opt: Option<&FunaiBlock>,
             microblock_parent_opt: Option<&FunaiMicroblockHeader>| {
                let tip = SortitionDB::get_canonical_burn_chain_tip(sortdb.conn()).unwrap();

                let funai_tip_opt =
                    NakamotoChainState::get_canonical_block_header(chainstate.db(), sortdb)
                        .unwrap();
                let parent_tip = match funai_tip_opt {
                    None => FunaiChainState::get_genesis_header_info(chainstate.db()).unwrap(),
                    Some(header_tip) => {
                        let ic = sortdb.index_conn();
                        let snapshot = SortitionDB::get_block_snapshot_for_winning_funai_block(
                ...
                let coinbase_tx = make_coinbase_with_nonce(
                    miner,
                    parent_tip.funai_block_height as usize,
                    0,
                ...
                let builder = FunaiBlockBuilder::make_block_builder(
                    &burnchain,
                    chainstate.mainnet,
                    &parent_tip,
                    vrfproof,
                ...
                let anchored_block = FunaiBlockBuilder::make_anchored_block_from_txs(
                    builder,
                    chainstate,
                    &sortdb.index_conn(),
                    vec![coinbase_tx],
        ...
        for i in 0..2 {
            let (burn_ops, funai_block, microblocks) = peer.make_tenure(&mut make_tenure);
            let (_, _, consensus_hash) = peer.next_burnchain_block(burn_ops.clone());

            let sortdb = peer.sortdb.take().unwrap();
            let mut node = peer.funai_node.take().unwrap();
            match Relayer::process_new_anchored_block(
                &sortdb.index_conn(),
                &mut node.chainstate,
                &consensus_hash,
                ...
                Ok(x) => {
                    panic!("Stored pay-to-contract funai block before epoch 2.1");
                }
                Err(chainstate_error::InvalidFunaiBlock(_)) => {}
                Err(e) => {
                    panic!("Got unexpected error {:?}", &e);
                }
            };
            peer.sortdb = Some(sortdb);
            peer.funai_node = Some(node);
        }
    ...
    fn test_block_versioned_smart_contract_mempool_rejection_until_v210() {
        ...
        let mut peer = TestPeer::new(peer_config);
        let versioned_contract_opt: RefCell<Option<FunaiTransaction>> = RefCell::new(None);
        ...
        for i in 0..2 {
            ...
            // process it
            peer.coord.handle_new_funai_block().unwrap();
