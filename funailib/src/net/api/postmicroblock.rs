...
use funai_common::util::hash::{hex_bytes, Hash160, Sha256Sum};
...
use crate::chainstate::funai::{
    Error as ChainError, FunaiBlockHeader, FunaiMicroblock, FunaiTransaction, TransactionPayload,
};
...
use crate::net::httpcore::{
    HttpPreambleExtensions, HttpRequestContentsExtensions, RPCRequestHandler, FunaiHttpRequest,
    FunaiHttpResponse,
};
use crate::net::p2p::PeerNetwork;
use crate::net::relay::Relayer;
use crate::net::{
    Attachment, Error as NetError, MicroblocksData, FunaiMessageType, FunaiNodeState, TipRequest,
};

#[derive(Clone)]
pub struct RPCPostMicroblockRequestHandler {
    pub microblock: Option<FunaiMicroblock>,
}
...
impl RPCPostMicroblockRequestHandler {
    ...
    fn parse_postmicroblock_octets(mut body: &[u8]) -> Result<FunaiMicroblock, Error> {
        let mblock = FunaiMicroblock::consensus_deserialize(&mut body).map_err(|e| {
...
impl RPCRequestHandler for RPCPostMicroblockRequestHandler {
    ...
    fn try_handle_request(
        ...
        let data_resp = node.with_node_state(|_network, sortdb, chainstate, _mempool, _rpc_args| {
            let funai_tip = match FunaiChainState::load_staging_block_info(chainstate.db(), &tip) {
                Ok(Some(tip_info)) => tip_info,
                Ok(None) => {
                    return Err(FunaiHttpResponse::new_error(&preamble, &HttpNotFound::new("No such funai tip".into())));
                },
                Err(e) => {
                    return Err(FunaiHttpResponse::new_error(&preamble, &HttpServerError::new(format!("Failed to load chain tip: {:?}", &e))));
                }
            };

            let consensus_hash = &funai_tip.consensus_hash;
            let block_hash = &funai_tip.anchored_block_hash;

            // make sure we can accept this
            let ch_sn = match SortitionDB::get_block_snapshot_consensus(sortdb.conn(), consensus_hash) {
                Ok(Some(sn)) => sn,
                Ok(None) => {
                    return Err(FunaiHttpResponse::new_error(&preamble, &HttpNotFound::new("No such snapshot for Funai tip consensus hash".to_string())));
                }
                Err(e) => {
                    debug!("No block snapshot for consensus hash {}", &consensus_hash);
                    return Err(FunaiHttpResponse::new_error(&preamble, &HttpBadRequest::new_json(ChainError::DBError(e).into_json())));
                }
            };

            let sort_handle = sortdb.index_handle(&ch_sn.sortition_id);
            let parent_block_snapshot = Relayer::get_parent_funai_block_snapshot(&sort_handle, consensus_hash, block_hash)
                .map_err(|e| FunaiHttpResponse::new_error(&preamble, &HttpServerError::new(format!("Failed to load parent block for Funai tip: {:?}", &e))))?;

            let ast_rules = SortitionDB::get_ast_rules(&sort_handle, parent_block_snapshot.block_height)
                .map_err(|e| FunaiHttpResponse::new_error(&preamble, &HttpServerError::new(format!("Failed to load AST rules for Bitcoin block height {}: {:?}", parent_block_snapshot.block_height, &e))))?;

            let epoch_id = self.get_funai_epoch(&preamble, sortdb, parent_block_snapshot.block_height)?.epoch_id;
            ...
            if !Relayer::static_check_problematic_relayed_microblock(
                ...
                return Err(FunaiHttpResponse::new_error(&preamble, &HttpBadRequest::new_json(ChainError::ProblematicTransaction(Txid([0x00; 32])).into_json())));
            }
            ...
            match chainstate.preprocess_streamed_microblock(consensus_hash, block_hash, &microblock) {
                Ok(accepted) => {
                    ...
                    return Ok((accepted, FunaiBlockHeader::make_index_block_hash(consensus_hash, block_hash)));
                },
                Err(e) => {
                    debug!("Failed to process microblock {}/{}-{}: {:?}", &consensus_hash, &block_hash, &microblock.block_hash(), &e);
                    return Err(FunaiHttpResponse::new_error(&preamble, &HttpBadRequest::new_json(e.into_json())));
                }
            }
        });
        ...
        if accepted {
            node.set_relay_message(FunaiMessageType::Microblocks(MicroblocksData {
...
impl FunaiHttpResponse {
    pub fn decode_funai_microblock_response(self) -> Result<BlockHeaderHash, NetError> {
