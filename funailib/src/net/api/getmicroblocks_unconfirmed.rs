...
use crate::net::{
    Error as NetError, FunaiNodeState, TipRequest, MAX_HEADERS, MAX_MICROBLOCKS_UNCONFIRMED,
};
use crate::util_lib::db::{DBConn, Error as DBError};

#[derive(Clone)]
pub struct RPCMicroblocksUnconfirmedRequestHandler {
    pub parent_block_id: Option<FunaiBlockId>,
...
impl FunaiUnconfirmedMicroblockStream {
    pub fn new(
        ...
        Ok(FunaiUnconfirmedMicroblockStream {
    ...
    fn try_handle_request(
        ...
        let stream_res =
            node.with_node_state(|_network, _sortdb, chainstate, _mempool, _rpc_args| {
                FunaiUnconfirmedMicroblockStream::new(chainstate, &block_id, start_seq)
            });
...
impl HttpChunkGenerator for FunaiUnconfirmedMicroblockStream {
    ...
    fn generate_next_chunk(&mut self) -> Result<Vec<u8>, String> {
        ...
        let next_mblock_opt = FunaiChainState::load_next_descendant_microblock(
            &self.chainstate_db,
...
impl FunaiHttpResponse {
    ...
    pub fn decode_microblocks_unconfirmed(self) -> Result<Vec<FunaiMicroblock>, NetError> {
        ...
        loop {
            let mblock: FunaiMicroblock = match read_next(&mut bound_reader) {
                Ok(mblock) => Ok(mblock),
                Err(e) => match e {
