...
impl FunaiIndexedMicroblockStream {
    /// Make a new indexed microblock streamer using the descendent Funai anchored block
    pub fn new_confirmed(
        ...
        FunaiIndexedMicroblockStream::new(chainstate, &tail_microblock_index_hash)
    }
}
    ...
    fn try_handle_request(
        ...
        let stream_res =
            node.with_node_state(|_network, _sortdb, chainstate, _mempool, _rpc_args| {
                let res = FunaiIndexedMicroblockStream::new_confirmed(chainstate, &block_id);
                res
            });
