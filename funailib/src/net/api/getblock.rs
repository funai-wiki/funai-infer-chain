...
pub struct RPCBlocksRequestHandler {
    pub block_id: Option<FunaiBlockId>,
}
...
impl FunaiBlockStream {
    pub fn new(chainstate: &FunaiChainState, block: &FunaiBlockId) -> Result<Self, ChainError> {
        let _ = FunaiChainState::load_staging_block_info(chainstate.db(), block)?
...
impl HttpRequest for RPCBlocksRequestHandler {
    ...
    fn try_parse_request(
        ...
        let block_id = FunaiBlockId::from_hex(block_id_str)
    ...
    fn try_handle_request(
        ...
        let stream_res =
            node.with_node_state(|_network, _sortdb, chainstate, _mempool, _rpc_args| {
                FunaiBlockStream::new(chainstate, &block_id)
            });
...
impl HttpChunkGenerator for FunaiBlockStream {
    #[cfg(test)]
    ...
    fn generate_next_chunk(&mut self) -> Result<Vec<u8>, String> {
        let block_path =
            FunaiChainState::get_index_block_path(&self.blocks_path, &self.index_block_hash)
                .map_err(|e| {
                    let msg = format!(
...
impl FunaiHttpRequest {
    pub fn new_getblock(host: PeerHost, index_block_hash: FunaiBlockId) -> FunaiHttpRequest {
...
impl FunaiHttpResponse {
    #[cfg(test)]
    pub fn new_getblock(block: FunaiBlock, with_content_length: bool) -> FunaiHttpResponse {
    ...
    pub fn decode_block(self) -> Result<FunaiBlock, NetError> {
        let contents = self.get_http_payload_ok()?;

        // contents will be raw bytes
        let block_bytes: Vec<u8> = contents.try_into()?;
        let block = FunaiBlock::consensus_deserialize(&mut &block_bytes[..])?;
