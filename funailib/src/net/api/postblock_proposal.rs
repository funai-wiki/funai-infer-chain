...
use funai_common::codec::{
    read_next, write_next, Error as CodecError, FunaiMessageCodec, MAX_PAYLOAD_LEN,
};
use funai_common::consts::CHAIN_ID_MAINNET;
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, ConsensusHash, FunaiBlockId, FunaiPublicKey,
};
use funai_common::types::net::PeerHost;
use funai_common::types::FunaiPublicKeyBuffer;
use funai_common::util::get_epoch_time_ms;
use funai_common::util::hash::{hex_bytes, to_hex, Hash160, Sha256Sum, Sha512Trunc256Sum};
...
use crate::chainstate::funai::miner::{BlockBuilder, BlockLimitFunction, TransactionResult};
use crate::chainstate::funai::{
    Error as ChainError, FunaiBlock, FunaiBlockHeader, FunaiTransaction, TransactionPayload,
};
...
use crate::net::{
    Attachment, BlocksData, BlocksDatum, Error as NetError, FunaiMessageType, FunaiNodeState,
};
...
///  that the funai-node thinks should be rejected.
...
/// A response for block proposal validation
///  that the funai-node thinks is acceptable.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
impl NakamotoBlockProposal {
    fn spawn_validation_thread(
        self,
        sortdb: SortitionDB,
        mut chainstate: FunaiChainState,
    ...
    pub fn validate(
        &self,
        sortdb: &SortitionDB,
        chainstate: &mut FunaiChainState, // not directly used; used as a handle to open other chainstates
        ...
        let parent_funai_header = NakamotoChainState::get_block_header(
            chainstate.db(),
        ...
        info!(
            ...
            "parent_funai_block_id" => %block.header.parent_block_id,
            "block_size" => size,
            "execution_cost" => %cost,
