...
pub struct RPCGetAttachmentsInvRequestHandler {
    pub index_block_hash: Option<FunaiBlockId>,
    ...
    fn try_parse_request(
        ...
        for (key, value) in form_urlencoded::parse(query_str.as_bytes()) {
            if key == "index_block_hash" {
                index_block_hash = FunaiBlockId::from_hex(&value).ok();
...
impl RPCRequestHandler for RPCGetAttachmentsInvRequestHandler {
    ...
    fn try_handle_request(
        ...
        if page_indexes.len() > MAX_ATTACHMENT_INV_PAGES_PER_REQUEST {
            ...
            warn!("{}", msg);
            return FunaiHttpResponse::new_error(&preamble, &HttpBadRequest::new(msg))
                .try_into_contents()
        ...
        for page_index in page_indexes.iter() {
            ...
            match page_res {
                ...
                Err(msg) => {
                    return FunaiHttpResponse::new_error(&preamble, &HttpNotFound::new(msg))
                        .try_into_contents()
                        .map_err(NetError::from);
                }
            }
