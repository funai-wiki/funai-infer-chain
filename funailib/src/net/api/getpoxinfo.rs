...
use crate::chainstate::funai::boot::{POX_1_NAME, POX_2_NAME, POX_3_NAME, POX_4_NAME};
...
impl From<FunaiEpoch> for RPCPoxEpoch {
    fn from(epoch: FunaiEpoch) -> Self {
        Self {
...
impl RPCPoxInfoData {
    pub fn from_db(
        ...
        //  evaluated in the last mined Funai block, not the most recent burn block).
        let reward_cycle_id = burnchain
            .block_height_to_reward_cycle(burnchain_tip.block_height)
            .ok_or_else(|| {
                NetError::ChainstateError("Current burn block height is before funai start".into())
            })?;
        ...
        let cur_cycle_threshold = FunaiChainState::get_threshold_from_participation(
            total_liquid_supply_ustx as u128,
            cur_cycle_stacked_ustx,
            reward_slots as u128,
        ) as u64;

        let next_threshold = FunaiChainState::get_threshold_from_participation(
            total_liquid_supply_ustx as u128,
        ...
        let epochs: Vec<_> = SortitionDB::get_funai_epochs(sortdb.conn())?
            .into_iter()
...
impl FunaiHttpRequest {
    ...
    pub fn new_getpoxinfo(host: PeerHost, tip_req: TipRequest) -> FunaiHttpRequest {
