...
use funai_common::codec::{read_next, FunaiMessageCodec, MAX_MESSAGE_LEN};
...
use crate::chainstate::funai::{Error as ChainError, FunaiBlockHeader, FunaiMicroblock};
use crate::net::http::{
...
pub struct RPCMicroblocksIndexedRequestHandler {
    pub tail_microblock_id: Option<FunaiBlockId>,
}
...
#[derive(Debug)]
pub struct FunaiIndexedMicroblockStream {
    /// length prefix
    ...
    pub parent_index_block_hash: FunaiBlockId,

    /// connection to the chain state
    chainstate_db: DBConn,
}

impl FunaiIndexedMicroblockStream {
    pub fn new(
        chainstate: &FunaiChainState,
        tail_index_microblock_hash: &FunaiBlockId,
    ) -> Result<Self, ChainError> {
        // look up parent
        let mblock_info = FunaiChainState::load_staging_microblock_info_indexed(
            &chainstate.db(),
        ...
        Ok(FunaiIndexedMicroblockStream {
    ...
    fn try_handle_request(
        ...
        let stream_res =
            node.with_node_state(|_network, _sortdb, chainstate, _mempool, _rpc_args| {
                FunaiIndexedMicroblockStream::new(chainstate, &tail_microblock_id)
            });
...
impl HttpChunkGenerator for FunaiIndexedMicroblockStream {
    #[cfg(not(test))]
    ...
    fn generate_next_chunk(&mut self) -> Result<Vec<u8>, String> {
        ...
        let mblock_info_opt = FunaiChainState::load_staging_microblock_indexed(
            &self.chainstate_db,
...
impl FunaiHttpResponse {
    ...
    pub fn decode_microblocks(self) -> Result<Vec<FunaiMicroblock>, NetError> {
        let contents = self.get_http_payload_ok()?;

        // contents will be a SIP-003 bytestream
        let mblock_bytes: Vec<u8> = contents.try_into()?;
        let microblocks: Vec<FunaiMicroblock> = read_next(&mut &mblock_bytes[..])?;
