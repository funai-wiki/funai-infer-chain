...
use crate::net::api::getblock::FunaiBlockStream;
use crate::net::api::*;
...
fn test_try_parse_request() {
    ...
    let request = FunaiHttpRequest::new_getblock(addr.into(), FunaiBlockId([0x11; 32]));
    ...
    // consumed path args
    assert_eq!(handler.block_id, Some(FunaiBlockId([0x11; 32])));
...
fn test_try_make_response() {
    ...
    let request = FunaiHttpRequest::new_getblock(addr.into(), funai_chain_tip.clone());
    ...
    assert_eq!(
        FunaiBlockHeader::make_index_block_hash(&consensus_hash, &resp.block_hash()),
        funai_chain_tip
    );
...
fn test_stream_blocks() {
    ...
    assert!(FunaiBlockStream::new(&chainstate, &index_block_header).is_err());
    ...
    // should succeed now
    let mut stream = FunaiBlockStream::new(&chainstate, &index_block_header).unwrap();
    ...
    let staging_block = FunaiBlock::consensus_deserialize(&mut &all_block_bytes[..]).unwrap();
