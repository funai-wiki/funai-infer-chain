...
use funai_common::codec::{read_next, Error as CodecError, FunaiMessageCodec};
use funai_common::types::chainstate::{
    BlockHeaderHash, ConsensusHash, FunaiAddress, FunaiPrivateKey,
};
...
use crate::chainstate::funai::db::test::{chainstate_path, instantiate_chainstate};
use crate::chainstate::funai::db::{ExtendedFunaiHeader, FunaiChainState};
use crate::chainstate::funai::{
    Error as chainstate_error, FunaiTransaction, TokenTransferMemo, TransactionAnchorMode,
...
use crate::net::api::postmempoolquery::FunaiMemPoolStream;
use crate::net::api::*;
...
fn test_try_parse_request() {
    ...
    let request = FunaiHttpRequest::new_mempool_query(
        addr.into(),
...
fn test_stream_mempool_txs() {
    ...
    let mut buf = vec![];
    let mut tx_stream_data = FunaiMemPoolStream::new(
    ...
    let mut ptr = &buf[..];
    loop {
        let tx: FunaiTransaction = match read_next::<FunaiTransaction, _>(&mut ptr) {
            Ok(tx) => tx,
            Err(e) => match e {
