...
use crate::net::api::getmicroblocks_indexed::FunaiIndexedMicroblockStream;
use crate::net::api::*;
...
fn test_try_parse_request() {
    ...
    let request =
        FunaiHttpRequest::new_getmicroblocks_indexed(addr.into(), FunaiBlockId([0x22; 32]));
    ...
    assert_eq!(handler.tail_microblock_id, Some(FunaiBlockId([0x22; 32])));
...
fn test_try_make_response() {
    ...
    let request =
        FunaiHttpRequest::new_getmicroblocks_indexed(addr.into(), index_microblock_hash.clone());
    requests.push(request);

    // query non-existant microblock stream
    let request =
        FunaiHttpRequest::new_getmicroblocks_indexed(addr.into(), FunaiBlockId([0x11; 32]));
    requests.push(request);
...
fn test_stream_indexed_microblocks() {
    ...
    for i in 0..mblocks.len() {
        ...
        let microblock_index_header =
            FunaiBlockHeader::make_index_block_hash(&consensus_hash, &mblocks[i].block_hash());

        let mut stream =
            FunaiIndexedMicroblockStream::new(&chainstate, &microblock_index_header).unwrap();
        ...
        let mut confirmed_mblocks =
            Vec::<FunaiMicroblock>::consensus_deserialize(&mut &confirmed_mblock_bytes[..])
                .unwrap();
