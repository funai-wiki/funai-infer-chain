...
fn test_try_parse_request() {
    ...
    .unwrap();
    let request =
        FunaiHttpRequest::new_list_funaidb_replicas(addr.into(), contract_identifier.clone());
    let bytes = request.try_serialize().unwrap();

    debug!("Request:\n{}\n", std::str::from_utf8(&bytes).unwrap());

    let (parsed_preamble, offset) = http.read_preamble(&bytes).unwrap();
    let mut handler = listfunaidbreplicas::RPCListFunaiDBReplicasRequestHandler::new();
...
fn test_try_make_response() {
    ...
    // no contract
    let request = FunaiHttpRequest::new_list_funaidb_replicas(
        addr.into(),
    ...
    let resp = response.decode_funaidb_replicas().unwrap();
