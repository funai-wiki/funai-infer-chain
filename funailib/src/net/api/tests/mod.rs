...
use funai_common::types::chainstate::{
    BlockHeaderHash, ConsensusHash, FunaiAddress, FunaiBlockId, FunaiPrivateKey, FunaiPublicKey,
};
use funai_common::util::get_epoch_time_secs;
use funai_common::util::hash::{Hash160, Sha512Trunc256Sum};
use funai_common::util::pipe::Pipe;
...
use crate::chainstate::funai::{
    CoinbasePayload, FunaiBlock, FunaiBlockBuilder, FunaiBlockHeader, FunaiMicroblock,
...
use crate::net::httpcore::{FunaiHttpRequest, FunaiHttpResponse};
...
use crate::net::{
    Attachment, AttachmentInstance, RPCHandlerArgs, FunaiDBConfig, FunaiNodeState, UrlString,
};
...
pub struct TestRPC<'a> {
    pub privk1: FunaiPrivateKey,
    pub privk2: FunaiPrivateKey,
    ...
    pub canonical_tip: FunaiBlockId,
    ...
    pub next_block: Option<(ConsensusHash, FunaiBlock)>,
    /// next microblock to post (may already be posted)
    pub next_microblock: Option<FunaiMicroblock>,
    /// transactions that can be posted to the mempool
    pub sendable_txs: Vec<FunaiTransaction>,
...
impl<'a> TestRPC<'a> {
    ...
    pub fn setup_ex(test_name: &str, process_microblock: bool) -> TestRPC<'a> {
        ...
        let addr1 = FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(&privk1)],
        )
        .unwrap();
        let addr2 = FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(&privk2)],
        )
        .unwrap();
        ...
        // funai DBs get initialized thru reconfiguration when the above block gets processed
        peer_1_config.add_funai_db(
            QualifiedContractIdentifier::new(addr1.clone().into(), "hello-world".into()),
            FunaiDBConfig::noop(),
        );
        peer_2_config.add_funai_db(
            QualifiedContractIdentifier::new(addr1.clone().into(), "hello-world".into()),
            FunaiDBConfig::noop(),
        );
        ...
        let mut tx_cc = FunaiTransaction::new(
            TransactionVersion::Testnet,
            TransactionAuth::from_p2pkh(&privk1).unwrap(),
        ...
        let mut tx_signer = FunaiTransactionSigner::new(&tx_cc);
        tx_signer.sign_origin(&privk1).unwrap();
        ...
        let mut tx_unconfirmed_contract = FunaiTransaction::new(
            TransactionVersion::Testnet,
            TransactionAuth::from_p2pkh(&privk1).unwrap(),
            TransactionPayload::new_smart_contract(
        ...
        let mut tx_signer = FunaiTransactionSigner::new(&tx_unconfirmed_contract);
        tx_signer.sign_origin(&privk1).unwrap();
        ...
        let (burn_ops, funai_block, microblocks) = peer_1.make_tenure(
        ...
        peer_1.process_funai_epoch_at_tip(&funai_block, &vec![]);
        peer_2.process_funai_epoch_at_tip(&funai_block, &vec![]);
        ...
        let mut peer_1_funai_node = peer_1.funai_node.take().unwrap();
        let _ = peer_1
            .network
            .refresh_burnchain_view(
                &peer_1_indexer,
                &peer_1_sortdb,
                &mut peer_1_funai_node.chainstate,
                false,
            )
            .unwrap();
        peer_1.sortdb = Some(peer_1_sortdb);
        peer_1.funai_node = Some(peer_1_funai_node);

        let peer_2_sortdb = peer_2.sortdb.take().unwrap();
        let mut peer_2_funai_node = peer_2.funai_node.take().unwrap();
        let _ = peer_2
            .network
            .refresh_burnchain_view(
                &peer_2_indexer,
                &peer_2_sortdb,
                &mut peer_2_funai_node.chainstate,
                false,
            )
            .unwrap();
        peer_2.sortdb = Some(peer_2_sortdb);
        peer_2.funai_node = Some(peer_2_funai_node);
        ...
        let attachment_instance = AttachmentInstance {
            ...
            contract_id: QualifiedContractIdentifier::parse("ST000000000000000000002AMW42H.bns")
                .unwrap(),
            tx_id: Txid([0x22; 32]),
            canonical_funai_tip_height: Some(1),
        };
        ...
        let (next_burn_ops, next_funai_block, _) = peer_1.make_tenure(
            |ref mut miner, ref mut sortdb, ref mut chainstate, vrf_proof, ref parent_opt, _| {
                let parent_tip = match parent_opt {
                    None => FunaiChainState::get_genesis_header_info(chainstate.db()).unwrap(),
                    Some(block) => {
        ...
        // store a chunk in the peers' funaidb
        ...
        TestRPC {
            ...
            mempool_txids,
            microblock_txids,
            next_block: Some((next_consensus_hash, next_funai_block)),
    ...
    pub fn run(self, requests: Vec<FunaiHttpRequest>) -> Vec<FunaiHttpResponse> {
        ...
        for request in requests.into_iter() {
...

            {
                ...
                let mut node_state = FunaiNodeState::new(
                    &mut peer_1.network,
                    &peer_1_sortdb,
                    &mut peer_1_funai_node.chainstate,
                    &mut peer_1_mempool,
...
/// and will return the `FunaiHttpResponse` generated by the second peer.
pub fn test_rpc(test_name: &str, requests: Vec<FunaiHttpRequest>) -> Vec<FunaiHttpResponse> {
