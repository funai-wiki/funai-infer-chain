...
fn test_try_parse_request() {
    ...
    let request =
        FunaiHttpRequest::new_post_block(addr.into(), ConsensusHash([0x11; 20]), block.clone());
    ...
    let mut http = FunaiHttp::new(addr.clone(), &ConnectionOptions::default());
    let request = FunaiHttpRequest::new_post_block(
        addr.into(),
...
fn test_try_make_response() {
    ...
    let funai_block_id =
        FunaiBlockHeader::make_index_block_hash(&next_block.0, &next_block.1.block_hash());
    let mut requests = vec![];

    // post the block
    let request =
        FunaiHttpRequest::new_post_block(addr.into(), next_block.0.clone(), next_block.1.clone());
    ...
    let resp = response.decode_funai_block_accepted().unwrap();
    assert_eq!(resp.accepted, true);
    assert_eq!(resp.funai_block_id, funai_block_id);
