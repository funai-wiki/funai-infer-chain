...
fn test_try_parse_request() {
    ...
    let request = FunaiHttpRequest::new_getneighbors(addr.into());
    let bytes = request.try_serialize().unwrap();

    debug!("Request:\n{}\n", std::str::from_utf8(&bytes).unwrap());

    let (parsed_preamble, offset) = http.read_preamble(&bytes).unwrap();
    let mut handler = getneighbors::RPCNeighborsRequestHandler::new();
...
fn test_try_make_response() {
    ...
    // all neighboring peers (there's one) report funaidbs
    for n in resp.sample.iter() {
        assert!(n.funaidbs.is_some());
    }
