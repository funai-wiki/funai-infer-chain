...
fn test_try_parse_request() {
    ...
    let request =
        FunaiHttpRequest::new_getmicroblocks_confirmed(addr.into(), FunaiBlockId([0x22; 32]));
    ...
    assert_eq!(handler.block_id, Some(FunaiBlockId([0x22; 32])));
...
fn test_try_make_response() {
    ...
    let child_index_block_hash =
        FunaiBlockHeader::make_index_block_hash(&child_consensus_hash, &child_block.block_hash());
    ...
    let request = FunaiHttpRequest::new_getmicroblocks_confirmed(
        addr.into(),
    ...
    let request =
        FunaiHttpRequest::new_getmicroblocks_confirmed(addr.into(), FunaiBlockId([0x11; 32]));
    requests.push(request);
...
fn test_stream_confirmed_microblocks() {
    ...
    let mut stream =
        FunaiIndexedMicroblockStream::new_confirmed(&chainstate, &child_block_header).unwrap();
    ...
    let mut confirmed_mblocks =
        Vec::<FunaiMicroblock>::consensus_deserialize(&mut &confirmed_mblock_bytes[..]).unwrap();
