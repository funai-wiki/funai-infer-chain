...
fn test_try_parse_request() {
    ...
    let request = FunaiHttpRequest::new_get_nakamoto_block(addr.into(), FunaiBlockId([0x11; 32]));
...
fn test_try_make_response() {
    ...
    let request =
        FunaiHttpRequest::new_get_nakamoto_block(addr.into(), nakamoto_chain_tip.clone());
    ...
    assert_eq!(
        FunaiBlockHeader::make_index_block_hash(&consensus_hash, &resp.header.block_hash()),
        nakamoto_chain_tip
    );
...
fn test_stream_nakamoto_blocks() {
    ...
    let nakamoto_tip_block_id = FunaiBlockId::new(&nakamoto_tip.0, &nakamoto_tip.1);
    let nakamoto_header = {
