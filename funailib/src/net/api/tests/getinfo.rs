...
fn test_try_parse_request() {
    ...
    let request = FunaiHttpRequest::new_getinfo(addr.into(), Some(123));
    ...
    parsed_request.add_header(
        "X-Canonical-Funai-Tip-Height".to_string(),
        "123".to_string(),
    );
...
fn test_try_make_response() {
    ...
    assert_eq!(
        response.preamble().get_canonical_funai_tip_height(),
        Some(1)
    );
