...
fn test_try_parse_request() {
    ...
    let request = FunaiHttpRequest::new_getattachment(addr.into(), Hash160([0x11; 20]));
...
fn test_try_make_response() {
    ...
    let request = FunaiHttpRequest::new_getattachment(addr.into(), attachment_hash.clone());
    requests.push(request);

    // query non-existant
    let request = FunaiHttpRequest::new_getattachment(addr.into(), Hash160([0x22; 20]));
    requests.push(request);
