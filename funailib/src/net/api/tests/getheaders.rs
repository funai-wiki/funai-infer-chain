...
use crate::net::api::getheaders::FunaiHeaderStream;
use crate::net::api::*;
...
fn test_try_parse_request() {
    ...
    let request = FunaiHttpRequest::new_getheaders(
        addr.into(),
        2100,
...
fn test_try_make_response() {
    ...
    let funai_chain_tip = rpc_test.canonical_tip.clone();
    ...
    let request =
        FunaiHttpRequest::new_getheaders(addr.into(), 2100, TipRequest::UseLatestAnchoredTip);
    requests.push(request);

    // this fails if we use a microblock tip
    let request =
        FunaiHttpRequest::new_getheaders(addr.into(), 2100, TipRequest::UseLatestUnconfirmedTip);
    requests.push(request);
    ...
    let request = FunaiHttpRequest::new_getheaders(
        addr.into(),
        2100,
        TipRequest::SpecificTip(funai_chain_tip.clone()),
    );
    requests.push(request);
    ...
    let request = FunaiHttpRequest::new_getheaders(
        addr.into(),
        2100,
        TipRequest::SpecificTip(FunaiBlockId([0x11; 32])),
    );
    requests.push(request);
    ...
    let resp = response.decode_funai_headers().unwrap();
...
fn stream_headers_to_vec(stream: &mut FunaiHeaderStream) -> Vec<u8> {
...
fn test_stream_getheaders() {
    ...
    let mut blocks_index_hashes: Vec<FunaiBlockId> = vec![];
    ...
    for i in 0..32 {
        ...
        blocks_index_hashes.push(FunaiBlockHeader::make_index_block_hash(
            &consensus_hash,
            &block.block_hash(),
        ));
        blocks.push(block);
    ...
    for i in 16..32 {
        ...
        blocks_fork_index_hashes.push(FunaiBlockHeader::make_index_block_hash(
            &consensus_hash,
            &block.block_hash(),
        ));
    ...
    assert!(FunaiHeaderStream::new(&chainstate, &FunaiBlockId([0x11; 32]), 1).is_err());

    // stream back individual headers
    for i in 0..blocks.len() {
        let mut stream = FunaiHeaderStream::new(&chainstate, &blocks_index_hashes[i], 1).unwrap();
        ...
        let header: Vec<ExtendedFunaiHeader> =
    ...
    let block_expected_headers: Vec<FunaiBlockHeader> =
        blocks.iter().rev().map(|blk| blk.header.clone()).collect();

    let block_expected_index_hashes: Vec<FunaiBlockId> = blocks_index_hashes
        .iter()
        .rev()
        .map(|idx| idx.clone())
        .collect();

    let block_fork_expected_headers: Vec<FunaiBlockHeader> = blocks_fork
        .iter()
        .rev()
        .map(|blk| blk.header.clone())
        .collect();

    let block_fork_expected_index_hashes: Vec<FunaiBlockId> = blocks_fork_index_hashes
        .iter()
        .rev()
        .map(|idx| idx.clone())
        .collect();
    ...
    let mut stream =
        FunaiHeaderStream::new(&chainstate, blocks_index_hashes.last().unwrap(), 4096).unwrap();
    ...
    let headers: Vec<ExtendedFunaiHeader> =
    ...
    let mut stream =
        FunaiHeaderStream::new(&chainstate, blocks_fork_index_hashes.last().unwrap(), 4096)
            .unwrap();
    let header_bytes = stream_headers_to_vec(&mut stream);
    let fork_headers: Vec<ExtendedFunaiHeader> =
        serde_json::from_reader(&mut &header_bytes[..]).unwrap();
    ...
    // ask for only a few
    let mut stream =
        FunaiHeaderStream::new(&chainstate, blocks_index_hashes.last().unwrap(), 10).unwrap();
    let header_bytes = stream_headers_to_vec(&mut stream);
    eprintln!(
        "header bytes: {}",
        String::from_utf8(header_bytes.clone()).unwrap()
    ...
    // ask for only a few
    let mut stream =
        FunaiHeaderStream::new(&chainstate, &blocks_fork_index_hashes.last().unwrap(), 10)
            .unwrap();
    let header_bytes = stream_headers_to_vec(&mut stream);
    let headers: Vec<ExtendedFunaiHeader> =
        serde_json::from_reader(&mut &header_bytes[..]).unwrap();
