...
fn test_try_parse_request() {
    ...
    let request =
        FunaiHttpRequest::new_getattachmentsinv(addr.into(), FunaiBlockId([0x11; 32]), pages);
    ...
    assert_eq!(handler.index_block_hash, Some(FunaiBlockId([0x11; 32])));
    assert_eq!(
...
fn test_try_make_response() {
    ...
    let request = FunaiHttpRequest::new_getattachmentsinv(
        addr.into(),
        funai_chain_tip.clone(),
        pages.clone(),
    );
    ...
    assert_eq!(resp.block_id, funai_chain_tip);
    ...
    assert_eq!(resp.block_id, FunaiBlockId([0x11; 32]));
    assert_eq!(resp.pages.len(), 1);
    assert_eq!(resp.pages[0].index, 1);
