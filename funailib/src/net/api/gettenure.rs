...
pub struct RPCNakamotoTenureRequestHandler {
    ...
    pub last_block_id: Option<FunaiBlockId>,
}
...
impl NakamotoTenureStream {
    pub fn new(
        ...
        last_block_id: Option<FunaiBlockId>,
    ...
    pub fn next_block(&mut self) -> Result<bool, ChainError> {
        ...
        let Some(parent_nakamoto_header) = parent_header.anchored_header.as_funai_nakamoto()
        else {
            return Ok(false);
        };
...
impl HttpRequest for RPCNakamotoTenureRequestHandler {
    ...
    fn try_parse_request(
        ...
        let block_id = FunaiBlockId::from_hex(block_id_str).map_err(|_| {
        ...
        let last_block_id = req_contents
            .get_query_arg("stop")
            .map(|last_block_id_hex| FunaiBlockId::from_hex(&last_block_id_hex))
            .transpose()
            .map_err(|e| {
    ...
    fn try_handle_request(
        ...
        let stream_res =
            node.with_node_state(|_network, _sortdb, chainstate, _mempool, _rpc_args| {
                ...
                let Some(nakamoto_header) = header.anchored_header.as_funai_nakamoto() else {
                    return Err(ChainError::NoSuchBlockError);
                };
