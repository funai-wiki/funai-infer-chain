...
use crate::chainstate::funai::{Error as ChainError, FunaiTransaction};
...
impl HttpChunkGenerator for FunaiMemPoolStream {
...
impl RPCRequestHandler for RPCMempoolQueryRequestHandler {
    ...
    fn try_handle_request(
        ...
        let stream_res = node.with_node_state(|network, sortdb, chainstate, mempool, _rpc_args| {
            let height = self.get_funai_chain_tip(&preamble, sortdb, chainstate).map(|hdr| hdr.anchored_header.height()).unwrap_or(0);
            ...
            let mempool_db = match mempool.reopen(false) {
                Ok(db) => db,
                Err(e) => {
                    return Err(FunaiHttpResponse::new_error(&preamble, &HttpServerError::new(format!("Failed to open mempool DB: {:?}", &e))));
                }
            };

            Ok(FunaiMemPoolStream::new(mempool_db, mempool_query, max_txs, height, page_id))
        });
    ...
    pub fn new_mempool_query(
        ...
        FunaiHttpRequest::new_for_peer(
            ...
            if let Some(page_id) = page_id_opt {
                ...
                HttpRequestContents::new().payload_funai(&query)
            },
        )
...
impl FunaiHttpResponse {
    ...
    pub fn decode_mempool_txs_page(
        self,
    ) -> Result<(Vec<FunaiTransaction>, Option<Txid>), NetError> {
