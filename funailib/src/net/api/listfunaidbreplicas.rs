...
use crate::net::{Error as NetError, NeighborAddress, FunaiNodeState, TipRequest, MAX_HEADERS};
...
#[derive(Clone)]
pub struct RPCListFunaiDBReplicasRequestHandler {
    pub contract_identifier: Option<QualifiedContractIdentifier>,
}

impl RPCListFunaiDBReplicasRequestHandler {
    pub fn new() -> Self {
        Self {
...
impl HttpRequest for RPCListFunaiDBReplicasRequestHandler {
    ...
    fn path_regex(&self) -> Regex {
        Regex::new(&format!(
            r#"^/v2/funaidb/(?P<address>{})/(?P<contract>{})/replicas$"#,
...
impl RPCRequestHandler for RPCListFunaiDBReplicasRequestHandler {
    ...
    fn try_handle_request(
        ...
        let (replicas_resp, local_peer, allow_private) =
            node.with_node_state(|network, _sortdb, _chainstate, _mempool, _rpc_args| {
                let replicas_resp = PeerDB::find_funai_db_replicas(
                    network.peerdb_conn(),
                ...
                .map_err(|e| {
                    warn!("Failed to find funaidb replicas"; "contract_id" => %contract_identifier, "error" => %e);
                    FunaiHttpResponse::new_error(
                        &preamble,
                        &HttpServerError::new("Unable to list replicas of FunaiDB".to_string())
                    )
                });
...
impl HttpResponse for RPCListFunaiDBReplicasRequestHandler {
...
impl FunaiHttpRequest {
    pub fn new_list_funaidb_replicas(
        host: PeerHost,
...
impl FunaiHttpResponse {
    ...
    pub fn decode_funaidb_replicas(self) -> Result<Vec<NeighborAddress>, NetError> {
