...
use crate::net::httpcore::{
    HttpPreambleExtensions, RPCRequestHandler, FunaiHttp, FunaiHttpRequest, FunaiHttpResponse,
};
use crate::net::p2p::PeerNetwork;
use crate::net::{Error as NetError, NeighborKey, FunaiNodeState, MAX_NEIGHBORS_DATA_LEN};
...
pub struct RPCNeighbor {
    ...
    pub funaidbs: Option<Vec<QualifiedContractIdentifier>>,
}
...
impl RPCNeighbor {
    pub fn from_neighbor_key_and_pubkh(
        ...
        pkh: Hash160,
        auth: bool,
        funaidbs: Vec<QualifiedContractIdentifier>,
    ) -> RPCNeighbor {
        RPCNeighbor {
        ...
        let bootstrap = bootstrap_nodes
            ...
            .map(|n| {
                let funaidb_contract_ids =
                    PeerDB::static_get_peer_funai_dbs(peerdb_conn, &n).unwrap_or(vec![]);
        ...
        for event_id in network.iter_peer_event_ids() {
            ...
            if convo.is_outbound() {
                outbound.push(RPCNeighbor::from_neighbor_key_and_pubkh(
                    ...
                    convo.get_funaidb_contract_ids().to_vec(),
                ));
            } else {
...
impl FunaiHttpRequest {
    ...
    pub fn new_getneighbors(host: PeerHost) -> FunaiHttpRequest {
