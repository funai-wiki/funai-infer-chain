...
use crate::chainstate::funai::{
    FunaiBlock, FunaiBlockHeader, FunaiTransaction, TransactionPayload,
};
...
pub struct FunaiBlockAcceptedData {
    pub funai_block_id: FunaiBlockId,
    pub accepted: bool,
}

#[derive(Clone)]
pub struct RPCPostBlockRequestHandler {
    pub block: Option<FunaiBlock>,
...
impl RPCPostBlockRequestHandler {
    ...
    fn parse_postblock_octets(mut body: &[u8]) -> Result<FunaiBlock, Error> {
        let block = FunaiBlock::consensus_deserialize(&mut body).map_err(|e| {
...
impl RPCRequestHandler for RPCPostBlockRequestHandler {
    ...
    fn try_handle_request(
        ...
        let data_resp = match data_resp {
            Ok(accepted) => FunaiBlockAcceptedData {
                accepted,
                funai_block_id: FunaiBlockHeader::make_index_block_hash(
                    &consensus_hash,
                    &block_hash,
                ),
            },
        ...
        if data_resp.accepted {
            node.set_relay_message(FunaiMessageType::Blocks(BlocksData {
...
impl HttpResponse for RPCPostBlockRequestHandler {
    fn try_parse_response(
        ...
        let accepted: FunaiBlockAcceptedData = parse_json(preamble, body)?;
...
impl FunaiHttpRequest {
    ...
    pub fn new_post_block(
        ...
        FunaiHttpRequest::new_for_peer(
            ...
            HttpRequestContents::new().payload_funai(&block),
        )
...
impl FunaiHttpResponse {
    pub fn decode_funai_block_accepted(self) -> Result<FunaiBlockAcceptedData, NetError> {
        let contents = self.get_http_payload_ok()?;
        let response_json: serde_json::Value = contents.try_into()?;
        let result: FunaiBlockAcceptedData = serde_json::from_value(response_json)
