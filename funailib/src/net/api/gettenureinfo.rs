...
pub struct RPCGetTenureInfo {
    ...
    pub parent_tenure_start_block_id: FunaiBlockId,
    /// The highest Funai block ID in the current tenure
    pub tip_block_id: FunaiBlockId,
...
impl RPCRequestHandler for RPCNakamotoTenureInfoRequestHandler {
    ...
    fn try_handle_request(
        ...
        let info = node.with_node_state(|network, _sortdb, _chainstate, _mempool, _rpc_args| {
            RPCGetTenureInfo {
                consensus_hash: network.funai_tip.0.clone(),
                tenure_start_block_id: network.tenure_start_block_id.clone(),
                parent_consensus_hash: network.parent_funai_tip.0.clone(),
                parent_tenure_start_block_id: FunaiBlockId::new(
                    &network.parent_funai_tip.0,
                    &network.parent_funai_tip.1,
                ),
                tip_block_id: FunaiBlockId::new(&network.funai_tip.0, &network.funai_tip.1),
                tip_height: network.funai_tip.2,
...
impl FunaiHttpRequest {
    ...
    pub fn new_get_nakamoto_tenure_info(host: PeerHost) -> FunaiHttpRequest {
