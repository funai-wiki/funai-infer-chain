...
use libfunaidb::{
    SlotMetadata, FunaiDBChunkAckData, FunaiDBChunkData, STACKERDB_MAX_CHUNK_SIZE,
};
...
use crate::net::{
    Error as NetError, FunaiDBPushChunkData, FunaiMessageType, FunaiNodeState, TipRequest,
};
use crate::util_lib::db::{DBConn, Error as DBError};

#[derive(Clone)]
pub struct RPCPostFunaiDBChunkRequestHandler {
    pub contract_identifier: Option<QualifiedContractIdentifier>,
    pub chunk: Option<FunaiDBChunkData>,
}
impl RPCPostFunaiDBChunkRequestHandler {
    pub fn new() -> Self {
        Self {
...
impl HttpRequest for RPCPostFunaiDBChunkRequestHandler {
    ...
    fn path_regex(&self) -> Regex {
        Regex::new(&format!(
            r#"^/v2/funaidb/(?P<address>{})/(?P<contract>{})/chunks$"#,
    ...
    fn try_parse_request(
        ...
        if preamble.get_content_length() > MAX_MESSAGE_LEN {
            return Err(Error::DecodeError(
                "Invalid Http request: PostFunaiDBChunk body is too big".to_string(),
            ));
        }

        let contract_identifier = request::get_contract_address(captures, "address", "contract")?;
        let chunk: FunaiDBChunkData = serde_json::from_slice(body).map_err(Error::JsonError)?;
...
pub enum FunaiDBErrorCodes {
    DataAlreadyExists,
    NoSuchSlot,
    BadSigner,
}

impl FunaiDBErrorCodes {
    ...
    pub fn reason(&self) -> &'static str {
        match self {
            ...
            Self::NoSuchSlot => "No such FunaiDB slot",
...
impl RPCRequestHandler for RPCPostFunaiDBChunkRequestHandler {
    ...
    fn try_handle_request(
        ...
        let ack_resp =
            node.with_node_state(|network, _sortdb, _chainstate, _mempool, _rpc_args| {
                let tx = if let Ok(tx) = network.funaidbs_tx_begin(&contract_identifier) {
                    tx
                } else {
                    return Err(FunaiHttpResponse::new_error(
                        &preamble,
                        &HttpNotFound::new("FunaiDB not found".to_string()),
                    ));
                };
                if let Err(_e) = tx.get_funaidb_id(&contract_identifier) {
                ...
                if let Err(e) = tx.try_replace_chunk(
                    &contract_identifier,
                    &funaidb_chunk.get_slot_metadata(),
                    &funaidb_chunk.data,
                ) {
                    test_debug!(
                        "Failed to replace chunk {}.{} in {}: {:?}",
                        funaidb_chunk.slot_id,
                        funaidb_chunk.slot_version,
                        &contract_identifier,
                        &e
                    );
                    let slot_metadata_opt =
                        match tx.get_slot_metadata(&contract_identifier, funaidb_chunk.slot_id) {
                            Ok(slot_opt) => slot_opt,
                            Err(e) => {
                                // some other error
                                error!("Failed to load replaced FunaiDB chunk metadata";
                                ...
                                return Err(FunaiHttpResponse::new_error(
                                    ...
                                    &HttpServerError::new(format!(
                                        "Failed to load FunaiDB chunk for {}: {:?}",
                    ...
                    let err_code = if slot_metadata_opt.is_some() {
                        if let NetError::BadSlotSigner(..) = e {
                            FunaiDBErrorCodes::BadSigner
                        } else {
                            FunaiDBErrorCodes::DataAlreadyExists
                        }
                    } else {
                        FunaiDBErrorCodes::NoSuchSlot
                    };
                ...
                let slot_metadata = if let Ok(Some(md)) =
                    tx.get_slot_metadata(&contract_identifier, funaidb_chunk.slot_id)
                {
                    md
                } else {
                    return Err(FunaiHttpResponse::new_error(
                ...
                if let Err(e) = tx.commit() {
                    return Err(FunaiHttpResponse::new_error(
                        &preamble,
                        &HttpServerError::new(format!("Failed to commit FunaiDB tx: {:?}", &e)),
                    ));
                }

                debug!(
                    "Wrote {}-byte chunk to {} slot {} version {}",
                    &funaidb_chunk.data.len(),
                    &contract_identifier,
                    funaidb_chunk.slot_id,
                    funaidb_chunk.slot_version
                );
        ...
        if ack_resp.accepted {
            let push_chunk_data = FunaiDBPushChunkData {
                ...
                chunk_data: funaidb_chunk,
            };
            node.set_relay_message(FunaiMessageType::FunaiDBPushChunk(push_chunk_data));
        }
...
impl HttpResponse for RPCPostFunaiDBChunkRequestHandler {
    ...
    fn try_parse_response(
        ...
        let ack: FunaiDBChunkAckData = parse_json(preamble, body)?;
        Ok(HttpResponsePayload::try_from_json(ack)?)
    }
}

impl FunaiHttpRequest {
    pub fn new_post_funaidb_chunk(
        host: PeerHost,
        ...
        FunaiHttpRequest::new_for_peer(
            ...
            HttpRequestContents::new().payload_json(
                serde_json::to_value(FunaiDBChunkData {
                    slot_id,
                    slot_version,
                    sig,
                    data,
...
impl FunaiHttpResponse {
    ...
    pub fn decode_funaidb_chunk_ack(self) -> Result<FunaiDBChunkAckData, NetError> {
        let contents = self.get_http_payload_ok()?;
        let response_json: serde_json::Value = contents.try_into()?;
        let data: FunaiDBChunkAckData = serde_json::from_value(response_json)
