...
use funai_common::util::hash::{bin_bytes, hex_bytes, to_bin, to_hex, Hash160};
...
/// These instances are initially written as a new Funai block is processed, and marked
...
impl FromRow<AttachmentInstance> for AttachmentInstance {
    fn from_row<'a>(row: &'a Row) -> Result<AttachmentInstance, db_error> {
        ...
        let index_block_hash = FunaiBlockId::from_column(row, "index_block_hash")?;
    ...
    fn insert_attachment_instance(
        ...
        sql_tx.execute(
            ...
            rusqlite::params![
                ...
                &attachment.index_block_hash,
                &attachment.attachment_index,
                &u64_to_sql(attachment.funai_block_height)?,
                &is_available,
                &attachment.metadata,
                &attachment.contract_id.to_string(),
                &attachment.tx_id,
