...
use funai_common::util::hash::{Hash160, MerkleHashFunc};
...
impl AttachmentsDownloader {
    ...
    {
        let mut attachments_batches: HashMap<FunaiBlockId, AttachmentsBatch> = HashMap::new();
        let mut resolved_attachments = vec![];
...
pub struct AttachmentsBatchStateContext {
    ...
    pub inventories: HashMap<
        (QualifiedContractIdentifier, Vec<u32>, FunaiBlockId),
            ...
            for (peer_url, reliability_report) in self.peers.iter() {
                for pages in pages_batches.iter() {
                    let request = AttachmentsInventoryRequest {
                        ...
                        pages: pages.clone(),
                        funai_block_height: self.attachments_batch.funai_block_height,
                        index_block_hash: self.attachments_batch.index_block_hash,
                        canonical_funai_tip_height: self
                            .attachments_batch
        ...
        for ((contract_id, pages, _), peers_responses) in self.inventories.iter() {
            ...
            for (attachment_index, content_hash) in missing_attachments.iter() {
                ...
                let request = AttachmentRequest {
                    ...
                    funai_block_height: self.attachments_batch.funai_block_height,
                    canonical_funai_tip_height: self.attachments_batch.canonical_funai_tip_height,
                };
...
pub struct BatchedRequestsResult<T: Requestable> {
    ...
    pub succeeded: HashMap<T, Option<FunaiHttpResponse>>,
...
pub struct AttachmentsInventoryRequest {
    ...
    pub canonical_funai_tip_height: Option<u64>,
}
...
impl Hash for AttachmentsInventoryRequest {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        self.funai_block_height.hash(state);
    }
}

impl AttachmentsInventoryRequest {
    pub fn key(&self) -> (QualifiedContractIdentifier, Vec<u32>, FunaiBlockId) {
        (
...
impl AttachmentsBatch {
    pub fn new() -> AttachmentsBatch {
        AttachmentsBatch {
            ...
            index_block_hash: FunaiBlockId([0u8; 32]),
    ...
    pub fn track_attachment(&mut self, attachment: &AttachmentInstance) {
        if self.attachments_instances.is_empty() {
            self.funai_block_height = attachment.funai_block_height.clone();
            self.index_block_hash = attachment.index_block_hash.clone();
            self.canonical_funai_tip_height = attachment.canonical_funai_tip_height;
        } else {
            if self.funai_block_height != attachment.funai_block_height
...
impl Ord for AttachmentsBatch {
    fn cmp(&self, other: &AttachmentsBatch) -> Ordering {
        other
            ...
            .then_with(|| other.funai_block_height.cmp(&self.funai_block_height))
    }
}
