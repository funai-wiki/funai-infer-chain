...
use crate::chainstate::funai::db::blocks::{CheckError, MINIMUM_TX_FEE_RATE_PER_BYTE};
use crate::chainstate::funai::db::FunaiChainState;
use crate::chainstate::funai::{Error as chain_error, FunaiBlockHeader, *};
...
use crate::net::httpcore::{
    FunaiHttp, FunaiHttpMessage, FunaiHttpRequest, FunaiHttpResponse, HTTP_REQUEST_ID_RESERVED,
};
use crate::net::p2p::{PeerMap, PeerNetwork};
use crate::net::relay::Relayer;
use crate::net::funaidb::{FunaiDBTx, FunaiDBs};
use crate::net::{Error as net_error, FunaiMessageType, FunaiNodeState};
...
pub struct ConversationHttp {
    ...
    connection_time: u64,
    /// funai canonical chain tip that this peer reported
    canonical_funai_tip_height: Option<u32>,
    /// Ongoing replies
    reply_streams: VecDeque<(ReplyHandleHttp, HttpResponseContents, bool)>,
    /// outstanding request
    pending_request: Option<ReplyHandleHttp>,
    /// outstanding response
    pending_response: Option<FunaiHttpResponse>,
...
impl ConversationHttp {
    pub fn new(
        ...
        let funai_http = FunaiHttp::new(peer_addr.clone(), conn_opts);
        ConversationHttp {
            connection: ConnectionHttp::new(funai_http, conn_opts, None),
            conn_id,
            ...
            outbound_url,
            peer_host,
            canonical_funai_tip_height: None,
    ...
    fn start_request(&mut self, req: FunaiHttpRequest) -> Result<ReplyHandleHttp, net_error> {
        test_debug!(
        ...
        let funai_msg = FunaiHttpMessage::Request(req);
        self.connection.send_message(&mut handle, &funai_msg)?;
        Ok(handle)
    }
    ...
    pub fn send_request(&mut self, req: FunaiHttpRequest) -> Result<(), net_error> {
    ...
    pub fn reply_error(&mut self, res: FunaiHttpResponse) -> Result<(), net_error> {
    ...
    /// Returns a FunaiMessageType option -- it's Some(...) if we need to forward a message to the
    /// peer network (like a transaction or a block or microblock)
    pub fn handle_request(
        &mut self,
        req: FunaiHttpRequest,
        node: &mut FunaiNodeState,
    ) -> Result<Option<FunaiMessageType>, net_error> {
    ...
    /// If we finish doing so, then extract the FunaiHttpResponse
    /// If we are not done yet, then return Ok(reply-handle) if we can try again, or net_error if
    /// we cannot.
    fn try_send_recv_response(
        req: ReplyHandleHttp,
    ) -> Result<FunaiHttpResponse, Result<ReplyHandleHttp, net_error>> {
        match req.try_send_recv() {
            Ok(message) => match message {
                FunaiHttpMessage::Request(_) => {
                    warn!("Received response: not a HTTP response");
                    return Err(Err(net_error::InvalidMessage));
                }
                FunaiHttpMessage::Response(http_response) => Ok(http_response),
                FunaiHttpMessage::Error(_, http_response) => Ok(http_response),
            },
            Err(res) => Err(res),
        }
    }
    ...
    pub fn try_get_response(&mut self) -> Option<FunaiHttpResponse> {
    ...
    pub fn chat(
        &mut self,
        node: &mut FunaiNodeState,
    ) -> Result<Vec<FunaiMessageType>, net_error> {
        ...
        for _i in 0..num_inbound {
            ...
            match msg {
                ...
                FunaiHttpMessage::Error(path, resp) => {
                    ...
                    info!("Handled FunaiHTTPRequest Error"; "path" => %path, "processing_time_ms" => start_time.elapsed().as_millis(), "conn_id" => self.conn_id, "peer_addr" => &self.peer_addr);
                }
                FunaiHttpMessage::Response(resp) => {
                    // Is there someone else waiting for this message?  If so, pass it along.
                    // (this _should_ be our pending_request handle)
                    match self
                        .connection
                        .fulfill_request(FunaiHttpMessage::Response(resp))
                    {
                        None => {
