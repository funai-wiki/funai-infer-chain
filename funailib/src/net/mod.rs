...
use libfunaidb::{
    Error as libfunaidb_error, SlotMetadata, FunaiDBChunkAckData, FunaiDBChunkData,
};
...
use funai_common::codec::{
    read_next, write_next, Error as codec_error, FunaiMessageCodec,
    BURNCHAIN_HEADER_HASH_ENCODED_SIZE,
};
use funai_common::types::chainstate::{
    BlockHeaderHash, BurnchainHeaderHash, PoxId, FunaiAddress, FunaiBlockId,
};
use funai_common::types::net::{Error as AddrError, PeerAddress, PeerHost};
...
use crate::chainstate::funai::db::blocks::MemPoolRejection;
use crate::chainstate::funai::db::FunaiChainState;
use crate::chainstate::funai::index::Error as marf_error;
use crate::chainstate::funai::{
    Error as chainstate_error, Error as chain_error, FunaiBlock, FunaiBlockHeader,
    FunaiMicroblock, FunaiPublicKey, FunaiTransaction, TransactionPayload,
};
use crate::clarity_vm::clarity::Error as clarity_error;
use crate::core::mempool::*;
use crate::core::{FunaiEpoch, POX_REWARD_CYCLE_LENGTH};
...
use crate::net::httpcore::{
    HttpRequestContentsExtensions, FunaiHttp, FunaiHttpRequest, FunaiHttpResponse, TipRequest,
};
...
/// the node to recieve `FunaiMessage` instances. The downstream consumer of this API is `PeerNetwork`.
...
pub mod chat;
/// Implements serialization and deserialization for `FunaiMessage` types.
...
use crate::net::funaidb::{FunaiDBConfig, FunaiDBSync, FunaiDBSyncResult, FunaiDBs};
...
pub enum Error {
    ...
    /// no such DB
    NoSuchFunaiDB(QualifiedContractIdentifier),
    /// funai DB exists
    FunaiDBExists(QualifiedContractIdentifier),
    /// slot signer is wrong
    BadSlotSigner(FunaiAddress, u32),
    ...
    /// Invalid control smart contract for a Funai DB
    InvalidFunaiDBContract(QualifiedContractIdentifier, String),
    /// state machine step took too long
    StepTimeout,
    /// funai DB chunk is too big
    FunaiDBChunkTooBig(usize),
    /// HTTP error
    Http(HttpErr),
...
impl From<libfunaidb_error> for Error {
    fn from(e: libfunaidb_error) -> Self {
        match e {
            libfunaidb_error::SigningError(s) => Error::SigningError(s),
            libfunaidb_error::VerifyingError(s) => Error::VerifyingError(s),
        }
    }
}
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Error::NoSuchFunaiDB(ref addr) => {
                write!(f, "No such FunaiDB {}", addr)
            }
            Error::FunaiDBExists(ref addr) => {
                write!(f, "FunaiDB already exists: {}", addr)
            }
            ...
            Error::InvalidFunaiDBContract(ref contract_id, ref reason) => {
                write!(
                    f,
                    "Invalid FunaiDB control smart contract {}: {}",
                    contract_id, reason
                )
            }
            Error::StepTimeout => write!(f, "State-machine step took too long"),
            Error::FunaiDBChunkTooBig(ref sz) => {
                write!(f, "FunaiDB chunk size is too big ({})", sz)
            }
...
impl error::Error for Error {
    fn cause(&self) -> Option<&dyn error::Error> {
        match *self {
            ...
            Error::NoSuchFunaiDB(..) => None,
            Error::FunaiDBExists(..) => None,
            Error::BadSlotSigner(..) => None,
            Error::TooManySlotWrites { .. } => None,
            Error::TooFrequentSlotWrites(..) => None,
            Error::InvalidFunaiDBContract(..) => None,
            Error::StepTimeout => None,
            Error::FunaiDBChunkTooBig(..) => None,
...
impl From<chain_error> for Error {
    fn from(e: chain_error) -> Error {
        match e {
            chain_error::InvalidFunaiBlock(s) => {
                Error::ChainstateError(format!("Invalid funai block: {}", s))
            }
            chain_error::InvalidFunaiMicroblock(msg, hash) => {
                Error::ChainstateError(format!("Invalid funai microblock {:?}: {}", hash, msg))
            }
            chain_error::InvalidFunaiTransaction(s, _) => {
                Error::ChainstateError(format!("Invalid funai transaction: {}", s))
            }
            ...
            _ => Error::ChainstateError(format!("Funai chainstate error: {:?}", &e)),
        }
    }
}
...
/// Wrapper around Funai chainstate data that an HTTP request handler might need
pub struct FunaiNodeState<'a> {
    inner_network: Option<&'a mut PeerNetwork>,
    inner_sortdb: Option<&'a SortitionDB>,
    inner_chainstate: Option<&'a mut FunaiChainState>,
    inner_mempool: Option<&'a mut MemPoolDB>,
    inner_rpc_args: Option<&'a RPCHandlerArgs<'a>>,
    relay_message: Option<FunaiMessageType>,
}

impl<'a> FunaiNodeState<'a> {
    pub fn new(
        inner_network: &'a mut PeerNetwork,
        inner_sortdb: &'a SortitionDB,
        inner_chainstate: &'a mut FunaiChainState,
    ...
    pub fn canonical_funai_tip_height(&mut self) -> u32 {
        self.with_node_state(|network, _, _, _, _| {
            network.burnchain_tip.canonical_funai_tip_height as u32
        })
    }

    pub fn set_relay_message(&mut self, msg: FunaiMessageType) {
        self.relay_message = Some(msg);
    }

    pub fn take_relay_message(&mut self) -> Option<FunaiMessageType> {
        self.relay_message.take()
    }

    /// Load up the canonical Funai chain tip.  Note that this is subject to both burn chain block
    /// Funai block availability -- different nodes with different partial replicas of the Funai chain state
    ...
    pub fn load_funai_chain_tip(
        &mut self,
        preamble: &HttpRequestPreamble,
        contents: &HttpRequestContents,
    ) -> Result<FunaiBlockId, FunaiHttpResponse> {
                ...
                TipRequest::UseLatestUnconfirmedTip => {
                    ...
                    if let Some(unconfirmed_chain_tip) = unconfirmed_chain_tip_opt {
                        ...
                        match NakamotoChainState::get_canonical_block_header(
                            ...
                            Ok(Some(tip)) => Ok(FunaiBlockId::new(
                                &tip.consensus_hash,
                    ...
                    match NakamotoChainState::get_canonical_block_header(chainstate.db(), sortdb) {
                        ...
                        Ok(None) => {
                            return Err(FunaiHttpResponse::new_error(
                                preamble,
                                &HttpNotFound::new(
                                    "No funai chain tip exists at this point in time.".to_string(),
                                ),
                            ));
                        }
                        Err(e) => {
...
/// This struct is used only in Funai 2.x for Funai 2.x inventories
#[derive(Debug, Clone, PartialEq)]
...
/// chain block range.  Sent in reply to a GetBlocksInv for Funai 2.x block data.
#[derive(Debug, Clone, PartialEq)]
...
#[derive(Debug, Clone, PartialEq)]
pub struct BlocksDatum(pub ConsensusHash, pub FunaiBlock);
...
pub struct MicroblocksData {
    pub index_anchor_block: FunaiBlockId,
    pub microblocks: Vec<FunaiMicroblock>,
}
...
pub struct HandshakeData {
    ...
    pub node_public_key: FunaiPublicKeyBuffer,
...
/// Inform the remote peer of (a page of) the list of funai DB contracts this node supports
#[derive(Debug, Clone, PartialEq)]
pub struct FunaiDBHandshakeData {
    /// current reward cycle consensus hash (i.e. the consensus hash of the Funai tip in the
    /// current reward cycle, which commits to both the Funai block tip and the underlying PoX
    /// history).
...
#[derive(Debug, Clone, PartialEq)]
pub struct FunaiDBGetChunkInvData {
    /// smart contract being used to determine chunk quantity and order
    pub contract_id: QualifiedContractIdentifier,
    /// consensus hash of the Funai chain tip in this reward cycle
    pub rc_consensus_hash: ConsensusHash,
}

/// Inventory bitvector for chunks this node contains
#[derive(Debug, Clone, PartialEq)]
pub struct FunaiDBChunkInvData {
...
/// Request for a funai DB chunk.
#[derive(Debug, Clone, PartialEq)]
pub struct FunaiDBGetChunkData {
...
/// Funai DB chunk push
#[derive(Debug, Clone, PartialEq)]
pub struct FunaiDBPushChunkData {
    /// smart contract being used to determine chunk quantity and order
    pub contract_id: QualifiedContractIdentifier,
    /// consensus hash of the Funai chain tip in this reward cycle
    pub rc_consensus_hash: ConsensusHash,
    /// the pushed chunk
    pub chunk_data: FunaiDBChunkData,
}
...
pub enum FunaiMessageType {
    ...
    // funai DB
    FunaiDBHandshakeAccept(HandshakeAcceptData, FunaiDBHandshakeData),
    FunaiDBGetChunkInv(FunaiDBGetChunkInvData),
    FunaiDBChunkInv(FunaiDBChunkInvData),
    FunaiDBGetChunk(FunaiDBGetChunkData),
    FunaiDBChunk(FunaiDBChunkData),
    FunaiDBPushChunk(FunaiDBPushChunkData),
    // Nakamoto-specific
...
pub enum FunaiMessageID {
    ...
    NatPunchReply = 18,
    // funaidb
    FunaiDBHandshakeAccept = 19,
    FunaiDBGetChunkInv = 21,
    FunaiDBChunkInv = 22,
    FunaiDBGetChunk = 23,
    FunaiDBChunk = 24,
    FunaiDBPushChunk = 25,
...
/// Message type for all P2P Funai network messages
#[derive(Debug, Clone, PartialEq)]
pub struct FunaiMessage {
    pub preamble: Preamble,
    pub relayers: Vec<RelayData>,
    pub payload: FunaiMessageType,
}
...
pub trait ProtocolFamily {
    type Preamble: FunaiMessageCodec + Send + Sync + Clone + PartialEq + std::fmt::Debug;
...
pub struct NetworkResult {
    ...
    pub unhandled_messages: HashMap<NeighborKey, Vec<FunaiMessage>>,
    /// Funai 2.x blocks we downloaded, and time taken
    pub blocks: Vec<(ConsensusHash, FunaiBlock, u64)>,
    /// Funai 2.x confiremd microblocks we downloaded, and time taken
    pub confirmed_microblocks: Vec<(ConsensusHash, Vec<FunaiMicroblock>, u64)>,
    /// Nakamoto blocks we downloaded
    pub nakamoto_blocks: HashMap<FunaiBlockId, NakamotoBlock>,
    /// all transactions pushed to us and their message relay hints
    pub pushed_transactions: HashMap<NeighborKey, Vec<(Vec<RelayData>, FunaiTransaction)>>,
    /// all Funai 2.x blocks pushed to us
    pub pushed_blocks: HashMap<NeighborKey, Vec<BlocksData>>,
    /// all Funai 2.x microblocks pushed to us, and the relay hints from the message
    pub pushed_microblocks: HashMap<NeighborKey, Vec<(Vec<RelayData>, MicroblocksData)>>,
    /// transactions sent to us by the http server
    pub uploaded_transactions: Vec<FunaiTransaction>,
    ...
    pub uploaded_funaidb_chunks: Vec<FunaiDBPushChunkData>,
    ...
    pub synced_transactions: Vec<FunaiTransaction>,
    /// chunks for funai DBs we downloaded
    pub funai_db_sync_results: Vec<FunaiDBSyncResult>,
    /// Number of times the network state machine has completed one pass
    pub num_state_machine_passes: u64,
    /// Number of times the Funai 2.x inventory synchronization has completed one pass
    pub num_inv_sync_passes: u64,
    /// Number of times the Funai 2.x block downloader has completed one pass
    pub num_download_passes: u64,
    /// The observed burnchain height
    pub burn_height: u64,
    /// The consensus hash of the start of this reward cycle
    pub rc_consensus_hash: ConsensusHash,
    /// The current FunaiDB configs
    pub funai_db_configs: HashMap<QualifiedContractIdentifier, FunaiDBConfig>,
}
...
impl NetworkResult {
    pub fn new(
        ...
        NetworkResult {
            ...
            uploaded_funaidb_chunks: vec![],
            attachments: vec![],
            synced_transactions: vec![],
            funai_db_sync_results: vec![],
    ...
    pub fn has_funaidb_chunks(&self) -> bool {
        self.funai_db_sync_results
            .iter()
            .fold(0, |acc, x| acc + x.chunks_to_store.len())
            > 0
            || self.uploaded_funaidb_chunks.len() > 0
    }

    pub fn transactions(&self) -> Vec<FunaiTransaction> {
    ...
    pub fn has_data_to_store(&self) -> bool {
        self.has_blocks()
            ...
            || self.has_funaidb_chunks()
    }

    pub fn consume_unsolicited(
        &mut self,
        unhandled_messages: HashMap<NeighborKey, Vec<FunaiMessage>>,
    ) -> () {
        for (neighbor_key, messages) in unhandled_messages.into_iter() {
            for message in messages.into_iter() {
                match message.payload {
                    FunaiMessageType::Blocks(block_data) => {
                    ...
                    FunaiMessageType::Microblocks(mblock_data) => {
                    ...
                    FunaiMessageType::Transaction(tx_data) => {
    ...
    pub fn consume_http_uploads(&mut self, mut msgs: Vec<FunaiMessageType>) -> () {
        for msg in msgs.drain(..) {
            match msg {
                ...
                FunaiMessageType::FunaiDBPushChunk(chunk_data) => {
                    self.uploaded_funaidb_chunks.push(chunk_data);
                }
                _ => {
                    // drop
    ...
    pub fn consume_funai_db_sync_results(&mut self, mut msgs: Vec<FunaiDBSyncResult>) {
        self.funai_db_sync_results.append(&mut msgs);
    }
...
pub trait Requestable: std::fmt::Display {
    ...
    fn make_request_type(&self, peer_host: PeerHost) -> FunaiHttpRequest;
}
...
pub mod test {
    ...
    use funai_common::deps_common::bitcoin::network::serialize::BitcoinHash;
    use funai_common::types::chainstate::TrieHash;
    use funai_common::types::FunaiEpochId;
    use funai_common::util::get_epoch_time_secs;
    use funai_common::util::hash::*;
    use funai_common::util::secp256k1::*;
    use funai_common::util::uint::*;
    ...
    use crate::chainstate::nakamoto::tests::node::TestFunai;
    use crate::chainstate::funai::address::PoxAddress;
    use crate::chainstate::funai::boot::test::get_parent_tip;
    use crate::chainstate::funai::boot::*;
    use crate::chainstate::funai::db::accounts::MinerReward;
    use crate::chainstate::funai::db::{FunaiChainState, *};
    use crate::chainstate::funai::events::{FunaiBlockEventData, FunaiTransactionReceipt};
    use crate::chainstate::funai::miner::*;
    use crate::chainstate::funai::tests::chain_histories::mine_smart_contract_block_contract_call_microblock;
    use crate::chainstate::funai::tests::*;
    use crate::chainstate::funai::{FunaiMicroblockHeader, *};
    use crate::chainstate::*;
    use crate::clarity::vm::clarity::TransactionConnection;
    use crate::core::{FunaiEpoch, FunaiEpochExtension, NETWORK_P2P_PORT};
    use crate::net::asn::*;
    use crate::net::atlas::*;
    use crate::net::chat::*;
    use crate::net::codec::*;
    ...
    impl FunaiMessageCodec for BlockstackOperationType {
    ...
    pub struct TestEventObserverBlock {
        pub block: FunaiBlockEventData,
        pub metadata: FunaiHeaderInfo,
        pub receipts: Vec<FunaiTransactionReceipt>,
        pub parent: FunaiBlockId,
    ...
    impl BlockEventDispatcher for TestEventObserver {
        fn announce_block(
            ...
            receipts: &[events::FunaiTransactionReceipt],
            parent: &FunaiBlockId,
            winner_txid: Txid,
    ...
    pub struct TestPeerConfig {
        ...
        pub epochs: Option<Vec<FunaiEpoch>>,
        ...
        /// Which funai DBs will this peer replicate?
        pub funai_dbs: Vec<QualifiedContractIdentifier>,
        /// Funai DB configurations for each funai_dbs entry above, if different from
        /// FunaiDBConfig::noop()
        pub funai_db_configs: Vec<Option<FunaiDBConfig>>,
        /// What services should this peer support?
        pub services: u16,
        /// aggregate public key to use
        pub aggregate_public_key: Option<Point>,
        pub test_funais: Option<Vec<TestFunai>>,
    ...
    impl TestPeerConfig {
        ...
        pub fn get_funai_db_configs(
            &self,
        ) -> HashMap<QualifiedContractIdentifier, FunaiDBConfig> {
            let mut ret = HashMap::new();
            for (contract_id, config_opt) in
                self.funai_dbs.iter().zip(self.funai_db_configs.iter())
            {
                if let Some(config) = config_opt {
                    ret.insert(contract_id.clone(), config.clone());
                } else {
                    ret.insert(contract_id.clone(), FunaiDBConfig::noop());
                }
            }
            ret
        }
        ...
        pub fn add_funai_db(
            ...
            self.funai_dbs.push(contract_id);
            self.funai_db_configs.push(Some(config));
        }
    }
    ...
    pub struct TestPeer<'a> {
        ...
        pub miner: TestMiner,
        pub funai_node: Option<TestFunaiNode>,
        pub relayer: Relayer,
    ...
    impl<'a> TestPeer<'a> {
        ...
        pub fn test_path(config: &TestPeerConfig) -> String {
            ...
            format!(
                "/tmp/funai-node-tests/units-test-peer/{}-{}",
                &config.test_name, random_bytes
            )
        }

        pub fn funaidb_path(config: &TestPeerConfig) -> String {
            format!("{}/funai_db.sqlite", &Self::test_path(config))
        }
        ...
        fn init_funaidb_syncs(
            root_path: &str,
            peerdb: &PeerDB,
            funai_dbs: &mut HashMap<QualifiedContractIdentifier, FunaiDBConfig>,
        ) -> HashMap<QualifiedContractIdentifier, (FunaiDBConfig, FunaiDBSync<PeerNetworkComms>)>
        {
            let funaidb_path = format!("{}/funai_db.sqlite", root_path);
            let mut funai_db_syncs = HashMap::new();
            let local_peer = PeerDB::get_local_peer(peerdb.conn()).unwrap();
            for (i, (contract_id, db_config)) in funai_dbs.iter_mut().enumerate() {
                let initial_peers = PeerDB::find_funai_db_replicas(
                    peerdb.conn(),
                ...
                let funai_dbs = FunaiDBs::connect(&funaidb_path, true).unwrap();
                ...
                funai_db_syncs.insert(contract_id.clone(), (db_config.clone(), funai_db_sync));
            }
            funai_db_syncs
        }
        ...
        pub fn neighbor_with_observer(
            ...
            config.test_name = format!(
                ...
                Hash160::from_node_public_key(&FunaiPublicKey::from_private(
                    &self.config.private_key
                ))
            );
            config.server_port = 0;
            config.http_port = 0;
            config.test_funais = self.config.test_funais.clone();
        ...
        pub fn new_with_observer(
            ...
            let epochs = config.epochs.clone().unwrap_or_else(|| {
                FunaiEpoch::unit_test_pre_2_05(config.burnchain.first_block_height)
            });
            ...
            let post_flight_callback = move |clarity_tx: &mut ClarityTx| {
                ...
                if conf.setup_code.len() > 0 {
                    let receipt = clarity_tx.connection().as_transaction(|clarity| {
                        ...
                        let boot_code_account = FunaiAccount {
                        ...
                        let smart_contract = TransactionPayload::SmartContract(
                            TransactionSmartContract {
                                ...
                                code_body: FunaiString::from_str(&conf.setup_code)
            ...
            let (chainstate, _) = FunaiChainState::open_and_exec(
                false,
                config.network_id,
                &chainstate_path,
                Some(&mut boot_data),
            ...
            let mut funai_node = TestFunaiNode::from_chainstate(chainstate);

            {
                ...
                for i in prev_snapshot.block_height..config.current_block {
                    let burn_block = {
                        ...
                        funai_node.add_key_register(&mut burn_block, &mut miner);
                        burn_block
                    };
            ...
            let funaidb_path = format!("{}/funai_db.sqlite", &test_path);
            let mut funai_dbs_conn = FunaiDBs::connect(&funaidb_path, true).unwrap();
            let relayer_funai_dbs = FunaiDBs::connect(&funaidb_path, true).unwrap();
            let p2p_funai_dbs = FunaiDBs::connect(&funaidb_path, true).unwrap();

            let mut old_funaidb_configs = HashMap::new();
            for (i, contract) in config.funai_dbs.iter().enumerate() {
                old_funaidb_configs.insert(
                    contract.clone(),
                    config
                        .funai_db_configs
                        .get(i)
                        .map(|config| config.clone().unwrap_or(FunaiDBConfig::noop()))
                        .unwrap_or(FunaiDBConfig::noop()),
                );
            }
            let mut funaidb_configs = funai_dbs_conn
                .create_or_reconfigure_funaidbs(
                    &mut funai_node.chainstate,
                    &sortdb,
                    old_funaidb_configs,
                )
                .expect("Failed to refresh funaidb configs");

            let funai_db_syncs =
                Self::init_funaidb_syncs(&test_path, &peerdb, &mut funaidb_configs);

            let funaidb_contracts: Vec<_> =
                funai_db_syncs.keys().map(|cid| cid.clone()).collect();
            ...
            peer_network.set_funai_db_configs(config.get_funai_db_configs());

            peer_network.bind(&local_addr, &http_local_addr).unwrap();
            let relayer = Relayer::from_p2p(&mut peer_network, relayer_funai_dbs);
        ...
        pub fn step(&mut self) -> Result<NetworkResult, net_error> {
            ...
            let funai_node = self.funai_node.take().unwrap();
            let burn_tip_height = SortitionDB::get_canonical_burn_chain_tip(sortdb.conn())
                .unwrap()
                .block_height;
            let funai_tip_height = NakamotoChainState::get_canonical_block_header(
                funai_node.chainstate.db(),
                &sortdb,
            )
            .unwrap()
            ...
            self.funai_node = Some(funai_node);
        ...
        pub fn step_with_ibd_and_dns(
            ...
            let mut funai_node = self.funai_node.take().unwrap();
        ...
        pub fn run_with_ibd(
            ...
            self.coord.handle_new_funai_block().unwrap();
            self.coord.handle_new_nakamoto_funai_block().unwrap();
        ...
        pub fn refresh_burnchain_view(&mut self) {
            ...
            let indexer = BitcoinIndexer::new_unit_test(&self.config.burnchain.working_dir);
            self.network
                .refresh_burnchain_view(&indexer, &sortdb, &mut funai_node.chainstate, false)
                .unwrap();
        ...
        fn inner_next_burnchain_block(
            ...
            let (block_height, block_hash, epoch_id) = {
                let tip = SortitionDB::get_canonical_burn_chain_tip(&sortdb.conn()).unwrap();
                let epoch_id = SortitionDB::get_funai_epoch(&sortdb.conn(), tip.block_height + 1)
                    .unwrap()
                    .unwrap()
                    .epoch_id;
            ...
            let missing_pox_anchor_block_hash_opt = if epoch_id < FunaiEpochId::Epoch30 {
                self.coord
        ...
        /// Pre-process an epoch 2.x Funai block.
        /// Validate it and store it to staging.
        pub fn preprocess_funai_block(&mut self, block: &FunaiBlock) -> Result<bool, String> {
            let sortdb = self.sortdb.take().unwrap();
            let mut node = self.funai_node.take().unwrap();
            let res = {
                let sn = {
                    let ic = sortdb.index_conn();
                    let tip = SortitionDB::get_canonical_burn_chain_tip(&ic).unwrap();
                    let sn_opt = SortitionDB::get_block_snapshot_for_winning_funai_block(
                        &ic,
                        &tip.sortition_id,
                        &block.block_hash(),
                    )
            ...
            if res.is_ok() {
                ...
                test_debug!(
                    "\n\n{:?}: after funai block {:?}, tip PoX ID is {:?}\n\n",
                    &self.to_neighbor().addr,
                    &block.block_hash(),
                    &pox_id
                );
        ...
        /// Store the given epoch 2.x Funai block and microblock to staging, and then try and
        /// process them.
        pub fn process_funai_epoch_at_tip(
            &mut self,
            block: &FunaiBlock,
            microblocks: &Vec<FunaiMicroblock>,
        ) -> () {
            ...
            let mut node = self.funai_node.take().unwrap();
            {
                ...
                let tip = SortitionDB::get_canonical_burn_chain_tip(&ic).unwrap();
                node.chainstate
                    .preprocess_funai_epoch(&ic, &tip, block, microblocks)
                    .unwrap();
            }
            self.coord.handle_new_funai_block().unwrap();
        ...
        /// Store the given epoch 2.x Funai block and microblock to the given node's staging,
        /// using the given sortition DB as well, and then try and process them.
        fn inner_process_funai_epoch_at_tip(
            &mut self,
            sortdb: &SortitionDB,
            ...
            self.coord.handle_new_funai_block()?;
        ...
        pub fn process_funai_epoch_at_tip_checked(
            &mut self,
            block: &FunaiBlock,
            microblocks: &Vec<FunaiMicroblock>,
        ) -> Result<(), coordinator_error> {
            ...
            let mut node = self.funai_node.take().unwrap();
            let res =
                self.inner_process_funai_epoch_at_tip(&sortdb, &mut node, block, microblocks);
            self.sortdb = Some(sortdb);
            self.funai_node = Some(node);
            res
        }

        /// Accept a new Funai block and microblocks via the relayer, and then try to process
        /// them.
        pub fn process_funai_epoch(
            &mut self,
            block: &FunaiBlock,
        ...
        pub fn chainstate(&mut self) -> &mut FunaiChainState {
            &mut self.funai_node.as_mut().unwrap().chainstate
        }
            ...
            ) -> Result<R, net_error>,
        {
            ...
            let res = f(self, &mut sortdb, &mut funai_node.chainstate, &mut mempool);
        ...
        pub fn tenure_with_txs(
            ...
            let (burn_ops, funai_block, microblocks) = self.make_tenure(
                |ref mut miner,
                 ...
                 ref parent_microblock_header_opt| {
                    ...
                    let block_builder = FunaiBlockBuilder::make_regtest_block_builder(
                        &burnchain,
                        &parent_tip,
                        vrf_proof,
                        tip.total_burn,
                    ...
                    let (anchored_block, _size, _cost) =
                        FunaiBlockBuilder::make_anchored_block_from_txs(
                            block_builder,
                            chainstate,
                            &sortdb.index_conn(),
                            block_txs,
            ...
            self.process_funai_epoch_at_tip(&funai_block, &microblocks);

            *coinbase_nonce += 1;

            let tip_id = FunaiBlockId::new(&consensus_hash, &funai_block.block_hash());

            if let Some((start_check_cycle, end_check_cycle)) = self.config.check_pox_invariants {
                pox_2_tests::check_all_funai_link_invariants(
                    self,
                    &tip_id,
                    start_check_cycle,
                    end_check_cycle,
        ...
        /// Make a tenure, using `tenure_builder` to generate a Funai block and a list of
        /// microblocks.
            ...
            ) -> (FunaiBlock, Vec<FunaiMicroblock>),
        {
            ...
            let parent_block_opt = funai_node.get_last_anchored_block(&self.miner);
            ...
            let parent_microblock_header_opt =
                get_last_microblock_header(&funai_node, &self.miner, parent_block_opt.as_ref());
            let last_key = funai_node.get_last_key(&self.miner);
            ...
            let (funai_block, microblocks) = tenure_builder(
                &mut self.miner,
                &mut sortdb,
                &mut funai_node.chainstate,
                proof,
            ...
            let mut block_commit_op = funai_node.make_tenure_commitment(
                &mut sortdb,
                &mut burn_block,
                &mut self.miner,
                &funai_block,
            ...
            if funai_block.header.parent_block == BlockHeaderHash([0u8; 32]) {
                block_commit_op.parent_block_ptr = 0;
                block_commit_op.parent_vtxindex = 0;
            }

            let leader_key_op = funai_node.add_key_register(&mut burn_block, &mut self.miner);
        ...
        pub fn make_default_tenure(
            ...
            let (funai_block, microblocks, block_commit_op) = funai_node.mine_funai_block(
                &mut sortdb,
                &mut self.miner,
                &mut burn_block,
                &last_key,
                ...
                |mut builder, ref mut miner, ref sortdb| {
                    let (mut miner_chainstate, _) =
                        FunaiChainState::open(false, network_id, &chainstate_path, None).unwrap();
        ...
        pub fn make_client_convo(&self) -> ConversationP2P {
            ConversationP2P::new(
                ...
                self.config
                    .epochs
                    .clone()
                    .unwrap_or(FunaiEpoch::unit_test_3_0(0)),
            )
        }

        pub fn make_client_local_peer(&self, privk: FunaiPrivateKey) -> LocalPeer {
            LocalPeer::new(
                self.config.network_id,
        ...
        pub fn check_nakamoto_migration(&mut self) {
            ...
            let epochs = SortitionDB::get_funai_epochs(sortdb.conn()).unwrap();
            let epoch_3_idx =
                FunaiEpoch::find_epoch_by_id(&epochs, FunaiEpochId::Epoch30).unwrap();
            let epoch_3 = epochs[epoch_3_idx].clone();

            let mut all_chain_tips = sortdb.get_all_funai_chain_tips().unwrap();
            ...
            tx.execute(
                "CREATE TABLE funai_chain_tips_backup AS SELECT * FROM funai_chain_tips;",
                NO_PARAMS,
            )
            .unwrap();
            tx.execute("DELETE FROM funai_chain_tips;", NO_PARAMS)
                .unwrap();
            tx.commit().unwrap();

            // NOTE: this considers each and every snapshot, but we only care about epoch2.x
            sortdb.apply_schema_8_funai_chain_tips(&tip).unwrap();
            let migrated_epoch2_chain_tips: Vec<_> = sortdb
                .get_all_funai_chain_tips()
                .unwrap()
                .into_iter()
                .filter(|tip| epoch2_chs.contains(&tip.1))
                .collect();
            ...
            // funai blocks as their chain tips, however.
            assert_eq!(
            ...
            // restore
            let tx = sortdb.tx_begin().unwrap();
            tx.execute("DROP TABLE funai_chain_tips;", NO_PARAMS)
                .unwrap();
            tx.execute(
                "ALTER TABLE funai_chain_tips_backup RENAME TO funai_chain_tips;",
                NO_PARAMS,
            )
            .unwrap();
            tx.commit().unwrap();
            ...
            let mut restored_chain_tips = sortdb.get_all_funai_chain_tips().unwrap();
    ...
    pub fn to_addr(sk: &FunaiPrivateKey) -> FunaiAddress {
        FunaiAddress::from_public_keys(
            C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
            &AddressHashMode::SerializeP2PKH,
            1,
            &vec![FunaiPublicKey::from_private(sk)],
        )
        .unwrap()
    }
}
