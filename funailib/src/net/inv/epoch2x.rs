...
use funai_common::types::chainstate::{BlockHeaderHash, PoxId, SortitionId};
...
use crate::net::{
    Error as net_error, GetBlocksInv, Neighbor, NeighborKey, PeerAddress, FunaiMessage, FunaiP2P,
    *,
};
...
impl NeighborBlockStats {
        ...
        let next_request = match request.try_send_recv() {
            Ok(message) => {
                match message.payload {
                    FunaiMessageType::PoxInv(poxinv_data) => {
                        debug!(
    ...
    pub fn getblocksinv_try_finish(
        ...
        let next_request = match request.try_send_recv() {
            Ok(message) => {
                match message.payload {
                    FunaiMessageType::BlocksInv(blocks_inv_data) => {
                        // got a BlocksInv!
...
impl PeerNetwork {
    ...
    fn get_block_scan_start(&self, sortdb: &SortitionDB, highest_remote_reward_cycle: u64) -> u64 {
        // see if the funai tip affirmation map and heaviest affirmation map diverge.  If so, then
        ...
        // tip.
        // NOTE: This code path only works in Funai 2.x, but that's okay because this whole state
        // machine is only used in Funai 2.x
        let (consensus_hash, _) = SortitionDB::get_canonical_funai_chain_tip_hash(sortdb.conn())
            .unwrap_or((ConsensusHash::empty(), BlockHeaderHash([0u8; 32])));

        let funai_tip_burn_block_height =
        ...
        let funai_tip_rc = self
            .burnchain
            .block_height_to_reward_cycle(funai_tip_burn_block_height)
            .unwrap_or(0);
    ...
    fn inv_getpoxinv_begin(
        ...
        let payload = FunaiMessageType::GetPoxInv(getpoxinv);
    ...
    fn inv_getblocksinv_begin(
        ...
        let payload = FunaiMessageType::GetBlocksInv(getblocksinv);
        let message = self.sign_for_neighbor(nk, payload)?;
        let request = self
            .send_neighbor_message(nk, message, request_timeout)
            .map_err(|e| {
    ...
    pub fn get_local_blocks_inv(
        ...
        match blocks_inv {
            FunaiMessageType::BlocksInv(blocks_inv) => {
    ...
    pub fn work_inv_sync_epoch2x(
        ...
        let start_download_sortition = if let Some(ref inv_state) = self.inv_state {
            let (consensus_hash, _) = SortitionDB::get_canonical_funai_chain_tip_hash(
                sortdb.conn(),
            )
            .expect("FATAL: failed to load canonical funai chain tip hash from sortition DB");
            let funai_tip_sortition_height =
            ...
            let sortition_height_start =
                cmp::min(funai_tip_sortition_height, inv_state.block_sortition_start);
