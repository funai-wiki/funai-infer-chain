...
use crate::net::{
    ...
    NeighborKey, FunaiMessage, FunaiMessageType,
};
...
impl NakamotoTenureInv {
    ...
    pub fn getnakamotoinv_try_finish(
        ...
        match reply.payload {
            FunaiMessageType::NakamotoInv(inv_data) => {
                debug!(
                    "{:?}: got NakamotoInv: {:?}",
                    network.get_local_peer(),
                    &inv_data
...
impl<NC: NeighborComms> NakamotoInvStateMachine<NC> {
    ...
    fn make_getnakamotoinv(&self, reward_cycle: u64) -> Option<FunaiMessageType> {
        let Some(ch) = self.reward_cycle_consensus_hashes.get(&reward_cycle) else {
            return None;
        };
        Some(FunaiMessageType::GetNakamotoInv(GetNakamotoInvData {
    ...
    pub fn process_getnakamotoinv_begins(
        ...
        let nakamoto_start_height = network
            .get_epoch_by_epoch_id(FunaiEpochId::Epoch30)
            .start_height;
