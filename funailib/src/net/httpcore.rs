...
/// This module binds the http library to Funai as a `ProtocolFamily` implementation
...
use funai_common::codec::{read_next, Error as CodecError, FunaiMessageCodec, MAX_MESSAGE_LEN};
use funai_common::types::chainstate::{
    ConsensusHash, FunaiAddress, FunaiBlockId, FunaiPublicKey,
};
use funai_common::types::net::PeerHost;
use funai_common::types::Address;
use funai_common::util::chunked_encoding::*;
use funai_common::util::get_epoch_time_ms;
use funai_common::util::retry::{BoundReader, RetryReader};
use url::Url;
...
use crate::core::{MemPoolDB, FunaiEpoch};
...
use crate::net::{Error as NetError, MessageSequence, ProtocolFamily, FunaiNodeState, UrlString};

const CHUNK_BUF_LEN: usize = 32768;

/// canonical funai tip height header
pub const STACKS_HEADER_HEIGHT: &'static str = "X-Canonical-Funai-Tip-Height";

/// request ID header
pub const STACKS_REQUEST_ID: &'static str = "X-Request-Id";

/// Request ID to use or expect from non-Funai HTTP clients.
...
/// from non-Funai nodes (like Gaia hubs, CDNs, vanilla HTTP servers, and so on).
...
impl From<&str> for TipRequest {
    fn from(s: &str) -> TipRequest {
        ...
        } else if let Ok(block_id) = FunaiBlockId::from_hex(s) {
...
/// Extension to HttpRequestPreamble to give it awareness of Funai-specific fields
pub trait HttpPreambleExtensions {
    /// Set the node's canonical Funai chain tip
    fn set_canonical_funai_tip_height(&mut self, height: Option<u32>);
    /// Set the node's request ID
    fn set_request_id(&mut self, req_id: u32);
    /// Get the canonical funai chain tip
    fn get_canonical_funai_tip_height(&self) -> Option<u32>;
    /// Get the request ID
    fn get_request_id(&self) -> Option<u32>;
}

impl HttpPreambleExtensions for HttpRequestPreamble {
    /// Set the canonical Funai chain tip height
    fn set_canonical_funai_tip_height(&mut self, height_opt: Option<u32>) {
        if let Some(height) = height_opt {
            self.add_header(
                "X-Canonical-Funai-Tip-Height".into(),
                format!("{}", &height),
            );
        } else {
            self.remove_header("X-Canonical-Funai-Tip-Height".to_string());
        }
    }
    ...
    /// Get the canonical Funai chain tip
    fn get_canonical_funai_tip_height(&self) -> Option<u32> {
        self.get_header("X-Canonical-Funai-Tip-Height".to_string())
...
pub mod request {
    ...
    pub fn get_contract_address(
        ...
        let address = if let Some(address_str) = captures.name(address_key) {
            if let Some(addr) = FunaiAddress::from_string(&address_str.as_str()) {
                addr
            } else {
    ...
    /// Get and parse a FunaiBlockId from a path's captures, given the name of the regex field.
    pub fn get_block_hash(captures: &Captures, key: &str) -> Result<FunaiBlockId, HttpError> {
        let block_id = if let Some(block_id) = captures.name(key) {
            match FunaiBlockId::from_hex(block_id.as_str()) {
                Ok(bhh) => bhh,
                Err(_e) => {
...
/// Extension to HttpRequestContents to give it awareness of Funai-specific fields
pub trait HttpRequestContentsExtensions {
    /// Chain constructor: Request a specific tip
    fn for_specific_tip(self, tip: FunaiBlockId) -> Self;
...
pub trait RPCRequestHandler: HttpRequest + HttpResponse + RPCRequestHandlerClone {
    ...
    fn get_canonical_burn_chain_tip(
        ...
        sortdb: &SortitionDB,
    ) -> Result<BlockSnapshot, FunaiHttpResponse> {
    ...
    /// Helper to get the current Funai epoch
    fn get_funai_epoch(
        &self,
        preamble: &HttpRequestPreamble,
        sortdb: &SortitionDB,
        block_height: u64,
    ) -> Result<FunaiEpoch, FunaiHttpResponse> {
        SortitionDB::get_funai_epoch(sortdb.conn(), block_height)
            .map_err(|e| {
                FunaiHttpResponse::new_error(&preamble, &HttpServerError::new(format!("Could not load Funai epoch for canonical burn height: {:?}", &e)))
            })?
            .ok_or_else(|| {
                let msg = format!(
                    "Failed to get fee rate estimate because could not load Funai epoch for canonical burn height = {}",
                    block_height
                );
                warn!("{}", &msg);
                FunaiHttpResponse::new_error(&preamble, &HttpServerError::new(msg))
            })
    }

    /// Helper to get the Funai tip
    fn get_funai_chain_tip(
        &self,
        preamble: &HttpRequestPreamble,
        sortdb: &SortitionDB,
        chainstate: &FunaiChainState,
    ) -> Result<FunaiHeaderInfo, FunaiHttpResponse> {
        NakamotoChainState::get_canonical_block_header(chainstate.db(), sortdb)
            .map_err(|e| {
                let msg = format!("Failed to load funai chain tip header: {:?}", &e);
                warn!("{}", &msg);
                FunaiHttpResponse::new_error(&preamble, &HttpServerError::new(msg))
            })?
            .ok_or_else(|| {
                let msg =
                    "No funai tip exists yet. Perhaps no blocks have been processed by this node"
                        .to_string();
                warn!("{}", &msg);
                FunaiHttpResponse::new_error(&preamble, &HttpNotFound::new(msg))
            })
    }
}

/// A decoded HttpRequest for use in Funai
#[derive(Debug, Clone, PartialEq)]
pub struct FunaiHttpRequest {
    preamble: HttpRequestPreamble,
    contents: HttpRequestContents,
    start_time: u128,
    /// Cache result of `FunaiHttp::find_response_handler` so we don't have to do the regex matching twice
...
impl FunaiHttpRequest {
    ...
    /// Instantiate a request to a remote Funai peer
    /// `path` is just the request path.  Query arguments are added via `contents`.  Any query
    /// component to `path` will be silently dropped.
    ///
    /// In Funai, all requests must have a known content length.  If it cannot be calculated, then
...
impl From<FunaiHttpResponse> for Result<(HttpResponsePreamble, HttpResponseContents), NetError> {
    fn from(resp: FunaiHttpResponse) -> Self {
        resp.try_into_contents()
    }
}

impl FunaiHttpResponse {
    pub fn new(preamble: HttpResponsePreamble, body: HttpResponsePayload) -> FunaiHttpResponse {
        FunaiHttpResponse { preamble, body }
    }
    ...
    pub fn new_error(
        ...
        FunaiHttpResponse::new(preamble, payload)
    }

    /// Make a new HTTP error response for text, apropos of nothing
    pub fn new_empty_error(error: &dyn HttpErrorResponse) -> FunaiHttpResponse {
        let code = error.code();
        let payload = error.payload();
...
#[derive(Debug, Clone, PartialEq)]
pub enum FunaiHttpMessage {
    Request(FunaiHttpRequest),
    Response(FunaiHttpResponse),
    Error(String, FunaiHttpResponse),
}

/// HTTP message preamble
#[derive(Debug, Clone, PartialEq)]
pub enum FunaiHttpPreamble {
    Request(HttpRequestPreamble),
    Response(HttpResponsePreamble),
}

impl FunaiHttpPreamble {
    #[cfg(test)]
...
impl FunaiMessageCodec for FunaiHttpPreamble {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), CodecError> {
        match *self {
            FunaiHttpPreamble::Request(ref req) => req.consensus_serialize(fd),
            FunaiHttpPreamble::Response(ref res) => res.consensus_serialize(fd),
        }
    }

    fn consensus_deserialize<R: Read>(fd: &mut R) -> Result<FunaiHttpPreamble, CodecError> {
...
impl MessageSequence for FunaiHttpMessage {
...
/// Internally used by FunaiHttp to keep track of chunk-decoding state.
#[derive(Debug, Clone, PartialEq)]
struct FunaiHttpRecvStream {
    state: HttpChunkedTransferReaderState,
    data: Vec<u8>,
    total_consumed: usize, // number of *encoded* bytes consumed
}

impl FunaiHttpRecvStream {
    pub fn new(max_size: u64) -> FunaiHttpRecvStream {
        FunaiHttpRecvStream {
...
#[derive(Debug, Clone, PartialEq)]
struct FunaiHttpReplyData {
    request_id: u32,
    stream: FunaiHttpRecvStream,
}

/// Funai HTTP state machine implementation, for bufferring up data.
...
pub struct FunaiHttp {
    ...
    reply: Option<FunaiHttpReplyData>,
...
impl FunaiHttp {
    pub fn new(peer_addr: SocketAddr, conn_opts: &ConnectionOptions) -> FunaiHttp {
        let mut http = FunaiHttp {
            peer_addr,
            body_start: None,
    ...
    pub fn handle_try_parse_request(
        ...
        preamble: &HttpRequestPreamble,
        body: &[u8],
    ) -> Result<FunaiHttpRequest, NetError> {
        ...
        let request = FunaiHttpRequest::new(preamble.clone(), payload);
        Ok(request)
    }
    ...
    pub fn try_parse_request(
        ...
        for (verb, regex, request) in self.request_handlers.iter_mut() {
            ...
            debug!("Handle FunaiHttpRequest"; "verb" => %verb, "peer_addr" => %self.peer_addr, "path" => %decoded_path, "query" => %query);
            let request = FunaiHttpRequest::new(preamble.clone(), payload);
            return Ok(request);
        }
    ...
    pub fn try_parse_error_response(
        preamble: &HttpResponsePreamble,
        body: &[u8],
    ) -> Result<FunaiHttpResponse, NetError> {
        ...
        Ok(FunaiHttpResponse::new(preamble.clone(), payload))
    }
    ...
    pub fn try_parse_response(
        ...
        let response = FunaiHttpResponse::new(preamble.clone(), payload);
        return Ok(response);
    }
    ...
    pub fn try_handle_request(
        ...
        let (response_preamble, response_contents) = match request_result {
            Ok((rp, rc)) => (rp, rc),
            Err(NetError::Http(e)) => {
                return FunaiHttpResponse::new_error(&request_preamble, &*e.into_http_error())
                    .try_into_contents()
            }
            Err(e) => {
    ...
    fn set_pending(&mut self, preamble: &HttpResponsePreamble) {
        self.reply = Some(FunaiHttpReplyData {
            request_id: preamble
                .get_request_id()
                .unwrap_or(HTTP_REQUEST_ID_RESERVED),
            stream: FunaiHttpRecvStream::new(MAX_MESSAGE_LEN as u64),
        });
    }
    ...
    fn set_preamble(&mut self, preamble: &FunaiHttpPreamble) -> Result<(), NetError> {
        match preamble {
            FunaiHttpPreamble::Response(ref http_response_preamble) => {
    ...
    /// For details see https://github.com/funai-network/funai-core/issues/4574
    pub fn metrics_identifier(&self, req: &mut FunaiHttpRequest) -> &str {
    ...
    pub fn parse_response(
        ...
        request_path: &str,
        response_buf: &[u8],
    ) -> Result<FunaiHttpMessage, NetError> {
        let mut http = FunaiHttp::new(
        ...
        let is_chunked = match preamble {
            FunaiHttpPreamble::Response(ref resp) => resp.is_chunked(),
            _ => {
                return Err(NetError::DeserializeError(
...
impl ProtocolFamily for FunaiHttp {
    type Preamble = FunaiHttpPreamble;
    type Message = FunaiHttpMessage;
    ...
    fn payload_len(&mut self, preamble: &FunaiHttpPreamble) -> Option<usize> {
        match *preamble {
            FunaiHttpPreamble::Request(ref http_request_preamble) => {
                Some(http_request_preamble.get_content_length() as usize)
            }
            FunaiHttpPreamble::Response(ref http_response_preamble) => http_response_preamble
                .content_length
    ...
    fn read_preamble(&mut self, buf: &[u8]) -> Result<(FunaiHttpPreamble, usize), NetError> {
        ...
        let preamble = {
            ...
            let preamble: FunaiHttpPreamble = read_next(&mut rd)?;
            preamble
        };
    ...
    fn stream_payload<R: Read>(
        &mut self,
        preamble: &FunaiHttpPreamble,
        fd: &mut R,
    ) -> Result<(Option<(FunaiHttpMessage, usize)>, usize), NetError> {
        if self.payload_len(preamble).is_some() {
            return Err(NetError::InvalidState);
        }
        match preamble {
            FunaiHttpPreamble::Request(_) => {
                ...
                match message_bytes_opt {
                    Some((message_bytes, total_bytes_consumed)) => {
                        ...
                        match parse_res {
                            Ok(data_response) => Ok((
                                Some((
                                    FunaiHttpMessage::Response(data_response),
    ...
    fn read_payload(
        ...
        preamble: &FunaiHttpPreamble,
        buf: &[u8],
    ) -> Result<(FunaiHttpMessage, usize), NetError> {
        match preamble {
            FunaiHttpPreamble::Request(ref http_request_preamble) => {
                ...
                match self.try_parse_request(http_request_preamble, &buf[0..len]) {
                    Ok(data_request) => Ok((FunaiHttpMessage::Request(data_request), len)),
                    Err(NetError::Http(http_error)) => {
                        // convert into a response
                        let resp = FunaiHttpResponse::new_error(
                            http_request_preamble,
                            &*http_error.into_http_error(),
                        );
                        self.reset();
                        return Ok((
                            FunaiHttpMessage::Error(
            ...
            FunaiHttpPreamble::Response(ref http_response_preamble) => {
                ...
                self.reset();
                res.map(|data_response| (FunaiHttpMessage::Response(data_response), buf.len()))
            }
        }
    }
    ...
    ///
    /// NOTE: If we're sending a FunaiHttpMessage::Request(..), then the next preamble and payload
    /// received _must be_ a FunaiHttpMessage::Response(..) in response to the request.
    /// If it is not, then that decode will fail.
    fn write_message<W: Write>(
        &mut self,
        fd: &mut W,
        message: &FunaiHttpMessage,
    ) -> Result<(), NetError> {
        match *message {
            FunaiHttpMessage::Request(ref req) => {
            ...
            FunaiHttpMessage::Response(ref resp) => resp.send(fd),
            FunaiHttpMessage::Error(_, ref resp) => resp.send(fd),
        }
    }
}
