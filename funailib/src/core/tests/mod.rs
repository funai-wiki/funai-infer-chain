...
use funai_common::codec::{read_next, Error as codec_error, FunaiMessageCodec};
...
use funai_common::util::{get_epoch_time_ms, get_epoch_time_secs, log, sleep_ms};
...
use crate::chainstate::funai::index::{MarfTrieId, TrieHashExtension};
use crate::chainstate::funai::miner::TransactionResult;
...
use crate::chainstate::funai::{
    ...
    FunaiBlockHeader, FunaiMicroblockHeader, FunaiPrivateKey, FunaiPublicKey,
...
pub fn make_block(
    ...
    let anchored_header = FunaiBlockHeader {
        version: 1,
    ...
    let c_tx = FunaiChainState::chainstate_block_begin(
        &chainstate_tx,
        clar_tx,
        &TEST_BURN_STATE_DB,
        &parent.0,
    ...
    let new_index_hash = FunaiBlockId::new(&block_consensus, &block_hash);

    // instantiate the inner MARF
    chainstate_tx
        .put_indexed_all(
            &FunaiBlockId::new(&parent.0, &parent.1),
            &new_index_hash,
            &vec![],
            &vec![],
        )
...
fn mempool_walk_over_fork() {
    ...
    for ix in 0..3 {
        ...
        let origin_address = FunaiAddress {
            version: 22,
            bytes: Hash160::from_data(&[ix as u8; 32]),
        };
        let sponsor_address = FunaiAddress {
            version: 22,
    ...
    chainstate.with_read_only_clarity_tx(
        &TEST_BURN_STATE_DB,
        &FunaiBlockHeader::make_index_block_hash(&b_5.0, &b_5.1),
        |clarity_conn| {
            let mut count_txs = 0;
            mempool
                .iterate_candidates::<_, ChainstateError, _>(
    ...
    chainstate.with_read_only_clarity_tx(
        &TEST_BURN_STATE_DB,
        &FunaiBlockHeader::make_index_block_hash(&b_3.0, &b_3.1),
        |clarity_conn| {
            let mut count_txs = 0;
            mempool
                .iterate_candidates::<_, ChainstateError, _>(
    ...
    chainstate.with_read_only_clarity_tx(
        &TEST_BURN_STATE_DB,
        &FunaiBlockHeader::make_index_block_hash(&b_4.0, &b_4.1),
        |clarity_conn| {
            let mut count_txs = 0;
            mempool
                .iterate_candidates::<_, ChainstateError, _>(
                    ...
                    |_, available_tx, _| {
                        ...
                        let result = if count_txs == 2 {
                            TransactionResult::error(
                                &available_tx.tx.tx,
                                crate::chainstate::funai::Error::FunaiTransactionSkipped(
