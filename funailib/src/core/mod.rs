...
use funai_common::types::chainstate::{BlockHeaderHash, BurnchainHeaderHash, FunaiBlockId};
use funai_common::types::FunaiEpoch as GenericFunaiEpoch;
pub use funai_common::types::FunaiEpochId;
use funai_common::util::log;

pub use self::mempool::MemPoolDB;
use crate::burnchains::bitcoin::indexer::get_bitcoin_funai_epochs;
...
pub type FunaiEpoch = GenericFunaiEpoch<ExecutionCost>;
...
// chain id
pub use funai_common::consts::{CHAIN_ID_MAINNET, CHAIN_ID_TESTNET, STACKS_EPOCH_MAX};
...
// Number of previous burnchain blocks to search to find burnchain-hosted Funai operations
...
pub const GENESIS_EPOCH: FunaiEpochId = FunaiEpochId::Epoch20;

/// The number of blocks which will share the block bonus
///   from burn blocks that occurred without a sortition.
///   (See: https://forum.funai.org/t/pox-consensus-and-stx-future-supply)
#[cfg(test)]
...
pub const MAINNET_2_0_GENESIS_ROOT_HASH: &str =
    "4c86ee69598aca8c7183fe9ebf5ddb888de6a3d0208c0a5d7c420de3dc4681eb";
...
pub const BITCOIN_MAINNET_FIRST_BLOCK_HEIGHT: u64 = 105;
pub const BITCOIN_MAINNET_FIRST_BLOCK_TIMESTAMP: u32 = 1732851341;
pub const BITCOIN_MAINNET_FIRST_BLOCK_HASH: &str =
    "003e18ffe96725591979e7e4d7337f106437c9b0b9d7aacadedcade55d9cd494";
pub const BITCOIN_MAINNET_INITIAL_REWARD_START_BLOCK: u64 = 0;
pub const BITCOIN_MAINNET_STACKS_2_05_BURN_HEIGHT: u64 = 105;
pub const BITCOIN_MAINNET_STACKS_21_BURN_HEIGHT: u64 = 106;
/// This is Epoch-2.2 activation height proposed in SIP-022
pub const BITCOIN_MAINNET_STACKS_22_BURN_HEIGHT: u64 = 107;
/// This is Epoch-2.3 activation height proposed in SIP-023
pub const BITCOIN_MAINNET_STACKS_23_BURN_HEIGHT: u64 = 108;
/// This is Epoch-2.3, now Epoch-2.4, activation height proposed in SIP-024
pub const BITCOIN_MAINNET_STACKS_24_BURN_HEIGHT: u64 = 109;
/// This is Epoch-2.5, activation height proposed in SIP-021
pub const BITCOIN_MAINNET_STACKS_25_BURN_HEIGHT: u64 = 115;
/// This is Epoch-3.0, activation height proposed in SIP-021
pub const BITCOIN_MAINNET_STACKS_30_BURN_HEIGHT: u64 = 9223372036854775806;

pub const BITCOIN_TESTNET_FIRST_BLOCK_HEIGHT: u64 = 0;
pub const BITCOIN_TESTNET_FIRST_BLOCK_TIMESTAMP: u32 = 1732851341;
pub const BITCOIN_TESTNET_FIRST_BLOCK_HASH: &str =
    "00e69486d5c8aadd3f15374bc51fc6777a9230c14c2779616e558e5a59ae0ceb";
...
lazy_static! {
    pub static ref FIRST_STACKS_BLOCK_ID: FunaiBlockId =
...
pub const POX_THRESHOLD_STEPS_USTX: u128 = 10 * (MICROSTACKS_PER_STACKS as u128);
...
// Funai 1.0 did not allow smart contracts so all limits are 0.
...
// Block limit for the testnet in Funai 2.0.
...
lazy_static! {
    pub static ref STACKS_EPOCHS_MAINNET: [FunaiEpoch; 9] = [
        FunaiEpoch {
...
lazy_static! {
    pub static ref STACKS_EPOCHS_TESTNET: [FunaiEpoch; 9] = [
        FunaiEpoch {
            epoch_id: FunaiEpochId::Epoch10,
            start_height: 0,
            end_height: 0,
...
lazy_static! {
    pub static ref STACKS_EPOCHS_REGTEST: [FunaiEpoch; 9] = [
        FunaiEpoch {
            epoch_id: FunaiEpochId::Epoch10,
            start_height: 0,
            end_height: 0,
...
/// Funai 2.05 epoch marker.  All block-commits in 2.05 must have a memo bitfield with this value
/// *or greater*.
pub static STACKS_EPOCH_2_05_MARKER: u8 = 0x05;

/// Funai 2.1 epoch marker.  All block-commits in 2.1 must have a memo bitfield with this value
/// *or greater*.
pub static STACKS_EPOCH_2_1_MARKER: u8 = 0x06;

/// Funai 2.2 epoch marker.  All block-commits in 2.2 must have a memo bitfield with this value
/// *or greater*.
pub static STACKS_EPOCH_2_2_MARKER: u8 = 0x07;

/// Funai 2.3 epoch marker.  All block-commits in 2.3 must have a memo bitfield with this value
/// *or greater*.
pub static STACKS_EPOCH_2_3_MARKER: u8 = 0x08;

/// Funai 2.4 epoch marker.  All block-commits in 2.4 must have a memo bitfield with this value
/// *or greater*.
pub static STACKS_EPOCH_2_4_MARKER: u8 = 0x09;

/// Funai 2.5 epoch marker.  All block-commits in 2.5 must have a memo bitfield with this value
/// *or greater*.
pub static STACKS_EPOCH_2_5_MARKER: u8 = 0x0a;

/// Funai 3.0 epoch marker.  All block-commits in 3.0 must have a memo bitfield with this value
/// *or greater*.
pub static STACKS_EPOCH_3_0_MARKER: u8 = 0x0b;

#[test]
fn test_ord_for_funai_epoch() {
...
#[test]
fn test_ord_for_funai_epoch_id() {
    assert_eq!(
        FunaiEpochId::Epoch10.cmp(&FunaiEpochId::Epoch20),
        Ordering::Less
    );
    assert_eq!(
        FunaiEpochId::Epoch20.cmp(&FunaiEpochId::Epoch2_05),
        Ordering::Less
    );
    assert_eq!(
        FunaiEpochId::Epoch10.cmp(&FunaiEpochId::Epoch2_05),
        Ordering::Less
    );
    assert_eq!(
        FunaiEpochId::Epoch10.cmp(&FunaiEpochId::Epoch10),
        Ordering::Equal
    );
    assert_eq!(
        FunaiEpochId::Epoch20.cmp(&FunaiEpochId::Epoch20),
        Ordering::Equal
    );
    assert_eq!(
        FunaiEpochId::Epoch2_05.cmp(&FunaiEpochId::Epoch2_05),
        Ordering::Equal
    );
    assert_eq!(
        FunaiEpochId::Epoch2_05.cmp(&FunaiEpochId::Epoch20),
        Ordering::Greater
    );
    assert_eq!(
        FunaiEpochId::Epoch2_05.cmp(&FunaiEpochId::Epoch10),
        Ordering::Greater
    );
    assert_eq!(
        FunaiEpochId::Epoch20.cmp(&FunaiEpochId::Epoch10),
        Ordering::Greater
    );
}
pub trait FunaiEpochExtension {
    #[cfg(test)]
    fn unit_test(funai_epoch_id: FunaiEpochId, epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_2_05(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_2_05_only(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_pre_2_05(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_2_1(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_2_2(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_2_3(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_2_4(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_2_5(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_3_0(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_2_1_only(epoch_2_0_block_height: u64) -> Vec<FunaiEpoch>;
    #[cfg(test)]
    fn unit_test_3_0_only(first_burnchain_height: u64) -> Vec<FunaiEpoch>;
    fn all(
        epoch_2_0_block_height: u64,
        epoch_2_05_block_height: u64,
        epoch_2_1_block_height: u64,
    ) -> Vec<FunaiEpoch>;
    fn validate_epochs(epochs: &[FunaiEpoch]) -> Vec<FunaiEpoch>;
    ...
    fn get_epochs(
        ...
        configured_epochs: Option<&Vec<FunaiEpoch>>,
    ) -> Vec<FunaiEpoch>;
}

impl FunaiEpochExtension for FunaiEpoch {
    fn get_epochs(
        bitcoin_network: BitcoinNetworkType,
        configured_epochs: Option<&Vec<FunaiEpoch>>,
    ) -> Vec<FunaiEpoch> {
        match configured_epochs {
            ...
            None => get_bitcoin_funai_epochs(bitcoin_network),
        }
    }

    #[cfg(test)]
    fn unit_test_pre_2_05(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test first_burn_height = {}",
    ...
    #[cfg(test)]
    fn unit_test_2_05(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test_2_05_only(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test_2_1(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test_2_2(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test_2_3(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test_2_3 first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test_2_4(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test_2_4 first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test_2_5(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test_2_5 first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test_3_0(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test_3_0 first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test_2_1_only(first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        info!(
            "FunaiEpoch unit_test first_burn_height = {}",
            first_burnchain_height
        );
    ...
    #[cfg(test)]
    fn unit_test(funai_epoch_id: FunaiEpochId, first_burnchain_height: u64) -> Vec<FunaiEpoch> {
        match funai_epoch_id {
            FunaiEpochId::Epoch10 | FunaiEpochId::Epoch20 => {
                FunaiEpoch::unit_test_pre_2_05(first_burnchain_height)
            }
            FunaiEpochId::Epoch2_05 => FunaiEpoch::unit_test_2_05(first_burnchain_height),
            FunaiEpochId::Epoch21 => FunaiEpoch::unit_test_2_1(first_burnchain_height),
            FunaiEpochId::Epoch22 => FunaiEpoch::unit_test_2_2(first_burnchain_height),
            FunaiEpochId::Epoch23 => FunaiEpoch::unit_test_2_3(first_burnchain_height),
            FunaiEpochId::Epoch24 => FunaiEpoch::unit_test_2_4(first_burnchain_height),
            FunaiEpochId::Epoch25 => FunaiEpoch::unit_test_2_5(first_burnchain_height),
            FunaiEpochId::Epoch30 => FunaiEpoch::unit_test_3_0(first_burnchain_height),
        }
    }
    ...
    fn validate_epochs(epochs_ref: &[FunaiEpoch]) -> Vec<FunaiEpoch> {
        ...
        if max_epoch.epoch_id == FunaiEpochId::Epoch30 {
            assert!(PEER_NETWORK_EPOCH >= u32::from(PEER_VERSION_EPOCH_2_5));
        } else {
            assert!(
                max_epoch.network_epoch as u32 <= PEER_NETWORK_EPOCH,
                "funai-blockchain static network epoch should be greater than or equal to the max epoch's"
            );
        }

        assert!(
            FunaiEpochId::latest() >= max_epoch.epoch_id,
            "FunaiEpochId::latest() should be greater than or equal to any epoch defined in the node"
        );
