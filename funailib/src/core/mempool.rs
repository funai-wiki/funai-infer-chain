...
use crate::chainstate::funai::db::{ClarityTx, FunaiChainState};
use crate::chainstate::funai::events::FunaiTransactionReceipt;
use crate::chainstate::funai::index::Error as MarfError;
use crate::chainstate::funai::miner::TransactionEvent;
use crate::chainstate::funai::{
    Error as ChainstateError, FunaiBlock, FunaiMicroblock, FunaiTransaction, TransactionPayload,
};
use crate::clarity_vm::clarity::ClarityConnection;
use crate::core::{
    ExecutionCost, FunaiEpochId, FIRST_BURNCHAIN_CONSENSUS_HASH, FIRST_STACKS_BLOCK_HASH,
};
...
// how far back in time (in Funai blocks) does the bloom counter maintain tx records?
...
impl FunaiMessageCodec for TxTag {
...
impl FunaiMessageCodec for MemPoolSyncData {
    fn consensus_serialize<W: Write>(&self, fd: &mut W) -> Result<(), codec_error> {
        match *self {
...
pub fn decode_tx_stream<R: Read>(
    fd: &mut R,
) -> Result<(Vec<FunaiTransaction>, Option<Txid>), net_error> {
    ...
    let mut txs: Vec<FunaiTransaction> = vec![];
    ...
    loop {
        ...
        let next_msg: Result<FunaiTransaction, _> = read_next(&mut retry_reader);
        match next_msg {
            Ok(tx) => {
                if expect_eof {
...
pub struct MemPoolTxInfoPartial {
    ...
    pub origin_address: FunaiAddress,
    pub origin_nonce: u64,
    pub sponsor_address: FunaiAddress,
...
impl MemPoolTxMetadata {
    pub fn get_unknown_nonces(&self) -> Vec<FunaiAddress> {
...
impl MemPoolWalkTxTypes {
    pub fn all() -> HashSet<MemPoolWalkTxTypes> {
        [
            ...
            MemPoolWalkTxTypes::RegisterModel,
        ]
        .into_iter()
        .collect()
    }
...
impl FromRow<MemPoolTxMetadata> for MemPoolTxMetadata {
    fn from_row<'a>(row: &'a Row) -> Result<MemPoolTxMetadata, db_error> {
        ...
        let origin_address = FunaiAddress::from_column(row, "origin_address")?;
        let origin_nonce = u64::from_column(row, "origin_nonce")?;
        let sponsor_address = FunaiAddress::from_column(row, "sponsor_address")?;
...
const MEMPOOL_INITIAL_SCHEMA: &'static [&'static str] = &[r#"
    CREATE TABLE mempool(
        ...
        height INTEGER NOT NULL,    -- funai block height
...
struct NonceCache {
    cache: HashMap<FunaiAddress, u64>,
...
impl NonceCache {
        ...
        match self.cache.get(address) {
            ...
            None => {
                ...
                match opt_nonce {
                    ...
                    None => {
                        let nonce =
                            FunaiChainState::get_nonce(clarity_tx, &address.clone().into());
    ...
    fn update(&mut self, address: FunaiAddress, value: u64, mempool_db: &DBConn) -> bool {
        // Sqlite cache
...
fn db_set_nonce(conn: &DBConn, address: &FunaiAddress, nonce: u64) -> Result<(), db_error> {
...
fn db_get_nonce(conn: &DBConn, address: &FunaiAddress) -> Result<Option<u64>, db_error> {
    let addr_str = address.to_string();
...
#[cfg(test)]
pub fn db_get_all_nonces(conn: &DBConn) -> Result<Vec<(FunaiAddress, u64)>, db_error> {
    ...
    while let Ok(Some(row)) = iter.next() {
        let addr = FunaiAddress::from_column(row, "address")?;
...
impl MemPoolDB {
    ...
    pub fn open(
        ...
        let (chainstate, _) = FunaiChainState::open(mainnet, chain_id, chainstate_path, None)
    ...
    fn find_origin_addresses_by_descending_fees(
        ...
        offset: u32,
        count: u32,
    ) -> Result<Vec<FunaiAddress>, db_error> {
    ...
    fn save_nonce_for_retry(
        retry_store: &mut HashMap<FunaiAddress, u64>,
        max_size: u64,
        addr: FunaiAddress,
        new_nonce: u64,
    ) {
        ...
        E: From<db_error> + From<ChainstateError>,
    {
        ...
        loop {
            ...
            let (tx_type, do_consider) = match &tx_info.tx.payload {
                ...
                TransactionPayload::RegisterModel(..) => (
                    "RegisterModel".to_string(),
                    settings
                        .txs_to_consider
                        .contains(&MemPoolWalkTxTypes::RegisterModel),
                ),
                TransactionPayload::Infer(_, _, ref userInput, ref context, _) => {
                    ...
                    ("Infer".to_string(), settings_do_consider && infer_task_done)
                }
                _ => ("".to_string(), true),
            };
            if !do_consider {
    ...
    fn are_blocks_in_same_fork(
        ...
        first_funai_block: &BlockHeaderHash,
        second_consensus_hash: &ConsensusHash,
        second_funai_block: &BlockHeaderHash,
    ) -> Result<bool, db_error> {
        let first_block = FunaiBlockId::new(first_consensus_hash, first_funai_block);
        let second_block = FunaiBlockId::new(second_consensus_hash, second_funai_block);
    ...
    pub fn try_add_tx(
        ...
        tx_fee: u64,
        height: u64,
        origin_address: &FunaiAddress,
        origin_nonce: u64,
        sponsor_address: &FunaiAddress,
        sponsor_nonce: u64,
    ...
    fn tx_submit(
        ...
        let block_id = FunaiBlockId::new(consensus_hash, block_hash);
        let height = match NakamotoChainState::get_block_header(chainstate.db(), &block_id) {
            Ok(Some(header)) => header.funai_block_height,
            Ok(None) => {
    ...
    pub fn find_next_missing_transactions(
        ...
        max_txs: u64,
        max_run: u64,
    ) -> Result<(Vec<FunaiTransaction>, Option<Txid>, u64), db_error> {
