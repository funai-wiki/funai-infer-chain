...
// publish the non-generic FunaiEpoch form for use throughout module
pub use crate::vm::database::clarity_db::FunaiEpoch;
use crate::vm::errors::{
    ...
    if let CallableType::SpecialFunction(_, function) = function {
        ...
        let mut resp = match function {
            ...
            CallableType::NativeFunction205(_, function, cost_function, cost_input_handle) => {
                let cost_input = if env.epoch() >= &FunaiEpochId::Epoch2_05 {
