...
macro_rules! epochs_template {
    ($($epoch:ident,)*) => {
        ...
        #[rstest]
        $(
            #[case::$epoch(FunaiEpochId::$epoch)]
        )*
        pub fn test_epochs(#[case] epoch: FunaiEpochId) {}
...
macro_rules! clarity_template {
    ($(($epoch:ident, $clarity:ident),)*) => {
        ...
        #[rstest]
        $(
            #[case::$epoch(ClarityVersion::$clarity, FunaiEpochId::$epoch)]
        )*
        pub fn test_clarity_versions(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {}
        ...
        fn epoch_clarity_pairs_covered() {
            ...
            match (epoch, clarity) {
                // don't test Epoch-1.0
                (FunaiEpochId::Epoch10, _) => (),
                (FunaiEpochId::Epoch30, _) => (),
                // don't test these pairs, because they aren't supported:
                (FunaiEpochId::Epoch20, ClarityVersion::Clarity2) => (),
                (FunaiEpochId::Epoch2_05, ClarityVersion::Clarity2) => (),
                // this will lead to a compile time failure if a pair is left out
                //  of the clarity_template! macro list
                $((FunaiEpochId::$epoch, ClarityVersion::$clarity))|* => (),
            }
        }
    }
}
...
impl MemoryEnvironmentGenerator {
    fn get_env(&mut self, epoch: FunaiEpochId) -> OwnedEnvironment {
...
impl TopLevelMemoryEnvironmentGenerator {
    pub fn get_env(&mut self, epoch: FunaiEpochId) -> OwnedEnvironment {
