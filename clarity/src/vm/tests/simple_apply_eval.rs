...
use crate::vm::types::{
    ...
    FunaiAddressExtensions, TypeSignature,
};
use crate::vm::{
...
fn test_simple_let(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    /*
      test program:
      (let ((x 1) (y 2))
        (+ x
...
fn test_secp256k1() {
    ...
    eprintln!("from_private {:?}", &FunaiPublicKey::from_private(&privk));
...
fn test_principal_of_fix() {
    ...
    let mainnet_principal = FunaiAddress::from_public_keys(
    ...
    let testnet_principal = FunaiAddress::from_public_keys(
        C32_ADDRESS_VERSION_TESTNET_SINGLESIG,
        &AddressHashMode::SerializeP2PKH,
        1,
        &vec![FunaiPublicKey::from_hex(
...
#[apply(test_clarity_versions)]
fn test_simple_if_functions(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    //
    //  test program:
...
#[apply(test_clarity_versions)]
fn test_simple_arithmetic_errors(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    let tests = [
        "(>= 1)",
        "(+ 1 true)",
        "(/ 10 0)",
