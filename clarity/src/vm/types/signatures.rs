...
impl FunctionArgSignature {
    pub fn canonicalize(&self, epoch: &FunaiEpochId) -> FunctionArgSignature {
        match self {
...
impl FunctionReturnsSignature {
    pub fn canonicalize(&self, epoch: &FunaiEpochId) -> FunctionReturnsSignature {
        match self {
...
impl FunctionType {
    pub fn canonicalize(&self, epoch: &FunaiEpochId) -> FunctionType {
        match self {
...
impl TypeSignature {
    ...
    pub fn admits(&self, epoch: &FunaiEpochId, x: &Value) -> Result<bool> {
        let x_type = TypeSignature::type_of(x)?;
        self.admits_type(epoch, &x_type)
    }

    pub fn admits_type(&self, epoch: &FunaiEpochId, other: &TypeSignature) -> Result<bool> {
        match epoch {
            FunaiEpochId::Epoch20 | FunaiEpochId::Epoch2_05 => self.admits_type_v2_0(other),
            FunaiEpochId::Epoch21
            | FunaiEpochId::Epoch22
            | FunaiEpochId::Epoch23
            | FunaiEpochId::Epoch24
            | FunaiEpochId::Epoch25
            | FunaiEpochId::Epoch30 => self.admits_type_v2_1(other),
        ...
        match self {
            ...
            TupleType(ref tuple_sig) => {
                if let TupleType(ref other_tuple_sig) = other {
                    tuple_sig.admits(&FunaiEpochId::Epoch2_05, other_tuple_sig)
                } else {
                    Ok(false)
                }
            }
    ...
    fn admits_type_v2_1(&self, other: &TypeSignature) -> Result<bool> {
        ...
        match self {
            ...
            TupleType(ref tuple_sig) => {
                if let TupleType(ref other_tuple_sig) = &other {
                    tuple_sig.admits(&FunaiEpochId::Epoch21, other_tuple_sig)
                } else {
                    Ok(false)
                }
            }
...
impl TupleTypeSignature {
    ...
    pub fn admits(&self, epoch: &FunaiEpochId, other: &TupleTypeSignature) -> Result<bool> {
...
impl FunctionSignature {
    ...
    pub fn canonicalize(&self, epoch: &FunaiEpochId) -> FunctionSignature {
...
impl TypeSignature {
    ...
    pub fn least_supertype(
        ...
        match epoch {
            FunaiEpochId::Epoch20 | FunaiEpochId::Epoch2_05 => Self::least_supertype_v2_0(a, b),
            FunaiEpochId::Epoch21
            | FunaiEpochId::Epoch22
            | FunaiEpochId::Epoch23
            | FunaiEpochId::Epoch24
            | FunaiEpochId::Epoch25
            | FunaiEpochId::Epoch30 => Self::least_supertype_v2_1(a, b),
            FunaiEpochId::Epoch10 => {
                return Err(CheckErrors::Expects("epoch 1.0 not supported".into()))
            }
        }
    ...
    pub fn least_supertype_v2_0(a: &TypeSignature, b: &TypeSignature) -> Result<TypeSignature> {
        match (a, b) {
            ...
            (ResponseType(resp_a), ResponseType(resp_b)) => {
                let ok_type =
                    Self::factor_out_no_type(&FunaiEpochId::Epoch2_05, &resp_a.0, &resp_b.0)?;
                let err_type =
                    Self::factor_out_no_type(&FunaiEpochId::Epoch2_05, &resp_a.1, &resp_b.1)?;
                Ok(Self::new_response(ok_type, err_type)?)
            }
            (OptionalType(some_a), OptionalType(some_b)) => {
                let some_type =
                    Self::factor_out_no_type(&FunaiEpochId::Epoch2_05, some_a, some_b)?;
    ...
    pub fn least_supertype_v2_1(a: &TypeSignature, b: &TypeSignature) -> Result<TypeSignature> {
        match (a, b) {
            ...
            (ResponseType(resp_a), ResponseType(resp_b)) => {
                let ok_type =
                    Self::factor_out_no_type(&FunaiEpochId::Epoch21, &resp_a.0, &resp_b.0)?;
                let err_type =
                    Self::factor_out_no_type(&FunaiEpochId::Epoch21, &resp_a.1, &resp_b.1)?;
                Ok(Self::new_response(ok_type, err_type)?)
            }
            (OptionalType(some_a), OptionalType(some_b)) => {
                let some_type = Self::factor_out_no_type(&FunaiEpochId::Epoch21, some_a, some_b)?;
                Ok(Self::new_option(some_type)?)
            }
            (
                SequenceType(SequenceSubtype::BufferType(buff_a)),
...
impl TypeSignature {
    ...
    #[cfg(test)]
    pub fn from_string(val: &str, version: ClarityVersion, epoch: FunaiEpochId) -> Self {
...
mod test {
    ...
    fn fail_parse(val: &str, version: ClarityVersion, epoch: FunaiEpochId) -> CheckErrors {
        use crate::vm::ast::parse;
        let expr = &parse(
            &QualifiedContractIdentifier::transient(),
            val,
    ...
    fn type_of_list_of_buffs(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    ...
    #[apply(test_clarity_versions)]
    fn type_signature_way_too_big(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    ...
    #[apply(test_clarity_versions)]
    fn test_construction(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
