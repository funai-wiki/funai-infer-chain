...
impl SequenceData {
    ...
    pub fn replace_at(self, epoch: &FunaiEpochId, index: usize, element: Value) -> Result<Value> {
    ...
    pub fn concat(&mut self, epoch: &FunaiEpochId, other_seq: SequenceData) -> Result<()> {
...
impl Value {
    ...
    pub fn cons_list(list_data: Vec<Value>, epoch: &FunaiEpochId) -> Result<Value> {
        // Constructors for TypeSignature ensure that the size of the Value cannot
        //   be greater than MAX_VALUE_SIZE (they error on such constructions)
...
impl ListData {
    ...
    fn append(&mut self, epoch: &FunaiEpochId, other_seq: ListData) -> Result<()> {
...
impl From<FunaiAddress> for StandardPrincipalData {
    fn from(addr: FunaiAddress) -> StandardPrincipalData {
        StandardPrincipalData(addr.version, addr.bytes.0)
    }
}

impl From<FunaiAddress> for PrincipalData {
    fn from(addr: FunaiAddress) -> PrincipalData {
        PrincipalData::from(StandardPrincipalData::from(addr))
    }
}

impl From<StandardPrincipalData> for FunaiAddress {
    fn from(o: StandardPrincipalData) -> FunaiAddress {
        FunaiAddress {
            version: o.0,
