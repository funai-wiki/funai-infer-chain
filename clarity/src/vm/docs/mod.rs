...
mod test {
    ...
    use funai_common::types::chainstate::{
        BlockHeaderHash, BurnchainHeaderHash, ConsensusHash, SortitionId, FunaiAddress,
        FunaiBlockId, VRFSeed,
    };
    use funai_common::types::{Address, FunaiEpochId, PEER_VERSION_EPOCH_2_1};
    ...
    use crate::vm::{
        ...
        LimitedCostTracker, QualifiedContractIdentifier, FunaiEpoch, Value,
    };
    ...
    impl HeadersDB for DocHeadersDB {
        ...
        fn get_consensus_hash_for_block(&self, _bhh: &FunaiBlockId) -> Option<ConsensusHash> {
        ...
        fn get_burn_block_time_for_block(&self, _id_bhh: &FunaiBlockId) -> Option<u64> {
            Some(1557860301)
        }
        fn get_burn_block_height_for_block(&self, _id_bhh: &FunaiBlockId) -> Option<u32> {
            Some(567890)
        }
            ...
            .execute(|g| {
                for (segment, type_result) in segments.iter().zip(type_results.iter()) {
                    ...
                    if let Some(expected) = expected {
                        ...
                        assert!(
                            type_result.as_ref().unwrap().admits(&FunaiEpochId::latest(), result.as_ref().unwrap()).unwrap(),
    ...
    fn test_examples() {
        ...
        for func_api in apis.functions.iter() {
            ...
            // and give the doc environment's contract some STX
            {
                ...
                let mut env = OwnedEnvironment::new(conn, FunaiEpochId::latest());
