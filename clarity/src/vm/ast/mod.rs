...
pub fn build_ast_with_rules<T: CostTracker>(
    ...
    match ruleset {
        ...
        ASTRules::Typical if epoch < FunaiEpochId::Epoch21 => build_ast_typical(
            contract_identifier,
            source_code,
            cost_track,
            clarity_version,
...
fn inner_build_ast<T: CostTracker>(
    ...
    let (pre_expressions, mut diagnostics, mut success) = if epoch >= FunaiEpochId::Epoch21 {
        if error_early {
...
mod test {
    ...
    fn test_expression_identification_tuples() {
        for version in &[ClarityVersion::Clarity1, ClarityVersion::Clarity2] {
            for epoch in &[FunaiEpochId::Epoch2_05, FunaiEpochId::Epoch21] {
                let progn = "{ a: (+ 1 2 3),
                           b: 1,
                           c: 3 }";
