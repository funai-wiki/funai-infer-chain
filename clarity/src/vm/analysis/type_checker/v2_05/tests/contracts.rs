...
fn test_bad_map_usage() {
    ...
    for contract in tests.iter() {
        let err = mem_type_check(contract, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05)
            .unwrap_err();
        assert!(matches!(err.err, CheckErrors::TypeError(_, _)));
    }
...
fn test_expects() {
    ...
    mem_type_check(okay, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05).unwrap();
