...
fn type_check_helper(exp: &str) -> TypeResult {
    mem_type_check(exp, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05)
...
fn test_destructuring_opts() {
    ...
    for (bad_test, expected) in bad.iter() {
        assert_eq!(
            expected,
            &mem_type_check(bad_test, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05)
                .unwrap_err()
                .err
        );
    }
...
fn test_lists_in_defines() {
    ...
    assert_eq!(
        ...
        &format!(
            "{}",
            mem_type_check(good, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05)
                .unwrap()
                .0
                .unwrap()
        )
...
fn test_define() {
    ...
    for bad_test in bad.iter() {
        mem_type_check(bad_test, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05).unwrap_err();
    }
}
...
";
    ...
    assert_eq!(
        &mem_type_check(snippet, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05)
            .unwrap()
            .0
            .unwrap()
            .to_string(),
...
fn test_factorial() {
    ...
    mem_type_check(contract, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05).unwrap();
}
...
fn test_options() {
    ...
    assert!(
        match mem_type_check(contract, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05)
            .unwrap_err()
            .err
        {
...
fn test_list_nones() {
    ...
    assert_eq!(
        ...
        &format!(
            "{}",
            mem_type_check(contract, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05)
                .unwrap()
                .0
                .unwrap()
        )
...
fn test_using_merge() {
    let t = "(define-map users uint
        ...
        ";
    mem_type_check(t, ClarityVersion::Clarity1, FunaiEpochId::Epoch2_05).unwrap();
}
