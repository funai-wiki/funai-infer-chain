        ...
        match self {
            FunctionType::Variadic(expected_type, return_type) => {
                ...
                for found_type in args.iter() {
                    ...
                    if !expected_type.admits_type(&FunaiEpochId::Epoch2_05, found_type)? {
            ...
            match (&expected_arg.signature, arg) {
                (
                    ...
                    let contract_to_check = db
                        .load_contract(contract, &FunaiEpochId::Epoch2_05)?
                ...
                (expected_type, value) => {
                    if !expected_type.admits(&FunaiEpochId::Epoch2_05, &value)? {
...
impl<'a, 'b> TypeChecker<'a, 'b> {
    ...
    pub fn run(&mut self, contract_analysis: &ContractAnalysis) -> CheckResult<()> {
        ...
        let mut local_context =
            TypingContext::new(FunaiEpochId::Epoch2_05, ClarityVersion::Clarity1);
    ...
    pub fn type_check_expects(
        ...
        match (&expr.expr, expected_type) {
            (
                ...
                let contract_to_check = self
                    .db
                    .load_contract(&contract_identifier, &FunaiEpochId::Epoch2_05)?
        ...
        if !expected_type.admits_type(&FunaiEpochId::Epoch2_05, &actual_type)? {
    ...
    fn type_check_define_function(
        ...
        let args = parse_name_type_pairs::<()>(FunaiEpochId::Epoch2_05, args, &mut ())
    ...
    fn type_check_define_map(
        ...
        let key_type = TypeSignature::parse_type_repr(FunaiEpochId::Epoch2_05, key_type, &mut ())
            .map_err(|_| CheckErrors::BadMapTypeDefinition)?;
        let value_type =
            TypeSignature::parse_type_repr(FunaiEpochId::Epoch2_05, value_type, &mut ())
                .map_err(|_| CheckErrors::BadMapTypeDefinition)?;
    ...
    fn type_check_define_persisted_variable(
        ...
        let expected_type =
            TypeSignature::parse_type_repr::<()>(FunaiEpochId::Epoch2_05, var_type, &mut ())
    ...
    fn type_check_define_nft(
        ...
        let asset_type =
            TypeSignature::parse_type_repr::<()>(FunaiEpochId::Epoch2_05, nft_type, &mut ())
