        ...
        match self {
            FunctionType::Variadic(expected_type, return_type) => {
                ...
                for found_type in args.iter() {
                    ...
                    if !expected_type.admits_type(&FunaiEpochId::Epoch21, found_type)? {
                ...
                match (&expected_arg.signature, arg) {
                    (
                        ...
                        let contract_to_check = db
                            .load_contract(contract, &FunaiEpochId::Epoch21)?
                            .ok_or_else(|| {
                    ...
                    (expected_type, value) => {
                        if !expected_type.admits(&FunaiEpochId::Epoch21, &value)? {
...
fn check_function_arg_signature<T: CostTracker>(
    ...
    match expected_sig {
        FunctionArgSignature::Single(expected_type) => {
            ...
            if !expected_type.admits_type(&FunaiEpochId::Epoch21, actual_type)? {
                return Err(
    ...
    match (actual_type, expected_type) {
        ...
        (
            ...
            let contract_to_check = match db
                .load_contract(&contract_identifier, &FunaiEpochId::Epoch21)?
            {
                Some(contract) => {
                    runtime_cost(
...
impl<'a, 'b> TypeChecker<'a, 'b> {
    ...
    pub fn run(&mut self, contract_analysis: &ContractAnalysis) -> CheckResult<()> {
        ...
        let mut local_context = TypingContext::new(FunaiEpochId::Epoch21, self.clarity_version);
    ...
    fn type_check_define_function(
        ...
        let args = parse_name_type_pairs::<()>(FunaiEpochId::Epoch21, args, &mut ())
    ...
    fn type_check_define_map(
        ...
        let key_type = TypeSignature::parse_type_repr(FunaiEpochId::Epoch21, key_type, &mut ())
            .map_err(|_| CheckErrors::BadMapTypeDefinition)?;
        let value_type =
            TypeSignature::parse_type_repr(FunaiEpochId::Epoch21, value_type, &mut ())
                .map_err(|_| CheckErrors::BadMapTypeDefinition)?;
    ...
    fn type_check_define_persisted_variable(
        ...
        let expected_type =
            TypeSignature::parse_type_repr::<()>(FunaiEpochId::Epoch21, var_type, &mut ())
    ...
    fn type_check_define_nft(
        ...
        let asset_type =
            TypeSignature::parse_type_repr::<()>(FunaiEpochId::Epoch21, nft_type, &mut ())
