...
fn mem_type_check_v1(snippet: &str) -> CheckResult<(Option<TypeSignature>, ContractAnalysis)> {
    mem_run_analysis(snippet, ClarityVersion::Clarity1, FunaiEpochId::latest())
}
...
fn test_names_tokens_contracts(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
...
#[apply(test_clarity_versions)]
fn test_names_tokens_contracts_bad(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    let broken_public = "
...
#[apply(test_clarity_versions)]
fn test_same_function_name(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
...
#[apply(test_clarity_versions)]
fn clarity_trait_experiments_impl(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    let mut marf = MemoryBackingStore::new();
    let mut db = marf.as_analysis_db();
...
#[apply(test_clarity_versions)]
fn clarity_trait_experiments_use(#[case] version: ClarityVersion, #[case] epoch: FunaiEpochId) {
    let mut marf = MemoryBackingStore::new();
    let mut db = marf.as_analysis_db();
...
fn clarity_trait_experiments_constant_to_trait(
    ...
    match result {
        Ok(_) if version == ClarityVersion::Clarity2 => (),
        Err(err) if epoch <= FunaiEpochId::Epoch2_05 => {
