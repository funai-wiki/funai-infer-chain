...
use funai_common::types::{
    Address, FunaiEpoch as GenericFunaiEpoch, FunaiEpochId, PEER_VERSION_EPOCH_2_0,
};
use funai_common::util::hash::{to_hex, Hash160, Sha256Sum, Sha512Trunc256Sum};
...
pub trait HeadersDB {
    ...
    fn get_burn_header_hash_for_block(&self, id_bhh: &FunaiBlockId)
...
impl HeadersDB for &dyn HeadersDB {
    fn get_funai_block_header_hash_for_block(
        ...
        (*self).get_funai_block_header_hash_for_block(id_bhh)
    }
    fn get_burn_header_hash_for_block(&self, bhh: &FunaiBlockId) -> Option<BurnchainHeaderHash> {
        (*self).get_burn_header_hash_for_block(bhh)
    }
    fn get_consensus_hash_for_block(&self, id_bhh: &FunaiBlockId) -> Option<ConsensusHash> {
        (*self).get_consensus_hash_for_block(id_bhh)
    }
    fn get_vrf_seed_for_block(&self, bhh: &FunaiBlockId) -> Option<VRFSeed> {
        (*self).get_vrf_seed_for_block(bhh)
    }
    fn get_burn_block_time_for_block(&self, bhh: &FunaiBlockId) -> Option<u64> {
        (*self).get_burn_block_time_for_block(bhh)
    }
    fn get_burn_block_height_for_block(&self, bhh: &FunaiBlockId) -> Option<u32> {
        (*self).get_burn_block_height_for_block(bhh)
    }
    fn get_miner_address(&self, bhh: &FunaiBlockId) -> Option<FunaiAddress> {
...
impl BurnStateDB for &dyn BurnStateDB {
    ...
    fn get_funai_epoch_by_epoch_id(&self, epoch_id: &FunaiEpochId) -> Option<FunaiEpoch> {
        (*self).get_funai_epoch_by_epoch_id(epoch_id)
    }
...
impl HeadersDB for NullHeadersDB {
    ...
    fn get_consensus_hash_for_block(&self, _id_bhh: &FunaiBlockId) -> Option<ConsensusHash> {
        None
    }
    fn get_burn_block_time_for_block(&self, id_bhh: &FunaiBlockId) -> Option<u64> {
        if *id_bhh == FunaiBlockId::new(&FIRST_BURNCHAIN_CONSENSUS_HASH, &FIRST_STACKS_BLOCK_HASH)
    ...
    fn get_burnchain_tokens_spent_for_block(&self, _id_bhh: &FunaiBlockId) -> Option<u128> {
        None
    }
    fn get_burnchain_tokens_spent_for_winning_block(
        &self,
        _id_bhh: &FunaiBlockId,
    ) -> Option<u128> {
        None
    }
    fn get_tokens_earned_for_block(&self, _id_bhh: &FunaiBlockId) -> Option<u128> {
        None
    }
}
...
impl<'a> ClarityDatabase<'a> {
    ...
    pub fn put_value(&mut self, key: &str, value: Value, epoch: &FunaiEpochId) -> Result<()> {
    ...
    pub fn get_contract_size(
        ...
        debug!("contract_size: {}", contract_size);
    ...
    pub fn get_clarity_epoch_version(&mut self) -> Result<FunaiEpochId> {
        let out = match self.get_data(Self::clarity_state_epoch_key())? {
            ...
            None => FunaiEpochId::Epoch20,
        };
        Ok(out)
    }

    /// Should be called _after_ all of the epoch's initialization has been invoked
    pub fn set_clarity_epoch_version(&mut self, epoch: FunaiEpochId) -> Result<()> {
...
impl<'a> ClarityDatabase<'a> {
    ...
    pub fn get_index_block_header_hash(&mut self, block_height: u32) -> Result<FunaiBlockId> {
        self.store
    ...
    pub fn get_v2_unlock_height(&mut self) -> Result<u32> {
        if self.get_clarity_epoch_version()? >= FunaiEpochId::Epoch22 {
    ...
    pub fn get_v3_unlock_height(&mut self) -> Result<u32> {
        if self.get_clarity_epoch_version()? >= FunaiEpochId::Epoch25 {
    ...
    pub fn get_block_header_hash(&mut self, block_height: u32) -> Result<BlockHeaderHash> {
        ...
        self.headers_db
            .get_funai_block_header_hash_for_block(&id_bhh)
    ...
    /// 2. Get the parent block's FunaiBlockId, which is SHA512-256(consensus_hash, block_hash).
    ///    This is the highest Stacks block in this fork whose consensus hash is known.
    /// 3. Resolve the parent FunaiBlockId to its consensus hash
    ...
    fn get_sortition_id_for_stacks_tip(&mut self) -> Result<Option<SortitionId>> {
        ...
        // this is the FunaiBlockId of the last block evaluated in this fork
        let parent_id_bhh = self.get_index_block_header_hash(current_stacks_height - 1)?;

        // infallible, since we always store the consensus hash with the FunaiBlockId in the
        // headers DB
        let consensus_hash = self
            ...
            .ok_or_else(|| {
                InterpreterError::Expect(format!(
                    "FATAL: no consensus hash found for FunaiBlockId {}",
                    &parent_id_bhh
                ))
            })?;
    ...
    pub fn get_burnchain_block_height(&mut self, id_bhh: &FunaiBlockId) -> Option<u32> {
...
impl<'a> ClarityDatabase<'a> {
    ...
    pub fn get_funai_epoch(&self, height: u32) -> Option<FunaiEpoch> {
        self.burn_state_db.get_funai_epoch(height)
    }
}
