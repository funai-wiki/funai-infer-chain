...
use funai_common::types::chainstate::{BlockHeaderHash, FunaiBlockId, VRFSeed};
...
use crate::vm::events::FunaiTransactionEvent;
...
pub trait ClarityBackingStore {
    ...
    fn set_block_hash(&mut self, bhh: FunaiBlockId) -> Result<FunaiBlockId>;
    ...
    ) -> Result<(FunaiBlockId, Sha512Trunc256Sum)> {
...
impl ClarityBackingStore for NullBackingStore {
    fn set_block_hash(&mut self, _bhh: FunaiBlockId) -> Result<FunaiBlockId> {
    ...
    fn get_block_at_height(&mut self, _height: u32) -> Option<FunaiBlockId> {
