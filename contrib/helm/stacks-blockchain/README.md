...
[funai-blockchain](https://github.com/funai-wiki/funai-infer-chain) is a layer-1 blockchain that connects to Bitcoin for security and enables decentralized apps and predictable smart contracts.

## TL;DR

```bash
$ helm repo add funai https://charts.funai.xyz
$ helm install my-release funai/funai-blockchain
```

## Introduction

This chart bootstraps a [funai-blockchain](https://github.com/funai-wiki/funai-infer-chain) deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
To install the chart with the release name `my-release` and run the node as a **miner** using your private key:

```bash
$ helm repo add funai https://charts.funai.xyz
$ helm install my-release funai/funai-blockchain --set config.node.miner=true --set config.node.seed="REPLACE-WITH-YOUR-PRIVATE-KEY"
```

These commands deploy the FunAI Blockchain node on a Kubernetes cluster in the default configuration.
...
The following tables lists the configurable parameters of the funai-blockchain chart and their default values.

| Parameter | Description | Default |
|-|-|-|
| nameOverride | String to partially override funai-blockchain.fullname template (will maintain the release name) | nil |
| fullnameOverride | String to fully override funai-blockchain.fullname template | nil |
| node.image.repository | funai-blockchain container image repository | funai/funai-blockchain |
| node.image.pullPolicy | funai-blockchain container image pull policy | IfNotPresent |
| node.image.tag | funai-blockchain container image tag | latest |
| node.imagePullSecrets | Docker registry secret names as an array | [] |
| node.replicaCount | Number of funai-blockchain nodes to run | 1 |
...
| node.command | Overriding this is typically not necessary, unless you wish to run the node in a different way | ["/bin/funai-node"] |
| node.args | Overriding this is typically not necessary, unless you wish to run the node in a different way | ["start", "--config", "/src/funai-node/config.toml"] |
...
| config | More configs can be added than what's shown below.All children fields under the node, burnchain, and ufai_balance fields will be converted from YAML to valid TOML format in the Configmap.<br><br>For info on more available config fields, please reference to our [example config files located here](https://github.com/funai-wiki/funai-infer-chain/tree/master/testnet/funai-node/conf). |  |
...
| config.ufai_balance |  | See values.yaml |
...
| service.externalTrafficPolicy | Set external traffic policy to "Local" to preserve source IP on providers supporting it.<br>This field is only used when service.type is set to "NodePort" or "LoadBalancer".<br>Setting to "Local" can help the FunAI network if using service type NodePort or LoadBalancer. | Local |
| service.rpcPort | Will use node.rpcPort if omitted | nil |
| service.p2pPort | Will use node.p2pPort if omitted | nil |
| service.annotations | Annotations to be added to the Service | {} |
| serviceAccount.create | Specifies whether a ServiceAccount should be created.<br>If true, the funai-blockchain node will be ran using this ServiceAccount. | true |
...
```bash
$ helm install my-release -f values.yaml funai/funai-blockchain
```
...
If you encounter an issue with this Helm chart, please submit an issue to this Github repo [here](https://github.com/funai-wiki/funai-infer-chain/issues).
